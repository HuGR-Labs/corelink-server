...
//! + [`BackendOutcome`] + [`ErasureRequest`] + [`ErasurePlan`] +
//! [`BackendCompletion`] + canonical SLA constants.
//!
...
//! sites (e.g. WI-S14 BYOK may add a `CryptoEraseInitiated` decision
//! arm; future jurisdictions may add new backends to the canonical
//! 12-arm list).
...
impl ErasureSalt {
    ...
    #[must_use]
    pub const fn synthetic_for_test(seed: u8) -> Self {
...
impl ErasureRequest {
    /// Construct a fresh erasure request.
    #[must_use]
    pub const fn new(
        dsr_id: Uuid,
        tenant_id: Uuid,
...
impl ErasurePlanEntry {
    ...
    pub fn idempotency_key_for(dsr_id: Uuid, backend: BackendKind, retry_count: u32) -> String {
        let dsr_short = dsr_id.simple().to_string();
        let dsr_short = dsr_short.get(..8).unwrap_or("00000000");
        format!(
...
/// WI-S11-002 §1 BackendErasureOutcome.
///
...
impl core::fmt::Display for BackendOutcome {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        f.write_str(self.as_str())
    }
}
...
pub struct BackendCompletion {
    ...
    /// [`crate::backend::CANONICAL_EMPTY_TENANT_HASH`] (effective
...
/// these arms; the canonical audit envelope
/// `corelink.erasure.<arm>` fires BEFORE the state mutation on each
/// arm.
///
/// `#[non_exhaustive]` reserves additive growth.
#[derive(Clone, Debug, PartialEq, Eq, Serialize, Deserialize)]
...
pub enum ErasureDecision {
    /// Plan generated: 12 canonical backend ledger entries enumerated;
    /// `plan_generated` audit row emitted; no backend mutation yet.
    PlanGenerated {
        /// Canonical 12-arm plan.
        plan: ErasurePlan,
    },
    /// Fan-out initiated: per-backend erase calls dispatched (parallel
    /// bounded async in production wiring; sequential in the in-memory
    /// fake for deterministic test pinning); `fanout_initiated` audit
    /// row emitted.
    FanoutInitiated {
        /// Canonical 12-arm plan.
        plan: ErasurePlan,
        /// Number of backends successfully dispatched.
        dispatched_count: usize,
    },
    /// Verification 24h sweep complete: every backend reports a
    /// successful outcome; `verified_complete` audit row emitted;
    /// canonical `dsr.completed.v1` event fires; report uploaded to
    /// R2 evidence-dsr.
    VerifiedComplete {
    ...
    /// `verified_partial` audit row emitted; `dsr_tickets.status`
...
impl ErasureDecision {
    ...
    pub const fn as_str(&self) -> &'static str {
        match self {
            ...
            Self::FanoutInitiated { .. } => "fanout_initiated",
    ...
    pub const fn is_sev1(&self) -> bool {
        matches!(
            self,
            Self::VerifiedPartial { .. } | Self::SlaBreached { .. }
        )
    }
}
...
mod tests {
    ...
    fn backend_outcome_strings_unique() {
        let arms = [
            BackendOutcome::Erased {
                records_deleted: 1,
            },
            BackendOutcome::Pseudonymized {
                records_redacted: 1,
            },
            BackendOutcome::PartialFailure {
                records_succeeded: 1,
                records_failed: 1,
            },
    ...
    fn backend_outcome_is_successful_pinned() {
        assert!(BackendOutcome::Erased {
            records_deleted: 0
        }
        .is_successful());
        assert!(BackendOutcome::Pseudonymized {
            records_redacted: 0
        }
        .is_successful());
        assert!(BackendOutcome::NotApplicable.is_successful());
        assert!(!BackendOutcome::PartialFailure {
            records_succeeded: 0,
            records_failed: 1
        }
        .is_successful());
        assert!(!BackendOutcome::Failed {
            retry_after_seconds: 60
        }
        .is_successful());
    }
    ...
    #[test]
    fn erasure_decision_canonical_strings_unique() {
        let arms = [
            ErasureDecision::PlanGenerated {
                plan: ErasurePlan {
                    dsr_id: Uuid::now_v7(),
            ...
            ErasureDecision::FanoutInitiated {
                plan: ErasurePlan {
                    dsr_id: Uuid::now_v7(),
                    tenant_id: Uuid::now_v7(),
                    generated_at_ms: 1,
            ...
            ErasureDecision::VerifiedComplete { completions: vec![] },
    ...
    fn erasure_decision_sev1_pinned() {
        assert!(ErasureDecision::VerifiedPartial {
            completions: vec![],
            failed_count: 1
        }
        .is_sev1());
        assert!(ErasureDecision::SlaBreached {
            unverified_count: 1,
            elapsed_ms: 1
        }
        .is_sev1());
        assert!(!ErasureDecision::VerifiedComplete { completions: vec![] }.is_sev1());
    }
}
