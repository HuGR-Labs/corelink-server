...
### Fixed
- **fix(docs): the RBAC guides documented a `corelink admin …` CLI that has never existed, and 24 blocks of fabricated output — in four languages.** 28 phantom invocations across 12 files: `admin members invite|list`, `admin pat mint`, `admin audit query|prove`, `pat revoke`, plus a phantom endpoint `POST /v1/admin/pat/{id}/revoke` that exists nowhere. The real surface shipped long ago and is what the guides now describe: the **dashboard** (`/customer/team`, `/customer/keys`) and the **dual-auth HTTP API** — `/v1/customer/team{,/invite,/{user_id}}`, `/v1/customer/keys{,/{pat_id}/revoke}`, `/v1/audit/analytics/*`, `/v1/audit/{tenant}/export` — each documented against the route table, with responses described in prose rather than sampled. `corelink audit export|verify|verify-ndjson|tail` replaces the invented `admin audit` verbs. **The fabrications were the larger half.** Deleted: invented `USER/EMAIL_HASH/ROLE/SINCE` and `ACTOR_HASH/TARGET_HASH/DETAIL` tables no command ever printed; an invented JSON body for a non-existent `GET /v1/admin/audit/events`; an invented `region="enam"`; and four CloudEvent type names (`corelink.admin.member.invited|joined|role_granted|role_revoked`) with **zero hits in the codebase**. **Five further claims were verified FALSE and corrected**, none of which were in scope when the sweep began: (1) *"a Clerk invitation email is sent"* — there is **no Clerk call on invite** (`customer_d1.rs:1489`: "NO synchronous Clerk call (OB-1) — the invitation EMAIL round-trip is an owner follow-up"), so the dependent "invites expire in 14 days" pitfall went with it; (2) *"a pending membership row is NOT yet written"* — the `team_member` row **is** written at invite time with status `invited` (`customer_d1.rs:1531`), which is precisely why the dashboard has a Pending-invitations list; (3) the revoke workflow's `reason=CompromiseSuspected` — `handle_keys_revoke` takes **no request body** (`routes/customer.rs:828-831`) and writes **no** `revocation_log` row (only `UPDATE pat SET revoked_at_ms`, `customer_d1.rs:1396`; the table is referenced zero times in the container crate); (4) *"clients receive 401 on next request"* — 401 is right (revoked rows stop matching `revoked_at_ms IS NULL`, `pat_verify_cache.ts:203` → `pat_not_found` → 401, with the 403/503 arms ruled out) but **"next request" is not**: the edge verify cache holds a positive result ≤5 s (`pat_verify_cache.ts:153`, ADR-0030 bounds revocation at ≤60 s p99); (5) the role-grant rules — `owner` is **never** grantable via invite (403 for everyone, `routes/customer.rs:936`) and only an owner may invite an `admin` (`:951`), so the dropdown is not "grayed out" as claimed. **Method note, since it is the point:** the sweep ran as four parallel agents against a frozen contract, and the contract itself carried two of the falsehoods above — the audit-event names and the Clerk email. Two agents independently flagged the first; one caught the second. A verification claim made by me that `GET /v1/customer/team` leaks a raw `email` was **also wrong** — `routes/customer.rs:894` is the in-memory stub, and the D1 production path returns the placeholder `"—"` (`customer_d1.rs:1471`), the raw address never being persisted. Every assertion in these guides is now backed by a read line, or absent. Validated before merge against a measured baseline rather than assumed: `translation-quality-check --strict` reports **796 errors on `origin/main` and 796 with this change** (delta: +1 warning, the literal API field name `email` in a German banned-word regex), and the Docusaurus build is clean for **en** and **pt-BR** — the locale build being the gate that catches exactly this class, having rejected a doubled `/docs/` link prefix earlier in the same sweep.
