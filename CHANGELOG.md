...
### Added
- **feat(remediation): enable infrastructure for 6 gated enterprise/compliance features (BYOK, erasure attestation, GC/Eviction, failover heartbeat, audit export, rollout controller).** Phase 0+1 of REMEDIATION_PLAN.md: provisioned R2 bucket `corelink-audit-weur`, set `ERASURE_ATTESTATION_SEED_HEX`/`_KEY_ID`/`_SINGLE_REGION`/`_REGION` + `R2_AUDIT_BUCKET` secrets, added secrets to Container boot gate (`main.rs:340-437`), enabled `byok-aws-real` feature in Dockerfile, added R2 audit bucket binding + cron triggers (GC daily, DSR verify hourly, heartbeat 5min) to `wrangler.toml`. All validation gates green (`validate_specs.py` 469/0, `secrets-matrix` code_only=0, `secrets-checklist` no drift). Remaining: RealGcWorker/EvictionPhase impl, Worker read reroute, R2AuditExporter, WASM bridge.
