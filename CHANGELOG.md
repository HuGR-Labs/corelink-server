...
### Performance
- **perf(worker): the per-request PAT-verify D1 read on the auth hot path is now served from a 5 s per-isolate cache — closing the ~1 s/request authenticated-command latency (finding #99, independently reported by the clw team).** `extractAuth` (`worker/src/index.ts`) did a synchronous D1 read of the `pat` row (`SELECT … FROM pat WHERE token_id = ?1 AND revoked_at_ms IS NULL`) on EVERY authenticated request. Profiling against prod (a SAM/São-Paulo client, 2026-07-19) isolated it as the SOLE uncached read left on the WARM hot path (the tenant suspend gate already caches): `/health` (no auth) ~40 ms; a bad token that fails HMAC PRE-D1 ~45 ms; a valid token on a COLD isolate (pat + suspend read) ~1.4 s; a valid token on a WARM isolate (suspend cached) ~0.7 s — that residual ~0.7 s being exactly this uncached pat read, ~0.7 s each being the trans-continental D1-over-HTTP cost. The auth-only tenant-mismatch `403` (returned BEFORE any DO/container forward) measured identical to the full served path, proving the ~1 s is entirely the Worker's D1 reads, not the DO or the container. New `worker/src/lib/pat_verify_cache.ts` (`verifyPatRowCached`) mirrors the established edge cache pattern (`tenant_suspend_gate.ts`; #667's container `CachedTierResolver`): a per-isolate `Map` keyed by the non-secret `token_id`, single-flight (a same-token burst collapses to ONE D1 read), LRU-capped. **Revocation-correctness (this is a credential cache):** TTL = **5 s**, sized against ADR-0030 `SLO-FRESH-PAT-REVOKE ≤ 60 s p99` and matching the container `NativePatGate` verify cache (red-team C3's deliberate 60 s→5 s bound) — a mid-entry revoke is honored for ≤5 s then denied, well within the 60 s SLA. POSITIVE results only (a freshly-minted token authenticates immediately — no negative-cache delay); D1 stays the source-of-truth on every miss (INV-AUTH-NEON-IS-SOT); expiry is re-evaluated by the caller on every hit (zero added staleness on expiry — only revoke has the ≤5 s window); a D1 fault is never cached and never serves a stale/expired entry (the `d1_lookup_error → 503` posture is unchanged), while a still-fresh (<5 s) hit is served through a transient blip for availability. The cache is consulted ONLY AFTER the HMAC possession proof, so a wrong-secret token never reaches it, and it holds the D1 row (tenant/scope/expiry/runner marker) only — never the secret. Regression-locked in `worker/tests/pat_verify_cache.test.ts` (12 cases incl. the revocation-window bound — honored within TTL, denied after — and an integration proof that the SAME PAT twice does exactly ONE pat D1 read); a global `beforeEach` cache reset was added in `tests/setup.ts` so the per-isolate singleton is isolated per case. Requires a worker deploy (`corelink-api`) to ship.
