...
### Added
- **feat(f3.2): the `_public` revocation enforcement floor (BLOCKER-1) — a `public_blocklist` table (migration 0097) + a blocklist-aware `_public` read path, so a revoked cross-tenant public content-hash can never be served or re-mirrored.** F3.2 (the cross-tenant public OCI base-layer cache) shares `_public` (`adapter_cache.rs`) content-addressed CAS bytes across every tenant, but until now a poisoned or misclassified `_public` blob was physically UN-erasable: `cas_erase` erases by a tenant HMAC prefix and never reaches the shared public principal, and `MoatCache.delete` removes only the map row. This lays the enforcement floor the revocation endpoint (next increment) writes into: (1) `public_blocklist(content_hash PK, revoked_at_ms, reason, approver, audit_event_id)` is the atomic, cross-region revocation guard — durable where the multi-region R2 byte-delete is best-effort; (2) the `UrlMapStore` D1 `get` for `_public` filters `NOT EXISTS (public_blocklist)` in the SAME statement, so a revoked digest is never resolved (no map-read → blocklist-read TOCTOU; the private-namespace path is byte-for-byte unchanged); (3) `BEFORE INSERT`/`BEFORE UPDATE OF content_hash` triggers on `adapter_cache_map` RAISE-ABORT any `_public` mapping to a blocklisted hash — the DB-layer backstop that closes the re-mirror race even if an application check is bypassed. Additive + inert until the revocation endpoint + R2 hard-delete + `cas_erase` `_public` refusal land (B1b). Deepseek-hardened design; part of the gated F3.2 mini-campaign per `docs/design/2026-08-13-f3-cross-tenant-public-layer-cache.md`.
...
- **fix(security): the last `gitleaks` finding is a REVOKED credential, and the weekly sweep
  was going to exit 1 on it every Saturday forever — closed by an AND-pinned allowlist entry,
  proved not to be a blanket.** #1012 drove the full-history sweep from 17 findings to 1. That
  survivor is real — a Stripe webhook signing secret genuinely committed at
  `docs/operator/stripe-checkout-e2e-2026-05-29.md:73` — but `gitleaks detect` scans the COMMIT
  GRAPH, so it lives in `b59c4862` no matter what HEAD says. It was left visible pending "owner
  rotation." **There is nothing to rotate: the credential is revoked.** A Stripe signing secret
  is scoped to exactly ONE endpoint, and `we_1TcaMDLh0hhAZjwol9KDCJTp` was DELETED on 2026-05-29
  — by the very incident that leaked it — and replaced by `we_1TcaeeLh0hhAZjwoWambOOhJ`; deleting
  the endpoint revokes its secret. Three independent confirmations: the operator runbook's
  §"Issue 3"; the **live Stripe API enumeration** of every v1 webhook endpoint on the production
  account in `docs/handoff/2026-07-03-REPLY-from-clw-coordinator-webhook-reconcile-DONE-and-no-stray-endpoint-exists.md`
  (2026-07-03, exactly three endpoints, the deleted one absent); and the owner's read of the live
  dashboard on 2026-08-03 (absent again). Left as-is, the cron added by #1002 (`17 5 * * 6`, first
  firing **2026-08-08**) would have opened red on run one and stayed red — the exact failure mode
  #1012 existed to eliminate, since a permanently-red gate is a gate nobody reads and a NEW leak
  would arrive as the second line of an alert everyone already ignores. **This is a
  REVOKED-credential closure, not a false positive and not a deferral**, so it is pinned where the
  scanner enforces it rather than narrated in a triage doc: one `[[rules.allowlists]]` on the
  `stripe-webhook-secret` rule with `condition = "AND"` over three predicates — that single
  historical commit SHA, that one file path, and (`regexTarget = "line"`) a line that also names
  the dead endpoint id, a PUBLIC Stripe object id. No rule disable, no path-only mute, no global
  value allowlist, no tree exclusion, no `--exit-code 0`, and the workflow is untouched. The
  secret literal is deliberately NOT written into `.gitleaks.toml`: the bytes of `b59c4862` are
  frozen, so it would add no tightness while permanently re-disclosing a credential at HEAD.
  **Measured with the sweep's own command.** Before: `leaks found: 1`, exit 1. After:
  `no leaks found`, exit 0. Set difference between the two runs is exactly the one fingerprint
  `b59c4862…:docs/operator/stripe-checkout-e2e-2026-05-29.md:stripe-webhook-secret:73` and nothing
  else — zero collateral suppression. **Proved not to be a blanket** against a scratch commit
  planting three NEW `whsec_` values: one in a different file, one in *this same file*, and one in
  this same file on a line that *also* names the dead endpoint — i.e. satisfying two of the three
  predicates. **All three fire** (`leaks found: 3`, exit 1); only the commit SHA separates them
  from the closed finding, which is the point of pinning it. Also closes the stale "confirm
  rotation in Stripe" action item in the runbook and in
  `docs/security/2026-06-18-gitleaks-baseline-triage.md`, whose own "20 historical FPs, triaged
  out of band" habit is what let three rule defects sit unexamined for six weeks.
...
- **fix(security): the weekly full-history `gitleaks` sweep would have opened RED on its very first firing (Sat 2026-08-08) with 16 findings that cannot be secrets — three detection defects in the RULES, not a triage backlog.** Measured on `fcc517c3` with the sweep's own command: **17 findings / 2,561 commits / 85.47 MB / exit 1.** A gate that is red on run one and stays red teaches everyone to ignore it, so the sixteen were traced to their cause instead of being written off again. All sixteen came from three rules that match the SHAPE of a credential without requiring the VALUE to be one. **(1) `curl-auth-user` fired on shell variable REFERENCES** — `-u "${STRIPE_SECRET_KEY}:"` × 9, in `scripts/e2e-stripe-checkout.sh`, `scripts/ops/stripe-setup-runners.sh` and an e2e operator doc. The committed bytes are the variable's *name*; there is no value there. Upstream gitleaks already encodes exactly this principle — its own rule allowlist carries `[^:]+:\$(?:\d|\w+|\{...\})` — but only for the PASSWORD half, and has no mirror for a credential in the USER half with an empty password, which is the standard Stripe/REST `-u "${KEY}:"` idiom. The pattern now requires the user half to start with something other than `$` and to contain no braces. **(2) `curl-auth-header` fired on an ELIDED token** — `Authorization: Bearer corelink_pat_...` in an audit report. Every token class now ends `[\w=~@.+/-]{7,}[\w=~@+/-]`: a token may not END in `.`, because no bearer token, JWT, opaque token or base64url value does — a JWT's dots are interior separators. Length floor and interior dots are unchanged, so real 3-segment JWTs still fire. **(3) `generic-api-key` fired on three placeholder shapes** — ascending alphabet/digit walks (`ABCDEFGH01234567` ×3, `0123456789abcdef`), a value that states its own size in prose (`…-20-char-…`), and a PagerDuty `dedup_key`. These are added as `[[rules.allowlists]]` on the rule *without* redeclaring its `regex`, which gitleaks merges — so the upstream pattern, the 3.5 entropy floor, the keyword list and ~1.3k upstream stopwords all stay in force, and the three additions extend families upstream already has (`000000`/`aaaaaa` walks; `(?:bucket|foreign|natural|primary|schema|sequence)[_.-]?key`). `routing_key` is deliberately excluded — PagerDuty routing keys ARE secrets. **Nothing was suppressed by path, commit, file or literal**, no `--exit-code 0`, no narrowed range: every predicate is a property of the value, so it holds repo-wide and for commits not yet written. **Proved in both directions.** Forward: the sweep now returns **exactly 1** finding, the genuine `whsec_` at `docs/operator/stripe-checkout-e2e-2026-05-29.md:73` (`b59c4862`), which is left untouched and still exits 1 — owner rotation was pending since 2026-06-18 and one visible real item was the intended steady state. (Superseded: that item has since been proved a REVOKED credential and pinned closed — see the `fix(security)` entry at the top of this section.) Backward: a planted commit carrying a `whsec_`, an `AKIA…`, a `ghp_…`, an `sk_live_…`, a literal (non-variable) `curl -u` basic-auth pair, a real 3-segment `Bearer` token, a 64-hex `CORELINK_INTERNAL_AUTH_KEY` and an AWS secret-access-key was scanned under the OLD and the NEW config: **10 caught under both, set-identical, zero lost.** A rule loosened until nothing fires would be worse than the false positives. Writing this entry produced a bonus proof: the first draft quoted the planted `curl -u` pair verbatim and the rebuilt `generic-api-key` immediately flagged it in `CHANGELOG.md` — the literal was removed, and the sweep is clean apart from the `whsec_`. **The reason this had to be a rule change and could not be a fixture change:** `gitleaks detect` scans the COMMIT GRAPH, so a finding lives forever in the commit that introduced it — three of the 17 were reported at `apps/server/src/routes/signup.rs` and `crates/corelink-cli/src/{auth,lib}.rs`, paths that no longer exist at HEAD. Editing today's file removes nothing. (The `token_id` literals were checked anyway and are `#[cfg(test)] mod tests` fixtures, not production defaults; they are left alone because changing them would buy the gate nothing.) The PR lane is untouched — still `~10.8 KB`, `grafts in range: 0`, clean.
...
- **fix(ci): `cargo install` on the shared Mac corrupted every OTHER concurrent Rust
  ...
  `~/.cargo` (and the founder's local builds use it too). `cargo install` resolves and
  compiles the tool's own dependency tree into that shared registry, so a fuzz job
  installing `cargo-fuzz` mutates the registry that an unrelated clippy or proptest job
  is compiling from. The failing crate names in the logs — `aws-lc-sys`, `ctutils`,
  `syn`, `tokio`, `parking_lot` — are cargo-fuzz's own dependencies, not the repo's.
  `cargo-fuzz` is **not** pre-installed on the host, so this fired on every fuzz run.

  This is the same class as the incident already documented in `fuzz-nightly.yml`
  (2026-06-15: `dtolnay/rust-toolchain@nightly` mutated the shared `~/.rustup`,
  dangling every `cargo` symlink and killing every Rust gate on the host for a day) —
  one level down, in `~/.cargo` instead of `~/.rustup`. The repo had also already
  worked around it twice, in `mutation-pr.yml` and `lockfile-diff.yml`, whose comments
  name the identical "`cargo install` compile race (os error 2 / aws-lc-sys)".

  Two fixes, chosen per tool rather than uniformly:
  - **12 `cargo-fuzz` sites** (7 workflows) now build into a **per-runner
    `CARGO_HOME`** (`$HOME/.cargo-tools/$RUNNER_NAME`) — isolated, so the shared
    registry is never written, and persistent, so each runner compiles it at most once.
    A prebuilt binary was **not** an option here: `cargo-fuzz` has no manifest in
    `taiki-e/install-action` at the SHA this repo pins, so that route would have
    silently fallen back to the same from-source install while looking fixed.
  - **`cargo-zigbuild` and `cargo-cyclonedx`** switch to SHA-pinned **prebuilt
    binaries** via `taiki-e/install-action` (both verified present at the pinned SHA) —
    no compile at all, matching the pattern `mutation-pr.yml` already uses.

  `release-cli.yml`'s "install once per host and reuse" guard is removed with the same
  reasoning: it only narrowed the window to the first build after a version bump, and
  the whole target matrix reaches that window simultaneously.

### Added
- **feat(worker): the runner mint now forwards `max_vcpu_h` — the prerequisite for warning a customer BEFORE they cross into overage.** With overage priced at $0.30/vCPU-h (3× COGS), blowing past the included allowance is expensive, and an SMB self-serve customer who first learns about it on the invoice is a churn event, not a billing event. The dispatcher is the only component that sees CONSUMPTION, but it could not see the ALLOWANCE: the mint read `SELECT max_concurrency` and returned only that, so `runners_entitlement.max_vcpu_h` — the monthly compute ceiling, present in prod for every paid tier (starter 100 / pro 240 / team 600) — never left the server. The query now selects both columns and the envelope carries `max_vcpu_h` alongside `max_concurrency`. **Additive and non-breaking:** the field is OMITTED (not null, not 0) when the tenant has no metered ceiling on file, so the wire is byte-identical for every tenant that had none — an existing consumer cannot tell the field was added, and the frozen `conformance/corelink-introspect.json` seam is untouched. **Advisory, never a gate:** `max_concurrency` remains the only ceiling that can deny a mint. Enforcement — warn, then charge — belongs to the dispatcher, which knows consumption; the mint knows only entitlement and must not guess. A dedicated test pins that a tenant sitting at any ceiling is still minted, so this field cannot quietly become a deny path. **Guarded at the source:** migration 0072's column is NULLABLE, and a `0` or negative value is treated as ABSENT rather than forwarded — a 0 would make the dispatcher compute an infinite percentage and warn every tenant on their first job, and a negative is corruption; neither is a real ceiling, and guarding here means no consumer has to re-derive that. Regression-locked with each behaviour proven RED independently: dropping the field reds the two forwarding tests; relaxing the `> 0` guard reds the zero/negative test alone. 616 worker tests green. Next in this chain: the dispatcher accumulates vCPU-seconds per (tenant, period) and warns on threshold crossings — the mint half had to land first because the dispatcher cannot warn against a number it never receives.
- **feat(billing): `runner_vcpu_seconds` — the BILLABLE runner compute unit, replacing a meter that would have under-billed by exactly 4×.** The owner superseded the ratified *"concurrency priced, minutes unlimited"* runner model on 2026-08-02: minutes above a tier's included `runners_entitlement.max_vcpu_h` are now billed as overage at **$0.30/vCPU-h** (3× the measured $0.10/vCPU-h COGS). That decision was **written into the code** — `UsageEventKind::RunnerSlotSeconds`'s doc-comment asserted the old model verbatim and marked the kind NON-Stripe-billable — so this change updates the taxonomy, not just a config. **The defect it fixes:** the emitter pushed `runner_slot_seconds` (wall-clock seconds a SLOT was held) while the entitlement it meters against is denominated in vCPU-**HOURS**. On the current 4-vCPU box those differ by exactly 4×, and *both read as "seconds"* — so arming the existing pipeline as-is would have billed a quarter of what it should, and looked entirely reasonable doing it. A new kind was added rather than redefining `runner_slot_seconds`, because silently changing what an existing kind's `qty` MEANS is invisible to every consumer already reading it; the capacity kind survives unchanged for cost reconciliation, with the superseded pricing sentence preserved-and-struck in its doc rather than deleted (a pricing model that lives in a doc-comment is exactly what gets silently re-derived from stale code). **Size-aware by construction:** `qty = allocated_seconds × the box's vCPU count`, multiplied at the EMITTER — the only component that knows the box it just tore down — so an 8-vCPU or high-memory SKU bills correctly the day it ships, with no migration and no conversion table that could drift from the hardware. A fixed ×4 anywhere in this path would under-bill every future box; `RUNNER_BOX_VCPU` is a defaulted PARAMETER, not a constant read, and carries the warning explicitly. **ALLOCATED wall-clock, not CPU time:** Cloudflare bills memory + disk by allocation (measured: 490 allocated-s vs 170 cpu-s on a real run), so metering `cpuTimeSec` would under-count COGS ~3× and break the ladder's loss-proof floor. Taxonomy 7 → 8 kinds; both cardinality pins (`src/event.rs` + the cross-component `prop_billing_emit.rs`) were updated deliberately, which is the record that the WI-S10-001 §6.1.6 Finance+Compliance sign-off happened — they are SUPPOSED to fail on a silent kind addition. **Verification:** the frozen `conformance/UsageEvent.json` is UNCHANGED (it is fabricd's golden and fabricd still emits the capacity kind), so the fabricd consumer test is untouched by construction — only TypeScript changed on the emitter side. `event_kind`/`qty` moved off equality-with-the-vector onto the billing ARITHMETIC itself (`qty === allocatedSeconds × vCPU`, plus `qty === vector.qty × RUNNER_BOX_VCPU` so the two kinds stay reconcilable), following the convention that file already used for `source` and `idem_key`; this is a STRONGER pin, not a weakened one — an equality against one frozen example passed just as happily with the multiplier missing. Removing the multiplier now reds 4 tests (`expected 3 to be 12`, `expected 120 to be 480`, `expected 3 to be 48`), proven. New cases cover an explicit `vcpu` override (a 16-vCPU box bills 4× a standard-4) and the guard that a nonsense vCPU count (0 / negative / NaN / absent) falls back to the fleet default rather than zeroing the bill — a zeroed bill is indistinguishable from a job that never ran. 64 Rust + 335 TypeScript tests green. **NOT yet live:** the push remains unarmed (`BILLING_INGEST_URL` unset on `corelink-spawn-worker`) and no Stripe metered price is wired — arming, the 60-day ledger backfill, and the overage calculation are the tracked follow-ups.
- **feat(telemetry): `first_cli_authed` is finally ON — the emitter now dispatches over the keyless `AnalyticsIngest` RPC entrypoint, which deletes its last operator prerequisite.** The producer shipped able to run but not actually running: `emitFirstCliAuthed` (`worker/src/lib/onboarding_events.ts`) POSTed through the `ANALYTICS_SVC` service binding with `X-Corelink-Ingest-Key`, and `first_cli_authed` is on `SERVER_ONLY_EVENT_NAMES` (`apps/analytics-worker/src/ingest.ts:51`), so an unkeyed POST is a guaranteed `server_only_event` reject — the emitter therefore gated itself on `env.ANALYTICS_INGEST_KEY` and returned early. That secret **is not bound on `corelink-prod`** (CF API secret-name listing, secrets-matrix row #187), so in production every `GET /v1/users/me` took the early return and logged one `console.warn`: zero rows, `/welcome` still dark, waiting on a human to run `wrangler secret put`. The receiver half (`AnalyticsIngest.ingestServerEvent`, `apps/analytics-worker/src/index.ts:170`) needs **no key at all** — a service binding is resolved by the Cloudflare control plane from the caller's own deployed config and never leaves the runtime, so the platform authenticates the caller's identity and `ingestServerEvent` passes `trusted = true` on that basis, which is exactly what satisfies the server-only rule. Joining the halves is therefore both a simplification and the on-switch: `[[env.prod.services]]` gains `entrypoint = "AnalyticsIngest"` (`wrangler.toml` — **load-bearing**: without it the binding resolves to the target's default `fetch` export, which exposes no RPC methods), the dispatch becomes a single `await svc.ingestServerEvent({ id, event_name, tenant_id, properties })` with no `Request`, no URL and no header, and the `ANALYTICS_INGEST_KEY` guard, env field, `console.warn` and the now-dead `ANALYTICS_INGEST_URL`/`ANALYTICS_INGEST_KEY_HEADER` constants are deleted. **Every safety property is preserved and re-proved, not assumed:** the emit still returns `void` (structurally un-awaitable), still rides `ctx.waitUntil` (a bare floating promise is cancelled on response return, #859), still fires only on `GET /v1/users/me` behind the PAT gate, still refuses the `_anonymous`/`_system`/`_pending` sentinels and over-long tenants, and still uses the deterministic id `first_cli_authed:<tenant_id>` under the 64-char cap so the `analytics_events` PRIMARY KEY + `INSERT OR IGNORE` remain the once-per-tenant lock. `worker/tests/onboarding_events.test.ts` (25 cases) is mutation-verified rather than merely green: stubbing `emitFirstCliAuthed` to `return;` fails 10 tests, and widening the route guard to any authed `/v1/*` fails 2. It asserts the RPC method is called and `svc.fetch` is **never** touched, that a throwing / rejecting / never-settling RPC leaves the `/v1/users/me` status and body byte-identical, that a stub carrying only `fetch` (i.e. a deploy missing `entrypoint =`) degrades to a silent no-op instead of a `TypeError` on the customer's request path, and that the module exports no `*KEY*`/`*URL*` symbol — so re-introducing the shared secret fails the suite. Secrets matrix updated: row #187 drops `corelink-prod` as a consumer and row #138's lockstep-rotation list now names only `corelink-signup-worker`; `INGEST_KEY` itself stays real for the HTTP callers. **⚠️ Deploy order is load-bearing: `corelink-analytics-prod` must ship `AnalyticsIngest` BEFORE `corelink-prod` deploys with `entrypoint =` set** — a binding naming an entrypoint the target does not export is rejected at deploy time. Regional envs (`prod-sam`/`lhr`/`nrt`/`syd`) and `staging` declare no `ANALYTICS_SVC` binding, so the emit stays a no-op there, unchanged.
- **feat(okf): the anti-drift gate now baselines each cited file on its git BLOB sha, not on a commit id — deleting a recurring re-anchor tax and an accepted security residual.** C5 compares "the file's content at authoring time" against the working tree, but it named that content with `checkpoint_sha`, a COMMIT id — and rebase, squash and cherry-pick all destroy commit ids. The dead object lingers in the author's clone and is absent from CI's fresh one, so C4 warns "orphaned" and **C5 silently re-anchors to the base ref** (`scripts/validate_okf.py`), which still holds the OLD content — so every citation the branch legitimately moved reads STALE. Measured cost: the same 6 concepts were re-anchored three times in one day (`a7c7cfb6` alone remapped **110 occurrences** across 6 concepts, its own message recording "nothing cited changed meaning; the content slid off its line numbers"), each cycle undone by the next rebase, and `worker/src/index.ts` is cited **118 times** across 6 concepts so one merge to it re-breaks every open PR citing it. The residual was worse than the churn: `validate_okf.py` carried a ~40-line comment ACCEPTING that a forged, well-formed, unreachable `checkpoint_sha` is indistinguishable from a genuine squash-orphan, so it launders already-landed drift past C5 (the base ref it re-anchors to already contains that drift). **Reproduced end-to-end before fixing it:** forged 40-hex + one cosmetic body edit — all C5b asks for — turned a real `[C5] STALE` into `✅ profile valid` on a citation pointing at content the author never saw. **The fix:** a new optional per-file frontmatter key `source_blobs: ["<path>@<40-hex blob>"]`. A blob id is a content hash — immutable under rebase, squash and cherry-pick — so a blob that was ever pushed is still in the `fetch-depth: 0` clone. When a file has an anchor, C5 compares against that blob and **the squash-orphan tolerance and base-ref fallback do not apply to it**; an absent blob is not an orphan, it is a typo or a forgery, and it REDs the new **C4b** instead of falling back to anything. **C4c** makes it a ratchet, so the hole is closed rather than moved: an anchored path may not lose its anchor while it is still a declared source (otherwise an author could un-migrate one line and walk back onto the carve-out). `checkpoint_sha` is untouched and still required — it is the provenance record and the input C-AGE / C-REV / C5b read, which need a `commit_date`. The shape is a FLAT list of strings deliberately: this frontmatter is read by five hand-rolled parsers plus PyYAML, and a nested `path: sha` mapping is the one shape they disagree on — PyYAML yields a dict while both hand readers yield `[]`, i.e. "no anchors", a **fail-open** whose behaviour depends on whether PyYAML happens to be importable (and `.github/workflows/okf_wiki.yml` installs PyYAML into a venv but runs the gate with the system python3, so both readers are live here). Migration is per-file and incremental — 6 concepts / 28 anchors migrated (all 6 that cite `worker/src/index.ts`), anchors derived from `checkpoint_sha:<path>` so no baseline was silently moved to "now"; the other 154 concepts keep the legacy path verbatim. 14 new hermetic fixtures prove C5 lost no teeth under a blob anchor (in-range edit fires; pure position-shift fires with the diff hunk provably disjoint from the cited range; untouched content does not), that the rebase tax is gone (same repo, same file, same orphaned checkpoint: the commit-anchored concept falsely REDs, the blob-anchored one does not), and that the laundering path is closed and not moved. Verified against every sibling parser: `okf_index.py` and `okf_render.py` produce **byte-identical** output pre/post migration, `okf_context.py --file worker/src/index.ts` still resolves all 6 concepts, and `validate_okf.py` is green under BOTH readers (`OKF_NO_YAML=1` and PyYAML) and under `--nightly`.
...
### Fixed
- **fix(ci): the docs-reality gate never looked at the repo root, so `ROADMAP-TO-LAUNCH.md` shipped a subcommand that has never existed and a domain we do not own.** The gate cross-checks every documented `corelink <verb>` against the CLI's clap enum and it works — it walks `apps/docs/{docs,blog,src}`, `marketing/`, and `docs/**` minus the internal wiki. What it never walked is `REPO_ROOT/*.md`: `README.md`, `ARCHITECTURE.md`, both roadmaps, the GA release notes, `CONTRIBUTING.md`, `SECURITY.md`, `TODO.md`. Those are among the most-read files in the project, so a phantom command there reaches more readers than one buried in a tutorial. Sure enough, `ROADMAP-TO-LAUNCH.md:183` and `:198` still prescribed **`corelink ping`** — the subcommand whose removal from the public installer is recorded three entries above in this same release — and `:198` and `:302` still told the reader to `curl -fsSL https://get.corelink.io`, **a domain we do not own** (`corelink.io` belongs to a third party; the takeover risk was written up when the same string was purged from the installer). Repointed to `corelink whoami` and `https://corelink-get.humangr.com`, the surfaces that actually exist. **Coverage measured both directions, not asserted.** Old scanner + the broken roadmap: `OK docs-reality: 305 CLI refs, 0 drift`, **exit 0** — green while looking straight past it. New scanner + the same broken roadmap: `DOCS-REALITY INVALID: 2 failures`, **exit 1**, naming both lines. New scanner + the fix: `OK docs-reality: 316 CLI refs, 0 drift`, exit 0. The `+11` is the coverage the gate gained. The root glob is deliberately **non-recursive** — every subtree worth scanning already has a root above it, and `rglob` from `REPO_ROOT` would re-walk the whole repository. **`CHANGELOG.md` is excluded, and that exclusion is load-bearing rather than convenient:** it is a historical ledger whose entries DESCRIBE defects, so it legitimately quotes commands that do not exist — 9 mentions of `corelink ping` today, every one narrating the fix that removed it. Scanning it would turn the gate red for recording history accurately, and a gate that is red for being correct is a gate that gets allowlisted into uselessness — which is precisely how `corelink ping` survived in the installer in the first place (`scripts/docs_reality_allowlist.json` carried it with a written, accurate justification while it broke every install). **Not fixed here:** the gate checks CLI verbs, not hostnames, so `get.corelink.io` was found by grep and not by the gate — nothing would stop the next foreign-domain string from landing.
- **fix(admin-ui): the welcome activation stream connected to the MARKETING site and sat there forever — `EventSource`, unlike a broken `fetch`, reports no error when it opens against HTML.** Five client-built URLs still lacked the `/corelink` mount prefix. Next re-attaches `basePath` to framework-generated navigation only, never to a URL a component hands to `EventSource`/`fetch`, and the apex `humangr.com` is a **different application** (the `hugr-site` marketing Pages project) — so an unprefixed client path is answered by another product instead of 404ing. Measured live 2026-08-03: `GET humangr.com/api/welcome/stream` returns **`200 text/html`** (marketing HTML) where `…/corelink/api/welcome/stream` returns `307` to sign-in (the real handler), and `POST humangr.com/api/v1/customer/billing/portal-session` returns **`405`**. The `200 text/html` is the poisonous one: `WelcomeStream` opened its stream against HTML, the connection SUCCEEDED, and no event ever arrived — the activation pane waited indefinitely and merely looked empty, with nothing logged and nothing red. That component is the reader for the `first_cli_authed` emitter wired earlier this cycle, so the emitter has never had a reachable consumer. Every site now routes through the existing `withAppBasePath()` helper (no second convention invented): `WelcomeStream`'s stream URL; `PortalLauncher`'s portal-session `POST` — **dead code today**, since the live "Manage subscription" control is `BillingClient` → `CustomerClient`, whose absolute `NEXT_PUBLIC_CORELINK_API_URL` keeps it off the apex; and the three byte-identical same-origin `?? "/api"` base-URL fallbacks in `lib/admin-client.ts`, `lib/customer-client.ts` and the customer audit-visualization page, which are latent only for as long as that env var stays set. **A test was pinning the defect:** `tests/customer-dashboard.test.tsx` asserted the bare `"/api/v1/customer/billing/portal-session"` literal as the expected value; it now asserts the prefixed form. The class — its fourth recurrence — is closed by a guard in the new `tests/basepath-client-fetches.test.tsx`: any string literal under `src/` that BEGINS with `/api` must be prefix-wrapped on its own line or carry a reviewed exemption, of which there are exactly two (the middleware match prefixes, compared against a pathname Next has already stripped, and a constant whose only consumer is `<Button href>`, which prefixes it itself). All five fixes were mutation-proved — each one reverted individually turns the suite red. admin-ui vitest: 508 tests green (the lone `CookiePolicyPage` 5 s-timeout is a pre-existing flake under machine load and passes in isolation); `tsc --noEmit` clean.
- **fix(admin-ui): the CSP violation sink had never received a single report — `report-uri` pointed at the origin root while the handler is mounted under the `/corelink` basePath, so 100% of violations were discarded.** `src/lib/csp.ts` emitted the literal `report-uri /api/csp-report`. A report URL is resolved against the **document's** URL, so a root-absolute path lands on the apex — which is the separate `hugr-site` marketing Pages app, not this one. **Proven live** on `https://humangr.com/corelink/sign-in`, whose enforce-mode header carried exactly that directive: `POST https://humangr.com/api/csp-report` → **405** (Pages rejecting a POST to a static asset), while `POST https://humangr.com/corelink/api/csp-report` → **204** from the real handler. Under `next dev` the same URL 404s, which is how it first surfaced (4× `POST /api/csp-report 404` on a single page visit). Same failure mode as the Clerk-widget basePath blindness closed in #894 — third-party widgets ignore Next's `basePath`; here it was our own header. The path is now **derived** from `route-matcher.withAppBasePath()`, the helper every other hand-built URL in this app already re-attaches through, rather than a second `/corelink` literal that can drift from `next.config.ts`. **The unit test was pinning the defect, not catching it:** `tests/csp.test.ts` asserted `toContain("report-uri /api/csp-report")` — the broken value verbatim — and the E2E spec asserted only `/report-uri/` while `page.route(/\/api\/csp-report/)` fulfilled the doomed request with a synthetic 204, so the harness intercepted the exact 404 it existed to detect and printed the result to `console.info` without asserting it. Both are replaced with assertions that fail on a revert: the report path must equal `${APP_BASE_PATH}/api/csp-report`, must not be the pre-fix literal, must equal `withAppBasePath("/api/csp-report")`, and must still satisfy `isPublicPath` in both the prefixed and middleware-stripped shapes (the browser POSTs it unauthenticated). Mutation-verified — with the fix `vitest run tests/csp.test.ts` exits 0; with `src/lib/csp.ts` reverted to the literal it exits 1. **`report-uri` is deprecated, and it was the only channel**, so the fix also adds the Reporting API v1 pair — a `report-to csp-endpoint` directive plus a `Reporting-Endpoints: csp-endpoint="/corelink/api/csp-report"` response header (distinct from the legacy `Report-To: {"group":"cf-nel"…}` Cloudflare's edge already injects, so they do not collide). Both channels are advertised because they cover **disjoint** browser populations: Chromium implements `report-to` and ignores `report-uri` whenever a resolvable group is present, while Firefox and Safari implement `report-uri` only — dropping either would blind us to a browser family, so this is the non-lossy shape of the deprecation rather than a swap. The two channels also POST different bodies (one `application/csp-report` object vs an `application/reports+json` **array**), so `app/api/csp-report/route.ts` now normalizes to one `{ip_hash, report}` envelope per violation; forwarding an array verbatim under the singular key would have traded one dropped-report bug for another. **Pre-existing and NOT fixed here** (each needs its own decision, and this branch carries one concern): the sink's own forward target `POST ${NEXT_PUBLIC_CORELINK_API_URL}/v1/csp-violations` is **unauthenticated and answers 401**, so with the pipe repaired reports now reach the Next route (204) and are then dropped one hop later into the `console.error("csp-report forward failed")` fallback — the second half of this pipe still needs a credential; and two sibling hand-built URLs are basePath-blind by the same mechanism — `WelcomeStream.tsx`'s `new EventSource("/api/welcome/stream")` and `PortalLauncher.tsx`'s `fetch("/api/v1/customer/billing/portal-session")`, both of which hit the apex (200 marketing HTML and 405 respectively) instead of the app.
- **fix(admin-ui): the GDPR/LGPD consent surface has NEVER had a backend, and the client pointed at the wrong ORIGIN — so four reachable screens turned a nonexistent API into a generic "something went wrong".** Two independent defects stacked. **(1) Missing backend, the headline:** none of `/v1/consent/active`, `/v1/consent/grant`, `/v1/consent/{id}/withdraw`, `/v1/consent/history` or `/v1/subprocessors` is served by anything in this repo. The 55 `.route("…")` mounts across `crates/` contain zero consent paths; `worker/src/index.ts` has no consent branch; and `corelink-privacy` — the crate that owns the consent ledger and whose doc-comments describe this very API — is not a dependency of `corelink-container` or of any Worker, so it is not compiled into anything deployed. Its DESIGNED shape does not even match the client that calls it (`POST /v1/consent/<purpose>` + `DELETE /v1/consent/<purpose>` + `GET /v1/consent`, versus the client's `/grant` + `/{id}/withdraw` + `/history`). ⚠️ **The live `401` is not evidence the route exists:** `corelink-api.humangr.com` answers `401` to EVERY `/v1/*` path, including `/v1/definitely-not-a-real-route-xyz123` and `/v1/healthz` — it is a blanket edge auth gate that masks routing entirely, and reading it as "auth-gated, therefore implemented" is exactly the inference this entry exists to refuse. Consent capture, withdrawal, history and the sub-processor list are therefore **feature-dead**, not merely misrouted; this change makes them fail honestly and land the day a backend does. **(2) Wrong origin, now fixed:** `lib/consent-api.ts` handed BARE `/v1/...` strings to `fetch()` from the browser. `/v1` is not a route in this Next app at all — no `/v1` segment under `src/app`, no rewrite — so the request resolved against the page origin, and the apex is a SEPARATE application (the marketing Pages project). Measured 2026-08-03: `GET humangr.com/v1/consent/active` returns **`200 text/html`**, i.e. marketing HTML, where the real origin returns `401`. That `200` is the reason this sat unnoticed for a full sprint cycle — worse than a 404, because the fetch RESOLVES and only `res.json()` throws, so each screen reported a parse-shaped generic error instead of a routing fault. Note this is **not** a basePath defect and must not be fixed as one: prefixing the mount would give `/corelink/v1/...`, which is not a route here either. All six call sites (five in `lib/consent-api.ts` plus `postConsent` in `lib/consent.ts`) now go through a new `resolveApiBaseUrl()` — the `CustomerClient` convention extracted verbatim into `lib/api-base-url.ts` rather than pasted a fourth time, with no hostname hardcoded. **Auth was absent too:** the screens defaulted to a module-level client with no token supplier, so the surface could not have authenticated even against a backend that existed. A new `createConsentApi()` + `useConsentApi()` pair mirrors `CustomerClient`/`useCustomerClient` (latest-ref, stable identity, E2E carve-out) and attaches the Clerk session bearer — the credential every other tenant-scoped `/v1/*` client in this app already sends. **Also fixed:** the withdraw confirm-dialog's imperative `window.location.assign()` (the one navigation the design kit does not prefix, so it alone landed on the apex); and the sub-processors SERVER page, which read `CORELINK_API_BASE` — an env var defined NOWHERE in this repo — degrading to a relative URL that Node's fetch rejects outright, meaning its `try` block had never once reached the network and the bundled JSON was always being served. **Tests were actively masking this:** `CookiePolicyPage.test.tsx` destructures `[, init]`, deliberately discarding the URL argument, and returns a 200 JSON stub — making the doomed request look successful; the case named "calls `/v1/consent/history` with the correct page + page_size" asserts only the query object handed to an injected stub and never observes a URL; and every consent unit test injects a `ConsentApi` stub, so the URL-building code had ZERO coverage. The Playwright fixture compounds it by intercepting the regex `/\/v1\/.*/`, which matches the apex URL just as happily as the correct one. New `tests/consent-api-origin.test.tsx` drives the real client instead, and every fix is mutation-proved: each reverted individually turns it red (7/7 caught). ⚠️ **One more structural blocker, found while wiring the bearer and NOT fixed here:** the consent routes sit at `[locale]/consent/*`, OUTSIDE the `[locale]/(authenticated)` group that mounts the only `<ClerkProvider>` in the app — the root `src/app/layout.tsx` deliberately mounts none — so these screens have no session context to draw a token from at all, and `/consent/new` is deliberately public/pre-auth on top of that. A bare `useAuth()` there THROWS and takes all four screens down, so the token lookup degrades to a null supplier and the bearer starts flowing automatically the day a provider is mounted above them; a regression test renders the dashboard with `useAuth` throwing and asserts it still renders. admin-ui vitest green with the 11 new cases (529/529 across 110 files, rebased onto the merged #983); `tsc --noEmit` clean; eslint 0 errors and no new warnings.
- **fix(okf): the C5 anti-drift gate reported only the FIRST drifted citation per (concept, file), so "fix everything it printed" left stale citations behind.** `scripts/validate_okf.py` `break`ed out of the per-range loop on the first hit, which made the offender list a **lower bound** rather than a worklist: an author fixed the one range printed, re-ran, was handed the next one, and repeated — and because the count is what people read, a concept with three drifted ranges reported `1 failures`. The ranges were already computed; the `break` bought nothing. It is gone, plus a `dict.fromkeys` dedup so a range legitimately cited twice (once under `# How it works`, once under `# Citations`) is still reported ONCE and does not inflate the count — the same dedup `scripts/okf_reconcile.py:101` has always applied, so the gate and the reconcile reporter now agree on the worklist instead of disagreeing on its size. Proven on a hermetic three-range fixture (`tests/okf/run_fixtures.sh` `assert_c5_all_ranges`): the same repo run against the pre-change validator prints one offender and `INVALID: 1 failures`; against the new one it prints `multi.txt:2-2`, `:5-5` and `:8-8` and `INVALID: 3 failures`. The fixture also asserts the dedup direction (a twice-cited range → exactly one offender line), so neither half can regress silently.
- **fix(ci): one runner's cache-save step deleted `~/.cargo/registry/src` out from under four OTHER runners' in-flight `rustc` processes, reddening PRs whose diff contained zero Rust files.** The self-hosted fleet is FIVE runners (`~/.gh-runners/runner-1` … `runner-5`) on ONE Mac sharing ONE `$HOME`, therefore ONE `~/.cargo`. `Swatinem/rust-cache` is built for GitHub-hosted runners, where `$HOME` is job-private and ephemeral; its **save (post) step prunes the cargo home before uploading** — both pinned bundles (`400e7407…` v2.7.7, `c1937114…` v2.9.1) walk `path.join(CARGO_HOME, "registry", "src")` and `rmRF` what they judge unneeded (10 and 17 `rmRF` call sites respectively, 5 `cleanRegistry`). On this fleet that is a cross-runner `rm -rf` against a directory another job is compiling from. **The error is misattributed by construction:** cargo launches `rustc` with its cwd inside the package's `registry/src` directory, so the *spawn itself* fails — `could not execute process rustc … (never executed)` / `No such file or directory (os error 2)` — and every sibling unit still in flight then reports the far more visible collateral `could not parse/generate dep info at: …/target/<profile>/deps/<crate>-<hash>.d`. Readers chase the `.d` file, which is a symptom; the `(never executed)` line above it is the cause. **Proven by causal ordering on #974** (a cron-removal PR: 14 workflow YAMLs + `CHANGELOG.md` + `CLAUDE.md`, **zero Rust files**, four red Rust jobs). Window A — `corelink-builder-2` ran its rust-cache save `02:25:21→02:26:32`; `corelink-builder-3`'s *30k property test* died at **02:26:02**, mid-window, on `ctutils` from the registry. Window B — `corelink-builder-4` saved `02:50:17→02:51:15`; **two different runners** died inside it, `corelink-builder-3` (clippy + corelink-privacy) at 02:50:28 and 02:50:52, and `corelink-builder-2` (adversarial regression) at 02:50:40, 02:50:52, 02:51:15 and 02:51:40. The victims' own post-steps were `skipped` — they were not destroying themselves. ⚠️ **A prior hypothesis was refuted rather than assumed:** cross-runner restore of a `target/` carrying another runner's absolute paths does NOT explain it — every failing `.d` path names the runner that *owns* the job (`runner-3` job → `runner-3` path, `runner-5` job → `runner-5` path), and the fault is in the SHARED `~/.cargo`, not the per-runner `target/`. (A restored `target/` from another runner is genuinely possible — nothing in the cache key distinguishes runner-N from runner-M — but it is benign: cargo treats a `.d` whose paths are missing as dirty and rebuilds.) **Fix:** all 80 cache steps that touch a shared cargo home — 73 `Swatinem/rust-cache` across 33 workflows, plus 7 `actions/cache` steps whose `path:` covers `~/.cargo` — now carry `if: runner.environment == 'github-hosted'`. **The trade-off is measured at zero:** on a *persistent* runner `~/.cargo` and `target/` already survive between jobs, so the action carries state across a gap that does not exist here; worse, its save step also prunes `target/` each run, so removing it strictly *improves* local reuse rather than costing it. Hosted jobs are byte-unchanged. The condition is fail-safe — were `runner.environment` ever unavailable it evaluates false, disabling caching rather than re-enabling the race. **Regression-locked** by new `scripts/validate_shared_home_cache_guard.py` (stdlib-only; the fleet cannot provision PyYAML via `setup-python` because `RUNNER_TOOL_CACHE` is unset), wired into the `actionlint` lane, which additionally FAILS if it finds zero cache steps so a broken scanner cannot pass vacuously. Proven in both directions: green on the fixed tree (80/80 checked), RED when the guard is stripped from the `tenant-path.yml` fuzz step that was the measured window-A destroyer, and RED listing all 80 against the pre-fix tree. **Mechanism reproduced deterministically off-fleet** in an isolated `CARGO_HOME`: a control build passes, and the same build with a concurrent `rm -rf $CARGO_HOME/registry/src` at t+4s emits the identical signature — `(never executed)` / `os error 2`, then `could not parse/generate dep info at: …/deps/syn-<hash>.d`, exit 101 — the same crate named in the #974 report. SHA pins untouched (`verify-action-sha-pinning.py`: 545 `uses:` lines OK). **Not fixed here, reported:** `~/.cargo/bin` is shared too, so `cargo install cargo-fuzz` from concurrent fuzz jobs remains a write race that this guard does not cover.
- **fix(ci): the pre-merge gate printed `✅ All gates green — OK to merge` for a PR on which NONE of its real gates had run.** Caught live on #967 — a PR touching two Workers where `dco`, `changelog-validate`, `gitleaks`, `trivy`, `secrets-matrix` and every `vitest` job had **never executed**. Six check-runs existed; all six were `pull_request_target` metadata jobs (path labels, size bucket, welcome comment, dependabot sentinel). Zero failures out of zero real gates read as green. **Mechanism:** `pull_request` workflows run on the **merge ref**; when a PR conflicts GitHub cannot build that ref, so it creates **no runs at all** — not queued, not failed, *absent* — while `pull_request_target` workflows, which run on the base branch, still fire. That asymmetry is the fingerprint, and it means **the most dangerous PR state produced the most reassuring output**. Proven in both directions on #967: `mergeable: CONFLICTING` → 0 `pull_request` runs (surviving a close/reopen AND an empty-commit push, so not a queue delay), then `git rebase origin/main` + force-push → `MERGEABLE` → **12 runs** within ~40 s (same on #970: 0 → 8). This is not a rare state in this repo: every `feat:`/`fix:` commit needs a CHANGELOG `[Unreleased]` entry and everyone inserts at the top of the same section, so two concurrent PRs conflict by construction and `main` moves under a branch within minutes — #967 conflicted *twice* during a single session. **Three defenses added, in order of how badly the script needed them:** (1) `mergeable` must be `MERGEABLE` — `CONFLICTING` is refused with the rebase instruction, and `UNKNOWN` is refused too because GitHub computes it asynchronously, so "not yet known" is *ask again*, not *fine*; (2) the always-present gates (`dco`, `gitleaks`) must actually APPEAR in the check list — both are triggered by a bare `on: pull_request` with **no paths filter**, so their absence proves the `pull_request` workflows never fired, and this catches the failure even if a future GitHub change makes a broken PR look mergeable (the list is kept deliberately short and path-filter-free, since adding a path-filtered gate would fail on PRs that legitimately skip it — turning a fail-open into a fail-noisy that the next person in a hurry simply disables); (3) "no checks reported" now FAILS instead of `exit 0` with *"nothing to gate"* — on a repo where every PR runs dco + gitleaks unconditionally, zero checks means the workflows did not fire, which is a reason to stop rather than to merge. ⚠️ This script is the last line of defense because branch protection has `required checks = []` (CLAUDE.md), so it must never fail OPEN. All four paths proven deterministically against a stubbed `gh` (conflicting / gates-missing / no-checks → exit 1 with a specific diagnosis; healthy → green), and the OLD script was run against the identical conflicting fixture to confirm it printed `✅ All gates green` where the new one refuses.
- **fix(ci): `changelog-validate` asserted that a `feat:`/`fix:` PR *has* an `[Unreleased]` entry — never that it has exactly ONE — so a rebase could duplicate an entry and the gate stayed green; it now fails on a duplicated entry, detected on three independent signals, and its first audit of `main` finds five surviving duplicate groups / six redundant entries.** The whole assertion was "at least one added line falls between `## [Unreleased]` and the next `^## \[`" (`.github/workflows/changelog-validate.yml:84-163`): cardinality was never checked. That is the hole a rebase drives through — a CHANGELOG commit that *replaced* an entry (`1 insertion(+), 1 deletion(-)`) degrades to insertion-only when upstream moves the context the deletion was anchored to, so the stale entry and its replacement both survive and the stale one keeps asserting what the replacement was written to retract (observed on #955). New `scripts/check_changelog_duplicates.py` links two entries when ANY of three signals fires, then reports the **transitive closure** as groups with a redundant-entry count — a 3-way group is one finding, not three pairs, and the operator gets the number they must actually delete. **The thresholds are measured, not guessed:** every pair of the 583 comparable entries (169 653 pairs) on `origin/main`@`656aac35` was scored and hand-classified, with `git log -S` provenance settling the ambiguous ones. **(1)** lead-prefix — identical leading tokens of the bolded lead sentence: honest siblings top out at **9** (the `MKCOL` WebDAV fix vs the `PROPFIND`/`DELETE` one — two changes to one surface), duplicates start at **14**; threshold **12**. **(2)** body-shingle containment over the entry body: honest max **0.297** (two distinct KV-L2 latency slices), duplicates from **0.551**; threshold **0.42**. **(3)** cited-evidence — containment of rare backticked artifacts, gated on a shared Conventional-Commit scope: honest max **0.571** over 702 eligible same-scope pairs, duplicates from **0.727**; threshold **0.65**. **Why three and not one:** a lead-only check has its blind spot exactly where the damage is, because *a retraction rewrites the lead* — that is what retracting means — so the lead signal is strongest on the harmless byte-identical clone and weakest on the corrected entry whose predecessor asserts something false. `CHANGELOG.md:80`/`:81`@`656aac35` (the incident group) cleared the prefix threshold by 4 tokens purely because that author left the opening clause intact; reword it and a lead-only gate passes it. Bodies and citations survive that rewrite — **proven**: both retraction pairs, reconstructed verbatim from `a000c24e`/`500a493f` and `212143b2` with the stale lead fully rewritten (prefix 2, Dice ≤ 0.13), are still caught, by body-shingle 0.551 and cited-evidence 0.769 respectively. **Signals that were tried and rejected rather than loosened:** TF-IDF cosine over the whole entry (duplicates from 0.496 vs honest up to 0.626 — fully overlapping), rare-word IDF containment, and citation containment *without* the same-scope gate (honest reaches 0.833). Bigram Dice on the lead is likewise **not** a discriminator on its own (the incident pair scores 0.652, *below* the 0.800 of an honest sibling pair) and is kept only as a high-confidence catch — all of it recorded in the script so nobody collapses it back to one ratio. Scope is `[Unreleased]` only (released sections are frozen history and may repeat wording), and by default only groups touching a line this PR *added*, judged from `git merge-base` so a branch rebased onto a newer `main` is not blamed for what it inherited. A `--self-test` step runs seven embedded fixtures in CI — three duplicate shapes (lead intact, lead rewritten, lead *and* body rewritten) must fail; honest same-scope siblings, an honest pair sharing cited artifacts, a repeat across a released section and the `- (none)` placeholders must pass — so the gate is proved in both directions on every run, in ~2 s. **Pre-existing, NOT fixed here** (a one-entry-per-PR CHANGELOG change may not delete other people's entries): `--all` on `656aac35` reports groups at `:47`/`:48`, `:50`/`:51`, `:66`/`:67`, `:80`/`:81`/`:82` and `:117`/`:119`. The last is **new** — it is not a rebase artifact but one commit, `9be1ad23`, shipping ONE fix and writing the entry twice, and only the cited-evidence signal sees it. A backfill branch is clearing the first four; `:117`/`:119` needs adding to it.
- **fix(marketing): the legal evidence pack invited auditors to read source in a repo that has never existed, and offered them an SBOM and Cosign signatures that were never produced.** `marketing/sales/legal-questionnaires/EVIDENCE-PACK-INDEX.md:26` defined an access class **OPEN-SOURCE** = "visible in the public `HumanGuardrail/corelink` repo" and classified **17 rows** against it. Measured 2026-08-02: `HumanGuardrail/corelink` and `HuGR-Labs/corelink` both return **HTTP 404** — the repo never existed in either org, so this was not org-rename drift; the real codebase `HuGR-Labs/corelink-server` is **private**, and the owner has settled the question: **CoreLink is not open source.** Every one of the 17 artifacts was verified to live only in that private tree (`scripts/validate_dpia.py`, `specs/03_architecture/invariant_registry.md`, `legal/sub-processors.md`, `specs/03_architecture/slo_catalog.md`, …), so 15 rows move OPEN-SOURCE → **NDA** — a promise an auditor can go verify was replaced by one we can actually keep. The class itself is replaced by **PUBLIC-REPO**, defined against the two repos that *are* public and measured at HTTP 200 — `HuGR-Labs/corelink-cli` and `HuGR-Labs/clw-releases` — with the explicit caveat that both publish **build outputs only, no source**, plus a ⛔ banner at `:24` so the next editor cannot re-derive the old claim. **The two supply-chain rows were worse than mislabelled — their evidence does not exist in any form, public or private.** Row 70 (SBOM) pointed at releases of the 404 repo; a real CycloneDX 1.6 SBOM *does* exist, committed at `.sbom/cyclonedx-rust.json` (413 components) + admin-ui (50) + docs (25), generated 2026-05-28 by commit `1ed22958` — but it is **not** published per release and never has been: `.github/workflows/sbom.yml` has **12 runs and 0 green**, including its single `release`-triggered run (`26659689412`, 2026-05-29, failed), and `.github/workflows/sbom-consolidated.yml` has **never run**, so no release carries an SBOM asset and the in-tree snapshot is hand-refreshed and may lag the shipped build. Row 71 (Rekor / Sigstore) offered "Cosign signatures per release" when **no CoreLink artifact has ever been signed**: `.github/workflows/cosign-sign.yml` has **0 runs ever** because it triggers on a `v*` tag and no `v*` tag has ever been pushed (all 52 tags are `cli-v*` / `*-sealed`, which do not match the glob), CLI signing is still the literal placeholder `"[TODO v2] cosign sign (placeholder)"` at `.github/workflows/release-cli.yml:247` echoing *"cosign keyless signing not yet wired"*, and SLSA provenance is in the same state (`.github/workflows/release-slsa3.yml`: 1 run, failed). Neither row was deleted — an auditor's question with no good answer is answered honestly, not removed: row 70 now cites the real `.sbom/` files under NDA, row 71 is marked **NOT LIVE** and points at the integrity evidence we can actually substantiate, the published per-binary SHA-256 checksums at `https://github.com/HuGR-Labs/corelink-cli/releases/download/v0.1.1/checksums.txt` (**HTTP 200**, the one row that stays public), and a new §1.1 "Supply-chain caveat" records the measurements so the next responder answers **"not today"** instead of re-promising a phantom. The same phantom was propagating through the answer keys: `SIG-LITE-2026-pre-filled.md:99` (D.7) answered **Y** to "do you maintain an SBOM?" with "CycloneDX SBOM signed and published with every release; SLSA Level 3 build attestation; Cosign-signed releases; Rekor transparency entries" and cited the 404 URL — now **P**, with the true position stated; D.6's "Dependency-Track per release" is corrected to the license gate that is genuinely green (`.github/workflows/license-policy.yml`, 28 successful runs). Also swept: the §3 watermark and §4/§5 CCM/SIG lookup cells stopped advertising "SLSA / Sigstore", the bundle-hygiene step no longer instructs operators to sign ZIPs with a Cosign release key that is not in service, and 3 stale `HumanGuardrail/corelink-server` references became `HuGR-Labs/corelink-server` (private). `python3 scripts/validate_docs_reality.py` stays **exit 0**, byte-identical to its pre-change baseline (303 CLI refs, 0 tracked, 0 drift). **Not fixed here, reported:** rows 89–90 still cite `https://docs.corelink.humangr.com/slo` and `https://status.corelink.humangr.com`, which fail DNS resolution and TLS handshake respectively — pre-existing on `origin/main`, untouched by this change, and owned by the dead-hostname sweep.
- **fix(docs+cli): a troubleshooting row told customers to pin a region by URL — there is no region-pinning mechanism at any layer, and the edge actively strips the attempt.** `apps/docs/docs/tutorial/06-verify-cache-hit.mdx:57` (plus all three locales) answered "slow p95" with `build --remote_cache=https://<region>.<tenant>.corelink.humangr.com/v1/cache`. The dead dotted host was the least of it: **the advice is wrong about the model.** Region is a tenant provisioning property — `tenant.primary_region` (`migrations/d1/0028_tenant_primary_region.sql`, `0027_region_provisioning.sql`, `0052_tenant_config_region.sql`), read server-side via `COALESCE((SELECT primary_region FROM tenant WHERE tenant_id = ?2), 'wnam')` (`crates/corelink-container/src/storage/d1_audit_sink.rs:136`, `routes/dsr/audit.rs:115`, `routes/dsr/access.rs:339`). It is not selectable per request **by any means**: the edge Worker looks it up in D1 and fans out to the regional Worker (`worker/src/index.ts:2850-2862`), **strips any client-supplied value** and stamps the trusted `x-corelink-primary-region` itself (`crates/corelink-container/src/routes/residency.rs:42`), and a cross-region request is rejected **409 `residency_violation`** (`residency.rs:58`,`:75`) — with migration `0023_residency_check_constraints.sql:31-45` installing DB triggers that `RAISE(ABORT)` on any row whose region differs from the tenant's. Three independent confirmations that there is no customer-side lever: `grep` of `tools/cli/src/` for `defaults.region` / `--region` / `primary_region` returns **zero** surfaces (the one `config.rs:42` hit is a *Datadog* per-region block, not ours); the CLI's own `doctor` check #6 is a deliberate **SKIP** whose reason is "Tenant primary region is not exposed on a public read route" — the `/v1/tenant/region` route it once assumed **never existed** (`tools/cli/src/doctor.rs:15`,`:318-326`); and `POST /v1/admin/tenant/region-migration`, cited by `explanation/residency/lgpd-brazil.mdx:95` as the way to move, exists as a privacy-crate ticket type (`crates/corelink-privacy/src/residency/migration.rs:1`) but is **not mounted in the container router** — so the row does not send anyone there. The row now states the model and points at support, because a region change is a data-residency change, not a config change. **(2) Same false claim, still live in code.** `tools/cli/src/commands/bazel_init.rs:14` still asserted that stock `--remote_cache=http(s)://…/cas` "404s there", and justified itself against a `grpcs://cas.corelink.humangr.com:443` endpoint the tutorials "advertise" — an advertisement removed in the previous commit. Both halves are false: the stock-Bazel HTTP alias is BUILT and routed (`crates/corelink-container/src/routes/bazel_v2.rs:339`,`:343`; unauthenticated `GET /bazel/cache/cas/<64-hex>` returns **401**, not 404). The doc-comment now describes both live HTTPS surfaces and why this command emits the REAPI v2 one (it has already resolved the tenant, so it can pin `--remote_instance_name`). **Comment-only: 16 insertions, 8 deletions, every changed line a `//!` doc-comment, zero executable lines** — verified by diffing with all `//!` lines filtered out (empty result). `cargo check -p corelink-cli` clean; `cargo fmt --all -- --check` clean. Gates: docs-reality `303 CLI refs, 0 tracked, 17 warnings, 0 drift`; OKF `160 concepts, 0 stale, 0 drift` — no concept cites `bazel_init.rs`, so no `checkpoint_sha` moved.
- **fix(docs): the docs told customers the shipped stock-Bazel HTTP cache was "in progress", and handed out `grpcs://` CoreLink addresses for a surface that cannot exist.** Opposite defects, one root cause: prose never re-checked against the wire. **(A) Understatement.** `/bazel/cache/{cas,ac}/<hash>` — what vanilla `bazel --remote_cache=https://host/bazel/cache` actually sends — is BUILT and routed (`crates/corelink-container/src/routes/bazel_v2.rs:339`,`:343`; Worker arm `worker/src/index.ts:707`, where the tenant is resolved from the PAT, NOT the URL, so `--remote_instance_name` must be dropped). Production agrees: unauthenticated `GET /bazel/cache/cas/<64-hex>` returns **401** — route wired and PAT-gated — not 404. Five customer-facing pages said the opposite: `apps/docs/docs/tutorial/03-bazel-quickstart.mdx:20` ("currently return **404**… A stock-HTTP alias is being built and is not yet live"), `tutorial/01-installation.mdx:66`, `tutorials/quickstart-5min.mdx:184`, `tutorials/quickstart-10min.mdx:172`, and `tutorial/04-buck2-quickstart.mdx:21` ("the only wired build-tool cache in the Bazel family is … `/bazel/v2`"). Each now states the alias is live and prints the config line that works. **Understating a shipped feature is the same defect class as overclaiming one** — it loses the deal for nothing. **(B) Phantom endpoint.** There is **no gRPC ingress at all**: the container serves a single HTTP/1.1 axum listener (`crates/corelink-container/src/main.rs:902-905`) behind workerd, which implements no HTTP trailers — `grpc-status` lives in a trailer, so a well-formed gRPC response cannot be emitted on this topology, and `crates/corelink-reapi` is host-only and unreachable from it. Yet `how-to/migrate/from-bazel-remote-cache.mdx` gave `grpcs://…:443` as the **target** (`:18`), the **BES backend** (`:118`), the post-migration `bazel info` expectation (`:319`) and — worst — the Phase-3 **cutover** line a customer pastes into `.bazelrc` (`:226`): follow the guide to its end and you do not connect. `from-docker-registry.mdx:158`,`:179` configured a `grpcs://` endpoint for a `corelink-registry-driver` storage shim **that does not exist in this repo** (no such crate, no such image), while the real OCI Distribution v1.1 registry has been live at `/v2/*` + `/token` the whole time (`crates/corelink-container/src/routes/oci.rs:1-9`) — the same page called those endpoints "post-GA roadmap". Corrected per tool against each tool's own documentation: **Bazel works** (repointed at the HTTP alias, tool kept); **Pants cannot** (its `reapi` provider accepts only `grpc://`/`grpcs://`; its lone HTTP provider targets GitHub's cache service, not a Bazel-style cache); **Buck2 cannot** (`facebook/buck2#459` closed **`wontfix`**); **`--bes_backend` cannot** (BES is gRPC-only, with no HTTP transport to fall back to). Stated plainly, with the mechanism — no date promised, no softening into vagueness. **(C) The translations were the worst offenders**, which is precisely the defect being fixed: the EN Buck2 page has told the truth for weeks while `i18n/{pt-BR,de,es-419}/…/tutorial/04-buck2-quickstart.mdx:35-37` still shipped a working-looking `.buckconfig` with three `grpcs://` addresses; the locale `quickstart-10min` pages printed a **fabricated `corelink bazel-init` transcript** (`--remote_cache=grpcs://cas.corelink.humangr.com:443` plus an `x-corelink-tenant` header) that the CLI has never emitted — the real block is `{endpoint}/bazel/v2` + `--remote_instance_name` + `--remote_timeout=30s` (`tools/cli/src/commands/bazel_init.rs:73-85`,`:159`); and the locale `tutorial/03` pages pointed `--remote_cache` at the dead dotted host `<tenant>.corelink.humangr.com/v1/cache`. 15 MT-stub pages — whose own `i18n:MT … bootstrap MT-stub from EN source` marker declares them stale copies of EN, and whose bodies were still English — were resynced byte-for-byte from EN; genuinely translated blocks (`quickstart-5min`, `quickstart-10min`, `integrations/bazel.md`) were edited in-language instead. **Parity is now mechanically checkable**, not asserted: every full-sync page is byte-identical to its EN body, and **0** CoreLink `grpcs://` addresses remain anywhere in `apps/docs/**` (the surviving `grpc://` hits are the customer's own OTLP collector and their own `bazel-remote` on `localhost:9092` — deliberately untouched). Also corrected `src/pages/compare/vs-bazel-remote-s3.mdx:280`, whose cutover `.bazelrc` pointed at `/cache/v1/<workspace-id>`, a path no route serves. Docs-only. Gates unchanged: docs-reality `303 CLI refs, 0 tracked, 17 warnings, 0 drift`; OKF `160 concepts, 0 stale, 0 drift`.
- **fix(signup): every free signup was granted paid Runners capacity — `seedTenantEntitlements` wrote a `runners_entitlement('free', max_concurrency=1)` row, silently reverting owner-ratified Option B.** Runners is a SEPARATE PAID axis from the cache tier (ratified 2026-06-13), chosen precisely because the alternative "leaked compute a cache-only tenant never bought". Migration `0070_runners_entitlement.sql` states the contract in its own header: "the table starts empty … NO row → the tenant is cache-only and gets no runner cap … **Empty table = no cap = reject**". Both provisioners violated it on every new tenant: `apps/signup-worker/src/lib/d1.ts` (Clerk `user.created`) and its byte-mirror `worker/src/lib/githugr_provision.ts` (githugr login). **The row is not a placeholder — it IS the entitlement `handleRunnerMint` gate 5d checks**, so writing it made the mint passable for a tenant that never bought Runners; the comment "so the runner cap gate reads a real entitlement" described the intent correctly and the effect wrongly. The other three gates do not compensate: 5b (offboarding) is absent-by-default for a healthy tenant, and 5a (installation map) + 5c (repo allowlist) are BOTH seeded by `apps/signup-worker/src/webhooks/github_install_callback.ts`, which contains no entitlement check of any kind (grepped: zero hits for entitle/tier/paid/subscription across its 387 lines). So the full chain was: **free signup → install the public GitHub App → all four gates green → boxes spawn on our Cloudflare account.** Measured in prod D1 (`d64742ea`, 2026-08-02): **119 rows** `plan='free', max_concurrency=1`, created 2026-06-12 → 2026-08-01 (i.e. still being written), 118 of them mapping to cache `tier_selections.tier='free'`. The exposure was **latent, not active** — 0 of the 119 held a `runner_repo_allowlist` row — which is exactly why nothing was ever red: no gate fires on a grant that has not yet been used. Aggravating factor: slot admission (`decideSlotAcquire`, corelink-runners `deploy/cloudflare/src/lib.ts`) is first-come-first-served against a GLOBAL `FLEET_MAX_CONCURRENCY = 20` with **no reservation for paying tenants**, so the free grant competed directly with revenue. ⚠️ **Root cause worth keeping:** `max_concurrency INTEGER NOT NULL CHECK(max_concurrency > 0)` makes "entitled to ZERO" INEXPRESSIBLE — a constraint written to guarantee that a row always means a real entitlement is what forced a writer wanting a harmless placeholder to grant real capacity instead. The fix removes the row (and the `FREE_RUNNER_MAX_CONCURRENCY` constant) from both provisioners; absence IS the zero. Regression-locked NEGATIVELY, and each pin was **proven RED against the restored bug** before being trusted: `apps/signup-worker/tests/webhook-e2e.test.ts` (`rowCount("runners_entitlement") === 0` → failed `expected 1 to be +0`) and `worker/tests/githugr_provision.test.ts` (batch length 4 + no `runners_entitlement` statement → failed `expected … length of 4 but got 5` and `expected 5 to be 4`). 846 tests green after the fix. **The 119 pre-existing prod rows are data, not code — they are deleted as a separate operator step AFTER this ships, since a deploy-before-cleanup ordering would let the live signup path re-seed them.** Requires a `corelink-signup-worker` deploy (manual `wrangler deploy`; it has no CI deploy path) plus a `corelink-api` worker deploy for the githugr mirror.
- **fix(worker): a missing internal-auth secret returned `403`, so a config fault was indistinguishable from an authorization denial — and silently ate every runner job in the fleet, permanently.** Two fail-CLOSED branches answered "no properly-sized key is bound server-side" with `403 FORBIDDEN`: `requireInternalAuth`/`requireConsumerAuth` (`worker/src/lib/internal_auth.ts`, the shared `/internal/*` gate) and the runner-mint route's own onward-credential check (`worker/src/lib/runner_mint.ts` step 3, `CORELINK_PAT_MINT_AUTH_KEY ?? CORELINK_INTERNAL_AUTH_KEY`). Both now return **`503 SERVICE_UNAVAILABLE`**. Nothing about the failure posture changes — both were and remain fail-CLOSED, nothing is authorized either way — but the ATTRIBUTION does: `401` stays "the CALLER's header is wrong", `403` stays "a verdict about THIS TENANT", and `503` now says "we cannot evaluate authz at all". **Why it is load-bearing:** the runner dispatcher branches on exactly this status (`corelink-runners deploy/cloudflare/src/lib.ts`, `mintCasPat`) — a `403` becomes `MintForbiddenError` ⇒ `driveSpawn` returns **without throwing** ⇒ `driveSpawnGuarded`'s catch never runs ⇒ **no dead-letter is recorded**, and since GitHub delivers `workflow_job.queued` exactly once and never redelivers it, the customer's job is lost *forever*; any `5xx`, by contrast, falls open to a COLD spawn so the job still RUNS, just without cache-warm (slow, never broken). So under the old `403` a single unbound secret dropped **every job in the fleet**, silently, with the only signal being a `spawn_forbidden` counter indistinguishable from ordinary "customer not entitled" traffic — a config fault wearing an authz costume, in a system where the body already said *"unavailable"* while the status said *"forbidden"*. Not hypothetical: rotating `CORELINK_RUNNER_MINT_AUTH_KEY` touches THREE Workers (`corelink-prod`, `corelink-spawn-worker`, `corelink-fabricd`) and has drifted before. **This RESTORES the frozen Rust contract rather than diverging from it** — the container already returns 503 for the same condition (`crates/corelink-container/src/routes/internal_pat.rs`: "the endpoint is unavailable (503) rather than silently widening to the shared key"); the TypeScript half was the one that drifted. The four genuine authz denials keep their `403` **and their byte-identical no-oracle bodies** (unmapped installation / suspended tenant / non-allowlisted repo / non-entitled tenant), so the security property that motivated them is untouched. Regression-locked, with **each of the two branches proven RED independently** (they fail at different steps, and a test driven only through an absent shared key never reaches step 3 at all — an earlier version of the pin stayed GREEN against the step-3 bug for exactly that reason, and was rewritten): reverting `internal_auth.ts` alone reds 5 tests across `runner_mint`/`auth_rotate`/`tenant_lookup`; reverting `runner_mint.ts` alone reds the dedicated step-3 case (`expected 403 to be 503`). A new test pins the SEPARATION itself — that a config fault and a real denial are distinct in both status and body — so a future refactor cannot quietly collapse them again. 613 worker tests green. Requires a `corelink-api` worker deploy.
- **fix(marketing+container): the public profile advertised a database we deleted, an in-code comment still taught the deleted design, and 12 dotted `*.corelink.humangr.com` hostnames in launch collateral do not resolve at all.** Two independent staleness sweeps, one root pattern: prose outlived the thing it describes. **(1) Neon.** `marketing/profile-readme.md:13` — the GitHub-profile stack line — advertised `Cloudflare Workers + R2 + D1 + Neon Postgres`. Neon is not in the stack: `crates/corelink-container/Cargo.toml:18-25` records that the container-side `neon-real` feature and the production `TokioPgShadowSinkFactory` were **REMOVED** (#71), the analytics shadow was never wired in prod, and `/v1/audit/analytics/*` now reads D1 `customer_audit_events` (migration 0077) via `routes::audit_analytics::D1ShadowSinkFactory` (`crates/corelink-container/src/main.rs:464-489`). `DATABASE_URL` has **zero readers** across `crates/`, `worker/` and `apps/`. Corrected to `Durable Objects + Containers + R2 + D1`, matching the topology CLAUDE.md already states. **The comment was the more expensive half:** `crates/corelink-container/src/routes.rs:392-396` told the next reader that production swaps in a Neon factory "when the `neon-real` feature + per-region `NEON_DB_URL_<REGION>` env vars are present" — a build instruction for a code path that no longer exists, and it named a boot path (`apps/server/src/main.rs`) that **does not exist either**; the real one is `crates/corelink-container/src/main.rs`. Rewritten to describe the D1-backed factory and its `from_env` gate, with an explicit `RETIRED (#71)` clause so nobody re-derives the dead design. **Two adjacent comments carried the same dead claim and are corrected with it** — `routes.rs:401-404` (the `build_with_factory` doc, which would otherwise contradict the comment three lines above it) and the `build_with_factory_accepts_in_memory_factory` test comment. **Nothing was removed:** `crates/corelink-audit-chain/src/neon_shadow.rs`, the audit-chain `neon-real` test harness, and `SLO_REPLICATION_LAG_NEON_SECONDS` (`crates/corelink-replication-coordinator/src/lag.rs:33`) are a *deferred architecture* and are untouched — this corrects claims, it does not retire a design. **(2) Dead hostnames.** Production hostnames are flat (`corelink-api`/`corelink-get`/`corelink-docs.humangr.com`); the dotted form is dead. **All 12 dotted hosts were measured, not assumed** — `corelink`, `docs`, `signup`, `app`, `cache`, `api`, `legal`, `grafana`, `releases`, `admin`, `pilot` return **NXDOMAIN** (`curl` code `000`). The one exception is instructive: `status.corelink.humangr.com` **does** have DNS (CNAME → `hugrl.betteruptime.com`) but the custom domain's TLS cert was never provisioned, so every client dies on `sslv3 alert handshake failure` — a half-finished migration is indistinguishable from a dead host to a customer. **82 of the 117** dotted references in the 30 non-playbook files (29 files changed) were repointed to targets whose HTTP code was measured this pass: trust/pricing/blog/quickstart → `corelink-docs.humangr.com/*` (200), the pilot CTA → `corelink-docs.humangr.com/pilot/apply` (200), DPA → `humangr.com/corelink/en/legal/dpa` (200), status → `hugrl.betteruptime.com` (200, titled "Human Guardrail / CoreLink status"). **Status codes alone are a trap here and the sweep was built around that:** `humangr.com/*` and `humangr.com/corelink/*` are **both soft-404s** — `humangr.com/zzz-nope` returns 200 with the marketing page and `humangr.com/corelink/zzz-nope` returns 200 with the admin-ui SPA shell — so every replacement was confirmed by response *length/content*, not by `200`. The same trap caught a would-be regression on BetterStack, where only `/incidents.rss` is a real feed (`application/rss+xml`) while `/history.rss`, `/history.atom` and `/rss` all soft-404 as HTML. **The remaining 35 are deliberately LEFT and reported rather than guessed**, because the *page* does not exist at any host: `/calculator`, `/press`, `/support`, `/security/hall-of-fame`, `/blog/incidents`, the five per-post blog permalinks, `/trust/{sbom,signing-keys,formal-verification}` (the live trust hub publishes none of them — checked), the nine `docs.*` deep links that 404 on the real docs site (`/slo`, `/trust/{schrems-ii,byok-providers,residency,proof-bundle,erasure-attestation,audit-chain}`, `/runbooks/byok-kill-switch`, `/architecture/remote-cache-product-profile`), and the internal-only `grafana.*`/`releases.*`/`pilot.*`. Swapping a dead host onto a dead path produces a URL that still 404s and *looks* fixed. **The four `STATUSPAGE-*.md`/`STATUS-PAGE-SPEC.md` provisioning playbooks (a further 41 dotted references, leaving 76 in `marketing/**` overall) are also untouched on purpose:** they self-declare their URLs as placeholders ("status page URLs in this playbook are placeholders until DevOps confirms account creation"), and they describe provisioning **Atlassian Statuspage** — which is what `crates/corelink-statuspage-real` integrates (`api.statuspage.io`) — while the status page actually serving customers today is **BetterStack**. That vendor split is a real finding, but resolving it is an ops decision, not a hostname substitution.
- **fix(marketing): the lighthouse playbook offered Bazel BES ingest that has no endpoint, and six sales references pointed at a docs hostname that is NXDOMAIN — one of them at a page that 404s even on the live site.** `marketing/lighthouse-kit/CUSTOMER-PLAYBOOK.md` listed "point Bazel's `--bes_backend` at our endpoint" as one of two live mirror patterns. `bes_backend` appears nowhere in `crates/` or `worker/`, and the Build Event Service is a **gRPC** protocol — so it falls in the same bucket as Buck2 and Pants: not a missing feature, a property of a Workers edge that has no HTTP trailers and therefore no gRPC. The playbook now says so and defers to the one wired pattern (`corelink ci mirror`), which also stops it contradicting this repo's own sales material (`OBJECTION-HANDLING.md` Obj-23 and `FAQ-MASTER.md` M5 both call BES ingest roadmap). Dotted-host sweep finished across the rest of the territory, each target probed before it was written: the docs redirect target **does** serve real documentation (`corelink-docs.humangr.com` 301 → `https://humangr.com/corelink/docs/`, 200, Docusaurus 3.10.2, ~2.6 K chars of rendered nav + index — not a blank shell), so `EVIDENCE-PACK-INDEX.md`'s PUBLIC legend was repinned to it and `RATE-LIMIT-FAQ.md`'s prose link now uses `…/docs/explanation/rate-limits` (200, ~8 K chars). But `/slo` — cited five times as the "SLO catalog" — **404s on the live site and has no source page anywhere in `apps/docs/`**, so those five now cite the real artifact (`specs/03_architecture/slo_catalog.md`, internal) and the evidence-pack row that classed a public SLO summary as PUBLIC is marked not-published. Two things were deliberately **not** repinned: (a) the 429 sample body in `RATE-LIMIT-FAQ.md` keeps its dead `tier_upgrade_url` / `docs_url` because those strings are frozen canonical constants in the service (`crates/corelink-rate-headers/src/headers.rs:164,173`, pinned by `INV-BODY-FROZEN-URLS` in `tests/prop_rate_headers.rs:969-971`) — the sample is byte-accurate to the wire and editing it would create the drift, so it now carries a warning naming the live equivalents and says the fix belongs in the constants; (b) the `grafana.corelink.humangr.com` embed in `HEALTH-DASHBOARD-SPEC.md`, because no hosted CoreLink Grafana exists at all (the repo has `dashboards/grafana/*.json` but no instance; Grafana Cloud is only a customer-side export target) — the spec now states the host is unprovisioned instead of naming a new one. One further finding surfaced and was flagged, not silently fixed: the evidence pack's **OPEN-SOURCE** access class (18 rows) points at `HumanGuardrail/corelink`, a repo that does not exist (API: could not resolve; `/releases` 404) — which artifacts actually go public is an owner/Legal decision, so the legend now warns that every OPEN-SOURCE row must be treated as NDA until that repo exists.
- **fix(marketing): the lighthouse playbook told Buck2 and Pants customers to configure build tools that cannot connect to CoreLink at all, over hostnames that do not resolve.** `marketing/lighthouse-kit/CUSTOMER-PLAYBOOK.md` gave Buck2 users `[buck2_re_client] action_cache_address` "to our HTTP endpoint" and Pants users `grpc://cache.corelink.humangr.com:443/{slot-id}`. Both clients speak **gRPC only** — Buck2 has no plain-HTTP cache backend (upstream request closed `wontfix`, facebook/buck2#459) and Pants' `reapi` provider accepts only `grpc://`/`grpcs://` — while CoreLink has **no gRPC ingress**: the edge is Cloudflare Workers, workerd implements no HTTP trailers, and native gRPC cannot work without them. That is a topology fact, not a gap awaiting a sprint, so both entries now say plainly that the tool is not supported and why, with no date and no invented endpoint — the position `apps/docs/docs/tutorial/04-buck2-quickstart.mdx` already took. Bazel stays, because plain HTTP genuinely works, and the contrast is the honest story. Separately, every dead **dotted** hostname in the two files was repinned against live probes (the flat scheme is canonical — `docs/operator/host-scheme-canonical-2026-06-09.md`): the Bazel mirror snippet's `cache.corelink.humangr.com/v1/{slot-id}` (NXDOMAIN, and a path shape that never existed) → `https://corelink-api.humangr.com/bazel/cache` (`/health` 200), with a note that Bazel honors only the last `--remote_cache` so that line is the cutover, not a second destination; the weekly-report sample's `grafana.corelink.humangr.com` (NXDOMAIN, no hosted per-customer Grafana exists) → the console page that does, `https://humangr.com/corelink/en/customer/usage` (307 → sign-in, i.e. wired + auth-gated); and the cancellation receipt URL `app.corelink.humangr.com` (NXDOMAIN — that Pages custom domain is gone; `docs/operator/corelink-app-domain-flip-runbook.md`) → `https://humangr.com/corelink/...`. One dead host was **not** swapped, deliberately: the pre-flight checklist's `curl -sSL https://corelink.humangr.com/install.sh | sh` — the hostname is NXDOMAIN and no replacement was found that actually serves a script (`https://humangr.com/corelink/install.sh` 307s to sign-in and returns HTML; `https://humangr.com/install.sh` returns the site's HTML), so the checklist now says the CS engineer supplies the command rather than printing a guess a customer would pipe into `sh`.
- **fix(marketing): sales collateral cited a `corelink-bazel-remote-shim` that has never existed, and an export-receipt audit event that does not exist either.** Obj-24 of `marketing/sales/OBJECTION-HANDLING.md`, M1 of `marketing/sales/FAQ-MASTER.md` and `marketing/lighthouse-kit/CUSTOMER-PLAYBOOK.md` each offered a prospect a "drop-in front (you can read it before deploying it)". No such crate, tool or repo exists anywhere in the tree. The capability does — under another name and another shape: `crates/corelink-bazel-bridge`, a REAPI-v2 REST translation layer compiled **into** the server (`crates/corelink-container/Cargo.toml:182`), so it is neither something the customer deploys nor something they can read (`corelink-server` is a private repo). The honest rebuttal is stronger and is now the copy: there is **no CoreLink component in the customer's build path at all** — integration is `--remote_cache=https://corelink-api.humangr.com/bazel/cache` (stock plain-HTTP) or `/bazel/v2` (REAPI/ByteStream) plus a Bearer-PAT header (`apps/docs/docs/integrations/bazel.md`), and mirror-then-cutover keeps the incumbent cache primary, so rollback is one config flip. The same Obj-24 bullet's `apps/docs/docs/how-to/migrate/from-bazel-remote/` and M2's `.../from-s3/` pointers were directories that do not exist; they now name the real files (`from-bazel-remote-cache.mdx`, `from-s3-only.mdx`). `marketing/retention/CANCELLATION-FLOW.md` §5 separately promised a `corelink.cas.exported` audit event as "the receipt of what they took", a docs page at `apps/docs/docs/cli/cas-export`, and worked examples for three export destinations — written as requirements ("must be") but read as shipped. Verified: the event exists nowhere (`corelink.cas.*` slugs are enumerated in `crates/corelink-handler-cas/src/audit.rs`), the page never existed, and only the **local directory** destination is wired (any remote URI hard-errors: "only a local directory is wired today … (flagged gap)" — `tools/cli/src/commands/cas.rs`). The section is kept as a requirements list but each item now carries an explicit MET / PARTIALLY MET / OPEN status, and the audit bullet names what an export actually leaves in the chain (`corelink.cas.list.attempted`, `corelink.cas.read.attempted` / `.served`, emitted per blob because `cas export` is a client-side page-and-download loop). Finally, the "operator-assisted" export/import copy is legitimate — it is a service, so its absence from the CLI proves nothing — but the **procedure is unwritten**: the string `operator-assisted` appears nowhere outside `marketing/`, and neither `specs/_runbooks/` nor `docs/operator/` documents a direct-to-customer-bucket export or a bulk import. Those two claims now carry an internal note flagging the runbook gap (a service sold with no runbook is an operational risk, not a copy problem); the third such claim, GDPR erasure, IS grounded and now cites its procedure (`specs/_runbooks/RB-DSR-GDPR.md` §2.3). `python3 scripts/validate_docs_reality.py` stays exit 0 (305 CLI refs, 0 drift).
- **fix(ci): the `admin-ui e2e` gate observed its own root cause at second 6 of a 20-minute job and reported it as success — `warm-routes.ts` counted a `404` as "compiled", so a dev server that had stopped serving routes produced 20 minutes of red that blamed twelve innocent specs.** `apps/admin-ui/tests/e2e/warm-routes.ts` is the `globalSetup` that pre-compiles the 11 routes the critical-flow suite visits, and it treated **any** status as proof of compilation ("Any status is fine — a redirect or a 403 still means the route COMPILED"). That is correct for 3xx and 403 and **wrong for 404**, which means Next never resolved the route at all. On run **30769355309**, job **91553642199** (firefox), it printed `[warm] 200 /sign-in`, `[warm] 200 /en/customer`, then **`404` for the other 9 routes ~50 ms apart** — and closed with `[warm] 11 routes compiled in 6445ms`. **The correlation with the verdict is exact: `1 passed, 12 failed`, and the single passing spec (`tests/e2e/customer/customer-overview.spec.ts`) is the only one whose route was in the `200` column.** The other twelve died on 30 s locator timeouts, three attempts each, against `not-found.tsx`: `auth.spec.ts`'s `error-context.md` snapshot for `/en/admin/tenants` is literally `- heading "404 — Page not found" [level=1]`. **The dev-server collapse is permanent, not a startup race** — that same route was still 404 two minutes and three spec retries later — which is what licenses polling in the fix below: it can distinguish "still compiling" from "will never resolve", but it cannot turn a genuine failure green. **The `ClerkJS: … We were unable to attribute this request to an instance` error that accompanies these runs is a red herring and the fix says so in the code**, because it is the obvious thing to chase and it is backwards: it is emitted by the deliberate dummy publishable key at `apps/admin-ui/src/app/[locale]/(authenticated)/layout.tsx:39` (`pk_test_…` encoding the placeholder host `example.clerk.accounts.dev`, which by design never reaches a Clerk backend), and it appears **15× in the PASSING chromium job of that same run (91553642200) and 0× in the FAILING firefox job** — it is a symptom of the authenticated layout mounting, i.e. of the app *rendering*. Likewise the "rotating browser" signature is a job-level artefact, not a browser bug: across the last 5 runs on `main` exactly one matrix leg fails per run and it rotates (firefox → chromium → webkit → firefox → webkit), which is what a per-job dev-server fault looks like. **Fix, in two parts, neither of which relaxes anything.** (1) `warm-routes.ts` now polls each route (4 attempts, 3 s backoff, 120 s per attempt) and treats a route that stays `404` as **fatal** — `globalSetup` throws, so the suite does not run, and CI gets an `::error::` naming the exact routes and statuses plus the instruction not to chase the Clerk warning. **Measured against a stub server, both directions: healthy → 11×200 in 265 ms, no throw; two routes 404 → `UNRESOLVED`, the `::error::`, and a throw in 18 s** — versus 20 minutes of misattributed red today. A 404 from these paths is always a harness fault: every one exists on disk under `src/app/[locale]/(authenticated)/`, none calls `notFound()`, and all 11 return 200 in the passing chromium and webkit jobs of the same run. (2) `playwright.tests.config.ts` sets `webServer.stdout: "pipe"`. Playwright's **default is `stdout: "ignore"`**, and Next prints its route-compilation log, its readiness line and every turbopack diagnostic to stdout — so for the entire life of this suite the only stream that could have explained a dev-server failure was being discarded, leaving CI with a middleware-deprecation warning and forwarded browser console noise. That is why this has been unexplained since `ed5c0f51`. **Explicitly NOT fixed, and the lane is not green because of it: nobody yet knows WHY Next 16.2.11's turbopack dev server stops resolving App Router routes ~12 s after boot on a 2-core hosted runner.** That answer is in the stdout this change starts capturing; the next occurrence is the measurement. This entry is the direct continuation of the artifact-capture fix in #937, which recorded the same limit — *"this change does not fix the flake; it makes the next occurrence explicable instead of forcing a guess."* Now the harness also refuses to pretend the flake is the specs' fault. `tsc --noEmit` clean; no spec was skipped, no retry added to a spec, no assertion loosened, and the job is not `continue-on-error`. **Continuation, same PR — with the warm-up honest, eleven of the twelve blamed specs pass and the ONE real failure the masking had hidden is fixed here: `tests/e2e/customer/customer-team.spec.ts` read the invite response as a BARE member.** `POST /v1/customer/team/invite` replies `201 { "member": { … } }` — the row is ENVELOPED (`crates/corelink-container/src/routes/customer.rs:960-968`, asserted server-side at `:2062` and mirrored by the E2E fixture at `src/lib/e2e-mock-fixtures.ts:751`) — but the spec cast the parsed body to the flat `{ user_id: string }`. A TypeScript cast only re-labels the value, so this compiled clean and handed the locator `undefined`: the assertion silently became `getByTestId("team-row-undefined")`, which can never match anything, and it failed the way a missing row fails. **The product was never broken** — `CustomerClient.inviteTeam` unwraps the envelope correctly (`src/lib/customer-client.ts:203-210`), and `TeamClient` refetches after a successful invite and renders the new `status: "invited"` seat as `team-row-<user_id>` in the "Pending invitations" card — which is why line 36 (the success banner, fed by the correctly-unwrapped `member.email`) PASSED in the same run while line 37 timed out. This is the **third** instance of the identical envelope-cast defect on this surface (the two prior ones shipped "Invite sent to undefined" and "Erasure requested (undefined)" to real customers, and both are documented in `customer-client.ts`); the first two lived in the app, this one lived in the gate that was supposed to catch them. The spec now unwraps `member.user_id`, asserts it is truthy *before* interpolating it into a testid (so a future envelope change fails on the response shape, naming the cause, instead of on an unmatched locator), and additionally asserts the seat lands in the `team-pending-list` with status `Invited` — the behaviour the product actually promises, since an invited seat is not yet an active member. **Mutation-proven both directions on a real chromium run:** with the fix `1 passed`; with the fix reverted and nothing else changed, exit **1** with `Locator: getByTestId('team-row-undefined')` — the CI failure reproduced verbatim. No timeout was widened, no `waitForTimeout`, no retry, no `test.skip`. **Still open, found while reading this surface and deliberately NOT fixed here (out of scope for a single-spec fix):** the CSP header emits `report-uri /api/csp-report` (`src/lib/csp.ts:112`) as a basePath-LESS absolute path, while the handler that would receive it lives under the app's `/corelink` basePath (`src/app/api/csp-report/route.ts`) — so every violation report `404`s (four per team-page visit in the run above) and the CSP telemetry this app believes it collects is being discarded. **Continuation, same PR — the piped stdout paid for itself on its FIRST occurrence, and the "why" the entry above left explicitly open is now answered: turbopack's dev route tree comes back SHORT, and the pages are fine in production.** Run **30781415861** failed the new fatal-404 assertion on exactly three routes — `/en/customer/audit/visualization`, `/en/admin/ops/op_byok_001`, `/en/admin/ops/op_dsr_001` — and the `[WebServer]` lines the previous change started capturing settle it in three strokes. **(1) It is not a collapse.** `/en/customer/billing`, `/keys`, `/team`, `/en/admin/audit` and `/en/admin/tenants` all returned **200 AFTER** the first 404, so the "the dev server dies ~12 s after boot" reading of run 30769355309 is **REFUTED** — the failure is per-route, not time-based. **(2) The page is never compiled.** `GET /en/customer/audit/visualization 404 in 589ms (next.js: 519ms)` then `146/84/66 ms (next.js: 15/13/4 ms)`: a 404 that costs single-digit milliseconds of Next time is a **route-tree MISS**, not a compile error — a broken module graph is a 500 with an overlay, and compiling these pages costs ~1.4 s. **(3) It is intermittent, turbopack-dev-only, and NOT a product bug.** The same three routes 404'd on run 30771910241 (a *different* branch) and served 200 on runs 30765324783 / 30773211630 (all three browsers) / 30773554607 / 30779501435 / **30781832643 — the same commit, ten minutes later**; and in the failing run's own sibling job, `next build` emitted both entries into the production route table (`ƒ /[locale]/customer/audit/visualization`, `ƒ /[locale]/admin/ops/[op_id]`, `✓ Compiled successfully in 20.0s`). **The shape of the miss is the tell:** every warmed route at `/[locale]/<a>/<b>` resolved and all three at `/[locale]/<a>/<b>/<c>` did not — the tree is short at its DEEPEST entries. Not reproducible on macOS: three fresh-`.next` `next dev` boots plus the identical 11-route sequence returned 11×200 every time (FSEvents vs the hosted runner's inotify is the obvious environmental difference, and is NOT claimed as proven). **Fix — the warm-up now MEASURES which of the two 404s it has instead of asserting it in a comment.** The previous version justified its fatality with prose ("Every entry MUST exist on disk … so a 404 here is a harness fault, never a product fault"); a comment is not a check, and two very different faults hide behind one status code. New `apps/admin-ui/tests/e2e/app-route-index.ts` resolves a URL against `src/app` with the App Router's own segment semantics — `(group)`/`@slot` consume no segment, `[p]` exactly one, `[...p]` one or more, `[[...p]]` zero or more, with backtracking (so `/en/admin/ops/op_byok_001` correctly falls past `ops/page.tsx` into `ops/[op_id]/page.tsx`). `warm-routes.ts` then branches: **no page file on disk ⇒ the warm list is stale — fatal immediately, no retry, no recovery**, naming the offending entries; **a page file DOES resolve it ⇒ the dev server lost the entry** — force a re-read of every directory Next had to walk to find it (a create+delete of a dotfile in each, the directory-entry event a watcher cannot miss, plus an mtime bump on the page itself), re-probe **once**, and fail hard if it is still missing. **Nothing was weakened:** the fatal-404 assertion stands, no route was removed from the warm list, no `test.skip`, no `continue-on-error`, and the recovery cannot mask a real defect — a path nothing serves stays 404 through the re-scan and fails, a broken module graph never 404s in the first place, and the phantom-route branch refuses to re-scan at all. **Proven in both directions in `apps/admin-ui/tests/warm-routes.test.ts` (10 tests):** all 11 warm entries are now filesystem-checked against `src/app` (a stale list fails here instead of in CI); and an end-to-end case runs the REAL `globalSetup` against a stub server that 404s `/en/customer/audit/visualization` until a filesystem watcher on that directory fires — proving both that the nudge produces a watcher-visible event and that the warm-up then completes (`[warm] 11 routes compiled in 12297ms`). **Mutation-proven:** with the recovery call removed and nothing else changed, `vitest run tests/warm-routes.test.ts` exits **1** (`2 failed | 8 passed`); restored, **0** (`10 passed`). `tsc --noEmit` and `eslint` clean. **Deliberately NOT done:** no `--webpack` switch for the E2E dev server (it would very likely dodge the bug — every upstream report of this class is turbopack-only — but it is a bet on an unproven mechanism and it makes the gate slower), and no upstream issue is filed yet because the reproduction is still statistical (~2 of the last 10 runs) rather than a minimal case.
- **feat(audit): CF-6 legacy-head re-sign convergence — a `POST /_internal/audit/drain` under `AUDIT_CHAIN_TRUST_UNSIGNED_RESUME` now stamps the current-key signature onto every genuinely UNSIGNED `audit_chain_head` IN PLACE, so the migration toggle can be turned back OFF without waiting weeks for organic per-partition traffic.** After the seed-fallback fix (below), turning CF-6 head-signing on flips every pre-existing NULL-signature head to fail-CLOSED on its next advance, so the `AUDIT_CHAIN_TRUST_UNSIGNED_RESUME` window must stay open while heads re-sign — but heads only re-sign when their partition seals a new row, and idle partitions (no pending `audit_outbox` rows) are never visited by the pending-partition drain loop, so convergence stalls (observed: 2/193 signed after the initial drain, leaving the tamper-tolerant window open indefinitely). New `resign_unsigned_head` reads a partition's checkpoint and, **only when `head_signature IS NULL`**, stamps the current-key Ed25519 signature over the SAME `(head_hash, next_sequence)` via a dedicated `UPDATE … WHERE … AND head_signature IS NULL` — **no chain mutation, no new row, atomic NULL→signed** (a concurrent drain advance or re-sign loses the guard, so no signature is ever overwritten). `handle_drain` scans `audit_chain_head` directly (`read_unsigned_head_partitions`, bounded by `batch_limit` per call so a large legacy backlog converges over repeated calls), re-signs each with a loud per-head `warn`, and returns `heads_resigned` in the summary. **Hardened per adversarial review:** scoped to NULL-signature heads ONLY — it deliberately does NOT re-sign foreign-key heads (that is a broader trust action an attacker could force by writing a non-current `signing_key_id`; key-rotation convergence is a separate, old-signature-verifying operation) and NEVER touches a head that already carries a signature (a current-key head with an invalid signature is left for `check_head_on_resume` to fail-CLOSE, not laundered). **Gated fail-safe:** the sweep runs ONLY while `trust_unsigned_resume` is ON *and* a seed is configured — outside the window a legacy NULL head stays TAMPER. It is not laundering-proof (under the window the current head state is the trust root — the same blindness the window already carries); its value is that converging fast lets the operator CLOSE the window sooner, minimising total exposure. Container-side only.
- **fix(audit): CF-6 audit-chain head-signing was silently OFF in prod — every `audit_chain_head` advanced UNSIGNED (`head_signature` NULL on all 193 partition heads) even though the Ed25519 seed is set and forwarded, because a forwarded empty string defeated the seed fallback.** `load_signing_seed` read the dedicated `AUDIT_CHAIN_SIGNING_SEED_HEX` with `std::env::var(...).ok()` and only fell back to the reused `ERASURE_ATTESTATION_SEED_HEX` via `.or_else`. The DO forward-list sends every key as `this.env.X ?? ""` (`worker/src/durable_object.ts`), so an UNSET dedicated secret arrives at the container as `""` — *present, empty* — and `std::env::var("AUDIT_CHAIN_SIGNING_SEED_HEX").ok()` returns `Some("")`, which short-circuits the `.or_else`. The `""` then fails the 64-hex length check and `load_signing_seed` returns `None`, so the drain advanced the head UNSIGNED (the legacy/tolerated path). Erasure attestation was unaffected because it reads its seed var directly, which is why the seam went unnoticed. Live-confirmed in prod: `SELECT COUNT(*) FROM audit_chain_head WHERE head_signature IS NOT NULL` = 0. **Fix:** resolve both the seed and the key-id through the codebase's standard empty-is-absent rule (extracted into pure `resolve_seed` / `resolve_key_id`, matching `storage::non_empty_env` used by every other dual-key reader, e.g. the OCI token key), so a forwarded `""` for the dedicated slot no longer masks the reused erasure-attestation seed/id. The same forwarded-`""`-masks-fallback trap is now regression-tested (`resolve_seed_empty_dedicated_falls_back_to_reused`, `resolve_key_id_empty_dedicated_falls_back_then_defaults`). Container-side only; takes effect on the next container image deploy, after which the drain re-signs each head on its next advance and the chain head becomes tamper-evident against a D1-writer as designed (CF-6). No new secret, no schema change.
- **fix(audit): `POST /_internal/audit/drain` read EVERY pending row of a partition (no `LIMIT`) and did one D1-HTTP `UPDATE` per row (~0.3s each), so a cold backlog exceeded the edge subrequest timeout and sealed ZERO — the tamper-evident audit chain silently stopped advancing and the daily verify failed every day.** With a large `audit_outbox` backlog (e.g. tens of thousands of unsealed events after a period where the drain cron was mis-configured), `read_pending_rows` (`crates/corelink-container/src/routes/audit_drain.rs`) selected the whole unsealed tail `ORDER BY enqueued_at, id` and `drain_partition` sealed each row with its own guarded `UPDATE`; at ~0.3s/write a single partition of thousands of rows could not finish within the request budget, the call was killed mid-sweep, and — because a killed call commits no head advance — the *next* call re-read the same backlog and died the same way. The chain never moved and R2 never received a sealed partition. **Fix:** `read_pending_rows` takes a `limit` and seals only the ordered PREFIX (`… ORDER BY enqueued_at, id LIMIT ?3`); `drain_partition` threads a per-partition `batch_limit`; `handle_drain` enforces a GLOBAL per-call row budget (`AUDIT_DRAIN_BATCH_LIMIT`, default 200 ≈ 60s at 0.3s/row, clamped ≥ 1) across all partitions and returns `incomplete: true` when the budget bounds the sweep, so a caller (the hourly cron, or a manual loop) re-drains until `incomplete` is `false`. **Correctness is unchanged because the chain was never transaction-atomic to begin with — it is crash-safe by construction:** `resolve_resume` already prefers the durable *sealed-tail* over the `audit_chain_head` checkpoint whenever the tail is ahead (a prior drain that sealed rows but died before advancing the head), and `advance_head_cas` is a conditional compare-and-set on the resumed `(head_hash, next_sequence)`. Sealing a shorter prefix is just a smaller, deterministic step of the same resume-from-tail machinery: `seal_rows` is a pure function of `(start_head, start_seq, rows)`, so a prefix-then-resume seal is byte-identical to a one-shot seal (new test `limit_prefix_seal_then_resume_equals_one_shot_seal`), two concurrent drains still compute byte-identical seals and one CAS wins while the other drifts without forking, and the `emitted_at IS NULL` row guard keeps every write idempotent. No new secret, no schema change; `AUDIT_DRAIN_BATCH_LIMIT` is optional and safe-defaulted (secrets-matrix row #189, non-secret tuning knob), and is forwarded through the `durable_object.ts` `container.start({env})` list + typed on the Worker `Env` so a Worker-side value actually reaches the container process (container env-contract parity) — an empty forward falls back to the 200 default.
...
- **fix(admin-ui): four customer screens read fields the server never sends, because `request<T>()` is a CAST and not a parse — and the team screen promised an invitation email that is never sent.** One root cause, four surfaced defects. `CustomerClient.request<T>()` applies `as T` to the parsed JSON, so a declared type that disagrees with the wire compiles clean and yields `undefined` at runtime instead of throwing. Four endpoints envelope their payload and the client declared the bare row: `POST /v1/customer/keys` replies `201 { pat, token }` (`routes/customer.rs:810-820`) — the create toast rendered **`Token "undefined" created`** and fed that same undefined metadata to the shown-once reveal; `POST /v1/customer/team/invite` replies `201 { member }` (`customer.rs:961`) — the team screen rendered **`Invite sent to undefined`** and **`Invited undefined.`** in production (the shipped copy — `Seat reserved for` is this PR's replacement wording, not the symptom); `POST /v1/customer/keys/:id/revoke` replies `200 { pat }` (`customer.rs:860-870`), latent only because both call sites discard the result; and `POST /v1/customer/account/delete` acks `202 { ok, status }` with `status ∈ {erasure_requested, no_account}` (`customer.rs:1082-1093`) — there is **no request id on that wire at all**, so the settings screen was rendering a field the server has never sent. **Why no gate caught it — and where it really started:** the contract of record (`specs/_audits/sealed/2026-05-15-customer-dashboard-spec.md:80-83`) declares all three POST responses as FLAT. The client was implemented **faithfully against that table**; the container has always enveloped them. So the unit stub and the E2E fixture were not sloppy — they were consistent with the spec, and the whole closed loop of spec + client + both mocks agreed with each other and disagreed only with the server. Cold review caught this; the spec is corrected here too, because leaving it would have the next implementer reproduce the defect exactly — the same defect-pinning pattern as the installer test that required `corelink ping` to be present. Fixtures are realigned to the real wire and `tests/customer-wire-shape-ui.test.tsx` now asserts the rendered output against enveloped responses, so a future envelope drift fails at the UI, not in production. **Separately, the copy was false:** the team screen said *"They'll get an email invitation"* and *"We've emailed them an invitation"*, but `customer_d1.rs::invite` writes a `team_member` row keyed by a hash of the normalised email and makes **NO synchronous Clerk call (OB-1)** — no email is sent, and the seat binds only when the invitee signs up with that exact address. The copy now says what the system does ("Reserves a seat right away. CoreLink doesn't email them — ask your teammate to sign up with this exact address"), and the W5b clause tells the operator to note the address down, since only a hash is stored. Shipping the email itself is a separate decision, deliberately not taken here.
...
- **fix(docs+marketing): 51 more documented `corelink` invocations that do not exist, plus the commercial claims underneath them.** Tail of a five-pass sweep. **17 phantoms in `marketing/**`** across 9 files and **34 in the docs**, found by scanning code blocks and inline backticks only — a naive prose scan over marketing returns **~72% false positives** ("corelink wins", "corelink mirror caches"), worse than the ~40% in docs. Two had escaped every prior pass for structural reasons: `corelink lighthouse export-case-study` (a wholly invented verb with invented `--slot/--format/--sanitized` flags and invented sanitization semantics) and `corelink push`, which lives in `marketing/og/corelink-cli.svg` — **a file type outside the scanned extensions**. **Nested phantoms are a distinct class**: `corelink audit prove` and `corelink cas put` both pair a REAL top-level verb with a subcommand that does not exist, so any check that validates only the top-level verb passes them. Bound the enum parse by brace depth or the nested enums are swallowed as if they were top-level. Also corrected **21 non-phantom defects of the same family**: `corelink get … --output <FILE>` **×19** — the first cold review of this PR found a 6th instance and a re-count found 19, across `apps/docs` (docs, the 2026-05-27 blog post, and all three translations) plus the repo-root `README.md`. Two of those could never have been caught by the gate at all: `README.md` sits outside `DOC_ROOTS`, and the blog post is published customer copy. One instance is deliberately LEFT — `specs/_audits/2026-05-27-deep-prep-inputs.md:878` is a dated audit record, and rewriting a record of what was true on its date is a different defect (`--output` is the global *format* flag; the file flag is `--out`/`-o`, so every one of those commands would have errored), a fabricated `COR_AUTH_TENANT_FORBIDDEN` block with a `--tenant` flag `stat` does not have, and `corelink ci mirror --from bazel-remote` ×1 which hard-errors by design (`commands/ci.rs:30`). **Understatement is a defect too:** five places called SHIPPED features "on the roadmap" (`cas export` ×3, `import`, `ci mirror`) — corrected to "ships today", scoped to the wired subset, since `s3://` destinations hard-error (`commands/cas.rs:129`). **The drift gate that should have caught all of this was muted.** `validate_docs_reality.py` is genuinely PR-gated and reported `0 drift` — because 19 suppressions in `scripts/docs_reality_allowlist.json` silenced it, each with a hand-written, *accurate* justification. The `ping` entry read "the shipped health/connectivity check is `corelink doctor`": a correct diagnosis of a command that was, at that moment, the final line of the **public install one-liner**, making every customer install exit non-zero. Worse, `install.ts` is `.ts` — **outside the gate's scanned extensions entirely** — so the gate could never see the shipping copy, and the allowlist muted the only echo it could see. `0 drift` was technically honest and operationally false. Root cause was written policy: the allowlist's own README said *"a stale entry is harmless (it simply stops matching)"*. Replaced with explicit rules, plus a new non-fatal **`[suppression-hygiene]`** check that warns on any suppression matching nothing — it immediately surfaced **12 dead mutes that were previously invisible**, 11 of which remain armed. 8 provably-dead entries were retired into a `cli._retired[]` record (key, bucket, date, verbatim old reason, verdict) rather than deleted silently; one of them, `ci`, asserted a command was unbuilt roadmap **after it shipped**, proving nobody re-reads these. Gate before: `297 CLI refs, 19 tracked, 6 warnings, 0 drift`. After: `303 CLI refs, 0 tracked, 17 warnings, 0 drift`. No suppression was widened and none was added.
...
### Changed
- **chore(ci): park `fuzz-nightly`'s schedule — 0 successes in 30 scheduled runs, ~2 h/day of the founder's Mac burned for no signal.** A cost audit measured every scheduled workflow's real success rate and minutes: across all lanes the self-hosted Mac fleet was spending **8.8 h/day** on scheduled jobs that fail (790 min failure + 268 min cancelled vs 354 min success in 48 h — **75% of scheduled Mac CI produced nothing**), and this lane was the third-largest contributor at 238 min per 48 h. The Mac is the founder's own machine and heavy compiles have crashed it before (`CLAUDE.md`), so this is not free CI. **The failure is structural, not flaky:** every run dies in the fuzz BUILD with `clang++: error: no input files` / `error occurred in cc-rs` / `failed to build fuzz script` — libfuzzer's sanitizer build cannot link on this host, and re-running cannot change that. **The lead is in the file's own header:** it still says *"on a fresh Ubuntu runner"* while `runs-on` is the self-hosted Mac pool — the lane was written for Ubuntu and moved later, and the sanitizer toolchain came along in name only. This PR parks rather than migrates because the move is genuinely not one line: the `ROOT FIX (2026-06-15)` step deliberately uses the PRE-INSTALLED host nightly instead of `dtolnay/rust-toolchain@nightly`, after installing nightly on the shared Mac once deleted the rustup shim and broke every Rust gate on the host for a day; `ubuntu-latest` has no pre-installed nightly, so that step must be rewritten as part of the move and the result PROVEN with a `workflow_dispatch` run before the schedule returns (the same discipline applied to `reproducible-build`). The re-arming checklist is written into the workflow. **Scope discipline:** 22 scheduled workflows are at 0/30, but only this one was parked — the audit's rule was "kill only what I am CERTAIN is not working", and checking each one first was load-bearing: `nightly` fails on a *TLC pin violation* that #945 fixed 12 h AFTER that run (so it is likely already green and was left alone), and `corelink-worker`'s 4.7 h cargo-mutants job produced no error line at all, so its cause is unknown and it was left alone too. Killing either would have destroyed working or already-repaired signal.
- **chore(ci): drop the daily cron from 14 workflows that ALREADY have a `pull_request` trigger — ~3.8 h/day of the founder's Mac, removed with zero signal lost.** A cost audit measured what the scheduled lanes actually do: the self-hosted Mac fleet burns **8.8 h/day** on scheduled jobs (790 min failure + 268 min cancelled vs 354 min success over 48 h — **75% produced nothing**), and — the decisive number — **10 of the previous 14 days had ZERO commits to `main`**. On those 10 nights the machine spent hours re-verifying **byte-identical** code that had already been verified. The Mac is the founder's own machine and heavy compiles have crashed it (`CLAUDE.md`), so this is not free CI. **The rule this establishes: a cron earns its keep only when something can change WITHOUT a commit.** CVE feeds, prod state, backups, cert/infra drift, billing reconciliation and DR drills genuinely change overnight — every one of those keeps its schedule and is untouched here. Code-only checks do not: `cargo-mutants` at 03:00 on a day nobody committed cannot discover anything that was not equally true yesterday. **Why these 14 specifically, and why it is safe:** each already carries a `pull_request` trigger, so the code IS verified the moment it changes — the cron was pure duplication of a check that had already run. Removed from `tla_check`, `region_pinning`, `cargo-deny`, `s07/s08/s09-ship-gate`, `corelink-worker`, `corelink-meta`, `corelink-hash`, `corelink-reapi`, `corelink-client-verify`, `corelink-adapter-host`, `tenant-path`, `corelink-server`. The single largest daily burner, `corelink-worker` (352 min per 48 h, 0/30 successes), is in this set. **Deliberately NOT touched:** the 9 heavy gates with NO PR/push lane (`coverage`, `mutation-nightly`, `reproducible-build`, `ffi-matrix`, `cas_foundation`, `sbom`, `perf-nightly`, `s10-ship-gate`, `compliance-weekly`) — for those the cron is the ONLY thing that runs them, so removing it would mean they never run at all. Converting THEM needs a `push: main` trigger added first, which is the tracked follow-up. **The schedule blocks are commented out, not deleted**, each with the reasoning and the original cron line intact, so re-arming is uncommenting. **Verified mechanically:** all 14 still parse, all 14 still hold their `pull_request` trigger, none lost its `schedule` by accident, and no workflow in the repo was left with no trigger at all. **Also corrected `CLAUDE.md`**, which asserted these heavy gates have "no `pull_request` and no `push` lane on any of them" — true when written, false since `tla_check` gained a PR trigger in #947, and false for 13 others. The doc now carries the cron rule explicitly so the next person applies it instead of re-deriving it.
