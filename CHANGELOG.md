...
- **fix(deps): clear all 10 trivy HIGHs — Next.js 16.2.10 → 16.2.11 (auth bypass CVE-2026-64642, SSRF CVE-2026-64645/CVE-2026-64649, DoS CVE-2026-64641 — the PROD admin-ui framework, not dev tooling) + 4 transitive floors via `pnpm.overrides`.** The `trivy fs` PR gate went red again on 2026-08-01 with NEW findings (the prior chronic red was root-fixed in #896 — a red trivy now means a real finding, and this one was: an authentication-bypass + two SSRF advisories in the exact Next.js line `apps/admin-ui` ships to prod behind `humangr.com/corelink`). Direct bump: `next` 16.2.10 → 16.2.11 (upstream patch release). Transitive floors added/raised in root `pnpm.overrides`, each targeting every resolved major line per the #896 lesson (floor = the actual fix version, never one short): `brace-expansion` → ≥1.1.17/≥2.1.3/≥5.0.8 (CVE-2026-14257 DoS; raises the three existing #896 floors), `fast-uri` → ≥3.1.3 (CVE-2026-13676 security-policy bypass + CVE-2026-16221), `sharp` → ≥0.35.0 (GHSA-f88m-g3jw-g9cj, inherited libvips CVEs), `svgo` → ≥3.3.4 (GHSA-2p49-hgcm-8545, removeScripts leaves executable scripts). `pnpm install --lockfile-only` regen; resolved lines verified in `pnpm-lock.yaml` (next 16.2.11, brace-expansion 1.1.18/2.1.4/5.0.9, fast-uri 3.1.5, sharp 0.35.3, svgo 3.3.4). admin-ui vitest re-run against the bumped framework. Ships on the next admin-ui deploy (auto on `main`); no Rust/container change.
