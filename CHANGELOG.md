...
- **fix(testing): the E2E ship-gate can no longer print GREEN by vacuum, and a deny-probe no longer scores a 404 as a security PASS (finding H18).** Two rigor defects in `tests/e2e-user-journeys`. **(a) Green-by-vacuum:** the runner exited 0 (GREEN/SHIP) whenever `fail == 0`, so an all-gated / zero-assertion run — one that provisioned no tokens and asserted nothing positive — printed GREEN. Fix: the verdict is now a pure, unit-tested `ship_verdict(pass, fail, gated, min_pass, max_gated)` with an anti-vacuum floor — `pass < min_pass` (default 1) is RED and exits non-zero ("NO POSITIVE ASSERTIONS — this is NOT a green"), plus an optional Pass→Gated ceiling (`CORELINK_E2E_MAX_GATED`). **(b) 404-as-deny:** deny-probes routed through `expect_denied`, which accepted `401|403|404` — so a `404` (route unmounted / renamed / typo'd) scored as "correctly denied", masking a real gap as security. Fix: a new `expect_gate_denied` accepts ONLY `401`/`403` for genuine authorization gates (a 404 is a FAILURE naming the "route absent, gate never ran" reason); the gate call-sites (billing/edge/security/team) now use it, while `expect_denied` stays deliberately 404-tolerant for content-isolation READS (a cross-tenant read legitimately 404s to hide existence). Regression-locked: `expect_gate_denied` rejects 404 + accepts only 401/403, `expect_denied` stays 404-tolerant for isolation reads, and `ship_verdict` is RED on a zero-PASS vacuum, on a real failure, and on a gated-over-ceiling regression (GREEN only at/above the floor with no failures).
