...
### Fixed
- **fix(deps): clear the 10 HIGH CVEs that kept `trivy fs` RED on `main` and on EVERY PR — the chronic red that was being `--admin`'d past (e.g. #885).** Four packages, all with a published fix: `brace-expansion` 1.1.16 (CVE-2026-14257, ReDoS), `fast-uri` 3.1.2 (CVE-2026-13676, security-policy bypass via improper parsing), `next` 16.2.10 (CVE-2026-64641) and `sharp` 0.34.5 (GHSA-f88m-g3jw-g9cj) and — surfaced by the resulting re-resolve — `svgo` 3.3.3 (GHSA-2p49-hgcm-8545, `removeScripts` leaves some executable scripts intact). Three are transitive and are pinned via the existing `pnpm.overrides` ladder — the stale `brace-expansion` floors were raised (`<1.1.16`→`<1.1.17`, `<2.1.2`→`<2.1.3`, `<5.0.7`→`<5.0.8`) and `fast-uri`/`sharp` floors added; `next` is a DIRECT `apps/admin-ui` dependency and was bumped 16.2.10 → 16.2.11 (patch). Lockfile resolves to `brace-expansion` 1.1.18/2.1.4/5.0.9, `fast-uri` 3.1.5, `sharp` 0.35.3, `next` 16.2.11, `svgo` 3.3.4 — all at or above the fixed version. **Why it mattered beyond the CVEs:** `trivy fs` is a required PR check, so a permanently-red scanner meant every merge had to either wave it through or burn a documented `--admin` — which trains the fleet to ignore the one gate whose whole job is to shout. All four are dev/build-tool or framework-patch level with no established prod exploit path; the value here is restoring a *believable* gate. No runtime behaviour change beyond the Next patch bump.
