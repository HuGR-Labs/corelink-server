...
- **chore(ci): CI cost diet — cut GitHub-hosted Actions minutes ~60-65% without deleting a single quality gate.** The repo moved to the `HuGR-Labs` org on a **Free plan (2,000 hosted min/month)** while the measured June 2026 burn was **13,758 min** (~7x the allowance), so the paid-minute budget — not the Mac fleet — is now the scarce resource. Four levers, no gate removed: **(1) runner** — 27 jobs whose entire body is `python3`/`bash`/`git` (dco-check, changelog-validate, spec_validation, canonical-consistency, docs-reality, dashboard_validation, action-sha-audit, actionlint, proptest-density-gate, slo-instrumentation, quickstart-validate, api-deprecation-check, the 4 light `region_pinning` legs, and the parse-smoke + aggregate legs of s07/s08/s09/s10/gc) move to the idle self-hosted mac fleet, where they cost $0. Hosted minutes bill with a **1-minute-per-job floor**, so a 15-second check was costing a full paid minute on every PR — `dco-check` + `changelog-validate` alone are ~1,400 paid min/month for ~6 minutes of real work. Two of these moves (`actionlint` and `mutation-pr`) merely **restore the runner the workflow's own header already documents** — `actionlint`'s step comment reads "Native actionlint on the macOS self-hosted fleet… the fleet ships actionlint on PATH", and `mutation-pr` explicitly reads "Runner: self-hosted Mac on purpose — cargo-mutants + cargo-nextest are already installed there and the build cache is warm", yet `runs-on` had drifted to hosted, making it simultaneously the most expensive PR gate (~1,140 min/month) and a slower one (cold cache). Every moved script was executed natively on the fleet host (macOS/BSD userland, system `python3` 3.14, `git` 2.51, `bash` 5.3, `actionlint` 1.7.12 on PATH) before the move. `secrets-drift` is SPLIT by event on purpose: its PR/push lane moves to the fleet, but its **daily SOC 2 CC6.1 evidence cron stays GitHub-hosted**, because a scheduled compliance run that silently does not fire on an offline runner is an invisible evidence gap (a PR gate that does not fire is a visible pending check — a fleet outage can never pass silently). **(2) frequency** — the heavy nightlies that were moved OFF per-PR on 2026-06-02 go daily→weekly on staggered days (`codeql` Mon, `coverage` Tue, `cas_foundation` Wed, `ffi-matrix-ci` Thu, `docs-ci` Mon, `lighthouse-ci` Tue, `smoke-install` Wed, `admin-ui-e2e` Thu, and s07/s08/s09/s10/gc ship-gates Tue-Sat); each still runs on `workflow_dispatch` and each still has its `pull_request` paths lane gating every real diff, so what weekly costs is drift-detection latency on an UNCHANGED tree, not per-change coverage. `codeql`'s header already claimed "weekly Monday" — the cron had silently drifted to daily. `cas-canary` goes hourly→every 4h: it is the one job that **must** stay GitHub-hosted (its whole assertion is that a DATACENTER IP can reach the cache, which a residential-IP fleet cannot prove), so cadence is its only lever — hourly is ~720 paid min/month, 36% of the entire allowance, for a canary. `admin-ui-e2e`'s browser matrix becomes event-scoped (chromium on `pull_request`; the full chromium/firefox/webkit matrix still runs on every `push` to main, on the weekly cron, and on dispatch) — cross-engine coverage is de-duplicated off the per-push lane, not deleted. **(3) paths** — `rustfmt` gains a `**/*.rs` + `**/rustfmt.toml` filter; the door stays welded because `cargo fmt --check` reads exactly that input surface and nothing else, so a docs-only PR was installing a Rust toolchain to format-check a tree with no Rust in the diff. **(4) concurrency** — already saturated — 59 of the 61 PR-triggered workflows already cancelled superseded runs, and of the two that did not, `welcome-first-pr` correctly must not; that left only `pentest-findings-sync`, now cancelling superseded PR runs while still never cancelling a `main` run. Deliberately NOT touched, and why: `dependabot.yml` (already weekly + grouped + day-staggered + limited — nothing left to win), `cargo-audit`/`cargo-deny`/`pnpm-audit`/`gitleaks`/`semgrep`/`trivy` (advisory-freshness and secret/CVE scanning — cutting their cadence or narrowing their paths trades security latency for money, which is the wrong trade), `e2e-prod` (daily prod monitoring of a LAUNCHED product, and only ~114 min/month), and the operational/compliance daily crons (`backup-daily`, `backup-daily-verify`, `audit-chain-daily-verify`, `billing-reconcile-daily`, `ac-bucket-acl-cron`, `gc-sweep-dry-run`) whose cadence is an obligation, not a preference. `gitleaks` and `trivy` also pin Linux-x64 release binaries and so cannot move to the fleet without re-pinning. **This is necessary but not sufficient:** the modeled residual is still ~5,000 min/month against a 2,000 allowance, dominated by lanes this PR deliberately left alone (dependabot's own update jobs ~1,030, `cargo-audit` ~800, `admin-ui-ci` ~800, `perf-regression` ~740) — a second pass, or a paid plan, is still required.
