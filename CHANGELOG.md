...
### Fixed
- **fix(ci): move the macOS-safe PR gates onto the self-hosted mac fleet — GitHub-hosted runners are billing-blocked and every hosted job dies in ~2 s.** From ~2026-08-02 00:40 UTC every `ubuntu-latest` job fails **before executing a single step**, with `runner: ""` (no runner ever allocated) and the definitive check-run annotation: *"The job was not started because recent account payments have failed or your spending limit needs to be increased."* Same block as 2026-06-25/26/28, first recurrence under the migrated `HuGR-Labs` org. It is OWNER-only to clear (org Settings → Billing & plans) and it kills PR gates AND `cf-deploy-prod`/`container-build-push` alike. **Interim fix (explicitly TEMPORARY — revert when billing clears or the corelink runner fleet is GA):** the PR-blocking gates that are macOS-safe move to `[self-hosted, mac, corelink-builder]` — `admin-ui-ci`, `admin-ui-e2e` (both jobs), `worker-vitest`, `signup-worker-vitest`, `okf_wiki`, `lighthouse-ci`, `cargo-audit` (both jobs) — each carrying an inline banner naming the reason and the revert condition, so this cannot silently become permanent. `playwright install --with-deps` also drops `--with-deps` on these: it shells out to **apt** and hard-fails on macOS. **Deliberately NOT moved** (they pin a Linux binary/toolchain and would need a checksum change to run on darwin — a security-relevant edit not worth making at speed): `trivy`, `semgrep`, `cosign-sign`, `cf-deploy-prod`, `ffi-matrix-ci`, `cas_foundation`, `smoke-install`, `backup-daily*`. Those stay red/blocked until billing is cleared. Fleet sanity checked first: 4 mac builders online (3 idle), 69 GB free disk (the "~98 % full" note in the ops memory is stale), node 22.17.1 + pnpm 10.32.1 + a warm chromium playwright cache already present; firefox/webkit download on first use.
