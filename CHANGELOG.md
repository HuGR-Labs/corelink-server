...
### Fixed
- **fix(ci): `changelog-validate` asserted that a `feat:`/`fix:` PR *has* an `[Unreleased]` entry — never that it has exactly ONE — so a rebase could duplicate an entry and the gate stayed green; it now fails on a duplicated lead sentence, and its first run finds three surviving duplicates already on `main`.** The whole assertion was "at least one added line falls between `## [Unreleased]` and the next `^## \[`" (`.github/workflows/changelog-validate.yml:84-163`): cardinality was never checked. That is exactly the hole a rebase drives through — a CHANGELOG commit that *replaced* an entry (`1 insertion(+), 1 deletion(-)`) degrades to insertion-only when upstream moves the context the deletion was anchored to, so the stale entry and its replacement both survive and the stale one keeps asserting what the replacement was written to retract. Observed on #955; still resident in this very file. New `scripts/check_changelog_duplicates.py` compares the **bolded lead sentence** of each `[Unreleased]` bullet — not the body, because the real pair diverged in its tail and an exact-match check would have missed it — on two signals over the first 24 normalised tokens: common token prefix ≥ 12, or bigram Dice ≥ 0.85. **The thresholds are measured, not guessed:** scored over all 166 753 pairs of the 578 comparable entries and hand-classified, honest siblings top out at prefix **9** (`CHANGELOG.md:100` vs `:102` — the `MKCOL` WebDAV fix vs the `PROPFIND`/`DELETE` one, two distinct changes to one surface) while real duplicates start at prefix **14** (`:63` vs `:64`); 12 is the middle of that empty band, ≥ 3 tokens of margin either way. Dice **alone cannot separate the classes** — the incident pair `:77`/`:78` scores 0.652, *below* the 0.800 of the honest sibling pair `:3971`/`:3984` — so it is kept only as a high-confidence OR above the measured honest maximum; that trap is written into the script so nobody "simplifies" it to one ratio. Scope is `[Unreleased]` only (released sections are frozen history and may repeat wording), and by default only pairs touching a line this PR *added*, judged from `git merge-base` so a branch rebased onto a newer `main` is not blamed for what it inherited. The failure message prints both lead sentences with both line numbers and names the rebase as the usual cause. A `--self-test` step runs four embedded fixtures in CI — the reconstructed incident must fail, the honest `fix(container):` siblings, a repeat across a released section, and the `- (none)` placeholders must all pass — so the gate is proved in both directions on every run. **Pre-existing, NOT fixed here** (a one-entry-per-PR CHANGELOG change may not delete other people's entries): `python3 scripts/check_changelog_duplicates.py --all` reports three duplicate groups already merged — `:47`/`:48` and `:78`/`:79` byte-identical, and `:63`/`:64` where the second entry retracts the first's claim. They need a backfill PR.
