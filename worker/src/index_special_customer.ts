  ...
  route: RouteMatch,
    ...
    if (route.routeKind === "devenv_v1") {
      ...
      try {
        ...
        devStub = env.RUNNER_DEVENV_DO.get(devDoId);
      ...
      if (isStart) {
        ...
        const mintKey = env.CORELINK_PAT_MINT_AUTH_KEY;
        const revokeKey = env.CORELINK_RUNNER_MINT_AUTH_KEY;
        ...
        const result = await relayAuthorizedDevenvStart(request, devTenantId, {
          ...
          mint: (operationId) => mintScopedPat(env, requestId, MintGrant.fromDevenvSession(devTenantId, principalSource, lifecycleGeneration), DEVENV_MAX_TTL_SECONDS, "cas:rw", mintKey, undefined, undefined, { operationId, tenantId: devTenantId, lifecycleGeneration }),
          ...
          cleanupFailed: () => console.error("devenv_start_revoke_failed"),
        });
