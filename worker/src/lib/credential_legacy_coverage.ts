  ...
  const row = await db.prepare(`
    WITH floor AS (
      ...
      SELECT p.pat_id,
        ...
        (p.pat_id IS NULL OR typeof(p.pat_id) <> 'text' OR length(p.pat_id) = 0 OR p.tenant_id IS NULL OR typeof(p.tenant_id) <> 'text' OR p.tenant_id <> ?1 OR typeof(p.token_id) <> 'text' OR length(p.token_id) = 0 OR typeof(p.expires_ms) <> 'integer' OR p.expires_ms < 0 OR (p.revoked_at_ms IS NOT NULL AND (typeof(p.revoked_at_ms) <> 'integer' OR p.revoked_at_ms < 0)) OR (p.runner_job_ac_key IS NOT NULL AND NOT (typeof(p.runner_job_ac_key) = 'text' AND (p.runner_job_ac_key = '*' OR (length(p.runner_job_ac_key) = 64 AND p.runner_job_ac_key NOT GLOB '*[^0-9a-f]*')))) OR (p.lifecycle_generation IS NOT NULL AND (length(p.lifecycle_generation) = 0 OR length(p.lifecycle_generation) > 19 OR p.lifecycle_generation GLOB '*[^0-9]*' OR (length(p.lifecycle_generation) > 1 AND substr(p.lifecycle_generation,1,1) = '0') OR (length(p.lifecycle_generation) = 19 AND p.lifecycle_generation > '9223372036854775807')))) AS malformed,
    ...
    SELECT (SELECT revoked_through FROM floor) AS floor_value,
      COALESCE(SUM(CASE WHEN malformed THEN 1 ELSE 0 END), 0) AS malformed,
      ...
      COALESCE(SUM(CASE WHEN active AND NOT known_runner AND NOT known_customer AND within_floor THEN 1 ELSE 0 END), 0) AS active_unknown
    FROM classified`).bind(tenantId, throughGeneration).first<unknown>();
  if (!row || typeof row !== "object") throw new Error("coverage unavailable");
  const value = row as Record<string, unknown>;
  if (typeof value["floor_value"] !== "string" || value["known_inventory"] === 0 || value["malformed"] !== 0 || value["active_known_runner"] !== 0 || value["active_unknown"] !== 0) return { complete: false };
  try {
    validateLifecycleGeneration(value["floor_value"]);
    return { complete: BigInt(value["floor_value"]) >= BigInt(throughGeneration) };
