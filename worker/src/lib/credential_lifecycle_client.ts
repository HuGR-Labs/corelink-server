...
function validateEndpoint(raw: string): URL {
  ...
  if (url.protocol !== "https:" || url.username || url.password || url.pathname !== "/" || url.search || url.hash) fail("credential authority URL must be an HTTPS origin");
  return url;
}

function validateKey(key: string): void {
  if (typeof key !== "string" || key.length < 32 || key.length > 4096 || [...key].some(c => c.charCodeAt(0) < 0x21 || c.charCodeAt(0) > 0x7e)) fail("credential authority key is invalid");
}
...
async function boundedBody(response: Response, signal: AbortSignal): Promise<string> {
  ...
  const aborted = new Promise<never>((_, reject) => {
    onAbort = () => { void reader.cancel("credential lifecycle request aborted").catch(() => {}); reject(new Error("credential lifecycle request timed out")); };
  ...
  try {
    for (;;) {
      ...
      if (total > MAX_BODY_BYTES) { void reader.cancel("response too large").catch(() => {}); fail("credential lifecycle response too large"); }
...
function decodedTopLevelKeys(raw: string): string[] {
  ...
  for (let i = 0; i < raw.length; i++) {
    ...
    if (char === '"') {
      ...
      for (; i < raw.length; i++) { const current = raw[i]; if (escaped) { escaped = false; continue; } if (current === "\\") { escaped = true; continue; } if (current === '"') break; }
      if (i >= raw.length) fail("invalid credential lifecycle response");
      if (depth === 1 && expectKey) { let decoded: unknown; try { decoded = JSON.parse(raw.slice(start, i + 1)); } catch { fail("invalid credential lifecycle response"); } if (typeof decoded !== "string") fail("invalid credential lifecycle response"); keys.push(decoded); expectKey = false; }
      continue;
    }
...
function parseSnapshot(raw: string, tenantId: string): CredentialLifecycleSnapshot {
  ...
  if (Object.keys(value).length !== 3 || value["tenant_id"] !== tenantId || !validateGeneration(value["generation"]) || value["suspended"] !== false) fail("invalid credential lifecycle response");
...
export async function readCredentialLifecycle(env: CredentialLifecycleEnv, tenantId: string): Promise<CredentialLifecycleSnapshot> {
  ...
  validateKey(key);
  const controller = new AbortController(); let timedOut = false; let rejectDeadline!: (error: Error) => void;
