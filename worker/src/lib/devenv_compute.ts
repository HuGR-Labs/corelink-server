...
export async function prepareDevenvCompute(
  ...
  if (row === null || !validPositiveConcurrency(row.max_concurrency)) {
    throw new Error("invalid DevEnv runners entitlement");
  }
  ...
  const token = await issueComputeGrant(env, {
    tenantId,
    workloadKind: "devenv",
    workloadId: sessionUuid,
    reservationId: sessionUuid,
    maxVcpuHours: row.max_vcpu_h,
    vcpuCount: DEVENV_COMPUTE_VCPU_COUNT,
