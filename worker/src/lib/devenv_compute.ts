...
function validPositiveConcurrency(value: unknown): value is number {
  return typeof value === "number" && Number.isSafeInteger(value) && value > 0;
}
function validPositiveU32(value: unknown): value is number {
  return typeof value === "number" && Number.isSafeInteger(value) && value > 0 && value <= 0xffff_ffff;
}

/** Reserve the DevEnv compute lease before minting its credential. */
export async function prepareDevenvCompute(
  ...
  if (row === null || !validPositiveConcurrency(row.max_concurrency)) throw new Error("invalid DevEnv runners entitlement");
  if (row.max_vcpu_h === null || row.max_vcpu_h === undefined) return null;
  if (!validPositiveU32(row.max_vcpu_h)) throw new Error("invalid DevEnv compute ceiling");
  const reservationId = sessionUuid;
  const token = await issueComputeGrant(env, {
    tenantId, workloadKind: "devenv", workloadId: sessionUuid, reservationId,
    maxVcpuHours: row.max_vcpu_h, vcpuCount: DEVENV_COMPUTE_VCPU_COUNT,
    maximumWallMs: DEVENV_COMPUTE_MAXIMUM_WALL_MS,
  }, nowMs);
  const binding: ComputeBinding = {
    token, reservationId, tenantId, workloadKind: "devenv", workloadId: sessionUuid,
