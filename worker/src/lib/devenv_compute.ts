...
export async function prepareDevenvCompute(
  ...
  const { token, maximumWallMs } = await issueComputeGrant(env, { tenantId, workloadKind: "devenv", workloadId: sessionUuid, reservationId: sessionUuid,
    maxVcpuHours: row.max_vcpu_h, vcpuCount: DEVENV_COMPUTE_VCPU_COUNT }, nowMs);
  const binding: ComputeBinding = { token, reservationId: sessionUuid, tenantId, workloadKind: "devenv", workloadId: sessionUuid,
    vcpuCount: DEVENV_COMPUTE_VCPU_COUNT, maximumWallMs };
  await stub.prepareAuthorizedCompute(binding);
  return { reservationId: sessionUuid, maximumWallMs };
}
