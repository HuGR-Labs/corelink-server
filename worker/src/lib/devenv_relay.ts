...
export function devenvPath(path: string): string | null {
  for (const base of BASES) {
    if (path === base || path.startsWith(`${base}/`)) return path.slice(base.length).replace(/\/+$/, "");
  }
  return null;
}
export function isDevenvStart(method: string, path: string): boolean {
  return method === "POST" && devenvPath(path) === "";
}
...
function failure(status: number): Response {
  return Response.json({ error: status === 400 ? "BAD_REQUEST" : "SERVICE_UNAVAILABLE", message: status === 400 ? "invalid DevEnv start configuration" : "DevEnv start unavailable" }, { status });
}
...
export async function relayAuthorizedDevenvStart(request: Request, tenantId: string, deps: RelayDeps): Promise<Response> {
  let body: unknown;
  try { body = await bounded(request.json()); } catch { return failure(400); }
  ...
  const profileName = body["profile_name"] ?? "default";
  const tier = body["tier"] ?? "standard-4";
  ...
  const deadline = deps.now() + DEVENV_MAX_TTL_SECONDS * 1000;
  const sessionUuid = deps.sessionId();
  if (!UUID.test(sessionUuid) || !UUID.test(tenantId) || !/^\d+$/.test(deps.lifecycleGeneration)) return failure(503);
  let patId: string | null = null;
  let armed = false;
  let accepted = false;
  let computeReservationId: string | null = null;
  try {
    const preparedCompute = await bounded(deps.prepareCompute(sessionUuid, tier));
    if (preparedCompute !== null && preparedCompute !== sessionUuid) return failure(503);
    computeReservationId = preparedCompute;
    ...
    const issued: unknown = await bounded(minted.json());
    if (!record(issued)) return failure(503);
    const id = issued["pat_id"];
    if (typeof id === "string" && id.trim().length > 0 && id.length <= 256) patId = id;
    const token = issued["token_plaintext"];
    const expires = issued["expires_ms"];
    const now = deps.now();
    ...
    accepted = await bounded(deps.adopt(sessionUuid, patId));
    if (!accepted) return failure(503);
    return Response.json({ sessionUuid, status: ack.status, lifecycle_generation: deps.lifecycleGeneration }, { status: 201 });
  } catch { return failure(503); }
  finally {
    if (!accepted && computeReservationId !== null) { try { await bounded(deps.abandonCompute(computeReservationId)); } catch { /* marker drain retains obligation */ } }
    if (armed && !accepted) { let revoked = false; for (let attempt = 0; attempt < 2 && !revoked; attempt++) { try { revoked = await bounded(deps.revoke(sessionUuid)); } catch { /* retry */ } } if (!revoked) deps.cleanupFailed(); }
  }
}
