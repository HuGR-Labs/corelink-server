...
import { applyCors, handlePreflight, stripClientTrustHeaders } from "./index_auth.js";
import { matchRoute, type RouteMatch } from "./route_match.js";
import { reapiError, serverGetOpts, internalConsumerForPath, INTERNAL_DO_D1_PROBE_PREFIX } from "./index_common.js";
import { resolveConsumerKey, requireDedicatedAdminAuth } from "./lib/internal_auth.js";
...
export async function handlePublicAndInternalRoute(
  ...
  route: RouteMatch,
    ...
    // The resolved internal-auth secret NEVER leaves the backend and is NEVER
    // accepted from the client (inbound trust headers are stripped before
    // injection). Money endpoints use their dedicated-first resolver below;
    // other onboarding routes retain the shared contract.
