...
describe("runQuotaBatch — D1 failure posture (CAA-360 #25 / F21)", () => {
  it("fails CLOSED on a READ when the batch throws, and does not count the request", async () => {
    ...
    expect(result.storage, "reads retain availability during a transient D1 outage").toEqual({ ok: true });
    expect(
      result.increment,
  ...
  it("skips the storage read entirely when the TIER is unconfirmed (F21), but still meters", async () => {
    ...
    expect(read.storage, "reads retain availability with an unconfirmed cap").toEqual({ ok: true });
