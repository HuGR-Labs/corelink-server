...
describe("B-081 PAT rotation siblings reach container.start", () => {
  ...
  it("maps whitespace siblings to a malformed marker so the container fails closed", async () => {
    ...
    expect(env.PAT_SIGNING_KEY_PREV).toBe("__invalid_blank_pat_signing_key__");
    expect(env.PAT_SIGNING_KEY_NEW).toBe("__invalid_blank_pat_signing_key__");
  });
