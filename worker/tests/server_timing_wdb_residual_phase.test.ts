/**
 * `qother` — the unattributed remainder of `wdb` after its four named
 ...
 * inside `wdb` and was never timed. `qdo` closes that gap; `wdbResidualPhase`
 ...
 *
 * Invariant under test: qtier + qdo + qbatch + qresid + qother === wdb,
 * exactly, always.
 */

import { describe, it, expect } from "vitest";
import { wdbResidualPhase } from "../src/index.js";

describe("wdbResidualPhase", () => {
  it("computes the exact difference when all four phases ran and sum < wdb", () => {
    ...
    expect(wdbResidualPhase(265, [30, 50, 20, 22])).toBe("qother;dur=143");
  });
  ...
  it("excludes a -1 (did-not-run) phase from the sum instead of treating it as 0", () => {
    ...
    expect(wdbResidualPhase(wdb, phases)).toBe(`qother;dur=${wdb - sum}`);
  });

  it("emits dur=0 when the phases sum exactly to wdb — a real zero, not omitted", () => {
    expect(wdbResidualPhase(100, [25, 25, 25, 25])).toBe("qother;dur=0");
  });
  ...
  it("refuses the split with desc=unreconciled when the phases overshoot wdb", () => {
    ...
    expect(wdbResidualPhase(100, [30, 50, 20, 22])).toBe('qother;dur=100;desc="unreconciled"');
  });

  it("attributes the entire wdb to qother when every phase is -1 (none ran)", () => {
    expect(wdbResidualPhase(77, [-1, -1, -1, -1])).toBe("qother;dur=77");
  });
});
