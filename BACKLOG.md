...
- **`owner: owner` means the NEXT STEP is physically impossible without him RIGHT
  ...
  *"it is ready, you just have to press it"*, the item is `owner: tl`, and what is left
  ...
  the downstream decision does not block the item. Everything else is `owner: tl`.
  Tightened 2026-08-31 after the field, read loosely, parked 18 items behind a person
...
**Só ele (reconfirmado 2026-08-31):** ler o incident log exige uma credencial de **leitura**
da PagerDuty que **não existe em lugar nenhum** — o único segredo provisionado, no
`.env.local` e nos secrets do repo, é `PAGERDUTY_ROUTING_KEY`, de escrita. `PAGERDUTY_API_KEY`
aparece em três doc-comments e em nenhum código que o leia. Provisioná-la é a conta dele.
...
**Só ele (reconfirmado 2026-08-31):** o bloqueio de R2 é da Cloudflare, e nenhuma decisão dele
o destrava — mas a única saída que **não** espera a Cloudflare é a (b), pagar por um segundo
backend com Object Lock. É o dinheiro dele. Não leia esta linha como pergunta pendente na
mesa dele sobre o R2 em si.
...
**Só ele (reconfirmado 2026-08-31):** não há credencial do Drata em lugar nenhum, e a ausência
não é inofensiva — `crates/corelink-ops/src/drata/drata.rs` faz `std::env::var("DRATA_API_KEY")`
de verdade e devolve `Misconfigured`. O código **quer** a chave que só a conta dele emite.
...
**Reclassificado 2026-08-31 — `owner: tl`.** Próximo passo: nenhum; o item está `done`. A
decisão de custo do owner (*"usa corelink runners"*) é **procedência datada**, não bloqueio
presente, e o campo carrega estado, não história.
...
**Só ele (reconfirmado 2026-08-31):** os dois caminhos honestos terminam em counsel contratado
por ele e em instrumento assinado. Redigir o `v1.0.1` eu faço; **aprová-lo** sem counsel seria
uma segunda mentira.
...
inside each block is the authority.** Measured 2026-08-31: 20 of the 28 items filed
under it already carried `owner: tl`, and four more moved here (B-041, B-042, B-043,
B-058). Items are left where they sit on purpose: relocating blocks produces a diff
that conflicts with every sibling PR and buys nothing, since no gate reads the heading.
Read the field.
...
**Só ele (reconfirmado 2026-08-31):** o GitHub **não expõe API para cunhar fine-grained PAT** —
`POST /authorizations` saiu em 2020 sem substituto — e criar GitHub App exige o app-manifest
web. É fluxo de UI autenticado na conta dele.
...
**Só ele (reconfirmado 2026-08-31):** `~/Downloads` é diretório do usuário, fora de qualquer
repo e vedado a agentes, e a ação é **deleção permanente** de chave privada — que eu não
executo por regra.
...
regressão. A decisão do owner de 2026-08-24 (*não pagar*) já foi tomada e está registrada
acima; um item `done` não pode estar bloqueado em ninguém.
...
**Só ele (reconfirmado 2026-08-31):** as três saídas são desbloquear o gasto hospedado, pagar
um box Linux multi-core self-hosted, ou apagar as lanes. As duas primeiras são o dinheiro
dele; a terceira ele tem de autorizar, porque apaga cobertura (mutation, coverage,
cas-foundation).
...
**Só ele (reconfirmado 2026-08-31):** conta Apple Developer paga e certificado Authenticode
emitido por CA são **compra** em nome dele, com verificação de identidade. Nenhuma parte é
configuração.
...
**Só ele (reconfirmado 2026-08-31):** aposentar o endpoint redundante é clique no painel Stripe
da conta dele. Eu não tenho — e não devo ter — sessão autenticada nesse painel.
...
**Só ele (reconfirmado 2026-08-31):** dos dois reparos, um emenda
`legal/dpa-residency-amendment.md`, que é **instrumento assinado** — mesma classe que mantém
...
**Reclassificado 2026-08-31 — `owner: tl`.** Próximo passo: corrigir as três linhas do CAIQ
para `N`/`P` com plano datado — edição de texto, minha. O resíduo de owner é decidir se os
...
**Reclassificado 2026-08-31 — `owner: tl`.** Próximo passo: varrer o resíduo medido (30
arquivos, 65 posições) e retratar a afirmação. Corrigir texto que afirma um pentest que não
existe é conserto, não decisão comercial — e o repositório já contém a instrução de não
afirmá-lo. Distribuir o comunicado corrigido é dele; escrevê-lo é meu.
...
**Só ele (reconfirmado 2026-08-31):** emendar o SLA é assinatura, e ligar a emissão automática
de crédito é comprometer a empresa a **devolver dinheiro contra fatura**. Escrever o código e
ligá-lo são atos diferentes; o segundo gasta o dinheiro dele.
...
**Só ele (reconfirmado 2026-08-31):** pedir aumento de cota é abrir ticket na conta comercial
da Cloudflare — relação de fornecedor, nome e faturamento dele. Medir e documentar o teto é
meu e já está feito.
...
⚠️ **Correção 2026-08-31 — a frase abaixo era verdadeira quando escrita e ficou FALSA sem
que nada apitasse, que é exatamente a tese do item.** Quando este item foi redigido, zero
itens `done` carregavam `owner: owner`. Na medição desta árvore eram **cinco** — B-031,
B-041, B-042, B-043 e B-085 — e o `backlog_verify` passou verde o tempo todo, porque o
`verify` deste item mede a **ausência do portão** e não a contagem viva. Os cinco foram
reclassificados na mesma passada que reescreveu o campo `owner:`, então a contagem voltou a
...
**Só ele — e apenas isto (precisado 2026-08-31):** a **assinatura**. O texto do aditivo, a
minuta da notificação formal e o parecer de qual das três saídas é mais barata por instrumento
**eu entrego prontos** — isso é redação, e é minha. O que não é executável sem ele é executar
o instrumento: assinar, notificar a contraparte, ou pagar a construção da capacidade.
...
original continua literalmente verdadeira e é o que sobrou: um gate autenticado com credencial
de **operador** mede permissão e finge medir realidade. O caminho que eu percorro com a chave
de mint **não é o caminho que o cliente tem**, e essa diferença é o defeito. Toda medição feita
por essa rota deve declarar, na própria medição, que percorreu a rota de operador.
...
verify-means: |
  ...
  **`tl` pelo critério estrito (reclassificado 2026-08-31).** O item era `owner:` porque o
  ...
  `CORELINK_PAT_MINT_AUTH_KEY` no lugar, então esse bloqueio deixou de existir. O que o
  `verify` mede **não mudou uma linha** — ele sempre mediu a ausência da rota, nunca a falta
  do meu acesso — e é essa ausência que continua sendo o item.
