...
- Cleanup now carries the authorized cache inode through descriptor-relative recursive removal; materialization locking holds an exclusive flock on the Git common-directory descriptor, so replacing every named marker cannot split ownership. Bounded Git/report/command helpers stream captured output under a byte cap and terminate process groups on timeout.
