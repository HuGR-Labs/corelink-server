...
- **Public PAT mint aliases could drift into separate rate-limit budgets (B-160).**
  ...
  ten-per-hour policy and its 360-second retry boundary. The bucket is now
  durably serialized by the tenant Durable Object, survives restart/container
  replacement, and fails closed when state is malformed or unavailable.
