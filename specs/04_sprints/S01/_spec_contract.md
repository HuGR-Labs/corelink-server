...
| 1.9.0 | 2026-04-29 | Gustavo (Lote S-01 SPRINT-CLOSE CEREMONY — orchestrator finalisation) | **Sprint S-01 implementation phase COMPLETE: all 7 WIs SEALED end-to-end.** Sprint contract closed via orchestrator: `sprint.md` doc_status DRAFT → FROZEN; work_status READY → COMPLETE; `sprint.md` v1.0.0 → v1.1.0 with sign-off table §14.1 preenchida com 11 papéis canonical (solo-tier waiver per ADR-0034 + codex adversarial substituindo external reviewers; per-WI scores ≥ 8.5/10 em 100% das WIs). Aggregate state: **5 Rust crates** (`corelink-{tenant-path,hash,worker,meta,reapi}`) + 1 server app + 8 CI workflows (tenant-path, corelink-hash, corelink-worker, corelink-meta, corelink-reapi, cas_foundation sprint-level, nightly extended-bound TLC + 1h fuzz × 9 + workspace mutants, tla_check + spec_validation pre-existentes); **2 ADRs novos** (ADR-0043 HMAC tenant prefix algorithm + ADR-0044 SBOM CycloneDX 1.5+ toolchain); **~250+ tests verde** aggregate with 10k iter property tests (3 CRITICAL invariants cross-component em prop_cas), canonical regression vectors (BLAKE3 chunk boundaries, HMAC16 cross-implementation, R2 path templates, D1 SQL traces), criterion benches (~3 μs/call tenant-path, ~3 GiB/s BLAKE3, μs-tier R2 path), cargo-fuzz 60s smoke × 9 targets zero panics, cargo-mutants kill-rate ≥ 80% por crate (94.7% corelink-worker, 100% tenant-path); validators 264 docs schema + INV registry 136/136 verde. WI commits: WI-S01-001 `43b925d`, WI-S01-002 `1cb2c9c`, WI-S01-003 `9a876a8`, WI-S01-004 `6a45ff6`, WI-S01-005 `82d49d8`, WI-S01-006 `eb092f9`, WI-S01-007 `3339e1d`. Sprint-close commit (this one) + tag `s01-impl-sealed` for visible checkpoint. **Próximo:** S-02 sprint (CAS read path; WIs S02-001..S02-005). |
| 1.8.0 | 2026-04-29 | Gustavo (S-01 implementation Lote — WI-S01-005 SEAL post codex adversarial review rounds 1+2) | **WI-S01-005 SEALED**. Workspace agora carrega 5 impl crates (`tenant-path`, `corelink-hash`, `corelink-worker`, `corelink-meta`, `corelink-reapi`). New deliverable: `crates/corelink-reapi/` — REAPI v2 gRPC handlers (`BatchUpdateBlobs`, `Capabilities.GetCapabilities`, `ByteStream::Write`) with PAT-stub auth + R2 PUT + D1 batch + audit_outbox emission orchestration. Highlights: (a) **Vendored REAPI proto subset** under `crates/corelink-reapi/proto/build/bazel/remote/execution/v2/remote_execution.proto` against `bazelbuild/remote-apis @ v2.12.0` (canonical-source correction: WI-S01-005 v1.1.0 body cited `v2.13.0`, which does not exist upstream — `v2.12.0` is the latest stable tag at SEAL date and the one the vendored subset tracks; field tags retained byte-for-byte to preserve wire-compat with Bazel/Buck2 clients). (b) **`Capabilities.GetCapabilities`** now advertises canonical `MaxBatchTotalSizeBytes=4MiB` + `max_cas_blob_size_bytes=5MiB` + `digest_functions=[BLAKE3]` only (BLAKE3-only post codex round-1 P1 fix; SHA256 advertisement removed because the write path verifies BLAKE3 only). (c) **CloudEvents 1.0 audit envelope** — CE `time` deliberately omitted to keep `audit_outbox.payload_json` content-stable across retries; row enqueue timestamp lives in `audit_outbox.enqueued_at` only. (d) **Per-blob `audit_request_id`** derivation = `<client_request_id>:<tenant_uuid>:<digest_canonical_text>` closes both the multi-blob batch UNIQUE collision (codex P0) and the cross-tenant `x-request-id` reuse risk (codex P1); the CE `data.request_id` field continues to carry the raw client correlation id for SIEM linking. (e) **Dual-write reconciliation** per ADR-0027: production `R2DeleteReconciler` wired (the `R2Backend::delete` trait extension is the pending forward-WI step; current reconciler logs orphan + defers to GC sweep S-06 — a real and load-bearing wire-up vs. the prior `Noop`-only stub). (f) **`error_taxonomy.md` v0.2.0** registers 4 new CAS codes consumed here: `COR_CAS_BATCH_TOO_LARGE` (HTTP 413, gRPC RESOURCE_EXHAUSTED), `COR_CAS_BAD_RESOURCE_NAME` (HTTP 400, gRPC INVALID_ARGUMENT), `COR_CAS_DIGEST_FUNCTION_UNSUPPORTED` (HTTP 400, INVALID_ARGUMENT), `COR_CAS_BAD_DIGEST` (HTTP 400, INVALID_ARGUMENT). (g) **WI v1.1.0 → v1.2.0** com §31 changelog row + frontmatter `doc_status: FROZEN`/`work_status: DONE`. (h) Test surface: 33 lib unit + 6 canonical regression vectors + 1 capabilities harness + 10 e2e gRPC scenarios (per Gherkin AC matrix incl. multi-blob non-collision regression test) + 4 property tests at proptest 256 cases (PK enforcement, audit dedup, tenant scoping, hash-mismatch never reaches storage) = **54 tests verde** (debug + release). cargo clippy `-D warnings` workspace clean; cargo check `--target wasm32-unknown-unknown --no-default-features` clean (pure-logic core; tonic/tokio gated behind `host-server` feature). New CI workflow `.github/workflows/corelink-reapi.yml`. |
