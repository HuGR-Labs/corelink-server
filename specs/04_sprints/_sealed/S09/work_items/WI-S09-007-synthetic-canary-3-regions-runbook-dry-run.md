...
| 1.2.0 | 2026-04-26 | Gustavo (Lote 10.9-quaters **SEALED**) | Sonnet R5 quinquies 8.5/10 APPROVED. NEW-P1-2 (mostly fixed): IATA codes IAD/LHR/BOM em Gherkin scenarios + §2 adversarial + §3 Persona 2 + §6.1.11 chaos #1 + §18 Dependencies replaced com enam/weur/apac R2 region hints. P2 residuals: 1 Gherkin line + §27 KT prose + document title still reference IAD/LHR/BOM/us-east/eu-west/ap-south (downgraded P2 non-blocking; deferred housekeeping). P0-A canary 12960 math + P0-E underscores verified PASS. **WI sealed pre-implementation**. |
| 1.3.0 | 2026-05-03 | Gustavo (via Claude Opus 4.7 1M; autonomous WI-S09-007 SEAL) | **WI-S09-007 SEALED — `crates/corelink-canary/` v0.1.0 shipped (no D1 migration this WI per `trait-abstraction-defer`) + `scripts/rb_fm_153_dry_run.sh` + `scripts/rb_obs_cardinality_001_dry_run.sh` host-side dry-run scripts + `specs/05_quality/runbooks/RB-FM-153-grafana-cloud-outage.md` + `specs/05_quality/runbooks/RB-OBS-CARDINALITY-001.md` flipped DRAFT → FROZEN with 2026-05-03 dry-run-executed timestamp + `specs/_audits/sealed/2026-05-03-rb-fm-153-dry-run.md` + `specs/_audits/sealed/2026-05-03-rb-obs-cardinality-001-dry-run.md` audit traces + `specs/_audits/sealed/2026-05-03-adversarial-s09.md` 12-scenario adversarial review aggregation + `specs/04_sprints/_sealed/S09/PRR-S09.md` HIGH_RISK 11-sign-off PRR APPROVED (4 ✅ + 7 ⚠️ WAIVED via ADR-0034 dual-hat); promotion STAGING-STABLE.** Implementation honors the `trait-abstraction-defer` charter pattern (per `corelink-analytics::CardinalityValidator` ship gate from WI-S09-001 SEAL + `corelink-logpush::PiiRedactor` ship gate from WI-S09-002 SEAL + `corelink-tracing::TracingService` ship gate from WI-S09-003 SEAL + `corelink-audit-chain::ChainVerifier` ship gate from WI-S09-004 SEAL + `dashboards/grafana/DASH-*.json` corpus from WI-S09-005 SEAL + `corelink-slo::MultiBurnRateAlert` ship gate from WI-S09-006 SEAL); the runtime in-memory orchestrator IS the load-bearing 3-region canonical + canonical assertion ladder boundaries + BLAKE3 digest match correctness + audit-emit-BEFORE-mutation fail-CLOSED envelope + synthetic tenant SLI exclusion (Lote 10.8bis P1-NEW-3 inheritance) falsifiability target — the production CF Workers cron-trigger 24/7 sustained 72h sem gap (12_960 loops/72h target per Lote 10.9bis P0-A corrected from 38_880 triple-counted) + `worker::Fetch::send` HTTP client + real R2 PUT/GET/AC lookup + `blake3::hash` digest verify + Mimir/Loki/Tempo/Grafana health probe HTTPS endpoints + RB dry-runs in real staging environment + 100k nightly via `PROPTEST_CASES=100000` + chaos 10 are deferred alongside S-20 GA gate. New crate ships 7 source modules (~1900 LOC src + ~440 LOC tests = ~2340 LOC total): `region` (`CanaryRegion` `#[non_exhaustive]` 3-canonical Enam/Weur/Apac per `data_model.md §2.1` R2 region hints + `primary_cf_region()` mapping to `corelink_analytics::Region` 22-canonical CF colocode for cross-crate label cartesian consistency); `assertion` (`CanaryAssertion` `#[non_exhaustive]` 4-canonical CasPutP99/CasGetP99/AcLookupP99/DigestMatch + `AssertionCeilings` canonical 100/50/30 ms p99 ladder + `CanaryLatenciesMs` observed snapshot + `within_ceilings` strict-≤ boundary + `first_breach` canonical PUT > GET > LOOKUP ordering); `result` (`CanaryDecision` `#[non_exhaustive]` 3-canonical Pass/Degraded/FailedRegion + `counts_toward_ship_gate()` predicate + `HealthComponent` `#[non_exhaustive]` 4-canonical Mimir/Loki/Tempo/Dashboard + `ObservabilityHealthReport` canonical 30s/5s/30s/3s ceilings + `CanaryLoopResult::compute_decision` precedence-aware computation where digest mismatch + observability unhealthy + dispatch lag > 90s all take precedence over latency ceiling); `config` (canonical constants pinned at type system layer: `CRON_INTERVAL_SECS=60` + `DISPATCH_LAG_SEV3_MS=90_000` + `SUSTAINED_72H_LOOPS_PER_REGION=4_320` + `SUSTAINED_72H_LOOP_TARGET=12_960` per Lote 10.9bis P0-A + `SYNTHETIC_CANARY_TENANT_ID="00000000-0000-0000-0000-canary000000"` 36-char canonical UUID format); `audit` (`CanaryAuditEventType` `#[non_exhaustive]` 5-canonical taxonomy `corelink.canary.{loop_executed, assertion_failed, digest_mismatch, observability_unhealthy, dispatch_lag_exceeded}` + `CanaryAuditRecord` typed shape + `CanaryAuditSink` trait + `InMemoryCanaryAuditSink` capture + `FailingCanaryAuditSink` adversarial fixture + `CanaryAuditEmitError::Store` lifting via `From<CanaryAuditEmitError> for CanaryError`); `error` (`CanaryError` `#[non_exhaustive]` taxonomy with AssertionFailed carrying region + assertion + DigestMismatch carrying hex digests + ObservabilityUnhealthy carrying component + DispatchLagExceeded carrying observed + threshold + Audit + Internal mutex poisoning); `probe` (`CanaryProbe` trait + `InMemoryCanaryProbe<A>` orchestrator parameterized over `CanaryAuditSink` + per-region ledger under per-instance `Arc<Mutex<HashMap<CanaryRegion, LedgerEntry>>>` F-001 closure for INV-TENANT-ISOLATION canary at synthetic tenant boundary + canonical audit-emit-BEFORE-mutation envelope per S-07 P1-1 fix + Lote 10.6bis pattern: 1) LoopExecuted audit emit, 2) branch on Degraded → AssertionFailed audit per first-breach assertion, 3) branch on FailedRegion arms (digest mismatch / observability unhealthy / dispatch lag > 90s) emit canonical audit + return typed error, 4) per-region ledger advance AFTER all emits committed; ledger tracks pass_loops + degraded_loops + failed_region_loops + consecutive_non_pass for SEV-2 alert source after 3+ consecutive in same region per WI §1 invariant 8; `RegionProbeStats::total_loops` + `total_pass_loops` cross-region cumulative for ship-gate progress tracking; `FailingCanaryProbe` adversarial fixture). Tests: **75 tests across all targets, 0 failures, parallel-safe**: 55 inline lib unit (assertion 6 + audit 11 + config 4 + error 6 + probe 9 + region 6 + result 13) + 20 prop_canary including the 6 required property tests at 10k iter PR-gate (`PROPTEST_CASES` env var override at runtime per S-07 P1-2 fix): `prop_canary_assertion_ladder_canonical` (load-bearing falsifiability target — for any (latencies, digest_match, health, lag) tuple the canonical decision matches the assertion ladder boundaries exactly), `prop_canary_digest_correctness` (digest mismatch always FailedRegion regardless of every other input; healthy + within-ceiling + ≤90s lag + digest match always Pass), `prop_synthetic_tenant_excluded_from_sli` (every audit record carries the canonical synthetic canary tenant_id literal; Lote 10.8bis P1-NEW-3 inheritance from WI-S08-005 ManualOverride exclusion pattern; runtime guarantee that the recording rule filter has a structural anchor), `prop_dispatch_lag_threshold_canonical` (observed lag > 90_000 ms always FailedRegion; ≤ 90_000 never does), `prop_three_region_decision_isolation` (per-region ledger isolation; canary loops in region A never perturb region B's count; INV-TENANT-ISOLATION canary at synthetic tenant boundary), `prop_audit_emit_per_decision_arm` (pass arm = 1 audit LoopExecuted; non-pass arms = 2 audits with first always LoopExecuted per audit-emit-BEFORE-mutation envelope ordering). Plus surface-pinning regression tests covering canonical 3 regions Enam/Weur/Apac + canonical 4 assertions + canonical 5 audit event strings + canonical 4 health components + canonical 3 decisions + canary_schema_version() = 1 + 12_960 sustained loop target + 90_000 dispatch lag threshold + 60s cron interval + 36-char canonical UUID synthetic tenant_id + first_unhealthy canonical Mimir > Loki > Tempo > Dashboard ordering + first_breach canonical PUT > GET > LOOKUP ordering + audit-failure-no-ledger-advance fail-CLOSED pin. Cross-module patches: workspace `Cargo.toml` adds `crates/corelink-canary` member + workspace dep; reuses `corelink-analytics::Region` 22-canonical CF colocode for cross-crate label cartesian consistency. RB-FM-153 dry-run script (~190 LOC bash) drives 7-step host-side walkthrough + drift detection + tail-evidence summary; exit 0 on canonical green; runbook RB-FM-153 flipped DRAFT → FROZEN. RB-OBS-CARDINALITY-001 dry-run script (~190 LOC bash) drives 7-step host-side walkthrough + drift detection + tail-evidence summary; exit 0 on canonical green; runbook RB-OBS-CARDINALITY-001 flipped DRAFT → FROZEN. PRR-S09 HIGH_RISK 11 sign-offs canonical (per framework §33.5.4.3 + ADR-0034): 4 ✅ APPROVED (Owner / Final Approver / Engineer S-09 lead / Product) + 7 ⚠️ WAIVED via ADR-0034 dual-hat (Architect — incl. specialization for INV-OBS-CARDINALITY-BUDGET cartesian closure + INV-OBS-AUDIT-CHAIN-INTEGRITY RFC 8785 JCS canonical determinism + Google SRE Workbook Ch 5 Table 4 multi-burn-rate boundary discipline / Security Lead / SRE Lead / QA Lead / Compliance Officer / Privacy Officer + ADR-0017 DPO interim / AppSec advisor); promotion STAGING-STABLE. Adversarial review aggregates 12 scenarios across WI-S09-001..007; cumulative invariant interaction matrix; zero HIGH/CRITICAL findings. **2 NEW INVs §3.12 promoted** (INV-OBS-CARDINALITY-BUDGET HIGH + INV-OBS-AUDIT-CHAIN-INTEGRITY HIGH; Sprint-driven invariants Lote 10.9 S-09 NEW group). **CI ship-gate workflow** `.github/workflows/s09-ship-gate.yml` runs validators chain + cross-component prop suite at 10k iter PR-gate across 6 S-09 crates + RB-FM-153 + RB-OBS-CARDINALITY-001 dry-run scripts + 12 canonical Grafana dashboards JSON parse smoke + alert rules YAML parse smoke + dashboard validator + aggregate fan-in gate. **Nightly proptest extension**: `.github/workflows/nightly.yml::proptest-extended` extended with 6 S-09 crates × `prop_analytics` + `prop_logpush` + `prop_tracing` + `prop_audit_chain` + `prop_slo` + `prop_canary` 100k iter; ZERO INV-OBS-CARDINALITY-BUDGET / INV-OBS-AUDIT-CHAIN-INTEGRITY / INV-AUDIT-EMIT-ATOMIC-WITH-HANDLER / INV-TENANT-ISOLATION violations sustained over 100k iter is the spec contract DoD §6 gate. **Trait-abstraction-defer per charter (consolidated alongside S-20 GA gate)**: real CF Workers cron-trigger 24/7 sustained 72h sem gap + `worker::Fetch::send` HTTP client + real R2 PUT/GET/AC lookup paths + Mimir/Loki/Tempo/Grafana health probe HTTPS endpoints + Twilio backup SMS fallback + Terraform IaC for 3 PagerDuty services + production `cardinality_check.py` PR-time static analyzer + production OTLP HTTP exporter + Grafana Tempo tenant config + W3C `tracestate` vendor extension propagation + real PagerDuty Events API v2 HTTPS POST `/v2/enqueue` + `promtool test rules` CI gate + auto-quarantine flapping cron + screenshot snapshot CI cron via Grafana headless API + freshness CI hook PR-path-correlation + idempotent 10× Terraform replay + 30d sustained chaos + cargo-fuzz expansion + cost regression gate + real client integration smoke. Quality gates verde: `cargo test -p corelink-canary --all-targets` 75 tests 0 failures debug + release; `cargo clippy --workspace --all-targets --features corelink-worker/tower-middleware -- -D warnings` clean; `validate_specs.py` clean (280 schema + 6 YAML = 286 docs); `check_migrations_additive.py` clean (16 migrations; no new migration this WI per `trait-abstraction-defer`); `validate_dashboards.py` clean (12/12 canonical + 3 legacy structural checks PASS); both RB dry-run scripts exit 0; pre-existing dangling [INV] / [SLO] / [RB] references inherited across S-13/S-14/S-17/S-19/S-20 forward-looking specs (NOT introduced by this WI; baseline parity confirmed via stash test). No per-WI codex per 2026-04-30 protocol; sprint-close Sonnet review covers full S-09 corpus AFTER WI-S09-007 SEAL. S-09 ship gate ready for sprint-close Sonnet review. |
