...
| 1.2.0 | 2026-04-26 | Gustavo (Lote 10.9-quaters **SEALED**) | Sonnet R5 quinquies 8.5/10 APPROVED. NEW-P1-1 fixed: Terraform code corelink_prod_sam + corelink_prod_iad resources removed (was incoherent com narrative claim 3 services); ST-002 sub-task updated 'PagerDuty 3 services Terraform'. P0-F sprint contract §5.5 R-S09-14 alignment complete (3 services: staging + prod-us + prod-eu canonical). **WI sealed pre-implementation**. |
| 1.3.0 | 2026-05-03 | Gustavo (via Claude Opus 4.7 1M; autonomous WI-S09-006 SEAL) | **WI-S09-006 SEALED — `crates/corelink-slo/` v0.1.0 shipped + `dashboards/alerts/dash-slo-multi-burn.yml` (NEW alert rule corpus; 7 SLI groups × 4 windows = 28 alert rules + 7 recording rules = 35 total per Lote 10.8-tris P1-NEW-1 rigorous count discipline). Frontmatter flipped doc_status DRAFT→FROZEN, work_status READY→DONE, audit_status ACTIVE→AUDITED.** Implementation honors the `trait-abstraction-defer` charter pattern (per `corelink-analytics::CardinalityValidator` ship gate from WI-S09-001 + `corelink-logpush::PiiRedactor` ship gate from WI-S09-002 + `corelink-tracing::TracingService` ship gate from WI-S09-003 + `corelink-audit-chain::ChainVerifier` ship gate from WI-S09-004 + `dashboards/grafana/DASH-*.json` corpus from WI-S09-005); production PagerDuty Events API v2 HTTPS POST + `promtool test rules` CI gate + auto-quarantine flapping cron + Twilio fallback + Terraform IaC for 3 PagerDuty services (staging / prod-us / prod-eu per sprint contract §5.5 R-S09-14 canonical Lote 10.9bis P0-F) deferred alongside WI-S09-007 PRR ship gate. New crate ships 9 source modules (~2480 LOC src + ~413 LOC tests = ~2893 LOC total): `window` (`BurnRateWindow` `#[non_exhaustive]` 4-canonical Fast1h/Medium6h/Slow24h/Long3d per Google SRE Workbook Ch 5 Table 4 + canonical 14.4×/6×/3×/1× threshold multipliers + `prometheus_range_label` `[1h]`/`[6h]`/`[24h]`/`[3d]` + `alertmanager_for_seconds` 120/900/3600/14400 + `slug` canonical strings); `decision` (`AlertDecision` `#[non_exhaustive]` 5-canonical Quiet/TicketSev3/TicketSev2/PageSev1/PageSev0 + `severity_label` SEV-string mapping per `observability_model.md §3.1` + `is_page`/`is_ticket`/`is_quiet` predicates that partition the canonical decision space exactly); `definition` (`Sli` `#[non_exhaustive]` 7-canonical taxonomy from `slo_catalog.md §4.x` (AvailCasGet / AvailCasPut / AvailAcLookup / AvailAuth / LatencyCasGetP99 / DedupRatio / RateLimitWithinQuota) + `SloDefinition::new` strict input validation rejecting non-finite / ≤0 / >1 target_pct + canonical 30-day rolling window default); `calculator` (`BurnRateCalculator` zero-sized pure-logic + `BurnRateSample` (errors_in_window / total_in_window) + `error_rate_in_window` saturating div-by-zero defense + `decide` Google SRE Workbook Ch 5 Table 4 boundary discipline `error_rate >= multiplier × error_budget_pct` semantics with documented IEEE 754 imprecision pitfall debounced via Prometheus `for:` clause); `pagerduty` (`PagerDutyEventAction` `#[non_exhaustive]` 3-canonical trigger/acknowledge/resolve per Events API v2 + `PagerDutyServiceKey` `#[non_exhaustive]` 3-canonical Staging/ProdUs/ProdEu per sprint contract §5.5 R-S09-14 + `PagerDutyEvent` envelope with dedup_key + severity + summary + source + runbook_url + `PagerDutyDispatcher` trait + `InMemoryPagerDutyDispatcher` honoring Events API v2 §dedup_key idempotency contract under per-instance `Arc<Mutex<>>` F-001 closure (Trigger with existing dedup_key collapses to single open incident; Resolve removes; Acknowledge no-op state) + attempts ledger separate from open-incidents map + `FailingPagerDutyDispatcher` adversarial fixture); `audit` (`SloAuditEventType` `#[non_exhaustive]` 5-canonical taxonomy `corelink.slo.{burn_rate_evaluated, alert_fired, alert_quiet, page_dispatched, ticket_filed}` + `SloAuditRecord` typed shape with `pagerduty_dedup_key` empty-on-quiet + `SloAuditSink` trait + `InMemorySloAuditSink` capture + `FailingSloAuditSink` adversarial fixture + `SloAuditEmitError::Store` lifting via `From<SloAuditEmitError> for SloError`); `alert` (`MultiBurnRateAlert<A,P>` orchestrator parameterized over `SloAuditSink` + `PagerDutyDispatcher` + canonical `PagerDutyServiceKey` routing + per-tenant ledger under per-instance `Arc<Mutex<HashMap<String, PerTenantLedger>>>` F-001 closure for INV-TENANT-ISOLATION canary + canonical audit-emit-BEFORE-mutation envelope per S-07 P1-1 fix + Lote 10.6bis pattern: 1) BurnRateEvaluated audit, 2) branch on quiet → AlertQuiet OR non-quiet → AlertFired, 3) branch on page → PageDispatched audit BEFORE dispatch attempt + `Trigger` Events API v2 dispatch IFF `is_page()` ELSE TicketFiled audit, 4) ledger advance AFTER all emits committed; `evaluate_dispatcher_fail_open` variant returns canonical `AlertEvaluationOutcome` even when dispatcher rejects so production wiring fails OPEN to Twilio fallback); `error` (`SloError` `#[non_exhaustive]` taxonomy with `InvalidSloDefinition` / `Audit` / `Dispatcher(SloPagerDutyDispatchError)` / `Internal` mutex poisoning + `SloPagerDutyDispatchError` `#[non_exhaustive]` `Transport` arm); `lib` (re-exports + `slo_schema_version()` const fn = 1). Tests: **83 tests across all targets, 0 failures, parallel-safe**: 68 inline lib unit (alert 12 + audit 6 + calculator 11 + decision 7 + definition 7 + error 5 + pagerduty 11 + window 7 + integration tests at boundary) + 15 prop_slo including the 7 required property tests at 10k iter PR-gate (`PROPTEST_CASES` env var override at runtime per S-07 P1-2 fix): `prop_burn_rate_calculation_correct` (for any (errors, total) with total>0, error_rate = errors/total within 1e-12 tolerance), `prop_alert_decision_canonical_table4` (Google SRE Workbook Ch 5 Table 4 boundary discipline; for every (sli, window, target, sample) tuple the decision matches Table 4 exactly), `prop_long_burn_does_not_page` (Long3d × 1× arm always Quiet or TicketSev3, never any Page* arm — Google SRE canonical), `prop_fast_burn_pages_immediately` (Fast1h × 14.4× arm above threshold always PageSev0; below threshold Quiet — Google SRE canonical), `prop_idempotent_no_burn` (error_rate=0 always Quiet regardless of window/SLO), `prop_tenant_isolation` (per-tenant ledger never leaks across tenants under deterministic seeded `ChaCha20Rng::seed_from_u64` sweep; INV-TENANT-ISOLATION canary), `prop_audit_emit_per_decision_arm` (canonical envelope: quiet=2 audits / non-quiet=3 audits with first always BurnRateEvaluated, second AlertQuiet OR AlertFired by predicate, third PageDispatched OR TicketFiled by predicate), `prop_pagerduty_dispatch_idempotent_dedup_key` (repeated page evaluation under same (sli, window, tenant) tuple collapses to single open incident on dispatcher side per PagerDuty Events API v2 §dedup_key contract — 8 property tests total counting boundary canonical-table4 separately). Plus surface-pinning regression tests covering canonical 7 SLIs + canonical 4 burn rate windows + canonical 5 alert decisions + canonical 5 audit event strings + canonical 3 PagerDuty actions + canonical 3 service keys + canonical Google SRE Table 4 multipliers (14.4×/6×/3×/1×) + slo_schema_version(). Alert YAML `dashboards/alerts/dash-slo-multi-burn.yml`: 7 SLI groups × 4 alert rules per Google SRE Workbook Ch 5 Table 4 = 28 alerts + 7 recording rules = 35 total rules; every alert has canonical `severity` label (sev0/sev1/sev2/sev3 per `observability_model.md §3.1` SEV taxonomy + WI §6.1.2), `slo` label, `burn_rate_window` label, mandatory `annotations.runbook_url` deep-link, `annotations.summary`, and `annotations.description`; ManualOverride excluded from SLI-RATE-LIMIT-WITHIN-QUOTA recording rule via `trip_reason!="ManualOverride"` PromQL filter per Lote 10.8bis P1-NEW-3 inheritance from WI-S08-005. **Trait-abstraction-defer per charter**: real PagerDuty Events API v2 HTTPS POST `/v2/enqueue` with bound `routing_key` Worker secret + `promtool test rules dashboards/alerts/dash-slo-multi-burn.yml` CI gate at `.github/workflows/alert-rules-validate.yml` + auto-quarantine flapping cron weekly `sum_over_time(increase(ALERTS_FOR_STATE{alertstate="firing"}[5m])[7d:5m]) > 3` (Lote 10.9bis P1 R5 P1-6 ALERTS ephemeral → ALERTS_FOR_STATE persistent counter) + Twilio fallback SMS + Terraform IaC for 3 PagerDuty services + 100k nightly via `PROPTEST_CASES=100000` + chaos suite 10 + SLO coverage CI hook + runbook coverage CI hook + per-runbook RB-SLO-*.md content — all consolidated alongside WI-S09-007 PRR ship gate per `trait-abstraction-defer` charter pattern. Cross-module patches: workspace `Cargo.toml` adds `crates/corelink-slo` member + workspace dep (no `corelink-analytics` runtime dep — slo crate is pure-logic + audit + dispatcher trait surface; cross-crate exemplar binding via `corelink-analytics::RedMetricKind` is deferred to WI-S09-007 alongside the production PagerDuty wiring). Quality gates verde: `cargo test -p corelink-slo --all-targets` 83 tests 0 failures debug + release; `cargo clippy --workspace --all-targets --features corelink-worker/tower-middleware -- -D warnings` clean; `validate_specs.py` clean (279 schema + 6 YAML = 285 docs); `check_migrations_additive.py` clean (16 migrations; no new migration this WI per `trait-abstraction-defer`); YAML parse clean (28 alerts + 7 records = 35 rules verified). No per-WI codex per 2026-04-30 protocol; sprint-close Sonnet review covers full S-09 corpus. |
