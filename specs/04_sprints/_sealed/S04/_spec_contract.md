...
| 1.9.0 | 2026-05-01 | Gustavo (via Claude Opus 4.7 1M) | **WI-S04-006 SEALED** — final S-04 WI; ship-gate Lote. Ship list: (a) **Cross-component property suite** `crates/corelink-worker/tests/prop_ac_full.rs` — 4 release-mode properties at 1 × 100k SHIP-GATE (`prop_ac_full_stack_tenant_isolation_100k` exercises INV-AC-TENANT-SCOPED across handler+meta+envelope+merkle+sig+outputs+audit+ttl pipeline; 0 violation tolerated per WI §10.s04.006.2) + 3 × 10k PR (`prop_ac_full_stack_idempotent_under_concurrent_update` for INV-AC-IDEMPOTENT + INV-AC-RESULT-HASH-IMMUTABLE + INV-AC-CANONICAL-BYTES-STABLE; `prop_ac_full_stack_negative_cache_invalidation_on_update` for cross-tenant neg-cache non-aliasing; `prop_ac_full_stack_ttl_eviction_tenant_scoped` for INV-AC-EVICT-TENANT-SCOPED at the cron seam); 130 000 iter PR; release-mode 9.2 s wall-clock. (b) **REAPI v2 conformance subset** `crates/corelink-worker/tests/reapi_v2_ac_conformance.rs` — 10 pinned tests against ADR-0036 §Annex A canonical AC subset: 4 GetActionResult (hit / miss / cross-tenant / sig roundtrip) + 6 UpdateActionResult (success / idempotent refresh / outputs missing 422 / scope insufficient / region mismatch / result_hash immutable on idempotent refresh); 100% pass non-negotiable per WI §6.1.1 + spec contract §19; **REAPI v2 has NO BatchUpdateActionResult** (Lote 10.4bis P0 fix #5). (c) **DASH-AC dashboard** `dashboards/grafana/DASH-AC.json` — 8 canonical panels per WI §6.1.2 (cache hit ratio business metric per `tenant_tier × region` with SLO baseline ≥ 70%; latency p99 GET/UPDATE warm+cold per region heatmap; tenant isolation cross-tenant alert stat must always be 0; sig invalid rate alert ≥ 5/h; TTL eviction rate per region; negative cache hit/populate/invalidate rates; REAPI conformance status pass/fail stat; cost per-op tracker with ±10% threshold). (d) **Alert rules** `dashboards/alerts/dash-ac-alerts.yml` — 11 rules: SEV-0 (`AC_CrossTenantBreach`, `AC_SigInvalidSustained`, `AC_ConformanceRegression`, `AC_CacheHitRatioDrop`); SEV-1 (`AC_SLOAvailabilityBreach`, `AC_LatencyWarmP99Breach`); SEV-2 (`AC_OutputsValidDrift`, `AC_TtlEvictionStalled`, `AC_TtlEvictionStorm`, `AC_CostRegressionGate`). PagerDuty + Slack routing per WI §6.1.2. (e) **RB-FM-303 host-side dry-run** `scripts/rb_fm_303_dry_run.sh` — cargo-driven walkthrough of 8 runbook steps (Detection / Communication / Mitigação imediata / Diagnóstico / Resolução / Forensics + Notificação / Post-mortem hooks / Drift detection); EVT-017 evidence summary captured; full staging dry-run with chaos PR + on-call drill deferred until staging account provisioned per charter `trait-abstraction-defer` pattern. (f) **Audit trail** — `specs/_audits/sealed/2026-05-01-rb-fm-303-dry-run.md` + `specs/_audits/sealed/2026-05-01-pentest-s04-internal.md` (six attack surfaces audited: REAPI surface / D1+R2 schema / Merkle codec / HKDF sig / TTL cron / audit chain; zero HIGH/CRITICAL findings) + `specs/_audits/sealed/2026-05-01-adversarial-s04.md` (50+ adversarial scenarios catalogued across WI-S04-001..006 with > 270 000 release-mode iter exercised at SEAL). (g) **OWASP ASVS V5/V6/V8/V10/V14 self-checklist** `specs/04_sprints/_sealed/S04/asvs-v5-v6-v8-v10-v14-checklist.md` — 67 PASS / 1 WAIVED (V8.3.8 customer-facing DPA disclosures — S-19 onboarding scope) / 14 N/A; revalidation triggers pinned. (h) **PRR-S04 canonical doc** `specs/04_sprints/_sealed/S04/PRR-S04.md` (`doc_status: FROZEN`, `work_status: APPROVED`, `audit_status: AUDITED`) — 11 sign-offs canonical (5 ✅ APPROVED [Owner / Final Approver / Architect (incl. Crypto SME specialization for HKDF + Merkle + canonical preimage + key rotation methodology) / Engineer / Privacy] + 6 ⚠️ WAIVED via ADR-0034 dual-hat [Security Lead / SRE Lead / QA Lead / Compliance / AppSec] each with explicit revalidation triggers); promotion decision **PROMOTE TO STAGING-STABLE**; residual risk register 13 rows all LOW (or NONE for the closed F-001 carry-over); outbound dependencies cleared (S-07 / S-09 / S-13 / S-14 / S-15 / S-19 / S-20). DEFERRED items: 72h SLO sustained staging + real Cloudflare R2/D1/KV/Cron-DO bindings + real Bazel/Buck2 client integration smoke + customer-facing communication (SLA addendum / release notes / Bazel onboarding doc) — all forward-looking gates with explicit revalidation triggers; none blocks S-04 SEAL per spec contract §6 partial-bullet pattern + charter `trait-abstraction-defer` pattern. Quality gates verde: full workspace `cargo test --workspace --all-targets --features corelink-worker/tower-middleware` 0 failures (debug + release); `cargo clippy --workspace --all-targets --features corelink-worker/tower-middleware -- -D warnings` clean; `python3 scripts/validate_specs.py` clean; `python3 scripts/validate_references.py` no new dangling refs; `python3 scripts/check_migrations_additive.py` clean; `bash scripts/rb_fm_303_dry_run.sh` exit 0. **S-04 implementation phase: 6 of 6 WIs SEALED**; sprint-close Sonnet adversarial review will run after this SEAL. |
| 1.8.0 | 2026-05-01 | Gustavo (via Claude Opus 4.7 1M) | **WI-S04-005 SEALED** — TTL infrastructure (Cron Durable Object pure-logic core + refresh-on-hit threshold gate + tenant-scoped batched eviction + `TierTtlResolver` boundary trait per ADR-0019 + RB-FM-303 forward defense-in-depth). Ship list: (a) **`crates/corelink-worker/src/reapi/ac/ttl/`** — 4 sub-modules per `mod_module_files = "deny"`: `ttl.rs` parent + `ttl/resolver.rs` (`TierTtlResolver` trait + `TenantTier` 5-variant enum + `EnvConfigTierTtlResolver` S-04 GA fallback + `MockTierTtlResolver::s07_canonical()` returning the post-S-07 7d/30d/90d/365d/365d profile per ADR-0019; `DEFAULT_TIER_TTL_MS` aliased to handler `DEFAULT_AC_TTL_EXTEND_MS`) + `ttl/refresh.rs` (`refresh_if_needed(now_ms, last_hit_at_ms, threshold)` pure-logic predicate; `DEFAULT_REFRESH_THRESHOLD_MS = 60_000` per WI §6.1.3; saturating-sub guard against NTP rollback) + `ttl/evict.rs` (`EvictBatch::run_one_batch(tenant_id, now_ms, limit, request_id)` tenant-scoped batched eviction in canonical R2-DELETE → D1-DELETE → audit-emit → KV-invalidate order; `MAX_BATCH_SIZE = 250` D1-100KB-aligned ceiling per WI Lote 10.4bis P0; `EvictRowOutcome` 5-variant per-row enum + `EvictBatchOutcome` aggregate counters + `hit_cap()` storm signal; `EvictError::BatchSizeExceeded` defense-in-depth on caller-cap violation; `EvictError::RegionMismatch` defense-in-depth on per-row region check) + `ttl/worker.rs` (`TtlWorker` trait + `InMemoryTtlWorker` fake — per-charter trait-abstraction-defer pattern; round-robin tenant sweep + per-tick row ceiling + `TtlWorkerTickOutcome::hit_row_ceiling` storm signal; real Cloudflare Cron DO binding shim deferred to WI-S04-006). (b) **`AcMetaStore` trait extension** — 3 new methods: `select_expired_for_region(region, tenant_id, now_ms, limit)` (tenant + region scoped; ASC by `expires_at`); `tenants_with_expired(region, now_ms)` (DISTINCT enumeration); `delete_tenant_scoped(tenant_id, action_digest, region)` (defense-in-depth re-asserts region match; trait surface intentionally has NO "DELETE WHERE expires_at < ?" bulk method per `INV-AC-EVICT-TENANT-SCOPED`). (c) **`AcEventType` audit taxonomy extension** — 3 new variants: `EvictTtlExpired` (`corelink.ac.evict.ttl_expired`) + `EvictR2Failed` + `EvictD1Failed`; total 11 canonical event types. (d) **Property tests** (release-mode 10k iter PR per WI §10.s04.005.1): 6 new `tests/prop_ac_ttl.rs` cases — `prop_refresh_threshold_gates_d1_writes`, `prop_ttl_tenant_isolation_under_sweep`, `prop_evict_idempotent_across_repeated_ticks`, `prop_evict_region_pinning_holds`, `prop_select_expired_bounded_by_limit_and_orderly`, `prop_refresh_extends_expires_at_monotonic` — **60 000 iter PR**. (e) **Lib unit tests**: 26 new tests across `meta.rs` + `audit.rs` + `ttl/{resolver,refresh,evict,worker}.rs`; full `cargo test -p corelink-worker --features tower-middleware --lib` → **214 passed / 0 failed** (was 188 pre-WI). (f) **Runbooks**: `RB-FM-AC-TTL-DRIFT.md` v0.1.0 (P2; cron stale / not firing) + `RB-FM-AC-TTL-STORM.md` v0.1.0 (P2; sustained workload outpaces cron interval). (g) **`RB-FM-303` v0.2.0** — added defense-in-depth section documenting that `INV-AC-EVICT-TENANT-SCOPED` makes cross-tenant DELETE structurally unreachable + AC TTL cron disable step in immediate mitigation flow. (h) **`ADR-0019` v1.1.0** ratified post-implementation — new §Boundary mechanism + §TTL infrastructure delivery sections documenting the runtime trait surface (`TierTtlResolver` swap point) + S-04 / S-07 / S-13 separation of concerns. **Cross-WI patch (charter §inflection)**: `crates/corelink-audit/tests/prop_audit.rs::prop_no_raw_pii_in_canonical_bytes` had a pre-existing alphabet collision bug — random suffix `[a-z0-9]{4,12}` could shrink to `regi` (substring of envelope field name `region`) producing a false-positive PII leak; fixed by restricting alphabet to `[bcfghjkpqvwxz1-9]{6,12}` (excludes every character used in canonical envelope field names). Lesson logged in WI §31 changelog (cross-WI patch into SEALED crate per charter "Real bug detected; don't paper over"). **Deferred per charter trait-abstraction-defer**: real Cloudflare Cron Durable Object binding shim (alongside WI-S04-006 conformance suite + miniflare/wrangler-dev integration); real R2 envelope `delete` adapter; `S07PerTierTtlResolver` real impl (lands when S-07 SEALED); migration worker emitting `corelink.ac.tier_migration` audit events (S-13 owned per ADR-0019 §Migration plan cycle 4); 100k nightly property iter; cargo-fuzz nightly target; criterion p99 ≤ 10ms refresh-on-hit benchmark + ≤ 30s cron tick benchmark (alongside conformance suite). Quality gates verde: `cargo test -p corelink-worker --features tower-middleware --lib` 0 failures; `cargo test -p corelink-worker --features tower-middleware --test prop_ac_ttl` 6 properties × 10k iter green; `cargo test -p corelink-ac-schema --all-targets` 0 failures; `cargo clippy --workspace --all-targets --features corelink-worker/tower-middleware -- -D warnings` clean; `python3 scripts/validate_specs.py` clean; `python3 scripts/validate_references.py` no new dangling refs. |
