...
| 1.3.0 | 2026-05-01 | Gustavo (via Claude Opus 4.7 1M) | **WI-S04-005 SEALED — TTL infrastructure phase**. New module `crates/corelink-worker/src/reapi/ac/ttl/` (4 sub-modules per `mod_module_files = "deny"`: `ttl.rs` parent + `ttl/resolver.rs` (`TierTtlResolver` trait + `TenantTier` 5-variant enum + `EnvConfigTierTtlResolver` S-04 GA fallback + `MockTierTtlResolver::s07_canonical()` returning the post-S-07 7d/30d/90d/365d/365d profile per ADR-0019; `DEFAULT_TIER_TTL_MS` aliased to handler `DEFAULT_AC_TTL_EXTEND_MS`) + `ttl/refresh.rs` (`refresh_if_needed(now_ms, last_hit_at_ms, threshold)` pure-logic predicate; `DEFAULT_REFRESH_THRESHOLD_MS = 60_000` per WI §6.1.3; saturating-sub guard against NTP rollback) + `ttl/evict.rs` (`EvictBatch::run_one_batch(tenant_id, now_ms, limit, request_id)` tenant-scoped batched eviction in canonical R2-DELETE → D1-DELETE → audit-emit → KV-invalidate order; `MAX_BATCH_SIZE = 250` D1-100KB-aligned ceiling per WI Lote 10.4bis P0; `EvictRowOutcome` 5-variant per-row enum + `EvictBatchOutcome` aggregate counters + `hit_cap()` storm signal; `EvictError::BatchSizeExceeded` defense-in-depth on caller-cap violation; `EvictError::RegionMismatch` defense-in-depth on per-row region check) + `ttl/worker.rs` (`TtlWorker` trait + `InMemoryTtlWorker` fake — per-charter trait-abstraction-defer pattern; round-robin tenant sweep + per-tick row ceiling + `TtlWorkerTickOutcome::hit_row_ceiling` storm signal; real Cloudflare Cron Durable Object binding shim deferred to WI-S04-006). **`AcMetaStore` trait extension**: 3 new methods — `select_expired_for_region(region, tenant_id, now_ms, limit)` (tenant + region scoped; ASC by `expires_at`); `tenants_with_expired(region, now_ms)` (DISTINCT enumeration); `delete_tenant_scoped(tenant_id, action_digest, region)` (defense-in-depth re-asserts region match; trait surface intentionally has NO "DELETE WHERE expires_at < ?" bulk method per `INV-AC-EVICT-TENANT-SCOPED`); 6 new in-memory fake tests pinning the canonical filter / ordering / region-pinning / idempotent / no-op-on-missing semantics. **Audit taxonomy extension**: 3 new `AcEventType` variants — `EvictTtlExpired` (`corelink.ac.evict.ttl_expired`) + `EvictR2Failed` + `EvictD1Failed`; total 11 canonical event types; updated unit tests (`each_event_type_has_unique_canonical_string`, `sev1_set_is_sig_invalid_only`). **Property tests** (release-mode 10k iter PR per WI §10.s04.005.1): 6 new `tests/prop_ac_ttl.rs` cases — `prop_refresh_threshold_gates_d1_writes` (pure-logic threshold predicate), `prop_ttl_tenant_isolation_under_sweep` (cross-tenant DELETE structurally impossible), `prop_evict_idempotent_across_repeated_ticks` (chaos-recovery pre-condition), `prop_evict_region_pinning_holds` (per-region shard contract), `prop_select_expired_bounded_by_limit_and_orderly` (canonical SELECT shape), `prop_refresh_extends_expires_at_monotonic` (`INV-AC-TTL-MONOTONIC` at meta-store level); **60 000 iter PR**. **Lib unit tests**: 26 new tests across `meta.rs` (6: select / tenants_with_expired / delete idempotent / cross-tenant / region-mismatch) + `audit.rs` (extended for new variants) + `ttl/resolver.rs` (4) + `ttl/refresh.rs` (5) + `ttl/evict.rs` (6) + `ttl/worker.rs` (5); `cargo test -p corelink-worker --features tower-middleware --lib` → **214 passed / 0 failed** (was 188 before this WI). **Runbook stubs**: `RB-FM-AC-TTL-DRIFT.md` (P2; cron stale / not firing detection + manual mitigation + dry-run deferred to WI-S04-006) + `RB-FM-AC-TTL-STORM.md` (P2; sustained workload outpaces cron interval; `hit_row_ceiling` signal). **`RB-FM-303` (`RB-FM-303-ac-cross-tenant`) v0.2.0** updated with WI-S04-005 forward defense-in-depth section: `INV-AC-EVICT-TENANT-SCOPED` makes cross-tenant DELETE structurally unreachable; the only legal DELETE is `delete_tenant_scoped(tenant_id, action_digest, region)`; sibling runbooks cross-referenced; AC TTL cron disable step added to immediate mitigation flow. **`ADR-0019` v1.1.0**: ratified post-implementation; new §Boundary mechanism + §TTL infrastructure delivery sections documenting the runtime trait surface (`TierTtlResolver` swap point) + S-04 / S-07 / S-13 separation of concerns. **Cross-WI patch (charter §inflection)**: `crates/corelink-audit/tests/prop_audit.rs::prop_no_raw_pii_in_canonical_bytes` had a pre-existing alphabet collision bug — random suffix `[a-z0-9]{4,12}` could shrink to `regi` (substring of envelope field name `region`) producing a false-positive PII leak; fixed by restricting the alphabet to `[bcfghjkpqvwxz1-9]{6,12}` (excludes every character used in canonical envelope field names: vowels + 0/l + n/r/s/t/d). Lesson: property-test alphabets that overlap with envelope field names produce structural false-positives masquerading as PII leaks; future PII-leak proptests must restrict the alphabet to characters disjoint from the canonical JSON field-name corpus. **Quality gates verde**: `cargo test -p corelink-worker --features tower-middleware --lib` 0 failures; `cargo test -p corelink-worker --features tower-middleware --test prop_ac_ttl` 6 properties × 10k iter green; `cargo test -p corelink-ac-schema --all-targets` 0 failures; `cargo clippy --workspace --all-targets --features corelink-worker/tower-middleware -- -D warnings` clean; `python3 scripts/validate_specs.py` clean; `python3 scripts/validate_references.py` no new dangling refs; `python3 scripts/check_migrations_additive.py` clean (no migrations touched). **Deferred per charter trait-abstraction-defer**: real Cloudflare Cron Durable Object binding shim (alongside WI-S04-006 conformance suite + miniflare/wrangler-dev integration); real R2 envelope `delete` adapter; `S07PerTierTtlResolver` real impl (lands when S-07 SEALED); migration worker emitting `corelink.ac.tier_migration` audit events (S-13 owned per ADR-0019 §Migration plan cycle 4); 100k nightly property iter; cargo-fuzz nightly target; criterion p99 ≤ 10ms refresh-on-hit benchmark + ≤ 30s cron tick benchmark (alongside conformance suite). |
