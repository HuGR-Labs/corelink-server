...
| 1.4.0 | 2026-05-01 | Gustavo (via Claude Opus 4.7) | **S-03 IMPLEMENTATION SPRINT SEALED** (sprint-close ceremony). Sonnet independent adversarial review post WI-S03-008 SEAL: **8.6/10 PASS** (no P0; 6 P1 all addressed in this Lote). P1-1 + P1-4 + P1-5: doc_status sweep — `sprint.md` → FROZEN/DONE/SEALED v1.1.0; `_spec_contract.md` (this doc) → FROZEN/SEALED v1.4.0; `specs/_audits/2026-05-01-{adversarial-s03,pentest-s03-internal,rb-fm-160-dry-run}.md` → FROZEN/SEALED v1.1.0; `specs/04_sprints/_sealed/S03/asvs-v2-v3-v4-v6-v8-checklist.md` → FROZEN/SEALED v1.1.0. P1-2: 13/13 sprint.md DoD items checked off with evidence rows or DEFERRED with explicit revalidation trigger. P1-3: SBOM CycloneDX 1.5+ signed deferred to S-20 GA gate (added to PRR-S03.md §3 DoD table as ⚠️ DEFERRED row); rationale — S-01 ships baseline SBOM, S-03 adds no supply-chain divergence material, no customer-facing supply-chain commitment until GA. P1-6: `crates/corelink-pat/src/argon.rs::argon2_hasher` `Params::default()` silent fallback replaced with `unreachable!` carrying explicit upstream-API-drift diagnostic — refuses silent underprovisioning of the cold-path pad below OWASP-2024 floor. Quality gates verde across the post-fix re-validation sweep. **S-03 implementation phase 8/8 WIs SEALED** (`3f03796` 003 / `603b13e` 004 / `585a2e8` 005 / `31e7e8c` 006 / `3296f86` 007 / `69cea46` 008 + earlier S-03 cycle commits for 001/002). Outbound deps cleared per PRR-S03.md §9: S-04 (AC tenant-scoped keying), S-08 (rate limit per-PAT/tenant), S-09 (audit chain + EVT-047 emission), S-10 (Stripe org→customer derivation), S-13/S-16 (admin self-service via WebAuthn step-up), S-14 (BYOK + DPA), S-19 (onboarding via PRR ship gate), S-20 (GA via external pentest closing ASVS WAIVED items). |
| 1.3.3 | 2026-05-01 | Gustavo (via Claude Opus 4.7) | **WI-S03-008 SEALED** (charter `2026-04-30 protocol`: no per-WI codex; sprint-close Sonnet adversarial review covers). Property test suite `crates/corelink-worker/tests/prop_auth_full.rs` lands 4 cross-component release-mode tests at 10k iter (no-alg-none acceptance / 5-layer defense propagation / revocation race full-stack / Argon2 calibration stable) gated behind `tower-middleware`. DSR PAT export integration coverage in `crates/corelink-auth-schema/tests/integration_dsr_pat_export.rs` (cascade + pseudonym preservation + cross-tenant + surface-shape). Internal pentest report `specs/_audits/sealed/2026-05-01-pentest-s03-internal.md` documents zero HIGH/CRITICAL with one MEDIUM (FM-249 follow-up). RB-FM-160 host-side dry-run automated `scripts/rb_fm_160_dry_run.sh` (cargo-driven, drift-detectable) + audit `specs/_audits/sealed/2026-05-01-rb-fm-160-dry-run.md`. OWASP ASVS V2/V3/V4/V6/V8 self-checklist `specs/04_sprints/_sealed/S03/asvs-v2-v3-v4-v6-v8-checklist.md` published with WAIVED items revalidation-bound to S-08/S-13/S-19/S-20. Adversarial summary `specs/_audits/sealed/2026-05-01-adversarial-s03.md`. Constant-time variance gate in `corelink-client-verify` hardened to outlier-robust 10/80/10 trimmed-mean estimator (preserves leak-detection sensitivity while rejecting wall-clock contention spikes). All quality gates verde: 0 failures across 75 test groups; clippy `-D warnings` clean; validators clean (no new dangling refs). |
