...
0. **Commit-before-scan ordering invariant** (R4-P1-002-1 elevation, 2026-05-15; operational sub-property of `INV-GC-MARK-STARTED-AT-ATOMIC` per registry §3.17): `mark_started_at_ms` is durably committed to D1 (commit ack received from D1 driver) BEFORE the first batch read in `pass_blob_meta` / `pass_ac_meta` / `pass_manifest_chunks` executes. This is the explicit Rust-impl-level statement of the TLA+ obligation `GCMarkStart` strictly preceding any `GCMarkStep`; the operational guarantee is that any observer of D1 (including a concurrent UpdateActionResult writer) seeing the scan reads will already see `mark_started_at_ms` set (TLA+ canonical protect-if-equal-or-newer at `ac.created_at_ms >= mark_started_at_ms` L152-154). The property-test pin `prop_mark_anchor_commits_before_first_batch_read` (forward-deferred to S-20 GA gate per debt register cumulative-track row DEBT-S06-P1-002-1 — see `2026-05-15-debt-register.md`) cross-validates the WAL/D1 commit ordering at impl level; the operational implementation at SEAL ships the `GcRunStore::transition_phase` immutable-set guard which atomically captures the anchor before the orchestrator invokes any scan pass (see `crates/corelink-gc/src/mark.rs::InMemoryMarkPhase::execute`). Cross-ref: R4-Opus-Part1 §3.2 P1-002-1; `PRINC-TLA-001`; canonical registry entry `INV-GC-MARK-STARTED-AT-ATOMIC` §3.17.
