...
| S15-D6 | Ship gate — cargo-fuzz 1M + Apple notarize + Authenticode + 2 OSS proof-of-conversion + PRR STANDARD | `fuzz/fuzz_targets/cli_input.rs` + `.github/workflows/notarize.yml` + `examples/case-studies.md` + `specs/04_sprints/S15/PRR-S15.md` | cargo-fuzz 1M iter green (0 panics, 0 secrets leaked); macOS notarized + Linux GPG + Windows Authenticode (or waiver+ADR se cert slip); 2 OSS engaged + case-study docs; PRR STANDARD 5-8 sign-offs canonical |
