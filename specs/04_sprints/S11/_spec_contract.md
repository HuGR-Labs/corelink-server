...
- **R-S11-1**: Endpoints DSR (todos requerem auth via PAT + **CTRL-AUTH-010 step-up MFA WebAuthn re-auth** — canonical em `security_model.md §242`; `CTRL-PRIV-016` aplica ao caso "support read access requer consent" e NÃO substitui CTRL-AUTH-010). SLAs por direito conforme `privacy_model.md §6.1` (clock semantics F-11):
  ...
  - `POST /v1/privacy/dsr/consent_revoke` → SLA **≤ 5min** (LGPD Art. 18 IX — revogação do consentimento, c/c Art. 8 §5; LGPD Art. 18 VI — eliminação consequente dos dados tratados com consentimento; GDPR Art. 7.3 — withdraw consent; Lote 10.11.0-ter legal-citation re-validation 2026-05-15 corrigiu atribuição anterior "Art. 18 VI" para incluir Art. 18 IX que é o direito de revogação per se, conforme `LGPD-FULL-AUDIT-2026-05-15.md §1.8`); revoga consents granted; cascade unsubscribe; aplica `CTRL-PRIV-CONSENT-002`.
...
- **R-S11-17**: Worker routing: requests roteados para region pinned via custom domain mapping (`<tenant_id>.<region>.corelink.humangr.com` onde `<region>` é canonical 6-code); cross-region queries denied 403 com `Reason: residency-violation`. Reuse routing layer de S-09 multi-region observability.
...
Companion closure doc: `specs/_audits/sealed/2026-05-15-s11-truth-table-sweep-v2.md`.
...
Implementation phase landing (no spec changes; impl-only changelog row per charter §spec contract changelog discipline). WI-S11-006 SEALED at the canonical impl quality gates:

- **`crates/corelink-privacy-breach-emit/`** new small crate (~850 LOC; 4 src modules + 2 test files). `BreachSeverity` `#[non_exhaustive]` 3-arm enum (Sev1/Sev2/Sev3) + `NotificationJurisdiction` `#[non_exhaustive]` 3-arm enum (Anpd/IrishDpc/CaliforniaAg) + `CustomerLocale` `#[non_exhaustive]` 3-arm enum (PtBr/EnUs/EsMx). `BreachNotificationDispatch` canonical audit payload (breach_id + severity + jurisdictions_notified + customer_notifications_sent + customer_locales + ts_ms + retry_attempt + breach_detected_at_ms). `BreachAuditSink` trait + `InMemoryBreachAuditSink` + `FailingBreachAuditSink`. `escalation_policy_for(severity)` deterministic fn → canonical PagerDuty service + ordered escalation steps. 4 Prometheus metric name constants. `within_72h_sla()` helper. Per-instance `Arc<Mutex<>>` F-001 closure. wasm32-unknown-unknown clean.
- **`legal/breach-notification/`** 7 files: `rb-breach-notif-decision-tree.yaml` (canonical SEV-1/2/3 × jurisdiction → notify-whom-when) + 3 jurisdictional templates (`lgpd-anpd-template.pt-br.md`, `gdpr-irish-dpc-template.en.md`, `ccpa-state-ag-template.en.md`) + 3 customer notification MJML templates (pt-BR/en-US/es-MX locales).
- **`legal/breach-notification/dry-run-scenarios/`** 3 scenarios: PII-leak-via-log (SEV-2; Q1 cadence), R2-cross-tenant-exposure (SEV-1; Q3 cadence), audit-chain-integrity-break (SEV-3 near-miss; ad-hoc onboarding).
- **`specs/05_quality/runbooks/RB-BREACH-NOTIF.md`** production-grade expansion (Lote 5.12 stub → S-11 production-grade): 8 sections (decision tree summary + severity classification + notification flow per jurisdiction + templates fill-in instructions + dry-run procedure + post-mortem hooks + status page banner + PagerDuty escalation matrix) + Appendices (contacts + regulatory references + metrics).
- **`specs/03_architecture/adrs/ADR-S11-009-breach-notification-3-jurisdictional-coverage-rationale.md`** NEW: 3-jurisdiction coverage rationale (BR + EU + US-CA; UK ICO + other state AGs + India DPDPA + China PIPL deferred post-GA per quarterly review trigger).
- **`schemas/cloudevents/breach-notification-dispatched.v1.json`** NEW: `dev.hugr.corelink.breach.notification_dispatched.v1` JSON Schema (CloudEvents 1.0 envelope + typed data: breach_id + severity enum + jurisdictions_notified array + customer_notifications_sent + customer_locales + ts + retry_attempt + breach_detected_at + dispatch_to_notification_seconds).
- **Audit fail behavior canonical**: dispatch priority over audit completeness per WI-S11-006 §9.3 DD — distinct from billing fail-OPEN (Lote 10.6bis split-tier); breach is documented-priority with SEV-1 alert + idempotent retry via `breach_id` + `retry_attempt` counter. NOT silent-skip. `regression_audit_fail_closed_behavior` tests pin `state_unchanged_when_audit_emit_fails` (S-06 P0-2 / S-07 P1-1 lessons absorbed).
- **Quality gates verde**: 34 tests across 3 test files all green (25 lib unit + 7 regression + 2 property); `PROPTEST_CASES=10000 cargo test --test prop_breach_emit_escalation_matrix` green; `cargo clippy --all-targets -- -D warnings` clean; `cargo build --target wasm32-unknown-unknown -p corelink-privacy-breach-emit` clean.
- **Charter constraints** verified: no `unsafe`; no `unwrap`/`expect`/`panic`/indexing in lib code; F-001 `Arc<Mutex<>>` per-instance closure; `#[non_exhaustive]` on all 3 public enums; wasm32-clean (no ring / no C dep / no getrandom direct dep); PROPTEST_CASES env-var override at runtime; no `prop_assert!(matches!(...))` anti-pattern (S-08 P1-1) — every property assert lifts match into separate `let` binding then `prop_assert!`.

### v1.9.0 (2026-05-13) — WI-S11-005 implementation phase SEALED
...
- **JWT receipt anti-replay**: 90d expiration cap per `RECEIPT_EXPIRY_DAYS` constant; `iss = corelink.humangr.com/privacy`; alg = RS256 pinned. In-memory deterministic FNV-1a-based fake covers the canonical sign/verify roundtrip + cross-key reject + tampered + expired arms; production wiring at WI-S11-008 substitutes `jsonwebtoken = 9.3` (S-03 inheritance) + KMS-backed key rotation (HKDF info=`corelink/v1/dsr-receipt`).
