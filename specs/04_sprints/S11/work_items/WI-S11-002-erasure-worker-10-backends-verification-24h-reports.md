...
> under wave-16.
>
> **Wave-17 closure note (2026-05-15):** scheduler-binding wired
> wave-17 (commits `49901da` + `dd58b27`). The native publish
> scheduler (`crates/corelink-dsr-statuspage-scheduler`) composes the
> wave-16 layers into a single 24h-cron-firable orchestration with
> per-day `(date, metric_id)` idempotency dedupe (`CronRunLog`) +
> a four-event scheduler audit envelope
> (`statuspage_publish_{scheduled,succeeded,failed,skipped}.v1`).
> The wasm32 `#[event(scheduled)]` entry point landed at
> `corelink-clerk-cf::dsr_statuspage_cron` with `wrangler.toml`
> `[triggers] crons = ["0 6 * * *"]` (06:00 UTC daily). The wave-17
> wasm32 handler short-circuited with the canonical
...
|---|---|---|---|
| 1.2.0 | 2026-05-07 | Opus 4.7 (autonomous builder) | **SEALED — implementation complete**. Created `corelink-privacy-pseudonymize` (canonical sha256 helper + verify_pseudonym + pii_redacted marker; ~430 LoC; wasm32-clean) + `corelink-privacy-erasure-worker` (~6500 LoC) with: `BackendKind` 12-arm `#[non_exhaustive]` enum (8 effective + 4 pseudonymized canonical), `BackendErasureOutcome` 5-arm enum, `ErasureDecision` 6-arm enum, `ErasureCloudEventType` 5-arm canonical CloudEvents type taxonomy (`dev.hugr.corelink.dsr.erasure.{started,backend_completed,verification_passed,verification_failed,completed}.v1`), `ErasureWorker` trait + `InMemoryErasureWorker` orchestrator with F-001 per-instance `Arc<Mutex<>>` closure + audit fail-CLOSED `audit→mutate` ordering (S-06 P0-2 / S-07 P1-1 absorbed), 12 `BackendErasureAdapter` trait + `InMemoryBackendErasureAdapter` (one per backend kind, refcount-aware R2 CAS for S-07 dedup safety, legal_hold gate for effective backends, pseudonymized backends preserve rows + insert canonical marker), `ErasureIdempotencyLedger` trait + `InMemoryErasureIdempotencyLedger` + `FailingErasureIdempotencyLedger` (canonical D1 `dsr_erasure_log` UNIQUE `(dsr_id, backend)` surface), `ErasureAuditSink` trait + `InMemoryErasureAuditSink` + `FailingErasureAuditSink`, `ReportSigner` trait + `InMemoryReportSigner` (BLAKE3-keyed MAC over JCS-canonical preimage), `VerificationJob` 24h sweep entry point + `VerificationOutcome` shape + `elapsed_dsr_ids` cron tick helper. Tests: 83 lib + 6 chaos + 2 integration + 5 prop_idempotency_replay_100x (PROPTEST_CASES env-runtime read per S-07 P1-2) + 6 prop_pseudonymization_correctness + 3 regression_refcount_aware_scrub + 3 regression_stripe_invoice_preserved + 5 verification_job_24h = **113 tests across 8 files**, all green; cargo clippy `--all-targets -- -D warnings` clean; cargo build `--target wasm32-unknown-unknown` clean for both crates. Artefacts: D1 migration `migrations/d1/0022_dsr_erasure_log.sql` (UNIQUE `(dsr_id, backend)` constraint + 5-arm CHECK + 12-arm backend CHECK + 3 indexes), 5 CloudEvents JSON schemas in `schemas/cloudevents/dsr-erasure-{started,backend_completed,verification_passed,verification_failed,completed}.v1.json`, ADR-S11-003 (erasure_salt interim) + ADR-S11-004 (cross-backend eventual consistency). FM-450 + FM-452 already declared in `failure_modes.md` (verified); RB-DSR-ERASURE-INCOMPLETE + RB-CONSENT-TAMPERING runbooks already exist (verified). Workspace `Cargo.toml` updated to register both new crates (alphabetic insertion). Salvageable archive `_archive/wi-s11-002-partial/` mined for taxonomy patterns (event/error/audit) but not blindly renamed — patterns adapted to spec's 5-event canonical CloudEvents prefix `dev.hugr.corelink.dsr.erasure.<arm>.v1` (vs archive's 8-event `corelink.erasure.<arm>` discrimination) and pseudonymize separated into its own crate per spec §6.3 mandate. |
