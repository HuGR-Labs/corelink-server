...
S-11 entrega 4 layers (Lote 10.11.0-bis-prime cycle 3 reconciled): (a) custom domain routing `<tenant_id>.<region>.corelink.dev`; (b) insert checks reject cross-region writes via D1 CHECK constraint + worker pre-flight assertion; (c) 20k property test (10k weur + 10k enam) → 0 cross-region leaks; (d) **TLA+ partial coverage via `dsr_erasure_atomicity.tla`** (InvResidencyPinned: ticket pinning canonical; InvResidencyMonotonic temporal: no cross-region migration). **FULL TLA+ proof deferred to S-14** (`region_residency.tla` adiciona backend region dimension + cross-region routing actions). S-11 cobertura sufficient para regulatory baseline + Schrems II defensibility (3-layer defense + 1-layer formal partial).
