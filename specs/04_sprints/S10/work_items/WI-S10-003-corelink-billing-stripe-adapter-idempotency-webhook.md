...
| 1.5.0 | 2026-05-16 | Gustavo (via Claude Opus 4.7; wave-19 R-PREP stripe-wasm32-gate-lift) | **§ closure-note — wasm32 gate lift (wave-19 R-PREP)**: `corelink-stripe-real` no longer carries a crate-root `#![cfg(not(target_arch = "wasm32"))]`. wasm32-safe surface (`webhook` signature verify + `webhook_dispatch` trait pipeline + `error` taxonomy + `retry` pure-logic + `dlq` types + `portal` types) now builds on `wasm32-unknown-unknown`. The native-only HTTPS client (`client.rs`, depends on `reqwest::blocking`) is per-module gated to `#[cfg(not(target_arch = "wasm32"))]`; `reqwest` workspace dep is gated to `[target.'cfg(not(target_arch = "wasm32"))'.dependencies]` so wasm32 builds skip TLS / blocking deps entirely. **Unblocks**: `cargo build --target wasm32-unknown-unknown -p corelink-billing-stripe-materializer` (wave-18 follow-on, was failing with `unresolved import corelink_stripe_real::webhook_dispatch`). **Pattern**: mirrors `corelink-cf-bindings` r2_real/d1_real/kv_real/do_real per-module gates documented in `specs/_audits/sealed/2026-05-15-cf-binding-real-pattern.md`. **Audit doc**: `specs/_audits/sealed/2026-05-16-stripe-wasm32-gate-lift.md`. **Gates green**: `cargo build --target wasm32-unknown-unknown -p corelink-stripe-real` + `cargo build --target wasm32-unknown-unknown -p corelink-billing-stripe-materializer` + `cargo build -p corelink-stripe-real` + `cargo test -p corelink-stripe-real` (51 lib + 12 integration, 0 failed) + `cargo clippy --workspace --all-targets -- -D warnings`. Charter compliance preserved: `#![forbid(unsafe_code)]` retained; no `unwrap`/`expect`/`panic` outside test code; no semantic change to webhook signature verify (`subtle::ConstantTimeEq` path untouched); no audit emit path touched; `#[non_exhaustive]` preserved on every public enum. Per `trait-abstraction-defer` charter, production callers of Stripe HTTPS still run from `apps/server` (native Rust); CF Worker (wasm32) callers consume the in-memory fake or proxy to the native worker — wasm32 build provides type-level visibility for the materializer crate without enabling network calls. Trivial: `corelink-stripe-real::client` re-export now `#[cfg(not(target_arch = "wasm32"))]` gated at lib.rs L48 (mirrors the module gate); STRIPE_API_BASE constant moves with the module. |
