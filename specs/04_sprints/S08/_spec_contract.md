...
| 1.6.0 | 2026-05-02 | Gustavo (via Claude Opus 4.7 1M; autonomous WI-S08-004 SEAL) | **WI-S08-004 SEALED — `crates/corelink-abuse/` v0.1.0 shipped + `migrations/d1/0013_abuse_scores.sql` (NEW table; durable per-tenant abuse score history with 4-feature breakdown + rendered decision tier; companion abuse_appeals + abuse_response_actions + abuse_calibration_weights deferred to WI-S08-006 PRR ship gate per `trait-abstraction-defer` charter pattern).** New crate ships the canonical heuristic abuse-detection plane (camada 4 of the 4-layer rate-limit bulkhead PAT-RATE-LIMIT-001; CAP-ABUSE-001 + CAP-ABUSE-002 implementation): 7 source modules (~3098 LOC src + ~1262 LOC tests = 4360 LOC total): `features` (AbuseFeatures FROZEN 4-feature canonical observation shape `cpu_wallclock_ratio` + `egress_bytes_per_min` + `action_digest_entropy_bits` + `concurrent_exec_count` per sprint contract §5 R-S08-7 + WI §6.1.3 with NaN-safe constructor); `score` (AbuseScore newtype clamped [0.0, 1.0] + AbuseDecision `#[non_exhaustive]` 3-arm gradient Benign/Suspicious/Malicious + canonical 4-feature weighted-sum compute_score with tier-aware baselines + decide threshold map at SUSPICIOUS_THRESHOLD=0.5 / MALICIOUS_THRESHOLD=0.8 per WI §1 invariant 3 with ≥ boundary semantics); `config` (AbuseConfig per-instance F-001 closure + AbuseFeatureWeights canonical 0.30/0.25/0.25/0.20 sum-to-one defensive validator + egress_baseline_for_tier / exec_baseline_for_tier 5-tier canonical baselines per Lote 10.7bis P0-7); `audit` (7-event taxonomy `corelink.abuse.{score_computed, decision_benign, decision_suspicious, decision_malicious, downgrade_applied, admin_review_triggered, suspend_applied}` `#[non_exhaustive]` + fail-closed envelope per `INV-AUDIT-EMIT-ATOMIC-WITH-HANDLER` + Lote 10.6bis pattern + S-07 sprint-close P1-1 fix); `metrics` (5-canonical metric ladder: `corelink.abuse.{score, tier_count_total, auto_suspend_attempts_total, downgrade_applied_total, cross_tenant_feature_leak_total}` with SEV-1 LGPD violation canary on `auto_suspend_attempts_total > 0` + SEV-1 INV-TENANT-ISOLATION canary on `cross_tenant_feature_leak_total > 0`); `error` (`#[non_exhaustive]` taxonomy with explicit AutoSuspendForbidden arm carrying tenant_id + score for LGPD Art. 20 + GDPR Art. 22 audit trail); `scorer` (AbuseScorer trait + InMemoryAbuseScorer orchestrator with per-instance `Arc<Mutex<HashMap<Uuid, AbuseRollingWindow>>>` mirroring DO actor model + audit-emit-BEFORE-state-mutation fail-closed envelope + cross-WI integration with `corelink-ratelimit::RateLimiter::update_plan` for SilentDowngrade tier 50% refill rate halving per WI §6.1.4 CI-2 mechanism + AutoSuspendForbidden trap on programmatic suspend attempts per LGPD Art. 20 humane response sprint contract §7.10.s08.3). Migration `0013_abuse_scores.sql` (180 LOC): composite PK `(tenant_id, computed_at)` tenant-leftmost; 14 inline CHECK constraints + 3 indices including SEV-1 partial index on `decision = 'Malicious'` + SEV-3 partial index on `decision = 'Suspicious'`; idempotent + additive; canonical schema version 13; column drift no `_ms` suffix per Lote 10.7bis P0-3. Tests: 70 inline lib unit + 17 migration_canonical_0013 + 15 prop_abuse @ 10k iter (100k via PROPTEST_CASES override) + 7 calibration_abuse = **109 tests across all targets, 0 failures**. Property tests pin INV-TENANT-ISOLATION per-tenant scoring (`prop_tenant_isolation` + `prop_per_tenant_isolation_concurrent` 100 tenants), score clamping ([0.0, 1.0] structurally; `prop_score_in_range_0_1`), score monotonicity in each feature (`prop_score_monotonic_in_cpu` + `prop_score_monotonic_in_egress` + `prop_score_monotonic_in_exec` + `prop_score_monotonic_inverse_in_entropy`), threshold boundary semantics (`prop_decision_threshold_boundary` ≥ on each), audit-emit-per-decision-arm (`prop_audit_emit_per_decision_arm`), AutoSuspendForbidden idempotence (`prop_suspend_idempotent`), cross-WI downgrade integration (`prop_downgrade_applied_reduces_refill_rate` 50% rate + burst halving), saturating-arithmetic safety (`prop_overshoot_does_not_panic`). **Calibration test fixture** (sprint contract §6 DoD; Lote 10.8bis P0-E corrected): synthetic n=50 benign + n=50 high-intensity abusive workloads via `ChaCha20Rng::seed_from_u64` (deterministic PRNG-pinned per charter constraint); Wilson 95% CI statistical assertion FP rate ≤ 5% upper-bound + TP rate ≥ 80% lower-bound. **Calibration outcome: 0/50 FP (Wilson upper 0.071) + 50/50 TP (Wilson lower 0.929)**, well above the sprint contract acceptance bound (FP ≤ 0.10 / TP ≥ 0.67 acceptable; target FP ≤ 0.058 / TP ≥ 0.78). **Camada-4 composition** documented in module rustdoc: edge → ratelimit → quota-CAS → abuse layered bulkhead PAT-RATE-LIMIT-001. **Trait-abstraction-defer per charter**: real CF DO singleton `AbuseScoreCron-<region>` + real D1 atomic batch + Tower-layer production wiring (gated `corelink-worker/tower-middleware`) + 100k nightly proptest sustained 7d + chaos 11 + customer self-service endpoints (S-13 dependency) + admin endpoints + appeal queue + 30d shadow re-calibration workflow — all consolidated alongside WI-S08-006 PRR ship gate. Cross-module patches: workspace `Cargo.toml` adds `crates/corelink-abuse` member + workspace dep; reuses `corelink-eviction::Tier` + `corelink-ratelimit::RateLimiter`. Quality gates verde: `cargo test -p corelink-abuse --all-targets` 109 tests 0 failures; `cargo clippy --workspace --all-targets --features corelink-worker/tower-middleware -- -D warnings` clean; `validate_specs.py` clean (278 schema + 6 YAML = 284 docs); `check_migrations_additive.py` clean (13 migrations including new 0013). No per-WI codex per 2026-04-30 protocol; sprint-close Sonnet review covers full S-08 corpus. |
...
| 2.0.0 | 2026-05-02 | Gustavo (via Claude Opus 4.7 1M; autonomous WI-S08-006 SEAL) | **WI-S08-006 SEALED — S-08 ship gate consolidated; PRR HIGH_RISK 11 sign-offs canonical APPROVED (3 ✅ + 8 ⚠️ WAIVED via ADR-0034 dual-hat); promotion STAGING-STABLE.** Artefatos shipped: `dashboards/grafana/DASH-RATE.json` 14-panel canonical S-08 ship-gate dashboard agregando 4-camada bulkhead operational visibility (camada 0 global circuit + camada 1 per-tenant DO RateLimiter + camada 2 CF edge per-IP + camada 3 quota CAS hard-block + camada 4 abuse heuristic) + SLI distinction within-quota vs over-quota panel (sprint contract §7.10.s08.1 critical) + INV-AVAIL-ISOLATION canary + INV-RATE-LIMIT-PROPORTIONALITY canary + LGPD canary auto_suspend_attempts (Art. 20 + GDPR Art. 22) + RFC 9331 header emission rate + PRR ship gate status table; `dashboards/alerts/dash-rate-alerts.yml` 15 alert rules canonical (Lote 10.8-tris P1-NEW-1 corrected: 4 SEV-1 + 6 SEV-2 + 5 SEV-3); `specs/05_quality/runbooks/RB-FM-250-ddos-volumetric.md` flipped DRAFT → FROZEN with 2026-05-02 dry-run-executed timestamp; `scripts/rb_fm_250_dry_run.sh` host-side cargo-driven harness (~7 steps + drift detection); `specs/_audits/sealed/2026-05-02-rb-fm-250-dry-run.md` audit trace; `specs/04_sprints/S08/PRR-S08.md` HIGH_RISK 11 sign-offs canonical (per framework §33.5.4.3 + ADR-0034); `specs/_audits/sealed/2026-05-02-adversarial-s08.md` 20 scenarios catalogued + cumulative invariant interaction matrix; `.github/workflows/s08-ship-gate.yml` aggregate ship-gate workflow; `.github/workflows/nightly.yml` extended with 5 S-08 crates × 100k iter proptest tier (prop_ratelimit + prop_edge + prop_quota_cas + prop_abuse + prop_rate_headers). **Quality gates verde**: cumulative `cargo test -p corelink-{ratelimit,edge,quota-cas,abuse,rate-headers} --all-targets` 511 tests 0 failures (101 + 96 + 100 + 109 + 105); clippy `-D warnings` clean; validators chain clean; RB-FM-250 dry-run exit 0; DASH-RATE.json + dash-rate-alerts.yml parse smoke green. **Calibration outcome** Wilson 95% CI 0/50 FP (upper 0.071) + 50/50 TP (lower 0.929) per Lote 10.8bis P0-E corrected. **DoD totals 14/19 ✅ + 5/19 ⚠️ DEFERRED** (chaos isolation 1 tenant flood + 30d sustained chaos + real CF bindings + cost regression gate + cargo-fuzz expansion + real client integration smoke; all forward-looking gates with explicit revalidation triggers; none blocks S-08 SEAL per spec contract §6 partial-bullet pattern + charter `trait-abstraction-defer` pattern). **1 NEW INV §3.19 promoted** (INV-RATE-LIMIT-PROPORTIONALITY HIGH; Sprint-driven invariant Lote 10.8 S-08 NEW group). **PRR sign-off matrix**: 3 ✅ APPROVED (Owner / Final Approver / Engineer S-08 lead / Product) + 8 ⚠️ WAIVED via ADR-0034 dual-hat (Architect+Crypto SME co-sign for race-correctness atomic CAS / Security Lead / SRE Lead / QA Lead / Compliance Officer / Privacy Officer / AppSec advisor). **No per-WI codex per 2026-04-30 protocol; sprint-close Sonnet review covers full S-08 corpus AFTER WI-S08-006 SEAL**. S-08 ship gate ready for sprint-close Sonnet review. |

---

| 1.7.0 | 2026-05-02 | Gustavo (via Claude Opus 4.7 1M; orchestrator-finalized after agent rate-limit) | **WI-S08-005 SEALED — `crates/corelink-rate-headers/` v0.1.0 shipped + `migrations/d1/0014_global_circuit_state.sql` (NEW table; per-region durable circuit state + append-only trips_history).** New crate ships 6 src modules (~3619 LOC src + ~1147 LOC tests = 4766 total): `headers` (RateLimitHeaderBuilder composing canonical RFC 9331 `RateLimit` + `RateLimit-Policy` headers; XRateLimitTypeKind `#[non_exhaustive]` 5-arm enum mapping to R-S08-8 canonical taxonomy `tenant_quota / per_ip / per_pat / over_quota / global_circuit_open`); `circuit` (GlobalCircuitBreaker trait + InMemoryGlobalCircuitBreaker with canonical 3-state Closed/Open/HalfOpen + sliding-window error-rate trip at >50% sustained 5min + emergency degrade_mode); `audit` (4-event taxonomy `corelink.circuit.{tripped, half_open_probe, closed_recovery, request_rejected}` + fail-closed envelope); `metrics` (canonical metric ladder for circuit state transitions + RateLimit header emission rate); `error` (`#[non_exhaustive]` taxonomy); `lib` (public re-exports + `MIGRATION_0014_GLOBAL_CIRCUIT_STATE` const + `circuit_schema_version() = 14`). CircuitDecision `#[non_exhaustive]` 3-arm (Closed / Open { trip_reason } / HalfOpen). Migration `0014_global_circuit_state.sql`: per-region PK + composite (region, tripped_at) region-leftmost on append-only trips_history; idempotent + additive. Tests: 58 inline lib unit + 23 prop_rate_headers @ 10k iter + 24 migration canonical = **105 tests across all targets, 0 failures, parallel-safe**. Property tests pin RFC 9331 grammar conformance, RateLimit-remaining consistency with bucket state, canonical 3-state circuit transitions, exact-boundary trip at 50%/5min, half-open lets one through, tenant isolation (per-tenant headers; circuit state global), audit emit per decision arm, 5-arm canonical X-Rate-Limit-Type taxonomy. **Camada-0 composition** documented in module rustdoc: global-circuit consulted FIRST (zero-cost short-circuit when Open); edge → ratelimit → quota-cas → abuse layered SECOND. **Trait-abstraction-defer per charter**: real CF DO singleton + real D1 atomic batch + 100k nightly + cargo-fuzz target — all consolidated alongside WI-S08-006 PRR ship gate. Quality gates verde: `cargo test -p corelink-rate-headers --all-targets` 105 tests 0 failures; `cargo clippy --workspace --all-targets --features corelink-worker/tower-middleware -- -D warnings` clean (after orchestrator P0 fixes: prop_assert! pattern interaction with format-string interpretation in `matches!` arm rewritten as explicit `let valid = ...`; `clippy::type_complexity` resolved via type alias for CounterMap; `clippy::doc_lazy_continuation` allow on test module); `validate_specs.py` clean (284 docs); `check_migrations_additive.py` clean (14 migrations including new 0014). **Note**: agent hit Anthropic rate limit at 57 tool uses; orchestrator-finalized 3 P0 fixes + SEAL ceremony per charter `Real bug detected; don't paper over` pattern. No per-WI codex per 2026-04-30 protocol; sprint-close Sonnet review covers full S-08 corpus. |

---

**Fim de spec contract S-08 SOTA v2.0.0** (Upgrade history: v1.2 initial design; v1.3 WI-S08-001 SEAL; v1.4 WI-S08-002 SEAL; v1.5 WI-S08-003 SEAL; v1.6 WI-S08-004 SEAL; v1.7 WI-S08-005 SEAL; v2.0 WI-S08-006 SEAL — S-08 ship gate consolidated; PRR HIGH_RISK 11 sign-offs canonical APPROVED; promotion STAGING-STABLE).
