...
> **doc_status:** FROZEN · **work_status:** DONE · **audit_status:** AUDITED · **lane:** HIGH_RISK
...
| 1.1.0 | 2026-04-25 | Gustavo (Lote 10.8bis) | R4+R5 review remediation: P0-A bandwidth_quota discriminator REMOVED (subsumed sob over_quota; canonical 5-enum WI-S08-005 preserved; audit log reason field discriminates storage vs bandwidth); P0-C per-PAT camada 3 reframed as DETECTOR não enforcer (aggregate enforcement em camada 1; misuse threshold = 10× tenant refill alarm only); P0-D `next_month_first_utc_midnight()` correct chrono primitive (`tomorrow_at_utc_midnight()` retornava next-day NOT next-month-1st; off by up to 30× em Retry-After); P0-E persona 3.1 acknowledged limitation (medium-intensity abuse acceptably Noop); P1-3 FM-251 → FM-059 + FM-201 canonical; P1-6 sprint contract §5 R-S08-3 + R-S08-5 corrected in same cycle (não kicked-can); R5 P1-6 overshoot off-by-one (saturating_sub max NOT max-1); Migration pat_rate_state → pat_rate_observation (rolling rate EWMA NOT token bucket); aggregate score post-bis target ≥ 8.5/10 (R4 7.4 + R5 5.5 baselines). |
| 1.2.0 | 2026-04-25 | Gustavo (Lote 10.8-tris **SEALED**) | Sonnet R5 round-2 review remediation (tris score 8.4/10 from 5.5 round-1; +2.9 delta — largest WI improvement): P0-NEW-1 enforcement framing residuals fixed (§2 adversarial line 271 + §6.1 chaos 6 line 560 "90%/80% requests rejected" → "NOT blocked at camada 3"; TokenBucketState → PatRateObservationState rename; PatRateState::new → PatRateObservationState::new; prop_pat_rate_cap → prop_pat_rate_misuse_detection_threshold rename); P1-NEW-4 §13 artifacts + §9.11 design decisions stale `pat_rate_state.sql` → `pat_rate_observation.sql` matching DDL. **WI sealed pre-implementation**. |
| 1.3.0 | 2026-05-02 | Gustavo (via Claude Opus 4.7 1M; autonomous WI-S08-003 SEAL) | **WI-S08-003 SEALED — `crates/corelink-quota-cas/` v0.1.0 shipped + `migrations/d1/0012_quota_cas_attempts.sql` (NEW table; durable audit ring of every CAS attempt — successful / denied / race-detected — for SEV-3 race detection observability + admin-plane forensics).** New crate ships 7 source modules covering the canonical 100% hard-block path (CAP-QUOTA-001) per ADR-0020 FROZEN, superseding the S-07 PROVISIONAL transitional 429 + Retry-After arm: `retry_after` (canonical `days_until_month_reset_secs` + `next_month_first_utc_midnight_secs` + `decompose_utc` / `compose_utc` Howard Hinnant Gregorian primitives — wasm32-clean, no `chrono` dep; pinned at boundary cases Jan-1, Dec-31 year increment, Feb-28 non-leap, Feb-29 leap year), `state` (AtomicCasState trait + InMemoryAtomicCasState with monotone `cas_version` counter + race-aware strict-< boundary predicate; per-instance `Mutex` envelope mirrors DO actor model serialisation byte-for-byte; F-001 closure preserved), `cas` (AtomicQuotaChecker trait + InMemoryAtomicQuotaChecker orchestrator with bounded CAS retry loop canonical 3 attempts + audit-emit-BEFORE-write fail-closed envelope mirroring S-07 sprint-close P1-1 fix; QuotaCasDecision `#[non_exhaustive]` Allow / Deny429 carrying canonical `retry_after_secs: days-until-month-reset`; QuotaCasOutcome surfaces `cas_attempts` for race-detection observability; idempotent zero-byte read-path passthrough), `audit` (canonical 6-event taxonomy `corelink.quota.cas_{check_passed, denied_429_hard_block, race_detected, commit_succeeded, release_idempotent, retry_after_emitted}` `#[non_exhaustive]`; `CasDenied429HardBlock` SEV-1; fail-closed envelope per `INV-AUDIT-EMIT-ATOMIC-WITH-HANDLER`), `metrics` (5-canonical metric ladder: `cas_check_total{result=allow|deny|race}` + `cas_denials_total` + `cas_race_detected_total` + `cas_retry_after_secs` histogram + `cas_check_duration_us` histogram), `error` (`#[non_exhaustive]` taxonomy: TenantStorageStateMissing / CasRaceExhausted / State / Audit / Metrics / RequestBytesOverflow / Backend), `config` (QuotaCasConfig: hard_block_pct canonical 1.0 + max_cas_attempts canonical 3; defensive clamp `[0.5, 1.0]` and `[1, 16]`). Migration `0012_quota_cas_attempts.sql`: PK `attempt_id` (UUIDv7 surrogate); 12 inline CHECK constraints (event_type closed 6-literal, region closed 5-literal, retry_after_secs envelope `[0, 31×86_400]`, cas_attempt ≥ 1, cas_version ≥ 0, bytes ≥ 0, now_ms ≥ 0, length(text) ≥ 1); 3 indices (tenant scan + race-detected partial + denied-429 partial — drives SEV-1 + SEV-3 alarm queries); idempotent + additive; canonical schema version 12. Tests: 72 inline lib unit + 16 migration_canonical_0012 + 12 prop_quota_cas @ 10k iter (100k via PROPTEST_CASES override) = **100 tests across all targets, 0 failures**. Property tests pin INV-QUOTA-ENFORCEMENT race-aware strict-< predicate (`prop_cas_no_double_spend` — sequential acquires under per-instance Mutex envelope never exceed quota), CAS race retry happy path (`prop_cas_race_detected_retry_succeeds` — OneShotRaceState wrapper + bounded retry succeeds on attempt 2), canonical Retry-After bounds (`prop_retry_after_days_until_month_reset` — always within `[1, 31×86_400]`), tenant isolation (`prop_tenant_isolation`), audit-emit-per-decision-arm (`prop_audit_emit_per_decision_arm`), idempotent zero-byte (`prop_idempotent_zero_byte_check`), saturating-arithmetic safety (`prop_overshoot_does_not_panic`), informational SLO probe (`prop_check_duration_under_5ms_p99`). Cross-module patches: workspace `Cargo.toml` adds `crates/corelink-quota-cas` member + workspace dep + reuses `corelink-eviction::EvictionRegion` + composes with `corelink-quota` (S-07 PROVISIONAL stays intact for transitional window per ADR-0020 FROZEN). **Trait-abstraction-defer per charter**: real CF DO singleton + real D1 atomic batch + Tower-layer wiring (gated `corelink-worker/tower-middleware`) + 100k nightly + chaos 12 + crypto SME advisory race correctness — all consolidated alongside WI-S08-006 PRR ship gate. Quality gates verde: `cargo test -p corelink-quota-cas --all-targets` 100 tests 0 failures; `cargo clippy --workspace --all-targets --features corelink-worker/tower-middleware -- -D warnings` clean; `validate_specs.py` clean (284 docs); `check_migrations_additive.py` clean (12 migrations). No per-WI codex per 2026-04-30 protocol; sprint-close Sonnet review covers full S-08 corpus. |
