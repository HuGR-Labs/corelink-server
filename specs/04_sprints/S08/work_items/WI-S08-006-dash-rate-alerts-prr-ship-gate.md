...
|---|---|---|---|
| 1.3.0 | 2026-05-02 | Gustavo (via Claude Opus 4.7 1M; autonomous WI-S08-006 SEAL) | **WI-S08-006 SEALED — S-08 ship gate consolidated; PRR HIGH_RISK 11 sign-offs canonical APPROVED (3 ✅ APPROVED + 8 ⚠️ WAIVED via ADR-0034 dual-hat); promotion decision STAGING-STABLE.** Artefatos shipped: (a) `dashboards/grafana/DASH-RATE.json` 14-panel dashboard agregando 4-camada bulkhead operational visibility (camada 0 global circuit + camada 1 per-tenant DO RateLimiter + camada 2 CF edge per-IP + camada 3 quota CAS hard-block + camada 4 abuse heuristic); panels: SLI distinction within-quota vs over-quota + SLO calculation + global circuit state per region + global circuit trips/recoveries + per-tenant token bucket utilization heatmap + INV-AVAIL-ISOLATION canary + edge per-IP block decisions + CIDR blocklist size + quota CAS atomic check + abuse heuristic score distribution + LGPD canary auto_suspend_attempts + RFC 9331 header emission rate + bulkhead 4-camada decision rate + INV-RATE-LIMIT-PROPORTIONALITY canary + PRR ship gate status table. (b) `dashboards/alerts/dash-rate-alerts.yml` 15 alert rules canonical (Lote 10.8-tris P1-NEW-1 corrected: 4 SEV-1 + 6 SEV-2 + 5 SEV-3): SEV-1 (4) Rate_GlobalCircuitOpenSustained + Rate_InvAvailIsolationViolation + Rate_AutoSuspendAttemptLgpdCanary + Rate_SliDistinctionRegression; SEV-2 (6) Rate_AbuseAdminReviewTriggered + Rate_PatMisuseDetected + Rate_GlobalCircuitManualOverride + Rate_EdgeCidrBlocklistDrift + Rate_AppealsQueueOverflow + Rate_QuotaHardBlock100PctSustained; SEV-3 (5) Rate_QuotaSoftPressure95Pct + Rate_GlobalCircuitSingleSignalAlarm + Rate_AppealSlaBreach + Rate_RatelimitPlanSyncLag + Rate_AbuseCalibrationDrift. (c) RB-FM-250 (DDoS volumetric) flipped DRAFT → FROZEN with 2026-05-02 dry-run-executed timestamp; harness `scripts/rb_fm_250_dry_run.sh` host-side cargo-driven; audit trace `specs/_audits/sealed/2026-05-02-rb-fm-250-dry-run.md`. (d) `specs/04_sprints/S08/PRR-S08.md` HIGH_RISK 11 sign-offs canonical (per framework §33.5.4.3 + ADR-0034); 3 ✅ APPROVED (Owner + Final Approver + Engineer + Product) + 8 ⚠️ WAIVED via ADR-0034 dual-hat (Architect+Crypto SME co-sign for race-correctness atomic CAS / Security Lead / SRE Lead / QA Lead / Compliance / Privacy / AppSec). Promotion decision: STAGING-STABLE. (e) `specs/_audits/sealed/2026-05-02-adversarial-s08.md` 20 scenarios catalogued; cumulative invariant interaction matrix; zero HIGH/CRITICAL. (f) `.github/workflows/s08-ship-gate.yml` aggregate ship-gate workflow (validators + per-crate tests + clippy `-D warnings` + RB-FM-250 dry-run + dashboard parse smoke). (g) `.github/workflows/nightly.yml` extended with 5 S-08 crates × 100k iter proptest tier (prop_ratelimit + prop_edge + prop_quota_cas + prop_abuse + prop_rate_headers). **DoD totals: 14/19 ✅ + 5/19 ⚠️ DEFERRED** (chaos isolation 1 tenant flood + 30d sustained chaos + real CF bindings + cost regression gate + cargo-fuzz expansion + real client integration smoke; all forward-looking gates with explicit revalidation triggers; none blocks S-08 SEAL per spec contract §6 partial-bullet pattern + charter `trait-abstraction-defer` pattern). **Quality gates verde**: `cargo test -p corelink-{ratelimit,edge,quota-cas,abuse,rate-headers} --all-targets` 511 tests 0 failures (101 ratelimit + 96 edge + 100 quota-cas + 109 abuse + 105 rate-headers); `cargo clippy --workspace --all-targets --features corelink-worker/tower-middleware -- -D warnings` clean; `python3 scripts/validate_specs.py` clean; `python3 scripts/check_migrations_additive.py` clean; RB-FM-250 dry-run exit 0; DASH-RATE.json + dash-rate-alerts.yml parse smoke green. **Calibration outcome**: Wilson 95% CI 0/50 FP (upper 0.071) + 50/50 TP (lower 0.929). 1 NEW INV §3.19 promoted (INV-RATE-LIMIT-PROPORTIONALITY HIGH). No per-WI codex per 2026-04-30 protocol; sprint-close Sonnet review covers full S-08 corpus. |
