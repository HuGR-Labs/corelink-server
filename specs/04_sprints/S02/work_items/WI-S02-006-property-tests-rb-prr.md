...
- [x] **10.6.1** Property test 100k iter green at SEAL (`crates/corelink-reapi/tests/prop_cas_read.rs::cross_tenant_read_round_robin_100k`); 7d nightly = forward-looking once CI deploy enables nightly schedule (EVT-002).
...
- [x] Property test suite 10k+ iter (incl. 100k round-robin) green em CI (debug + release).
- [x] Bit rot integration test 10 scenarios + opt-out negative control.
...
> **Path-routing decision (v2.0.0 §31 changelog):** v1.1.0 named the
> Rust test paths under `crates/corelink-worker/tests/`. The
> cross-component scope dev-depends on `corelink-reapi::read` AND
> `corelink-reapi::find_missing`; `corelink-worker` cannot dev-depend on
> `corelink-reapi` without creating a workspace cycle. The canonical
> deliverable lives in `crates/corelink-reapi/tests/` — same path-
> routing decision documented for WI-S01-006 (write-path
> consolidation; commit `eb092f9`). The substantive deliverable —
> cross-component property tests at 10k+ iter + 100k round-robin
> schedule + bit-rot integration test — is unchanged.

| Artifact | Path | Tipo |
|---|---|---|
| Expanded property test (cross-component) | `crates/corelink-reapi/tests/prop_cas_read.rs` | Rust test |
| Bit rot integration test | `crates/corelink-reapi/tests/integration_bit_rot.rs` | Rust test |
