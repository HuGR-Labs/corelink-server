...
| GA-GATE-S09 | **Supply-chain provenance** | SLSA Level 2 provenance (self-hosted fleet), cosign keyless OIDC + Rekor; Rekor entry per release artefact; INV-SUPPLY-SIGNED-DEPLOY + INV-SUPPLY-PROVENANCE-IN-REKOR green; cosign verify passes against release tag. | Rekor log URL; `cosign verify --certificate-identity` exit 0 | Engineer Lead + Security Lead | NOT_STARTED |
...
| GA-GATE-O05 | **Status page live** | `https://hugrl.betteruptime.com` returns 200; auto-subscribed to Prometheus alerts for SEV-1/SEV-2; component map mirrors public capabilities; T-7 dry-run incident posted + reverted. | Status page URL; subscription audit | SRE Lead | NOT_STARTED |
