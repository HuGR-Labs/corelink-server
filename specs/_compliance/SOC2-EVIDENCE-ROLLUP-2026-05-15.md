...
| TSC | CTRL ID | Evidence type | Artifact location | Cadence | Status | Last update |
|---|---|---|---|---|---|---|
| CC6.1 | CTRL-AUTH-001/004/007/010 + CTRL-SECRETS-DRIFT-001 | credential_management | `specs/03_architecture/auth_model.md` + Clerk SSO + MFA + `compliance/byok-fips-matrix.md` + `docs/internal/secrets-checklist.md` (89-row matrix) + `scripts/validate_secrets_matrix.py` + daily drift gate `.github/workflows/secrets-drift.yml` (90d JSON artifact) + triage `specs/_runbooks/RB-SECRETS-DRIFT.md` + baseline `specs/_audits/2026-05-15-secrets-coverage-baseline.md` | AUTO | **G — blocking-GA (GAP-02 closing D+30; secrets-drift evidence ADDED 2026-05-15)** | f18acdc |
...
|---|---|---|---|---|---|---|
| 1 | **GAP-02** | BYOK FIPS attestation per provider (AWS L3 attested; GCP L1 + Azure pending) — **progress 2026-05-15**: attestation kit landed via `wt/gap02-fips-attestation-kit` (matrix `specs/_compliance/BYOK-FIPS-ATTESTATION-MATRIX.md` + 4 vendor letter templates `specs/_compliance/fips-attestation-letters/` + 30-question RFI `specs/_compliance/FIPS-RFI-QUESTIONNAIRE.md` + endpoint verifier `scripts/verify-fips-endpoints.py` (4/4 providers OK) + quarterly renewal runbook `specs/_runbooks/RB-FIPS-ATTESTATION-RENEWAL.md`). RFIs dispatched 2026-05-15; vendor SLA D+30 (response window 2026-06-14). | **blocking-GA** | CC6.1 + C1.1 → would force qualified opinion if open at fieldwork | S (collect 2 signed letters; update `compliance/byok-fips-matrix.md`) | **D+30 hard cap** |
...
| GAP-01-DPO | LGPD Art. 41 cross-framework (CC1.3 governance + CC2.3 communication) | DPO (interim) | M | **In progress (2026-05-15)** — D+90 to permanent appointment per handoff plan; hard cap before first EU enterprise tenant | Interim DPO formally designated via `specs/_compliance/DPO-APPOINTMENT-2026-05-15.md` (LGPD Art. 41 §1º + ANPD Resolução 18/2024 compliant); reporting line to Board; independence safeguards documented in §3 of appointment doc; conflict-of-interest analysis acceptable for SAM-only GA (not for EU enterprise). RACI = `DPO-RESPONSIBILITIES-MATRIX.md` (30 rows). Escalation triggers + SLAs = `RB-DPO-ESCALATION.md` (12 trigger families, 5-tier SLA matrix). 90-day handoff plan = `DPO-HANDOFF-PLAN.md`. Note: this row tracks the LGPD/`compliance_matrix.md §9` GAP-01 (DPO formal appointment), distinct from the GAP-01 row above which tracks CC6.3 access-review automation. |
...
| GAP-22 | CC9.2 cross-framework (LGPD Art. 33 §1º) | Privacy | M | **Implemented (2026-05-15)** | Attestation bundle `specs/_compliance/LGPD-RESIDENCY-ATTESTATION-2026-05-15.md` (7 sections, DPO + Security Lead countersign pending); DPO monthly checklist `LGPD-DPO-MONTHLY-CHECKLIST.md` (25 items); customer-facing explainer `apps/docs/docs/explanation/residency/lgpd-brazil.mdx`; nightly verifier `scripts/verify-lgpd-residency.py`. **GAP-22 followup (2026-05-15)** — LGPD full audit (`LGPD-FULL-AUDIT-2026-05-15.md`, 19 articles); LGPD-ROPA (`LGPD-ROPA-2026-05-15.md`, 14 dataflows); LGPD DSR runbook (`RB-DSR-LGPD-FULL.md`). **GDPR mirror (2026-05-15)** — GDPR full audit (`GDPR-FULL-AUDIT-2026-05-15.md`, 25 articles); SCC execution + Schrems II TIA (`GDPR-SCC-EXECUTION-2026-05-15.md`); DPIA library (`GDPR-DPIA-LIBRARY.md`); GDPR DSR runbook (`RB-DSR-GDPR.md`); customer explainer (`apps/docs/docs/explanation/privacy/gdpr.mdx`). |
| GAP-23 | CC9.2 cross-framework (EDPB SCCs) | Legal | M | **Implemented (2026-05-15)** | SCC Modules 2/3 already in DPA; **canonical SCC execution + TIA register landed via `specs/_compliance/GDPR-SCC-EXECUTION-2026-05-15.md`** (per-flow module selection; 3 TIA records for US transfers — Stripe/Clerk/Grafana; EU-US DPF + Schrems-III contingency). |
