...
| **Stripe, Inc.** | Payment processing + billing record | Global (Stripe processes globally; BR tenants billed via Stripe BR routing where available) | LGPD: Stripe [DPA + Brazil addendum](https://stripe.com/legal/dpa); SCC Modules 2/3; PCI-DSS Level 1; CoreLink only sends pseudonymised customer_id + amount; no card data ever touches CoreLink. Stripe's BR processing entity provides Art. 33 IX (c/c Art. 7 V execução de contrato) contractual basis (Lote 10.11.0-ter legal-citation re-validation 2026-05-15 corrigida cite anterior "Art. 33 V" — V = autorização ANPD, contrato é IX via remissão a Art. 7 V). | `legal/sub-processors.md:59-78` |
| **Neon Inc.** | Billing-detail Postgres (US or EU tenant-selectable) | Neon does not currently offer BR region — for BR tenants, billing detail (invoice, usage_event) goes to Neon EU (Frankfurt). | LGPD: Art. 33 IX (c/c Art. 7 V execução de contrato — contractual necessity for billing) + SCC Modules 2/3 (Neon's [DPA](https://neon.tech/dpa)); fiscal retention 5y per LGPD Art. 16 I (cumprimento de obrigação legal — CTN Art. 173/174) — Lote 10.11.0-ter legal-citation re-validation 2026-05-15 corrigidas cites anteriores "Art. 33 V" e "Art. 16 §3º". Marked as international transfer with documented SCC basis. | `legal/sub-processors.md:27-58` |
...
| **Clerk** | Auth (PAT / WebAuthn / SSO) | Currently US-hosted | LGPD: Clerk's [DPA + SCCs](https://clerk.com/legal/dpa); only auth tokens + email (account_data category) cross; not PII workload content. Art. 33 IX (c/c Art. 7 V execução de contrato) contractual basis (Lote 10.11.0-ter legal-citation re-validation 2026-05-15 corrigida cite anterior "Art. 33 V" — V = autorização ANPD, contrato é IX via remissão a Art. 7 V). | Clerk sub-processor row |
