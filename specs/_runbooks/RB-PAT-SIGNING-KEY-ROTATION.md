...
would create mixed key generations and can cause a PAT-auth outage. The B-081
code repair is complete; the separate population control dependency remains
open.
...
Undefined and empty Worker bindings are the intentional ABSENT sentinel. A
whitespace-only Worker binding is different: the edge rejects it as malformed,
and the DO's direct `container.start({ env })` payload carries a deterministic
non-hex marker so that path also fails closed; it is never silently treated as
an absent sibling at the Worker/container boundary.
...
can prove G-stage, G-promote, or G-retire across `P`; the B-081 code item is
complete after its focal proof, but production rotation remains blocked and this
runbook must remain non-executable until those controls are implemented and
