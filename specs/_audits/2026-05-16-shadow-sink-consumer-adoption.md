...
- **Production `TokioPgShadowSinkFactory` override** — the wave-21 `TokioPgShadowSinkFactory` impl in `apps/server/src/main.rs` continues to use the default `for_tenant_in_region → for_tenant` delegate (i.e. still calls `region_resolver.resolve_region`). Wave-28 will override `for_tenant_in_region` to skip the resolver round-trip and dispatch straight to `executors.get(region.as_str())`. The trait surface is in place; the override is a single-method add.
