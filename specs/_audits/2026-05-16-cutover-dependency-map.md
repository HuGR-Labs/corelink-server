...
| N-S-1 | Atlassian Statuspage tenant Option A provisioned (CNAME `status.corelink.dev` → `*.statuspage.io`) | T-10d (≥ 3d buffer before T-7d gate) | SRE Lead | Operator owns `corelink.dev` DNS zone (default GA assumption); Atlassian Business-tier subscription approved | `curl -sI https://status.corelink.dev` → HTTP 200; `summary.json page.url == "https://status.corelink.dev"`; 8 tracked components visible | **0d** (critical along Option A branch — see §6.2) |
