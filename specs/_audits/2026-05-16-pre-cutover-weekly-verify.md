...
| 5 | DEBT-016 Statuspage `status.corelink.dev` go-live | `specs/_audits/2026-05-16-debt-016-statuspage-urls.md` + (advisory) `reports/statuspage-live-state.json` from the workflow's live probe | dress-run snapshot detection + live-state lookup | `ENGINEERING_CLOSED_OPERATOR_BOUND` → `SIGNED` | T-7d |
