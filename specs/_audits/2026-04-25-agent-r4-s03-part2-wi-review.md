...
5. **`origin allowlist exact match` claim is incomplete**. WebAuthn origin matching has a specific algorithm in W3C §13.4.9: for `Top.origin` matching it's the *full* origin (scheme + host + port). The WI says "no prefix bypass" but doesn't specify port handling, scheme handling (ws://?), or `subdomain.app.corelink.dev` rejection. Concrete test: assert `https://app.corelink.dev:443` and `https://app.corelink.dev` both allowed if both intended; assert `https://app.corelink.dev.evil.com` rejected by exact match.
