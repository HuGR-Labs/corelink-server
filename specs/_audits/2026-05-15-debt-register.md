...
| DEBT-005 | **30 critical INVs without TLA model** (was 40 at baseline, 35 after batch 1) — canonical-lint baseline showed 40 CRITICAL INVs without TLC; cumulative closure 2026-05-15 brings to **30 remaining** (10 specs closed across 2 batches; batch 1 = `auth_jwt_validation`, `tenant_ctx_propagation`, `merkle_integrity`, `byok_envelope_aad`, `failover_no_split_brain`; batch 2 = `rollout_cosign_gate`, `backup_restore_ephemeral`, `gc_sweep_audit_fail_closed`, `ac_eviction_isolation`, `audit_emit_atomic`). | `specs/_audits/2026-05-15-canonical-consistency-baseline.md` | Orchestrator | T+30d (2026-06-14) | **PARTIAL (13/47)** — 13 cumulative INVs covered (auth_revocation cluster=4, batch 1=5 INVs, batch 2=8 INVs minus 4 already-counted-via-batch-1 / cross-counted; counting at canonical-INV granularity: batch 2 alone covers 8 distinct INVs across 5 specs). Wave 6 closed 1 (auth_revocation); DEBT-005 batch 1 dispatch 2026-05-15 closed 5 specs on branch `wt/debt-005-tla-critical-inv`; DEBT-005 batch 2 dispatch 2026-05-15 (re-run after wave-11 rate-limit) closed 5 specs on branch `wt/debt-005-tla-batch-2-v2`. Remaining 30 tracked in `tla-followup-tickets.md` addenda + FT-1..FT-9 backlog. Target ≥ 50% closure (≤ 20 remaining) pre-GA gate. Next dispatch should pick from: `auth_pat_hybrid` (FT-1), `key_lifecycle` (FT-2), `gc_lock_protocol` (FT-5), `multipart_finalize` (FT-6), `byok_dek_race` (FT-7). |
