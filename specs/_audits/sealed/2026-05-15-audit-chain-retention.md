...
| TLA+ spec stub | This document §3 | `specs/_audits/sealed/2026-05-15-audit-chain-retention.md` |
...
| Wave-18 Neon analytics shadow tier | Shadow sync + 2 customer-facing SQL aggregate endpoints | `crates/corelink-audit-chain/src/neon_shadow.rs` + `apps/server/src/routes/audit_analytics.rs` + `migrations/neon/0001_audit_events_shadow.sql` + `specs/_audits/sealed/2026-05-15-neon-analytics-shadow.md`. **Tier split:** R2 = canonical chain-integrity store (this doc, §2.1 Object Lock 7y retention); Neon = analytics convenience tier (≤ 5 min nominal lag; SEV-2 on lag ≥ 60 min; SEV-0 chain-break discipline preserved at the R2 source-of-truth). Daily-verify cron unchanged — it walks R2; Neon divergence is treated as analytics anomaly, not chain break. |
| Wave-19 audit-row `payload` column lift (closes wave-18 caveat #4) | `ExportAuditRow::payload: Option<serde_json::Value>` + D1 schema migrations + lifted SEV-0 mid-stream emit | `apps/server/src/routes/audit_export.rs::ExportAuditRow` + `migrations/d1/0049_export_audit_log.sql` + `migrations/d1/0050_export_audit_log_add_payload.sql` + `apps/server/tests/audit_export.rs::wave19_audit_row_payload_and_trailer_payload_byte_identical` (integration; load-bearing pin — asserts the schema-lifted `payload` column and the `X-CoreLink-Audit-Export-Aborted` trailer JSON are byte-identical, transitively pinning the canonical 4-key map AND `exit_status = EXIT_STATUS_VERIFY_FAILED_MID_STREAM`) + `apps/server/src/routes/audit_export.rs::tests::audit_anchor_emits_before_trailer_under_random_breaks` (proptest @ 10 000 iter) + `crates/corelink-cli/src/commands/verify_ndjson.rs::tests::unknown_row_envelope_fields_ignored_for_forwards_compat` (CLI; pins the `#[serde(default)]` forwards-compat contract). Wave-18 encoded the canonical `{break_at_seq, break_at_chunk, observed, expected}` JSON payload colon-prefix-inside `exit_status` because the row shape lacked a structured payload column; wave-19 lifts it into a first-class `payload` column. `exit_status` now carries the stable enum `EXIT_STATUS_VERIFY_FAILED_MID_STREAM = "verify_failed_mid_stream"`. The audit-row payload and the HTTP `X-CoreLink-Audit-Export-Aborted` trailer payload are **byte-identical** (constant-time `subtle::ConstantTimeEq` over the canonical-serialized bytes) — pinned by the wave-19 integration test. `#[serde(default)]` on `payload` keeps wave-18 row JSON without a `payload` key parseable post-lift (additive migration discipline); D1 schema migrations are additive-only (INV-AUTH-MIGRATION-ADDITIVE) and pass `scripts/check_migrations_additive.py`. The audit-anchor-BEFORE-trailer ordering invariant from wave-18 is preserved byte-for-byte (only the encoding of the SAME payload changed). |
