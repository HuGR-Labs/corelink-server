...
**Current pinned values** (re-pinned 2026-08-24; RATIFIED — see the 2026-08-24 ceremony below):
- Version: `v1.8.0`
- SHA-256: `eabd140a70f49eb9305a3bd3f3df944eddf87e5a90d329789085f8953a80533a`
- Source (since 2026-08-25): `https://corelink-artifacts.humangr.com/tlaplus/v1.8.0/<the SHA-256 above>/tla2tools.jar`
  — **our own copy**, R2 bucket `corelink-ci-artifacts` fronted by a proxied
  custom domain on the `humangr.com` zone. The key CONTAINS the SHA-256, so the
  pin is the address: re-pinning without uploading the matching jar 404s loudly
  instead of silently fetching different bytes. The SHA-256 check in every
  carrier stays, and can now only fail if OUR copy changed — a real signal
  rather than the recurring false alarm.
- Upstream of record (no longer fetched by CI):
  `https://github.com/tlaplus/tlaplus/releases/download/v1.8.0/tla2tools.jar`
- Artifact size: 4487757 bytes

> **SUPERSEDED pins** (historical record only):
> - pinned 2026-08-02 — `e22f8ffb4bacdea0a871f444dd94fe5fb0d8013b3388ae39e82e26f852c735d5`,
>   4486015 bytes. Superseded when upstream re-cut the `v1.8.0` asset on
>   2026-08-21. **Fifth pin value for one immutable-looking version tag.**
...
> `CARRIERS` in `scripts/check_tlc_pin_consistency.py`** — 3 entries as of 2026-08-02 (a 4th,
> `cas_foundation.yml`, went the same day with its duplicate `tlc-canonical`
> job). The
...
**FOLLOW-UP — DONE 2026-08-25 (B-039).** The recommendation below was made on
2026-06-02, restated 2026-08-02, restated again 2026-08-24, and is now executed:
the verified jar lives at `corelink-artifacts.humangr.com` (R2 bucket
`corelink-ci-artifacts`), keyed by its own SHA-256, and both downloading carriers
fetch from there. Maven Central was checked as a neutral immutable source and
does not carry this artifact (`org/lamport/tla2tools/1.8.0` → 404), so the copy
had to be ours. The original text is kept below unedited, because a
recommendation that survived three outages before being acted on is itself part
of the record.
...
| 1.1.5 | 2026-08-02 | Gustavo (via Claude Opus 5) | **Carriers 4 -> 3.** `cas_foundation.yml`'s `tlc-canonical` job was deleted: it model-checked `tenant_isolation`, `cas_integrity`, `gc_correctness` and `audit_immutability` through the SAME `.cfg` files as `tla_check.yml`, so its copy of the pin went with it. Formal-verification coverage is unchanged. That file's own cadence rationale had also claimed those specs were re-verified **DAILY** by `tla_check` — the cron is weekly, and when the claim was written `tla_check` had 0 successes in 195 runs, so the compensating lane it cited verified nothing. `nightly.yml` is deliberately KEPT as a carrier: it is not a duplicate — it checks the same specs against separate `<spec>_nightly.cfg` bounds. No pin VALUE changed; inventory only. |
...
| 1.1.4 | 2026-08-24 | Gustavo (via Claude Opus 5) | **§A1 re-pin RATIFIED + COMPLETED (5th pin value, same `v1.8.0` tag).** Upstream re-cut the asset on 2026-08-21T16:04:52Z; the gate refused it, correctly, and blocked the first PR that touched a trigger path. NEW SHA-256 `eabd140a…80533a` (4487757 B), verified by the chain this addendum requires plus two links the 2026-08-02 ceremony could not use. Provenance: the jar's embedded `X-Git-Revision: 9787e657…` **exists in `tlaplus/tlaplus`** (commit 2026-08-21T15:45:47Z) and the tag `v1.8.0` **resolves to exactly that commit**; the jar's `Build-TimeStamp` 15:59:22Z falls between that commit and the 16:04:52Z asset upload, so the three timestamps are mutually consistent. Transfer: two independent network paths (the CI runner and a separate host) produced byte-identical downloads. **New this time — upstream DOES now publish a checksum**, contradicting the 2026-08-02 ceremony's statement that none exists: the release body carries a `### Checksums` table, and its `sha1sum` for `tla2tools.jar` (`0e4cfdb9…`) matches the artifact. Recorded as corroborating, NOT dispositive: SHA-1 is not collision-resistant and the release body is editable by whoever can replace the asset, so it does not upgrade the assurance to origin authenticity. The jar remains unsigned; an upstream pipeline compromise would still look identical. Updated in all 3 carriers (`tla_check.yml`, `nightly.yml`, `scripts/run_tlc_corelink.sh`) and re-verified by `scripts/check_tlc_pin_consistency.py`. **The standing recommendation is now made for the THIRD time and is escalated to a tracked item (B-039): vendor the verified jar to storage we control and point CI at that.** Five pin values for one tag, three documented breakages, and one 195-run stretch of gates that proved nothing is no longer a recommendation — it is the defect. |
