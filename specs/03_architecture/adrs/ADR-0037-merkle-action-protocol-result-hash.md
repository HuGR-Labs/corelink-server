...
### Result hash semantic (Lote 10.4-tris P0-R5-003)

**`result_hash` is `BLAKE3-256(merkle_root)` — used as D1 INDEX COLUMN ONLY, not a cryptographic binding.**

```rust
// Computed at INSERT time:
let result_hash = blake3::hash(&merkle_root).to_hex();  // 64 hex chars (32 bytes BLAKE3 of merkle_root)
...
16..49   action_digest       (33 bytes = 1-byte algo + 32-byte hash)
49..81   merkle_root         (32 bytes; the cripto authority for output binding)
81..113  result_hash         (32 bytes = BLAKE3-256 of merkle_root; INDEX column; redundant binding for defense-in-depth — see §Rationale)
