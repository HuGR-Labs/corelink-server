...
| **INV-AVAIL-DOS** | Log redaction regex hot-paths são DoS-resistant (no catastrophic backtracking) | HIGH | Falsifiability canary em `corelink-telemetry::logpush::redaction`: every regex used in log shipping is constrained to linear-time semantics. Catastrophic backtracking on attacker-controlled log lines could brown out the worker and degrade availability. | Property test corpus de adversarial inputs + ad-hoc `regex` config audit + bounded-time CI fuzz (DEBT-004 promotion) |
...
| **INV-AUTH-ISS-EXACT-MATCH** | Issuer compared exact match (não prefix) | CRITICAL | Allowlist `Vec<String>` exact eq; previne `https://clerk.corelink.humangr.com.attacker.com` confusion | Property test 10k iter random origin/issuer combinations | **TLA+ em `specs/tla/auth_jwt_validation.tla`** (DEBT-005 closure 2026-05-15) |
...
| **INV-AUTH-NEON-IS-SOT** | ⚠️ **DEFERRED / roadmap — design-intent, NOT the deployed path.** The Neon-Postgres `pat.revoked_at IS NULL` SoT + DO/KV/Queue revocation backbone is feature-gated (`tower-middleware`) and is NOT in the production verify hot path. **The LIVE PAT-revocation SoT is D1:** the container verifier checks `revoked_at_ms IS NULL` against the D1 `pat` table (migration `0063_pat_customer_keys`) on every verify — see `crates/corelink-container/src/adapter_pat.rs` — enforced under **INV-PAT-REVOKE-PROPAGATION** (§3.28). Revocation IS enforced in production today (via D1); only this Neon SoT design is deferred. | HIGH (roadmap) | DEFERRED design: the verify cold path WOULD query Neon; DO `is_revoked()` is the admin/audit query plane; KV a session cache. Cycle 4 codex SEAL canonical alignment per data_model.md §4.1 describes the TARGET Neon architecture, not the running system. LIVE enforcement path: `crates/corelink-container/src/adapter_pat.rs` (`revoked_at_ms IS NULL`, migration 0063). | Architecture review; the LIVE D1 revoke path is covered by INV-PAT-REVOKE-PROPAGATION integration + property tests | Deferred Neon queue-side design modelled by `auth_revocation.tla`; the LIVE D1 revoke path is TLA-verified by `auth_pat_revoke.tla` under INV-PAT-REVOKE-PROPAGATION |
...
| **INV-AUTH-WEBAUTHN-RP-ID-CANONICAL** | RP ID = "corelink.humangr.com" eTLD+1 (not subdomain) | CRITICAL | `WebAuthnAdapter::new` validates rp_id é eTLD+1; fails se subdomain | Static check em construtor | N/A (W3C spec compliance) |
...
| **INV-AUTH-PAT-OVERLOAD-SHED-UNIFORM** | O load shed do pool de permits Argon2id é uniforme em relação à existência da row em D1 | HIGH | Sob saturação do pool (global OU sub-cap per-tenant), **todo** desfecho do verifier do container é `VerifyError::Backend("pat verifier overloaded")` ⇒ HTTP 503 + `Retry-After`, independentemente de o `token_id` ter row viva em D1; fora de saturação, **toda** rejeição é `VerifyError::InvalidPat` ⇒ HTTP 401. Um shed assimétrico (503 com row / 401 sem row) seria um **row-existence oracle** mais forte que o de latência que o dummy-burn fecha — o burn é PULADO no shed, logo o status não pode carregar informação. Exceções deliberadas, ambas ainda `InvalidPat`: o HMAC fast-reject (a montante de qualquer permit ⇒ inalcançável sem a signing key) e a rejeição terminal depois do burn ter efetivamente rodado. As 4 superfícies adapter (cargo/brew/npm/pip) mapeiam `Backend` ⇒ 503, nunca 401. **ESCOPO PRECISO (2026-08-04):** a uniformidade é afirmada sob saturação de um tier que as DUAS arms compartilham — o pool GLOBAL, ou o tier per-tenant com o bucket de cada arm cheio; existe um tier intermediário em que elas **divergem por design** e isso é aceito: com o `UNKNOWN_TOKEN_BUCKET` compartilhado saturado, o bucket do tenant vivo LIVRE e o pool global folgado, a arm sem row faz shed (503) enquanto a arm com row viva roteia para o próprio bucket, paga um Argon2id real e resolve. É consequência direta da defesa do finding #12 (a arm row-NOT-FOUND tem bucket próprio e capado justamente para que um flood de `token_id`s bogus não drene o pool dos tenants reais) e não é um oracle explorável — as duas arms só existem a jusante do HMAC fast-reject, logo qualquer probing exige a PAT signing key (e quem a tem minta PATs, não precisa do bit de liveness), e a única superfície edge-unauthenticated (o `/token` do OCI) colapsa `InvalidPat` e `Backend` no MESMO 401 scrubbed. Não "consertar" para uniformidade: mover a arm viva para o bucket compartilhado transformaria um flood de tokens bogus num 503 fleet-wide, e tirar a arm desconhecida de lá restauraria o vetor de dreno | Unit tests em `crates/corelink-container/src/adapter_pat.rs` (`shed_is_indistinguishable_between_live_and_unknown_rows` = as duas arms byte-idênticas; `saturated_pool_sheds_an_unknown_row_as_backend`; `saturated_dummy_burn_bucket_sheds_an_unknown_row_as_backend`; `saturated_pool_sheds_a_live_row_as_backend`; `unsaturated_pool_still_rejects_an_unknown_row_as_invalid_pat`; `forged_token_stays_invalid_pat_under_saturation`; `a_saturated_shared_burn_bucket_sheds_only_the_unknown_arm` = a divergência ACEITA do escopo acima, pinada nas duas metades — shed na arm desconhecida E resolução Ok da arm viva no mesmo instante) + testes de status/`Retry-After` por superfície em `crates/corelink-adapter-host/src/{cargo,brew,npm,pip}/` | N/A (invariante de resposta; não-temporal — as duas arms são comparadas diretamente no teste) |
...
**Aliases históricos:** nenhum. Estes 38 IDs introduzidos em Lote 10.3 (sprint S-03 spec) e promovidos ao registry em Lote 10.3bis (P0 fix Agent R4 review remediation). Adições posteriores: **INV-AUTH-CONSTANT-TIME-COLD-PAD** (DEBT-004 promotion) e **INV-AUTH-PAT-OVERLOAD-SHED-UNIFORM** (2026-08-04, `fix/symmetric-overload-shed-503` — o split do shed não estava registrado, que foi exatamente por que ele foi achatado para `InvalidPat` sem ninguém notar).
...
| **INV-AC-TTL-REFRESH-MONOTONIC** | TTL refresh em `corelink-ac` infrastructure preserves monotonicity: refresh-on-hit clamps `last_hit_at` and `expires_at` to never move backward (sibling refinement of `INV-AC-TTL-MONOTONIC` covering the schema/sim layer) | HIGH | `corelink-ac::schema::sim` + `corelink-worker::reapi::ac::meta` clamp `last_hit_at` and `expires_at` via `MAX(prev, now)`; property test asserts cross-call monotonicity | Property test `prop_ac_ttl_refresh_monotonic` (10k iter) em `corelink-worker/tests/prop_ac_handlers.rs`; sim layer mirrored em `corelink-ac/src/schema/sim.rs` (DEBT-004 promotion) | N/A (architecture invariant; sibling INV-AC-TTL-MONOTONIC) |
...
| **INV-MULTIPART-CHUNK-DETERMINISTIC** | FastCDC mask seeds fixed em ChunkerConfig::default(); same input → same chunks byte-identical (Fixed and FastCDC) | CRITICAL | `corelink-cas::chunker` mask_s/mask_l constants; ADR-0022 stability commitment; test vectors Annex; CI byte-equal | Property test 1000 random × 100 chunkings = 100% byte-identical; ADR-0022 ratificada Lote 10.5 | N/A (cripto invariant) |
| **INV-MULTIPART-BOUNDED-PARSER** | Chunker MAX_BLOB_SIZE 160 GiB; MAX_CHUNKS_PER_BLOB 81920; manifest MAX_CHUNK_COUNT 81920 (Lote 10.5-tris P1-SR5-001/004 cross-crate alignment; 160 GiB / 2 MiB = 81920 exact); MAX_TOTAL_SIZE 160 GiB; enforced at decode time | HIGH | `corelink-cas::chunker::bounds` + `corelink-cas::manifest::bounds` constants share single source of truth; reject early; cargo-fuzz harness 1h CI nightly | Property test + cargo-fuzz × 3 targets (chunker fixed + chunker fastcdc + manifest decode) | N/A (parser invariant) |
| **INV-MULTIPART-STREAMING-MEMORY** | Per-request stack ≤ 4 MiB (chunker buffer 2 MiB + headroom); zero-allocation Iterator pattern | HIGH | `corelink-cas::chunker::Chunker::feed` returns `impl Iterator<Item = Chunk<'a>>` borrowing internal buffer; lifetime-bounded; integration test 1 GiB blob no OOM | Integration test + valgrind/MSAN no leak | N/A (architecture invariant) |
...
| **INV-MULTIPART-STREAMING-VERIFY-FAIL-FAST** | Mid-stream tampered chunk catches before chunk N+1 processed (SpliceBlob streaming verify) | HIGH | `corelink-cas::manifest::verify_streaming` per-chunk hash verify inline; fail-fast em mismatch; signal handler abort | Property test 100 manifests × 1 tampered chunk → 100% caught at chunk N | N/A (architecture invariant) |
...
| **INV-SUB-PROCESSOR-DKIM-TENANT-SCOPED** | DKIM signing key is derived per-tenant via HKDF-SHA256 (info=`corelink/v1/dkim-broadcast`); cross-tenant key isolation: tenant A's DKIM key NEVER equals tenant B's for any distinct tenant pair; prevents cross-tenant email spoofing | HIGH | `corelink-privacy::sub_processor::dkim::derive_dkim_key` HKDF derivation with salt=tenant_id; security_model.md §374 inheritance | Property test `prop_dkim_cross_tenant_isolation` 10k random tenant pairs → 0 key collisions (PROPTEST_CASES=10000 verified) | N/A (HKDF algorithmic isolation; statistical property; non-distributed) |
...
- `crates/corelink-privacy/src/sub_processor/` — trait surface + property tests (was `corelink-privacy-sub-processor-emit`).
...
INVs introduced by the `corelink-ops::dr::backup_verify` module (was `corelink-backup-verify` pre-Wave-35; R-prep continuous-verification scaffold). Promoted to registry in DEBT-004 closure pass — code-referenced but previously orphan in registry. Cross-references `RB-BACKUP-VERIFICATION.md` (forward) for operator workflow.

| ID | Nome | Severidade | Descrição | Enforcement | TLA+ file |
|---|---|---|---|---|---|
| **INV-BACKUP-FRESH** | Snapshot freshness gate: `verify_freshness` rejects any snapshot whose `age_seconds > rpo_seconds(tier)` | HIGH | `corelink-ops::dr::backup_verify::verify_freshness` strict comparison; tier-driven RPO budget table; rejection emits typed error preserving the existing snapshot generation | Property test boundary at exact RPO; integration test stale-snapshot rejected (DEBT-004 promotion) | N/A (algorithmic invariant; tier-table-driven) |
| **INV-BACKUP-INTEGRITY-SAMPLE-CAP** | Integrity sampler never exceeds `MAX_INTEGRITY_SAMPLES_PER_TENANT` per tenant per cycle (avoids accidental O(catalog) hot loops) | HIGH | `MAX_INTEGRITY_SAMPLES_PER_TENANT = 100` constant em `corelink-ops::dr::backup_verify`; sampler iterator is bounded; cargo-deny pinned bounds | Property test sampler bound enforcement + chaos test very-large-catalog (DEBT-004 promotion) | N/A (parser/bound invariant) |
...
**Aliases históricos:** nenhum. Promoted in DEBT-004 closure pass (2026-05-15) from `corelink-backup-verify` crate orphan refs (crate absorbed into `corelink-ops::dr::backup_verify` Wave-35 Phase 2).
...
**Aliases históricos:** nenhum. Promoted in DEBT-004 closure pass (2026-05-15) from `corelink-otel-export` crate orphan refs (crate absorbed into `corelink-telemetry::otel` Wave-35 Phase 2).

---

### 3.25 Tenant offboarding domain (domain OFFBOARDING) — DEBT-004 closure (2026-05-15)

INVs introduced by the `corelink-ops::tenant_offboarding` module (was `corelink-tenant-offboarding` pre-Wave-35; R-prep tenant-offboarding orchestrator scaffold). Promoted to registry in DEBT-004 closure pass.

| ID | Nome | Severidade | Descrição | Enforcement | TLA+ file |
|---|---|---|---|---|---|
| **INV-OFFBOARDING-GRACE-RESPECTED** | Every state advances only after the canonical timer threshold (or operator force-advance with audit trail); orchestrator validates the timer at the trait boundary; a request that asks for an advance before the threshold returns `TenantOffboardingError::IllegalTransition` | HIGH | `corelink-ops::tenant_offboarding::orchestrator` enforces `now >= state_started_at + grace_for(state)` on every advance call; force-advance path required to emit a typed audit row | Property test `prop_tenant_offboarding` 10k iter boundary + integration test force-advance audit (DEBT-004 promotion) | N/A (architecture invariant; sibling of INV-GC-GRACE-RESPECTED) |
| **INV-OFFBOARDING-AUDIT-COMPLETE** | Every state mutation is preceded by the canonical audit row (per ADR-S11-002 split-tier fail-CLOSED discipline); audit emit failure aborts the transition; durable store remains at pre-call state | CRITICAL | `corelink-ops::tenant_offboarding` audit-emit-BEFORE-mutate ordering; trait contract requires emit success precondition; `FailingAuditSink` test path verifies state unchanged | Property test `prop_tenant_offboarding` audit-mutation ordering 10k iter + chaos test simulate audit failure (DEBT-004 promotion) | Covered by `audit_immutability.tla` ✅ GREEN inheritance |
...
**Aliases históricos:** nenhum. Promoted in DEBT-004 closure pass (2026-05-15) from `corelink-tenant-offboarding` crate orphan refs (crate absorbed into `corelink-ops::tenant_offboarding` Wave-35 Phase 2).

---

### 3.26 Progressive rollout domain (domain ROLLOUT) — DEBT-004 closure (2026-05-15)

INVs introduced by the `corelink-replication::rollout_controller` module (was `corelink-replication::rollout_controller` pre-Wave-35; WI-S13-005 progressive rollout + 3-trigger auto-rollback). Promoted to registry in DEBT-004 closure pass.
...
| **INV-ROLLOUT-NO-STAGE-SKIP** | Stage advance enforces `RolloutStage::next()`; skip → 403 + audit emit | HIGH | `corelink-replication::rollout_controller::{state_machine, controller, types}` enforce monotonic `next()`; skip path returns typed `StageSkip` error + audit emit; CI grep gate | Property test `prop_rollout` 10k iter stage transitions + integration test (DEBT-004 promotion) | (planned `rollout_state_machine.tla`; PLANNED) |
| **INV-ROLLOUT-COSIGN-GATE** | Deploy without a valid Cosign signature is rejected at `start()` (S-12 supply chain herdada) | CRITICAL | `corelink-replication::rollout_controller::start` verifies Cosign signature inclusion in deployable; failure → typed `CosignMissing` error + audit emit (subsumido por INV-SUPPLY-SIGNED-DEPLOY at the rollout entry point) | Integration test sig-missing rejection + adversarial test (DEBT-004 promotion) | **`rollout_cosign_gate.tla` ✅ GREEN** (DEBT-005 batch 2 — 2026-05-15) — `InvActiveIsSigned` + `InvRejectionAudited` + `InvActivationAudited`; adversarial `AttemptUnsignedActivate` enumerated as FALSE-guarded unreachable trace. Also discharges runtime side of INV-SUPPLY-SIGNED-DEPLOY via `InvSupplySignedDeploy`. |
| **INV-ROLLOUT-BUDGET-CAP** | Auto-rollback consumes ≤ 30% monthly error budget; exceedance → freeze + SEV-2 | HIGH | `corelink-replication::rollout_controller::controller` tracks budget consumption per rollback fire; exceedance → freeze state + SEV-2 alert + audit emit | Property test `prop_rollout` budget-cap boundary + integration test SEV escalation (DEBT-004 promotion) | N/A (architecture invariant; SLO budget invariant) |
| **INV-ROLLOUT-AUTO-ROLLBACK-TRIGGERS** | 3-trigger auto-rollback (error-rate > baseline+3σ; SLO burn-rate > 14.4 1h window; p99 latency > baseline+50%) only fires after `SUSTAINED_THRESHOLD_SECS = 300s` continuous observation (filters transient variance); detection p99 ≤ 360s (6 probes × 60s) | HIGH | `corelink-replication::rollout_controller::auto_rollback::SustainedTrigger` tracks per-trigger elapsed; reset to 0 on clear; fire only at ≥ 300s. Google SRE Workbook Ch 16 alignment. | Property test `prop_rollout` 10k iter sustained-threshold boundary + chaos test transient-spike-no-fire (DEBT-004 promotion) | (planned `rollout_state_machine.tla` auto-rollback variant; PLANNED) |
...
**Container native-plane carve-out (WP-D M26, 2026-07-17):** the "no edge cache lookahead" language above, and `InvRevokedTokenNeverValidates`, scope the **Worker edge + adapter-D1 verify path** (`crates/corelink-container/src/adapter_pat.rs`, `revoked_at_ms IS NULL` checked on every call). They do **NOT** cover the container's separate `NativePatGate` in-memory verify cache (`crates/corelink-container/src/native_pat_gate.rs`, `VERIFY_CACHE_TTL = 5s`): a cache hit there returns `Ok` WITHOUT re-consulting D1, so a PAT revoked mid-window keeps NATIVE-plane (CAS/AC/Bazel/Turbo) access for up to 5s. This is a deliberate, BOUNDED-stale surface, not a violation of this INV — 5s ⊂ the ≤60s propagation-window SLA the INV enforces, and it does not widen it. Immediate invalidation there would require either a per-request D1 read on the native hot path (rejected — regresses the CAS hot path) or a cross-process revocation broadcast (the DEFERRED ADR-0030 Neon/DO/Queue backbone — see `docs/knowledge/adr/adr-0030-revocation-propagation.md`). Regression-tested: `crates/corelink-container/src/native_pat_gate.rs::tests::cache_expiry_forces_reverify_and_rejects_revoked_pat` (bound: cache hit → expiry → miss re-verifies → 401 once D1 shows revoked) + `::tests::cache_hit_does_not_extend_expiry` (no-slide: repeated hits never push `expires_at` out).
...
| ID | Nome | Severidade | Descricao | Enforcement | TLA+ file |
|---|---|---|---|---|---|
| **INV-CROSS-TENANT-DENIED** | Every customer handler request carries `caller_tenant` and optional `requested_tenant` (defaulting to caller); if the target differs, the handler emits a `*Denied` audit event BEFORE returning `CrossTenantDenied` | CRITICAL | Enforced by the shared `reject_cross_tenant` choke point in both `corelink-handler-customer` and the D1-backed customer handler (overview, usage, billing, keys, team, audit query); the target tenant is checked before lookup/mutation and audit failure returns `AuditFailed` | Behavioral + mutation-kill tests in `corelink-handler-customer/tests/` and `cross_tenant_handler_audit.tla` | `specs/tla/cross_tenant_handler_audit.tla` + `.cfg` |
...
| INV-CROSS-TENANT-DENIED | `specs/tla/cross_tenant_handler_audit.tla` + `.cfg` | ✅ GREEN (B-255) — independent handler refinement models all six customer groups and proves each `CrossTenantDenied` return has its matching `*Denied` audit immediately before it; audit sink failure has no denial return. |
...
| INV-AUTH-PAT-OVERLOAD-SHED-UNIFORM | HIGH | Propriedade de RESPOSTA, single-process e não-distribuída: é a igualdade de duas arms do MESMO `match` dentro de `PatVerifier::verify_capability`, decidida sem I/O e sem estado compartilhado entre requests. Não há espaço de interleaving para o TLC explorar — o teste `shed_is_indistinguishable_between_live_and_unknown_rows` compara os dois `VerifyError` DIRETAMENTE (mesma variante + mesma string) num único verifier saturado, o que é uma prova mais forte e mais barata que um modelo. Sibling em espírito de INV-AUTH-CONSTANT-TIME-COLD-PAD, que fecha o mesmo oráculo no eixo LATÊNCIA; este fecha o eixo STATUS, que a queima dummy explicitamente NÃO cobre (no shed a queima é pulada). O escopo exato (uniformidade sob saturação de um tier COMPARTILHADO; divergência aceita quando só o `UNKNOWN_TOKEN_BUCKET` está saturado) está no §3.14 e é pinado por `a_saturated_shared_burn_bucket_sheds_only_the_unknown_arm` — também uma comparação direta, sem espaço temporal a modelar. Cobertura complementar por superfície: testes de status/`Retry-After` em cargo/brew/npm/pip |
...
| INV-AUTH-NEON-IS-SOT | HIGH (roadmap) | ⚠️ DEFERRED design-intent: the Neon-Postgres SoT + DO/KV/Queue backbone is feature-gated (`tower-middleware`), NOT the deployed path. The LIVE PAT-revocation SoT is **D1** (`revoked_at_ms IS NULL`, migration `0063`, `crates/corelink-container/src/adapter_pat.rs`), enforced under INV-PAT-REVOKE-PROPAGATION and TLA-verified by `auth_pat_revoke.tla` (✅ GREEN §4.1); the deferred Neon queue-side design is modelled by `auth_revocation.tla` |
| INV-AUTH-SCHEMA-RLS-DEFAULT-ON | CRITICAL | Schema invariant; Postgres RLS WITH CHECK on every txn (`set_local app.current_tenant`); covered by `corelink-auth/src/schema/tests/integration/rls.rs` + property test 10k iter cross-tenant attempts → 0 leaks |
...
| INV-AUTH-WEBAUTHN-UV-REQUIRED-ADMIN | CRITICAL | Algorithmic; UV flag check on admin assertion; covered by `corelink-auth::webauthn::assertion` adversarial test (UV=0 admin → rejected) |
| INV-AUTH-WEBAUTHN-ATTESTATION-VERIFIED | CRITICAL | Algorithmic; FIDO Metadata Service AAGUID check + attestation chain verify; covered by `corelink-auth::webauthn::registration` test |
...
| INV-AUTH-WEBAUTHN-ORIGIN-EXACT | CRITICAL | Algorithmic; exact-string compare on `origin` from clientDataJSON; covered by `corelink-auth::webauthn` adversarial test |
| INV-AUTH-WEBAUTHN-RP-ID-CANONICAL | CRITICAL | Algorithmic; SHA-256 hash compare on rpIdHash from authData; covered by `corelink-auth::webauthn` adversarial test |
...
| INV-NEG-CACHE-MONOTONIC | HIGH | Algorithmic; negative-cache TTL only refreshable upward (max-of); covered by property test 10k iter + `corelink-cas::edge::neg_cache` unit test |
...
| INV-AC-IDEMPOTENT | HIGH | Algorithmic; AC execution keyed by `(action_digest_hash, blob_digest, tenant_id, action_input_root)`; covered by `corelink-ac/tests/ac_core_idempotency.rs` property test 10k iter |
...
| INV-AC-NEG-CACHE-INVALIDATED-ON-UPDATE | HIGH | Algorithmic; on AC result write, neg-cache for matching key TTL → 0; covered by `corelink-ac::ac_core::neg_cache` integration test |
| INV-AC-MERKLE-VALID | CRITICAL | Algorithmic; AC Merkle codec uses `corelink-hash` BLAKE3 with canonical-bytes encoding; covered by `corelink-ac::ac_core` property test 10k iter (malformed → reject) |
| INV-AC-MERKLE-DETERMINISTIC | CRITICAL | Algorithmic; same input bytes → same Merkle root; covered by `corelink-ac::ac_core::merkle::roundtrip` property test |
| INV-AC-BOUNDED-PARSER | HIGH | Algorithmic; AC parser bounded recursion depth + max-node-count; covered by `corelink-ac::ac_core::parser` adversarial test (deep nesting → bounded error) |
| INV-AC-CYCLE-FREE | HIGH | Algorithmic; AC DAG cycle detection via topological sort; covered by `corelink-ac::ac_core::dag` property test |
...
| INV-AC-DIGEST-SIGNED | CRITICAL | Algorithmic; AC digest signed via `Ed25519` from `corelink-erasure-attestation` key set; covered by `corelink-ac::ac_core::sig` test + `auth_audit_pseudonymization.tla` (✅ GREEN §4.1) for sig-key integrity |
| INV-AC-SIG-CONSTANT-TIME | HIGH | Algorithmic; `subtle::ConstantTimeEq` on Ed25519 sig verify; covered by `corelink-ac::ac_core` adversarial test |
...
| INV-AC-TTL-MONOTONIC | HIGH | Algorithmic; AC TTL only refreshable upward; covered by `corelink-ac::ac_core::ttl` property test |
...
| INV-AC-TTL-REFRESH-MONOTONIC | HIGH | Algorithmic; refresh path is monotonic (max-of); covered by `corelink-ac::ac_core::ttl::refresh` property test |
...
| INV-MULTIPART-MANIFEST-SIGNED | HIGH | Algorithmic; manifest signed via `corelink-erasure-attestation` Ed25519; covered by `corelink-cas::multipart_schema::manifest` adversarial test (forged sig → rejected) |
...
| INV-LRU-CONSISTENCY | HIGH | Algorithmic; LRU tracker monotonic recency stamps; covered by `corelink-cas::lru_tracker::recency` property test |
...
| INV-MULTIPART-CHUNK-DETERMINISTIC | CRITICAL | Algorithmic; chunker produces same BLAKE3 boundaries for same input bytes; covered by `corelink-cas::chunker` property test cross-platform |
...
| INV-MULTIPART-MANIFEST-VALID | CRITICAL | Algorithmic; manifest schema validates all parts present, sizes match, hashes valid; covered by `corelink-cas::multipart_schema::manifest::validate` property test |
...
| INV-OBS-CONFIG-NON-EXHAUSTIVE | HIGH | Type-level invariant; observability config types use `#[non_exhaustive]`; covered by compiler check + `corelink-telemetry::otel::config` test |
| INV-OBS-CT-SECRET-EQ | HIGH | Algorithmic; `subtle::ConstantTimeEq` on observability secret values (export-token compare); covered by `corelink-telemetry::otel::secret` adversarial test |
| INV-OBS-EXPORT-FAIL-OPEN | HIGH | Architectural; observability export failures do NOT block request path (fail-OPEN by design — opposite of audit fail-CLOSED); covered by `corelink-telemetry::otel::exporter` integration test |
| INV-QUOTA-RESERVATION-TTL | HIGH | Algorithmic; quota reservation TTL bounded (30s); auto-release on TTL; covered by `corelink-billing::quota::reservation` TTL test |
| INV-ROLLOUT-AUTO-ROLLBACK-TRIGGERS | HIGH | Algorithmic; auto-rollback triggers (error-rate / latency / SLO breach) defined as `const` thresholds; covered by `corelink-replication::rollout_controller::rollback::triggers` test + `rollout_cosign_gate.tla` (✅ GREEN §4.1) inheritance |
| INV-ROLLOUT-BUDGET-CAP | HIGH | Algorithmic; rollout error budget cap per stage; exceeding → halt; covered by `corelink-replication::rollout_controller::budget` test |
| INV-ROLLOUT-NO-STAGE-SKIP | HIGH | Algorithmic; rollout stages strictly sequential (canary → 10% → 50% → 100%); no skip; covered by `corelink-replication::rollout_controller::stage` state-machine test |
| INV-ROLLOUT-SINGLE-ACTIVE | HIGH | Architectural; only ONE rollout active per service at a time; DO lock enforces; covered by `corelink-replication::rollout_controller::lock` integration test |
| INV-S17-CHAOS-STAGING-ONLY | HIGH | Operational; chaos engineering runs ONLY in staging env; production chaos disabled by feature flag; covered by `corelink-telemetry::canary::chaos` env-gate test |
| INV-S17-ONCALL-FATIGUE-AUTOROTATE | HIGH | Operational; on-call fatigue detection auto-rotates to backup engineer after threshold; covered by `corelink-ops::oncall::fatigue` rotation test |
...
| INV-S17-SEV1-DRILL-PAUSE | HIGH | Operational; SEV-1 incidents pause active chaos drills automatically; covered by `corelink-ops::oncall::sev1` pause-drill integration test |
| INV-BAZEL-DIGEST-VALIDATE | HIGH | Parser/protocol invariant; `Digest::parse` enforces exactly-64-lowercase-hex + size_bytes <= 4 GiB + payload-length == size_bytes on PUT; non-distributed; covered by proptest in `corelink-bazel-bridge/tests/integration.rs` + `corelink-bazel-bridge::digest` unit tests |
| INV-BAZEL-FIND-MISSING-CAP | HIGH | Protocol bound invariant; batch cap of 4096 enforced at `FindMissingHandler` entry before any CAS query; algorithmic single-process guard; covered by `corelink-bazel-bridge/tests/integration.rs` over-limit test |
| INV-BAZEL-NO-GRPC | HIGH | Dependency/build invariant; `cargo-deny` + CI grep gate forbid `tonic`/`prost` in `corelink-bazel-bridge/Cargo.toml`; build-time enforced, non-runtime semantics |
| INV-CLERK-TENANT-SCOPE | HIGH | Algorithmic; exact byte compare in `ClerkHealthLogic::assert_tenant`; non-distributed; subsumed by INV-TENANT-ISOLATION; covered by proptest `prop_inv_parsed_route_tenant_scope_rejects_mismatch` (~10k iter) in `corelink-clerk-cf/tests/prop_clerk_health_logic.rs` |
| INV-CLERK-ROUNDTRIP | HIGH | Algorithmic; `upsert` -> `get` field-equality enforced by canonical map in `ClerkHealthLogic`; non-distributed; covered by proptest `prop_inv_upsert_get_roundtrip_preserves_record` in `corelink-clerk-cf/tests/prop_clerk_health_logic.rs` |
| INV-CLERK-SWEEP-BOUNDARY | HIGH | Algorithmic; strict `<` boundary in `ClerkHealthState::sweep`; deterministic, non-distributed; covered by proptest `prop_inv_sweep_ttl_boundary_exact` in `corelink-clerk-cf/tests/prop_clerk_health_logic.rs` |
| INV-STATUSPAGE-WIRE-ROUNDTRIP | HIGH | Algorithmic; serde_json roundtrip on `DsrCompletionReport`; non-distributed; covered by proptest `prop_inv_dsr_completion_report_serde_roundtrip` in `corelink-statuspage-real/tests/prop_statuspage_invariants.rs` |
| INV-STATUSPAGE-RATELIMIT-NO-DRIFT | HIGH | Algorithmic; `DenyBackoff` leaves `last_allowed_ms` unchanged in `StatuspageRateLimiter::decide`; deterministic, non-distributed; covered by proptest `prop_inv_rate_limiter_denied_does_not_advance_window` in `corelink-statuspage-real/tests/prop_statuspage_invariants.rs` |
| INV-STATUSPAGE-RETRY-TOTAL-AND-MONOTONE | HIGH | Algorithmic; `RetryPolicy::wave16_default` is total over all u16 statuses + `backoff_for` monotone non-decreasing; deterministic, non-distributed; covered by proptest `prop_inv_retry_policy_classification_total_and_monotone` in `corelink-statuspage-real/tests/prop_statuspage_invariants.rs` |
| INV-WASM-PUT-HEX64 | HIGH | Algorithmic; BLAKE3 32-byte -> 64 hex is a pure function of hash output; non-distributed; covered by proptest `prop_put_inner_emits_lowercase_hex64` in `corelink-wasm/src/lib.rs` |
| INV-WASM-PUT-DETERMINISTIC | HIGH | Algorithmic; BLAKE3 is a deterministic function of input bytes; non-distributed; covered by proptest `prop_put_inner_is_deterministic` in `corelink-wasm/src/lib.rs` |
| INV-WASM-PUT-ROUNDTRIP | HIGH | Algorithmic; hex encode -> parse -> re-encode identity via `Digest::from_hex`; non-distributed; covered by proptest `prop_put_inner_roundtrip_hex` in `corelink-wasm/src/lib.rs` |
| INV-WASM-GET-VERIFY-MATCH | HIGH | Algorithmic; BLAKE3 verify on `get_inner` with `client_verify=true`; mirrors INV-CAS-INTEGRITY at WASM/FFI boundary; non-distributed; covered by proptest `prop_get_inner_verify_match_or_mismatch` in `corelink-wasm/src/lib.rs` |
| INV-WASM-GET-PANIC-FREE | HIGH | FFI invariant; all arbitrary UTF-8 inputs to `get_inner` return `Ok` or typed `Err` (never panic); algorithmic, non-distributed; covered by proptest `prop_get_inner_panic_free` (any UTF-8 string) in `corelink-wasm/src/lib.rs` |
| INV-WASM-STAT-ECHO | HIGH | Algorithmic; `stat_inner` echoes parsed digest into `StatResult.digest`; stub echo contract; non-distributed; covered by proptest `prop_stat_inner_echoes_valid_digest` in `corelink-wasm/src/lib.rs` |
