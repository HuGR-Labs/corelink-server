...
>
> **GA-readiness unlock note (2026-05-16, wave-24):** per `specs/_audits/sealed/2026-05-16-ga-readiness-final.md` §17, Owner sign-off via the ADR-0034b 2-key path (Owner + on-call SRE, §13.2 + §13.3 of that audit) **explicitly unlocks** the v1.0.0 promotion at wave-27 (post-GA cutover, T+7d to T+30d stable-state window). The framework cannot be promoted on the GA-cutover-day signature itself; promotion requires a separate post-cutover stable-state period. This RC1 → v1.0.0 promotion is a wave-27 deliverable (`specs/_audits/2026-MM-DD-framework-v1.0.0-promotion.md` placeholder, to be authored at promotion time).
>
> **Wave-26 RC2 readiness note (2026-05-16):** version bump RC1 → **RC2** captures the wave-20 / wave-22 / wave-24 absorption arc: the `(a nomear)` slots in §43.1 now resolve to named role profiles `FW-H-1..4` (proposal + addendum + ADR-0034b dual-hat fallback). The §42 change-log entry template for the eventual FROZEN cut is pre-authored below (see §42 "TEMPLATE — v1.0.0 GA cut entry (pre-authored, awaiting Owner)"). **The engineering side of the freeze gate is now READY for Owner sign-off via either Option A (4 distinct nominations) or Option C (ADR-0034b dual-hat Pairing-Alpha/Beta).** Estimated Owner action: ~30 min read of `_proposals/2026-05-16-framework-reviewer-roles.md` + addendum + `ADR-0034b` + decision + fill-in of the pre-authored §42 template + tag `framework-v1-0-0-ga`. See `specs/_audits/sealed/2026-05-16-lote-6-v1-rc2-ready.md` for the consolidated readiness summary and `specs/_audits/sealed/2026-05-15-framework-v1-0-0-ga-audit.md §11.8` for the wave-26 closure entry.
> **Versão:** 1.0.0-rc2
...
| **1.0.0-rc2** | **2026-05-16** | **Claude Opus 4.7 (wave-26 Lote 6 RC2 readiness agent)** | **Wave-26 RC2 readiness cut — pre-FROZEN bump.** Closes the wave-20 / wave-22 / wave-24 absorption arc into a single Owner-actionable surface: §43.1 `(a nomear)` slots now resolve to named role profiles `FW-H-1..4` with placeholder convention (`PROPOSED-OWNER-DUAL-HAT` for ADR-0034b dual-hat path; `(a nomear)` for Option A 4-distinct path); §42 (this section) pre-authors the v1.0.0 GA FROZEN-cut entry template below so the Owner fills 1–3 lines to invoke Option A or Option C. **No promotion** — `doc_status` remains DRAFT; `version` moves RC1 → RC2 to signal "engineering-side READY for Owner sign-off via either path". The FROZEN cut itself remains a follow-up commit gated on Owner action per `specs/_audits/sealed/2026-05-15-framework-v1-0-0-ga-audit.md §6.3 / §11.8` (wave-26 closure) and `specs/_audits/sealed/2026-05-16-lote-6-v1-rc2-ready.md`. Estimated Owner action: ~30 min read of proposal + addendum + ADR-0034b + decision + fill-in of the template entry + tag `framework-v1-0-0-ga`. |

| **1.0.0** | **YYYY-MM-DD** *(TEMPLATE — v1.0.0 GA cut entry, pre-authored, awaiting Owner)* | **Gustavo Schneiter (Final Approver) + reviewers per §43.1** | **TEMPLATE — fill in at FROZEN cut.** Owner replaces this row with one of the two completed shapes below at the v1.0.0 GA tag commit; the other shape is deleted. `doc_status` flips DRAFT → FROZEN simultaneously; `version` moves 1.0.0-rc2 → 1.0.0. The `audit_status` remains ACTIVE per §41.5 trimestral cadence (§7.1 / §43.1 semantics).<br><br>**Shape A — Option A (4 distinct staffing).** *Fill: nominee names + dates + SHAs.* "Framework v1.0.0 GA FROZEN cut under Option A (4 distinct reviewers). §43.1 signatures: FW-H-1 `<name1> — <YYYY-MM-DD> — sha:<7>`; FW-H-2 `<name2> — <YYYY-MM-DD> — sha:<7>`; FW-H-3 `<name3> — <YYYY-MM-DD> — sha:<7>`; FW-H-4 `<name4> — <YYYY-MM-DD> — sha:<7>`; Final Approver `Gustavo Schneiter — <YYYY-MM-DD> — sha:<7>`. `authorizing_adr: <none — Option A>`. `owner_conflict_disclosed: <none — Option A>`. Acceptance deliverable comments docs filed at `specs/_audits/2026-MM-DD-fw-h-{1,2,3,4}-acceptance-comments.md`. Tag: `framework-v1-0-0-ga`."<br><br>**Shape C — Option C (ADR-0034b dual-hat fallback).** *Fill: pairing choice + 2 external advisor names + 2 dates + SHAs.* "Framework v1.0.0 GA FROZEN cut under Option C (ADR-0034b dual-hat fallback). `authorizing_adr: ADR-0034b`. `pairing: <alpha|beta>` (Pairing-Alpha → Owner takes FW-H-1 + FW-H-3, externals fill FW-H-2 + FW-H-4 / Pairing-Beta → Owner takes FW-H-2 + FW-H-4, externals fill FW-H-1 + FW-H-3). `owner_conflict_disclosed: dual-hat under ADR-0034b Pairing-<alpha|beta>; Final Approver structural COI per ADR-0034b §Forbidden pairings rationale + addendum §7.4`. §43.1 signatures (Pairing-Alpha example; mirror-swap for Beta): FW-H-1 `Gustavo Schneiter (Owner dual-hat) — <YYYY-MM-DD> — sha:<7>`; FW-H-2 `<external1-name> — <YYYY-MM-DD> — sha:<7>`; FW-H-3 `Gustavo Schneiter (Owner dual-hat) — <YYYY-MM-DD> — sha:<7>`; FW-H-4 `<external2-name> — <YYYY-MM-DD> — sha:<7>`; Final Approver `Gustavo Schneiter — <YYYY-MM-DD> — sha:<7>`. Owner self-attestation per ADR-0034b §Eligibility item 4 recorded inline. Eligibility check (all 5 of ADR-0034b §Eligibility): (1) headcount < 12 ✅ / (2) SOC2 / ISO27001 separation-of-duty not yet contractually required ✅ / (3) ≥ 2 external advisors retained ✅ / (4) Owner self-attests competence in dual-hatted slots ✅ / (5) ADR-0034b cited (this entry) ✅. Acceptance deliverable comments docs filed at `specs/_audits/2026-MM-DD-fw-h-{1,2,3,4}-acceptance-comments.md` (Owner authors 2 distinct docs per ADR-0034b §Sign-off mechanics). Auto-expiration triggers monitored at trimestral review (headcount ≥ 10 / SOC2 kickoff / 18-month ceiling / Owner role transition). Tag: `framework-v1-0-0-ga`." |
...
> **Wave-20 Lote 7 update.** Role definitions for the 3 "(a nomear)" slots below are proposed in [`specs/_proposals/2026-05-16-framework-reviewer-roles.md`][FW-REVIEWER-ROLES] (FW-H-1 Software Architecture Lead; FW-H-3 Security Lead; FW-H-4 Production Operations Lead; +1 extracted slot FW-H-2 Compliance & Privacy Lead — see proposal §8 OQ-1/OQ-2 for org-structure choices). Until the Owner nominates individuals and they complete the §3 acceptance deliverables, this section remains "(a nomear)" and v1.0.0 GA freeze stays DEFERRED per `specs/_audits/sealed/2026-05-15-framework-v1-0-0-ga-audit.md §6.3 Path A/B/C`.
>
> **Wave-24 dual-hat fallback option.** A small-org operating mode is available: the Owner MAY take 2 of the 4 FW-H-* slots personally (Pairing-Alpha: FW-H-1 + FW-H-3; Pairing-Beta: FW-H-2 + FW-H-4) with the remaining 2 slots filled by retained external advisors. The fallback is authorized by [`specs/03_architecture/adrs/ADR-0034b-framework-reviewer-dual-hat-fallback.md`][ADR-0034B] (status PROPOSED, transitions to ACCEPTED on Owner invocation) and specified operationally in [`specs/_proposals/2026-05-16-framework-reviewer-roles-addendum.md §1`][FW-REVIEWER-ROLES-ADDENDUM]. Auto-expiration triggers (headcount ≥ 10, SOC2 kickoff, 18-month ceiling, Owner role transition) apply.
>
> **Wave-26 RC2 placeholder convention.** Each FW-H-* slot below is labeled with one of two placeholder values, which the Owner replaces at FROZEN-cut time:
>
> - **`PROPOSED-OWNER-DUAL-HAT`** — slot will be filled by the Owner under ADR-0034b Pairing-Alpha or Pairing-Beta (Option C). The §42 change-log entry MUST cite `authorizing_adr: ADR-0034b` and `pairing: alpha | beta`.
> - **`(a nomear)`** — slot awaits 4-distinct staffing (Option A); no waiver invoked.
>
> The Owner replaces these placeholders with `<Name> — <YYYY-MM-DD> — sha:<7-char>` at the v1.0.0 FROZEN cut commit. Under dual-hat the Owner's name appears on BOTH of their slots per ADR-0034b §Sign-off mechanics. The pre-authored §42 change-log entry template (see §42) captures the exact field shape required.

- [ ] **Gustavo Schneiter** (Aprovador Final / FW-H-0) — ____________ YYYY-MM-DD
- [ ] **Revisor Técnico — FW-H-1 Software Architecture Lead** *(role definition: [FRAMEWORK-REVIEWER-ROLES][FW-REVIEWER-ROLES] §2.1; current placeholder: `PROPOSED-OWNER-DUAL-HAT` under ADR-0034b Pairing-Alpha OR `(a nomear)` under Option A)* — ____________ YYYY-MM-DD
- [ ] **Revisor de Compliance & Privacidade — FW-H-2 Compliance & Privacy Lead** *(role definition: [FRAMEWORK-REVIEWER-ROLES][FW-REVIEWER-ROLES] §2.2; current placeholder: `PROPOSED-OWNER-DUAL-HAT` under ADR-0034b Pairing-Beta OR external advisor under Pairing-Alpha OR `(a nomear)` under Option A)* — ____________ YYYY-MM-DD
- [ ] **Revisor de Segurança — FW-H-3 Security Lead** *(role definition: [FRAMEWORK-REVIEWER-ROLES][FW-REVIEWER-ROLES] §2.3; current placeholder: `PROPOSED-OWNER-DUAL-HAT` under ADR-0034b Pairing-Alpha OR external advisor under Pairing-Beta OR `(a nomear)` under Option A)* — ____________ YYYY-MM-DD
- [ ] **Revisor de Produção / Operações — FW-H-4 Production Operations Lead** *(role definition: [FRAMEWORK-REVIEWER-ROLES][FW-REVIEWER-ROLES] §2.4; original "Revisor de Produto" framing re-scoped per [FRAMEWORK-REVIEWER-ROLES][FW-REVIEWER-ROLES] §8 OQ-2; current placeholder: `PROPOSED-OWNER-DUAL-HAT` under ADR-0034b Pairing-Beta OR external advisor under Pairing-Alpha OR `(a nomear)` under Option A)* — ____________ YYYY-MM-DD

[FW-REVIEWER-ROLES]: _proposals/2026-05-16-framework-reviewer-roles.md
[FW-REVIEWER-ROLES-ADDENDUM]: _proposals/2026-05-16-framework-reviewer-roles-addendum.md
[ADR-0034B]: 03_architecture/adrs/ADR-0034b-framework-reviewer-dual-hat-fallback.md
