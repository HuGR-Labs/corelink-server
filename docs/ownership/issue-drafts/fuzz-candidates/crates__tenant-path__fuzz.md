<!-- corelink-ownership:v1:manifest=crates/tenant-path/fuzz/Cargo.toml -->
# [ownership] corelink-tenant-path-fuzz: skill, referência, blast radius e manutenção

## Identidade e preparação

| Campo | Valor |
|---|---|
| Package / manifesto | corelink-tenant-path-fuzz / crates/tenant-path/fuzz/Cargo.toml |
...
**Contexto canônico OKF:** Consultar `scripts/okf_context.py --file crates/tenant-path/fuzz/Cargo.toml` e arquivos de implementação depois de enumerá-los; confirmar os conceitos em `docs/knowledge/index.md`. A consulta não foi executada neste estudo.
**Riscos que não podem desaparecer na documentação:** `crates/tenant-path/fuzz/Cargo.toml` foi confirmado como package com workspace próprio. Registrar relação com `crates/tenant-path`, corpus/targets, limites de recursos, timeout, reprodução/minimização de caso local e diferenças em relação aos testes da crate-mãe. Não duplicar seu manual.
...
**Identidade:** `corelink-tenant-path-fuzz`; manifesto `crates/tenant-path/fuzz/Cargo.toml`; skill `own-corelink-tenant-path-fuzz`.
**Fonte imutável:** [declaração do package](https://github.com/HuGR-Labs/corelink-server/blob/cca798ff5bc2df660ecf2570ed243eb9775ff3d0/crates/tenant-path/fuzz/Cargo.toml#L3-L4); blob Git reportado `aaeeb71ed797939dfa745450b5a4640ef344e579`.
...
**Fonte desses campos:** [manifesto inspecionado](https://github.com/HuGR-Labs/corelink-server/blob/cca798ff5bc2df660ecf2570ed243eb9775ff3d0/crates/tenant-path/fuzz/Cargo.toml).
...
|---|---|---|
| `derive_prefix` | bin | `fuzz_targets/derive_prefix.rs` |
| `derive_prefix_extended` | bin | `fuzz_targets/derive_prefix_extended.rs` |

Esta tabela contém os targets explicitamente inspecionados, não um inventário automático completo.

**Chaves de dependência inspecionadas:** `corelink-tenant-path`, `libfuzzer-sys`, `uuid`, `zeroize`. Resolução de herança, aliases, features e inversas continua pendente.

**Arestas locais declaradas:** `corelink-tenant-path` via `path=..`.

**Política do harness:** `unsafe_code=forbid`; versão `0.0.0`; `[workspace]` próprio.

### Pontos específicos a investigar

- O manifesto descreve determinismo de prefixos e entradas arbitrárias de paths.
...
```sh
cargo metadata --locked --offline --no-deps --format-version=1 --manifest-path crates/tenant-path/fuzz/Cargo.toml
```
...
- `cargo metadata --locked --offline --no-deps --format-version=1 --manifest-path crates/tenant-path/fuzz/Cargo.toml` — READ_ONLY; NOT_EXECUTED.
- `python3 scripts/okf_context.py --file crates/tenant-path/fuzz/fuzz_targets/derive_prefix.rs --full` — READ_ONLY; NOT_EXECUTED.
- `cargo tree --locked --offline --manifest-path crates/tenant-path/fuzz/Cargo.toml -p corelink-tenant-path-fuzz --target x86_64-unknown-linux-gnu --edges normal,build` — READ_ONLY_RESOLUTION; NOT_EXECUTED.

### Evidência de fonte
- `crates/tenant-path/fuzz/Cargo.toml` @ `aaeeb71ed797939dfa745450b5a4640ef344e579`; source `37f96dba42cc159cd8ab8e9c4ab9d35b8e5634b5`.
- `crates/tenant-path/fuzz/fuzz_targets/derive_prefix.rs` @ `465fcb908f422101e87e4e7662e13e0dfaf77d97`; source `37f96dba42cc159cd8ab8e9c4ab9d35b8e5634b5`.
- `crates/tenant-path/fuzz/fuzz_targets/derive_prefix_extended.rs` @ `fa2a23560b4af0f5070363de6294e9b751228134`; source `37f96dba42cc159cd8ab8e9c4ab9d35b8e5634b5`.
...
- [ ] `.claude/skills/own-corelink-tenant-path-fuzz/SKILL.md`.
- [ ] `docs/ownership/crates/corelink-tenant-path-fuzz/REFERENCE.md`, único e indexado.
- [ ] `docs/ownership/crates/corelink-tenant-path-fuzz/BLAST_RADIUS.md`, único, com todas as relações de fronteira conciliadas.
- [ ] `docs/ownership/crates/corelink-tenant-path-fuzz/MAINTENANCE.md`, único, com procedimentos verificáveis.
