<!-- corelink-ownership:v1:manifest=crates/corelink-worker/fuzz/Cargo.toml -->
# [ownership] corelink-worker-fuzz: skill, referência, blast radius e manutenção

## Identidade e preparação

| Campo | Valor |
|---|---|
| Package / manifesto | corelink-worker-fuzz / crates/corelink-worker/fuzz/Cargo.toml |
...
**Contexto canônico OKF:** Consultar `scripts/okf_context.py --file crates/corelink-worker/fuzz/Cargo.toml` e arquivos de implementação depois de enumerá-los; confirmar os conceitos em `docs/knowledge/index.md`. A consulta não foi executada neste estudo.
**Riscos que não podem desaparecer na documentação:** `crates/corelink-worker/fuzz/Cargo.toml` foi confirmado como package com workspace próprio. Registrar relação com `crates/corelink-worker`, corpus/targets, limites de recursos, timeout, reprodução/minimização de caso local e diferenças em relação aos testes da crate-mãe. Não duplicar seu manual.
...
**Identidade:** `corelink-worker-fuzz`; manifesto `crates/corelink-worker/fuzz/Cargo.toml`; skill `own-corelink-worker-fuzz`.
**Fonte imutável:** [declaração do package](https://github.com/HuGR-Labs/corelink-server/blob/cca798ff5bc2df660ecf2570ed243eb9775ff3d0/crates/corelink-worker/fuzz/Cargo.toml#L3-L4); blob Git reportado `2d3c6cac3b69e9ecb6d29588bfc25028e2a6fcb2`.
...
**Fonte desses campos:** [manifesto inspecionado](https://github.com/HuGR-Labs/corelink-server/blob/cca798ff5bc2df660ecf2570ed243eb9775ff3d0/crates/corelink-worker/fuzz/Cargo.toml).
...
|---|---|---|
| `r2_path` | bin | `fuzz_targets/r2_path.rs` |
| `r2_put_get_roundtrip` | bin | `fuzz_targets/r2_put_get_roundtrip.rs` |

Esta tabela contém os targets explicitamente inspecionados, não um inventário automático completo.

**Chaves de dependência inspecionadas:** `corelink-worker`, `corelink-hash`, `corelink-tenant-path`, `libfuzzer-sys`, `bytes`, `uuid`, `zeroize`, `futures-executor`. Resolução de herança, aliases, features e inversas continua pendente.

**Arestas locais declaradas:** `corelink-worker` via `path=..`; `corelink-hash` via `path=../../corelink-hash`; `corelink-tenant-path` via `path=../../tenant-path`.

**Política do harness:** `unsafe_code=forbid`; versão `0.0.0`; `[workspace]` próprio.

### Pontos específicos a investigar

- Há três dependências locais explícitas: o escopo não termina apenas na crate-mãe.
...
```sh
cargo metadata --locked --offline --no-deps --format-version=1 --manifest-path crates/corelink-worker/fuzz/Cargo.toml
```
...
- `cargo metadata --locked --offline --no-deps --format-version=1 --manifest-path crates/corelink-worker/fuzz/Cargo.toml` — READ_ONLY; NOT_EXECUTED.
- `python3 scripts/okf_context.py --file crates/corelink-worker/fuzz/fuzz_targets/r2_path.rs --full` — READ_ONLY; NOT_EXECUTED.
- `cargo tree --locked --offline --manifest-path crates/corelink-worker/fuzz/Cargo.toml -p corelink-worker-fuzz --target x86_64-unknown-linux-gnu --edges normal,build` — READ_ONLY_RESOLUTION; NOT_EXECUTED.

### Evidência de fonte
- `crates/corelink-worker/fuzz/Cargo.toml` @ `2d3c6cac3b69e9ecb6d29588bfc25028e2a6fcb2`; source `37f96dba42cc159cd8ab8e9c4ab9d35b8e5634b5`.
- `crates/corelink-worker/fuzz/fuzz_targets/r2_path.rs` @ `4bda17ca5e7c1fd702d6c4f34843105650e408ec`; source `37f96dba42cc159cd8ab8e9c4ab9d35b8e5634b5`.
- `crates/corelink-worker/fuzz/fuzz_targets/r2_put_get_roundtrip.rs` @ `2d4dbbc07f9ef2c91082f3693043316bcbead24e`; source `37f96dba42cc159cd8ab8e9c4ab9d35b8e5634b5`.
...
- [ ] `.claude/skills/own-corelink-worker-fuzz/SKILL.md`.
- [ ] `docs/ownership/crates/corelink-worker-fuzz/REFERENCE.md`, único e indexado.
- [ ] `docs/ownership/crates/corelink-worker-fuzz/BLAST_RADIUS.md`, único, com todas as relações de fronteira conciliadas.
- [ ] `docs/ownership/crates/corelink-worker-fuzz/MAINTENANCE.md`, único, com procedimentos verificáveis.
