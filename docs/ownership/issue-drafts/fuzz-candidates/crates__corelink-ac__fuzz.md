<!-- corelink-ownership:v1:manifest=crates/corelink-ac/fuzz/Cargo.toml -->
# [ownership] corelink-ac-fuzz: skill, referência, blast radius e manutenção

## Identidade e preparação

| Campo | Valor |
|---|---|
| Package / manifesto | corelink-ac-fuzz / crates/corelink-ac/fuzz/Cargo.toml |
...
**Contexto canônico OKF:** Consultar `scripts/okf_context.py --file crates/corelink-ac/fuzz/Cargo.toml` e arquivos de implementação depois de enumerá-los; confirmar os conceitos em `docs/knowledge/index.md`. A consulta não foi executada neste estudo.
**Riscos que não podem desaparecer na documentação:** `crates/corelink-ac/fuzz/Cargo.toml` foi confirmado como package com workspace próprio. Registrar relação com `crates/corelink-ac`, corpus/targets, limites de recursos, timeout, reprodução/minimização de caso local e diferenças em relação aos testes da crate-mãe. Não duplicar seu manual.
...
**Identidade:** `corelink-ac-fuzz`; manifesto `crates/corelink-ac/fuzz/Cargo.toml`; skill `own-corelink-ac-fuzz`.
**Fonte imutável:** [declaração do package](https://github.com/HuGR-Labs/corelink-server/blob/cca798ff5bc2df660ecf2570ed243eb9775ff3d0/crates/corelink-ac/fuzz/Cargo.toml#L3-L4); blob Git reportado `d84afdbf77e3ffdc31d5c58b72ed40f2b45951c7`.
...
**Fonte desses campos:** [manifesto inspecionado](https://github.com/HuGR-Labs/corelink-server/blob/cca798ff5bc2df660ecf2570ed243eb9775ff3d0/crates/corelink-ac/fuzz/Cargo.toml).
...
|---|---|---|
| `hkdf_expand` | bin | `fuzz_targets/hkdf_expand.rs` |

Esta tabela contém os targets explicitamente inspecionados, não um inventário automático completo.

**Chaves de dependência inspecionadas:** `libfuzzer-sys`, `hkdf`, `sha2`. Resolução de herança, aliases, features e inversas continua pendente.

**Arestas locais declaradas:** nenhuma neste manifesto. Não inventar uma aresta Cargo para a crate-mãe a partir do diretório.

**Política do harness:** `unsafe_code=forbid`; versão `0.0.0`; `[workspace]` próprio.

### Pontos específicos a investigar

- Não existe dependência Cargo direta em corelink-ac neste manifesto. A associação pelo diretório não comprova exercício da implementação da crate-mãe.
...
```sh
cargo metadata --locked --offline --no-deps --format-version=1 --manifest-path crates/corelink-ac/fuzz/Cargo.toml
```
...
- `cargo metadata --locked --offline --no-deps --format-version=1 --manifest-path crates/corelink-ac/fuzz/Cargo.toml` — READ_ONLY; NOT_EXECUTED.
- `python3 scripts/okf_context.py --file crates/corelink-ac/fuzz/fuzz_targets/hkdf_expand.rs --full` — READ_ONLY; NOT_EXECUTED.
- `cargo tree --locked --offline --manifest-path crates/corelink-ac/fuzz/Cargo.toml -p corelink-ac-fuzz --target x86_64-unknown-linux-gnu --edges normal,build` — READ_ONLY_RESOLUTION; NOT_EXECUTED.

### Evidência de fonte
- `crates/corelink-ac/fuzz/Cargo.toml` @ `d84afdbf77e3ffdc31d5c58b72ed40f2b45951c7`; source `37f96dba42cc159cd8ab8e9c4ab9d35b8e5634b5`.
- `crates/corelink-ac/fuzz/fuzz_targets/hkdf_expand.rs` @ `3c2fde4864636eb77954cf9ae6c1a5e7760dfde1`; source `37f96dba42cc159cd8ab8e9c4ab9d35b8e5634b5`.
...
- [ ] `.claude/skills/own-corelink-ac-fuzz/SKILL.md`.
- [ ] `docs/ownership/crates/corelink-ac-fuzz/REFERENCE.md`, único e indexado.
- [ ] `docs/ownership/crates/corelink-ac-fuzz/BLAST_RADIUS.md`, único, com todas as relações de fronteira conciliadas.
- [ ] `docs/ownership/crates/corelink-ac-fuzz/MAINTENANCE.md`, único, com procedimentos verificáveis.
