<!-- corelink-ownership:v1:manifest=crates/corelink-hash/fuzz/Cargo.toml -->
# [ownership] corelink-hash-fuzz: skill, referência, blast radius e manutenção
...
**Contexto canônico OKF:** Consultar `scripts/okf_context.py --file crates/corelink-hash/fuzz/Cargo.toml` e arquivos de implementação depois de enumerá-los; confirmar os conceitos em `docs/knowledge/index.md`. A consulta não foi executada neste estudo.
**Riscos que não podem desaparecer na documentação:** `crates/corelink-hash/fuzz/Cargo.toml` foi confirmado como package com workspace próprio. Registrar relação com `crates/corelink-hash`, corpus/targets, limites de recursos, timeout, reprodução/minimização de caso local e diferenças em relação aos testes da crate-mãe. Não duplicar seu manual.
...
**Identidade:** `corelink-hash-fuzz`; manifesto `crates/corelink-hash/fuzz/Cargo.toml`; skill `own-corelink-hash-fuzz`.
**Fonte imutável:** [declaração do package](https://github.com/HuGR-Labs/corelink-server/blob/cca798ff5bc2df660ecf2570ed243eb9775ff3d0/crates/corelink-hash/fuzz/Cargo.toml#L3-L4); blob Git reportado `b9f559c2b44629cb7c1d9a18ddc95b2aa38ecbb2`.
...
**Fonte desses campos:** [manifesto inspecionado](https://github.com/HuGR-Labs/corelink-server/blob/cca798ff5bc2df660ecf2570ed243eb9775ff3d0/crates/corelink-hash/fuzz/Cargo.toml).
...
|---|---|---|
| `digest_parse` | bin | `fuzz_targets/digest_parse.rs` |
| `verify_body` | bin | `fuzz_targets/verify_body.rs` |

Esta tabela contém os targets explicitamente inspecionados, não um inventário automático completo.

**Chaves de dependência inspecionadas:** `corelink-hash`, `libfuzzer-sys`, `bytes`. Resolução de herança, aliases, features e inversas continua pendente.

**Arestas locais declaradas:** `corelink-hash` via `path=..`.

**Política do harness:** `unsafe_code=forbid`; versão `0.0.0`; `[workspace]` próprio.

### Pontos específicos a investigar

- O manifesto descreve parser de digest e verificação de corpo; o corpus e as asserções ainda precisam de inspeção.
...
```sh
cargo metadata --locked --offline --no-deps --format-version=1 --manifest-path crates/corelink-hash/fuzz/Cargo.toml
```
...
- `cargo metadata --locked --offline --no-deps --format-version=1 --manifest-path crates/corelink-hash/fuzz/Cargo.toml` — READ_ONLY; NOT_EXECUTED.
- `python3 scripts/okf_context.py --file crates/corelink-hash/fuzz/fuzz_targets/digest_parse.rs --full` — READ_ONLY; NOT_EXECUTED.
- `cargo tree --locked --offline --manifest-path crates/corelink-hash/fuzz/Cargo.toml -p corelink-hash-fuzz --target x86_64-unknown-linux-gnu --edges normal,build` — READ_ONLY_RESOLUTION; NOT_EXECUTED.

### Evidência de fonte
- `crates/corelink-hash/fuzz/Cargo.toml` @ `b9f559c2b44629cb7c1d9a18ddc95b2aa38ecbb2`; source `37f96dba42cc159cd8ab8e9c4ab9d35b8e5634b5`.
- `crates/corelink-hash/fuzz/fuzz_targets/digest_parse.rs` @ `559311a0909fc1143d622f90995d0dcda969dad7`; source `37f96dba42cc159cd8ab8e9c4ab9d35b8e5634b5`.
- `crates/corelink-hash/fuzz/fuzz_targets/verify_body.rs` @ `6585429b6a52ab1c8e924bd2cd9d0c59fb5aa631`; source `37f96dba42cc159cd8ab8e9c4ab9d35b8e5634b5`.
...
- [ ] `.claude/skills/own-corelink-hash-fuzz/SKILL.md`.
- [ ] `docs/ownership/crates/corelink-hash-fuzz/REFERENCE.md`, único e indexado.
- [ ] `docs/ownership/crates/corelink-hash-fuzz/BLAST_RADIUS.md`, único, com todas as relações de fronteira conciliadas.
- [ ] `docs/ownership/crates/corelink-hash-fuzz/MAINTENANCE.md`, único, com procedimentos verificáveis.
