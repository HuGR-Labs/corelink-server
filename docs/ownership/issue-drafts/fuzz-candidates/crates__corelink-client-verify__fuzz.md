<!-- corelink-ownership:v1:manifest=crates/corelink-client-verify/fuzz/Cargo.toml -->
# [ownership] corelink-client-verify-fuzz: skill, referência, blast radius e manutenção

## Identidade e preparação

| Campo | Valor |
|---|---|
| Package / manifesto | corelink-client-verify-fuzz / crates/corelink-client-verify/fuzz/Cargo.toml |
...
**Contexto canônico OKF:** Consultar `scripts/okf_context.py --file crates/corelink-client-verify/fuzz/Cargo.toml` e arquivos de implementação depois de enumerá-los; confirmar os conceitos em `docs/knowledge/index.md`. A consulta não foi executada neste estudo.
**Riscos que não podem desaparecer na documentação:** `crates/corelink-client-verify/fuzz/Cargo.toml` foi confirmado como package com workspace próprio. Registrar relação com `crates/corelink-client-verify`, corpus/targets, limites de recursos, timeout, reprodução/minimização de caso local e diferenças em relação aos testes da crate-mãe. Não duplicar seu manual.
...
**Identidade:** `corelink-client-verify-fuzz`; manifesto `crates/corelink-client-verify/fuzz/Cargo.toml`; skill `own-corelink-client-verify-fuzz`.
**Fonte imutável:** [declaração do package](https://github.com/HuGR-Labs/corelink-server/blob/cca798ff5bc2df660ecf2570ed243eb9775ff3d0/crates/corelink-client-verify/fuzz/Cargo.toml#L3-L4); blob Git reportado `7ee20e2fda1293652cc61956564417fba67b2047`.
...
**Fonte desses campos:** [manifesto inspecionado](https://github.com/HuGR-Labs/corelink-server/blob/cca798ff5bc2df660ecf2570ed243eb9775ff3d0/crates/corelink-client-verify/fuzz/Cargo.toml).
...
|---|---|---|
| `verify_sync` | bin | `fuzz_targets/verify_sync.rs` |
| `verify_ffi` | bin | `fuzz_targets/verify_ffi.rs` |

Esta tabela contém os targets explicitamente inspecionados, não um inventário automático completo.

**Chaves de dependência inspecionadas:** `corelink-client-verify`, `libfuzzer-sys`, `hex`. Resolução de herança, aliases, features e inversas continua pendente.

**Arestas locais declaradas:** `corelink-client-verify` via `path=..` com features `stream,ffi`.

**Política do harness:** `unsafe_code=allow`; versão `0.0.0`; `[workspace]` próprio.

### Pontos específicos a investigar

- O harness permite unsafe explicitamente e habilita stream+ffi; não impor forbid nem confundir a política com a da library.
...
```sh
cargo metadata --locked --offline --no-deps --format-version=1 --manifest-path crates/corelink-client-verify/fuzz/Cargo.toml
```
...
- 3 declarações de dependência e 0 registros inversos. O censo preserva kind, aliases, optional, cfg e features. A união inclui workspaces independentes e **não é um grafo resolvido de um build**.  Consumidores declarados: nenhum na população Cargo examinada.
...
- `cargo metadata --locked --offline --no-deps --format-version=1 --manifest-path crates/corelink-client-verify/fuzz/Cargo.toml` — READ_ONLY; NOT_EXECUTED.
- `python3 scripts/okf_context.py --file crates/corelink-client-verify/fuzz/fuzz_targets/verify_ffi.rs --full` — READ_ONLY; NOT_EXECUTED.
- `cargo tree --locked --offline --manifest-path crates/corelink-client-verify/fuzz/Cargo.toml -p corelink-client-verify-fuzz --target x86_64-unknown-linux-gnu --edges normal,build` — READ_ONLY_RESOLUTION; NOT_EXECUTED.

### Evidência de fonte
- `crates/corelink-client-verify/fuzz/Cargo.toml` @ `7ee20e2fda1293652cc61956564417fba67b2047`; source `37f96dba42cc159cd8ab8e9c4ab9d35b8e5634b5`.
- `crates/corelink-client-verify/fuzz/fuzz_targets/verify_ffi.rs` @ `8c92361ea688b2eea7b7ea10d067dd83c3e1501b`; source `37f96dba42cc159cd8ab8e9c4ab9d35b8e5634b5`.
- `crates/corelink-client-verify/fuzz/fuzz_targets/verify_sync.rs` @ `300ebb46f66d8d6b355b77ecbce836f879aa28bf`; source `37f96dba42cc159cd8ab8e9c4ab9d35b8e5634b5`.
...
- [ ] `.claude/skills/own-corelink-client-verify-fuzz/SKILL.md`.
- [ ] `docs/ownership/crates/corelink-client-verify-fuzz/REFERENCE.md`, único e indexado.
- [ ] `docs/ownership/crates/corelink-client-verify-fuzz/BLAST_RADIUS.md`, único, com todas as relações de fronteira conciliadas.
- [ ] `docs/ownership/crates/corelink-client-verify-fuzz/MAINTENANCE.md`, único, com procedimentos verificáveis.
