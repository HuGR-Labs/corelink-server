<!-- corelink-ownership:v1:manifest=crates/corelink-reapi/fuzz/Cargo.toml -->
# [ownership] corelink-reapi-fuzz: skill, referência, blast radius e manutenção
...
**Contexto canônico OKF:** Consultar `scripts/okf_context.py --file crates/corelink-reapi/fuzz/Cargo.toml` e arquivos de implementação depois de enumerá-los; confirmar os conceitos em `docs/knowledge/index.md`. A consulta não foi executada neste estudo.
**Riscos que não podem desaparecer na documentação:** `crates/corelink-reapi/fuzz/Cargo.toml` foi confirmado como package com workspace próprio. Registrar relação com `crates/corelink-reapi`, corpus/targets, limites de recursos, timeout, reprodução/minimização de caso local e diferenças em relação aos testes da crate-mãe. Não duplicar seu manual.
...
**Identidade:** `corelink-reapi-fuzz`; manifesto `crates/corelink-reapi/fuzz/Cargo.toml`; skill `own-corelink-reapi-fuzz`.
**Fonte imutável:** [declaração do package](https://github.com/HuGR-Labs/corelink-server/blob/cca798ff5bc2df660ecf2570ed243eb9775ff3d0/crates/corelink-reapi/fuzz/Cargo.toml#L1-L2); blob Git reportado `0be0b179e2442e2e44522b3f483cb636d44ece7f`.
...
**Fonte desses campos:** [manifesto inspecionado](https://github.com/HuGR-Labs/corelink-server/blob/cca798ff5bc2df660ecf2570ed243eb9775ff3d0/crates/corelink-reapi/fuzz/Cargo.toml).
...
|---|---|---|
| `proto_decode_batch_update` | bin | `fuzz_targets/proto_decode_batch_update.rs` |
| `audit_request_id_total` | bin | `fuzz_targets/audit_request_id_total.rs` |
| `parse_read_resource_name` | bin | `fuzz_targets/parse_read_resource_name.rs` |

Esta tabela contém os targets explicitamente inspecionados, não um inventário automático completo.

**Chaves de dependência inspecionadas:** `corelink-reapi`, `libfuzzer-sys`, `prost`. Resolução de herança, aliases, features e inversas continua pendente.

**Arestas locais declaradas:** `corelink-reapi` via `path=..`.

**Política do harness:** `unsafe_code=forbid`; versão `0.1.0`; `[workspace]` próprio.

### Pontos específicos a investigar

- Este manifesto usa versão 0.1.0 e não declara bench=false nos bins; não copiar defaults dos demais fuzzers.
...
```sh
cargo metadata --locked --offline --no-deps --format-version=1 --manifest-path crates/corelink-reapi/fuzz/Cargo.toml
```
...
- `cargo metadata --locked --offline --no-deps --format-version=1 --manifest-path crates/corelink-reapi/fuzz/Cargo.toml` — READ_ONLY; NOT_EXECUTED.
- `python3 scripts/okf_context.py --file crates/corelink-reapi/fuzz/fuzz_targets/audit_request_id_total.rs --full` — READ_ONLY; NOT_EXECUTED.
- `cargo tree --locked --offline --manifest-path crates/corelink-reapi/fuzz/Cargo.toml -p corelink-reapi-fuzz --target x86_64-unknown-linux-gnu --edges normal,build` — READ_ONLY_RESOLUTION; NOT_EXECUTED.

### Evidência de fonte
- `crates/corelink-reapi/fuzz/Cargo.toml` @ `0be0b179e2442e2e44522b3f483cb636d44ece7f`; source `37f96dba42cc159cd8ab8e9c4ab9d35b8e5634b5`.
- `crates/corelink-reapi/fuzz/fuzz_targets/audit_request_id_total.rs` @ `17ebfd6f62b690c0ceefd86d1f7adf839e3bb0dc`; source `37f96dba42cc159cd8ab8e9c4ab9d35b8e5634b5`.
- `crates/corelink-reapi/fuzz/fuzz_targets/parse_read_resource_name.rs` @ `6b9a36063f5c0ec0532b9e5ee39edb47e3997b49`; source `37f96dba42cc159cd8ab8e9c4ab9d35b8e5634b5`.
- `crates/corelink-reapi/fuzz/fuzz_targets/proto_decode_batch_update.rs` @ `0fb24df7e09edd76b290ffc9b0ddb0898fd60ab0`; source `37f96dba42cc159cd8ab8e9c4ab9d35b8e5634b5`.
...
- [ ] `.claude/skills/own-corelink-reapi-fuzz/SKILL.md`.
- [ ] `docs/ownership/crates/corelink-reapi-fuzz/REFERENCE.md`, único e indexado.
- [ ] `docs/ownership/crates/corelink-reapi-fuzz/BLAST_RADIUS.md`, único, com todas as relações de fronteira conciliadas.
- [ ] `docs/ownership/crates/corelink-reapi-fuzz/MAINTENANCE.md`, único, com procedimentos verificáveis.
