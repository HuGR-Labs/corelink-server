<!-- corelink-ownership:v1:manifest=crates/corelink-byok/fuzz/Cargo.toml -->
# [ownership] corelink-byok-fuzz: skill, referência, blast radius e manutenção

## Identidade e preparação

| Campo | Valor |
|---|---|
| Package / manifesto | corelink-byok-fuzz / crates/corelink-byok/fuzz/Cargo.toml |
...
**Contexto canônico OKF:** Consultar `scripts/okf_context.py --file crates/corelink-byok/fuzz/Cargo.toml` e arquivos de implementação depois de enumerá-los; confirmar os conceitos em `docs/knowledge/index.md`. A consulta não foi executada neste estudo.
**Riscos que não podem desaparecer na documentação:** `crates/corelink-byok/fuzz/Cargo.toml` foi confirmado como package com workspace próprio. Registrar relação com `crates/corelink-byok`, corpus/targets, limites de recursos, timeout, reprodução/minimização de caso local e diferenças em relação aos testes da crate-mãe. Não duplicar seu manual.
...
**Identidade:** `corelink-byok-fuzz`; manifesto `crates/corelink-byok/fuzz/Cargo.toml`; skill `own-corelink-byok-fuzz`.
**Fonte imutável:** [declaração do package](https://github.com/HuGR-Labs/corelink-server/blob/cca798ff5bc2df660ecf2570ed243eb9775ff3d0/crates/corelink-byok/fuzz/Cargo.toml#L3-L4); blob Git reportado `c2a1247a566e6b95baf3352dc8bef9f8cf41374e`.
...
**Fonte desses campos:** [manifesto inspecionado](https://github.com/HuGR-Labs/corelink-server/blob/cca798ff5bc2df660ecf2570ed243eb9775ff3d0/crates/corelink-byok/fuzz/Cargo.toml).
...
|---|---|---|
| `wrapped_dek_parse` | bin | `fuzz_targets/wrapped_dek_parse.rs` |
| `envelope_roundtrip` | bin | `fuzz_targets/envelope_roundtrip.rs` |

Esta tabela contém os targets explicitamente inspecionados, não um inventário automático completo.

**Chaves de dependência inspecionadas:** `corelink-byok`, `libfuzzer-sys`, `serde_json`, `arbitrary`, `tokio`, `async-trait`. Resolução de herança, aliases, features e inversas continua pendente.

**Arestas locais declaradas:** `corelink-byok` via `path=..`.

**Política do harness:** `unsafe_code=forbid`; versão `0.0.0`; `[workspace]` próprio.

### Pontos específicos a investigar

- O manifesto descreve parse/decrypt e roundtrip; verificar a implementação antes de atribuir cobertura a um provider KMS.
...
```sh
cargo metadata --locked --offline --no-deps --format-version=1 --manifest-path crates/corelink-byok/fuzz/Cargo.toml
```
...
- `cargo metadata --locked --offline --no-deps --format-version=1 --manifest-path crates/corelink-byok/fuzz/Cargo.toml` — READ_ONLY; NOT_EXECUTED.
- `python3 scripts/okf_context.py --file crates/corelink-byok/fuzz/fuzz_targets/envelope_roundtrip.rs --full` — READ_ONLY; NOT_EXECUTED.
- `cargo tree --locked --offline --manifest-path crates/corelink-byok/fuzz/Cargo.toml -p corelink-byok-fuzz --target x86_64-unknown-linux-gnu --edges normal,build` — READ_ONLY_RESOLUTION; NOT_EXECUTED.

### Evidência de fonte
- `crates/corelink-byok/fuzz/Cargo.toml` @ `c2a1247a566e6b95baf3352dc8bef9f8cf41374e`; source `37f96dba42cc159cd8ab8e9c4ab9d35b8e5634b5`.
- `crates/corelink-byok/fuzz/fuzz_targets/envelope_roundtrip.rs` @ `b892402c4edb1355dd389879651c4881381c86c6`; source `37f96dba42cc159cd8ab8e9c4ab9d35b8e5634b5`.
- `crates/corelink-byok/fuzz/fuzz_targets/wrapped_dek_parse.rs` @ `1e03062f6774b91e2a812e9a2eef96da3f49a106`; source `37f96dba42cc159cd8ab8e9c4ab9d35b8e5634b5`.
...
- [ ] `.claude/skills/own-corelink-byok-fuzz/SKILL.md`.
- [ ] `docs/ownership/crates/corelink-byok-fuzz/REFERENCE.md`, único e indexado.
- [ ] `docs/ownership/crates/corelink-byok-fuzz/BLAST_RADIUS.md`, único, com todas as relações de fronteira conciliadas.
- [ ] `docs/ownership/crates/corelink-byok-fuzz/MAINTENANCE.md`, único, com procedimentos verificáveis.
