<!-- corelink-ownership:v1:manifest=crates/corelink-config-do/Cargo.toml -->
# [ownership] corelink-config-do: skill, referência, blast radius e manutenção

## Identidade e preparação

| Campo | Valor |
|---|---|
| Package / manifesto | corelink-config-do / crates/corelink-config-do/Cargo.toml |
...
**Contexto canônico OKF:** Consultar `scripts/okf_context.py --file crates/corelink-config-do/Cargo.toml` e arquivos de implementação depois de enumerá-los; confirmar os conceitos em `docs/knowledge/index.md`. A consulta não foi executada neste estudo.
...
**Identidade:** `corelink-config-do`; manifesto `crates/corelink-config-do/Cargo.toml`; skill `own-corelink-config-do`.
**Fonte imutável:** [declaração do package](https://github.com/HuGR-Labs/corelink-server/blob/cca798ff5bc2df660ecf2570ed243eb9775ff3d0/crates/corelink-config-do/Cargo.toml#L1-L2); blob Git reportado `7fc03c93df78a67fb54f48d1d7a1228baf50c67e`.
...
- 12 declarações de dependência e 1 registros inversos. O censo preserva kind, aliases, optional, cfg e features. A união inclui workspaces independentes e **não é um grafo resolvido de um build**.  Consumidores declarados: `corelink-ops`.
...
- Risco: Fronteira a conferir: `crates/corelink-config-do/src/lib.rs:93` contém `#![forbid(unsafe_code)]`; provar seleção e efeito, não inferir runtime do nome.
- Risco: Fronteira a conferir: `crates/corelink-config-do/src/lib.rs:103` contém `pub use error::ConfigError;`; provar seleção e efeito, não inferir runtime do nome.
- Risco: Fronteira a conferir: `crates/corelink-config-do/src/lib.rs:104` contém `pub use metrics::{InMemoryMetrics, MetricsObserver, NoopMetrics};`; provar seleção e efeito, não inferir runtime do nome.

### Comandos candidatos — não executados
- `cargo metadata --locked --offline --no-deps --format-version=1` — READ_ONLY; NOT_EXECUTED.
- `python3 scripts/okf_context.py --file crates/corelink-config-do/src/lib.rs --full` — READ_ONLY; NOT_EXECUTED.
- `cargo tree --locked --offline --manifest-path crates/corelink-config-do/Cargo.toml -p corelink-config-do --target x86_64-unknown-linux-gnu --edges normal,build` — READ_ONLY_RESOLUTION; NOT_EXECUTED.

### Evidência de fonte
- `crates/corelink-config-do/Cargo.toml` @ `7fc03c93df78a67fb54f48d1d7a1228baf50c67e`; source `37f96dba42cc159cd8ab8e9c4ab9d35b8e5634b5`.
- `crates/corelink-config-do/src/lib.rs` @ `ca276589c7f24827d764428fdfec86cee81cf8a4`; source `37f96dba42cc159cd8ab8e9c4ab9d35b8e5634b5`.
...
- `docs/knowledge/storage/d1-config-db.md` @ `264ffb5648900d3f12e6147e3676df4a3271f62d`; source `37f96dba42cc159cd8ab8e9c4ab9d35b8e5634b5`.
...
- [ ] `.claude/skills/own-corelink-config-do/SKILL.md`.
- [ ] `docs/ownership/crates/corelink-config-do/REFERENCE.md`, único e indexado.
- [ ] `docs/ownership/crates/corelink-config-do/BLAST_RADIUS.md`, único, com todas as relações de fronteira conciliadas.
- [ ] `docs/ownership/crates/corelink-config-do/MAINTENANCE.md`, único, com procedimentos verificáveis.
