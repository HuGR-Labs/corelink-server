<!-- corelink-ownership:v1:manifest=tests/e2e-pilot-onboarding/Cargo.toml -->
# [ownership] e2e-pilot-onboarding: skill, referência, blast radius e manutenção

## Identidade e preparação

| Campo | Valor |
|---|---|
| Package / manifesto | e2e-pilot-onboarding / tests/e2e-pilot-onboarding/Cargo.toml |
...
**Contexto canônico OKF:** Consultar `scripts/okf_context.py --file tests/e2e-pilot-onboarding/Cargo.toml` e arquivos de implementação depois de enumerá-los; confirmar os conceitos em `docs/knowledge/index.md`. A consulta não foi executada neste estudo.
...
**Identidade:** `e2e-pilot-onboarding`; manifesto `tests/e2e-pilot-onboarding/Cargo.toml`; skill `own-e2e-pilot-onboarding`.
**Fonte imutável:** [declaração do package](https://github.com/HuGR-Labs/corelink-server/blob/cca798ff5bc2df660ecf2570ed243eb9775ff3d0/tests/e2e-pilot-onboarding/Cargo.toml#L1-L2); blob Git reportado `ed720d2eded7de1408185dcc415a0e39cf1efb79`.
...
### Fatos verificados
- Package `e2e-pilot-onboarding` em `tests/e2e-pilot-onboarding/Cargo.toml`; 6 targets devolvidos pelo Cargo na baseline.
- População fonte própria: 8 arquivos Rust rastreados, 1787 linhas físicas, excluídas raízes de packages aninhados.
- Entradas confirmadas: `tests/e2e-pilot-onboarding/src/lib.rs`.
- Declarações de navegação (amostra, não API completa): `tests/e2e-pilot-onboarding/src/lib.rs:60` — `pub mod harness;`; `tests/e2e-pilot-onboarding/src/lib.rs:62` — `pub use harness::{`
...
- Roteador existente: `python3 scripts/okf_context.py --file tests/e2e-pilot-onboarding/src/lib.rs --full`; ausência de match deve permanecer explícita.
- Risco: Fronteira a conferir: `tests/e2e-pilot-onboarding/src/harness.rs:900` contém `#[cfg(test)]`; provar seleção e efeito, não inferir runtime do nome.
- Risco: Fronteira a conferir: `tests/e2e-pilot-onboarding/src/lib.rs:56` contém `#![forbid(unsafe_code)]`; provar seleção e efeito, não inferir runtime do nome.
- Risco: Fronteira a conferir: `tests/e2e-pilot-onboarding/src/lib.rs:62` contém `pub use harness::{`; provar seleção e efeito, não inferir runtime do nome.

### Comandos candidatos — não executados
- `cargo metadata --locked --offline --no-deps --format-version=1` — READ_ONLY; NOT_EXECUTED.
- `python3 scripts/okf_context.py --file tests/e2e-pilot-onboarding/src/lib.rs --full` — READ_ONLY; NOT_EXECUTED.
- `cargo tree --locked --offline --manifest-path tests/e2e-pilot-onboarding/Cargo.toml -p e2e-pilot-onboarding --target x86_64-unknown-linux-gnu --edges normal,build` — READ_ONLY_RESOLUTION; NOT_EXECUTED.

### Evidência de fonte
- `tests/e2e-pilot-onboarding/Cargo.toml` @ `ed720d2eded7de1408185dcc415a0e39cf1efb79`; source `37f96dba42cc159cd8ab8e9c4ab9d35b8e5634b5`.
- `tests/e2e-pilot-onboarding/src/lib.rs` @ `36f43af4a941674aa535073c28900893bc4e77f9`; source `37f96dba42cc159cd8ab8e9c4ab9d35b8e5634b5`.
...
- [ ] `.claude/skills/own-e2e-pilot-onboarding/SKILL.md`.
- [ ] `docs/ownership/crates/e2e-pilot-onboarding/REFERENCE.md`, único e indexado.
- [ ] `docs/ownership/crates/e2e-pilot-onboarding/BLAST_RADIUS.md`, único, com todas as relações de fronteira conciliadas.
- [ ] `docs/ownership/crates/e2e-pilot-onboarding/MAINTENANCE.md`, único, com procedimentos verificáveis.
