<!-- corelink-ownership:v1:manifest=crates/corelink-analytics/Cargo.toml -->
# [ownership] corelink-analytics: skill, referência, blast radius e manutenção

## Identidade e preparação

| Campo | Valor |
|---|---|
| Package / manifesto | corelink-analytics / crates/corelink-analytics/Cargo.toml |
...
**Contexto canônico OKF:** Consultar `scripts/okf_context.py --file crates/corelink-analytics/Cargo.toml` e arquivos de implementação depois de enumerá-los; confirmar os conceitos em `docs/knowledge/index.md`. A consulta não foi executada neste estudo.
...
**Identidade:** `corelink-analytics`; manifesto `crates/corelink-analytics/Cargo.toml`; skill `own-corelink-analytics`.
**Fonte imutável:** [declaração do package](https://github.com/HuGR-Labs/corelink-server/blob/cca798ff5bc2df660ecf2570ed243eb9775ff3d0/crates/corelink-analytics/Cargo.toml#L1-L2); blob Git reportado `721c4c5e800a61eebfb22c3f77ed1cfac2b54b60`.
...
- 4 declarações de dependência e 10 registros inversos. O censo preserva kind, aliases, optional, cfg e features. A união inclui workspaces independentes e **não é um grafo resolvido de um build**.  Consumidores declarados: `corelink-audit`, `corelink-audit-chain`, `corelink-audit-chain-fuzz`, `corelink-billing-aggregator`, `corelink-billing-emit`, `corelink-billing-stripe`, `corelink-cli`, `corelink-server`, `corelink-telemetry`, `corelink-tracing`.
...
- Risco: Fronteira a conferir: `crates/corelink-analytics/src/audit.rs:146` contém `impl InMemoryAnalyticsAuditSink {`; provar seleção e efeito, não inferir runtime do nome.
- Risco: Fronteira a conferir: `crates/corelink-analytics/src/audit.rs:187` contém `impl AnalyticsAuditSink for InMemoryAnalyticsAuditSink {`; provar seleção e efeito, não inferir runtime do nome.
- Risco: Fronteira a conferir: `crates/corelink-analytics/src/audit.rs:220` contém `#[cfg(test)]`; provar seleção e efeito, não inferir runtime do nome.

### Comandos candidatos — não executados
- `cargo metadata --locked --offline --no-deps --format-version=1` — READ_ONLY; NOT_EXECUTED.
- `python3 scripts/okf_context.py --file crates/corelink-analytics/src/lib.rs --full` — READ_ONLY; NOT_EXECUTED.
- `cargo tree --locked --offline --manifest-path crates/corelink-analytics/Cargo.toml -p corelink-analytics --target x86_64-unknown-linux-gnu --edges normal,build` — READ_ONLY_RESOLUTION; NOT_EXECUTED.

### Evidência de fonte
- `crates/corelink-analytics/Cargo.toml` @ `721c4c5e800a61eebfb22c3f77ed1cfac2b54b60`; source `37f96dba42cc159cd8ab8e9c4ab9d35b8e5634b5`.
- `crates/corelink-analytics/src/lib.rs` @ `9c07a41131eeb2902dcd0ab6873c9a8e07978779`; source `37f96dba42cc159cd8ab8e9c4ab9d35b8e5634b5`.
...
- [ ] `.claude/skills/own-corelink-analytics/SKILL.md`.
- [ ] `docs/ownership/crates/corelink-analytics/REFERENCE.md`, único e indexado.
- [ ] `docs/ownership/crates/corelink-analytics/BLAST_RADIUS.md`, único, com todas as relações de fronteira conciliadas.
- [ ] `docs/ownership/crates/corelink-analytics/MAINTENANCE.md`, único, com procedimentos verificáveis.
