<!-- corelink-ownership:v1:manifest=crates/corelink-billing-stripe-traits/Cargo.toml -->
# [ownership] corelink-billing-stripe-traits: skill, referência, blast radius e manutenção

## Identidade e preparação

| Campo | Valor |
|---|---|
| Package / manifesto | corelink-billing-stripe-traits / crates/corelink-billing-stripe-traits/Cargo.toml |
...
**Contexto canônico OKF:** Consultar `scripts/okf_context.py --file crates/corelink-billing-stripe-traits/Cargo.toml` e arquivos de implementação depois de enumerá-los; confirmar os conceitos em `docs/knowledge/index.md`. A consulta não foi executada neste estudo.
...
**Identidade:** `corelink-billing-stripe-traits`; manifesto `crates/corelink-billing-stripe-traits/Cargo.toml`; skill `own-corelink-billing-stripe-traits`.
**Fonte imutável:** [declaração do package](https://github.com/HuGR-Labs/corelink-server/blob/cca798ff5bc2df660ecf2570ed243eb9775ff3d0/crates/corelink-billing-stripe-traits/Cargo.toml#L1-L2); blob Git reportado `dfd9a5821ee5d95bab3110266222e683a95b3f11`.
...
**Fonte desses campos:** [manifesto inspecionado](https://github.com/HuGR-Labs/corelink-server/blob/cca798ff5bc2df660ecf2570ed243eb9775ff3d0/crates/corelink-billing-stripe-traits/Cargo.toml).

- Manifesto sem dependência direta corelink-*; chaves declaradas serde, serde_json, blake3 e hex.
- O manifesto descreve ports da materialização e reexports de compatibilidade em stripe-real; conferir o código antes de assumir todos os símbolos.

**Chaves de dependência inspecionadas:** `serde`, `serde_json`, `blake3`, `hex`. Resolução de herança, aliases, features e inversas continua pendente.

### Pontos específicos a investigar

- Não reintroduzir dependência concreta de transporte ao documentar o ownership dos ports.
- Separar dono dos tipos/traits, reexport de compatibilidade, materializador e composition root.
...
- 4 declarações de dependência e 3 registros inversos. O censo preserva kind, aliases, optional, cfg e features. A união inclui workspaces independentes e **não é um grafo resolvido de um build**.  Consumidores declarados: `corelink-billing`, `corelink-billing-stripe-materializer`, `corelink-stripe-real`.
...
- Risco: Fronteira a conferir: `crates/corelink-billing-stripe-traits/src/lib.rs:66` contém `#![forbid(unsafe_code)]`; provar seleção e efeito, não inferir runtime do nome.
- Risco: Fronteira a conferir: `crates/corelink-billing-stripe-traits/src/lib.rs:549` contém `#[cfg(test)]`; provar seleção e efeito, não inferir runtime do nome.
- Risco: Coordenar contratos com consumidores declarados corelink-billing, corelink-billing-stripe-materializer, corelink-stripe-real. Aresta Cargo não certifica chamada ou produção.

### Comandos candidatos — não executados
- `cargo metadata --locked --offline --no-deps --format-version=1` — READ_ONLY; NOT_EXECUTED.
- `python3 scripts/okf_context.py --file crates/corelink-billing-stripe-traits/src/lib.rs --full` — READ_ONLY; NOT_EXECUTED.
- `cargo tree --locked --offline --manifest-path crates/corelink-billing-stripe-traits/Cargo.toml -p corelink-billing-stripe-traits --target x86_64-unknown-linux-gnu --edges normal,build` — READ_ONLY_RESOLUTION; NOT_EXECUTED.

### Evidência de fonte
- `crates/corelink-billing-stripe-traits/Cargo.toml` @ `dfd9a5821ee5d95bab3110266222e683a95b3f11`; source `37f96dba42cc159cd8ab8e9c4ab9d35b8e5634b5`.
- `crates/corelink-billing-stripe-traits/src/lib.rs` @ `65568f83b800c200c89f60614a710f029e9ba415`; source `37f96dba42cc159cd8ab8e9c4ab9d35b8e5634b5`.
...
- [ ] `.claude/skills/own-corelink-billing-stripe-traits/SKILL.md`.
- [ ] `docs/ownership/crates/corelink-billing-stripe-traits/REFERENCE.md`, único e indexado.
- [ ] `docs/ownership/crates/corelink-billing-stripe-traits/BLAST_RADIUS.md`, único, com todas as relações de fronteira conciliadas.
- [ ] `docs/ownership/crates/corelink-billing-stripe-traits/MAINTENANCE.md`, único, com procedimentos verificáveis.
