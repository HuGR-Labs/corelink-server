<!-- corelink-ownership:v1:manifest=crates/corelink-core/Cargo.toml -->
# [ownership] corelink-core: skill, referência, blast radius e manutenção

## Identidade e preparação

| Campo | Valor |
|---|---|
| Package / manifesto | corelink-core / crates/corelink-core/Cargo.toml |
...
**Contexto canônico OKF:** Consultar `scripts/okf_context.py --file crates/corelink-core/Cargo.toml` e arquivos de implementação depois de enumerá-los; confirmar os conceitos em `docs/knowledge/index.md`. A consulta não foi executada neste estudo.
...
**Identidade:** `corelink-core`; manifesto `crates/corelink-core/Cargo.toml`; skill `own-corelink-core`.
**Fonte imutável:** [declaração do package](https://github.com/HuGR-Labs/corelink-server/blob/cca798ff5bc2df660ecf2570ed243eb9775ff3d0/crates/corelink-core/Cargo.toml#L1-L2); blob Git reportado `a76458267dc5ed5e13ded2deb906a2126ba57377`.
...
- 6 declarações de dependência e 5 registros inversos. O censo preserva kind, aliases, optional, cfg e features. A união inclui workspaces independentes e **não é um grafo resolvido de um build**.  Consumidores declarados: `corelink-ac`, `corelink-adapter-host`, `corelink-cas`, `corelink-server`.
...
- Risco: Fronteira a conferir: `crates/corelink-core/src/errors.rs:14` contém `pub use digest_parse::DigestParseError;`; provar seleção e efeito, não inferir runtime do nome.
- Risco: Fronteira a conferir: `crates/corelink-core/src/errors.rs:54` contém `#[cfg(test)]`; provar seleção e efeito, não inferir runtime do nome.
- Risco: Fronteira a conferir: `crates/corelink-core/src/lib.rs:37` contém `//! via `pub use corelink_core::*` and drop the scattered duplicates.`; provar seleção e efeito, não inferir runtime do nome.

### Comandos candidatos — não executados
- `cargo metadata --locked --offline --no-deps --format-version=1` — READ_ONLY; NOT_EXECUTED.
- `python3 scripts/okf_context.py --file crates/corelink-core/src/lib.rs --full` — READ_ONLY; NOT_EXECUTED.
- `cargo tree --locked --offline --manifest-path crates/corelink-core/Cargo.toml -p corelink-core --target x86_64-unknown-linux-gnu --edges normal,build` — READ_ONLY_RESOLUTION; NOT_EXECUTED.

### Evidência de fonte
- `crates/corelink-core/Cargo.toml` @ `a76458267dc5ed5e13ded2deb906a2126ba57377`; source `37f96dba42cc159cd8ab8e9c4ab9d35b8e5634b5`.
- `crates/corelink-core/src/lib.rs` @ `b4e75d718ae2d7ea00798c018a16dacb2ea55097`; source `37f96dba42cc159cd8ab8e9c4ab9d35b8e5634b5`.
...
- [ ] `.claude/skills/own-corelink-core/SKILL.md`.
- [ ] `docs/ownership/crates/corelink-core/REFERENCE.md`, único e indexado.
- [ ] `docs/ownership/crates/corelink-core/BLAST_RADIUS.md`, único, com todas as relações de fronteira conciliadas.
- [ ] `docs/ownership/crates/corelink-core/MAINTENANCE.md`, único, com procedimentos verificáveis.
