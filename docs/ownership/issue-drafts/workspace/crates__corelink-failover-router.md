<!-- corelink-ownership:v1:manifest=crates/corelink-failover-router/Cargo.toml -->
# [ownership] corelink-failover-router: skill, referência, blast radius e manutenção
...
**Contexto canônico OKF:** Consultar `scripts/okf_context.py --file crates/corelink-failover-router/Cargo.toml` e arquivos de implementação depois de enumerá-los; confirmar os conceitos em `docs/knowledge/index.md`. A consulta não foi executada neste estudo.
...
**Identidade:** `corelink-failover-router`; manifesto `crates/corelink-failover-router/Cargo.toml`; skill `own-corelink-failover-router`.
**Fonte imutável:** [declaração do package](https://github.com/HuGR-Labs/corelink-server/blob/cca798ff5bc2df660ecf2570ed243eb9775ff3d0/crates/corelink-failover-router/Cargo.toml#L1-L2); blob Git reportado `48cfde0dc23261594577fef5e3b0dc3a2eb93b74`.
...
- Risco: Fronteira a conferir: `crates/corelink-failover-router/src/audit.rs:9` contém `pub use corelink_replica_worker::{`; provar seleção e efeito, não inferir runtime do nome.
- Risco: Fronteira a conferir: `crates/corelink-failover-router/src/failback.rs:116` contém `impl InMemoryAuditOutbox {`; provar seleção e efeito, não inferir runtime do nome.
- Risco: Fronteira a conferir: `crates/corelink-failover-router/src/failback.rs:138` contém `impl AuditOutboxRepository for InMemoryAuditOutbox {`; provar seleção e efeito, não inferir runtime do nome.

### Comandos candidatos — não executados
- `cargo metadata --locked --offline --no-deps --format-version=1` — READ_ONLY; NOT_EXECUTED.
- `python3 scripts/okf_context.py --file crates/corelink-failover-router/src/lib.rs --full` — READ_ONLY; NOT_EXECUTED.
- `cargo tree --locked --offline --manifest-path crates/corelink-failover-router/Cargo.toml -p corelink-failover-router --target x86_64-unknown-linux-gnu --edges normal,build` — READ_ONLY_RESOLUTION; NOT_EXECUTED.

### Evidência de fonte
- `crates/corelink-failover-router/Cargo.toml` @ `48cfde0dc23261594577fef5e3b0dc3a2eb93b74`; source `37f96dba42cc159cd8ab8e9c4ab9d35b8e5634b5`.
- `crates/corelink-failover-router/src/lib.rs` @ `c3e6a6dda601d18d33b18ad6e1fd38beeb46747a`; source `37f96dba42cc159cd8ab8e9c4ab9d35b8e5634b5`.
...
- [ ] `.claude/skills/own-corelink-failover-router/SKILL.md`.
- [ ] `docs/ownership/crates/corelink-failover-router/REFERENCE.md`, único e indexado.
- [ ] `docs/ownership/crates/corelink-failover-router/BLAST_RADIUS.md`, único, com todas as relações de fronteira conciliadas.
- [ ] `docs/ownership/crates/corelink-failover-router/MAINTENANCE.md`, único, com procedimentos verificáveis.
