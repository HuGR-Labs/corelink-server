<!-- corelink-ownership:v1:manifest=crates/corelink-handler-admin/Cargo.toml -->
# [ownership] corelink-handler-admin: skill, referência, blast radius e manutenção

## Identidade e preparação

| Campo | Valor |
|---|---|
| Package / manifesto | corelink-handler-admin / crates/corelink-handler-admin/Cargo.toml |
...
**Contexto canônico OKF:** Consultar `scripts/okf_context.py --file crates/corelink-handler-admin/Cargo.toml` e arquivos de implementação depois de enumerá-los; confirmar os conceitos em `docs/knowledge/index.md`. A consulta não foi executada neste estudo.
...
**Identidade:** `corelink-handler-admin`; manifesto `crates/corelink-handler-admin/Cargo.toml`; skill `own-corelink-handler-admin`.
**Fonte imutável:** [declaração do package](https://github.com/HuGR-Labs/corelink-server/blob/cca798ff5bc2df660ecf2570ed243eb9775ff3d0/crates/corelink-handler-admin/Cargo.toml#L1-L2); blob Git reportado `8e19466d7eb1a94872b313cbfd89269507e902f2`.
...
- 3 declarações de dependência e 2 registros inversos. O censo preserva kind, aliases, optional, cfg e features. A união inclui workspaces independentes e **não é um grafo resolvido de um build**.  Consumidores declarados: `corelink-ops`, `corelink-server`.
...
- Risco: Fronteira a conferir: `crates/corelink-handler-admin/src/audit.rs:77` contém `impl InMemoryAuditSink {`; provar seleção e efeito, não inferir runtime do nome.
- Risco: Fronteira a conferir: `crates/corelink-handler-admin/src/audit.rs:111` contém `impl AuditSink for InMemoryAuditSink {`; provar seleção e efeito, não inferir runtime do nome.
- Risco: Fronteira a conferir: `crates/corelink-handler-admin/src/handler.rs:245` contém `impl core::fmt::Debug for InMemoryAdminHandler {`; provar seleção e efeito, não inferir runtime do nome.

### Comandos candidatos — não executados
- `cargo metadata --locked --offline --no-deps --format-version=1` — READ_ONLY; NOT_EXECUTED.
- `python3 scripts/okf_context.py --file crates/corelink-handler-admin/src/lib.rs --full` — READ_ONLY; NOT_EXECUTED.
- `cargo tree --locked --offline --manifest-path crates/corelink-handler-admin/Cargo.toml -p corelink-handler-admin --target x86_64-unknown-linux-gnu --edges normal,build` — READ_ONLY_RESOLUTION; NOT_EXECUTED.

### Evidência de fonte
- `crates/corelink-handler-admin/Cargo.toml` @ `8e19466d7eb1a94872b313cbfd89269507e902f2`; source `37f96dba42cc159cd8ab8e9c4ab9d35b8e5634b5`.
- `crates/corelink-handler-admin/src/lib.rs` @ `07704dcf01e28217bbdaf7422a2a6c1db956238e`; source `37f96dba42cc159cd8ab8e9c4ab9d35b8e5634b5`.
...
- [ ] `.claude/skills/own-corelink-handler-admin/SKILL.md`.
- [ ] `docs/ownership/crates/corelink-handler-admin/REFERENCE.md`, único e indexado.
- [ ] `docs/ownership/crates/corelink-handler-admin/BLAST_RADIUS.md`, único, com todas as relações de fronteira conciliadas.
- [ ] `docs/ownership/crates/corelink-handler-admin/MAINTENANCE.md`, único, com procedimentos verificáveis.
