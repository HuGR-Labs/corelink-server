<!-- corelink-ownership:v1:manifest=crates/corelink-tier-selection/Cargo.toml -->
# [ownership] corelink-tier-selection: skill, referência, blast radius e manutenção

## Identidade e preparação

| Campo | Valor |
|---|---|
| Package / manifesto | corelink-tier-selection / crates/corelink-tier-selection/Cargo.toml |
...
**Contexto canônico OKF:** Consultar `scripts/okf_context.py --file crates/corelink-tier-selection/Cargo.toml` e arquivos de implementação depois de enumerá-los; confirmar os conceitos em `docs/knowledge/index.md`. A consulta não foi executada neste estudo.
...
**Identidade:** `corelink-tier-selection`; manifesto `crates/corelink-tier-selection/Cargo.toml`; skill `own-corelink-tier-selection`.
**Fonte imutável:** [declaração do package](https://github.com/HuGR-Labs/corelink-server/blob/cca798ff5bc2df660ecf2570ed243eb9775ff3d0/crates/corelink-tier-selection/Cargo.toml#L1-L2); blob Git reportado `ac742611a84dfdbc64a368762b5e92cb4bf6a836`.
...
- Risco: Fronteira a conferir: `crates/corelink-tier-selection/src/audit.rs:156` contém `impl InMemoryTierSelectionAuditSink {`; provar seleção e efeito, não inferir runtime do nome.
- Risco: Fronteira a conferir: `crates/corelink-tier-selection/src/audit.rs:194` contém `impl TierSelectionAuditSink for InMemoryTierSelectionAuditSink {`; provar seleção e efeito, não inferir runtime do nome.
- Risco: Fronteira a conferir: `crates/corelink-tier-selection/src/dpa.rs:33` contém `impl InMemoryDpaGate {`; provar seleção e efeito, não inferir runtime do nome.

### Comandos candidatos — não executados
- `cargo metadata --locked --offline --no-deps --format-version=1` — READ_ONLY; NOT_EXECUTED.
- `python3 scripts/okf_context.py --file crates/corelink-tier-selection/src/lib.rs --full` — READ_ONLY; NOT_EXECUTED.
- `cargo tree --locked --offline --manifest-path crates/corelink-tier-selection/Cargo.toml -p corelink-tier-selection --target x86_64-unknown-linux-gnu --edges normal,build` — READ_ONLY_RESOLUTION; NOT_EXECUTED.

### Evidência de fonte
- `crates/corelink-tier-selection/Cargo.toml` @ `ac742611a84dfdbc64a368762b5e92cb4bf6a836`; source `37f96dba42cc159cd8ab8e9c4ab9d35b8e5634b5`.
- `crates/corelink-tier-selection/src/lib.rs` @ `fa44dcdc51d6ce3edc817bc9446ec1368589bf68`; source `37f96dba42cc159cd8ab8e9c4ab9d35b8e5634b5`.
...
- [ ] `.claude/skills/own-corelink-tier-selection/SKILL.md`.
- [ ] `docs/ownership/crates/corelink-tier-selection/REFERENCE.md`, único e indexado.
- [ ] `docs/ownership/crates/corelink-tier-selection/BLAST_RADIUS.md`, único, com todas as relações de fronteira conciliadas.
- [ ] `docs/ownership/crates/corelink-tier-selection/MAINTENANCE.md`, único, com procedimentos verificáveis.
