<!-- corelink-ownership:v1:manifest=crates/corelink-signup/Cargo.toml -->
# [ownership] corelink-signup: skill, referência, blast radius e manutenção

## Identidade e preparação

| Campo | Valor |
|---|---|
| Package / manifesto | corelink-signup / crates/corelink-signup/Cargo.toml |
...
**Contexto canônico OKF:** Consultar `scripts/okf_context.py --file crates/corelink-signup/Cargo.toml` e arquivos de implementação depois de enumerá-los; confirmar os conceitos em `docs/knowledge/index.md`. A consulta não foi executada neste estudo.
...
**Identidade:** `corelink-signup`; manifesto `crates/corelink-signup/Cargo.toml`; skill `own-corelink-signup`.
**Fonte imutável:** [declaração do package](https://github.com/HuGR-Labs/corelink-server/blob/cca798ff5bc2df660ecf2570ed243eb9775ff3d0/crates/corelink-signup/Cargo.toml#L1-L2); blob Git reportado `cc2da2df3e08ccb109fb95ef06c9d256e1762312`.
...
- 3 declarações de dependência e 2 registros inversos. O censo preserva kind, aliases, optional, cfg e features. A união inclui workspaces independentes e **não é um grafo resolvido de um build**.  Consumidores declarados: `e2e-billing-flow`, `e2e-signup-flow`.
...
- Risco: Fronteira a conferir: `crates/corelink-signup/src/audit.rs:165` contém `impl InMemorySignupAuditSink {`; provar seleção e efeito, não inferir runtime do nome.
- Risco: Fronteira a conferir: `crates/corelink-signup/src/audit.rs:197` contém `impl SignupAuditSink for InMemorySignupAuditSink {`; provar seleção e efeito, não inferir runtime do nome.
- Risco: Fronteira a conferir: `crates/corelink-signup/src/audit.rs:221` contém `#[cfg(test)]`; provar seleção e efeito, não inferir runtime do nome.

### Comandos candidatos — não executados
- `cargo metadata --locked --offline --no-deps --format-version=1` — READ_ONLY; NOT_EXECUTED.
- `python3 scripts/okf_context.py --file crates/corelink-signup/src/lib.rs --full` — READ_ONLY; NOT_EXECUTED.
- `cargo tree --locked --offline --manifest-path crates/corelink-signup/Cargo.toml -p corelink-signup --target x86_64-unknown-linux-gnu --edges normal,build` — READ_ONLY_RESOLUTION; NOT_EXECUTED.

### Evidência de fonte
- `crates/corelink-signup/Cargo.toml` @ `cc2da2df3e08ccb109fb95ef06c9d256e1762312`; source `37f96dba42cc159cd8ab8e9c4ab9d35b8e5634b5`.
- `crates/corelink-signup/src/lib.rs` @ `44192a950718830490f183adbf77f8e397041d81`; source `37f96dba42cc159cd8ab8e9c4ab9d35b8e5634b5`.
...
- [ ] `.claude/skills/own-corelink-signup/SKILL.md`.
- [ ] `docs/ownership/crates/corelink-signup/REFERENCE.md`, único e indexado.
- [ ] `docs/ownership/crates/corelink-signup/BLAST_RADIUS.md`, único, com todas as relações de fronteira conciliadas.
- [ ] `docs/ownership/crates/corelink-signup/MAINTENANCE.md`, único, com procedimentos verificáveis.
