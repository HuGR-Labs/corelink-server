<!-- corelink-ownership:v1:manifest=tests/e2e-failover-router/Cargo.toml -->
# [ownership] e2e-failover-router: skill, referência, blast radius e manutenção

## Identidade e preparação

| Campo | Valor |
|---|---|
| Package / manifesto | e2e-failover-router / tests/e2e-failover-router/Cargo.toml |
...
**Contexto canônico OKF:** Consultar `scripts/okf_context.py --file tests/e2e-failover-router/Cargo.toml` e arquivos de implementação depois de enumerá-los; confirmar os conceitos em `docs/knowledge/index.md`. A consulta não foi executada neste estudo.
...
**Identidade:** `e2e-failover-router`; manifesto `tests/e2e-failover-router/Cargo.toml`; skill `own-e2e-failover-router`.
**Fonte imutável:** [declaração do package](https://github.com/HuGR-Labs/corelink-server/blob/cca798ff5bc2df660ecf2570ed243eb9775ff3d0/tests/e2e-failover-router/Cargo.toml#L1-L2); blob Git reportado `685fd810daf578ae394768697fde0db85e332ebb`.
...
- Risco: Fronteira a conferir: `tests/e2e-failover-router/src/lib.rs:42` contém `#![forbid(unsafe_code)]`; provar seleção e efeito, não inferir runtime do nome.
...
- `python3 scripts/okf_context.py --file tests/e2e-failover-router/src/lib.rs --full` — READ_ONLY; NOT_EXECUTED.
- `cargo tree --locked --offline --manifest-path tests/e2e-failover-router/Cargo.toml -p e2e-failover-router --target x86_64-unknown-linux-gnu --edges normal,build` — READ_ONLY_RESOLUTION; NOT_EXECUTED.
...
- `tests/e2e-failover-router/Cargo.toml` @ `685fd810daf578ae394768697fde0db85e332ebb`; source `37f96dba42cc159cd8ab8e9c4ab9d35b8e5634b5`.
- `tests/e2e-failover-router/src/lib.rs` @ `c9b2b84e362c0ebdc63db67d0b6a1938f42e5dee`; source `37f96dba42cc159cd8ab8e9c4ab9d35b8e5634b5`.
...
- [ ] `.claude/skills/own-e2e-failover-router/SKILL.md`.
- [ ] `docs/ownership/crates/e2e-failover-router/REFERENCE.md`, único e indexado.
- [ ] `docs/ownership/crates/e2e-failover-router/BLAST_RADIUS.md`, único, com todas as relações de fronteira conciliadas.
- [ ] `docs/ownership/crates/e2e-failover-router/MAINTENANCE.md`, único, com procedimentos verificáveis.
