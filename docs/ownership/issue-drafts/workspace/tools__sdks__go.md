<!-- corelink-ownership:v1:manifest=tools/sdks/go/Cargo.toml -->
# [ownership] corelink-go: skill, referência, blast radius e manutenção

## Identidade e preparação

| Campo | Valor |
|---|---|
| Package / manifesto | corelink-go / tools/sdks/go/Cargo.toml |
...
**Contexto canônico OKF:** Consultar `scripts/okf_context.py --file tools/sdks/go/Cargo.toml` e arquivos de implementação depois de enumerá-los; confirmar os conceitos em `docs/knowledge/index.md`. A consulta não foi executada neste estudo.
...
**Identidade:** `corelink-go`; manifesto `tools/sdks/go/Cargo.toml`; skill `own-corelink-go`.
**Fonte imutável:** [declaração do package](https://github.com/HuGR-Labs/corelink-server/blob/cca798ff5bc2df660ecf2570ed243eb9775ff3d0/tools/sdks/go/Cargo.toml#L1-L2); blob Git reportado `9413ad8e7d71ff12dd02964151ca0c5b0a637de0`.
...
**Fonte desses campos:** [manifesto inspecionado](https://github.com/HuGR-Labs/corelink-server/blob/cca798ff5bc2df660ecf2570ed243eb9775ff3d0/tools/sdks/go/Cargo.toml).

- Package corelink-go; library corelink_go; crate-types cdylib e staticlib.
- A dependência corelink-client-verify habilita ffi; publish=false no manifesto Cargo.

| Target declarado | Tipo | Path declarado |
|---|---|---|
| `corelink_go` | lib | `Path implícito: confirmar por Cargo` |

Esta tabela contém os targets explicitamente inspecionados, não um inventário automático completo.

**Chaves de dependência inspecionadas:** `corelink-client-verify`, `tracing`. Resolução de herança, aliases, features e inversas continua pendente.

### Pontos específicos a investigar

- Documentar as duas formas de link e seus consumidores cgo reais; o manifesto não prova ABI nem distribuição.
- Path implícito e targets automáticos continuam sujeitos ao censo Cargo.
...
- Risco: Fronteira a conferir: `tools/sdks/go/src/go_bridge.rs:50` contém `unsafe impl Send for CorelinkGoClient {}`; provar seleção e efeito, não inferir runtime do nome.
- Risco: Fronteira a conferir: `tools/sdks/go/src/go_bridge.rs:51` contém `unsafe impl Sync for CorelinkGoClient {}`; provar seleção e efeito, não inferir runtime do nome.
- Risco: Fronteira a conferir: `tools/sdks/go/src/go_bridge.rs:70` contém `pub unsafe extern "C" fn corelink_go_client_new(`; provar seleção e efeito, não inferir runtime do nome.

### Comandos candidatos — não executados
- `cargo metadata --locked --offline --no-deps --format-version=1` — READ_ONLY; NOT_EXECUTED.
- `python3 scripts/okf_context.py --file tools/sdks/go/src/lib.rs --full` — READ_ONLY; NOT_EXECUTED.
- `cargo tree --locked --offline --manifest-path tools/sdks/go/Cargo.toml -p corelink-go --target x86_64-unknown-linux-gnu --edges normal,build` — READ_ONLY_RESOLUTION; NOT_EXECUTED.

### Evidência de fonte
- `tools/sdks/go/Cargo.toml` @ `9413ad8e7d71ff12dd02964151ca0c5b0a637de0`; source `37f96dba42cc159cd8ab8e9c4ab9d35b8e5634b5`.
- `tools/sdks/go/src/lib.rs` @ `6e7122ff38d159bb096759b6d50c944f47ec6ec3`; source `37f96dba42cc159cd8ab8e9c4ab9d35b8e5634b5`.
...
- [ ] `.claude/skills/own-corelink-go/SKILL.md`.
- [ ] `docs/ownership/crates/corelink-go/REFERENCE.md`, único e indexado.
- [ ] `docs/ownership/crates/corelink-go/BLAST_RADIUS.md`, único, com todas as relações de fronteira conciliadas.
- [ ] `docs/ownership/crates/corelink-go/MAINTENANCE.md`, único, com procedimentos verificáveis.
