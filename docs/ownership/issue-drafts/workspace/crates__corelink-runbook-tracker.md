<!-- corelink-ownership:v1:manifest=crates/corelink-runbook-tracker/Cargo.toml -->
# [ownership] corelink-runbook-tracker: skill, referência, blast radius e manutenção

## Identidade e preparação

| Campo | Valor |
|---|---|
| Package / manifesto | corelink-runbook-tracker / crates/corelink-runbook-tracker/Cargo.toml |
...
**Contexto canônico OKF:** Consultar `scripts/okf_context.py --file crates/corelink-runbook-tracker/Cargo.toml` e arquivos de implementação depois de enumerá-los; confirmar os conceitos em `docs/knowledge/index.md`. A consulta não foi executada neste estudo.
...
**Identidade:** `corelink-runbook-tracker`; manifesto `crates/corelink-runbook-tracker/Cargo.toml`; skill `own-corelink-runbook-tracker`.
**Fonte imutável:** [declaração do package](https://github.com/HuGR-Labs/corelink-server/blob/cca798ff5bc2df660ecf2570ed243eb9775ff3d0/crates/corelink-runbook-tracker/Cargo.toml#L1-L2); blob Git reportado `40d42d3a98fa07df2acae4ef49472d81da5b9877`.
...
- 5 declarações de dependência e 2 registros inversos. O censo preserva kind, aliases, optional, cfg e features. A união inclui workspaces independentes e **não é um grafo resolvido de um build**.  Consumidores declarados: `corelink-cli`, `corelink-ops`.
...
- Risco: Fronteira a conferir: `crates/corelink-runbook-tracker/src/lib.rs:37` contém `#![forbid(unsafe_code)]`; provar seleção e efeito, não inferir runtime do nome.
- Risco: Fronteira a conferir: `crates/corelink-runbook-tracker/src/lib.rs:360` contém `#[cfg(test)]`; provar seleção e efeito, não inferir runtime do nome.
- Risco: Fronteira a conferir: `crates/corelink-runbook-tracker/src/lib.rs:602` contém `std::env::var("PROPTEST_CASES")`; provar seleção e efeito, não inferir runtime do nome.

### Comandos candidatos — não executados
- `cargo metadata --locked --offline --no-deps --format-version=1` — READ_ONLY; NOT_EXECUTED.
- `python3 scripts/okf_context.py --file crates/corelink-runbook-tracker/src/lib.rs --full` — READ_ONLY; NOT_EXECUTED.
- `cargo tree --locked --offline --manifest-path crates/corelink-runbook-tracker/Cargo.toml -p corelink-runbook-tracker --target x86_64-unknown-linux-gnu --edges normal,build` — READ_ONLY_RESOLUTION; NOT_EXECUTED.

### Evidência de fonte
- `crates/corelink-runbook-tracker/Cargo.toml` @ `40d42d3a98fa07df2acae4ef49472d81da5b9877`; source `37f96dba42cc159cd8ab8e9c4ab9d35b8e5634b5`.
- `crates/corelink-runbook-tracker/src/lib.rs` @ `c3c770e37961eef6451c51f047a28c669dc017c0`; source `37f96dba42cc159cd8ab8e9c4ab9d35b8e5634b5`.
...
- [ ] `.claude/skills/own-corelink-runbook-tracker/SKILL.md`.
- [ ] `docs/ownership/crates/corelink-runbook-tracker/REFERENCE.md`, único e indexado.
- [ ] `docs/ownership/crates/corelink-runbook-tracker/BLAST_RADIUS.md`, único, com todas as relações de fronteira conciliadas.
- [ ] `docs/ownership/crates/corelink-runbook-tracker/MAINTENANCE.md`, único, com procedimentos verificáveis.
