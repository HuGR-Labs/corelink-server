<!-- corelink-ownership:v1:manifest=crates/corelink-handler-cas/Cargo.toml -->
# [ownership] corelink-handler-cas: skill, referência, blast radius e manutenção

## Identidade e preparação

| Campo | Valor |
|---|---|
| Package / manifesto | corelink-handler-cas / crates/corelink-handler-cas/Cargo.toml |
...
**Contexto canônico OKF:** Consultar `scripts/okf_context.py --file crates/corelink-handler-cas/Cargo.toml` e arquivos de implementação depois de enumerá-los; confirmar os conceitos em `docs/knowledge/index.md`. A consulta não foi executada neste estudo.
...
**Identidade:** `corelink-handler-cas`; manifesto `crates/corelink-handler-cas/Cargo.toml`; skill `own-corelink-handler-cas`.
**Fonte imutável:** [declaração do package](https://github.com/HuGR-Labs/corelink-server/blob/cca798ff5bc2df660ecf2570ed243eb9775ff3d0/crates/corelink-handler-cas/Cargo.toml#L1-L2); blob Git reportado `ea5d3bf8274c29123ce0ecd7e0a5b9c5e40d72ed`.
...
- 5 declarações de dependência e 4 registros inversos. O censo preserva kind, aliases, optional, cfg e features. A união inclui workspaces independentes e **não é um grafo resolvido de um build**.  Consumidores declarados: `corelink-adapter-host`, `corelink-bazel-bridge`, `corelink-cas`, `corelink-server`.
...
- Risco: Fronteira a conferir: `crates/corelink-handler-cas/src/audit.rs:142` contém `impl InMemoryAuditSink {`; provar seleção e efeito, não inferir runtime do nome.
- Risco: Fronteira a conferir: `crates/corelink-handler-cas/src/audit.rs:180` contém `impl AuditSink for InMemoryAuditSink {`; provar seleção e efeito, não inferir runtime do nome.
- Risco: Fronteira a conferir: `crates/corelink-handler-cas/src/handler.rs:209` contém `impl core::fmt::Debug for InMemoryCasHandler {`; provar seleção e efeito, não inferir runtime do nome.

### Comandos candidatos — não executados
- `cargo metadata --locked --offline --no-deps --format-version=1` — READ_ONLY; NOT_EXECUTED.
- `python3 scripts/okf_context.py --file crates/corelink-handler-cas/src/lib.rs --full` — READ_ONLY; NOT_EXECUTED.
- `cargo tree --locked --offline --manifest-path crates/corelink-handler-cas/Cargo.toml -p corelink-handler-cas --target x86_64-unknown-linux-gnu --edges normal,build` — READ_ONLY_RESOLUTION; NOT_EXECUTED.

### Evidência de fonte
- `crates/corelink-handler-cas/Cargo.toml` @ `5cfdb176b8faba36ac7f4fd04845fc3beb13fe47`; source `37f96dba42cc159cd8ab8e9c4ab9d35b8e5634b5`.
- `crates/corelink-handler-cas/src/lib.rs` @ `04fbd71d542d6f8c86004019e5ab2660b86316a2`; source `37f96dba42cc159cd8ab8e9c4ab9d35b8e5634b5`.
...
- `docs/knowledge/ops/observability-plane.md` @ `5ad28a022e76557652b608880387034b0f9fc195`; source `37f96dba42cc159cd8ab8e9c4ab9d35b8e5634b5`.
...
- [ ] `.claude/skills/own-corelink-handler-cas/SKILL.md`.
- [ ] `docs/ownership/crates/corelink-handler-cas/REFERENCE.md`, único e indexado.
- [ ] `docs/ownership/crates/corelink-handler-cas/BLAST_RADIUS.md`, único, com todas as relações de fronteira conciliadas.
- [ ] `docs/ownership/crates/corelink-handler-cas/MAINTENANCE.md`, único, com procedimentos verificáveis.
