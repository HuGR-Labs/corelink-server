<!-- corelink-ownership:v1:manifest=crates/corelink-runner-aggregate/Cargo.toml -->
# [ownership] corelink-runner-aggregate: skill, referência, blast radius e manutenção

## Identidade e preparação

| Campo | Valor |
|---|---|
| Package / manifesto | corelink-runner-aggregate / crates/corelink-runner-aggregate/Cargo.toml |
...
**Contexto canônico OKF:** Consultar `scripts/okf_context.py --file crates/corelink-runner-aggregate/Cargo.toml` e arquivos de implementação depois de enumerá-los; confirmar os conceitos em `docs/knowledge/index.md`. A consulta não foi executada neste estudo.
...
**Identidade:** `corelink-runner-aggregate`; manifesto `crates/corelink-runner-aggregate/Cargo.toml`; skill `own-corelink-runner-aggregate`.
**Fonte imutável:** [declaração do package](https://github.com/HuGR-Labs/corelink-server/blob/cca798ff5bc2df660ecf2570ed243eb9775ff3d0/crates/corelink-runner-aggregate/Cargo.toml#L1-L2); blob Git reportado `e05439cc7906b7ca76a64f6d62bfd39306ec6e1a`.
...
- Risco: Fronteira a conferir: `crates/corelink-runner-aggregate/src/lib.rs:41` contém `#![forbid(unsafe_code)]`; provar seleção e efeito, não inferir runtime do nome.
- Risco: Fronteira a conferir: `crates/corelink-runner-aggregate/src/lib.rs:404` contém `#[cfg(test)]`; provar seleção e efeito, não inferir runtime do nome.
- Risco: Fronteira a conferir: `crates/corelink-runner-aggregate/tests/runner_aggregate_run_bin.rs:28` contém `const BIN: &str = env!("CARGO_BIN_EXE_runner-aggregate-run");`; provar seleção e efeito, não inferir runtime do nome.

### Comandos candidatos — não executados
- `cargo metadata --locked --offline --no-deps --format-version=1` — READ_ONLY; NOT_EXECUTED.
- `python3 scripts/okf_context.py --file crates/corelink-runner-aggregate/src/bin/runner-aggregate-run.rs --full` — READ_ONLY; NOT_EXECUTED.
- `cargo tree --locked --offline --manifest-path crates/corelink-runner-aggregate/Cargo.toml -p corelink-runner-aggregate --target x86_64-unknown-linux-gnu --edges normal,build` — READ_ONLY_RESOLUTION; NOT_EXECUTED.

### Evidência de fonte
- `crates/corelink-runner-aggregate/Cargo.toml` @ `f1d87a5c0ad91acc6e693f92da599e87bc7a6616`; source `37f96dba42cc159cd8ab8e9c4ab9d35b8e5634b5`.
- `crates/corelink-runner-aggregate/src/bin/runner-aggregate-run.rs` @ `77af291ba6e4694e68c102f1fa7ab967caa9c012`; source `37f96dba42cc159cd8ab8e9c4ab9d35b8e5634b5`.
- `crates/corelink-runner-aggregate/src/lib.rs` @ `ec76f90d4a77f2cb66e2acfc748c79b9663d3b4a`; source `37f96dba42cc159cd8ab8e9c4ab9d35b8e5634b5`.
...
- [ ] `.claude/skills/own-corelink-runner-aggregate/SKILL.md`.
- [ ] `docs/ownership/crates/corelink-runner-aggregate/REFERENCE.md`, único e indexado.
- [ ] `docs/ownership/crates/corelink-runner-aggregate/BLAST_RADIUS.md`, único, com todas as relações de fronteira conciliadas.
- [ ] `docs/ownership/crates/corelink-runner-aggregate/MAINTENANCE.md`, único, com procedimentos verificáveis.
