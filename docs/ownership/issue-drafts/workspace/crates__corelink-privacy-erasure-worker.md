<!-- corelink-ownership:v1:manifest=crates/corelink-privacy-erasure-worker/Cargo.toml -->
# [ownership] corelink-privacy-erasure-worker: skill, referência, blast radius e manutenção

## Identidade e preparação

| Campo | Valor |
|---|---|
| Package / manifesto | corelink-privacy-erasure-worker / crates/corelink-privacy-erasure-worker/Cargo.toml |
...
**Contexto canônico OKF:** Consultar `scripts/okf_context.py --file crates/corelink-privacy-erasure-worker/Cargo.toml` e arquivos de implementação depois de enumerá-los; confirmar os conceitos em `docs/knowledge/index.md`. A consulta não foi executada neste estudo.
...
**Identidade:** `corelink-privacy-erasure-worker`; manifesto `crates/corelink-privacy-erasure-worker/Cargo.toml`; skill `own-corelink-privacy-erasure-worker`.
**Fonte imutável:** [declaração do package](https://github.com/HuGR-Labs/corelink-server/blob/cca798ff5bc2df660ecf2570ed243eb9775ff3d0/crates/corelink-privacy-erasure-worker/Cargo.toml#L1-L2); blob Git reportado `a04b0f08337276d91c3e055fb5a3f3528e3ad460`.
...
- Risco: Fronteira a conferir: `crates/corelink-privacy-erasure-worker/src/audit_emit.rs:105` contém `impl InMemoryErasureAuditSink {`; provar seleção e efeito, não inferir runtime do nome.
- Risco: Fronteira a conferir: `crates/corelink-privacy-erasure-worker/src/audit_emit.rs:164` contém `impl ErasureAuditSink for InMemoryErasureAuditSink {`; provar seleção e efeito, não inferir runtime do nome.
- Risco: Fronteira a conferir: `crates/corelink-privacy-erasure-worker/src/audit_emit.rs:195` contém `#[cfg(test)]`; provar seleção e efeito, não inferir runtime do nome.

### Comandos candidatos — não executados
- `cargo metadata --locked --offline --no-deps --format-version=1` — READ_ONLY; NOT_EXECUTED.
- `python3 scripts/okf_context.py --file crates/corelink-privacy-erasure-worker/src/lib.rs --full` — READ_ONLY; NOT_EXECUTED.
- `cargo tree --locked --offline --manifest-path crates/corelink-privacy-erasure-worker/Cargo.toml -p corelink-privacy-erasure-worker --target x86_64-unknown-linux-gnu --edges normal,build` — READ_ONLY_RESOLUTION; NOT_EXECUTED.

### Evidência de fonte
- `crates/corelink-privacy-erasure-worker/Cargo.toml` @ `a04b0f08337276d91c3e055fb5a3f3528e3ad460`; source `37f96dba42cc159cd8ab8e9c4ab9d35b8e5634b5`.
- `crates/corelink-privacy-erasure-worker/src/lib.rs` @ `437136aa365e8ad7b943ba3485012a4f4b5deb2b`; source `37f96dba42cc159cd8ab8e9c4ab9d35b8e5634b5`.
...
- [ ] `.claude/skills/own-corelink-privacy-erasure-worker/SKILL.md`.
- [ ] `docs/ownership/crates/corelink-privacy-erasure-worker/REFERENCE.md`, único e indexado.
- [ ] `docs/ownership/crates/corelink-privacy-erasure-worker/BLAST_RADIUS.md`, único, com todas as relações de fronteira conciliadas.
- [ ] `docs/ownership/crates/corelink-privacy-erasure-worker/MAINTENANCE.md`, único, com procedimentos verificáveis.
