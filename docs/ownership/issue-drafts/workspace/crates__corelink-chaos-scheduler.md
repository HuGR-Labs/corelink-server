<!-- corelink-ownership:v1:manifest=crates/corelink-chaos-scheduler/Cargo.toml -->
# [ownership] corelink-chaos-scheduler: skill, referência, blast radius e manutenção

## Identidade e preparação

| Campo | Valor |
|---|---|
| Package / manifesto | corelink-chaos-scheduler / crates/corelink-chaos-scheduler/Cargo.toml |
...
**Contexto canônico OKF:** Consultar `scripts/okf_context.py --file crates/corelink-chaos-scheduler/Cargo.toml` e arquivos de implementação depois de enumerá-los; confirmar os conceitos em `docs/knowledge/index.md`. A consulta não foi executada neste estudo.
...
**Identidade:** `corelink-chaos-scheduler`; manifesto `crates/corelink-chaos-scheduler/Cargo.toml`; skill `own-corelink-chaos-scheduler`.
**Fonte imutável:** [declaração do package](https://github.com/HuGR-Labs/corelink-server/blob/cca798ff5bc2df660ecf2570ed243eb9775ff3d0/crates/corelink-chaos-scheduler/Cargo.toml#L1-L2); blob Git reportado `c153fefe9f40f10c87d42397422f8656348a30d5`.
...
- 1 declarações de dependência e 2 registros inversos. O censo preserva kind, aliases, optional, cfg e features. A união inclui workspaces independentes e **não é um grafo resolvido de um build**.  Consumidores declarados: `corelink-ops`, `e2e-chaos`.
...
- Risco: Fronteira a conferir: `crates/corelink-chaos-scheduler/src/catalog.rs:130` contém `#[cfg(test)]`; provar seleção e efeito, não inferir runtime do nome.
- Risco: Fronteira a conferir: `crates/corelink-chaos-scheduler/src/lib.rs:58` contém `#![forbid(unsafe_code)]`; provar seleção e efeito, não inferir runtime do nome.
- Risco: Fronteira a conferir: `crates/corelink-chaos-scheduler/src/lib.rs:65` contém `pub use catalog::{canonical_catalog, distinct_fm_count, lookup};`; provar seleção e efeito, não inferir runtime do nome.

### Comandos candidatos — não executados
- `cargo metadata --locked --offline --no-deps --format-version=1` — READ_ONLY; NOT_EXECUTED.
- `python3 scripts/okf_context.py --file crates/corelink-chaos-scheduler/src/lib.rs --full` — READ_ONLY; NOT_EXECUTED.
- `cargo tree --locked --offline --manifest-path crates/corelink-chaos-scheduler/Cargo.toml -p corelink-chaos-scheduler --target x86_64-unknown-linux-gnu --edges normal,build` — READ_ONLY_RESOLUTION; NOT_EXECUTED.

### Evidência de fonte
- `crates/corelink-chaos-scheduler/Cargo.toml` @ `c153fefe9f40f10c87d42397422f8656348a30d5`; source `37f96dba42cc159cd8ab8e9c4ab9d35b8e5634b5`.
- `crates/corelink-chaos-scheduler/src/lib.rs` @ `b60565cf6e2531b9dc97e9cbb88eaac160117924`; source `37f96dba42cc159cd8ab8e9c4ab9d35b8e5634b5`.
...
- [ ] `.claude/skills/own-corelink-chaos-scheduler/SKILL.md`.
- [ ] `docs/ownership/crates/corelink-chaos-scheduler/REFERENCE.md`, único e indexado.
- [ ] `docs/ownership/crates/corelink-chaos-scheduler/BLAST_RADIUS.md`, único, com todas as relações de fronteira conciliadas.
- [ ] `docs/ownership/crates/corelink-chaos-scheduler/MAINTENANCE.md`, único, com procedimentos verificáveis.
