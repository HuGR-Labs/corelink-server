<!-- corelink-ownership:v1:manifest=crates/corelink-dt-webhook/Cargo.toml -->
# [ownership] corelink-dt-webhook: skill, referência, blast radius e manutenção

## Identidade e preparação

| Campo | Valor |
|---|---|
| Package / manifesto | corelink-dt-webhook / crates/corelink-dt-webhook/Cargo.toml |
...
**Contexto canônico OKF:** Consultar `scripts/okf_context.py --file crates/corelink-dt-webhook/Cargo.toml` e arquivos de implementação depois de enumerá-los; confirmar os conceitos em `docs/knowledge/index.md`. A consulta não foi executada neste estudo.
...
**Identidade:** `corelink-dt-webhook`; manifesto `crates/corelink-dt-webhook/Cargo.toml`; skill `own-corelink-dt-webhook`.
**Fonte imutável:** [declaração do package](https://github.com/HuGR-Labs/corelink-server/blob/cca798ff5bc2df660ecf2570ed243eb9775ff3d0/crates/corelink-dt-webhook/Cargo.toml#L1-L2); blob Git reportado `5c428bdee832eba0c8edcfc9936bdf385ca4df48`.
...
- 12 declarações de dependência e 3 registros inversos. O censo preserva kind, aliases, optional, cfg e features. A união inclui workspaces independentes e **não é um grafo resolvido de um build**.  Consumidores declarados: `corelink-dt-cli`, `corelink-dt-reconcile`, `corelink-ops`.
...
- Risco: Fronteira a conferir: `crates/corelink-dt-webhook/src/dlq.rs:41` contém `impl Default for InMemoryDlq {`; provar seleção e efeito, não inferir runtime do nome.
- Risco: Fronteira a conferir: `crates/corelink-dt-webhook/src/dlq.rs:47` contém `impl InMemoryDlq {`; provar seleção e efeito, não inferir runtime do nome.
- Risco: Fronteira a conferir: `crates/corelink-dt-webhook/src/dlq.rs:125` contém `#[cfg(test)]`; provar seleção e efeito, não inferir runtime do nome.

### Comandos candidatos — não executados
- `cargo metadata --locked --offline --no-deps --format-version=1` — READ_ONLY; NOT_EXECUTED.
- `python3 scripts/okf_context.py --file crates/corelink-dt-webhook/src/lib.rs --full` — READ_ONLY; NOT_EXECUTED.
- `cargo tree --locked --offline --manifest-path crates/corelink-dt-webhook/Cargo.toml -p corelink-dt-webhook --target x86_64-unknown-linux-gnu --edges normal,build` — READ_ONLY_RESOLUTION; NOT_EXECUTED.

### Evidência de fonte
- `crates/corelink-dt-webhook/Cargo.toml` @ `5c428bdee832eba0c8edcfc9936bdf385ca4df48`; source `37f96dba42cc159cd8ab8e9c4ab9d35b8e5634b5`.
- `crates/corelink-dt-webhook/src/lib.rs` @ `031a090ad6cb9168fdc72ed12b6d714e73886c6d`; source `37f96dba42cc159cd8ab8e9c4ab9d35b8e5634b5`.
...
- [ ] `.claude/skills/own-corelink-dt-webhook/SKILL.md`.
- [ ] `docs/ownership/crates/corelink-dt-webhook/REFERENCE.md`, único e indexado.
- [ ] `docs/ownership/crates/corelink-dt-webhook/BLAST_RADIUS.md`, único, com todas as relações de fronteira conciliadas.
- [ ] `docs/ownership/crates/corelink-dt-webhook/MAINTENANCE.md`, único, com procedimentos verificáveis.
