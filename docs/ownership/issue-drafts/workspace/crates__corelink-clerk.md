<!-- corelink-ownership:v1:manifest=crates/corelink-clerk/Cargo.toml -->
# [ownership] corelink-clerk: skill, referência, blast radius e manutenção

## Identidade e preparação

| Campo | Valor |
|---|---|
| Package / manifesto | corelink-clerk / crates/corelink-clerk/Cargo.toml |
...
**Contexto canônico OKF:** Consultar `scripts/okf_context.py --file crates/corelink-clerk/Cargo.toml` e arquivos de implementação depois de enumerá-los; confirmar os conceitos em `docs/knowledge/index.md`. A consulta não foi executada neste estudo.
...
**Identidade:** `corelink-clerk`; manifesto `crates/corelink-clerk/Cargo.toml`; skill `own-corelink-clerk`.
**Fonte imutável:** [declaração do package](https://github.com/HuGR-Labs/corelink-server/blob/cca798ff5bc2df660ecf2570ed243eb9775ff3d0/crates/corelink-clerk/Cargo.toml#L1-L2); blob Git reportado `39273401e870c591c591dd7258b9eb44167384c2`.
...
- 20 declarações de dependência e 5 registros inversos. O censo preserva kind, aliases, optional, cfg e features. A união inclui workspaces independentes e **não é um grafo resolvido de um build**.  Consumidores declarados: `corelink-auth`, `corelink-clerk`, `corelink-clerk-cf`, `corelink-worker`.
...
- Risco: Investigar ativação e compatibilidade das features declaradas: default, http-fetcher, jwt-adapter, test-utils. Não assumir que --all-features é válido.
- Risco: Fronteira a conferir: `crates/corelink-clerk/src/config.rs:268` contém `#[cfg(test)]`; provar seleção e efeito, não inferir runtime do nome.
- Risco: Fronteira a conferir: `crates/corelink-clerk/src/env_config.rs:231` contém `#[cfg(test)]`; provar seleção e efeito, não inferir runtime do nome.

### Comandos candidatos — não executados
- `cargo metadata --locked --offline --no-deps --format-version=1` — READ_ONLY; NOT_EXECUTED.
- `python3 scripts/okf_context.py --file crates/corelink-clerk/src/lib.rs --full` — READ_ONLY; NOT_EXECUTED.
- `cargo tree --locked --offline --manifest-path crates/corelink-clerk/Cargo.toml -p corelink-clerk --target x86_64-unknown-linux-gnu --edges normal,build` — READ_ONLY_RESOLUTION; NOT_EXECUTED.

### Evidência de fonte
- `crates/corelink-clerk/Cargo.toml` @ `39273401e870c591c591dd7258b9eb44167384c2`; source `37f96dba42cc159cd8ab8e9c4ab9d35b8e5634b5`.
- `crates/corelink-clerk/src/lib.rs` @ `66d6db966cc4c6f6fa261e9f3db5f8bbb8bb7ef0`; source `37f96dba42cc159cd8ab8e9c4ab9d35b8e5634b5`.
- `docs/knowledge/crates/auth-pat.md` @ `0ad15d33ac4635397cec63073c47b3d06afd96c9`; source `37f96dba42cc159cd8ab8e9c4ab9d35b8e5634b5`.
...
- [ ] `.claude/skills/own-corelink-clerk/SKILL.md`.
- [ ] `docs/ownership/crates/corelink-clerk/REFERENCE.md`, único e indexado.
- [ ] `docs/ownership/crates/corelink-clerk/BLAST_RADIUS.md`, único, com todas as relações de fronteira conciliadas.
- [ ] `docs/ownership/crates/corelink-clerk/MAINTENANCE.md`, único, com procedimentos verificáveis.
