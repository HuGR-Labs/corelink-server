<!-- corelink-ownership:v1:manifest=crates/corelink-gc/Cargo.toml -->
# [ownership] corelink-gc: skill, referência, blast radius e manutenção

## Identidade e preparação

| Campo | Valor |
|---|---|
| Package / manifesto | corelink-gc / crates/corelink-gc/Cargo.toml |
...
**Contexto canônico OKF:** Consultar `scripts/okf_context.py --file crates/corelink-gc/Cargo.toml` e arquivos de implementação depois de enumerá-los; confirmar os conceitos em `docs/knowledge/index.md`. A consulta não foi executada neste estudo.
...
**Identidade:** `corelink-gc`; manifesto `crates/corelink-gc/Cargo.toml`; skill `own-corelink-gc`.
**Fonte imutável:** [declaração do package](https://github.com/HuGR-Labs/corelink-server/blob/cca798ff5bc2df660ecf2570ed243eb9775ff3d0/crates/corelink-gc/Cargo.toml#L1-L2); blob Git reportado `26c58af62284c2fba62559ae1270d88969856303`.
...
- 5 declarações de dependência e 1 registros inversos. O censo preserva kind, aliases, optional, cfg e features. A união inclui workspaces independentes e **não é um grafo resolvido de um build**.  Consumidores declarados: `corelink-server`.
...
- Risco: Fronteira a conferir: `crates/corelink-gc/src/admin.rs:229` contém `#[cfg(test)]`; provar seleção e efeito, não inferir runtime do nome.
- Risco: Fronteira a conferir: `crates/corelink-gc/src/audit.rs:232` contém `impl InMemoryGcAuditSink {`; provar seleção e efeito, não inferir runtime do nome.
- Risco: Fronteira a conferir: `crates/corelink-gc/src/audit.rs:273` contém `impl GcAuditSink for InMemoryGcAuditSink {`; provar seleção e efeito, não inferir runtime do nome.

### Comandos candidatos — não executados
- `cargo metadata --locked --offline --no-deps --format-version=1` — READ_ONLY; NOT_EXECUTED.
- `python3 scripts/okf_context.py --file crates/corelink-gc/src/bin/gc_sweep.rs --full` — READ_ONLY; NOT_EXECUTED.
- `cargo tree --locked --offline --manifest-path crates/corelink-gc/Cargo.toml -p corelink-gc --target x86_64-unknown-linux-gnu --edges normal,build` — READ_ONLY_RESOLUTION; NOT_EXECUTED.

### Evidência de fonte
- `crates/corelink-gc/Cargo.toml` @ `26c58af62284c2fba62559ae1270d88969856303`; source `37f96dba42cc159cd8ab8e9c4ab9d35b8e5634b5`.
- `crates/corelink-gc/src/bin/gc_sweep.rs` @ `b6fd7bf7f9d62ac2ce1dfdb236286f28177e39ab`; source `37f96dba42cc159cd8ab8e9c4ab9d35b8e5634b5`.
- `crates/corelink-gc/src/lib.rs` @ `a90ca4ba9de9b10d9a17bc14d764a21840765ebf`; source `37f96dba42cc159cd8ab8e9c4ab9d35b8e5634b5`.
...
- [ ] `.claude/skills/own-corelink-gc/SKILL.md`.
- [ ] `docs/ownership/crates/corelink-gc/REFERENCE.md`, único e indexado.
- [ ] `docs/ownership/crates/corelink-gc/BLAST_RADIUS.md`, único, com todas as relações de fronteira conciliadas.
- [ ] `docs/ownership/crates/corelink-gc/MAINTENANCE.md`, único, com procedimentos verificáveis.
