<!-- corelink-ownership:v1:manifest=crates/corelink-replication-coordinator/Cargo.toml -->
# [ownership] corelink-replication-coordinator: skill, referência, blast radius e manutenção

## Identidade e preparação

| Campo | Valor |
|---|---|
| Package / manifesto | corelink-replication-coordinator / crates/corelink-replication-coordinator/Cargo.toml |
...
**Contexto canônico OKF:** Consultar `scripts/okf_context.py --file crates/corelink-replication-coordinator/Cargo.toml` e arquivos de implementação depois de enumerá-los; confirmar os conceitos em `docs/knowledge/index.md`. A consulta não foi executada neste estudo.
...
**Identidade:** `corelink-replication-coordinator`; manifesto `crates/corelink-replication-coordinator/Cargo.toml`; skill `own-corelink-replication-coordinator`.
**Fonte imutável:** [declaração do package](https://github.com/HuGR-Labs/corelink-server/blob/cca798ff5bc2df660ecf2570ed243eb9775ff3d0/crates/corelink-replication-coordinator/Cargo.toml#L1-L2); blob Git reportado `7c651aa4798abed83bcae61a2821e268109e80ed`.
...
- 5 declarações de dependência e 2 registros inversos. O censo preserva kind, aliases, optional, cfg e features. A união inclui workspaces independentes e **não é um grafo resolvido de um build**.  Consumidores declarados: `corelink-replication`, `e2e-replication-failover`.
...
- Risco: Fronteira a conferir: `crates/corelink-replication-coordinator/src/audit.rs:88` contém `impl InMemoryCoordinatorAuditSink {`; provar seleção e efeito, não inferir runtime do nome.
- Risco: Fronteira a conferir: `crates/corelink-replication-coordinator/src/audit.rs:102` contém `impl CoordinatorAuditSink for InMemoryCoordinatorAuditSink {`; provar seleção e efeito, não inferir runtime do nome.
- Risco: Fronteira a conferir: `crates/corelink-replication-coordinator/src/audit.rs:140` contém `#[cfg(test)]`; provar seleção e efeito, não inferir runtime do nome.

### Comandos candidatos — não executados
- `cargo metadata --locked --offline --no-deps --format-version=1` — READ_ONLY; NOT_EXECUTED.
- `python3 scripts/okf_context.py --file crates/corelink-replication-coordinator/src/lib.rs --full` — READ_ONLY; NOT_EXECUTED.
- `cargo tree --locked --offline --manifest-path crates/corelink-replication-coordinator/Cargo.toml -p corelink-replication-coordinator --target x86_64-unknown-linux-gnu --edges normal,build` — READ_ONLY_RESOLUTION; NOT_EXECUTED.

### Evidência de fonte
- `crates/corelink-replication-coordinator/Cargo.toml` @ `7c651aa4798abed83bcae61a2821e268109e80ed`; source `37f96dba42cc159cd8ab8e9c4ab9d35b8e5634b5`.
- `crates/corelink-replication-coordinator/src/lib.rs` @ `912d83ab0826045aa07ceacc37ba1ebfe5d4d27a`; source `37f96dba42cc159cd8ab8e9c4ab9d35b8e5634b5`.
...
- [ ] `.claude/skills/own-corelink-replication-coordinator/SKILL.md`.
- [ ] `docs/ownership/crates/corelink-replication-coordinator/REFERENCE.md`, único e indexado.
- [ ] `docs/ownership/crates/corelink-replication-coordinator/BLAST_RADIUS.md`, único, com todas as relações de fronteira conciliadas.
- [ ] `docs/ownership/crates/corelink-replication-coordinator/MAINTENANCE.md`, único, com procedimentos verificáveis.
