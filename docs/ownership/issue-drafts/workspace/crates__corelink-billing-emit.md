<!-- corelink-ownership:v1:manifest=crates/corelink-billing-emit/Cargo.toml -->
# [ownership] corelink-billing-emit: skill, referência, blast radius e manutenção

## Identidade e preparação

| Campo | Valor |
|---|---|
| Package / manifesto | corelink-billing-emit / crates/corelink-billing-emit/Cargo.toml |
...
**Contexto canônico OKF:** Consultar `scripts/okf_context.py --file crates/corelink-billing-emit/Cargo.toml` e arquivos de implementação depois de enumerá-los; confirmar os conceitos em `docs/knowledge/index.md`. A consulta não foi executada neste estudo.
...
**Identidade:** `corelink-billing-emit`; manifesto `crates/corelink-billing-emit/Cargo.toml`; skill `own-corelink-billing-emit`.
**Fonte imutável:** [declaração do package](https://github.com/HuGR-Labs/corelink-server/blob/cca798ff5bc2df660ecf2570ed243eb9775ff3d0/crates/corelink-billing-emit/Cargo.toml#L1-L2); blob Git reportado `42cb84a54e4a4cb3d9f1642a63cad0249b9dd106`.
...
- Risco: Fronteira a conferir: `crates/corelink-billing-emit/src/audit.rs:136` contém `pub use crate::error::BillingAuditSinkError as BillingAuditEmitError;`; provar seleção e efeito, não inferir runtime do nome.
- Risco: Fronteira a conferir: `crates/corelink-billing-emit/src/audit.rs:166` contém `impl InMemoryBillingAuditSink {`; provar seleção e efeito, não inferir runtime do nome.
- Risco: Fronteira a conferir: `crates/corelink-billing-emit/src/audit.rs:207` contém `impl BillingAuditSink for InMemoryBillingAuditSink {`; provar seleção e efeito, não inferir runtime do nome.

### Comandos candidatos — não executados
- `cargo metadata --locked --offline --no-deps --format-version=1` — READ_ONLY; NOT_EXECUTED.
- `python3 scripts/okf_context.py --file crates/corelink-billing-emit/src/lib.rs --full` — READ_ONLY; NOT_EXECUTED.
- `cargo tree --locked --offline --manifest-path crates/corelink-billing-emit/Cargo.toml -p corelink-billing-emit --target x86_64-unknown-linux-gnu --edges normal,build` — READ_ONLY_RESOLUTION; NOT_EXECUTED.

### Evidência de fonte
- `crates/corelink-billing-emit/Cargo.toml` @ `42cb84a54e4a4cb3d9f1642a63cad0249b9dd106`; source `37f96dba42cc159cd8ab8e9c4ab9d35b8e5634b5`.
- `crates/corelink-billing-emit/src/lib.rs` @ `96c2416565dd3caf7bc646a2dc6ac824ed7c437e`; source `37f96dba42cc159cd8ab8e9c4ab9d35b8e5634b5`.
...
- [ ] `.claude/skills/own-corelink-billing-emit/SKILL.md`.
- [ ] `docs/ownership/crates/corelink-billing-emit/REFERENCE.md`, único e indexado.
- [ ] `docs/ownership/crates/corelink-billing-emit/BLAST_RADIUS.md`, único, com todas as relações de fronteira conciliadas.
- [ ] `docs/ownership/crates/corelink-billing-emit/MAINTENANCE.md`, único, com procedimentos verificáveis.
