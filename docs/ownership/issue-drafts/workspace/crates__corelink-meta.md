<!-- corelink-ownership:v1:manifest=crates/corelink-meta/Cargo.toml -->
# [ownership] corelink-meta: skill, referência, blast radius e manutenção

## Identidade e preparação

| Campo | Valor |
|---|---|
| Package / manifesto | corelink-meta / crates/corelink-meta/Cargo.toml |
...
**Contexto canônico OKF:** Consultar `scripts/okf_context.py --file crates/corelink-meta/Cargo.toml` e arquivos de implementação depois de enumerá-los; confirmar os conceitos em `docs/knowledge/index.md`. A consulta não foi executada neste estudo.
...
**Identidade:** `corelink-meta`; manifesto `crates/corelink-meta/Cargo.toml`; skill `own-corelink-meta`.
**Fonte imutável:** [declaração do package](https://github.com/HuGR-Labs/corelink-server/blob/cca798ff5bc2df660ecf2570ed243eb9775ff3d0/crates/corelink-meta/Cargo.toml#L1-L2); blob Git reportado `b46812f88a190cd56f16d509e9a31d380884f876`.
...
- 7 declarações de dependência e 3 registros inversos. O censo preserva kind, aliases, optional, cfg e features. A união inclui workspaces independentes e **não é um grafo resolvido de um build**.  Consumidores declarados: `corelink-cas`, `corelink-meta-fuzz`, `corelink-reapi`.
...
- Risco: Fronteira a conferir: `crates/corelink-meta/src/cas_query.rs:142` contém `#[cfg(test)]`; provar seleção e efeito, não inferir runtime do nome.
- Risco: Fronteira a conferir: `crates/corelink-meta/src/fake.rs:88` contém `impl Default for InMemoryMetaStore {`; provar seleção e efeito, não inferir runtime do nome.
- Risco: Fronteira a conferir: `crates/corelink-meta/src/fake.rs:96` contém `impl InMemoryMetaStore {`; provar seleção e efeito, não inferir runtime do nome.

### Comandos candidatos — não executados
- `cargo metadata --locked --offline --no-deps --format-version=1` — READ_ONLY; NOT_EXECUTED.
- `python3 scripts/okf_context.py --file crates/corelink-meta/src/lib.rs --full` — READ_ONLY; NOT_EXECUTED.
- `cargo tree --locked --offline --manifest-path crates/corelink-meta/Cargo.toml -p corelink-meta --target x86_64-unknown-linux-gnu --edges normal,build` — READ_ONLY_RESOLUTION; NOT_EXECUTED.

### Evidência de fonte
- `crates/corelink-meta/Cargo.toml` @ `b46812f88a190cd56f16d509e9a31d380884f876`; source `37f96dba42cc159cd8ab8e9c4ab9d35b8e5634b5`.
- `crates/corelink-meta/src/lib.rs` @ `c08845c65b4bdd800d8949850eafaee3ddaa4937`; source `37f96dba42cc159cd8ab8e9c4ab9d35b8e5634b5`.
...
- [ ] `.claude/skills/own-corelink-meta/SKILL.md`.
- [ ] `docs/ownership/crates/corelink-meta/REFERENCE.md`, único e indexado.
- [ ] `docs/ownership/crates/corelink-meta/BLAST_RADIUS.md`, único, com todas as relações de fronteira conciliadas.
- [ ] `docs/ownership/crates/corelink-meta/MAINTENANCE.md`, único, com procedimentos verificáveis.
