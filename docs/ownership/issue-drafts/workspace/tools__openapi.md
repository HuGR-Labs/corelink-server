<!-- corelink-ownership:v1:manifest=tools/openapi/Cargo.toml -->
# [ownership] corelink-openapi: skill, referência, blast radius e manutenção

## Identidade e preparação

| Campo | Valor |
|---|---|
| Package / manifesto | corelink-openapi / tools/openapi/Cargo.toml |
...
**Contexto canônico OKF:** Consultar `scripts/okf_context.py --file tools/openapi/Cargo.toml` e arquivos de implementação depois de enumerá-los; confirmar os conceitos em `docs/knowledge/index.md`. A consulta não foi executada neste estudo.
...
**Identidade:** `corelink-openapi`; manifesto `tools/openapi/Cargo.toml`; skill `own-corelink-openapi`.
**Fonte imutável:** [declaração do package](https://github.com/HuGR-Labs/corelink-server/blob/cca798ff5bc2df660ecf2570ed243eb9775ff3d0/tools/openapi/Cargo.toml#L1-L2); blob Git reportado `acac2c04012fd987435ecb8f2c11cd4025d67e41`.
...
**Fonte desses campos:** [manifesto inspecionado](https://github.com/HuGR-Labs/corelink-server/blob/cca798ff5bc2df660ecf2570ed243eb9775ff3d0/tools/openapi/Cargo.toml).

- Package corelink-openapi; o manifesto descreve constants e embedding do contrato OpenAPI YAML/JSON.

### Pontos específicos a investigar

- Confirmar fonte canônica, arquivo gerado e consumidores; não confundir os dois formatos com duas autoridades.
...
- Risco: Fronteira a conferir: `tools/openapi/src/lib.rs:42` contém `#![forbid(unsafe_code)]`; provar seleção e efeito, não inferir runtime do nome.
- Risco: Fronteira a conferir: `tools/openapi/src/lib.rs:63` contém `pub const PACKAGE_VERSION: &str = env!("CARGO_PKG_VERSION");`; provar seleção e efeito, não inferir runtime do nome.
- Risco: Fronteira a conferir: `tools/openapi/src/lib.rs:235` contém `#[cfg(test)]`; provar seleção e efeito, não inferir runtime do nome.

### Comandos candidatos — não executados
- `cargo metadata --locked --offline --no-deps --format-version=1` — READ_ONLY; NOT_EXECUTED.
- `python3 scripts/okf_context.py --file tools/openapi/src/lib.rs --full` — READ_ONLY; NOT_EXECUTED.
- `cargo tree --locked --offline --manifest-path tools/openapi/Cargo.toml -p corelink-openapi --target x86_64-unknown-linux-gnu --edges normal,build` — READ_ONLY_RESOLUTION; NOT_EXECUTED.

### Evidência de fonte
- `tools/openapi/Cargo.toml` @ `acac2c04012fd987435ecb8f2c11cd4025d67e41`; source `37f96dba42cc159cd8ab8e9c4ab9d35b8e5634b5`.
- `tools/openapi/src/lib.rs` @ `69a407d04d75d1b924172cf696b04dffb00bf1fe`; source `37f96dba42cc159cd8ab8e9c4ab9d35b8e5634b5`.
...
- [ ] `.claude/skills/own-corelink-openapi/SKILL.md`.
- [ ] `docs/ownership/crates/corelink-openapi/REFERENCE.md`, único e indexado.
- [ ] `docs/ownership/crates/corelink-openapi/BLAST_RADIUS.md`, único, com todas as relações de fronteira conciliadas.
- [ ] `docs/ownership/crates/corelink-openapi/MAINTENANCE.md`, único, com procedimentos verificáveis.
