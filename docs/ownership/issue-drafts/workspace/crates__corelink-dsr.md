<!-- corelink-ownership:v1:manifest=crates/corelink-dsr/Cargo.toml -->
# [ownership] corelink-dsr: skill, referência, blast radius e manutenção

## Identidade e preparação

| Campo | Valor |
|---|---|
| Package / manifesto | corelink-dsr / crates/corelink-dsr/Cargo.toml |
...
**Contexto canônico OKF:** Consultar `scripts/okf_context.py --file crates/corelink-dsr/Cargo.toml` e arquivos de implementação depois de enumerá-los; confirmar os conceitos em `docs/knowledge/index.md`. A consulta não foi executada neste estudo.
...
**Identidade:** `corelink-dsr`; manifesto `crates/corelink-dsr/Cargo.toml`; skill `own-corelink-dsr`.
**Fonte imutável:** [declaração do package](https://github.com/HuGR-Labs/corelink-server/blob/cca798ff5bc2df660ecf2570ed243eb9775ff3d0/crates/corelink-dsr/Cargo.toml#L1-L2); blob Git reportado `d7250a943771103c399bd8848c4aa98492190988`.
...
- 8 declarações de dependência e 5 registros inversos. O censo preserva kind, aliases, optional, cfg e features. A união inclui workspaces independentes e **não é um grafo resolvido de um build**.  Consumidores declarados: `corelink-privacy`, `corelink-privacy-erasure-worker`, `corelink-server`, `e2e-billing-flow`, `e2e-dsr`.
...
- Risco: Fronteira a conferir: `crates/corelink-dsr/src/audit.rs:215` contém `impl InMemoryDsrAuditSink {`; provar seleção e efeito, não inferir runtime do nome.
- Risco: Fronteira a conferir: `crates/corelink-dsr/src/audit.rs:265` contém `impl DsrAuditSink for InMemoryDsrAuditSink {`; provar seleção e efeito, não inferir runtime do nome.
- Risco: Fronteira a conferir: `crates/corelink-dsr/src/audit.rs:297` contém `#[cfg(test)]`; provar seleção e efeito, não inferir runtime do nome.

### Comandos candidatos — não executados
- `cargo metadata --locked --offline --no-deps --format-version=1` — READ_ONLY; NOT_EXECUTED.
- `python3 scripts/okf_context.py --file crates/corelink-dsr/src/lib.rs --full` — READ_ONLY; NOT_EXECUTED.
- `cargo tree --locked --offline --manifest-path crates/corelink-dsr/Cargo.toml -p corelink-dsr --target x86_64-unknown-linux-gnu --edges normal,build` — READ_ONLY_RESOLUTION; NOT_EXECUTED.

### Evidência de fonte
- `crates/corelink-dsr/Cargo.toml` @ `d7250a943771103c399bd8848c4aa98492190988`; source `37f96dba42cc159cd8ab8e9c4ab9d35b8e5634b5`.
- `crates/corelink-dsr/src/lib.rs` @ `9499a36d58012395ae7464ec961fa96af28ab64a`; source `37f96dba42cc159cd8ab8e9c4ab9d35b8e5634b5`.
...
- [ ] `.claude/skills/own-corelink-dsr/SKILL.md`.
- [ ] `docs/ownership/crates/corelink-dsr/REFERENCE.md`, único e indexado.
- [ ] `docs/ownership/crates/corelink-dsr/BLAST_RADIUS.md`, único, com todas as relações de fronteira conciliadas.
- [ ] `docs/ownership/crates/corelink-dsr/MAINTENANCE.md`, único, com procedimentos verificáveis.
