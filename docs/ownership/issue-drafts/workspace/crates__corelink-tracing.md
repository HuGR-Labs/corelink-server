<!-- corelink-ownership:v1:manifest=crates/corelink-tracing/Cargo.toml -->
# [ownership] corelink-tracing: skill, referência, blast radius e manutenção

## Identidade e preparação

| Campo | Valor |
|---|---|
| Package / manifesto | corelink-tracing / crates/corelink-tracing/Cargo.toml |
...
**Contexto canônico OKF:** Consultar `scripts/okf_context.py --file crates/corelink-tracing/Cargo.toml` e arquivos de implementação depois de enumerá-los; confirmar os conceitos em `docs/knowledge/index.md`. A consulta não foi executada neste estudo.
...
**Identidade:** `corelink-tracing`; manifesto `crates/corelink-tracing/Cargo.toml`; skill `own-corelink-tracing`.
**Fonte imutável:** [declaração do package](https://github.com/HuGR-Labs/corelink-server/blob/cca798ff5bc2df660ecf2570ed243eb9775ff3d0/crates/corelink-tracing/Cargo.toml#L1-L2); blob Git reportado `c80a90b8f45aec0bfc9dabd3cdade8d5a099de85`.
...
- 7 declarações de dependência e 1 registros inversos. O censo preserva kind, aliases, optional, cfg e features. A união inclui workspaces independentes e **não é um grafo resolvido de um build**.  Consumidores declarados: `corelink-telemetry`.
...
- Risco: Fronteira a conferir: `crates/corelink-tracing/src/audit.rs:108` contém `pub use crate::error::TracingAuditSinkError as TracingAuditEmitError;`; provar seleção e efeito, não inferir runtime do nome.
- Risco: Fronteira a conferir: `crates/corelink-tracing/src/audit.rs:133` contém `impl InMemoryTracingAuditSink {`; provar seleção e efeito, não inferir runtime do nome.
- Risco: Fronteira a conferir: `crates/corelink-tracing/src/audit.rs:174` contém `impl TracingAuditSink for InMemoryTracingAuditSink {`; provar seleção e efeito, não inferir runtime do nome.

### Comandos candidatos — não executados
- `cargo metadata --locked --offline --no-deps --format-version=1` — READ_ONLY; NOT_EXECUTED.
- `python3 scripts/okf_context.py --file crates/corelink-tracing/src/lib.rs --full` — READ_ONLY; NOT_EXECUTED.
- `cargo tree --locked --offline --manifest-path crates/corelink-tracing/Cargo.toml -p corelink-tracing --target x86_64-unknown-linux-gnu --edges normal,build` — READ_ONLY_RESOLUTION; NOT_EXECUTED.

### Evidência de fonte
- `crates/corelink-tracing/Cargo.toml` @ `c80a90b8f45aec0bfc9dabd3cdade8d5a099de85`; source `37f96dba42cc159cd8ab8e9c4ab9d35b8e5634b5`.
- `crates/corelink-tracing/src/lib.rs` @ `4d67172dac125b5c0dbe0d26c6b47ac9f3babc95`; source `37f96dba42cc159cd8ab8e9c4ab9d35b8e5634b5`.
...
- [ ] `.claude/skills/own-corelink-tracing/SKILL.md`.
- [ ] `docs/ownership/crates/corelink-tracing/REFERENCE.md`, único e indexado.
- [ ] `docs/ownership/crates/corelink-tracing/BLAST_RADIUS.md`, único, com todas as relações de fronteira conciliadas.
- [ ] `docs/ownership/crates/corelink-tracing/MAINTENANCE.md`, único, com procedimentos verificáveis.
