<!-- corelink-ownership:v1:manifest=tests/e2e-byok-revoke/Cargo.toml -->
# [ownership] e2e-byok-revoke: skill, referência, blast radius e manutenção
...
**Contexto canônico OKF:** Consultar `scripts/okf_context.py --file tests/e2e-byok-revoke/Cargo.toml` e arquivos de implementação depois de enumerá-los; confirmar os conceitos em `docs/knowledge/index.md`. A consulta não foi executada neste estudo.
...
**Identidade:** `e2e-byok-revoke`; manifesto `tests/e2e-byok-revoke/Cargo.toml`; skill `own-e2e-byok-revoke`.
**Fonte imutável:** [declaração do package](https://github.com/HuGR-Labs/corelink-server/blob/cca798ff5bc2df660ecf2570ed243eb9775ff3d0/tests/e2e-byok-revoke/Cargo.toml#L1-L2); blob Git reportado `eb9e290d9a6f71f06fefc6a23156d43db2a52df2`.
...
- Risco: Fronteira a conferir: `tests/e2e-byok-revoke/src/helpers.rs:569` contém `impl InMemoryTenantStatusStore {`; provar seleção e efeito, não inferir runtime do nome.
- Risco: Fronteira a conferir: `tests/e2e-byok-revoke/src/helpers.rs:592` contém `impl TenantStatusStore for InMemoryTenantStatusStore {`; provar seleção e efeito, não inferir runtime do nome.
- Risco: Fronteira a conferir: `tests/e2e-byok-revoke/src/lib.rs:56` contém `#![forbid(unsafe_code)]`; provar seleção e efeito, não inferir runtime do nome.

### Comandos candidatos — não executados
- `cargo metadata --locked --offline --no-deps --format-version=1` — READ_ONLY; NOT_EXECUTED.
- `python3 scripts/okf_context.py --file tests/e2e-byok-revoke/src/lib.rs --full` — READ_ONLY; NOT_EXECUTED.
- `cargo tree --locked --offline --manifest-path tests/e2e-byok-revoke/Cargo.toml -p e2e-byok-revoke --target x86_64-unknown-linux-gnu --edges normal,build` — READ_ONLY_RESOLUTION; NOT_EXECUTED.
...
- `tests/e2e-byok-revoke/Cargo.toml` @ `eb9e290d9a6f71f06fefc6a23156d43db2a52df2`; source `37f96dba42cc159cd8ab8e9c4ab9d35b8e5634b5`.
...
- [ ] `.claude/skills/own-e2e-byok-revoke/SKILL.md`.
- [ ] `docs/ownership/crates/e2e-byok-revoke/REFERENCE.md`, único e indexado.
- [ ] `docs/ownership/crates/e2e-byok-revoke/BLAST_RADIUS.md`, único, com todas as relações de fronteira conciliadas.
- [ ] `docs/ownership/crates/e2e-byok-revoke/MAINTENANCE.md`, único, com procedimentos verificáveis.
