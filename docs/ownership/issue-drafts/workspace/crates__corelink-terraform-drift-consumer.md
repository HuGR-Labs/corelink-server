<!-- corelink-ownership:v1:manifest=crates/corelink-terraform-drift-consumer/Cargo.toml -->
# [ownership] corelink-terraform-drift-consumer: skill, referência, blast radius e manutenção

## Identidade e preparação

| Campo | Valor |
|---|---|
| Package / manifesto | corelink-terraform-drift-consumer / crates/corelink-terraform-drift-consumer/Cargo.toml |
...
**Contexto canônico OKF:** Consultar `scripts/okf_context.py --file crates/corelink-terraform-drift-consumer/Cargo.toml` e arquivos de implementação depois de enumerá-los; confirmar os conceitos em `docs/knowledge/index.md`. A consulta não foi executada neste estudo.
...
**Identidade:** `corelink-terraform-drift-consumer`; manifesto `crates/corelink-terraform-drift-consumer/Cargo.toml`; skill `own-corelink-terraform-drift-consumer`.
**Fonte imutável:** [declaração do package](https://github.com/HuGR-Labs/corelink-server/blob/cca798ff5bc2df660ecf2570ed243eb9775ff3d0/crates/corelink-terraform-drift-consumer/Cargo.toml#L1-L2); blob Git reportado `dceb8cfb3e533f07600d15a09e4dabc1e9229f5b`.
...
- Risco: Fronteira a conferir: `crates/corelink-terraform-drift-consumer/src/audit.rs:117` contém `impl DriftAuditSink for InMemoryDriftAuditSink {`; provar seleção e efeito, não inferir runtime do nome.
- Risco: Fronteira a conferir: `crates/corelink-terraform-drift-consumer/src/classifier.rs:54` contém `#[cfg(test)]`; provar seleção e efeito, não inferir runtime do nome.
- Risco: Fronteira a conferir: `crates/corelink-terraform-drift-consumer/src/consumer.rs:131` contém `#[cfg(test)]`; provar seleção e efeito, não inferir runtime do nome.

### Comandos candidatos — não executados
- `cargo metadata --locked --offline --no-deps --format-version=1` — READ_ONLY; NOT_EXECUTED.
- `python3 scripts/okf_context.py --file crates/corelink-terraform-drift-consumer/src/lib.rs --full` — READ_ONLY; NOT_EXECUTED.
- `cargo tree --locked --offline --manifest-path crates/corelink-terraform-drift-consumer/Cargo.toml -p corelink-terraform-drift-consumer --target x86_64-unknown-linux-gnu --edges normal,build` — READ_ONLY_RESOLUTION; NOT_EXECUTED.

### Evidência de fonte
- `crates/corelink-terraform-drift-consumer/Cargo.toml` @ `dceb8cfb3e533f07600d15a09e4dabc1e9229f5b`; source `37f96dba42cc159cd8ab8e9c4ab9d35b8e5634b5`.
- `crates/corelink-terraform-drift-consumer/src/lib.rs` @ `98d5c60724334ad16602389fdb24f80a2a52e832`; source `37f96dba42cc159cd8ab8e9c4ab9d35b8e5634b5`.
...
- [ ] `.claude/skills/own-corelink-terraform-drift-consumer/SKILL.md`.
- [ ] `docs/ownership/crates/corelink-terraform-drift-consumer/REFERENCE.md`, único e indexado.
- [ ] `docs/ownership/crates/corelink-terraform-drift-consumer/BLAST_RADIUS.md`, único, com todas as relações de fronteira conciliadas.
- [ ] `docs/ownership/crates/corelink-terraform-drift-consumer/MAINTENANCE.md`, único, com procedimentos verificáveis.
