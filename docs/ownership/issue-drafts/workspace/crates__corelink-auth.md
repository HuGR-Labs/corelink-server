<!-- corelink-ownership:v1:manifest=crates/corelink-auth/Cargo.toml -->
# [ownership] corelink-auth: skill, referência, blast radius e manutenção

## Identidade e preparação

| Campo | Valor |
|---|---|
| Package / manifesto | corelink-auth / crates/corelink-auth/Cargo.toml |
...
**Contexto canônico OKF:** Consultar `scripts/okf_context.py --file crates/corelink-auth/Cargo.toml` e arquivos de implementação depois de enumerá-los; confirmar os conceitos em `docs/knowledge/index.md`. A consulta não foi executada neste estudo.
...
**Identidade:** `corelink-auth`; manifesto `crates/corelink-auth/Cargo.toml`; skill `own-corelink-auth`.
**Fonte imutável:** [declaração do package](https://github.com/HuGR-Labs/corelink-server/blob/cca798ff5bc2df660ecf2570ed243eb9775ff3d0/crates/corelink-auth/Cargo.toml#L1-L2); blob Git reportado `fa745a73c62675cd7d1fb50eb6a2f6d4fc449c50`.
...
- 20 declarações de dependência e 1 registros inversos. O censo preserva kind, aliases, optional, cfg e features. A união inclui workspaces independentes e **não é um grafo resolvido de um build**.  Consumidores declarados: `corelink-reapi`.
...
- Risco: Investigar ativação e compatibilidade das features declaradas: clerk-jwt-adapter, default, tower-middleware. Não assumir que --all-features é válido.
- Risco: Fronteira a conferir: `crates/corelink-auth/src/clerk.rs:11` contém `pub use corelink_clerk::*;`; provar seleção e efeito, não inferir runtime do nome.
- Risco: Fronteira a conferir: `crates/corelink-auth/src/clerk_cf.rs:13` contém `pub use corelink_clerk_cf::*;`; provar seleção e efeito, não inferir runtime do nome.

### Comandos candidatos — não executados
- `cargo metadata --locked --offline --no-deps --format-version=1` — READ_ONLY; NOT_EXECUTED.
- `python3 scripts/okf_context.py --file crates/corelink-auth/src/lib.rs --full` — READ_ONLY; NOT_EXECUTED.
- `cargo tree --locked --offline --manifest-path crates/corelink-auth/Cargo.toml -p corelink-auth --target x86_64-unknown-linux-gnu --edges normal,build` — READ_ONLY_RESOLUTION; NOT_EXECUTED.

### Evidência de fonte
- `crates/corelink-auth/Cargo.toml` @ `fa745a73c62675cd7d1fb50eb6a2f6d4fc449c50`; source `37f96dba42cc159cd8ab8e9c4ab9d35b8e5634b5`.
- `crates/corelink-auth/src/lib.rs` @ `c0ae769abadc375c1e2995d2ebceab1b4704640b`; source `37f96dba42cc159cd8ab8e9c4ab9d35b8e5634b5`.
...
- [ ] `.claude/skills/own-corelink-auth/SKILL.md`.
- [ ] `docs/ownership/crates/corelink-auth/REFERENCE.md`, único e indexado.
- [ ] `docs/ownership/crates/corelink-auth/BLAST_RADIUS.md`, único, com todas as relações de fronteira conciliadas.
- [ ] `docs/ownership/crates/corelink-auth/MAINTENANCE.md`, único, com procedimentos verificáveis.
