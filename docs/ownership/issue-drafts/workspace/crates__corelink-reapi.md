<!-- corelink-ownership:v1:manifest=crates/corelink-reapi/Cargo.toml -->
# [ownership] corelink-reapi: skill, referência, blast radius e manutenção

## Identidade e preparação

| Campo | Valor |
|---|---|
| Package / manifesto | corelink-reapi / crates/corelink-reapi/Cargo.toml |
...
**Contexto canônico OKF:** Consultar `scripts/okf_context.py --file crates/corelink-reapi/Cargo.toml` e arquivos de implementação depois de enumerá-los; confirmar os conceitos em `docs/knowledge/index.md`. A consulta não foi executada neste estudo.
...
**Identidade:** `corelink-reapi`; manifesto `crates/corelink-reapi/Cargo.toml`; skill `own-corelink-reapi`.
**Fonte imutável:** [declaração do package](https://github.com/HuGR-Labs/corelink-server/blob/cca798ff5bc2df660ecf2570ed243eb9775ff3d0/crates/corelink-reapi/Cargo.toml#L1-L2); blob Git reportado `eb9ab5b3d0f168ed83021815d7094600c76264bd`.
...
- 39 declarações de dependência e 2 registros inversos. O censo preserva kind, aliases, optional, cfg e features. A união inclui workspaces independentes e **não é um grafo resolvido de um build**.  Consumidores declarados: `corelink-adapter-host`, `corelink-reapi-fuzz`.
...
- Risco: Investigar ativação e compatibilidade das features declaradas: default, host-server. Não assumir que --all-features é válido.
- Risco: Fronteira a conferir: `crates/corelink-reapi/build.rs:14` contém `if std::env::var_os("CARGO_FEATURE_HOST_SERVER").is_none() {`; provar seleção e efeito, não inferir runtime do nome.
- Risco: Fronteira a conferir: `crates/corelink-reapi/src/audit.rs:323` contém `#[cfg(test)]`; provar seleção e efeito, não inferir runtime do nome.

### Comandos candidatos — não executados
- `cargo metadata --locked --offline --no-deps --format-version=1` — READ_ONLY; NOT_EXECUTED.
- `python3 scripts/okf_context.py --file crates/corelink-reapi/src/lib.rs --full` — READ_ONLY; NOT_EXECUTED.
- `cargo tree --locked --offline --manifest-path crates/corelink-reapi/Cargo.toml -p corelink-reapi --target x86_64-unknown-linux-gnu --edges normal,build` — READ_ONLY_RESOLUTION; NOT_EXECUTED.

### Evidência de fonte
- `crates/corelink-reapi/Cargo.toml` @ `eb9ab5b3d0f168ed83021815d7094600c76264bd`; source `37f96dba42cc159cd8ab8e9c4ab9d35b8e5634b5`.
- `crates/corelink-reapi/src/lib.rs` @ `a4e6596b8741aded884e05b445df1b13edf1419b`; source `37f96dba42cc159cd8ab8e9c4ab9d35b8e5634b5`.
...
- [ ] `.claude/skills/own-corelink-reapi/SKILL.md`.
- [ ] `docs/ownership/crates/corelink-reapi/REFERENCE.md`, único e indexado.
- [ ] `docs/ownership/crates/corelink-reapi/BLAST_RADIUS.md`, único, com todas as relações de fronteira conciliadas.
- [ ] `docs/ownership/crates/corelink-reapi/MAINTENANCE.md`, único, com procedimentos verificáveis.
