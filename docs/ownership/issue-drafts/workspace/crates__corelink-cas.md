<!-- corelink-ownership:v1:manifest=crates/corelink-cas/Cargo.toml -->
# [ownership] corelink-cas: skill, referência, blast radius e manutenção

## Identidade e preparação

| Campo | Valor |
|---|---|
| Package / manifesto | corelink-cas / crates/corelink-cas/Cargo.toml |
...
**Contexto canônico OKF:** Consultar `scripts/okf_context.py --file crates/corelink-cas/Cargo.toml` e arquivos de implementação depois de enumerá-los; confirmar os conceitos em `docs/knowledge/index.md`. A consulta não foi executada neste estudo.
...
**Identidade:** `corelink-cas`; manifesto `crates/corelink-cas/Cargo.toml`; skill `own-corelink-cas`.
**Fonte imutável:** [declaração do package](https://github.com/HuGR-Labs/corelink-server/blob/cca798ff5bc2df660ecf2570ed243eb9775ff3d0/crates/corelink-cas/Cargo.toml#L1-L2); blob Git reportado `a54143de52036f2842319b7f2ec3111add8fa46a`.
...
- 21 declarações de dependência e 3 registros inversos. O censo preserva kind, aliases, optional, cfg e features. A união inclui workspaces independentes e **não é um grafo resolvido de um build**.  Consumidores declarados: `corelink-cf-bindings`, `corelink-reapi`.
...
- Risco: Fronteira a conferir: `crates/corelink-cas/src/chunker.rs:68` contém `#![forbid(unsafe_code)]`; provar seleção e efeito, não inferir runtime do nome.
- Risco: Fronteira a conferir: `crates/corelink-cas/src/chunker.rs:76` contém `pub use error::ChunkerError;`; provar seleção e efeito, não inferir runtime do nome.
- Risco: Fronteira a conferir: `crates/corelink-cas/src/chunker.rs:77` contém `pub use kind::{Chunker, ChunkerKind, ChunkerStep};`; provar seleção e efeito, não inferir runtime do nome.

### Comandos candidatos — não executados
- `cargo metadata --locked --offline --no-deps --format-version=1` — READ_ONLY; NOT_EXECUTED.
- `python3 scripts/okf_context.py --file crates/corelink-cas/src/lib.rs --full` — READ_ONLY; NOT_EXECUTED.
- `cargo tree --locked --offline --manifest-path crates/corelink-cas/Cargo.toml -p corelink-cas --target x86_64-unknown-linux-gnu --edges normal,build` — READ_ONLY_RESOLUTION; NOT_EXECUTED.

### Evidência de fonte
- `crates/corelink-cas/Cargo.toml` @ `a54143de52036f2842319b7f2ec3111add8fa46a`; source `37f96dba42cc159cd8ab8e9c4ab9d35b8e5634b5`.
- `crates/corelink-cas/src/lib.rs` @ `5eef35d4a9c8ad713604a6df5c7493d88b27248e`; source `37f96dba42cc159cd8ab8e9c4ab9d35b8e5634b5`.
...
- [ ] `.claude/skills/own-corelink-cas/SKILL.md`.
- [ ] `docs/ownership/crates/corelink-cas/REFERENCE.md`, único e indexado.
- [ ] `docs/ownership/crates/corelink-cas/BLAST_RADIUS.md`, único, com todas as relações de fronteira conciliadas.
- [ ] `docs/ownership/crates/corelink-cas/MAINTENANCE.md`, único, com procedimentos verificáveis.
