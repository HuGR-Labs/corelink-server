<!-- corelink-ownership:v1:manifest=crates/corelink-slo/Cargo.toml -->
# [ownership] corelink-slo: skill, referência, blast radius e manutenção

## Identidade e preparação

| Campo | Valor |
|---|---|
| Package / manifesto | corelink-slo / crates/corelink-slo/Cargo.toml |
...
**Contexto canônico OKF:** Consultar `scripts/okf_context.py --file crates/corelink-slo/Cargo.toml` e arquivos de implementação depois de enumerá-los; confirmar os conceitos em `docs/knowledge/index.md`. A consulta não foi executada neste estudo.
...
**Identidade:** `corelink-slo`; manifesto `crates/corelink-slo/Cargo.toml`; skill `own-corelink-slo`.
**Fonte imutável:** [declaração do package](https://github.com/HuGR-Labs/corelink-server/blob/cca798ff5bc2df660ecf2570ed243eb9775ff3d0/crates/corelink-slo/Cargo.toml#L1-L2); blob Git reportado `cd12aa05323b31644e3b2efe6231bc49825e761d`.
...
- 4 declarações de dependência e 11 registros inversos. O censo preserva kind, aliases, optional, cfg e features. A união inclui workspaces independentes e **não é um grafo resolvido de um build**.  Consumidores declarados: `corelink-handler-ac`, `corelink-handler-admin`, `corelink-handler-cas`, `corelink-handler-customer`, `corelink-ops`, `corelink-privacy-erasure-worker`, `corelink-region`, `corelink-replica-worker`, `corelink-server`, `corelink-telemetry`.
...
- Risco: Fronteira a conferir: `crates/corelink-slo/src/alert.rs:351` contém `#[cfg(test)]`; provar seleção e efeito, não inferir runtime do nome.
- Risco: Fronteira a conferir: `crates/corelink-slo/src/audit.rs:157` contém `impl InMemorySloAuditSink {`; provar seleção e efeito, não inferir runtime do nome.
- Risco: Fronteira a conferir: `crates/corelink-slo/src/audit.rs:198` contém `impl SloAuditSink for InMemorySloAuditSink {`; provar seleção e efeito, não inferir runtime do nome.

### Comandos candidatos — não executados
- `cargo metadata --locked --offline --no-deps --format-version=1` — READ_ONLY; NOT_EXECUTED.
- `python3 scripts/okf_context.py --file crates/corelink-slo/src/lib.rs --full` — READ_ONLY; NOT_EXECUTED.
- `cargo tree --locked --offline --manifest-path crates/corelink-slo/Cargo.toml -p corelink-slo --target x86_64-unknown-linux-gnu --edges normal,build` — READ_ONLY_RESOLUTION; NOT_EXECUTED.

### Evidência de fonte
- `crates/corelink-slo/Cargo.toml` @ `cd12aa05323b31644e3b2efe6231bc49825e761d`; source `37f96dba42cc159cd8ab8e9c4ab9d35b8e5634b5`.
- `crates/corelink-slo/src/lib.rs` @ `ab88f25561880927d8919c0052cb4d887ca13c69`; source `37f96dba42cc159cd8ab8e9c4ab9d35b8e5634b5`.
...
- [ ] `.claude/skills/own-corelink-slo/SKILL.md`.
- [ ] `docs/ownership/crates/corelink-slo/REFERENCE.md`, único e indexado.
- [ ] `docs/ownership/crates/corelink-slo/BLAST_RADIUS.md`, único, com todas as relações de fronteira conciliadas.
- [ ] `docs/ownership/crates/corelink-slo/MAINTENANCE.md`, único, com procedimentos verificáveis.
