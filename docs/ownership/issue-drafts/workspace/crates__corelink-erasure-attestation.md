<!-- corelink-ownership:v1:manifest=crates/corelink-erasure-attestation/Cargo.toml -->
# [ownership] corelink-erasure-attestation: skill, referência, blast radius e manutenção

## Identidade e preparação

| Campo | Valor |
|---|---|
| Package / manifesto | corelink-erasure-attestation / crates/corelink-erasure-attestation/Cargo.toml |
...
**Contexto canônico OKF:** Consultar `scripts/okf_context.py --file crates/corelink-erasure-attestation/Cargo.toml` e arquivos de implementação depois de enumerá-los; confirmar os conceitos em `docs/knowledge/index.md`. A consulta não foi executada neste estudo.
...
**Identidade:** `corelink-erasure-attestation`; manifesto `crates/corelink-erasure-attestation/Cargo.toml`; skill `own-corelink-erasure-attestation`.
**Fonte imutável:** [declaração do package](https://github.com/HuGR-Labs/corelink-server/blob/cca798ff5bc2df660ecf2570ed243eb9775ff3d0/crates/corelink-erasure-attestation/Cargo.toml#L1-L2); blob Git reportado `3a29530e9be58ae2bf6d9f36f365c1938cc093f7`.
...
- 14 declarações de dependência e 2 registros inversos. O censo preserva kind, aliases, optional, cfg e features. A união inclui workspaces independentes e **não é um grafo resolvido de um build**.  Consumidores declarados: `corelink-crypto`, `corelink-server`.
...
- Risco: Fronteira a conferir: `crates/corelink-erasure-attestation/src/attestation.rs:133` contém `#[cfg(test)]`; provar seleção e efeito, não inferir runtime do nome.
- Risco: Fronteira a conferir: `crates/corelink-erasure-attestation/src/evidence.rs:88` contém `#[cfg(test)]`; provar seleção e efeito, não inferir runtime do nome.
- Risco: Fronteira a conferir: `crates/corelink-erasure-attestation/src/key.rs:223` contém `#[cfg(test)]`; provar seleção e efeito, não inferir runtime do nome.

### Comandos candidatos — não executados
- `cargo metadata --locked --offline --no-deps --format-version=1` — READ_ONLY; NOT_EXECUTED.
- `python3 scripts/okf_context.py --file crates/corelink-erasure-attestation/src/lib.rs --full` — READ_ONLY; NOT_EXECUTED.
- `cargo tree --locked --offline --manifest-path crates/corelink-erasure-attestation/Cargo.toml -p corelink-erasure-attestation --target x86_64-unknown-linux-gnu --edges normal,build` — READ_ONLY_RESOLUTION; NOT_EXECUTED.

### Evidência de fonte
- `crates/corelink-erasure-attestation/Cargo.toml` @ `3a29530e9be58ae2bf6d9f36f365c1938cc093f7`; source `37f96dba42cc159cd8ab8e9c4ab9d35b8e5634b5`.
- `crates/corelink-erasure-attestation/src/lib.rs` @ `8336a891946ea7c8999e4727e09ad183d0f7d30d`; source `37f96dba42cc159cd8ab8e9c4ab9d35b8e5634b5`.
...
- [ ] `.claude/skills/own-corelink-erasure-attestation/SKILL.md`.
- [ ] `docs/ownership/crates/corelink-erasure-attestation/REFERENCE.md`, único e indexado.
- [ ] `docs/ownership/crates/corelink-erasure-attestation/BLAST_RADIUS.md`, único, com todas as relações de fronteira conciliadas.
- [ ] `docs/ownership/crates/corelink-erasure-attestation/MAINTENANCE.md`, único, com procedimentos verificáveis.
