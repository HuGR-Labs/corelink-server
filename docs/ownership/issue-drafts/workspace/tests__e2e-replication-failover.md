<!-- corelink-ownership:v1:manifest=tests/e2e-replication-failover/Cargo.toml -->
# [ownership] e2e-replication-failover: skill, referência, blast radius e manutenção

## Identidade e preparação

| Campo | Valor |
|---|---|
| Package / manifesto | e2e-replication-failover / tests/e2e-replication-failover/Cargo.toml |
...
**Contexto canônico OKF:** Consultar `scripts/okf_context.py --file tests/e2e-replication-failover/Cargo.toml` e arquivos de implementação depois de enumerá-los; confirmar os conceitos em `docs/knowledge/index.md`. A consulta não foi executada neste estudo.
...
**Identidade:** `e2e-replication-failover`; manifesto `tests/e2e-replication-failover/Cargo.toml`; skill `own-e2e-replication-failover`.
**Fonte imutável:** [declaração do package](https://github.com/HuGR-Labs/corelink-server/blob/cca798ff5bc2df660ecf2570ed243eb9775ff3d0/tests/e2e-replication-failover/Cargo.toml#L1-L2); blob Git reportado `13ed9f963c625e3f06885e1aad711547d4ecee67`.
...
- Risco: Fronteira a conferir: `tests/e2e-replication-failover/src/lib.rs:14` contém `#![forbid(unsafe_code)]`; provar seleção e efeito, não inferir runtime do nome.
...
- `python3 scripts/okf_context.py --file tests/e2e-replication-failover/src/lib.rs --full` — READ_ONLY; NOT_EXECUTED.
- `cargo tree --locked --offline --manifest-path tests/e2e-replication-failover/Cargo.toml -p e2e-replication-failover --target x86_64-unknown-linux-gnu --edges normal,build` — READ_ONLY_RESOLUTION; NOT_EXECUTED.
...
- `tests/e2e-replication-failover/Cargo.toml` @ `13ed9f963c625e3f06885e1aad711547d4ecee67`; source `37f96dba42cc159cd8ab8e9c4ab9d35b8e5634b5`.
- `tests/e2e-replication-failover/src/lib.rs` @ `aba5e832046012cdbc11131a79daace51cdfa45a`; source `37f96dba42cc159cd8ab8e9c4ab9d35b8e5634b5`.
...
- [ ] `docs/ownership/crates/e2e-replication-failover/REFERENCE.md`, único e indexado.
- [ ] `docs/ownership/crates/e2e-replication-failover/BLAST_RADIUS.md`, único, com todas as relações de fronteira conciliadas.
- [ ] `docs/ownership/crates/e2e-replication-failover/MAINTENANCE.md`, único, com procedimentos verificáveis.
