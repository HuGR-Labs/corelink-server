<!-- corelink-ownership:v1:manifest=crates/corelink-turbo-bridge/Cargo.toml -->
# [ownership] corelink-turbo-bridge: skill, referência, blast radius e manutenção

## Identidade e preparação

| Campo | Valor |
|---|---|
| Package / manifesto | corelink-turbo-bridge / crates/corelink-turbo-bridge/Cargo.toml |
...
**Contexto canônico OKF:** Consultar `scripts/okf_context.py --file crates/corelink-turbo-bridge/Cargo.toml` e arquivos de implementação depois de enumerá-los; confirmar os conceitos em `docs/knowledge/index.md`. A consulta não foi executada neste estudo.
...
**Identidade:** `corelink-turbo-bridge`; manifesto `crates/corelink-turbo-bridge/Cargo.toml`; skill `own-corelink-turbo-bridge`.
**Fonte imutável:** [declaração do package](https://github.com/HuGR-Labs/corelink-server/blob/cca798ff5bc2df660ecf2570ed243eb9775ff3d0/crates/corelink-turbo-bridge/Cargo.toml#L1-L2); blob Git reportado `c531f2af379908107bf8b994077fae19e8103983`.
...
- 3 declarações de dependência e 1 registros inversos. O censo preserva kind, aliases, optional, cfg e features. A união inclui workspaces independentes e **não é um grafo resolvido de um build**.  Consumidores declarados: `corelink-server`.
...
- Risco: Fronteira a conferir: `crates/corelink-turbo-bridge/src/adapter.rs:97` contém `impl core::fmt::Debug for InMemoryKvStore {`; provar seleção e efeito, não inferir runtime do nome.
- Risco: Fronteira a conferir: `crates/corelink-turbo-bridge/src/adapter.rs:103` contém `impl InMemoryKvStore {`; provar seleção e efeito, não inferir runtime do nome.
- Risco: Fronteira a conferir: `crates/corelink-turbo-bridge/src/adapter.rs:113` contém `impl Default for InMemoryKvStore {`; provar seleção e efeito, não inferir runtime do nome.

### Comandos candidatos — não executados
- `cargo metadata --locked --offline --no-deps --format-version=1` — READ_ONLY; NOT_EXECUTED.
- `python3 scripts/okf_context.py --file crates/corelink-turbo-bridge/src/lib.rs --full` — READ_ONLY; NOT_EXECUTED.
- `cargo tree --locked --offline --manifest-path crates/corelink-turbo-bridge/Cargo.toml -p corelink-turbo-bridge --target x86_64-unknown-linux-gnu --edges normal,build` — READ_ONLY_RESOLUTION; NOT_EXECUTED.

### Evidência de fonte
- `crates/corelink-turbo-bridge/Cargo.toml` @ `c531f2af379908107bf8b994077fae19e8103983`; source `37f96dba42cc159cd8ab8e9c4ab9d35b8e5634b5`.
- `crates/corelink-turbo-bridge/src/lib.rs` @ `d5b6c8c3bf53f938fdb6dc7c0e2606695a5c29df`; source `37f96dba42cc159cd8ab8e9c4ab9d35b8e5634b5`.
- `docs/knowledge/surfaces/turborepo.md` @ `2f40d62a4c2fe29b6e2b0acc65ced97df4f57a3b`; source `37f96dba42cc159cd8ab8e9c4ab9d35b8e5634b5`.
...
- [ ] `.claude/skills/own-corelink-turbo-bridge/SKILL.md`.
- [ ] `docs/ownership/crates/corelink-turbo-bridge/REFERENCE.md`, único e indexado.
- [ ] `docs/ownership/crates/corelink-turbo-bridge/BLAST_RADIUS.md`, único, com todas as relações de fronteira conciliadas.
- [ ] `docs/ownership/crates/corelink-turbo-bridge/MAINTENANCE.md`, único, com procedimentos verificáveis.
