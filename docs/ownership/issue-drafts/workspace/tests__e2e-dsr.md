<!-- corelink-ownership:v1:manifest=tests/e2e-dsr/Cargo.toml -->
# [ownership] e2e-dsr: skill, referência, blast radius e manutenção

## Identidade e preparação

| Campo | Valor |
|---|---|
| Package / manifesto | e2e-dsr / tests/e2e-dsr/Cargo.toml |
...
**Contexto canônico OKF:** Consultar `scripts/okf_context.py --file tests/e2e-dsr/Cargo.toml` e arquivos de implementação depois de enumerá-los; confirmar os conceitos em `docs/knowledge/index.md`. A consulta não foi executada neste estudo.
...
**Identidade:** `e2e-dsr`; manifesto `tests/e2e-dsr/Cargo.toml`; skill `own-e2e-dsr`.
**Fonte imutável:** [declaração do package](https://github.com/HuGR-Labs/corelink-server/blob/cca798ff5bc2df660ecf2570ed243eb9775ff3d0/tests/e2e-dsr/Cargo.toml#L1-L2); blob Git reportado `2a7e9d070bce08dcd8d2e8f8db6ca184e4cf15f1`.
...
- Risco: Fronteira a conferir: `tests/e2e-dsr/src/lib.rs:40` contém `#![forbid(unsafe_code)]`; provar seleção e efeito, não inferir runtime do nome.
- Risco: Fronteira a conferir: `tests/e2e-dsr/src/lib.rs:48` contém `pub use helpers::{`; provar seleção e efeito, não inferir runtime do nome.
- Risco: Fronteira a conferir: `tests/e2e-dsr/src/lib.rs:53` contém `pub use policy::{PolicyDecision, RestrictionFlag, TenantPolicyLedger};`; provar seleção e efeito, não inferir runtime do nome.

### Comandos candidatos — não executados
- `cargo metadata --locked --offline --no-deps --format-version=1` — READ_ONLY; NOT_EXECUTED.
- `python3 scripts/okf_context.py --file tests/e2e-dsr/src/lib.rs --full` — READ_ONLY; NOT_EXECUTED.
- `cargo tree --locked --offline --manifest-path tests/e2e-dsr/Cargo.toml -p e2e-dsr --target x86_64-unknown-linux-gnu --edges normal,build` — READ_ONLY_RESOLUTION; NOT_EXECUTED.
...
- `tests/e2e-dsr/Cargo.toml` @ `2a7e9d070bce08dcd8d2e8f8db6ca184e4cf15f1`; source `37f96dba42cc159cd8ab8e9c4ab9d35b8e5634b5`.
- `tests/e2e-dsr/src/lib.rs` @ `6ef8cd16c4a9d2e0891fbd67fbe9355f7253cdea`; source `37f96dba42cc159cd8ab8e9c4ab9d35b8e5634b5`.
...
- [ ] `.claude/skills/own-e2e-dsr/SKILL.md`.
- [ ] `docs/ownership/crates/e2e-dsr/REFERENCE.md`, único e indexado.
- [ ] `docs/ownership/crates/e2e-dsr/BLAST_RADIUS.md`, único, com todas as relações de fronteira conciliadas.
- [ ] `docs/ownership/crates/e2e-dsr/MAINTENANCE.md`, único, com procedimentos verificáveis.
