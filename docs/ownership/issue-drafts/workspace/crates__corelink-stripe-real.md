<!-- corelink-ownership:v1:manifest=crates/corelink-stripe-real/Cargo.toml -->
# [ownership] corelink-stripe-real: skill, referência, blast radius e manutenção

## Identidade e preparação

| Campo | Valor |
|---|---|
| Package / manifesto | corelink-stripe-real / crates/corelink-stripe-real/Cargo.toml |
...
**Contexto canônico OKF:** Consultar `scripts/okf_context.py --file crates/corelink-stripe-real/Cargo.toml` e arquivos de implementação depois de enumerá-los; confirmar os conceitos em `docs/knowledge/index.md`. A consulta não foi executada neste estudo.
...
**Identidade:** `corelink-stripe-real`; manifesto `crates/corelink-stripe-real/Cargo.toml`; skill `own-corelink-stripe-real`.
**Fonte imutável:** [declaração do package](https://github.com/HuGR-Labs/corelink-server/blob/cca798ff5bc2df660ecf2570ed243eb9775ff3d0/crates/corelink-stripe-real/Cargo.toml#L1-L2); blob Git reportado `db2c9d4c48a5b3ec4d7c02db9acef19a37273970`.
...
- 18 declarações de dependência e 5 registros inversos. O censo preserva kind, aliases, optional, cfg e features. A união inclui workspaces independentes e **não é um grafo resolvido de um build**.  Consumidores declarados: `corelink-adapters-cloud`, `corelink-billing`, `corelink-billing-stripe-materializer`, `corelink-server`, `e2e-signup-flow`.
...
- Risco: Investigar ativação e compatibilidade das features declaradas: default, live-integration. Não assumir que --all-features é válido.
- Risco: Fronteira a conferir: `crates/corelink-stripe-real/src/client.rs:976` contém `#[cfg(test)]`; provar seleção e efeito, não inferir runtime do nome.
- Risco: Fronteira a conferir: `crates/corelink-stripe-real/src/clock.rs:116` contém `#[cfg(not(target_arch = "wasm32"))]`; provar seleção e efeito, não inferir runtime do nome.

### Comandos candidatos — não executados
- `cargo metadata --locked --offline --no-deps --format-version=1` — READ_ONLY; NOT_EXECUTED.
- `python3 scripts/okf_context.py --file crates/corelink-stripe-real/src/lib.rs --full` — READ_ONLY; NOT_EXECUTED.
- `cargo tree --locked --offline --manifest-path crates/corelink-stripe-real/Cargo.toml -p corelink-stripe-real --target x86_64-unknown-linux-gnu --edges normal,build` — READ_ONLY_RESOLUTION; NOT_EXECUTED.

### Evidência de fonte
- `crates/corelink-stripe-real/Cargo.toml` @ `db2c9d4c48a5b3ec4d7c02db9acef19a37273970`; source `37f96dba42cc159cd8ab8e9c4ab9d35b8e5634b5`.
- `crates/corelink-stripe-real/src/lib.rs` @ `6aeb650dcfd1d285568c43b45eb2fcc1144b2ff2`; source `37f96dba42cc159cd8ab8e9c4ab9d35b8e5634b5`.
...
- `docs/knowledge/launch/stripe-activation-webhook.md` @ `5f4b299716833c6adce3ab86d94df72aac4990b9`; source `37f96dba42cc159cd8ab8e9c4ab9d35b8e5634b5`.
...
- [ ] `.claude/skills/own-corelink-stripe-real/SKILL.md`.
- [ ] `docs/ownership/crates/corelink-stripe-real/REFERENCE.md`, único e indexado.
- [ ] `docs/ownership/crates/corelink-stripe-real/BLAST_RADIUS.md`, único, com todas as relações de fronteira conciliadas.
- [ ] `docs/ownership/crates/corelink-stripe-real/MAINTENANCE.md`, único, com procedimentos verificáveis.
