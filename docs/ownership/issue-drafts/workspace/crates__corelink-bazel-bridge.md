<!-- corelink-ownership:v1:manifest=crates/corelink-bazel-bridge/Cargo.toml -->
# [ownership] corelink-bazel-bridge: skill, referência, blast radius e manutenção
...
**Contexto canônico OKF:** Consultar `scripts/okf_context.py --file crates/corelink-bazel-bridge/Cargo.toml` e arquivos de implementação depois de enumerá-los; confirmar os conceitos em `docs/knowledge/index.md`. A consulta não foi executada neste estudo.
...
**Identidade:** `corelink-bazel-bridge`; manifesto `crates/corelink-bazel-bridge/Cargo.toml`; skill `own-corelink-bazel-bridge`.
**Fonte imutável:** [declaração do package](https://github.com/HuGR-Labs/corelink-server/blob/cca798ff5bc2df660ecf2570ed243eb9775ff3d0/crates/corelink-bazel-bridge/Cargo.toml#L1-L2); blob Git reportado `8b8d4859722d1cfc111d2d290d5250f354d4b7aa`.
...
- 10 declarações de dependência e 1 registros inversos. O censo preserva kind, aliases, optional, cfg e features. A união inclui workspaces independentes e **não é um grafo resolvido de um build**.  Consumidores declarados: `corelink-server`.
...
- Risco: Fronteira a conferir: `crates/corelink-bazel-bridge/src/adapter.rs:294` contém `#[cfg(test)]`; provar seleção e efeito, não inferir runtime do nome.
- Risco: Fronteira a conferir: `crates/corelink-bazel-bridge/src/digest.rs:195` contém `#[cfg(test)]`; provar seleção e efeito, não inferir runtime do nome.
- Risco: Fronteira a conferir: `crates/corelink-bazel-bridge/src/error.rs:132` contém `#[cfg(test)]`; provar seleção e efeito, não inferir runtime do nome.

### Comandos candidatos — não executados
- `cargo metadata --locked --offline --no-deps --format-version=1` — READ_ONLY; NOT_EXECUTED.
- `python3 scripts/okf_context.py --file crates/corelink-bazel-bridge/src/lib.rs --full` — READ_ONLY; NOT_EXECUTED.
- `cargo tree --locked --offline --manifest-path crates/corelink-bazel-bridge/Cargo.toml -p corelink-bazel-bridge --target x86_64-unknown-linux-gnu --edges normal,build` — READ_ONLY_RESOLUTION; NOT_EXECUTED.

### Evidência de fonte
- `crates/corelink-bazel-bridge/Cargo.toml` @ `8b8d4859722d1cfc111d2d290d5250f354d4b7aa`; source `37f96dba42cc159cd8ab8e9c4ab9d35b8e5634b5`.
- `crates/corelink-bazel-bridge/src/lib.rs` @ `97d1c683608fa1b8614c0d4ac06f71705413632d`; source `37f96dba42cc159cd8ab8e9c4ab9d35b8e5634b5`.
...
- [ ] `.claude/skills/own-corelink-bazel-bridge/SKILL.md`.
- [ ] `docs/ownership/crates/corelink-bazel-bridge/REFERENCE.md`, único e indexado.
- [ ] `docs/ownership/crates/corelink-bazel-bridge/BLAST_RADIUS.md`, único, com todas as relações de fronteira conciliadas.
- [ ] `docs/ownership/crates/corelink-bazel-bridge/MAINTENANCE.md`, único, com procedimentos verificáveis.
