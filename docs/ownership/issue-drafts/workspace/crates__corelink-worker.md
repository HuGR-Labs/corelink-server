<!-- corelink-ownership:v1:manifest=crates/corelink-worker/Cargo.toml -->
# [ownership] corelink-worker: skill, referência, blast radius e manutenção
...
**Contexto canônico OKF:** Consultar `scripts/okf_context.py --file crates/corelink-worker/Cargo.toml` e arquivos de implementação depois de enumerá-los; confirmar os conceitos em `docs/knowledge/index.md`. A consulta não foi executada neste estudo.
...
**Identidade:** `corelink-worker`; manifesto `crates/corelink-worker/Cargo.toml`; skill `own-corelink-worker`.
**Fonte imutável:** [declaração do package](https://github.com/HuGR-Labs/corelink-server/blob/cca798ff5bc2df660ecf2570ed243eb9775ff3d0/crates/corelink-worker/Cargo.toml#L1-L2); blob Git reportado `18e9450da475cd29e2a168ee670398542189b95c`.
...
- Risco: Investigar ativação e compatibilidade das features declaradas: default, tower-middleware. Não assumir que --all-features é válido.
- Risco: Fronteira a conferir: `crates/corelink-worker/src/auth/revocation.rs:111` contém `#[cfg(test)]`; provar seleção e efeito, não inferir runtime do nome.
- Risco: Fronteira a conferir: `crates/corelink-worker/src/auth/revocation.rs:119` contém `pub use in_memory_broadcast::InMemoryBroadcast;`; provar seleção e efeito, não inferir runtime do nome.

### Comandos candidatos — não executados
- `cargo metadata --locked --offline --no-deps --format-version=1` — READ_ONLY; NOT_EXECUTED.
- `python3 scripts/okf_context.py --file crates/corelink-worker/src/lib.rs --full` — READ_ONLY; NOT_EXECUTED.
- `cargo tree --locked --offline --manifest-path crates/corelink-worker/Cargo.toml -p corelink-worker --target x86_64-unknown-linux-gnu --edges normal,build` — READ_ONLY_RESOLUTION; NOT_EXECUTED.

### Evidência de fonte
- `crates/corelink-worker/Cargo.toml` @ `18e9450da475cd29e2a168ee670398542189b95c`; source `37f96dba42cc159cd8ab8e9c4ab9d35b8e5634b5`.
- `crates/corelink-worker/src/lib.rs` @ `a9e915b9f19cc5744371c25eab2e60501a189ccf`; source `37f96dba42cc159cd8ab8e9c4ab9d35b8e5634b5`.
...
- [ ] `.claude/skills/own-corelink-worker/SKILL.md`.
- [ ] `docs/ownership/crates/corelink-worker/REFERENCE.md`, único e indexado.
- [ ] `docs/ownership/crates/corelink-worker/BLAST_RADIUS.md`, único, com todas as relações de fronteira conciliadas.
- [ ] `docs/ownership/crates/corelink-worker/MAINTENANCE.md`, único, com procedimentos verificáveis.
