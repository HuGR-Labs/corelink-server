<!-- corelink-ownership:v1:manifest=crates/corelink-transparency-log/Cargo.toml -->
# [ownership] corelink-transparency-log: skill, referência, blast radius e manutenção

## Identidade e preparação

| Campo | Valor |
|---|---|
| Package / manifesto | corelink-transparency-log / crates/corelink-transparency-log/Cargo.toml |
...
**Contexto canônico OKF:** Consultar `scripts/okf_context.py --file crates/corelink-transparency-log/Cargo.toml` e arquivos de implementação depois de enumerá-los; confirmar os conceitos em `docs/knowledge/index.md`. A consulta não foi executada neste estudo.
...
**Identidade:** `corelink-transparency-log`; manifesto `crates/corelink-transparency-log/Cargo.toml`; skill `own-corelink-transparency-log`.
**Fonte imutável:** [declaração do package](https://github.com/HuGR-Labs/corelink-server/blob/cca798ff5bc2df660ecf2570ed243eb9775ff3d0/crates/corelink-transparency-log/Cargo.toml#L1-L2); blob Git reportado `08e00d5c52c21c2bb2563508399d31a1ed1f2ee8`.
...
- 10 declarações de dependência e 0 registros inversos. O censo preserva kind, aliases, optional, cfg e features. A união inclui workspaces independentes e **não é um grafo resolvido de um build**.  Consumidores declarados: nenhum na população Cargo examinada.
...
- Risco: Fronteira a conferir: `crates/corelink-transparency-log/src/entry.rs:166` contém `#[cfg(test)]`; provar seleção e efeito, não inferir runtime do nome.
- Risco: Fronteira a conferir: `crates/corelink-transparency-log/src/lib.rs:82` contém `#![forbid(unsafe_code)]`; provar seleção e efeito, não inferir runtime do nome.
- Risco: Fronteira a conferir: `crates/corelink-transparency-log/src/lib.rs:89` contém `pub use entry::{RekorHashedRekord, SignedEntry};`; provar seleção e efeito, não inferir runtime do nome.

### Comandos candidatos — não executados
- `cargo metadata --locked --offline --no-deps --format-version=1` — READ_ONLY; NOT_EXECUTED.
- `python3 scripts/okf_context.py --file crates/corelink-transparency-log/src/lib.rs --full` — READ_ONLY; NOT_EXECUTED.
- `cargo tree --locked --offline --manifest-path crates/corelink-transparency-log/Cargo.toml -p corelink-transparency-log --target x86_64-unknown-linux-gnu --edges normal,build` — READ_ONLY_RESOLUTION; NOT_EXECUTED.

### Evidência de fonte
- `crates/corelink-transparency-log/Cargo.toml` @ `08e00d5c52c21c2bb2563508399d31a1ed1f2ee8`; source `37f96dba42cc159cd8ab8e9c4ab9d35b8e5634b5`.
- `crates/corelink-transparency-log/src/lib.rs` @ `8e9f222412d0860c6c0259df9a6f10331fc5e1a1`; source `37f96dba42cc159cd8ab8e9c4ab9d35b8e5634b5`.
...
- [ ] `.claude/skills/own-corelink-transparency-log/SKILL.md`.
- [ ] `docs/ownership/crates/corelink-transparency-log/REFERENCE.md`, único e indexado.
- [ ] `docs/ownership/crates/corelink-transparency-log/BLAST_RADIUS.md`, único, com todas as relações de fronteira conciliadas.
- [ ] `docs/ownership/crates/corelink-transparency-log/MAINTENANCE.md`, único, com procedimentos verificáveis.
