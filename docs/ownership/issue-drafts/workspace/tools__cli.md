<!-- corelink-ownership:v1:manifest=tools/cli/Cargo.toml -->
# [ownership] corelink-cli: skill, referência, blast radius e manutenção

## Identidade e preparação

| Campo | Valor |
|---|---|
| Package / manifesto | corelink-cli / tools/cli/Cargo.toml |
...
**Contexto canônico OKF:** Consultar `scripts/okf_context.py --file tools/cli/Cargo.toml` e arquivos de implementação depois de enumerá-los; confirmar os conceitos em `docs/knowledge/index.md`. A consulta não foi executada neste estudo.
...
**Identidade:** `corelink-cli`; manifesto `tools/cli/Cargo.toml`; skill `own-corelink-cli`.
**Fonte imutável:** [declaração do package](https://github.com/HuGR-Labs/corelink-server/blob/cca798ff5bc2df660ecf2570ed243eb9775ff3d0/tools/cli/Cargo.toml#L1-L2); blob Git reportado `2f3f66a1b313588c0f4462084c59db254486134e`.
...
**Fonte desses campos:** [manifesto inspecionado](https://github.com/HuGR-Labs/corelink-server/blob/cca798ff5bc2df660ecf2570ed243eb9775ff3d0/tools/cli/Cargo.toml).

- Package corelink-cli; library corelink_cli; binário corelink.
- O manifesto declara wiremock 0.6 para testes locais. Isso não comprova a execução dos testes.

| Target declarado | Tipo | Path declarado |
|---|---|---|
| `corelink_cli` | lib | `src/lib.rs` |
| `corelink` | bin | `src/main.rs` |

Esta tabela contém os targets explicitamente inspecionados, não um inventário automático completo.

**Chaves de dependência inspecionadas:** `corelink-pat`, `corelink-runbook-tracker`, `corelink-audit-chain`, `corelink-analytics`. Resolução de herança, aliases, features e inversas continua pendente.

### Pontos específicos a investigar

- Preservar a distinção entre package e executável ao documentar instalação, comandos e gates.
- Examinar o fuzzer independente tools/cli/fuzz e a seleção dos parsers; não presumir versões transitivas iguais.
...
- 33 declarações de dependência e 1 registros inversos. O censo preserva kind, aliases, optional, cfg e features. A união inclui workspaces independentes e **não é um grafo resolvido de um build**.  Consumidores declarados: `corelink-cli-fuzz`.
...
- Risco: Fronteira a conferir: `tools/cli/examples/quickstart_audit.rs:15` contém `#![forbid(unsafe_code)]`; provar seleção e efeito, não inferir runtime do nome.
- Risco: Fronteira a conferir: `tools/cli/examples/quickstart_dsr_submit.rs:16` contém `#![forbid(unsafe_code)]`; provar seleção e efeito, não inferir runtime do nome.
- Risco: Fronteira a conferir: `tools/cli/examples/quickstart_get.rs:14` contém `#![forbid(unsafe_code)]`; provar seleção e efeito, não inferir runtime do nome.

### Comandos candidatos — não executados
- `cargo metadata --locked --offline --no-deps --format-version=1` — READ_ONLY; NOT_EXECUTED.
- `python3 scripts/okf_context.py --file tools/cli/src/lib.rs --full` — READ_ONLY; NOT_EXECUTED.
- `cargo tree --locked --offline --manifest-path tools/cli/Cargo.toml -p corelink-cli --target x86_64-unknown-linux-gnu --edges normal,build` — READ_ONLY_RESOLUTION; NOT_EXECUTED.
...
- `tools/cli/Cargo.toml` @ `2f3f66a1b313588c0f4462084c59db254486134e`; source `37f96dba42cc159cd8ab8e9c4ab9d35b8e5634b5`.
...
- [ ] `.claude/skills/own-corelink-cli/SKILL.md`.
- [ ] `docs/ownership/crates/corelink-cli/REFERENCE.md`, único e indexado.
- [ ] `docs/ownership/crates/corelink-cli/BLAST_RADIUS.md`, único, com todas as relações de fronteira conciliadas.
- [ ] `docs/ownership/crates/corelink-cli/MAINTENANCE.md`, único, com procedimentos verificáveis.
