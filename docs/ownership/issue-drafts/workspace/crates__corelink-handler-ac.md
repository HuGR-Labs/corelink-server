<!-- corelink-ownership:v1:manifest=crates/corelink-handler-ac/Cargo.toml -->
# [ownership] corelink-handler-ac: skill, referência, blast radius e manutenção

## Identidade e preparação

| Campo | Valor |
|---|---|
| Package / manifesto | corelink-handler-ac / crates/corelink-handler-ac/Cargo.toml |
...
**Contexto canônico OKF:** Consultar `scripts/okf_context.py --file crates/corelink-handler-ac/Cargo.toml` e arquivos de implementação depois de enumerá-los; confirmar os conceitos em `docs/knowledge/index.md`. A consulta não foi executada neste estudo.
...
**Identidade:** `corelink-handler-ac`; manifesto `crates/corelink-handler-ac/Cargo.toml`; skill `own-corelink-handler-ac`.
**Fonte imutável:** [declaração do package](https://github.com/HuGR-Labs/corelink-server/blob/cca798ff5bc2df660ecf2570ed243eb9775ff3d0/crates/corelink-handler-ac/Cargo.toml#L1-L2); blob Git reportado `1ba9f419d2bbaf4024e6b5dfbd0ea12da84bccb7`.
...
- 3 declarações de dependência e 3 registros inversos. O censo preserva kind, aliases, optional, cfg e features. A união inclui workspaces independentes e **não é um grafo resolvido de um build**.  Consumidores declarados: `corelink-ac`, `corelink-bazel-bridge`, `corelink-server`.
...
- Risco: Fronteira a conferir: `crates/corelink-handler-ac/src/audit.rs:112` contém `impl InMemoryAuditSink {`; provar seleção e efeito, não inferir runtime do nome.
- Risco: Fronteira a conferir: `crates/corelink-handler-ac/src/audit.rs:146` contém `impl AuditSink for InMemoryAuditSink {`; provar seleção e efeito, não inferir runtime do nome.
- Risco: Fronteira a conferir: `crates/corelink-handler-ac/src/handler.rs:392` contém `impl core::fmt::Debug for InMemoryAcHandler {`; provar seleção e efeito, não inferir runtime do nome.

### Comandos candidatos — não executados
- `cargo metadata --locked --offline --no-deps --format-version=1` — READ_ONLY; NOT_EXECUTED.
- `python3 scripts/okf_context.py --file crates/corelink-handler-ac/src/lib.rs --full` — READ_ONLY; NOT_EXECUTED.
- `cargo tree --locked --offline --manifest-path crates/corelink-handler-ac/Cargo.toml -p corelink-handler-ac --target x86_64-unknown-linux-gnu --edges normal,build` — READ_ONLY_RESOLUTION; NOT_EXECUTED.

### Evidência de fonte
- `crates/corelink-handler-ac/Cargo.toml` @ `1ba9f419d2bbaf4024e6b5dfbd0ea12da84bccb7`; source `37f96dba42cc159cd8ab8e9c4ab9d35b8e5634b5`.
- `crates/corelink-handler-ac/src/lib.rs` @ `3daa86bdb62b49a23c48c0ea75e3a0e5b4bf1f64`; source `37f96dba42cc159cd8ab8e9c4ab9d35b8e5634b5`.
...
- [ ] `.claude/skills/own-corelink-handler-ac/SKILL.md`.
- [ ] `docs/ownership/crates/corelink-handler-ac/REFERENCE.md`, único e indexado.
- [ ] `docs/ownership/crates/corelink-handler-ac/BLAST_RADIUS.md`, único, com todas as relações de fronteira conciliadas.
- [ ] `docs/ownership/crates/corelink-handler-ac/MAINTENANCE.md`, único, com procedimentos verificáveis.
