...
1 targets enumerados em `../census.json`, registro cujo `manifest` é `crates/corelink-runbook-tracker/Cargo.toml`. Abaixo estão apenas as entradas de implementação, não uma substituição do inventário completo.
