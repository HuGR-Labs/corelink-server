...
2 targets enumerados em `../census.json`, registro cujo `manifest` é `crates/corelink-turbo-bridge/Cargo.toml`. Abaixo estão apenas as entradas de implementação, não uma substituição do inventário completo.
