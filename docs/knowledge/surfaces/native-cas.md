...
5. Bulk uploads are split into a newline-framed manifest + concatenated payload at the first blank
   ...
   content-type is rejected 415 before parse (`crates/corelink-container/src/routes/cas.rs:538-545`).
...
- A write requires a `cas:rw` (write-capable) scope; a read-only token is rejected 403 (`crates/corelink-container/src/routes/cas.rs:741-743`).
...
6. `crates/corelink-container/src/routes/cas.rs:741-743` — the write-scope (`cas:rw`) gate.
...
9. `crates/corelink-container/src/routes/cas.rs:538-545` — `content_type_is` 415 gate for batch routes.
