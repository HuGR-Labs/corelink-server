...
2. A read is a digest-keyed lookup via `handle_read` (`crates/corelink-container/src/routes/cas.rs:752`).
3. A write rejects a path/auth tenant mismatch with 403, then validates the digest, then the write
   scope, then the native PAT possession gate, all before storage — `handle_write` (`crates/corelink-container/src/routes/cas.rs:855-886`).
...
5. Bulk uploads are split into a newline-framed manifest + concatenated payload at the first blank
   ...
   (`crates/corelink-container/src/routes/cas.rs:1000-1001`).
6. After the storage handler returns, the surface records a fire-and-forget usage-metering event into
   ...
   (`crates/corelink-container/src/routes/cas.rs:830-833`; `crates/corelink-container/src/routes/cas.rs:836-843`;
   `crates/corelink-container/src/routes/cas.rs:919-923`).

# Invariants
- A non-canonical `:hash` is rejected 400 BEFORE it derives an R2 key (`crates/corelink-container/src/routes/cas.rs:874-875`).
- A path `:tenant` that differs from the authenticated tenant is denied 403 BEFORE storage (`crates/corelink-container/src/routes/cas.rs:870-871`).
- A write requires a `cas:rw` (write-capable) scope; a read-only token is rejected 403 (`crates/corelink-container/src/routes/cas.rs:880-881`).
- A batch is capped at `BATCH_MAX_OBJECTS` objects and `BATCH_MAX_BYTES` of payload, over-cap → 413 (`crates/corelink-container/src/routes/cas.rs:121`; `crates/corelink-container/src/routes/cas.rs:127`; `crates/corelink-container/src/routes/cas.rs:1034`).
...
- Usage metering is fire-and-forget DISPLAY telemetry: the `record(...)` call is off the storage-decision path and never gates, bills, or fails a request (`crates/corelink-container/src/routes/cas.rs:830-833`).
...
2. `crates/corelink-container/src/routes/cas.rs:752` — `handle_read`, the digest-keyed read.
3. `crates/corelink-container/src/routes/cas.rs:855-886` — `handle_write`: cross-tenant 403, canonical-digest, scope, native PAT gate order.
4. `crates/corelink-container/src/routes/cas.rs:870-871` — the cross-tenant 403.
5. `crates/corelink-container/src/routes/cas.rs:874-875` — canonical-digest reject before storage.
6. `crates/corelink-container/src/routes/cas.rs:880-881` — the write-scope (`cas:rw`) gate.
6b. `crates/corelink-container/src/routes/cas.rs:883-886` — the native PAT possession gate (after scope, before storage).
...
9b. `crates/corelink-container/src/routes/cas.rs:1000-1001` — batch-handler call-site that returns 415 on wrong/absent content-type.
...
12. `crates/corelink-container/src/routes/cas.rs:1034` — over-cap 413 on bulk write.
13. `crates/corelink-container/src/routes/cas.rs:830-833` — read HIT usage-metering `record` (fire-and-forget display telemetry, no await/I/O).
14. `crates/corelink-container/src/routes/cas.rs:836-843` — read MISS usage-metering `record` (only on a genuine `NotFound`).
15. `crates/corelink-container/src/routes/cas.rs:919-923` — write usage-metering `record` (both fresh 201 and idempotent 200 count as a `Write`).
