...
The DO binding is declared with an explicit `script_name`, which is what makes it a cross-Worker reference rather than a local class: the class lives in `corelink-spawn-worker` and this Worker only holds a handle to it (`wrangler.toml:250-256`).
...
The cross-Worker binding means DevEnv availability depends on `corelink-spawn-worker` being deployed with the class; a `script_name` pointing at a Worker that does not export `RunnerDevEnvDO` fails at request time, not at deploy time. The mirrored prod binding is what keeps that failure from being prod-only — the omission that would otherwise ship green in dev and 503 in prod while the customer nav links straight at it.
...
8. `wrangler.toml:568-577` — the same binding mirrored into `env.prod`, because `durable_objects` is non-inheritable.
