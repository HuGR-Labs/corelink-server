...
9. **F3.2 flag-gated `_public` routing (WP-B write gate + WP-G M2 existence read).** The boot `dedup` flag
   ...
   **WP-G M3 activation:** both boot flags are now `"1"` in every prod env block —
   `OCI_PUBLIC_DEDUP_ENABLED` (`wrangler.toml:756`) and `OCI_UPSTREAM_ON_MISS` (`wrangler.toml:757`) —
...
30d. `wrangler.toml:756` (`OCI_PUBLIC_DEDUP_ENABLED = "1"`) + `wrangler.toml:757` (`OCI_UPSTREAM_ON_MISS = "1"`) — both keystone flags active in prod (all 5 prod env blocks); M3 activation.
