...
1. The router mounts lookup/update/delete on `AC_LOOKUP_ROUTE` plus the ref-list route, each carrying
   its SLO-emitting handler (`crates/corelink-container/src/routes/ac/part-00.rs:482-490`).
2. A lookup is served by `handle_lookup` (`crates/corelink-container/src/routes/ac/part-00-01.rs:21-74`); an update
   by `handle_update` (`crates/corelink-container/src/routes/ac/part-00-01.rs:120-138`).
3. The `:action_digest` segment is validated as exactly 64 lowercase-hex chars by `is_canonical_digest`
   BEFORE it derives an R2 key (`crates/corelink-container/src/routes/ac/part-00.rs:493-504`).
4. On the READ path (lookup / ref-list) the native PAT possession gate `pat_gate_reject` re-verifies the
   ...
   (`crates/corelink-container/src/routes/ac/part-00.rs:506-523`).
5. On the WRITE path (update / delete) the gate escalates to `pat_gate_reject_write`, which independently
   ...
   (`crates/corelink-container/src/routes/ac/part-00-01.rs:161-166` update / `crates/corelink-container/src/routes/ac/part-00-01.rs:264-266` delete; helper `pat_gate_reject_write` at `crates/corelink-container/src/routes/ac/part-00-01.rs:8-18`).
6. Delete and per-tenant ref enumeration are served by `handle_delete` and `handle_list_refs`
   (`crates/corelink-container/src/routes/ac/part-00-01.rs:230-244`; `crates/corelink-container/src/routes/ac/part-01.rs:4`).

# Invariants
- A non-canonical action digest (not 64 lowercase-hex) is rejected 400 before storage (`crates/corelink-container/src/routes/ac/part-00.rs:493-504`).
- Cross-tenant attempts are rejected **HTTP 403** by the route-level tenant check
  (`crates/corelink-container/src/routes/ac/part-00-01.rs:34-36`, `:34-36`, `crates/corelink-container/src/routes/ac/part-00-01.rs:136-138`, `crates/corelink-container/src/routes/ac/part-00-01.rs:238-244`), which returns
...
- A forged or wrong-tenant bearer PAT is rejected by the read-path possession gate before any storage access (`crates/corelink-container/src/routes/ac/part-00.rs:506-523`).
- An AC update/delete requires a WRITE-capable PAT re-derived at the container (`pat_gate_reject_write` → `NativePatGate::verify_write`), not just tenant possession — a read-only PAT is rejected 403 even if the Worker-set scope header claimed write (`crates/corelink-container/src/routes/ac/part-00-01.rs:161-166`, `crates/corelink-container/src/routes/ac/part-00-01.rs:264-266`).
- A **create-only (deny-overwrite)** runner-job cred may CREATE a new `(tenant, action_digest)` entry
  ...
  signal — a non-durable write is the conflict (`crates/corelink-container/src/routes/ac/part-00-01.rs:194-212`),
  and a divergent-body overwrite maps to the same conflict (`crates/corelink-container/src/routes/ac/part-00-01.rs:214-223`)
...
# Citations
1. `crates/corelink-container/src/routes/ac/part-00.rs:482-490` — the AC router (lookup/update/delete + ref list).
2. `crates/corelink-container/src/routes/ac/part-00-01.rs:21-74` — `handle_lookup`.
3. `crates/corelink-container/src/routes/ac/part-00-01.rs:120-138` — `handle_update`.
4. `crates/corelink-container/src/routes/ac/part-00.rs:493-504` — `is_canonical_digest` (64 lowercase-hex) gate.
5. `crates/corelink-container/src/routes/ac/part-00.rs:506-523` — `pat_gate_reject` native PAT possession gate (read path).
5b. `crates/corelink-container/src/routes/ac/part-00-01.rs:8-18` — `pat_gate_reject_write`: re-derives D1 `can_write` via `NativePatGate::verify_write` (line 506) on the update/delete write paths.
5c. `crates/corelink-container/src/routes/ac/part-00-01.rs:161-166` — `pat_gate_reject_write` call in `handle_update`.
5d. `crates/corelink-container/src/routes/ac/part-00-01.rs:264-266` — `pat_gate_reject_write` call in `handle_delete`.
6. `crates/corelink-container/src/routes/ac/part-00-01.rs:230-244` — `handle_delete`.
7. `crates/corelink-container/src/routes/ac/part-01.rs:4` — `handle_list_refs`.
8. `crates/corelink-container/src/routes/ac/part-00-01.rs:34-36` — cross-tenant 403 (lookup), route-level, before storage.
8b. `crates/corelink-container/src/routes/ac/part-00-01.rs:136-138` — cross-tenant 403 (update), route-level, before storage.
8c. `crates/corelink-container/src/routes/ac/part-00-01.rs:238-244` — cross-tenant 403 (delete), route-level, before storage.
