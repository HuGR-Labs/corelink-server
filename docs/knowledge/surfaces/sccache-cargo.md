...
source_files:
  ...
  - "crates/corelink-container/src/routes/cargo/part-00-01.rs"
  - "crates/corelink-container/src/d1_coread.rs"
source_blobs:
  - "crates/corelink-container/src/routes/cargo/part-00.rs@6dffca2ca6bb283f26eedb364f5de32c1345018a"
  - "crates/corelink-container/src/routes/cargo/part-00-01.rs@f11953e2aac5c590e677cec123efbd3143c252ad"
...
1. The router builds the per-tenant moat store and mounts the adapter via `nest_service("/cargo", …)`
   with the gate layered on (`crates/corelink-container/src/routes/cargo/part-00.rs:232-269`).
2. `nest_service` (not `nest`) is required so the adapter's `/:key` route is preserved rather than
   flattened, which would reject the 3-segment wire path (`crates/corelink-container/src/routes/cargo/part-00.rs:1`; `crates/corelink-container/src/routes/cargo/part-00.rs:269`).
3. The tenant is derived from the PAT via the shared `PatVerifier`, wrapped by `resolver_from_verifier`;
   the path `<tenant>` is never trusted for storage (`crates/corelink-container/src/routes/cargo/part-00.rs:188`; `crates/corelink-container/src/routes/cargo/part-00.rs:1`).
4. `cargo_gate` short-circuits a WebDAV `MKCOL` (opendal's parent-"directory" create issued before a
   ...
   cargo store is a FLAT content-addressed KV with implicit directories (`crates/corelink-container/src/routes/cargo/part-00-01.rs:24-229`).
5. `cargo_gate` short-circuits a WebDAV `PROPFIND` (opendal's stat) — gated on cache-read, it synthesizes
   ...
   collection path returns a minimal `207` (`crates/corelink-container/src/routes/cargo/part-00-01.rs:258-299`;
   `crates/corelink-container/src/routes/cargo/part-00-01.rs:258-299`; `crates/corelink-container/src/routes/cargo/part-00-01.rs:409-432`).
6. `cargo_gate` short-circuits a WebDAV `DELETE` (opendal's write-check cleanup) — gated on cache-write AND
   ...
   (`crates/corelink-container/src/routes/cargo/part-00-01.rs:319-381`; `crates/corelink-container/src/routes/cargo/part-00-01.rs:319-381`).
7. It then enforces per-operation scope for the adapter methods: PUT requires cache-write, GET/HEAD require
   cache-read, any other method fails closed 403 (`crates/corelink-container/src/routes/cargo/part-00-01.rs:24-229`).
8. For PUT, the F27 second layer requires the PAT's D1-verified `can_write` bit from the SAME single
   ...
   (`scope_ok`) and then `resolve_with_capability(...).can_write` (`crates/corelink-container/src/routes/cargo/part-00-01.rs:24-229`).
9. OUTSIDE the gate — added last, so it is the outermost layer and its scope covers the gate's own
   ...
   reading it (`crates/corelink-container/src/routes/cargo/part-00.rs:266-268`;
   `crates/corelink-container/src/routes/cargo/part-00.rs:300-321`).

# Invariants
- Tenant identity comes from the PAT, re-verified against D1; the path `<tenant>` is NEVER trusted for storage — for reads/PROPFIND and for PUT/DELETE alike (`crates/corelink-container/src/routes/cargo/part-00.rs:1`; `crates/corelink-container/src/routes/cargo/part-00-01.rs:319-381`).
- A write must pass BOTH the scope header AND the PAT-derived `can_write` bit (F27), closing the single-header-trust gap — enforced for PUT in `cargo_gate` and mirrored for the WebDAV `DELETE` (`crates/corelink-container/src/routes/cargo/part-00-01.rs:24-229`; `crates/corelink-container/src/routes/cargo/part-00-01.rs:319-381`).
- Per-operation scope is enforced before the adapter runs: PUT→write, GET/HEAD→read; PROPFIND is a read (cache-read), DELETE is a write (cache-write) (`crates/corelink-container/src/routes/cargo/part-00-01.rs:24-229`; `crates/corelink-container/src/routes/cargo/part-00-01.rs:258-299`; `crates/corelink-container/src/routes/cargo/part-00-01.rs:319-381`).
- The WebDAV control methods are handled in the gate, not routed to the adapter: `MKCOL`/`PROPFIND`/`DELETE` short-circuit `cargo_gate` because axum's `MethodRouter` cannot route them (they would 405) (`crates/corelink-container/src/routes/cargo/part-00-01.rs:24-229`; `crates/corelink-container/src/routes/cargo/part-00-01.rs:258-299`; `crates/corelink-container/src/routes/cargo/part-00-01.rs:319-381`).
- A PROPFIND on an existing key reports the stored blob's real byte length in `getcontentlength`; an absent key is `404` (opendal treats it as not-found and writes) (`crates/corelink-container/src/routes/cargo/part-00-01.rs:258-299`; `crates/corelink-container/src/routes/cargo/part-00-01.rs:409-432`).
- A DELETE removes only the url→content-hash map row (the CAS blob is left for GC, as it may be shared by dedup); it is idempotent — `204` even for an absent key (`crates/corelink-container/src/routes/cargo/part-00-01.rs:319-381`).
- An unmapped HTTP method is denied at the gate rather than assumed safe (fail-closed) (`crates/corelink-container/src/routes/cargo/part-00-01.rs:24-229`).
- The co-read hint is a FETCH-ORDER optimisation, never a trust source: its namespace is the Worker-set `x-corelink-tenant-id`, but the moat is still keyed by the PAT-derived tenant, and a prefetched row is served only under an exact `(namespace, url_hash)` match — a hint naming a different tenant is discarded unread and the storage read is re-issued (`crates/corelink-container/src/routes/cargo/part-00.rs:307-313`; `crates/corelink-container/src/d1_coread.rs:211-213`).
...
- The WebDAV handlers extract owned inputs (`path`, bearer) from the request BEFORE the first `.await`
  ...
  make `from_fn` reject the layer (`crates/corelink-container/src/routes/cargo/part-00-01.rs:258-299`).

# Citations
1. `crates/corelink-container/src/routes/cargo/part-00.rs:232-269` — the router: moat store + `nest_service` mount + gate layer.
2. `crates/corelink-container/src/routes/cargo/part-00.rs:269` — `nest_service("/cargo", …)` mount.
3. `crates/corelink-container/src/routes/cargo/part-00.rs:1` — why `nest_service` not `nest` (path-shape bridging).
4. `crates/corelink-container/src/routes/cargo/part-00.rs:188` — `resolver_from_verifier` (shared `PatVerifier`).
5. `crates/corelink-container/src/routes/cargo/part-00.rs:1` — PAT-derived tenant; path never trusted.
6. `crates/corelink-container/src/routes/cargo/part-00.rs:1` — tenant + scope trust model.
7. `crates/corelink-container/src/routes/cargo/part-00-01.rs:24-229` — `cargo_gate` per-operation scope mapping (PUT→write, GET/HEAD→read) + 403.
8. `crates/corelink-container/src/routes/cargo/part-00-01.rs:24-229` — fail-closed unmapped method (`_ => false`).
9. `crates/corelink-container/src/routes/cargo/part-00-01.rs:24-229` — `cargo_gate` F27 two-layer write enforcement (scope header + `resolve_with_capability` `can_write`), executed.
10. `crates/corelink-container/src/routes/cargo/part-00-01.rs:24-229` — `cargo_gate` WebDAV `MKCOL` success no-op (gated on cache-write).
11. `crates/corelink-container/src/routes/cargo/part-00-01.rs:258-299` — `cargo_gate` WebDAV `PROPFIND` short-circuit (cache-read gated), the opendal stat.
12. `crates/corelink-container/src/routes/cargo/part-00-01.rs:258-299` — `handle_propfind`: moat lookup → `207` (present) / `404` (absent) / collection.
13. `crates/corelink-container/src/routes/cargo/part-00-01.rs:409-432` — `PROPFIND_LAST_MODIFIED` + `propfind_file_response`: the `207 Multi-Status` XML with `getcontentlength` **and `getlastmodified`** (opendal's WebDAV stat deserializer treats `getlastmodified` as REQUIRED — a 207 without it fails `missing field getlastmodified` → sccache flags storage ReadOnly and never writes; a stable epoch httpdate is used since CAS objects are immutable).
14. `crates/corelink-container/src/routes/cargo/part-00-01.rs:319-381` — `cargo_gate` WebDAV `DELETE` short-circuit (cache-write gated).
15a. `crates/corelink-container/src/routes/cargo/part-00.rs:266-268` — the co-read hint layer added LAST (outermost), so its scope covers `cargo_gate`'s own PROPFIND handling.
15b. `crates/corelink-container/src/routes/cargo/part-00.rs:300-321` — `cargo_coread_hint`: READ verbs only; hint = trimmed `x-corelink-tenant-id` + `key_from_path` (the SAME normalization the moat will ask with).
...
15. `crates/corelink-container/src/routes/cargo/part-00-01.rs:319-381` — `handle_delete`: F27 `can_write` + PAT-resolved tenant → moat map-row removal → `204`.
