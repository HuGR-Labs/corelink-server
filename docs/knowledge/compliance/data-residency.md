...
- `crates/corelink-container/src/storage/region_map.rs:71-80` — `colo_for_macro`: the frozen MACRO→colo map (single source of truth), `None`/reject for `afr` + unrecognised.
- `crates/corelink-container/src/storage/region_map.rs:43-47`, `:88-92` — `PROVISIONED_MACROS` = `{wnam, enam, weur}` + `is_provisioned_macro` (sam/apac/afr routable-or-rejectable but NOT provisioned).
