...
- The provisioned macro set is EXACTLY `{wnam, enam, weur}` — the macros backed by a jurisdiction-correct R2 bucket — and signup rejects the rest; `sam`/`apac`/`afr` are valid, D1-accepted macro codes but NOT provisioned, so the single-bucket launch posture (ADR-S14-009) still holds for them (`crates/corelink-container/src/storage/region_map.rs:43-47`). `sam` in particular stays ROUTABLE (`colo_for_macro` still maps it) yet is deliberately NOT provisionable: PROD_SAM still points at the default US bucket, so a `sam`-labelled tenant would mis-land in US storage — an LGPD violation — until a real SAM-jurisdiction bucket exists (`crates/corelink-container/src/storage/region_map.rs:73-77`).
...
- `crates/corelink-container/src/storage/region_map.rs:56-65` — `colo_for_macro`: the frozen MACRO→colo map (single source of truth), `None`/reject for `afr` + unrecognised.
- `crates/corelink-container/src/storage/region_map.rs:43-47`, `:73-77` — `PROVISIONED_MACROS` = `{wnam, enam, weur}` + `is_provisioned_macro` (sam/apac/afr routable-or-rejectable but NOT provisioned).
