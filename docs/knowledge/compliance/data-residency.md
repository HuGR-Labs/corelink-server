...
- The **AC** path is genuinely per-region by construction (`corelink-ac-<region>` buckets). For native **CAS**, the env-selectable bucket path EXISTS: the prod-lhr env is *configured* to point `R2_CAS_BUCKET` at `corelink-cas-eu` (`jurisdiction = "eu"`, EU S3 endpoint) per the secreview (`docs/security/2026-06-23-secreview-gdpr-residency.md:99-108`) — but ADR-S14-009, the authoritative launch posture, records that a full *tenant-routed* per-region CAS topology is owner-gated (operator R2 provisioning, not code) and the default single bucket carries region only as a logical key tag; so native-CAS EU *physical* residency is NOT yet a blanket assurance and the do-not-onboard-EU-native-CAS guardrail stands (`specs/03_architecture/adrs/ADR-S14-009-cas-residency-single-bucket-launch-posture.md:40-51`, `:55-57`, `:75-80`).
