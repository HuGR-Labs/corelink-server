...
- **Self-serve account-delete uses the same DSR message + anchor.** A self-serve account delete routes through `routes/customer.rs::handle_account_delete` (`crates/corelink-container/src/routes/customer.rs:940`), which builds a `dsr.queued.v1` message and `INSERT OR IGNORE`s a `dsr_requested` anchor row, then hands erasure to the `DsrErasureSink` trait — production **enqueues to a queue (async)**, not a synchronous same-worker drive (the only in-tree sink impl is a test `MockSink`).
