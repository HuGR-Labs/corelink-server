...
- **Receipt + SLA.** On accept, a JWT (RS256) receipt is issued with a 90-day anti-replay expiry cap and an SLA deadline derived per jurisdiction (LGPD 15d / GDPR 30d / CCPA 45d) (`crates/corelink-dsr/src/receipt.rs:136-149`, `crates/corelink-dsr/src/lib.rs:116-119`).
...
- **Live erasure intake (container).** `POST /_internal/dsr/erase` deserialises the `dsr.queued.v1` message (forwarded by the signup-worker queue consumer for the Clerk `user.deleted` path), maps it to a canonical `ErasureRequest`, and drives the 12-backend orchestrator (`crates/corelink-container/src/routes/dsr.rs:429-470`).
- **Both delete paths converge on ONE engine.** A self-serve account-delete routes through `InProcessErasureSink`, which parses the same `dsr.queued.v1` shape and drives the same D1-backed worker synchronously — no second erasure implementation (`crates/corelink-container/src/routes/dsr.rs:278-309`).
...
- **24h verify sweep + attestation.** `POST /_internal/dsr/verify` re-fingerprints every backend; on a fully `VerifiedComplete` decision it signs and persists an Ed25519 erasure attestation (best-effort, fail-OPEN since the erasure is already done and audited) (`crates/corelink-container/src/routes/dsr.rs:498-507`).
...
- **MFA mandatory for Erasure + Rectification, never for read/policy arms.** The gate is structural in the orchestrator pipeline (`crates/corelink-dsr/src/endpoint.rs:354-379`); the verifier rejects `None`/empty/expired tokens (`crates/corelink-dsr/src/mfa.rs:146-154`).
...
- **Cannot attest "complete" while data survives.** The attestation is signed only on the `VerifiedComplete` arm, which the orchestrator emits iff every backend's verification hash is the canonical-empty sentinel — surviving CAS/AC bytes yield a non-canonical hash → `VerifiedPartial` → no signature (`crates/corelink-container/src/routes/dsr.rs:503-506`, `docs/security/2026-06-23-secreview-gdpr-residency.md:75-85`).
- **A tombstoned hash never resurrects.** A read of a tombstoned digest returns 410, a re-PUT is refused, and a gate transport fault fails CLOSED 503 (`docs/security/2026-06-23-secreview-gdpr-residency.md:53-64`); the bloom is re-seeded from D1 on every reload to close the cross-writer false-negative window (`crates/corelink-container/src/routes/cas_erase.rs:797-825`).
...
- Receipt 90d cap + SLA per jurisdiction: `crates/corelink-dsr/src/receipt.rs:136-149`, `crates/corelink-dsr/src/lib.rs:116-119`.
- CAS per-hash erase module + tombstone: `crates/corelink-container/src/routes/cas_erase.rs:1-8`, `crates/corelink-container/src/routes/cas_erase.rs:70`, `crates/corelink-container/src/routes/cas_erase.rs:830-850`.
