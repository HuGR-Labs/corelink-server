...
- **24h verify sweep + attestation.** `POST /_internal/dsr/verify` re-fingerprints every backend via `verify_erasure`; on a fully `VerifiedComplete` decision it signs and persists an Ed25519 erasure attestation (best-effort, fail-OPEN since the erasure is already done and audited) (`crates/corelink-container/src/routes/dsr.rs:498-507`; the `VerifiedComplete` sign gate `crates/corelink-container/src/routes/dsr.rs:503`).
...
- **Cannot attest "complete" while data survives — holds for CAS/AC, but NOT the Stripe arm.** The attestation is signed only on the `VerifiedComplete` arm (`crates/corelink-container/src/routes/dsr.rs:503`), which the orchestrator emits iff every backend's verification hash is the canonical-empty sentinel — surviving CAS/AC bytes yield a non-canonical re-fingerprint → `VerifiedPartial` → no signature. The canonical-empty/sentinel determination itself is CROSS-CRATE in `corelink-privacy-erasure-worker`'s orchestrator, not this route (`docs/security/2026-06-23-secreview-gdpr-residency.md:75-85`). **Caveat (currently NOT enforced for Stripe):** the Stripe backend's `verification_hash` does not re-query Stripe — it is a hardcoded `Ok(CANONICAL_EMPTY_TENANT_HASH)` (`crates/corelink-container/src/routes/dsr/adapter_stripe.rs:140-150`). So the verify sweep contributes the empty sentinel for Stripe regardless of whether the pseudonymisation actually completed or any customer record survives; the invariant is real for the byte planes but **vacuous for the Stripe arm** — a deeper per-customer `metadata.pii_redacted` sweep is deferred (ADR-S11-013).
...
- 24h verify sweep → best-effort/fail-OPEN sign+persist: `crates/corelink-container/src/routes/dsr.rs:498-507`; the `VerifiedComplete` sign gate: `crates/corelink-container/src/routes/dsr.rs:503`.
