...
- **Cannot attest "complete" while data survives.** The attestation is signed only on the `VerifiedComplete` arm (`crates/corelink-container/src/routes/dsr.rs:503`), which the orchestrator emits iff every backend's verification hash is the canonical-empty sentinel — surviving CAS/AC bytes yield a non-canonical hash → `VerifiedPartial` → no signature. The canonical-empty/sentinel determination itself is CROSS-CRATE in `corelink-privacy-erasure-worker`'s orchestrator, not this route (`docs/security/2026-06-23-secreview-gdpr-residency.md:75-85`).
