...
- **D1 control-plane erase (`BackendKind::D1`).** `D1EraseAdapter::erase` hard-deletes a tenant's subject PII in ordered groups — `tenant_id`-keyed child tables, then `namespace`-keyed registry tables (bound value is the tenant UUID, never the shared `'_public'`), then `signup_attempts` BEFORE `signup_orchestration`, then the root `tenant` row LAST so no orphan points at a deleted tenant; under `legal_hold` it preserves and returns `NotApplicable` (`crates/corelink-container/src/routes/dsr/adapter_d1.rs:194-227`).
- **D1 erase-set vs retain-set is an explicit const partition.** The erase-set is two compile-time constant table lists (`TENANT_ID_TABLES` / `NAMESPACE_TABLES`, injection-safe; only the tenant id is a bound `?1`), and the retain-set (fiscal `stripe_*`, the erasure log/attestation, audit WORM) is guarded against accidental erase-set membership (`crates/corelink-container/src/routes/dsr/adapter_d1.rs:54-103`).
...
- **24h verify sweep + evidence gate (signed attestation DEFERRED).** `POST /_internal/dsr/verify` re-fingerprints every backend; on a fully `VerifiedComplete` decision it invokes `attestation::sign_and_persist` (`crates/corelink-container/src/routes/dsr.rs:503-514`). STATUS: that function signs NOTHING and persists NO row — it is a fail-CLOSED, log-only evidence gate (see the next bullet). The signed + served Ed25519 erasure attestation is DEFERRED (brutal-review H1).
- **The verify-sweep attestation step is a fail-CLOSED evidence gate (signing DEFERRED).** `sign_and_persist` runs only on the `VerifiedComplete` arm and re-derives `verified_evidence_segments` from the real per-backend completions before doing anything; on incomplete/unverified evidence it logs and returns without writing a row, and even on full evidence it deliberately refuses to persist an UNSIGNED row that would masquerade as a cryptographic certificate — the signed + served attestation is DEFERRED (brutal-review H1) (`crates/corelink-container/src/routes/dsr/attestation.rs:131-167`).
...
- There is NO served erasure attestation today. On `VerifiedComplete` the verify sweep runs a fail-CLOSED, log-only evidence gate (`attestation::sign_and_persist`) that signs nothing and persists no row (`crates/corelink-container/src/routes/dsr/attestation.rs:131-167`); the signed + served Ed25519 attestation is DEFERRED (brutal-review H1; see the sibling `compliance/erasure-attestation` concept). A missing attestation does NOT mean the erasure failed — the erasure is already complete + audited + ledgered — but do not present the gate as if it emitted a cryptographic proof.
...
- Verify-sweep `sign_and_persist` fail-CLOSED evidence gate (refuses an unsigned attestation row; signing DEFERRED, brutal-review H1): `crates/corelink-container/src/routes/dsr/attestation.rs:131-167`.
