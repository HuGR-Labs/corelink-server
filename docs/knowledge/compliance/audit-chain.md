...
- `HashChainBuilder::append` is the pure-logic enforcer of the link — it rejects a wrong `sequence_number` and rejects an event whose `prev_hash` does not match the current chain head before advancing `crates/corelink-audit-chain/src/chain.rs:285-303`. **But no live producer calls it:** `append` is exercised only by this crate's own tests (`crates/corelink-audit-chain/src/chain.rs:455-496`); the deployed erasure/audit path does NOT seal events through it (see "How it works" below and the Gotcha), so "enforces the link at write time" describes the builder's contract, not the running container's write path.
- **What the live container actually writes (unsealed today).** The only deployed audit producer — the DSR erasure sink — appends plain, **UNCHAINED** CloudEvents to the D1 `audit_outbox` table with `emitted_at = NULL` and NO `prev_hash`/`sequence_number` linkage; the comment is explicit that "the S-09 drain worker later flips `emitted_at` when the event reaches the immutable chain" (`crates/corelink-container/src/routes/dsr/audit.rs:7-8`, the `INSERT OR IGNORE INTO audit_outbox` at `crates/corelink-container/src/routes/dsr/audit.rs:82-84`). That S-09 chain-sealing drain is the unbuilt WI-S09-007 piece, so **the live audit trail is currently a set of mutable, unsealed D1 rows** — the BLAKE3 chain logic above is real and property-tested, but nothing wires it to a live producer yet. Do not read "tamper-evident at write time" as a property of today's deployed trail.
