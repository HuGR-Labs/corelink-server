...
description: "CoreLink's tamper-EVIDENT (detect-at-verify) per-tenant BLAKE3 hash-chained audit log — sealed by the live hourly audit-drain into D1 (un-keyed hash + R2 Object-Lock unwired ⇒ evidence, not proof-against-insider) — plus its post-hoc sigstore/Rekor public-witness submission seam."
source_files:
...
**Honest scope of the guarantee (do NOT oversell it).** The chain is tamper-EVIDENCE at verify time, NOT tamper-PROOF against an insider with D1 write. Three things bound it: (1) the link hash is **UN-KEYED** BLAKE3 (`Hasher::new()`, no `keyed_hash`/HMAC — `crates/corelink-audit-chain/src/chain.rs:156-162`, `:183-193`), so anyone who holds the rows can recompute a fully self-consistent chain INCLUDING a forged head — there is no secret an insider lacks; (2) rows live PLAIN and mutable in `audit_outbox` for up to the ≤1h pre-seal window (`emitted_at IS NULL`) before they are chained, so a D1 writer can alter/delete an unsealed row with no trace at all (`crates/corelink-container/src/routes/audit_drain.rs:5-9`); (3) the storage-level immutability that would stop an insider rewriting the sealed rows + a fresh self-consistent chain — R2 Object-Lock — is **UNWIRED** (see Gotchas). So "trust even if CoreLink itself is compromised" is NOT the current property: a relying party who holds an INDEPENDENT prior copy of the head (or the Rekor witness, when wired) can detect divergence; a party with only what CoreLink hands them cannot distinguish an honest chain from an insider-recomputed one. This control supports the SOC 2 CC7.2 audit-chain discipline as detect-at-verify EVIDENCE; the storage-level 7-year append-only / keyed-or-signed-head / write-time chaining pieces are DEFERRED, not live.
...
- **The live producer is the drain, NOT `HashChainBuilder::append`.** `HashChainBuilder::append` is the write-time link primitive but is NOT how the live trail is sealed: the wired producer is the hourly `POST /_internal/audit/drain` `crates/corelink-container/src/routes/audit_drain.rs:208-235`, mounted in `crates/corelink-container/src/main.rs:650-654`. Rows are written PLAIN and UNCHAINED to `audit_outbox` first (`emitted_at IS NULL`), then sealed LATER at drain: the drain reads pending rows in deterministic order, JCS-canonicalizes each payload, links it via `link_chain_hash_from_canonical` (the verifier-path primitive), and writes the sealed columns + `audit_chain_head` checkpoint to D1 under a compare-and-set single-writer guard `crates/corelink-container/src/routes/audit_drain.rs:457-512`. So there is a ≤1h pre-seal window in which a row is mutable with no chain protection at all `crates/corelink-container/src/routes/audit_drain.rs:5-9`.
...
- INV-AUDIT-APPEND-ONLY: the LIVE property is tamper-EVIDENCE — a retroactive edit/delete of a SEALED row is DETECTABLE at verify time via the per-tenant chain, for a verifier holding an independent prior head. The crate doc-comment still frames this invariant as "R2 Object Lock Governance Mode 7y enforces append-only at storage level" `crates/corelink-audit-chain/src/lib.rs:89-92`, but that storage-level enforcement is NOT what runs: the live seal writes to D1 (`audit_outbox` / `audit_chain_head`), NOT R2, and the R2 Object-Lock binding is UNWIRED (Gotchas, and consistent with the lib doc-comment's own designed-vs-wired caveat). So the three pieces that would make this true append-only — (a) R2/Object-Lock storage immutability, (b) a KEYED or signed chain head an insider cannot forge, (c) write-time chaining that closes the ≤1h pre-seal mutable window — are all DEFERRED. The invariant as written OVERSTATES the live system; treat it as detect-at-verify evidence until those land.
...
- **R2 Object-Lock is UNWIRED — the seal lands in D1, which is mutable.** The `corelink-audit-chain` crate ships the pure link/verify logic (in-memory fakes); the live producer (the drain) seals into D1 (`audit_outbox` + `audit_chain_head`), and the real CF R2 PutObject with Object Lock, the scheduled DO verifier cron (UTC 02:00), and SIEM fan-out are deferred to the WI-S09-007 ship gate. Do NOT assume storage-level immutability exists just because the chain logic does — an insider with D1 write can rewrite a sealed row AND, because the hash is un-keyed, recompute a self-consistent chain + head over the rewritten rows, defeating an unwitting verifier. The chain only catches this for a verifier comparing against an INDEPENDENT prior head (a pinned head, or — when wired — the Rekor witness).
...
- `crates/corelink-container/src/routes/audit_drain.rs:38-48` — why the drain uses `link_chain_hash_from_canonical` (raw CloudEvents payloads), NOT `HashChainBuilder::append`.
- `crates/corelink-container/src/routes/audit_drain.rs:208-235` — `seal_rows` = `BLAKE3(prev || JCS(payload))` over the persisted bytes (the live seal).
- `crates/corelink-container/src/routes/audit_drain.rs:457-512` — `drain_partition`: seal to D1 + compare-and-set advance of `audit_chain_head` (single-writer, no R2).
