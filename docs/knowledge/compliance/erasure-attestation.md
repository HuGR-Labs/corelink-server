...
**What the live container actually signs (be honest about the proof's strength).** The deployed signer is `routes/dsr/attestation.rs::sign_and_persist`, invoked best-effort on a `VerifiedComplete` 24h sweep. It does NOT bind real KMS-destroy evidence: it constructs a **synthetic, self-referential** `EvidenceBundle` whose `audit_chain_segment_ids` is the single literal `dsr.erasure.completed.v1:{dsr_id}`, with `kms_key_id = "dsr:{dsr_id}"` and the verify instant as `kms_destroy_ts` (`crates/corelink-container/src/routes/dsr/attestation.rs:139-151`). It then hashes that bundle with `EvidenceBundle::compute_hash`, **NOT** the fail-closed `validated_hash` — and the bundle is a never-empty constant, so the "validation fails closed on any empty mandatory field" invariant below is **vacuous on the live path**. The signature therefore proves only that *a signature was produced over a self-describing record of the DSR id* — it does NOT independently prove that verifiable deletion evidence (a real KMS destroy / audit-chain segment) exists. Additionally, `resolve_region` (`crates/corelink-container/src/routes/dsr/attestation.rs:82-102`) normally **misses**: at `VerifiedComplete` the tenant row is already deleted, so the D1 `primary_region` lookup returns nothing and the region falls back to the `ERASURE_ATTESTATION_REGION` env default (the deployment's home region) — i.e. an EU tenant's erasure can be signed attributed to the env-default region, not the tenant's actual region.
...
- An attestation cannot be produced over an incomplete evidence bundle — the crate's `validated_hash` fails closed on any empty mandatory field `crates/corelink-erasure-attestation/src/evidence.rs:68-85`. **But this guard is NOT exercised by the live signer:** the container path calls `EvidenceBundle::compute_hash` over a never-empty synthetic bundle, not `validated_hash` (see Scope above) `crates/corelink-container/src/routes/dsr/attestation.rs:139-151` — so on the running system the fail-closed property is vacuous and the attestation proves a signature, not verified deletion evidence.
