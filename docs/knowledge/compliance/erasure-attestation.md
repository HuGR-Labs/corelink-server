...
**Scope / wiring status (lead-confirmed):** this crate is **pure-logic Ed25519 signing + offline verification only**. The surrounding pieces — the actual KMS CMK-destroy, the R2 (7-year) persistence of the signature, and the public `/v1/public/keys/erasure/{region}.pub` serving endpoint — are the **WI-S11-008 wiring and are NOT yet live** (the D1 index IS live — the deployed `sign_and_persist` writes the `erasure_public_keys` + `erasure_attestations` index rows on the `VerifiedComplete` path, `crates/corelink-container/src/routes/dsr/attestation.rs:166-212`); per the sibling `launch/go-live-readiness` NO-GO audit the attestation signature "is never persisted and has no serving endpoint" today (a non-functional stub), while erasure itself still completes fail-open. Read the present-tense "persisted to R2 / served at the endpoint" statements below as the designed contract, not the running system. The sibling control `compliance/dsr-erasure` covers the request orchestration that would drive this attestation.

**What the live container actually signs (the proof's real strength, accurately scoped).** The deployed signer is `routes/dsr/attestation.rs::sign_and_persist`, invoked best-effort on a `VerifiedComplete` 24h sweep. This is NOT attestation theater: the signature is a real Ed25519/JCS offline-verifiable artifact, and it is GATED on a fail-CLOSED cross-backend erase — the `VerifiedComplete` decision is only reached when EVERY backend's original erase succeeded AND re-fingerprints canonical-empty (the DUAL `hash != canonical || !prior.outcome.is_successful()` gate in the orchestrator; see the sibling `compliance/dsr-erasure`), so surviving subject bytes BLOCK the signature. The signer constructs a **synthetic, self-referential** `EvidenceBundle` whose `audit_chain_segment_ids` is the single literal `dsr.erasure.completed.v1:{dsr_id}`, with `kms_key_id = "dsr:{dsr_id}"` and the verify instant as `kms_destroy_ts` (`crates/corelink-container/src/routes/dsr/attestation.rs:139-151`). It hashes that bundle with `EvidenceBundle::compute_hash` rather than `validated_hash` — but this is an EQUIVALENCE, not a bypass: the synthetic bundle hardcodes all three of the exact mandatory fields `validated_hash` checks (`audit_chain_segment_ids`, `kms_key_id`, `tenant_id`) non-empty by construction (`crates/corelink-erasure-attestation/src/evidence.rs:68-85`), so `compute_hash ≡ validated_hash` on this path — the fail-closed guard is not *exercised* (the bundle can never be empty), but it is not *violated* either. The honest, V3-confirmed residual is narrower: the audit-chain segment id is synthetic and there is **no live producer** that emits/persists a `dsr.erasure.completed.v1` segment, so the attestation binds a *self-describing record of the DSR id* rather than an *independently-retrievable* audit-chain segment — a deepening (wire a real audit-chain producer), not a forge path. The `resolve_region` fallback is **correct by design**, not a defect: at `VerifiedComplete` the tenant row is already deleted (the erase deleted it), so the D1 `primary_region` lookup misses and the region falls back to the published `ERASURE_ATTESTATION_REGION` default (`crates/corelink-container/src/routes/dsr/attestation.rs:82-102`); the deployment is US-only, so the env default IS the tenant's region and the published per-region public key verifies the signature.
...
- An attestation cannot be produced over an incomplete evidence bundle — the crate's `validated_hash` fails closed on any empty mandatory field `crates/corelink-erasure-attestation/src/evidence.rs:68-85`. The live signer calls `EvidenceBundle::compute_hash` rather than `validated_hash` `crates/corelink-container/src/routes/dsr/attestation.rs:139-151`, but the synthetic bundle hardcodes all three checked fields (`audit_chain_segment_ids`, `kms_key_id`, `tenant_id`) non-empty, so `compute_hash ≡ validated_hash` here — an EQUIVALENCE, not a bypass: the guard is not exercised (the bundle can never be empty) but it is not violated. The real residual is upstream of the hash: the audit-chain segment id is synthetic and has no live producer, so the signature attests a self-describing DSR-id record, not an independently-retrievable deletion-evidence segment — AND the signing is gated on the fail-CLOSED cross-backend erase (sibling `compliance/dsr-erasure`), so a signature is never produced while subject bytes survive.
