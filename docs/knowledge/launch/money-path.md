...
- `crates/corelink-container/src/main.rs:728-870` — the SECOND live activation surface: the container's `STRIPE_WEBHOOK_SECRET`-gated Stripe webhook mount → durable `D1HttpBillingWriter` (`:764-802`) + `D1SubscriptionStateHandler` (`:820-822`, `build_tier_selector` resolves the tier off `STRIPE_PRICE_ID_*` at `:804-820`) materializing `subscription_state='active'`+tier to `tier_selections` over D1-HTTP.
- `worker/src/index.ts:764-774` — the worker routes `/v1/billing/stripe-webhook` to the `_system` DO → container as a pure pass-through; the container is the SOLE signature-verifier / routed authority.
