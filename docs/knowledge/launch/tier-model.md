...
checkpoint_sha: "473c32747f08bfa911d421aab2ff2edd23d90bb9"
provenance: "AUTHORED"
...
- Runner is a SEPARATE entitlement axis and MUST NOT persist to the cache tables: for a runner tier the orchestration skips `persist_pending_checkout` entirely (`!tier.is_runner()` guard) — a runner write would clobber the one-row-per-tenant cache `tier_selections` row and violate its cache-only `tier` CHECK. The runner entitlement is instead written by the signup-worker webhook into `runner_billing`/`runners_entitlement` (`crates/corelink-container/src/routes/tier_select.rs:926`; the exhaustive-but-runtime-unreachable runner arms of `tier_column` are documented at `crates/corelink-container/src/routes/tier_select_store.rs:56-83`).
- The per-tenant single-active-subscription guard is per-axis: a cache subscription never blocks a runner purchase and a runner subscription never blocks a cache purchase; only a second subscription of the SAME axis is refused `AlreadyActive` (`crates/corelink-container/src/routes/tier_select.rs:875`).
- The route is only mounted when the internal-auth secret, the Stripe config, AND `CORELINK_DPA_VERSION` are present; a missing DPA version means the endpoint is NOT mounted — the actual `CORELINK_DPA_VERSION` unset-⇒-`None` guard is `crates/corelink-container/src/routes/tier_select.rs:609-615` (inside `build_state_from_env`, whose header is at `:602`).
- Both money-path routes resolve their internal-auth secret through the SHARED helper rather than reading the shared env var raw, so they carry the same 32-char `INTERNAL_AUTH_KEY_MIN_LEN` floor as every other internal surface and each has a rotation path to its own credential: `resolve_internal_auth_key("CORELINK_TIER_SELECT_AUTH_KEY")` at `crates/corelink-container/src/routes/tier_select.rs:603` and `resolve_internal_auth_key("CORELINK_DPA_ACCEPT_AUTH_KEY")` at `crates/corelink-container/src/routes/dpa_accept.rs:593`. Both fail CLOSED — no key at or above the floor means the route is NOT mounted (404), never mounted with a weak gate.
- The DPA gate is fail-CLOSED: a transport error never reads as "accepted" `crates/corelink-container/src/routes/tier_select_store.rs:202-213`.
- Audit emit returning `Err` ABORTS the whole operation before mutation (audit-before-mutation) — the abort logic lives at `crates/corelink-container/src/routes/tier_select.rs:797-799`, and the production adapter (`crates/corelink-container/src/routes/tier_select_audit.rs:110`) now returns `Err` on a durable-D1 INSERT failure, so the abort is REAL (no longer vacuous).
...
3. `crates/corelink-container/src/routes/tier_select.rs:778` — `orchestrate_tier_select` fixed ordering (per-axis active-sub guard + runner-skips-persist).
3b. `crates/corelink-container/src/routes/tier_select.rs:875` — per-axis single-active-subscription guard (`is_runner` selects `has_active_runner_subscription` vs `has_active_subscription`).
3c. `crates/corelink-container/src/routes/tier_select.rs:926` — runner checkout SKIPS the cache `persist_pending_checkout` (`!tier.is_runner()` guard).
...
5. `crates/corelink-container/src/routes/tier_select.rs:859` — DPA-not-accepted → `return Err(TierSelectHttpError::DpaRequired)` (the enforcement, gated by the fail-closed store check).
5b. `crates/corelink-container/src/routes/tier_select.rs:334` — `parts()` status-mapping arm: `DpaRequired` → 403 `dpa_required`.
6. `crates/corelink-container/src/routes/tier_select.rs:609-615` — the `CORELINK_DPA_VERSION` unset-⇒-NOT-mounted guard (in `build_state_from_env`, header `:590`).
7. `crates/corelink-container/src/routes/tier_select_store.rs:202-213` — the fail-CLOSED DPA-first D1 check.
...
9b. `crates/corelink-container/src/routes/tier_select.rs:797-799` — emit-Err-aborts-before-mutation orchestration (now REAL: the adapter returns `Err` on durable-D1 failure).
...
15. `crates/corelink-container/src/routes/dpa_accept.rs:515` — `handle`: the `POST /v1/onboarding/dpa-accept` writer entrypoint (fail-CLOSED auth gate → `orchestrate_dpa_accept` at `:384`; mount fail-CLOSES on the RS256 key at `:608-611`).
...
17. `crates/corelink-container/src/routes/tier_select.rs:603` — the tier-select mount gate resolves the internal-auth secret via `resolve_internal_auth_key("CORELINK_TIER_SELECT_AUTH_KEY")`, inheriting the shared 32-char floor and a dedicated-credential rotation path (fail-CLOSED ⇒ route not mounted).
18. `crates/corelink-container/src/routes/dpa_accept.rs:593` — the same helper-resolved gate for the DPA consent-receipt route (`CORELINK_DPA_ACCEPT_AUTH_KEY`).
