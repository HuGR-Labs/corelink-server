...
checkpoint_sha: "0ab7a03392afa95fcf5ad20dd1905da29ff87019"
provenance: "AUTHORED"
...
- `orchestrate_tier_select` fixes the ordering audit → lock → DPA-first → active-sub → checkout → persist → release `crates/corelink-container/src/routes/tier_select.rs:777`.
...
- Audit emit returning `Err` ABORTS the whole operation before mutation (audit-before-mutation) — the abort logic lives in `crates/corelink-container/src/routes/tier_select.rs:794-798`, vacuous today since the `tracing`-only adapter (`crates/corelink-container/src/routes/tier_select_audit.rs:75`) never returns `Err`.
...
3. `crates/corelink-container/src/routes/tier_select.rs:777` — `orchestrate_tier_select` fixed ordering (per-axis active-sub guard + runner-skips-persist).
3b. `crates/corelink-container/src/routes/tier_select.rs:865-870` — per-axis single-active-subscription guard (`is_runner` selects `has_active_runner_subscription` vs `has_active_subscription`).
3c. `crates/corelink-container/src/routes/tier_select.rs:917-921` — runner checkout SKIPS the cache `persist_pending_checkout` (`!tier.is_runner()` guard).
...
5. `crates/corelink-container/src/routes/tier_select.rs:858` — DPA-not-accepted → `return Err(TierSelectHttpError::DpaRequired)` (the enforcement, gated by the fail-closed store check).
...
9b. `crates/corelink-container/src/routes/tier_select.rs:794-798` — emit-Err-aborts-before-mutation orchestration (vacuous today).
