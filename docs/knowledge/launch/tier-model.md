...
checkpoint_sha: "2fc73db0c95c8a8e36dc9e4e4daee997dc616b34"
provenance: "AUTHORED"
...
- `handle` is the axum entrypoint that drives the orchestration with the route state and verified tenant header `crates/corelink-container/src/routes/tier_select.rs:516`.
- `orchestrate_tier_select` fixes the ordering audit → lock → DPA-first → active-sub → checkout → persist → release `crates/corelink-container/src/routes/tier_select.rs:767`.
...
- Audit emit returning `Err` ABORTS the whole operation before mutation (audit-before-mutation) — the abort logic lives in `crates/corelink-container/src/routes/tier_select.rs:784-788`, vacuous today since the `tracing`-only adapter (`crates/corelink-container/src/routes/tier_select_audit.rs:75`) never returns `Err`.
...
2. `crates/corelink-container/src/routes/tier_select.rs:516` — the `handle` axum entrypoint.
3. `crates/corelink-container/src/routes/tier_select.rs:767` — `orchestrate_tier_select` fixed ordering (per-axis active-sub guard + runner-skips-persist).
3b. `crates/corelink-container/src/routes/tier_select.rs:855-860` — per-axis single-active-subscription guard (`is_runner` selects `has_active_runner_subscription` vs `has_active_subscription`).
3c. `crates/corelink-container/src/routes/tier_select.rs:905-909` — runner checkout SKIPS the cache `persist_pending_checkout` (`!tier.is_runner()` guard).
4. `crates/corelink-container/src/routes/tier_select.rs:477` — Enterprise direct-paid → `return Err(...UseInquiryForm)` (the enforcement).
4b. `crates/corelink-container/src/routes/tier_select.rs:329` — `parts()` status-mapping arm: `UseInquiryForm` → 422 `use_inquiry_form`.
5. `crates/corelink-container/src/routes/tier_select.rs:848` — DPA-not-accepted → `return Err(TierSelectHttpError::DpaRequired)` (the enforcement, gated by the fail-closed store check).
5b. `crates/corelink-container/src/routes/tier_select.rs:330` — `parts()` status-mapping arm: `DpaRequired` → 403 `dpa_required`.
6. `crates/corelink-container/src/routes/tier_select.rs:600-608` — the `CORELINK_DPA_VERSION` unset-⇒-NOT-mounted guard (in `build_state_from_env`, header `:590`).
...
9b. `crates/corelink-container/src/routes/tier_select.rs:784-788` — emit-Err-aborts-before-mutation orchestration (vacuous today).
...
15. `crates/corelink-container/src/routes/dpa_accept.rs:507` — `handle`: the `POST /v1/onboarding/dpa-accept` writer entrypoint (fail-CLOSED auth gate → `orchestrate_dpa_accept` at `:376`; mount fail-CLOSES on the RS256 key at `:570`).
16. `crates/corelink-container/src/routes/dpa_accept_store.rs:107` — `insert_acceptance`: the durable D1 `INSERT OR IGNORE INTO dpa_acceptances` that the tier-select gate then reads.
