...
4. Each D1 statement is driven through `block_in_place` + `Handle::current().block_on` because the trait
   ...
   (`crates/corelink-container/src/billing_d1_http.rs:102-119`).
...
9. `crates/corelink-container/src/billing_d1_http.rs:102-119` — `run`: `block_in_place` + `block_on` per D1 round-trip.
