...
checkpoint_sha: "ba5c9f230376464cc3f4e80a88212dc2d166e110"
provenance: "AUTHORED"
...
7. The production CAS handler builder wires a DURABLE audit trail, not a volatile one: with storage creds
   ...
   (`crates/corelink-container/src/storage/r2_s3.rs:2286-2324`).
8. Every native CAS/AC handler's R2/S3 object GET/PUT/DELETE/LIST call — made through the sync
   ...
   the representative example (`crates/corelink-container/src/storage/r2_s3.rs:1209-1213`). The `list()`
   ...
   (`crates/corelink-container/src/storage/r2_s3.rs:2107-2221`). See
...
9. The Bazel REAPI `findMissingBlobs` existence probe is the seam that removes an O(N) endpoint.
   ...
   bounded concurrency (`crates/corelink-container/src/storage/r2_s3.rs:1560-1662`), the bound deliberately small because the container is a
   0.25-vCPU `basic` instance and R2 request limits are shared across tenants on the account
   (`crates/corelink-container/src/storage/r2_s3.rs:572`). Results come back in REQUEST order, not completion order, and the first error wins.
   The capability is OPTIONAL — a handler without the async audit sink advertises none
   (`crates/corelink-container/src/storage/r2_s3.rs:1549-1554`) and the caller keeps the unchanged per-digest `exists()` loop. The storage half
   of a probe is ONE module-private helper that deliberately emits NO audit row (`crates/corelink-container/src/storage/r2_s3.rs:1711-1729`),
...
- On the production data plane the CAS audit sink MUST be durable: the builder wires the D1 `audit_outbox`
  ...
  volatile in-memory sink (`crates/corelink-container/src/storage/r2_s3.rs:2312`).
- The native `list()` concurrent seam never weakens the audit-before-response guarantee: the audit
  ...
  (`crates/corelink-container/src/storage/r2_s3.rs:2223-2226`).
- The `findMissingBlobs` batch seam obeys the same rule: cross-tenant denial is evaluated over the
  ...
  (`crates/corelink-container/src/storage/r2_s3.rs:1577-1594`); the batched audit result is then evaluated BEFORE any probe result is read, so a
  failed audit returns `AuditFailed` and no probe outcome reaches the caller (`crates/corelink-container/src/storage/r2_s3.rs:1641-1643`).
...
10. `crates/corelink-container/src/storage/r2_s3.rs:1209-1213` — `R2CasHandler::read`'s R2 GET wrapped into `Phase::Store` (`ostore`) via `PhaseScope::enter`, the same treatment every other native-plane R2 GET/PUT/HEAD/DELETE/LIST call in this file now gets.
11. `crates/corelink-container/src/storage/r2_s3.rs:2107-2221` — `R2CasHandler::list`: when `audit_async` is wired (production), the mandatory `ListAttempted` audit write and the R2 `ListObjectsV2` call run CONCURRENTLY (`tokio::join!`) under one `block_in_place`/`block_on`; the audit result is checked first (`crates/corelink-container/src/storage/r2_s3.rs:2223-2226`) so no rows are served on a failed audit write. `R2AcHandler::list` mirrors this exactly.
12. `crates/corelink-container/src/storage/r2_s3.rs:1560-1662` — `R2CasHandler::exists_batch_inner`: the `findMissingBlobs` seam — cross-tenant denial first, ONE batched `ReadAttempted` write joined with the bounded-concurrency R2 HEADs, audit result evaluated first.
13. `crates/corelink-container/src/storage/r2_s3.rs:572` — `MAX_CONCURRENT_EXISTS_PROBES`: the in-flight bound on those HEADs (0.25-vCPU instance; R2 limits shared across tenants).
14. `crates/corelink-container/src/storage/r2_s3.rs:1711-1729` — `probe_existence_unaudited`: the ONE deliberately non-auditing probe body, shared by the single-digest and batch seams; safe only because both callers gate every result on their audit write.
