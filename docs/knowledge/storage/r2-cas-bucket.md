...
7. The production CAS handler builder wires a DURABLE audit trail, not a volatile one: with storage creds
   ...
   `AuditFailed → 503` guard dead code) (`crates/corelink-container/src/storage/r2_s3.rs:1804-1823`).
8. Every native CAS/AC handler's R2/S3 object GET/PUT/DELETE/LIST call — made through the sync
   ...
   the representative example (`crates/corelink-container/src/storage/r2_s3.rs:1158-1162`). See
   [the request-flow concept](/planes/request-flow.md) for the full phase list.
...
- On the production data plane the CAS audit sink MUST be durable: the builder wires the D1 `audit_outbox`
  ...
  volatile in-memory sink (`crates/corelink-container/src/storage/r2_s3.rs:1813`).
...
8. `crates/corelink-container/src/storage/r2_s3.rs:1804-1823` — `build_r2_cas_handler_from_env` wires the DURABLE D1 `audit_outbox` sink (`cas_audit_sink_from_d1`) and fails CLOSED, replacing the former volatile `InMemoryAuditSink`.
...
10. `crates/corelink-container/src/storage/r2_s3.rs:1158-1162` — `R2CasHandler::read`'s R2 GET wrapped into `Phase::Store` (`ostore`) via `PhaseScope::enter`, the same treatment every other native-plane R2 GET/PUT/HEAD/DELETE/LIST call in this file now gets.
