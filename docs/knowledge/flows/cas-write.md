...
- R2 S3 credentials are env-sourced only and the real adapter is constructed solely when all creds are present; absent creds fall back to the dev/in-memory path (`crates/corelink-container/src/storage.rs:99-116`; `crates/corelink-container/src/routes/cas.rs:516-626`).
...
10. `crates/corelink-container/src/storage.rs:99-116` — `StorageEnv::from_env`: env-only R2/D1 credential loading.
