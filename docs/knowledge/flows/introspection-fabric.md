...
4. PAT verification: `state.verifier.verify(&req.token)` runs the full HMAC + D1 liveness + Argon2id + scope [PAT verification gauntlet](/flows/pat-gauntlet.md), returning the owning tenant (`crates/corelink-container/src/routes/auth_introspect.rs:596-597`; `crates/corelink-container/src/adapter_pat.rs:1762-1766`).
...
6. `crates/corelink-container/src/adapter_pat.rs:1762-1766` — `PatVerifier::verify` (the shared verification the route delegates to).
</content>
