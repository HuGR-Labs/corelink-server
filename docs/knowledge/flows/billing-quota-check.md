...
1. Placement: a billable handler holds `Option<QuotaGate>` and calls `gate.check(&tenant)` at the top, after scope/rate-limit and before the work; absent in dev/CI (`crates/corelink-container/src/routes.rs:225-239`).
...
1. `crates/corelink-container/src/routes.rs:225-239` — `QuotaGate` usage contract (called at the top of a billable handler).
