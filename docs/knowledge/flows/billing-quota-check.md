...
13. Read-only projection of the same `tenant_quota` row: `GET /_internal/tenant/{tenant_id}/quota` exposes `{monthly_budget_usd_micros, accrued_usd_micros, cycle_anchor_ms, unmetered}` for an out-of-band preflight (e.g. a coordinator asserting a CI tenant is unmetered) WITHOUT touching the accrual path. It is ordered fail-closed exactly like the gate: the constant-time internal-auth check runs FIRST (401 before any D1), then UUID validation (400), then a single parameterized `SELECT ... WHERE tenant_id = ?1`; a missing row is 404 `no_quota_row` (the caller inherits the `$1M` default ⇒ unmetered), a D1 transport/decode fault is 503 (never a fabricated answer) (`crates/corelink-container/src/routes/tenant_quota_read.rs:155-209`). The `unmetered` bit is derived, not stored: `monthly_budget_usd_micros >= DEFAULT_MONTHLY_BUDGET_USD_MICROS` (`crates/corelink-container/src/routes/tenant_quota_read.rs:238`).
...
10. `crates/corelink-container/src/routes/tenant_quota_read.rs:155-209` — `handle_get_quota`: the read-only `tenant_quota` projection, ordered fail-closed (constant-time internal-auth gate → 400 UUID → single parameterized SELECT → 404 `no_quota_row` / 503 on D1 fault); `crates/corelink-container/src/routes/tenant_quota_read.rs:238` — the derived `unmetered` bit (`monthly_budget_usd_micros >= DEFAULT_MONTHLY_BUDGET_USD_MICROS`).
</content>
