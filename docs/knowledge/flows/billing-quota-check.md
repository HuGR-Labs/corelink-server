...
1. Placement: a billable handler holds `Option<QuotaGate>` and calls `gate.check(&tenant)` at the top, after scope/rate-limit and before the work; absent in dev/CI (`crates/corelink-container/src/routes.rs:273-277`).
2. Cost resolution: `QuotaGate` resolves the flat per-op micro-dollar cost once at build and delegates `check` to `QuotaGuard::check` (`crates/corelink-container/src/routes.rs:279-297`).
...
1. `crates/corelink-container/src/routes.rs:273-277` — `pub struct QuotaGate` (the per-route handle a billable handler holds, checked at the top of a billable handler).
2. `crates/corelink-container/src/routes.rs:279-297` — `from_env` cost resolution + `check` delegating to `QuotaGuard::check`.
3. `crates/corelink-container/src/routes.rs:314-316` — `check_batch` delegating the proportional batch charge.
