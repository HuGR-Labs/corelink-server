...
6. Steady path — atomic check-and-accrue: `QuotaGuard::check` delegates to `store.check_and_accrue`, whose D1 override serializes the ceiling test WITH the increment in one statement (`UPDATE ... WHERE accrued + delta <= budget RETURNING accrued`, executed at `crates/corelink-container/src/tenant_quota.rs:1031-1036`); over-ceiling -> 402, store error -> 503 (the delegation/outcome mapping is `crates/corelink-container/src/tenant_quota.rs:838-876`; the trait sig is only `:250`). NOTE: this describes the INNER `D1QuotaStore`. The live guard does NOT run this statement per op — it is fronted by `LeasedQuotaStore` (step 6a).
...
- The ceiling check and the spend accrual are atomic (serialized in one DB statement) IN THE INNER `D1QuotaStore` — the executed `UPDATE tenant_quota SET accrued_usd_micros = accrued_usd_micros + ?2 ... WHERE accrued_usd_micros + ?2 <= monthly_budget_usd_micros RETURNING accrued_usd_micros` (the predicate+RETURNING at `crates/corelink-container/src/tenant_quota.rs:1031-1036`, inside the `D1QuotaStore::check_and_accrue` override `:1021-1060`), closing the TOCTOU over-admission window where concurrent ops each read the same pre-accrual baseline. The live guard reaches that statement through `LeasedQuotaStore::check_and_accrue` (the executed leased hot path `crates/corelink-container/src/tenant_quota.rs:658`, wired into the guard at `:112-125`), which pre-debits a 16-op chunk and serves ~15 ops from an in-memory lease without D1 — so the per-op serialization is an attribute of the lease-acquire, not of every billable op; the DURABLE budget still never over-admits, but the cap is enforced approximately, at lease-chunk granularity.
...
4. `crates/corelink-container/src/tenant_quota.rs:1021-1060` — the `D1QuotaStore::check_and_accrue` override that EXECUTES the atomic ceiling+accrual SQL (the `WHERE accrued + delta <= budget RETURNING` predicate at `:1031-1036`); the trait sig at `:250` is only the interface (its default impl is the non-atomic test fallback).
...
9. `crates/corelink-container/src/tenant_quota.rs:658` — the EXECUTED `LeasedQuotaStore::check_and_accrue` (the leased hot path: serve from the in-memory lease, else refill via the inner atomic statement); `DEFAULT_LEASE_OPS = 16` and the mechanism doc live at `:366-451`.
</content>
