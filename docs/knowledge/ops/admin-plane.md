...
running privileged logic (`crates/corelink-container/src/routes/admin.rs:784-799`).
...
- The container admin router exposes the read + mutate routes `crates/corelink-container/src/routes/admin.rs:676`.
...
- A mutate body is parsed into a request that carries the dual-approval `(approval_id, approver)` pair `crates/corelink-container/src/routes/admin.rs:801`.
...
9. `crates/corelink-container/src/routes/admin.rs:676` — the admin read+mutate router.
10. `crates/corelink-container/src/routes/admin.rs:804` — incomplete dual-approval pair rejected at request construction (the `_ => return Err(...)` arm).
10b. `crates/corelink-container/src/routes/admin.rs:784-799` — the only two LIVE mutate ops: `set_tenant_tier` + `rotate_admin_token` (`_ => return Err("unknown op_kind")`; the config-singleton CAS is design-plane, not wired here).
