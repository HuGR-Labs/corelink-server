...
- A mutate missing its dual-approval pair is rejected at request construction `crates/corelink-container/src/routes/admin.rs:588`.
- Pilot mutations emit a `corelink.admin.pilot_*` audit event on attempt and success `crates/corelink-container/src/routes/admin_pilot.rs:174`.

# Gotchas

- The pilot store baseline is `InMemoryPilotStore` (non-durable) — pilot ops do not survive a restart until a D1-durable store is wired `crates/corelink-container/src/routes/admin_pilot.rs:390`.
...
10. `crates/corelink-container/src/routes/admin.rs:588` — dual-approval pair on mutate request construction.
11. `crates/corelink-container/src/routes/admin_pilot.rs:110` — canonical grant-tier pilot route const.
12. `crates/corelink-container/src/routes/admin_pilot.rs:174` — pilot audit event types.
13. `crates/corelink-container/src/routes/admin_pilot.rs:390` — the `PilotStore` trait (InMemory baseline).
