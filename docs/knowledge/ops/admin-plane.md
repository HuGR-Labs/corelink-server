...
- Credential separation is enforced in code at boot, not left to operator discipline: `approver_key_distinct_or_none` constant-time-compares the resolved approver key against the internal/mutate key and, on a byte-equal match, collapses it to `None` (logs + fails approve CLOSED) so a mis-provisioned key equal to `CORELINK_INTERNAL_AUTH_KEY` cannot silently re-collapse approve+mutate onto one shared secret `crates/corelink-container/src/routes.rs:476`; it is wired into the boot path in `build_with_factory` `crates/corelink-container/src/routes.rs:746`.
...
10g. `crates/corelink-container/src/routes.rs:476` — `approver_key_distinct_or_none`: constant-time byte-equality vs the internal/mutate key; a byte-equal approver key collapses to `None` (boot-enforced credential separation).
10h. `crates/corelink-container/src/routes.rs:746` — `build_with_factory` wires the approver key through `approver_key_distinct_or_none` against the internal auth key at boot.
