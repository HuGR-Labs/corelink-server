...
3. All six non-determinism sources are documented with mitigation + residual risk (cargo timestamp, LLVM
   ...
   (`docs/build/reproducible.md:78-196`).
...
7. A customer re-verifies by downloading the `reproducible-build-report` artifact
   (`gh run download --repo HuGR-Labs/corelink-server`) and re-running the same
   toolchain pin + flags locally to compare the hash (`docs/build/reproducible.md:269-290`).
...
- The reproducible matrix uses `--jobs 1` to eliminate link-order non-determinism, even though release CI
  uses parallel jobs for throughput (`docs/build/reproducible.md:166-177`).
- The rustc toolchain is pinned to exact minor `1.91.1` via `rust-toolchain.toml`; a bump follows the §3.3
  ...
  land an ADR-0015 amendment (`docs/build/reproducible.md:128-137`, `rust-toolchain.toml:13`).
- A `build.rs` that bypasses `SOURCE_DATE_EPOCH` (e.g. `SystemTime::now()`) is blocked by the pre-commit
  lint `scripts/build_rs_lint.sh` (`docs/build/reproducible.md:227-252`).
...
- A >50% diff escalates to SEV-2 (compromised-builder suspicion), distinct from the SEV-3 regression alarm
  (`docs/build/reproducible.md:212-214`).
...
5. `docs/build/reproducible.md:78-196` — the six documented non-determinism sources.
6. `docs/build/reproducible.md:117-137` — rustc exact-minor pin + upgrade procedure.
7. `docs/build/reproducible.md:166-177` — `--jobs 1` for deterministic link order.
8. `docs/build/reproducible.md:212-214` — SEV-3 regression vs SEV-2 compromised-builder alert.
9. `docs/build/reproducible.md:227-252` — the build.rs timestamp pre-commit lint.
10. `docs/build/reproducible.md:269-290` — customer verification quickstart.
