...
- The entitlement boundary: runner capacity is a decoupled axis, so cache tier and runner caps move
  independently (`crates/corelink-container/src/routes/auth_introspect/part-00.rs:317`).
...
2. The request body is parsed ONLY after the auth gate passes; the token is never logged
   (`crates/corelink-container/src/routes/auth_introspect/part-00.rs:581-593`).
3. The PAT is verified (HMAC + D1 liveness + Argon2id + scope) before any tenant resolution
   (`crates/corelink-container/src/routes/auth_introspect/part-00.rs:595-596`).
4. The plan (cache tier wire string) is resolved; a tier-query fault fails CLOSED with 503
   (`crates/corelink-container/src/routes/auth_introspect/part-00.rs:598-630`).
5. The runner entitlement is read in one keyed lookup from `runners_entitlement` (migrations 0070+0072),
   ...
   (`crates/corelink-container/src/routes/auth_introspect/part-00.rs:352`).
...
7. A valid 200 carries `max_concurrency` only when the tenant has a row and `max_vcpu_h` only when that
   ...
   (`crates/corelink-container/src/routes/auth_introspect/part-00.rs:245`, `crates/corelink-container/src/routes/auth_introspect/part-00.rs:256`).
...
- Any D1 fault during plan OR entitlement resolution fails CLOSED with 503 — the fabric maps 503 to
  ...
  (`crates/corelink-container/src/routes/auth_introspect/part-00.rs:641-675`).
...
- `max_concurrency` and `max_vcpu_h` are asymmetric: an absent cap ⇒ reject placement, but an absent
  ...
  (`crates/corelink-container/src/routes/auth_introspect/part-00.rs:279-297`).
- The `plan` field is informational only for the runners decision — capacity comes from the entitlement
  ...
  (`crates/corelink-container/src/routes/auth_introspect/part-00.rs:625-640`).

# Citations
1. `crates/corelink-container/src/routes/auth_introspect/part-00.rs:245` — `max_concurrency: Option<u32>` wire field.
2. `crates/corelink-container/src/routes/auth_introspect/part-00.rs:256` — `max_vcpu_h: Option<u32>` wire field.
3. `crates/corelink-container/src/routes/auth_introspect/part-00.rs:279-297` — `valid()` constructor: cap=reject, vcpu_h=wall-off asymmetry.
4. `crates/corelink-container/src/routes/auth_introspect/part-00.rs:317` — the `runners_entitlement` keyed SQL (separate axis).
5. `crates/corelink-container/src/routes/auth_introspect/part-00.rs:352` — `runner_concurrency_for_tenant` one-lookup resolver.
6. `crates/corelink-container/src/routes/auth_introspect/part-00.rs:569-579` — constant-time OR multi-key auth gate.
...
8. `crates/corelink-container/src/routes/auth_introspect/part-00.rs:581-593` — body parsed only after the gate; token never logged.
9. `crates/corelink-container/src/routes/auth_introspect/part-00.rs:595-596` — PAT verify (HMAC+D1+Argon2id+scope).
10. `crates/corelink-container/src/routes/auth_introspect/part-00.rs:598-630` — plan resolution.
11. `crates/corelink-container/src/routes/auth_introspect/part-00.rs:625-640` — 200 valid response assembly (plan informational).
12. `crates/corelink-container/src/routes/auth_introspect/part-00.rs:641-675` — fail-CLOSED 503 on entitlement/tier/backend fault.
13. `crates/corelink-container/src/routes/auth_introspect/part-00.rs:652-654` — uniform `valid:false`, no oracle.
