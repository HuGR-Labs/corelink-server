...
8. The route is only mounted when `FABRIC_INTROSPECT_AUTH_KEY` is present and ≥32 chars; otherwise it
   returns `None` and is not mounted (fail-CLOSED) (`crates/corelink-container/src/routes/auth_introspect.rs:975-983`).
...
10. For the multi-tenant runner-CI path (cf-multitenant), the fabric plane shares the SAME identity/authorization
    ...
    (`crates/corelink-container/src/routes/auth_introspect.rs:877`); `repo_on_tenant_allowlist` reads
    `runner_repo_allowlist` (migration 0085) to confirm a `repo_full_name` is explicitly allowed for the tenant,
    fail-CLOSED (`crates/corelink-container/src/routes/auth_introspect.rs:928`).
...
- `FABRIC_INTROSPECT_AUTH_KEY` is a DEDICATED secret, NOT `CORELINK_INTERNAL_AUTH_KEY` — a leak of the mint
  ...
  `crates/corelink-container/src/routes/auth_introspect.rs:847`), and an optional `_HUGR` second consumer
  key is supported additively (`crates/corelink-container/src/routes/auth_introspect.rs:877-886`).
