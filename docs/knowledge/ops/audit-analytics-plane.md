...
23. `crates/corelink-container/src/routes/audit_drain.rs:282-298` — constant-time internal-auth gate (`internal_auth_ok`); `crates/corelink-container/src/routes/audit_drain.rs:307-372` — fail-CLOSED env mount (≥32-char key + D1 required) in `build_state_from_env`.
