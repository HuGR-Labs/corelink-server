...
- The S-09 drain endpoint is mounted (env-gated, internal-auth) as `POST /_internal/audit/drain` and is the LIVE chain-seal producer: `handle_drain` scans the `(tenant_id, region)` partitions with pending rows then seals each `crates/corelink-container/src/routes/audit_drain.rs:905-963`.
...
- The drain auth gate is the constant-time internal-auth check (`internal_auth_ok` `crates/corelink-container/src/routes/audit_drain.rs:278-294`) plus the ≥32-char key floor + fail-CLOSED env mount (unmounted without the erase/internal-auth key + D1) in `build_state_from_env` `crates/corelink-container/src/routes/audit_drain.rs:303-368`.
- CF-6 keyed head: every advance Ed25519-SIGNS the canonical head tuple (JCS of `{head_hash, next_sequence, region, tenant_id}` via `canonical_head_bytes` `crates/corelink-container/src/routes/audit_drain.rs:230-247`, signed by `sign_head` `crates/corelink-container/src/routes/audit_drain.rs:230-247`) and persists it alongside the checkpoint `crates/corelink-container/src/routes/audit_drain.rs:866-875`; on resume `check_head_on_resume` re-verifies the stored signature and returns `FailClosed` on tamper / signed-but-no-seed `crates/corelink-container/src/routes/audit_drain.rs:498-556`, which the partition drain converts into a SEV-1 `tamper_detected` abort `crates/corelink-container/src/routes/audit_drain.rs:811-826`.
...
- The chain seal is idempotent + fork-free: each row UPDATE is guarded by `emitted_at IS NULL` (`write_seal` `crates/corelink-container/src/routes/audit_drain.rs:656-682`) and the head advance is a compare-and-set on the resumed value (`advance_head_cas` `crates/corelink-container/src/routes/audit_drain.rs:735-789`), so a re-run or a concurrent drain never double-seals and never forks the chain.
...
20. `crates/corelink-container/src/routes/audit_drain.rs:905-963` — `handle_drain`: LIVE S-09 `POST /_internal/audit/drain` entry (partition scan → seal → summary).
21. `crates/corelink-container/src/routes/audit_drain.rs:790-900` — `drain_partition`: crash-safe resume (sealed-tail authoritative) + idempotent `emitted_at IS NULL`-guarded seal + compare-and-set head advance (anti-fork) + CF-6 resume-signature check (SEV-1 fail-CLOSED on tamper at `crates/corelink-container/src/routes/audit_drain.rs:811-826`).
...
23. `crates/corelink-container/src/routes/audit_drain.rs:278-294` — constant-time internal-auth gate (`internal_auth_ok`); `crates/corelink-container/src/routes/audit_drain.rs:303-368` — fail-CLOSED env mount (≥32-char key + D1 required) in `build_state_from_env`.
24. `crates/corelink-container/src/routes/audit_drain.rs:230-247` (`canonical_head_bytes`) + `crates/corelink-container/src/routes/audit_drain.rs:230-247` (`sign_head`) + `crates/corelink-container/src/routes/audit_drain.rs:249-271` (`verify_head`) + `crates/corelink-container/src/routes/audit_drain.rs:498-556` (`check_head_on_resume`) — CF-6 keyed (Ed25519-signed) `audit_chain_head` sign-on-advance + verify-on-resume (migration 0080 columns `head_signature`/`head_signed_at_ms`/`signing_key_id`).
