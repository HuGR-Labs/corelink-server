...
- CF-6 keyed head: every advance Ed25519-SIGNS the canonical head tuple (JCS of `{head_hash, next_sequence, region, tenant_id}` via `canonical_head_bytes` `crates/corelink-container/src/routes/audit_drain.rs:263-293`, signed by `sign_head` `crates/corelink-container/src/routes/audit_drain.rs:263-293`) and persists it alongside the checkpoint `crates/corelink-container/src/routes/audit_drain.rs:946-973`; on resume `check_head_on_resume` re-verifies the stored signature and returns `FailClosed` on tamper / signed-but-no-seed `crates/corelink-container/src/routes/audit_drain.rs:559-617`, which the partition drain converts into a SEV-1 `tamper_detected` abort `crates/corelink-container/src/routes/audit_drain.rs:890-905`.
...
- The chain seal is idempotent + fork-free: each row UPDATE is guarded by `emitted_at IS NULL` (`write_seal` `crates/corelink-container/src/routes/audit_drain.rs:758-776`) and the head advance is a compare-and-set on the resumed value (`advance_head_cas` `crates/corelink-container/src/routes/audit_drain.rs:789-859`), so a re-run or a concurrent drain never double-seals and never forks the chain.
...
20. `crates/corelink-container/src/routes/audit_drain.rs:989-1075` — `handle_drain`: LIVE S-09 `POST /_internal/audit/drain` entry (partition scan → seal → summary).
21. `crates/corelink-container/src/routes/audit_drain.rs:866-984` — `drain_partition`: crash-safe resume (sealed-tail authoritative) + idempotent `emitted_at IS NULL`-guarded seal + compare-and-set head advance (anti-fork) + CF-6 resume-signature check (SEV-1 fail-CLOSED on tamper at `crates/corelink-container/src/routes/audit_drain.rs:890-905`).
