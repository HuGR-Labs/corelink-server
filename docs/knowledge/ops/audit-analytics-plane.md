...
- The drain is invoked by an hourly Cloudflare Cron Trigger in the signup-worker. `runAuditDrainSweep` accepts ONLY the dedicated ≥32-char `CORELINK_ERASE_AUTH_KEY` — shared-only/blank/short configuration skips fail-CLOSED before a request — then POSTs `/_internal/audit/drain` `apps/signup-worker/src/webhooks/audit_drain_cron.ts:45-52`, `:171-191`; the deploy lane verifies that the secret NAME is bound before it deploys `scripts/verify-signup-worker-secrets.sh:49-57`, `.github/workflows/signup-worker-deploy.yml:105-109`. A 2xx is not automatically success: the caller decodes the complete typed handler contract (`ok`, six safe non-negative counters, `incomplete`), stops visibly on malformed data, `ok:false`, failed partitions, or incomplete-with-no-progress, and never blind-retries those states `apps/signup-worker/src/webhooks/audit_drain_cron.ts:110-162`, `:248-301`. Only a valid incomplete response with progress re-calls, bounded by `MAX_DRAIN_CALLS` and `DRAIN_WALL_BUDGET_MS`; a bounded early stop remains visibly incomplete `apps/signup-worker/src/webhooks/audit_drain_cron.ts:55-68`, `:303-307`.
...
25. `apps/signup-worker/src/webhooks/audit_drain_cron.ts:45-52`, `:110-162`, `:171-307` — `runAuditDrainSweep`: dedicated-key-only, strict typed 2xx contract, terminal non-complete arms, and bounded retry only after real progress; `scripts/verify-signup-worker-secrets.sh:49-57` + `.github/workflows/signup-worker-deploy.yml:105-109` — secret-name propagation gate before deploy.
