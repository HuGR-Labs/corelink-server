...
- **Terraform drift (`corelink-terraform-drift-consumer`) — real orchestrator, NO apply surface.**
  ...
  (`crates/corelink-terraform-drift-consumer/src/consumer.rs:58-103`). The crate deliberately has no
  `terraform apply` (`crates/corelink-terraform-drift-consumer/src/consumer.rs:132-136`).
...
- The drift consumer exposes no apply path by construction
  (`crates/corelink-terraform-drift-consumer/src/consumer.rs:132-136`).
...
20. `crates/corelink-terraform-drift-consumer/src/consumer.rs:58-103` — classify→audit-before-store→insert→metrics.
21. `crates/corelink-terraform-drift-consumer/src/consumer.rs:132-136` — no `terraform apply` surface by construction.
