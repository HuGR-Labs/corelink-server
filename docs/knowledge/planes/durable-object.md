...
source_blobs:
  - "worker/src/durable_object.ts@8c598c7333a557ef0bfd01ef7bdbff3d969fae9a"
...
- The materializer of the container env contract: every secret/credential the container reads is
  ...
  (`worker/src/durable_object.ts:774-987`).
...
5b. WHERE a brand-new DO — and the Rust container it owns — is created is now DETERMINISTIC and in-region,
   ...
   `.get(id)` — today's exact behaviour, never worse (`worker/src/index.ts:345-377`). This closes a
   ...
   (`worker/src/index.ts:3705`); the `_system`/`_oci`/`_anonymous` sentinel forwards route through the
   same helper.
5c. That same idempotency extends to the `container.start()` CALL itself. `container.running` can flip
   ...
   (`worker/src/durable_object.ts:1047-1070`).
...
7. `waitForContainerHealth` polls `GET /_health` on the container port until a 200 or the 90s startup
   timeout (`worker/src/durable_object.ts:1181-1211`). The `waitForContainerReady` queue FAST-EXITS the
   ...
   prompt `503` so the next request triggers a restart (`worker/src/durable_object.ts:1094-1123`).
8. The idle reaper lives INSIDE the alarm tick (durable), not in a `setTimeout`: it compares the
   ...
   (`worker/src/durable_object.ts:159-176`, `worker/src/durable_object.ts:1468-1497`). An in-memory
...
8b. That reaper is no longer the only one. The DO also arms Cloudflare's OWN idle auto-destroy,
   `container.setInactivityTimeout(IDLE_TIMEOUT_MS)` (`worker/src/durable_object.ts:1167`), so workerd
   ...
   `/_health` is still reaped (`worker/src/durable_object.ts:992`) — and re-armed from the alarm tick
   whenever the container is running and the reaper above did NOT find it idle
   (`worker/src/durable_object.ts:1515`). Both call sites exist because the type declaration carries no
   ...
   (`worker/src/durable_object.ts:1157`) — a container that cannot arm its idle timer must still serve.
...
9. `alarm()` is a `try/finally` shell that ALWAYS re-arms the chain unless the tick deliberately ended
   ...
   (`worker/src/durable_object.ts:1416-1432`, `worker/src/durable_object.ts:1440-1561`).
10. The Worker exports three SIBLING DO classes alongside `CoreLinkServer`
    (`worker/src/index.ts:4023`). `EventLogDO` is the ADR-0065 per-tenant append-only event-log
...
13. The `_system` `CoreLinkServer` DO (the `idFromName("_system")` instance that hosts this region's
    ...
    proxies the revoke onto that DO via `systemStub.fetch` (`worker/src/index.ts:2374`), which the DO
    ...
    (`ctx.waitUntil(writePublicBlocklistKv(...))`, `worker/src/index.ts:2416`). It is a Worker-side
...
- `/_do/health` is not only a liveness page: it also carries the `d1_probe` PLACEMENT INSTRUMENT —
  ...
  (`worker/src/durable_object.ts:1225-1350`). Edge delivery is `/_internal/do-d1-probe/{tenant_id}`,
  ...
  (`worker/src/index.ts:2133-2172`). CAVEAT: `/_internal/*` does not fan out, so a tenant whose
...
10. `worker/src/durable_object.ts:774-987` — the `container.start({ env })` env-contract forward.
11. `worker/src/durable_object.ts:1181-1211` — `waitForContainerHealth` polling `/_health`; `worker/src/durable_object.ts:1094-1123` — the M1 fast-exit on a terminal `"stopped"` container (no full ~90s spin on a dead container).
12. `worker/src/durable_object.ts:1468-1497` — the DURABLE idle reaper inside `alarmTick()`: an absent `lastActivityMs` backfills, an expired one emits the death event (audit-before-mutation), RE-CHECKS the clock (every `await` is a yield point where a queued request may have arrived), then destroys the container and signals chain-end so the DO hibernates instead of heartbeating a dead container forever.
13. `worker/src/durable_object.ts:1416-1432` — `alarm()`: a thin `try/finally` that ALWAYS re-arms the chain unless the tick reported a deliberate end, so a throwing tick can never strand the reaper (the posture `ReplicationCoordinatorDO.alarm()` already used); `worker/src/durable_object.ts:1440-1561` — `alarmTick()`: chain guard (dead container ⇒ end the chain), the idle reaper, the `degraded`-but-running arm (probe skipped, chain kept so the reaper still applies), the dedup arm, then the health re-probe + degrade.
14. `worker/src/index.ts:4023` — the Worker's named export of `CoreLinkServer`, `RolloutController`, `EventLogDO`, `ReplicationCoordinatorDO` (the DO-class exports at the module tail, immediately after the `export default handler` Sentry-wrapped fetch handler).
...
22. `worker/src/durable_object.ts:1225-1350` — `handleHealthProbe` + `probeD1Latency`: the `d1_probe` placement instrument on `/_do/health` (primary vs `withSession("first-unconstrained")` replica, feature-detected exactly as `worker/src/index.ts:1406-1409` does it, warm-up reported not hidden, a throw surfaced as an explicit `error` field, never on the request-serving path).
23. `worker/src/index.ts:2133-2172` — `/_internal/do-d1-probe/{tenant_id}` → that tenant's DO `/_do/health`: same forward shape as the `/_internal/replication` → `/_repl` precedent, behind the internal-auth gate on the low-privilege `quota_read` consumer (`worker/src/index.ts:435-443`), with the DO id derived by `idFromName(tenant)` so the probe measures the SAME instance that serves that tenant.
24. `worker/src/durable_object.ts:1167` — `armInactivityTimeout`: the PLATFORM idle reaper, `container.setInactivityTimeout(IDLE_TIMEOUT_MS)`, which workerd enforces without this Worker (it survives DO eviction, a broken alarm chain and a wedged isolate — the failure modes that made containers immortal). Armed right after `container.start()`, before the health poll (`worker/src/durable_object.ts:992`), and re-armed on a live non-idle alarm tick (`worker/src/durable_object.ts:1515`) because the type declaration does not specify whether the timer resets on activity or is absolute from arming. Feature-detected, and an arming failure is logged rather than thrown (`worker/src/durable_object.ts:1157`) — a container that cannot arm its idle timer must still serve.
25. `worker/src/index.ts:2374` — the B1b `/_public`-revoke forward onto the `_system` `CoreLinkServer` DO via `systemStub.fetch` (a Worker-side pass-through the DO proxies to the container unchanged); on the container's `ok` response the Worker best-effort-writes the content-hash-keyed edge blocklist KV (`ctx.waitUntil(writePublicBlocklistKv(...))`, `worker/src/index.ts:2416`). No new DO handler — the `_system` DO's proxy path is unchanged.
26. `worker/src/index.ts:345-377` — `doLocationHintForRegion` + `serverGetOpts`: derive a CF DO `locationHint` from this Worker's serving region (`R2_CAS_REGION`) so a newly-created `CoreLinkServer` DO and its container home in-region deterministically (unknown region ⇒ `undefined` ⇒ bare hint-less `.get(id)`, unchanged behaviour); the per-tenant CAS/AC forward passes it at `worker/src/index.ts:3705`, and every sentinel (`_system`/`_oci`/`_anonymous`) forward routes through the same helper.
