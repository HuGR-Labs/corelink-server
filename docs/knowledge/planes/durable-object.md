...
checkpoint_sha: "4b58faa573abbf8cef0d3a8ffacb54afc08e6401"
provenance: "AUTHORED"
...
5b. WHERE a brand-new DO — and the Rust container it owns — is created is now DETERMINISTIC and in-region,
   ...
   `.get(id)` — today's exact behaviour, never worse (`worker/src/index.ts:388-420`). This closes a
   ...
   (`worker/src/index.ts:3992`); the `_system`/`_oci`/`_anonymous` sentinel forwards route through the
   same helper.
5c. That same idempotency extends to the `container.start()` CALL itself. `container.running` can flip
   ...
   (`worker/src/durable_object.ts:1078-1101`).
...
7. `waitForContainerHealth` polls `GET /_health` on the container port until a 200 or the 90s startup
   timeout (`worker/src/durable_object.ts:1212-1242`). The `waitForContainerReady` queue FAST-EXITS the
   ...
   prompt `503` so the next request triggers a restart (`worker/src/durable_object.ts:1125-1154`).
8. The idle reaper lives INSIDE the alarm tick (durable), not in a `setTimeout`: it compares the
   ...
   (`worker/src/durable_object.ts:159-176`, `worker/src/durable_object.ts:1499-1528`). An in-memory
...
8b. That reaper is no longer the only one. The DO also arms Cloudflare's OWN idle auto-destroy,
   `container.setInactivityTimeout(IDLE_TIMEOUT_MS)` (`worker/src/durable_object.ts:1198`), so workerd
   ...
   `/_health` is still reaped (`worker/src/durable_object.ts:1023`) — and re-armed from the alarm tick
   whenever the container is running and the reaper above did NOT find it idle
   (`worker/src/durable_object.ts:1546`). Both call sites exist because the type declaration carries no
   ...
   (`worker/src/durable_object.ts:1188`) — a container that cannot arm its idle timer must still serve.
...
9. `alarm()` is a `try/finally` shell that ALWAYS re-arms the chain unless the tick deliberately ended
   ...
   (`worker/src/durable_object.ts:1447-1463`, `worker/src/durable_object.ts:1471-1592`).
10. The Worker exports three SIBLING DO classes alongside `CoreLinkServer`
    (`worker/src/index.ts:4496`). `EventLogDO` is the ADR-0065 per-tenant append-only event-log
...
13. The `_system` `CoreLinkServer` DO (the `idFromName("_system")` instance that hosts this region's
    ...
    proxies the revoke onto that DO via `systemStub.fetch` (`worker/src/index.ts:2481`), which the DO
    ...
    (`ctx.waitUntil(writePublicBlocklistKv(...))`, `worker/src/index.ts:2523`). It is a Worker-side
...
- `/_do/health` is not only a liveness page: it also carries the `d1_probe` PLACEMENT INSTRUMENT —
  ...
  (`worker/src/durable_object.ts:1256-1381`). Edge delivery is `/_internal/do-d1-probe/{tenant_id}`,
  ...
  (`worker/src/index.ts:2240-2279`). CAVEAT: `/_internal/*` does not fan out, so a tenant whose
...
10b. `worker/src/durable_object.ts:847-848` — the PAT rotation-overlap siblings `PAT_SIGNING_KEY_PREV` / `PAT_SIGNING_KEY_NEW` in the same forward-list (B-081). The container already folds them into its HMAC key set (its PAT adapter's `from_env`), so a PAT minted under either verifies during a rotation window; while only the CURRENT key was forwarded that mechanism was UNREACHABLE. The forward makes overlap reachable for containers **started after** a sibling is staged. It does not update already-running population members: `P` includes each active tenant container and the exactly-one persistent `_oci` container per environment, because `/token` and `/v2/*` route to `idFromName("_oci")` (`worker/src/index.ts:882-900`; `worker/src/index.ts:2731-2732`) and its container verifies OCI PATs. `/_internal/admin/recycle-system` reaches only the five `_system` instances, not `P`, and there is no population enumerator or boot-generation proof. B-081 therefore remains open until the population ritual in `specs/_runbooks/RB-PAT-SIGNING-KEY-ROTATION.md` can be executed. `?? ""` is load-bearing here: the container reads these through its `non_empty_env` helper, which treats EMPTY as ABSENT, so an unbound optional secret is omitted from the set instead of being read as present-but-malformed — which returns `None` from `from_env` and leaves the adapter routes unmounted entirely.
...
14. `worker/src/index.ts:4496` — the Worker's named export of `CoreLinkServer`, `RolloutController`, `EventLogDO`, `ReplicationCoordinatorDO` (the DO-class exports at the module tail, immediately after the `export default handler` Sentry-wrapped fetch handler).
...
22. `worker/src/durable_object.ts:1256-1381` — `handleHealthProbe` + `probeD1Latency`: the `d1_probe` placement instrument on `/_do/health` (primary vs `withSession("first-unconstrained")` replica, feature-detected exactly as `worker/src/index.ts:1471-1474` does it, warm-up reported not hidden, a throw surfaced as an explicit `error` field, never on the request-serving path).
23. `worker/src/index.ts:2240-2279` — `/_internal/do-d1-probe/{tenant_id}` → that tenant's DO `/_do/health`: same forward shape as the `/_internal/replication` → `/_repl` precedent, behind the internal-auth gate on the low-privilege `quota_read` consumer (`worker/src/index.ts:487-495`), with the DO id derived by `idFromName(tenant)` so the probe measures the SAME instance that serves that tenant.
24. `worker/src/durable_object.ts:1198` — `armInactivityTimeout`: the PLATFORM idle reaper, `container.setInactivityTimeout(IDLE_TIMEOUT_MS)`, which workerd enforces without this Worker (it survives DO eviction, a broken alarm chain and a wedged isolate — the failure modes that made containers immortal). Armed right after `container.start()`, before the health poll (`worker/src/durable_object.ts:1023`), and re-armed on a live non-idle alarm tick (`worker/src/durable_object.ts:1546`) because the type declaration does not specify whether the timer resets on activity or is absolute from arming. Feature-detected, and an arming failure is logged rather than thrown (`worker/src/durable_object.ts:1188`) — a container that cannot arm its idle timer must still serve.
25. `worker/src/index.ts:2481` — the B1b `/_public`-revoke forward onto the `_system` `CoreLinkServer` DO via `systemStub.fetch` (a Worker-side pass-through the DO proxies to the container unchanged); on the container's `ok` response the Worker best-effort-writes the content-hash-keyed edge blocklist KV (`ctx.waitUntil(writePublicBlocklistKv(...))`, `worker/src/index.ts:2523`). No new DO handler — the `_system` DO's proxy path is unchanged.
26. `worker/src/index.ts:388-420` — `doLocationHintForRegion` + `serverGetOpts`: derive a CF DO `locationHint` from this Worker's serving region (`R2_CAS_REGION`) so a newly-created `CoreLinkServer` DO and its container home in-region deterministically (unknown region ⇒ `undefined` ⇒ bare hint-less `.get(id)`, unchanged behaviour); the per-tenant CAS/AC forward passes it at `worker/src/index.ts:3992`, and every sentinel (`_system`/`_oci`/`_anonymous`) forward routes through the same helper.
