...
source_blobs:
  - "worker/src/durable_object.ts@ba9106aa2f1354b912ca6c34f63e6a17e4f1184d"
...
checkpoint_sha: "1b4be9591a8077dddf3d73f27933124dd17ac6bf"
provenance: "AUTHORED"
...
- The materializer of the container env contract: every secret/credential the container reads is
  ...
  `OCI_PUBLIC_DEDUP_ENABLED` (`worker/src/durable_object.ts:993`) and the WP-G M3
  `OCI_UPSTREAM_ON_MISS` (`worker/src/durable_object.ts:994`) — each `?? ""` so an unset var
  ...
  (`worker/src/durable_object.ts:774-996`).
...
5b. WHERE a brand-new DO — and the Rust container it owns — is created is now DETERMINISTIC and in-region,
   ...
   `.get(id)` — today's exact behaviour, never worse (`worker/src/index.ts:348-380`). This closes a
   ...
   (`worker/src/index.ts:3725`); the `_system`/`_oci`/`_anonymous` sentinel forwards route through the
   same helper.
5c. That same idempotency extends to the `container.start()` CALL itself. `container.running` can flip
   ...
   (`worker/src/durable_object.ts:1056-1079`).
...
7. `waitForContainerHealth` polls `GET /_health` on the container port until a 200 or the 90s startup
   timeout (`worker/src/durable_object.ts:1190-1220`). The `waitForContainerReady` queue FAST-EXITS the
   ...
   prompt `503` so the next request triggers a restart (`worker/src/durable_object.ts:1103-1132`).
8. The idle reaper lives INSIDE the alarm tick (durable), not in a `setTimeout`: it compares the
   ...
   (`worker/src/durable_object.ts:159-176`, `worker/src/durable_object.ts:1477-1506`). An in-memory
...
8b. That reaper is no longer the only one. The DO also arms Cloudflare's OWN idle auto-destroy,
   `container.setInactivityTimeout(IDLE_TIMEOUT_MS)` (`worker/src/durable_object.ts:1176`), so workerd
   ...
   `/_health` is still reaped (`worker/src/durable_object.ts:1001`) — and re-armed from the alarm tick
   whenever the container is running and the reaper above did NOT find it idle
   (`worker/src/durable_object.ts:1524`). Both call sites exist because the type declaration carries no
   ...
   (`worker/src/durable_object.ts:1166`) — a container that cannot arm its idle timer must still serve.
...
9. `alarm()` is a `try/finally` shell that ALWAYS re-arms the chain unless the tick deliberately ended
   ...
   (`worker/src/durable_object.ts:1425-1441`, `worker/src/durable_object.ts:1449-1570`).
10. The Worker exports three SIBLING DO classes alongside `CoreLinkServer`
    (`worker/src/index.ts:4043`). `EventLogDO` is the ADR-0065 per-tenant append-only event-log
...
13. The `_system` `CoreLinkServer` DO (the `idFromName("_system")` instance that hosts this region's
    ...
    proxies the revoke onto that DO via `systemStub.fetch` (`worker/src/index.ts:2377`), which the DO
    ...
    (`ctx.waitUntil(writePublicBlocklistKv(...))`, `worker/src/index.ts:2419`). It is a Worker-side
...
- `/_do/health` is not only a liveness page: it also carries the `d1_probe` PLACEMENT INSTRUMENT —
  ...
  (`worker/src/durable_object.ts:1234-1359`). Edge delivery is `/_internal/do-d1-probe/{tenant_id}`,
  ...
  (`worker/src/index.ts:2136-2175`). CAVEAT: `/_internal/*` does not fan out, so a tenant whose
...
10. `worker/src/durable_object.ts:774-996` — the `container.start({ env })` env-contract forward, including the S-09 offsite-archive knobs `R2_AUDIT_BUCKET` (`worker/src/durable_object.ts:893`) and `AUDIT_ARCHIVE_BATCH_LIMIT` (`worker/src/durable_object.ts:894`), each `?? ""` ⇒ container default (bucket `corelink-audit-weur`, 2000-row batch), and the WP-G OCI keystone flags `OCI_PUBLIC_DEDUP_ENABLED` (`worker/src/durable_object.ts:993`) and `OCI_UPSTREAM_ON_MISS` (`worker/src/durable_object.ts:994`), each `?? ""` ⇒ container default (OFF).
11. `worker/src/durable_object.ts:1190-1220` — `waitForContainerHealth` polling `/_health`; `worker/src/durable_object.ts:1103-1132` — the M1 fast-exit on a terminal `"stopped"` container (no full ~90s spin on a dead container).
12. `worker/src/durable_object.ts:1477-1506` — the DURABLE idle reaper inside `alarmTick()`: an absent `lastActivityMs` backfills, an expired one emits the death event (audit-before-mutation), RE-CHECKS the clock (every `await` is a yield point where a queued request may have arrived), then destroys the container and signals chain-end so the DO hibernates instead of heartbeating a dead container forever.
13. `worker/src/durable_object.ts:1425-1441` — `alarm()`: a thin `try/finally` that ALWAYS re-arms the chain unless the tick reported a deliberate end, so a throwing tick can never strand the reaper (the posture `ReplicationCoordinatorDO.alarm()` already used); `worker/src/durable_object.ts:1449-1570` — `alarmTick()`: chain guard (dead container ⇒ end the chain), the idle reaper, the `degraded`-but-running arm (probe skipped, chain kept so the reaper still applies), the dedup arm, then the health re-probe + degrade.
14. `worker/src/index.ts:4043` — the Worker's named export of `CoreLinkServer`, `RolloutController`, `EventLogDO`, `ReplicationCoordinatorDO` (the DO-class exports at the module tail, immediately after the `export default handler` Sentry-wrapped fetch handler).
...
22. `worker/src/durable_object.ts:1234-1359` — `handleHealthProbe` + `probeD1Latency`: the `d1_probe` placement instrument on `/_do/health` (primary vs `withSession("first-unconstrained")` replica, feature-detected exactly as `worker/src/index.ts:1409-1412` does it, warm-up reported not hidden, a throw surfaced as an explicit `error` field, never on the request-serving path).
23. `worker/src/index.ts:2136-2175` — `/_internal/do-d1-probe/{tenant_id}` → that tenant's DO `/_do/health`: same forward shape as the `/_internal/replication` → `/_repl` precedent, behind the internal-auth gate on the low-privilege `quota_read` consumer (`worker/src/index.ts:438-446`), with the DO id derived by `idFromName(tenant)` so the probe measures the SAME instance that serves that tenant.
24. `worker/src/durable_object.ts:1176` — `armInactivityTimeout`: the PLATFORM idle reaper, `container.setInactivityTimeout(IDLE_TIMEOUT_MS)`, which workerd enforces without this Worker (it survives DO eviction, a broken alarm chain and a wedged isolate — the failure modes that made containers immortal). Armed right after `container.start()`, before the health poll (`worker/src/durable_object.ts:1001`), and re-armed on a live non-idle alarm tick (`worker/src/durable_object.ts:1524`) because the type declaration does not specify whether the timer resets on activity or is absolute from arming. Feature-detected, and an arming failure is logged rather than thrown (`worker/src/durable_object.ts:1166`) — a container that cannot arm its idle timer must still serve.
25. `worker/src/index.ts:2377` — the B1b `/_public`-revoke forward onto the `_system` `CoreLinkServer` DO via `systemStub.fetch` (a Worker-side pass-through the DO proxies to the container unchanged); on the container's `ok` response the Worker best-effort-writes the content-hash-keyed edge blocklist KV (`ctx.waitUntil(writePublicBlocklistKv(...))`, `worker/src/index.ts:2419`). No new DO handler — the `_system` DO's proxy path is unchanged.
26. `worker/src/index.ts:348-380` — `doLocationHintForRegion` + `serverGetOpts`: derive a CF DO `locationHint` from this Worker's serving region (`R2_CAS_REGION`) so a newly-created `CoreLinkServer` DO and its container home in-region deterministically (unknown region ⇒ `undefined` ⇒ bare hint-less `.get(id)`, unchanged behaviour); the per-tenant CAS/AC forward passes it at `worker/src/index.ts:3725`, and every sentinel (`_system`/`_oci`/`_anonymous`) forward routes through the same helper.
