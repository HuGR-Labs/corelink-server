...
- The materializer of the container env contract: every secret/credential the container reads is
  ...
  (`worker/src/durable_object.ts:534-709`).
...
7. `waitForContainerHealth` polls `GET /_health` on the container port until a 200 or the 90s startup
   timeout (`worker/src/durable_object.ts:806-836`). The `waitForContainerReady` queue FAST-EXITS the
   ...
   prompt `503` so the next request triggers a restart (`worker/src/durable_object.ts:784-795`).
8. An idle timer destroys the container after `IDLE_TIMEOUT_MS` (30 min), emitting the death event first
   (`worker/src/durable_object.ts:80-93`, `worker/src/durable_object.ts:924-941`).
...
10. `worker/src/durable_object.ts:534-709` — the `container.start({ env })` env-contract forward.
