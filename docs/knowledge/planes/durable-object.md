...
5b. WHERE a brand-new DO — and the Rust container it owns — is created is now DETERMINISTIC and in-region,
   ...
   `.get(id)` — today's exact behaviour, never worse (`worker/src/index.ts:344-376`). This closes a
   ...
   (`worker/src/index.ts:3627`); the `_system`/`_oci`/`_anonymous` sentinel forwards route through the
   same helper.
...
10. The Worker exports three SIBLING DO classes alongside `CoreLinkServer`
    (`worker/src/index.ts:3945`). `EventLogDO` is the ADR-0065 per-tenant append-only event-log
...
13. The `_system` `CoreLinkServer` DO (the `idFromName("_system")` instance that hosts this region's
    ...
    proxies the revoke onto that DO via `systemStub.fetch` (`worker/src/index.ts:2373`), which the DO
    ...
    (`ctx.waitUntil(writePublicBlocklistKv(...))`, `worker/src/index.ts:2415`). It is a Worker-side
...
- `/_do/health` is not only a liveness page: it also carries the `d1_probe` PLACEMENT INSTRUMENT —
  ...
  (`worker/src/index.ts:2132-2171`). CAVEAT: `/_internal/*` does not fan out, so a tenant whose
...
14. `worker/src/index.ts:3945` — the Worker's named export of `CoreLinkServer`, `RolloutController`, `EventLogDO`, `ReplicationCoordinatorDO` (the DO-class exports at the module tail, immediately after the `export default handler` Sentry-wrapped fetch handler).
...
22. `worker/src/durable_object.ts:1225-1350` — `handleHealthProbe` + `probeD1Latency`: the `d1_probe` placement instrument on `/_do/health` (primary vs `withSession("first-unconstrained")` replica, feature-detected exactly as `worker/src/index.ts:1405-1408` does it, warm-up reported not hidden, a throw surfaced as an explicit `error` field, never on the request-serving path).
23. `worker/src/index.ts:2132-2171` — `/_internal/do-d1-probe/{tenant_id}` → that tenant's DO `/_do/health`: same forward shape as the `/_internal/replication` → `/_repl` precedent, behind the internal-auth gate on the low-privilege `quota_read` consumer (`worker/src/index.ts:434-442`), with the DO id derived by `idFromName(tenant)` so the probe measures the SAME instance that serves that tenant.
...
25. `worker/src/index.ts:2373` — the B1b `/_public`-revoke forward onto the `_system` `CoreLinkServer` DO via `systemStub.fetch` (a Worker-side pass-through the DO proxies to the container unchanged); on the container's `ok` response the Worker best-effort-writes the content-hash-keyed edge blocklist KV (`ctx.waitUntil(writePublicBlocklistKv(...))`, `worker/src/index.ts:2415`). No new DO handler — the `_system` DO's proxy path is unchanged.
26. `worker/src/index.ts:344-376` — `doLocationHintForRegion` + `serverGetOpts`: derive a CF DO `locationHint` from this Worker's serving region (`R2_CAS_REGION`) so a newly-created `CoreLinkServer` DO and its container home in-region deterministically (unknown region ⇒ `undefined` ⇒ bare hint-less `.get(id)`, unchanged behaviour); the per-tenant CAS/AC forward passes it at `worker/src/index.ts:3627`, and every sentinel (`_system`/`_oci`/`_anonymous`) forward routes through the same helper.
