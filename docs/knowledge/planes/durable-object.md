...
10. The Worker exports two SIBLING DO classes alongside `CoreLinkServer`
    ...
    primitive — one instance per tenant (`idFromName(tenant_id)`), pinned to its tenant and rejecting a
    cross-tenant id; `append` monotonically assigns `seq` under `blockConcurrencyWhile` (persist the
    entry, THEN advance the head, so a crash orphans rather than gaps) and `read` returns gap-free
    entries (`worker/src/event_log_do.ts:220-279`). It is bound in `wrangler.toml` for every env and
    exported, but NO edge route in `worker/src` currently dispatches to it — the `EVENT_LOG_DO` binding
    is referenced only as an OPTIONAL field of `Env` (`worker/src/index.ts:55-59`); it is a ready
    primitive awaiting its hugit-P2 seam-D consumer.
11. `RolloutController` is an UNWIRED stub: bound in `wrangler.toml` and exported, it answers
    ...
    bridged (`worker/src/rollout_controller.ts:48-52`).
...
- `EventLogDO` is per-tenant and append-only: a DO pinned to one tenant rejects a request carrying a
  different `x-corelink-tenant-id`, and `seq` is strictly monotonic + gap-free under
  `blockConcurrencyWhile` (persist-entry-then-advance-head) (`worker/src/event_log_do.ts:220-279`).
- Honest wiring: of the three exported DO classes, only `CoreLinkServer` is a live serving path.
  ...
  OPTIONAL `Env` field, `worker/src/index.ts:55-59`), and `RolloutController` is a bound+exported STUB
  returning `501 NOT_IMPLEMENTED` for everything but health (`worker/src/rollout_controller.ts:48-52`).
...
15. `worker/src/event_log_do.ts:220-279` — `EventLogDO` route dispatch + `handleAppend`: ADR-0065 per-tenant append-only log, monotonic gap-free `seq` under `blockConcurrencyWhile` (persist-then-advance-head).
16. `worker/src/index.ts:55-59` — the `EVENT_LOG_DO` binding declared OPTIONAL on `Env` (the only `worker/src` reference; no edge route dispatches to it yet).
17. `worker/src/rollout_controller.ts:48-52` — `RolloutController` UNWIRED stub: `501 NOT_IMPLEMENTED` "WASM bridge not yet wired (Phase C)" for all non-health requests.
18. `worker/src/index.ts:2618` — the Worker exports `CoreLinkServer`, `RolloutController`, `EventLogDO`.
