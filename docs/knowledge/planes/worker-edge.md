...
- The auth authority for the native plane: it is the only layer that reads the D1 `pat.scope` and
  forwards it as a server-trust header (`worker/src/index.ts:306-331`).
- The declarer of the container's env contract: every operator secret the container reads is first a
  field on the Worker `Env` interface here (`worker/src/index.ts:193`), then materialized onto the
...
1. The exported handler is `baseHandler.fetch`, which resolves a request-id, handles CORS preflight,
   and matches the route before doing anything else (`worker/src/index.ts:1529-1542`).
2. `matchRoute` is an ordered first-match table mapping each URL to a `RouteKind` + tenant, with
   ...
   (`worker/src/index.ts:552-840`).
3. Health routes short-circuit with no auth and no DO forward (`worker/src/index.ts:1547-1560`).
   ...
   (`worker/src/index.ts:844-845`), forward arm (`worker/src/index.ts:2111-2133`).
3b. Three EXACT-path fabric/ingest carve-outs are matched BEFORE the generic `/v1/*` PAT arm and are pure
   ...
   `/internal/v1/auth/introspect` (`worker/src/index.ts:723`) and the sibling
   `/internal/v1/auth/resolve-tenant` (`worker/src/index.ts:735`) — both share `routeKind="fabric_introspect"`
   ...
   billing usage-push (`/internal/v1/billing/usage`, `routeKind="billing_ingest"`, `worker/src/index.ts:747`)
...
4. `extractAuth` fails CLOSED (503) when `PAT_SIGNING_KEY` is absent or decodes to < 32 bytes — the
   signing key is the sole possession gate for the native plane (`worker/src/index.ts:902-929`).
5. PAT validation is HMAC-SHA256 fast-reject (with rotation siblings) BEFORE any D1 round-trip, then a
   D1 lookup by `token_id` and an application-side expiry check (`worker/src/index.ts:967-1114`). The
   ...
   reason `d1_lookup_error` (`worker/src/index.ts:1090-1099`). The caller maps BOTH the config fault
   ...
   `invalid_*`) still fall through to `401` (`worker/src/index.ts:2341-2370`). NOTE: the inline
...
6. `stripClientTrustHeaders` deletes every client-suppliable trust header on every forward, then the
   Worker re-sets its own verified values (`worker/src/index.ts:493-497`).
7. Per-tier quota (storage SUM + monthly request-count) runs after auth and before the DO forward —
   ...
   fail-closed) (`worker/src/index.ts:2375-2396`).
8. The request is routed to the per-tenant DO via `idFromName(resolvedTenantId)`
   (`worker/src/index.ts:2689-2691`) and dispatched with `stub.fetch` (`worker/src/index.ts:2762`). A
   multi-region tenant may first take the LOCAL (this-region) `_system` container branch above this
   forward (`worker/src/index.ts:1707-1709`); a non-resident tenant falls through to the per-tenant DO
   derivation here.
9. The forwarded request is augmented: strip-then-set the trusted tenant-id, scope, token-prefix, and
   client-ip headers (`worker/src/index.ts:2701-2737`).
10. The whole handler is wrapped by `Sentry.withSentry`, inert until `SENTRY_DSN` is set and with
    `sendDefaultPii=false` (`worker/src/index.ts:2821-2840`); its `beforeSend`/`beforeSendTransaction`
...
- OCI, the Stripe webhook, and the fabric-introspect / resolve-tenant / billing-ingest arms are pure
  ...
  them. `/internal/v1/auth/introspect` (`worker/src/index.ts:723`) and `/internal/v1/auth/resolve-tenant`
  (`worker/src/index.ts:735`) share `routeKind="fabric_introspect"` + the `FABRIC_INTROSPECT_AUTH_KEY` gate;
  `/internal/v1/billing/usage` (`worker/src/index.ts:747`) uses its own `BILLING_INGEST_AUTH_KEY`.

# Citations
1. `worker/src/index.ts:1-20` — the architecture header documenting the Worker → DO → container topology.
2. `worker/src/index.ts:306-331` — the `AuthResult` carrying the D1-resolved scope (the Worker is its sole authority).
3. `worker/src/index.ts:447-486` — the `CLIENT_TRUST_HEADERS` strip list.
4. `worker/src/index.ts:493-497` — `stripClientTrustHeaders` (delete-then-set discipline).
5. `worker/src/index.ts:552-840` — the `matchRoute` ordered route table.
5b. `worker/src/index.ts:723` / `worker/src/index.ts:735` / `worker/src/index.ts:747` — the three exact-path pure-pass-through carve-outs: `/internal/v1/auth/introspect` + `/internal/v1/auth/resolve-tenant` (both `fabric_introspect`, `FABRIC_INTROSPECT_AUTH_KEY`) and `/internal/v1/billing/usage` (`billing_ingest`, `BILLING_INGEST_AUTH_KEY`).
6. `worker/src/index.ts:902-929` — `extractAuth` fail-CLOSED on absent/short `PAT_SIGNING_KEY`.
7. `worker/src/index.ts:967-1114` — HMAC fast-reject + D1 lookup + expiry check (incl. the `expires_ms === 0` never-expires sentinel guard at `:1105`, and the try/catch that turns a transient D1 fault into `d1_lookup_error` at `:1082-1091`).
7b. `worker/src/index.ts:2341-2370` — caller reason→status mapping: BOTH `signing_key_not_configured` (config fault) AND `d1_lookup_error` (transient D1 fault) → retryable `503`; every other reason (`pat_not_found` / `pat_expired` / `invalid_*`) → `401`.
8. `worker/src/index.ts:1529-1542` — the `baseHandler.fetch` entry, request-id, CORS, route match.
9. `worker/src/index.ts:1547-1560` — health short-circuit (no auth, no DO).
10. `worker/src/index.ts:2375-2396` — per-tier quota enforcement after auth, before forward.
11. `worker/src/index.ts:2689-2691` — `idFromName(resolvedTenantId)` per-tenant DO routing.
12. `worker/src/index.ts:2701-2737` — the augmented forward (strip-then-set trust headers).
13. `worker/src/index.ts:2731` — forwarding the D1-resolved scope as `x-corelink-scope`.
14. `worker/src/index.ts:2762` — `stub.fetch` dispatch to the DO.
15. `worker/src/index.ts:2776-2789` — 404 timing-pad.
16. `worker/src/index.ts:2821-2840` — the `Sentry.withSentry` wrapper (inert until `SENTRY_DSN`; `sendDefaultPii=false`).
17. `worker/src/index.ts:2830-2835` — `beforeSend`/`beforeSendTransaction` → `scrubSentryEvent` (`worker/src/sentry-scrub.ts:101-188`): full-event PII/secret scrub (default-DENY sensitive keys + free-text secret/PII shapes), not just header keys.
