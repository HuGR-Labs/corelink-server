...
1. The exported handler is `baseHandler.fetch`, which resolves a request-id, handles CORS preflight,
   and matches the route before doing anything else (`worker/src/index.ts:1632-1645`).
2. `matchRoute` is an ordered first-match table mapping each URL to a `RouteKind` + tenant, with
   ...
   (`worker/src/index.ts:571-882`).
3. Health routes short-circuit with no auth and no DO forward (`worker/src/index.ts:1650-1663`).
   ...
   (`worker/src/index.ts:886-887`), forward arm (`worker/src/index.ts:2247-2269`).
3b. Three EXACT-path fabric/ingest carve-outs are matched BEFORE the generic `/v1/*` PAT arm and are pure
   ...
   `/internal/v1/auth/introspect` (`worker/src/index.ts:765`) and the sibling
   `/internal/v1/auth/resolve-tenant` (`worker/src/index.ts:777`) — both share `routeKind="fabric_introspect"`
   ...
   billing usage-push (`/internal/v1/billing/usage`, `routeKind="billing_ingest"`, `worker/src/index.ts:789`)
...
4. `extractAuth` fails CLOSED (503) when `PAT_SIGNING_KEY` is absent or decodes to < 32 bytes — the
   signing key is the sole possession gate for the native plane (`worker/src/index.ts:1001-1040`).
5. PAT validation is HMAC-SHA256 fast-reject (with rotation siblings) BEFORE any D1 round-trip, then a
   D1 lookup by `token_id` and an application-side expiry check (`worker/src/index.ts:1070-1217`). The
   ...
   reason `d1_lookup_error` (`worker/src/index.ts:1193-1202`). The caller maps BOTH the config fault
   ...
   `invalid_*`) still fall through to `401` (`worker/src/index.ts:2492-2521`). NOTE: the inline
...
6. `stripClientTrustHeaders` deletes every client-suppliable trust header on every forward, then the
   Worker re-sets its own verified values (`worker/src/index.ts:506-510`).
7. Per-tier quota (storage SUM + monthly request-count) runs after auth and before the DO forward —
   ...
   fail-closed) (`worker/src/index.ts:2526-2547`).
8. The request is routed to the per-tenant DO via `idFromName(resolvedTenantId)`
   (`worker/src/index.ts:2840-2842`) and dispatched with `stub.fetch` (`worker/src/index.ts:2913`). A
   multi-region tenant may first take the LOCAL (this-region) `_system` container branch above this
   forward (`worker/src/index.ts:1843-1845`); a non-resident tenant falls through to the per-tenant DO
   derivation here.
9. The forwarded request is augmented: strip-then-set the trusted tenant-id, scope, token-prefix, and
   client-ip headers (`worker/src/index.ts:2852-2888`).
10. The whole handler is wrapped by `Sentry.withSentry`, inert until `SENTRY_DSN` is set and with
    `sendDefaultPii=false` (`worker/src/index.ts:2972-2991`); its `beforeSend`/`beforeSendTransaction`
...
- OCI, the Stripe webhook, and the fabric-introspect / resolve-tenant / billing-ingest arms are pure
  ...
  them. `/internal/v1/auth/introspect` (`worker/src/index.ts:765`) and `/internal/v1/auth/resolve-tenant`
  (`worker/src/index.ts:777`) share `routeKind="fabric_introspect"` + the `FABRIC_INTROSPECT_AUTH_KEY` gate;
  `/internal/v1/billing/usage` (`worker/src/index.ts:789`) uses its own `BILLING_INGEST_AUTH_KEY`.
...
5b. `worker/src/index.ts:765` / `worker/src/index.ts:777` / `worker/src/index.ts:789` — the three exact-path pure-pass-through carve-outs: `/internal/v1/auth/introspect` + `/internal/v1/auth/resolve-tenant` (both `fabric_introspect`, `FABRIC_INTROSPECT_AUTH_KEY`) and `/internal/v1/billing/usage` (`billing_ingest`, `BILLING_INGEST_AUTH_KEY`).
6. `worker/src/index.ts:1001-1040` — `extractAuth` fail-CLOSED on absent/short `PAT_SIGNING_KEY`.
7. `worker/src/index.ts:1070-1217` — HMAC fast-reject + D1 lookup + expiry check (incl. the `expires_ms === 0` never-expires sentinel guard at `:1166`, and the try/catch that turns a transient D1 fault into `d1_lookup_error` at `:1143-1152`).
7b. `worker/src/index.ts:2492-2521` — caller reason→status mapping: BOTH `signing_key_not_configured` (config fault) AND `d1_lookup_error` (transient D1 fault) → retryable `503`; every other reason (`pat_not_found` / `pat_expired` / `invalid_*`) → `401`.
8. `worker/src/index.ts:1632-1645` — the `baseHandler.fetch` entry, request-id, CORS, route match.
9. `worker/src/index.ts:1650-1663` — health short-circuit (no auth, no DO).
10. `worker/src/index.ts:2526-2547` — per-tier quota enforcement after auth, before forward.
11. `worker/src/index.ts:2840-2842` — `idFromName(resolvedTenantId)` per-tenant DO routing.
12. `worker/src/index.ts:2852-2888` — the augmented forward (strip-then-set trust headers).
13. `worker/src/index.ts:2882` — forwarding the D1-resolved scope as `x-corelink-scope`.
14. `worker/src/index.ts:2913` — `stub.fetch` dispatch to the DO.
15. `worker/src/index.ts:2927-2940` — 404 timing-pad.
16. `worker/src/index.ts:2972-2991` — the `Sentry.withSentry` wrapper (inert until `SENTRY_DSN`; `sendDefaultPii=false`).
17. `worker/src/index.ts:2981-2986` — `beforeSend`/`beforeSendTransaction` → `scrubSentryEvent` (`worker/src/sentry-scrub.ts:101-188`): full-event PII/secret scrub (default-DENY sensitive keys + free-text secret/PII shapes), not just header keys.
