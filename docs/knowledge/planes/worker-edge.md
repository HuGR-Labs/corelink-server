...
- The auth authority for the native plane: it is the only layer that reads the D1 `pat.scope` and
  forwards it as a server-trust header (`worker/src/index.ts:290-305`).

# How it works
1. The exported handler is `baseHandler.fetch`, which resolves a request-id, handles CORS preflight,
   and matches the route before doing anything else (`worker/src/index.ts:1480-1493`).
2. `matchRoute` is an ordered first-match table mapping each URL to a `RouteKind` + tenant, with
   ...
   (`worker/src/index.ts:530-806`).
3. Health routes short-circuit with no auth and no DO forward (`worker/src/index.ts:1498-1511`).
   ...
   (`worker/src/index.ts:780-790`), forward arm (`worker/src/index.ts:1944-1968`).
4. `extractAuth` fails CLOSED (503) when `PAT_SIGNING_KEY` is absent or decodes to < 32 bytes — the
   signing key is the sole possession gate for the native plane (`worker/src/index.ts:868-895`).
5. PAT validation is HMAC-SHA256 fast-reject (with rotation siblings) BEFORE any D1 round-trip, then a
   D1 lookup by `token_id` and an application-side expiry check (`worker/src/index.ts:933-1053`). The
   ...
   reason `d1_lookup_error` (`worker/src/index.ts:1029-1039`). The caller maps BOTH the config fault
   ...
   `invalid_*`) still fall through to `401` (`worker/src/index.ts:2153-2183`). NOTE: the inline
   `extractAuth` comment at `:1038` ("for now we 401 to fail-closed") is STALE — it describes a
...
6. `stripClientTrustHeaders` deletes every client-suppliable trust header on every forward, then the
   Worker re-sets its own verified values (`worker/src/index.ts:478-482`).
...
2. `worker/src/index.ts:290-305` — the `AuthResult` carrying the D1-resolved scope (the Worker is its sole authority).
3. `worker/src/index.ts:432-471` — the `CLIENT_TRUST_HEADERS` strip list.
4. `worker/src/index.ts:478-482` — `stripClientTrustHeaders` (delete-then-set discipline).
5. `worker/src/index.ts:530-806` — the `matchRoute` ordered route table.
6. `worker/src/index.ts:868-895` — `extractAuth` fail-CLOSED on absent/short `PAT_SIGNING_KEY`.
7. `worker/src/index.ts:933-1053` — HMAC fast-reject + D1 lookup + expiry check (incl. the `expires_ms === 0` never-expires sentinel guard at `:1053`, and the try/catch that turns a transient D1 fault into `d1_lookup_error` at `:1029-1039`).
7b. `worker/src/index.ts:2153-2183` — caller reason→status mapping: BOTH `signing_key_not_configured` (config fault) AND `d1_lookup_error` (transient D1 fault) → retryable `503`; every other reason (`pat_not_found` / `pat_expired` / `invalid_*`) → `401`.
8. `worker/src/index.ts:1480-1493` — the `baseHandler.fetch` entry, request-id, CORS, route match.
9. `worker/src/index.ts:1498-1511` — health short-circuit (no auth, no DO).
10. `worker/src/index.ts:2216-2237` — per-tier quota enforcement after auth, before forward.
11. `worker/src/index.ts:2530-2533` — `idFromName(resolvedTenantId)` per-tenant DO routing.
12. `worker/src/index.ts:2535-2593` — the augmented forward (strip-then-set trust headers).
13. `worker/src/index.ts:2564-2567` — forwarding the D1-resolved scope as `x-corelink-scope`.
14. `worker/src/index.ts:2598` — `stub.fetch` dispatch to the DO.
15. `worker/src/index.ts:2611-2618` — 404 timing-pad.
16. `worker/src/index.ts:2651-2670` — the `Sentry.withSentry` wrapper (inert until `SENTRY_DSN`; `sendDefaultPii=false`).
17. `worker/src/index.ts:2660-2665` — `beforeSend`/`beforeSendTransaction` → `scrubSentryEvent` (`worker/src/sentry-scrub.ts:101-188`): full-event PII/secret scrub (default-DENY sensitive keys + free-text secret/PII shapes), not just header keys.
