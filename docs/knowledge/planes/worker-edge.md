...
1. The exported handler is `baseHandler.fetch`, which resolves a request-id, handles CORS preflight,
   and matches the route before doing anything else (`worker/src/index.ts:1751-1797`).
...
3. Health routes short-circuit with no auth and no DO forward (`worker/src/index.ts:1799-1816`).
   ...
   (`worker/src/index.ts:948-949`), forward arm (`worker/src/index.ts:2381-2409`).
...
7d. **RETIRED by the `qbatch` merge, kept for the reasoning.** While `qmeter` was its own phase, its
   ...
   length floor is REFUSED fail-closed (`worker/src/lib/internal_auth.ts:168-193`), precisely so a
...
- A client can never smuggle a server-trust header: the strip list is applied on every forward path
  ...
  (`worker/src/index.ts:594`, set at `worker/src/index.ts:2575`) — this is what lets the container gate
...
8. `worker/src/index.ts:1751-1797` — the `baseHandler.fetch` entry, request-id, CORS, route match.
9. `worker/src/index.ts:1799-1816` — health short-circuit (no auth, no DO).
