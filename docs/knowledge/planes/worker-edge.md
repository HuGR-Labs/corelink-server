...
- The auth authority for the native plane: it is the only layer that reads the D1 `pat.scope` and
  forwards it as a server-trust header (`worker/src/index.ts:294-319`).
...
3. Health routes short-circuit with no auth and no DO forward (`worker/src/index.ts:1513-1526`).
   ...
   (`worker/src/index.ts:795-805`), forward arm (`worker/src/index.ts:2054-2079`).
...
5. PAT validation is HMAC-SHA256 fast-reject (with rotation siblings) BEFORE any D1 round-trip, then a
   ...
   `invalid_*`) still fall through to `401` (`worker/src/index.ts:2266-2298`). NOTE: the inline
...
8. The request is routed to the per-tenant DO via `idFromName(resolvedTenantId)`
   (`worker/src/index.ts:2643-2645`) and dispatched with `stub.fetch` (`worker/src/index.ts:2710`). A
   multi-region tenant may first take the LOCAL (this-region) `_system` container branch above this
   forward (`worker/src/index.ts:1644-1645`); a non-resident tenant falls through to the per-tenant DO
   derivation here.
9. The forwarded request is augmented: strip-then-set the trusted tenant-id, scope, token-prefix, and
   client-ip headers (`worker/src/index.ts:2649-2685`).
...
- Tenant isolation is structural: the DO id is derived solely from the PAT-resolved tenant, never the
  URL path segment (`worker/src/index.ts:2643-2645`).
...
2. `worker/src/index.ts:294-319` — the `AuthResult` carrying the D1-resolved scope (the Worker is its sole authority).
...
7b. `worker/src/index.ts:2266-2298` — caller reason→status mapping: BOTH `signing_key_not_configured` (config fault) AND `d1_lookup_error` (transient D1 fault) → retryable `503`; every other reason (`pat_not_found` / `pat_expired` / `invalid_*`) → `401`.
...
11. `worker/src/index.ts:2643-2645` — `idFromName(resolvedTenantId)` per-tenant DO routing.
12. `worker/src/index.ts:2649-2685` — the augmented forward (strip-then-set trust headers).
13. `worker/src/index.ts:2679` — forwarding the D1-resolved scope as `x-corelink-scope`.
14. `worker/src/index.ts:2710` — `stub.fetch` dispatch to the DO.
15. `worker/src/index.ts:2724-2731` — 404 timing-pad.
