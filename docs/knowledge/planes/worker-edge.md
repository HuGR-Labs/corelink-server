...
1. The exported handler is `baseHandler.fetch`, which resolves a request-id, handles CORS preflight,
   and matches the route before doing anything else (`worker/src/index.ts:1541-1554`).
2. `matchRoute` is an ordered first-match table mapping each URL to a `RouteKind` + tenant, with
   ...
   (`worker/src/index.ts:552-852`).
3. Health routes short-circuit with no auth and no DO forward (`worker/src/index.ts:1559-1572`).
   ...
   (`worker/src/index.ts:856-857`), forward arm (`worker/src/index.ts:2123-2145`).
3b. Three EXACT-path fabric/ingest carve-outs are matched BEFORE the generic `/v1/*` PAT arm and are pure
   ...
   `/internal/v1/auth/introspect` (`worker/src/index.ts:735`) and the sibling
   `/internal/v1/auth/resolve-tenant` (`worker/src/index.ts:747`) — both share `routeKind="fabric_introspect"`
   ...
   billing usage-push (`/internal/v1/billing/usage`, `routeKind="billing_ingest"`, `worker/src/index.ts:759`)
...
4. `extractAuth` fails CLOSED (503) when `PAT_SIGNING_KEY` is absent or decodes to < 32 bytes — the
   signing key is the sole possession gate for the native plane (`worker/src/index.ts:914-941`).
5. PAT validation is HMAC-SHA256 fast-reject (with rotation siblings) BEFORE any D1 round-trip, then a
   D1 lookup by `token_id` and an application-side expiry check (`worker/src/index.ts:979-1126`). The
   ...
   reason `d1_lookup_error` (`worker/src/index.ts:1102-1111`). The caller maps BOTH the config fault
   ...
   `invalid_*`) still fall through to `401` (`worker/src/index.ts:2353-2382`). NOTE: the inline
...
7. Per-tier quota (storage SUM + monthly request-count) runs after auth and before the DO forward —
   ...
   fail-closed) (`worker/src/index.ts:2387-2408`).
8. The request is routed to the per-tenant DO via `idFromName(resolvedTenantId)`
   (`worker/src/index.ts:2701-2703`) and dispatched with `stub.fetch` (`worker/src/index.ts:2774`). A
   multi-region tenant may first take the LOCAL (this-region) `_system` container branch above this
   forward (`worker/src/index.ts:1719-1721`); a non-resident tenant falls through to the per-tenant DO
   derivation here.
9. The forwarded request is augmented: strip-then-set the trusted tenant-id, scope, token-prefix, and
   client-ip headers (`worker/src/index.ts:2713-2749`).
10. The whole handler is wrapped by `Sentry.withSentry`, inert until `SENTRY_DSN` is set and with
    `sendDefaultPii=false` (`worker/src/index.ts:2833-2852`); its `beforeSend`/`beforeSendTransaction`
    ...
    `[REDACTED]`), closing the WP4 PII/secret-leak gap (`worker/src/index.ts:2842-2847`).

# Invariants
- Tenant isolation is structural: the DO id is derived solely from the PAT-resolved tenant, never the
  URL path segment (`worker/src/index.ts:2701-2703`).
...
- OCI, the Stripe webhook, and the fabric-introspect / resolve-tenant / billing-ingest arms are pure
  ...
  them. `/internal/v1/auth/introspect` (`worker/src/index.ts:735`) and `/internal/v1/auth/resolve-tenant`
  (`worker/src/index.ts:747`) share `routeKind="fabric_introspect"` + the `FABRIC_INTROSPECT_AUTH_KEY` gate;
  `/internal/v1/billing/usage` (`worker/src/index.ts:759`) uses its own `BILLING_INGEST_AUTH_KEY`.
...
5. `worker/src/index.ts:552-852` — the `matchRoute` ordered route table.
5b. `worker/src/index.ts:735` / `worker/src/index.ts:747` / `worker/src/index.ts:759` — the three exact-path pure-pass-through carve-outs: `/internal/v1/auth/introspect` + `/internal/v1/auth/resolve-tenant` (both `fabric_introspect`, `FABRIC_INTROSPECT_AUTH_KEY`) and `/internal/v1/billing/usage` (`billing_ingest`, `BILLING_INGEST_AUTH_KEY`).
6. `worker/src/index.ts:914-941` — `extractAuth` fail-CLOSED on absent/short `PAT_SIGNING_KEY`.
7. `worker/src/index.ts:979-1126` — HMAC fast-reject + D1 lookup + expiry check (incl. the `expires_ms === 0` never-expires sentinel guard at `:1105`, and the try/catch that turns a transient D1 fault into `d1_lookup_error` at `:1082-1091`).
7b. `worker/src/index.ts:2353-2382` — caller reason→status mapping: BOTH `signing_key_not_configured` (config fault) AND `d1_lookup_error` (transient D1 fault) → retryable `503`; every other reason (`pat_not_found` / `pat_expired` / `invalid_*`) → `401`.
8. `worker/src/index.ts:1541-1554` — the `baseHandler.fetch` entry, request-id, CORS, route match.
9. `worker/src/index.ts:1559-1572` — health short-circuit (no auth, no DO).
10. `worker/src/index.ts:2387-2408` — per-tier quota enforcement after auth, before forward.
11. `worker/src/index.ts:2701-2703` — `idFromName(resolvedTenantId)` per-tenant DO routing.
12. `worker/src/index.ts:2713-2749` — the augmented forward (strip-then-set trust headers).
13. `worker/src/index.ts:2743` — forwarding the D1-resolved scope as `x-corelink-scope`.
14. `worker/src/index.ts:2774` — `stub.fetch` dispatch to the DO.
15. `worker/src/index.ts:2788-2801` — 404 timing-pad.
16. `worker/src/index.ts:2833-2852` — the `Sentry.withSentry` wrapper (inert until `SENTRY_DSN`; `sendDefaultPii=false`).
17. `worker/src/index.ts:2842-2847` — `beforeSend`/`beforeSendTransaction` → `scrubSentryEvent` (`worker/src/sentry-scrub.ts:101-188`): full-event PII/secret scrub (default-DENY sensitive keys + free-text secret/PII shapes), not just header keys.
