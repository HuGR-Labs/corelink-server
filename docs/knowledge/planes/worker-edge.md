...
source_blobs:
  - "worker/src/index.ts@cc8662917c0ce81cfe2a562ff028b4be78b3c319"
...
checkpoint_sha: "92056d69a5d3bf3ad2617d889a56ae8d0308525e"
provenance: "AUTHORED"
...
3. Health routes short-circuit with no auth and no DO forward (`worker/src/index.ts:1913-1930`).
   ...
   (`worker/src/index.ts:1062-1063`), forward arm (`worker/src/index.ts:2680-2708`).
...
3c. **B1b `_public` edge-revocation accelerator.** In the internal-forward chain (after the shared
   ...
   (`worker/src/index.ts:2330`, forward at `worker/src/index.ts:2341`) — and ONLY on the container's `ok`
   response AND when `METADATA_KV` is bound best-effort-writes a content-hash-keyed edge blocklist entry
   via `ctx.waitUntil(writePublicBlocklistKv(...))` (`worker/src/index.ts:2367`), gated on the parsed
   body's `content_hash` matching `^[0-9a-f]{64}$` (`worker/src/index.ts:2366`). This collapses the
   ...
   has already applied authoritatively (`worker/src/index.ts:2357`).
...
5. PAT validation is HMAC-SHA256 fast-reject (with rotation siblings) BEFORE any D1 round-trip, then a
   ...
   `invalid_*`) still fall through to `401` (`worker/src/index.ts:2962-2999`). NOTE: the
...
6b. The Worker is also the PRODUCER of the `first_cli_authed` onboarding signal. Immediately after the
   ...
   (`worker/src/index.ts:3048-3054`). It lives here rather than in the container because the container's
...
7. Per-tier quota (storage SUM + monthly request-count) runs after auth and before the DO forward —
   ...
   fail-closed) (`worker/src/index.ts:3057-3326`). The tier is resolved FIRST (cache-served) and the two
   UNCACHED statements — the monthly-counter UPSERT and the storage `SUM(bytes_used)` — then travel as a
   SINGLE `db.batch()` round trip (`runQuotaBatch` — see the [edge quota & tier serving](/launch/edge-quota-tier-serving.md) concept for that module; the call site is `worker/src/index.ts:3273-3276`) rather than two
...
7b. Data-residency (`primary_region`, for region fan-out) is resolved AFTER quota and BEFORE the DO
   ...
   for this near-immutable value (`worker/src/index.ts:3376-3381`,
   ...
   rather than IAD-leaking an EU tenant to US storage (`worker/src/index.ts:3383-3400`,
   `worker/src/lib/tenant_residency_cache.ts:284-291`); a `null` region (no tenant row / no pin) preserves
   the existing `primaryRegion === undefined` IAD-local fall-through (`worker/src/index.ts:3404`). A stale
...
7c. A response returned through **this PAT-gated native DO forward** carries a `Server-Timing` header
   ...
   (`worker/src/index.ts:3747-3788`). `origin` is decomposed the same way `wdb` is, and the split spans
   ...
   (`worker/src/index.ts:3783`), forwards verbatim (`worker/src/index.ts:3959`) and completes with the
   one number only the Worker can see: `ohop = origin − Σ(container phases)`
   (`worker/src/index.ts:3953`), covering the DO dispatch + placement, the DO's own prologue, the wire
   ...
   (`worker/src/index.ts:3926`, `worker/src/index.ts:3947`), and a report that cannot be reconciled,
   its phases exceeding `origin` or a consumed phase carrying an unreadable duration, is dropped WHOLE
   as `ohop;dur=<origin>;desc="unreconciled"` (`worker/src/index.ts:3950-3951`) rather than published
...
8. The request is routed to the per-tenant DO via `idFromName(resolvedTenantId)`
   (`worker/src/index.ts:3516-3518`) and dispatched with `stub.fetch` (`worker/src/index.ts:3663`). A
...
9. The forwarded request is augmented: strip-then-set the trusted tenant-id, scope, token-prefix, and
   client-ip headers (`worker/src/index.ts:3521-3552`).
10. The whole handler is wrapped by `Sentry.withSentry`, inert until `SENTRY_DSN` is set and with
    `sendDefaultPii=false` (`worker/src/index.ts:3815-3823`); its `beforeSend`/`beforeSendTransaction`
    ...
    `[REDACTED]`), closing the WP4 PII/secret-leak gap (`worker/src/index.ts:3824-3829`).

# Invariants
- Tenant isolation is structural: the DO id is derived solely from the PAT-resolved tenant, never the
  URL path segment (`worker/src/index.ts:3516-3518`).
- A client can never smuggle a server-trust header: the strip list is applied on every forward path
  ...
  (`worker/src/index.ts:708`, set at `worker/src/index.ts:2874`) — this is what lets the container gate
...
- The Worker forwards the D1-resolved `scope` as `x-corelink-scope` and is its sole setter
  (`worker/src/index.ts:3552`); for a find-only PAT (`pat.find_only = 1`) the forwarded value is narrowed
...
- On the customer (Clerk-session) forward the Worker sets `x-corelink-scope` from the D1-resolved team
  ...
  container's F-018 billing/PII gate; the Worker is the sole setter (`worker/src/index.ts:2861-2868`).
- A 404 from the DO is timing-padded to defeat cross-tenant enumeration
  (`worker/src/index.ts:3708-3716`).
- Data-residency is resolved FAIL-CLOSED before the DO forward: an unresolvable region (a D1 fault with
  ...
  (`worker/src/index.ts:3383-3400`, `worker/src/lib/tenant_residency_cache.ts:284-291`).
...
5c. `worker/src/index.ts:2330` — the `/_internal/public/revoke` internal-forward arm (B1b): it forwards the revoke to the `_system` DO → container (`worker/src/index.ts:2341`) and, only on an `ok` container response with `METADATA_KV` bound and a `^[0-9a-f]{64}$` `content_hash` (`worker/src/index.ts:2357`, `worker/src/index.ts:2366`), best-effort-writes the content-hash-keyed edge blocklist KV via `ctx.waitUntil(writePublicBlocklistKv(...))` (`worker/src/index.ts:2367`) — collapsing the `_public` edge-serve revocation window from the ~60 s map-cache TTL to KV propagation, never failing the revoke on a KV fault.
