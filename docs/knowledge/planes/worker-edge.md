...
- The auth authority for the native plane: it is the only layer that reads the D1 `pat.scope` and
  forwards it as a server-trust header (`worker/src/index.ts:483-513`).
- The declarer of the container's env contract: every operator secret the container reads is first a
  field on the Worker `Env` interface here (`worker/src/index.ts:255`), then materialized onto the
...
1. The exported handler is `baseHandler.fetch`, which resolves a request-id, handles CORS preflight,
   and matches the route before doing anything else (`worker/src/index.ts:1893-1939`).
2. `matchRoute` is an ordered first-match table mapping each URL to a `RouteKind` + tenant, with
   ...
   (`worker/src/index.ts:796-1108`).
3. Health routes short-circuit with no auth and no DO forward (`worker/src/index.ts:1941-1958`).
   ...
   (`worker/src/index.ts:1090-1091`), forward arm (`worker/src/index.ts:2724-2752`).
3b. Three EXACT-path fabric/ingest carve-outs are matched BEFORE the generic `/v1/*` PAT arm and are pure
   ...
   `/internal/v1/auth/introspect` (`worker/src/index.ts:990`) and the sibling
   `/internal/v1/auth/resolve-tenant` (`worker/src/index.ts:1002`) — both share `routeKind="fabric_introspect"`
   ...
   billing usage-push (`/internal/v1/billing/usage`, `routeKind="billing_ingest"`, `worker/src/index.ts:1014`)
...
3c. **B1b `_public` edge-revocation accelerator.** In the internal-forward chain (after the shared
   ...
   (`worker/src/index.ts:2358`, forward at `worker/src/index.ts:2369`) — and ONLY on the container's `ok`
   response AND when `METADATA_KV` is bound best-effort-writes a content-hash-keyed edge blocklist entry
   via `ctx.waitUntil(writePublicBlocklistKv(...))` (`worker/src/index.ts:2411`), gated on the RESOLVED
   `content_hash` the container returns in its buffered-and-rebuilt RESPONSE body (`worker/src/index.ts:2395`,
   ...
   (`worker/src/index.ts:2410`). This collapses the
   ...
   has already applied authoritatively (`worker/src/index.ts:2385`).
4. `extractAuth` fails CLOSED (503) when `PAT_SIGNING_KEY` is absent or decodes to < 32 bytes — the
   signing key is the sole possession gate for the native plane (`worker/src/index.ts:1222-1248`).
5. PAT validation is HMAC-SHA256 fast-reject (with rotation siblings) BEFORE any D1 round-trip, then a
   D1 lookup by `token_id` and an application-side expiry check (`worker/src/index.ts:1276-1434`). The
   ...
   the distinct reason `d1_lookup_error` (`worker/src/index.ts:1414-1418`). The caller maps BOTH the config fault
   ...
   `invalid_*`) still fall through to `401` (`worker/src/index.ts:3006-3043`). NOTE: the
   `extractAuth` error-branch comment at `:1345-1347` MATCHES that behaviour — it states the caller maps
...
6. `stripClientTrustHeaders` deletes every client-suppliable trust header on every forward, then the
   Worker re-sets its own verified values (`worker/src/index.ts:744-748`).
6b. The Worker is also the PRODUCER of the `first_cli_authed` onboarding signal. Immediately after the
   ...
   (`worker/src/index.ts:3092-3098`). It lives here rather than in the container because the container's
   ...
   over the `ANALYTICS_SVC` service binding (`worker/src/index.ts:293`) to the analytics Worker's
...
7. Per-tier quota (storage SUM + monthly request-count) runs after auth and before the DO forward —
   ...
   fail-closed) (`worker/src/index.ts:3101-3430`). The tier is resolved FIRST (cache-served) and the two
   UNCACHED statements — the monthly-counter UPSERT and the storage `SUM(bytes_used)` — then travel as a
   SINGLE `db.batch()` round trip (`runQuotaBatch` — see the [edge quota & tier serving](/launch/edge-quota-tier-serving.md) concept for that module; the call site is `worker/src/index.ts:3317-3320`) rather than two
...
7b. Data-residency (`primary_region`, for region fan-out) is resolved AFTER quota and BEFORE the DO
   ...
   for this near-immutable value (`worker/src/index.ts:3480-3485`,
   ...
   rather than IAD-leaking an EU tenant to US storage (`worker/src/index.ts:3487-3504`,
   `worker/src/lib/tenant_residency_cache.ts:284-291`); a `null` region (no tenant row / no pin) preserves
   the existing `primaryRegion === undefined` IAD-local fall-through (`worker/src/index.ts:3508`). A stale
...
7c. A response returned through **this PAT-gated native DO forward** carries a `Server-Timing` header
   ...
   (`worker/src/index.ts:3851-3892`). `origin` is decomposed the same way `wdb` is, and the split spans
   ...
   (`worker/src/index.ts:3887`), forwards verbatim (`worker/src/index.ts:4070`) and completes with the
   one number only the Worker can see: `ohop = origin − Σ(container phases)`
   (`worker/src/index.ts:4064`), covering the DO dispatch + placement, the DO's own prologue, the wire
   ...
   (`worker/src/index.ts:4037`, `worker/src/index.ts:4058`), and a report that cannot be reconciled,
   its phases exceeding `origin` or a consumed phase carrying an unreadable duration, is dropped WHOLE
   as `ohop;dur=<origin>;desc="unreconciled"` (`worker/src/index.ts:4061-4062`) rather than published
...
8. The request is routed to the per-tenant DO via `idFromName(resolvedTenantId)`
   (`worker/src/index.ts:3620-3622`) and dispatched with `stub.fetch` (`worker/src/index.ts:3767`). A
   multi-region tenant may first take the LOCAL (this-region) `_system` container branch above this
   forward (`worker/src/index.ts:2169-2171`); a non-resident tenant falls through to the per-tenant DO
   derivation here.
9. The forwarded request is augmented: strip-then-set the trusted tenant-id, scope, token-prefix, and
   client-ip headers (`worker/src/index.ts:3625-3656`).
10. The whole handler is wrapped by `Sentry.withSentry`, inert until `SENTRY_DSN` is set and with
    `sendDefaultPii=false` (`worker/src/index.ts:3919-3927`); its `beforeSend`/`beforeSendTransaction`
    ...
    `[REDACTED]`), closing the WP4 PII/secret-leak gap (`worker/src/index.ts:3928-3933`).

# Invariants
- Tenant isolation is structural: the DO id is derived solely from the PAT-resolved tenant, never the
  URL path segment (`worker/src/index.ts:3620-3622`).
- A client can never smuggle a server-trust header: the strip list is applied on every forward path
  before the Worker sets its own values (`worker/src/index.ts:668-737`). The team-RBAC role header
  ...
  (`worker/src/index.ts:736`, set at `worker/src/index.ts:2918`) — this is what lets the container gate
...
- The signing-key gate is mandatory — a missing/short `PAT_SIGNING_KEY` is a 503, never a silent skip
  (`worker/src/index.ts:1222-1248`).
- The Worker forwards the D1-resolved `scope` as `x-corelink-scope` and is its sole setter
  (`worker/src/index.ts:3656`); for a find-only PAT (`pat.find_only = 1`) the forwarded value is narrowed
  to `find-missing` rather than the stored base `read-only` (ADR-0071, `worker/src/index.ts:1474`).
- On the customer (Clerk-session) forward the Worker sets `x-corelink-scope` from the D1-resolved team
  ...
  container's F-018 billing/PII gate; the Worker is the sole setter (`worker/src/index.ts:2905-2912`).
- A 404 from the DO is timing-padded to defeat cross-tenant enumeration
  (`worker/src/index.ts:3812-3820`).
- Data-residency is resolved FAIL-CLOSED before the DO forward: an unresolvable region (a D1 fault with
  ...
  (`worker/src/index.ts:3487-3504`, `worker/src/lib/tenant_residency_cache.ts:284-291`).
...
- OCI, the Stripe webhook, and the fabric-introspect / resolve-tenant / billing-ingest arms are pure
  ...
  them. `/internal/v1/auth/introspect` (`worker/src/index.ts:990`) and `/internal/v1/auth/resolve-tenant`
  (`worker/src/index.ts:1002`) share `routeKind="fabric_introspect"` + the `FABRIC_INTROSPECT_AUTH_KEY` gate;
  `/internal/v1/billing/usage` (`worker/src/index.ts:1014`) uses its own `BILLING_INGEST_AUTH_KEY`.

# Citations
1. `worker/src/index.ts:1-20` — the architecture header documenting the Worker → DO → container topology.
2. `worker/src/index.ts:483-513` — the `AuthResult` carrying the D1-resolved scope (the Worker is its sole authority).
3. `worker/src/index.ts:668-737` — the `CLIENT_TRUST_HEADERS` strip list.
4. `worker/src/index.ts:744-748` — `stripClientTrustHeaders` (delete-then-set discipline).
5. `worker/src/index.ts:796-1108` — the `matchRoute` ordered route table.
5b. `worker/src/index.ts:990` / `worker/src/index.ts:1002` / `worker/src/index.ts:1014` — the three exact-path pure-pass-through carve-outs: `/internal/v1/auth/introspect` + `/internal/v1/auth/resolve-tenant` (both `fabric_introspect`, `FABRIC_INTROSPECT_AUTH_KEY`) and `/internal/v1/billing/usage` (`billing_ingest`, `BILLING_INGEST_AUTH_KEY`).
5c. `worker/src/index.ts:2358` — the `/_internal/public/revoke` internal-forward arm (B1b): it forwards the revoke to the `_system` DO → container (`worker/src/index.ts:2369`) and, only on an `ok` container response with `METADATA_KV` bound (`worker/src/index.ts:2385`), reads the RESOLVED `content_hash` from the container's buffered RESPONSE body (`worker/src/index.ts:2395`, F-1: authoritative for both the raw `content_hash` and the `upstream_digest` revoke spaces) and, gated on it matching `^[0-9a-f]{64}$` (`worker/src/index.ts:2410`), best-effort-writes the content-hash-keyed edge blocklist KV via `ctx.waitUntil(writePublicBlocklistKv(...))` (`worker/src/index.ts:2411`) — collapsing the `_public` edge-serve revocation window from the ~60 s map-cache TTL to KV propagation, never failing the revoke on a KV fault.
6. `worker/src/index.ts:1222-1248` — `extractAuth` fail-CLOSED on absent/short `PAT_SIGNING_KEY`.
7. `worker/src/index.ts:1276-1434` — HMAC fast-reject + cached D1 lookup + expiry check (incl. the `expires_ms === 0` never-expires sentinel guard at `:1401-1406`, and the cached PAT-row lookup `verifyPatRowCached` whose `error` kind becomes `d1_lookup_error` at `:1388-1392`).
7b. `worker/src/index.ts:3006-3043` — caller reason→status mapping: BOTH `signing_key_not_configured` (config fault) AND `d1_lookup_error` (transient D1 fault) → retryable `503`; the distinct `tenant_suspended` reason → `403` (an authorization denial, `worker/src/index.ts:3033-3038`); and only then the 401 fall-through for a genuinely bad credential (`pat_not_found` / `pat_expired` / `invalid_*`). The mapping is THREE-way, not two-way — an earlier draft of this line said "every other reason → 401" while citing a range that contains the 403 arm.
8. `worker/src/index.ts:1893-1939` — the `baseHandler.fetch` entry, request-id, CORS, route match.
9. `worker/src/index.ts:1941-1958` — health short-circuit (no auth, no DO).
9b. `worker/src/index.ts:3092-3098` — the `first_cli_authed` emit site (`GET /v1/users/me`, after the PAT-resolved tenant clears the path-spoof guard), handed to `ctx.waitUntil`. The producer module is `worker/src/lib/onboarding_events.ts:170-224` (`emitFirstCliAuthed` returns `void` and swallows every error, so the emit can never add latency to — or fail — the response); every skip path is silent by design, including the one where the binding resolved to the target's default `fetch` export because `entrypoint = "AnalyticsIngest"` is missing, which degrades to a no-op rather than a `TypeError`; the deterministic id is built at `worker/src/lib/onboarding_events.ts:158-160`; the `ANALYTICS_SVC` service binding is declared on `Env` at `worker/src/index.ts:293` and bound with `entrypoint = "AnalyticsIngest"` at `wrangler.toml` `[[env.prod.services]]`. There is NO caller-side ingest key any more: the RPC path is authenticated by the platform (`ingestServerEvent` passes `trusted = true` on that basis), which is what removed the `ANALYTICS_INGEST_KEY` operator prerequisite that kept this emit dark in prod.
10. `worker/src/index.ts:3101-3430` — per-tier quota enforcement after auth, before forward.
10a. `worker/src/index.ts:3204-3209` — `resolveTenantTierCached` three-tier tier resolution (L1 isolate → L2 KV `ttier:` 60 s → L3 D1 `getTierForTenant`) for the storage-quota header + request-count cap, replacing the former inline per-request tier D1-PRIMARY read (latency WP slice 2). FAIL-OPEN like the suspend gate: an unconfirmed (`d1Error`) result is returned unchanged and NEVER cached (a transient D1 fault can't pin a tenant to 'free' — F21 preserved), so a stale worker tier is bounded (`≤ 60 s`) and never authoritative (the DO quota FSM re-derives the hard caps). The cache module is `worker/src/lib/tenant_tier_cache.ts:182`.
10b. `worker/src/index.ts:3480-3485` — `resolveTenantResidency` three-tier residency resolution (L1 isolate → L2 KV `tres:` 60 s → L3 D1) replacing the former inline `SELECT primary_region` per-request D1-PRIMARY read; FAIL-CLOSED `503 RESIDENCY_UNAVAILABLE` on the `RESIDENCY_UNRESOLVED` sentinel (`worker/src/index.ts:3487-3504`), `null` region ⇒ IAD-local (`worker/src/index.ts:3508`). The cache module is `worker/src/lib/tenant_residency_cache.ts:231-298`, whose catch serves a cached region on a transient D1 fault but returns `RESIDENCY_UNRESOLVED` (never caches the error) when none is held (`worker/src/lib/tenant_residency_cache.ts:284-291`).
11. `worker/src/index.ts:3620-3622` — `idFromName(resolvedTenantId)` per-tenant DO routing.
12. `worker/src/index.ts:3625-3656` — the augmented forward (strip-then-set trust headers).
13. `worker/src/index.ts:3656` — forwarding the D1-resolved scope as `x-corelink-scope` (narrowed to `find-missing` for a find-only PAT, ADR-0071 `worker/src/index.ts:1474`).
14. `worker/src/index.ts:3767` — `stub.fetch` dispatch to the DO.
14a. `worker/src/index.ts:3851-3892` — the `Server-Timing` emission: `auth` / `wdb` / `origin` / `total`, with `wdb` decomposed into `qtier` / `qbatch` / `qresid` (`qbatch` = the one D1 round trip carrying both quota statements; it replaced the retired `qmeter` + `qstor` pair when those two serial round trips were merged). A phase that ran is emitted even at `dur=0`; a phase that was skipped is omitted (the `-1` sentinel at `worker/src/index.ts:1926-1928`), so a cache-served phase is never mistaken for one that did not execute.
14b. `worker/src/index.ts:4036-4073` — `originSubPhases`, the merge that decomposes `origin` across the Worker↔container boundary. The container's four phases are named at `worker/src/index.ts:3990` (`opat` / `oquota` / `ostore` / `oother`) and arrive on the DO response's own `Server-Timing`, read at `worker/src/index.ts:3887`; the Worker re-emits them unchanged (`worker/src/index.ts:4070`) and derives `ohop` as `origin` minus their sum (`worker/src/index.ts:4064`). No container report ⇒ no split (`worker/src/index.ts:4037`, `worker/src/index.ts:4058`), which is what lets the Worker deploy ahead of the container image; an irreconcilable report ⇒ the whole split is refused as `ohop;dur=<origin>;desc="unreconciled"` (`worker/src/index.ts:4061-4062`). Same emission contract as the `wdb` children: a container phase that ran is forwarded even at `dur=0`, one that did not run is absent (`worker/src/index.ts:4066-4070`).
15. `worker/src/index.ts:3812-3820` — 404 timing-pad.
16. `worker/src/index.ts:3919-3927` — the `Sentry.withSentry` wrapper (inert until `SENTRY_DSN`; `sendDefaultPii=false`).
17. `worker/src/index.ts:3928-3933` — `beforeSend`/`beforeSendTransaction` → `scrubSentryEvent` (`worker/src/sentry-scrub.ts:101-188`): full-event PII/secret scrub (default-DENY sensitive keys + free-text secret/PII shapes), not just header keys.
