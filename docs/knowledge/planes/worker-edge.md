...
1. The exported handler is `baseHandler.fetch`, which resolves a request-id, handles CORS preflight,
   and matches the route before doing anything else (`worker/src/index.ts:1691-1704`).
...
3. Health routes short-circuit with no auth and no DO forward (`worker/src/index.ts:1710-1723`).
   ...
   (`worker/src/index.ts:915-916`), forward arm (`worker/src/index.ts:2267-2296`).
...
5. PAT validation is HMAC-SHA256 fast-reject (with rotation siblings) BEFORE any D1 round-trip, then a
   D1 lookup by `token_id` and an application-side expiry check (`worker/src/index.ts:1099-1237`). The
   ...
   the distinct reason `d1_lookup_error` (`worker/src/index.ts:1218-1223`). The caller maps BOTH the config fault
   ...
   `invalid_*`) still fall through to `401` (`worker/src/index.ts:2530-2561`). NOTE: the
   `extractAuth` error-branch comment at `:1220-1222` MATCHES that behaviour — it states the caller maps
...
7. Per-tier quota (storage SUM + monthly request-count) runs after auth and before the DO forward —
   ...
   fail-closed) (`worker/src/index.ts:2594-2746`).
8. The request is routed to the per-tenant DO via `idFromName(resolvedTenantId)`
   (`worker/src/index.ts:2923-2925`) and dispatched with `stub.fetch` (`worker/src/index.ts:3006`). A
   multi-region tenant may first take the LOCAL (this-region) `_system` container branch above this
   forward (`worker/src/index.ts:1864-1866`); a non-resident tenant falls through to the per-tenant DO
   derivation here.
9. The forwarded request is augmented: strip-then-set the trusted tenant-id, scope, token-prefix, and
   client-ip headers (`worker/src/index.ts:2936-2959`).
10. The whole handler is wrapped by `Sentry.withSentry`, inert until `SENTRY_DSN` is set and with
    `sendDefaultPii=false` (`worker/src/index.ts:3059-3077`); its `beforeSend`/`beforeSendTransaction`
    ...
    `[REDACTED]`), closing the WP4 PII/secret-leak gap (`worker/src/index.ts:3068-3072`).

# Invariants
- Tenant isolation is structural: the DO id is derived solely from the PAT-resolved tenant, never the
  URL path segment (`worker/src/index.ts:2923-2925`).
...
7. `worker/src/index.ts:1099-1237` — HMAC fast-reject + cached D1 lookup + expiry check (incl. the `expires_ms === 0` never-expires sentinel guard at `:1237`, and the cached PAT-row lookup `verifyPatRowCached` whose `error` kind becomes `d1_lookup_error` at `:1218-1223`).
7b. `worker/src/index.ts:2530-2561` — caller reason→status mapping: BOTH `signing_key_not_configured` (config fault) AND `d1_lookup_error` (transient D1 fault) → retryable `503`; every other reason (`pat_not_found` / `pat_expired` / `invalid_*`) → `401`.
8. `worker/src/index.ts:1691-1704` — the `baseHandler.fetch` entry, request-id, CORS, route match.
9. `worker/src/index.ts:1710-1723` — health short-circuit (no auth, no DO).
10. `worker/src/index.ts:2594-2746` — per-tier quota enforcement after auth, before forward.
11. `worker/src/index.ts:2923-2925` — `idFromName(resolvedTenantId)` per-tenant DO routing.
12. `worker/src/index.ts:2936-2959` — the augmented forward (strip-then-set trust headers).
13. `worker/src/index.ts:2959` — forwarding the D1-resolved scope as `x-corelink-scope`.
14. `worker/src/index.ts:3006` — `stub.fetch` dispatch to the DO.
15. `worker/src/index.ts:3019-3026` — 404 timing-pad.
16. `worker/src/index.ts:3059-3077` — the `Sentry.withSentry` wrapper (inert until `SENTRY_DSN`; `sendDefaultPii=false`).
17. `worker/src/index.ts:3068-3072` — `beforeSend`/`beforeSendTransaction` → `scrubSentryEvent` (`worker/src/sentry-scrub.ts:101-188`): full-event PII/secret scrub (default-DENY sensitive keys + free-text secret/PII shapes), not just header keys.
