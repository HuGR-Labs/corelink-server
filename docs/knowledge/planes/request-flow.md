...
1. The request enters `baseHandler.fetch`: request-id, CORS, then `matchRoute` selects a `RouteKind` +
   tenant (`worker/src/index.ts:1707-1720`).
2. The Worker authenticates the Bearer PAT — HMAC fast-reject then a D1 `token_id` lookup + expiry —
   ...
   forward arm `worker/src/index.ts:2292-2320`). The edge expiry check honors the
...
3. The Worker derives the per-tenant DO with `idFromName(resolvedTenantId)`, making isolation structural
   (`worker/src/index.ts:2952-2954`). A non-local-region tenant may first be routed to the LOCAL
   (this-region) `_system` container branch above this forward; the fall-through then lands on the
   per-tenant DO derivation here (`worker/src/index.ts:1888-1890`).
4. It strips any client-supplied trust headers (delete-then-set discipline), sets its own verified
   tenant-id/scope/token-prefix, and dispatches via `stub.fetch` (`worker/src/index.ts:2957-3035`).
...
3. `worker/src/index.ts:1707-1720` — the Worker `fetch` entry + `matchRoute`.
4. `worker/src/index.ts:2952-2954` — `idFromName(resolvedTenantId)` DO derivation (structural isolation).
5. `worker/src/index.ts:2965-2988` — strip-then-set trust headers on the forward.
6. `worker/src/index.ts:2957-3035` — the augmented forward + `stub.fetch` dispatch to the DO.
