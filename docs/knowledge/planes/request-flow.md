...
source_blobs:
  ...
  - "crates/corelink-container/src/routes.rs@cd24a84c87c024b8b9c601890d9d03114444492b"
...
1. The request enters `baseHandler.fetch`: request-id, CORS, then `matchRoute` selects a `RouteKind` +
   tenant (`worker/src/index.ts:1898-1944`).
2. The Worker authenticates the Bearer PAT — HMAC fast-reject then a D1 `token_id` lookup + expiry —
   resolving the trusted tenant (`worker/src/index.ts:1281-1439`). EXCEPTION (Artifact 1): the unauth
   ...
   verifiable, so this route skips the auth hop entirely (matchRoute arm `worker/src/index.ts:1095-1096`,
   forward arm `worker/src/index.ts:2729-2757`). The edge expiry check honors the
...
3. The Worker derives the per-tenant DO with `idFromName(resolvedTenantId)`, making isolation structural
   (`worker/src/index.ts:3702-3704`). A non-local-region tenant may first be routed to the LOCAL
   (this-region) `_system` container branch above this forward; the fall-through then lands on the
   per-tenant DO derivation here (`worker/src/index.ts:2174-2176`).
4. It strips any client-supplied trust headers (delete-then-set discipline), sets its own verified
   ...
   `stub.fetch` (`worker/src/index.ts:3707-3738`, `worker/src/index.ts:3849`).
4b. **F3.3 F2 SERVE short-circuits the DO→container hop entirely on a `brew`/`pip` `_public` cache HIT.**
   ...
   (`worker/src/index.ts:3783-3844`). A served HIT now sets `Content-Type: <PUBLIC_BLOB_CONTENT_TYPE>` and
   ...
   full `200` body otherwise (`worker/src/index.ts:3808-3832`). This edge HIT deliberately does **not**
   ...
   exemption for the cheapest, most-shared traffic class (`worker/src/index.ts:3800-3807`). The Worker
...
4c. **The `_public` edge-serve cache of 4b is invalidated out-of-band by `/_internal/public/revoke` (B1b).**
   ...
   + `cache_map` delete + R2 erase (`worker/src/index.ts:2374`) — and, only on the container's `ok` response
   with `METADATA_KV` bound (`worker/src/index.ts:2390`), reads the RESOLVED `content_hash` from the
   container's buffered-and-rebuilt RESPONSE body (`worker/src/index.ts:2400`, F-1: authoritative for BOTH
   ...
   `ctx.waitUntil(writePublicBlocklistKv(...))` (`worker/src/index.ts:2416`), so a revoked `_public` hash
...
7. The container's composed router (built by `build_with_factory`) receives the request and routes it to
   the matching handler (`crates/corelink-container/src/routes.rs:441-443`).
8. The shared CAS/AC handler objects — wrapped once with byte-accounting, the erasure tombstone gate,
   ...
   (`crates/corelink-container/src/routes.rs:543-652`).
9. The hop is MEASURED end to end, and the measurement is split across the same boundary the request
   ...
   `crates/corelink-container/src/routes.rs:1127-1129`); the Worker forwards those four and derives the
   one term only it can see, `ohop = origin − Σ(container phases)` — the dispatch, the DO's prologue and
   the wire (`worker/src/index.ts:4146`). Recording is a task-local ledger, so an instrumented region
...
- The container re-verifies possession at the shared handler chokepoint rather than trusting the hop
  blindly (`crates/corelink-container/src/routes.rs:543-652`).
...
- The `origin` split always reconciles: the container's residue phase is computed against its OWN
  ...
  make add up (`worker/src/index.ts:4143-4144`).
- The DO→container hop is not mandatory on every request: a `brew`/`pip` `_public` edge-serve HIT sets
  ...
  (`worker/src/index.ts:3783-3844`). An edge-served HIT is exempt from the container's $-ceiling gate but
  not from request-count/storage quota, which the Worker has already enforced upstream of this block
  (`worker/src/index.ts:3800-3807`).
...
2. `worker/src/index.ts:1281-1439` — edge PAT auth (HMAC fast-reject + D1 lookup + expiry).
3. `worker/src/index.ts:1898-1944` — the Worker `fetch` entry + `matchRoute`.
4. `worker/src/index.ts:3702-3704` — `idFromName(resolvedTenantId)` DO derivation (structural isolation).
5. `worker/src/index.ts:3707-3738` — strip-then-set trust headers on the forward.
6. `worker/src/index.ts:3707-3738` — the augmented forward (strip-then-set trust headers); `worker/src/index.ts:3849` — the `stub.fetch` dispatch to the DO (reached only when 6b did not already set `doResponse`).
6b. `worker/src/index.ts:3783-3844` — F3.3 F2 SERVE: the `EDGE_PUBLIC_READ === "serve"` short-circuit on a `brew`/`pip` `_public` HIT — `readPublicHit` at `worker/src/index.ts:3799`, the $-ceiling exemption rationale at `worker/src/index.ts:3800-3807`, the `Content-Type`/`Accept-Ranges` headers at `worker/src/index.ts:3813-3815`, and the `parseByteRange`-driven `200`/`206`/`416` branch at `worker/src/index.ts:3817-3832`; the deliberate omission of `stOrigin*` (so `Server-Timing` proves the bypass by omitting `origin`) at `worker/src/index.ts:3790-3791`.
6c. `worker/src/index.ts:2374` — the `/_internal/public/revoke` invalidation seam (B1b): the revoke forwards to the `_system` DO → container (authoritative D1 blocklist + `cache_map` delete + R2 erase) and, on an `ok` response with `METADATA_KV` bound (`worker/src/index.ts:2390`), reads the RESOLVED `content_hash` from the container's buffered RESPONSE body (`worker/src/index.ts:2400`, F-1: authoritative for both the raw `content_hash` and the `upstream_digest` revoke spaces) and best-effort-writes the content-hash-keyed edge blocklist KV via `ctx.waitUntil(writePublicBlocklistKv(...))` (`worker/src/index.ts:2416`), collapsing the `_public` edge-serve revocation window from the ~60 s map-cache TTL to KV propagation without ever failing the revoke on a KV fault.
...
10. `crates/corelink-container/src/routes.rs:441-443` — the container's composed router receiving the request.
11. `crates/corelink-container/src/routes.rs:543-652` — the shared CAS/AC handlers (accounting + tombstone + PAT gate) executing the op.
12. `crates/corelink-container/src/origin_timing.rs:265-273` — the container's outermost data-plane layer: scope a task-local phase ledger over the request, clock the whole of it, and stamp the phases on the response's `Server-Timing`. Wired last (so it wraps every inner layer) at `crates/corelink-container/src/routes.rs:1127-1129`; the residue that makes the parts sum to the whole is `crates/corelink-container/src/origin_timing.rs:225`; `timed` is the pass-through recorder at `crates/corelink-container/src/origin_timing.rs:247-252`.
