...
1. The request enters `baseHandler.fetch`: request-id, CORS, then `matchRoute` selects a `RouteKind` +
   tenant (`worker/src/index.ts:1668-1681`).
2. The Worker authenticates the Bearer PAT — HMAC fast-reject then a D1 `token_id` lookup + expiry —
   resolving the trusted tenant (`worker/src/index.ts:1070-1217`). EXCEPTION (Artifact 1): the unauth
   ...
   verifiable, so this route skips the auth hop entirely (matchRoute arm `worker/src/index.ts:886-887`,
   forward arm `worker/src/index.ts:2247-2269`). The edge expiry check honors the
...
3. The Worker derives the per-tenant DO with `idFromName(resolvedTenantId)`, making isolation structural
   (`worker/src/index.ts:2840-2842`). A non-local-region tenant may first be routed to the LOCAL
   (this-region) `_system` container branch above this forward; the fall-through then lands on the
   per-tenant DO derivation here (`worker/src/index.ts:1843-1845`).
4. It strips any client-supplied trust headers (delete-then-set discipline), sets its own verified
   tenant-id/scope/token-prefix, and dispatches via `stub.fetch` (`worker/src/index.ts:2852-2909`).
...
7. The container's composed router (built by `build_with_factory`) receives the request and routes it to
   the matching handler (`crates/corelink-container/src/routes.rs:381-389`).
...
- The container re-verifies possession at the shared handler chokepoint rather than trusting the hop
  blindly (`crates/corelink-container/src/routes.rs:450-559`).
...
2. `worker/src/index.ts:1070-1217` — edge PAT auth (HMAC fast-reject + D1 lookup + expiry).
3. `worker/src/index.ts:1668-1681` — the Worker `fetch` entry + `matchRoute`.
4. `worker/src/index.ts:2840-2842` — `idFromName(resolvedTenantId)` DO derivation (structural isolation).
5. `worker/src/index.ts:2852-2888` — strip-then-set trust headers on the forward.
6. `worker/src/index.ts:2852-2913` — the augmented forward + `stub.fetch` dispatch to the DO.
...
10. `crates/corelink-container/src/routes.rs:381-389` — the container's composed router receiving the request.
11. `crates/corelink-container/src/routes.rs:450-559` — the shared CAS/AC handlers (accounting + tombstone + PAT gate) executing the op.
