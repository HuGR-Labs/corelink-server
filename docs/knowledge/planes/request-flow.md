...
3. The Worker derives the per-tenant DO with `idFromName(resolvedTenantId)`, making isolation structural
   (`worker/src/index.ts:2548-2551`).
4. It strips any client-supplied trust headers (delete-then-set discipline), sets its own verified
   tenant-id/scope/token-prefix, and dispatches via `stub.fetch` (`worker/src/index.ts:2553-2615`).
...
4. `worker/src/index.ts:2548-2551` — `idFromName(resolvedTenantId)` DO derivation (structural isolation).
5. `worker/src/index.ts:2553-2611` — strip-then-set trust headers on the forward.
6. `worker/src/index.ts:2553-2616` — the augmented forward + `stub.fetch` dispatch to the DO.
