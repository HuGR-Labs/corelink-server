...
3. The Worker derives the per-tenant DO with `idFromName(resolvedTenantId)`, making isolation structural
   (`worker/src/index.ts:3720-3722`). A non-local-region tenant may first be routed to the LOCAL
...
4. It strips any client-supplied trust headers (delete-then-set discipline), sets its own verified
   ...
   `stub.fetch` (`worker/src/index.ts:3725-3756`, `worker/src/index.ts:3867`).
4b. **F3.3 F2 SERVE short-circuits the DO→container hop entirely on a `brew`/`pip` `_public` cache HIT.**
   ...
   (`worker/src/index.ts:3801-3862`). A served HIT now sets `Content-Type: <PUBLIC_BLOB_CONTENT_TYPE>` and
   ...
   full `200` body otherwise (`worker/src/index.ts:3826-3850`). This edge HIT deliberately does **not**
   ...
   exemption for the cheapest, most-shared traffic class (`worker/src/index.ts:3818-3825`). The Worker
...
9. The hop is MEASURED end to end, and the measurement is split across the same boundary the request
   ...
   the wire (`worker/src/index.ts:4164`). Recording is a task-local ledger, so an instrumented region
...
- The `origin` split always reconciles: the container's residue phase is computed against its OWN
  ...
  make add up (`worker/src/index.ts:4161-4162`).
- The DO→container hop is not mandatory on every request: a `brew`/`pip` `_public` edge-serve HIT sets
  ...
  (`worker/src/index.ts:3801-3862`). An edge-served HIT is exempt from the container's $-ceiling gate but
  not from request-count/storage quota, which the Worker has already enforced upstream of this block
  (`worker/src/index.ts:3818-3825`).
...
4. `worker/src/index.ts:3720-3722` — `idFromName(resolvedTenantId)` DO derivation (structural isolation).
5. `worker/src/index.ts:3725-3756` — strip-then-set trust headers on the forward.
6. `worker/src/index.ts:3725-3756` — the augmented forward (strip-then-set trust headers); `worker/src/index.ts:3867` — the `stub.fetch` dispatch to the DO (reached only when 6b did not already set `doResponse`).
6b. `worker/src/index.ts:3801-3862` — F3.3 F2 SERVE: the `EDGE_PUBLIC_READ === "serve"` short-circuit on a `brew`/`pip` `_public` HIT — `readPublicHit` at `worker/src/index.ts:3817`, the $-ceiling exemption rationale at `worker/src/index.ts:3818-3825`, the `Content-Type`/`Accept-Ranges` headers at `worker/src/index.ts:3831-3833`, and the `parseByteRange`-driven `200`/`206`/`416` branch at `worker/src/index.ts:3835-3850`; the deliberate omission of `stOrigin*` (so `Server-Timing` proves the bypass by omitting `origin`) at `worker/src/index.ts:3808-3809`.
