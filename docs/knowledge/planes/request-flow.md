...
1. The request enters `baseHandler.fetch`: request-id, CORS, then `matchRoute` selects a `RouteKind` +
   tenant (`worker/src/index.ts:1866-1912`).
2. The Worker authenticates the Bearer PAT — HMAC fast-reject then a D1 `token_id` lookup + expiry —
   resolving the trusted tenant (`worker/src/index.ts:1249-1407`). EXCEPTION (Artifact 1): the unauth
   ...
   verifiable, so this route skips the auth hop entirely (matchRoute arm `worker/src/index.ts:1063-1064`,
   forward arm `worker/src/index.ts:2681-2709`). The edge expiry check honors the
...
3. The Worker derives the per-tenant DO with `idFromName(resolvedTenantId)`, making isolation structural
   (`worker/src/index.ts:3517-3519`). A non-local-region tenant may first be routed to the LOCAL
   (this-region) `_system` container branch above this forward; the fall-through then lands on the
   per-tenant DO derivation here (`worker/src/index.ts:2142-2144`).
4. It strips any client-supplied trust headers (delete-then-set discipline), sets its own verified
   ...
   `stub.fetch` (`worker/src/index.ts:3522-3553`, `worker/src/index.ts:3664`).
4b. **F3.3 F2 SERVE short-circuits the DO→container hop entirely on a `brew`/`pip` `_public` cache HIT.**
   ...
   (`worker/src/index.ts:3598-3659`). A served HIT now sets `Content-Type: <PUBLIC_BLOB_CONTENT_TYPE>` and
   ...
   full `200` body otherwise (`worker/src/index.ts:3623-3647`). This edge HIT deliberately does **not**
   ...
   exemption for the cheapest, most-shared traffic class (`worker/src/index.ts:3615-3622`). The Worker
...
4c. **The `_public` edge-serve cache of 4b is invalidated out-of-band by `/_internal/public/revoke` (B1b).**
   ...
   + `cache_map` delete + R2 erase (`worker/src/index.ts:2342`) — and, only on the container's `ok` response
   with `METADATA_KV` bound, best-effort-writes a content-hash-keyed edge blocklist KV via
   `ctx.waitUntil(writePublicBlocklistKv(...))` (`worker/src/index.ts:2368`), so a revoked `_public` hash
   ...
   body just falls back to the map-cache window (`worker/src/index.ts:2358`).
...
9. The hop is MEASURED end to end, and the measurement is split across the same boundary the request
   ...
   the wire (`worker/src/index.ts:3954`). Recording is a task-local ledger, so an instrumented region
...
- The `origin` split always reconciles: the container's residue phase is computed against its OWN
  ...
  make add up (`worker/src/index.ts:3951-3952`).
- The DO→container hop is not mandatory on every request: a `brew`/`pip` `_public` edge-serve HIT sets
  ...
  (`worker/src/index.ts:3598-3659`). An edge-served HIT is exempt from the container's $-ceiling gate but
  not from request-count/storage quota, which the Worker has already enforced upstream of this block
  (`worker/src/index.ts:3615-3622`).
...
2. `worker/src/index.ts:1249-1407` — edge PAT auth (HMAC fast-reject + D1 lookup + expiry).
3. `worker/src/index.ts:1866-1912` — the Worker `fetch` entry + `matchRoute`.
4. `worker/src/index.ts:3517-3519` — `idFromName(resolvedTenantId)` DO derivation (structural isolation).
5. `worker/src/index.ts:3522-3553` — strip-then-set trust headers on the forward.
6. `worker/src/index.ts:3522-3553` — the augmented forward (strip-then-set trust headers); `worker/src/index.ts:3664` — the `stub.fetch` dispatch to the DO (reached only when 6b did not already set `doResponse`).
6b. `worker/src/index.ts:3598-3659` — F3.3 F2 SERVE: the `EDGE_PUBLIC_READ === "serve"` short-circuit on a `brew`/`pip` `_public` HIT — `readPublicHit` at `worker/src/index.ts:3614`, the $-ceiling exemption rationale at `worker/src/index.ts:3615-3622`, the `Content-Type`/`Accept-Ranges` headers at `worker/src/index.ts:3628-3630`, and the `parseByteRange`-driven `200`/`206`/`416` branch at `worker/src/index.ts:3632-3647`; the deliberate omission of `stOrigin*` (so `Server-Timing` proves the bypass by omitting `origin`) at `worker/src/index.ts:3605-3606`.
6c. `worker/src/index.ts:2342` — the `/_internal/public/revoke` invalidation seam (B1b): the revoke forwards to the `_system` DO → container (authoritative D1 blocklist + `cache_map` delete + R2 erase) and, on an `ok` response with `METADATA_KV` bound (`worker/src/index.ts:2358`), best-effort-writes the content-hash-keyed edge blocklist KV via `ctx.waitUntil(writePublicBlocklistKv(...))` (`worker/src/index.ts:2368`), collapsing the `_public` edge-serve revocation window from the ~60 s map-cache TTL to KV propagation without ever failing the revoke on a KV fault.
