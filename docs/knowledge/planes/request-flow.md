...
checkpoint_sha: "ed4722a79bf4e3b44e67984ead47378bd9f55003"
provenance: "AUTHORED"
...
9. The hop is MEASURED end to end, and the measurement is split across the same boundary the request
   ...
   `crates/corelink-container/src/routes/build.rs:659-661`); the Worker forwards those four and derives the
...
9c. **`ortier` and `oaudit` are NOT gated — they publish unconditionally.** Both sat behind the same
   ...
   (`crates/corelink-container/src/storage/d1_audit_sink.rs:412-419`). Separately, and UNGATED, the same
...
16. `crates/corelink-container/src/storage/d1_audit_sink.rs:412-419` — `D1AuditOutboxSink::write_blocking`: the choke point every SYNC native CAS/AC `AuditSink::emit`/`append` call routes through, timed into `Phase::Audit` (`oaudit`) via `PhaseScope::enter`.
...
24. `crates/corelink-container/src/storage/d1_audit_sink.rs:367-390` — `append_batch_async` plus `build_batch_statements`: the audit half of 21 — N rows in bounded JSON1 statements, zero statements for an empty batch and, like 19, deliberately without its own `PhaseScope`.
