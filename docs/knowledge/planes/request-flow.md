...
1. The request enters `baseHandler.fetch`: request-id, CORS, then `matchRoute` selects a `RouteKind` +
   tenant (`worker/src/index.ts:1929-1997`).
2. The Worker authenticates the Bearer PAT — HMAC fast-reject then a D1 `token_id` lookup + expiry —
   resolving the trusted tenant (`worker/src/index.ts:1312-1470`). EXCEPTION (Artifact 1): the unauth
   ...
   verifiable, so this route skips the auth hop entirely (matchRoute arm `worker/src/index.ts:1126-1127`,
   forward arm `worker/src/index.ts:2782-2810`). The edge expiry check honors the
...
3. The Worker derives the per-tenant DO with `idFromName(resolvedTenantId)`, making isolation structural
   (`worker/src/index.ts:3854-3856`). A non-local-region tenant may first be routed to the LOCAL
   (this-region) `_system` container branch above this forward; the fall-through then lands on the
   per-tenant DO derivation here (`worker/src/index.ts:2227-2229`).
4. It strips any client-supplied trust headers (delete-then-set discipline), sets its own verified
   ...
   `stub.fetch` (`worker/src/index.ts:3872-3903`, `worker/src/index.ts:4014`).
4b. **F3.3 F2 SERVE short-circuits the DO→container hop entirely on a `brew`/`pip` `_public` cache HIT.**
   ...
   (`worker/src/index.ts:3948-4009`). A served HIT now sets `Content-Type: <PUBLIC_BLOB_CONTENT_TYPE>` and
   ...
   full `200` body otherwise (`worker/src/index.ts:3973-3997`). This edge HIT deliberately does **not**
   ...
   exemption for the cheapest, most-shared traffic class (`worker/src/index.ts:3965-3972`). The Worker
...
4c. **The `_public` edge-serve cache of 4b is invalidated out-of-band by `/_internal/public/revoke` (B1b).**
   ...
   + `cache_map` delete + R2 erase (`worker/src/index.ts:2427`) — and, only on the container's `ok` response
   with `METADATA_KV` bound (`worker/src/index.ts:2443`), reads the RESOLVED `content_hash` from the
   container's buffered-and-rebuilt RESPONSE body (`worker/src/index.ts:2453`, F-1: authoritative for BOTH
   ...
   `ctx.waitUntil(writePublicBlocklistKv(...))` (`worker/src/index.ts:2469`), so a revoked `_public` hash
...
4d. **The `wdb` window also carries an optional P3 EDGE_DO_METER hop — a SEPARATE DO pair from the
   ...
   `worker/src/index.ts:3258`), the Worker AWAITS `serveViaDO(...)` (the request-meter shard/coordinator
   pair) inside a `try/finally` so the elapsed time (`stQDoMs`) is captured even on the fail-open `catch`
   path (`worker/src/index.ts:3311-3357`). That duration is reported as the `qdo` Server-Timing phase,
...
9. The hop is MEASURED end to end, and the measurement is split across the same boundary the request
   ...
   the wire (`worker/src/index.ts:4555`). Recording is a task-local ledger, so an instrumented region
...
- The `origin` split always reconciles: the container's residue phase is computed against its OWN
  ...
  make add up (`worker/src/index.ts:4552-4553`). That reconciliation holds with `oargon`/`opermit`/
...
- The DO→container hop is not mandatory on every request: a `brew`/`pip` `_public` edge-serve HIT sets
  ...
  (`worker/src/index.ts:3948-4009`). An edge-served HIT is exempt from the container's $-ceiling gate but
  not from request-count/storage quota, which the Worker has already enforced upstream of this block
  (`worker/src/index.ts:3965-3972`).
...
2. `worker/src/index.ts:1312-1470` — edge PAT auth (HMAC fast-reject + D1 lookup + expiry).
3. `worker/src/index.ts:1929-1997` — the Worker `fetch` entry + `matchRoute`.
4. `worker/src/index.ts:3854-3856` — `idFromName(resolvedTenantId)` DO derivation (structural isolation).
5. `worker/src/index.ts:3872-3903` — strip-then-set trust headers on the forward.
6. `worker/src/index.ts:3872-3903` — the augmented forward (strip-then-set trust headers); `worker/src/index.ts:4014` — the `stub.fetch` dispatch to the DO (reached only when 6b did not already set `doResponse`).
6b. `worker/src/index.ts:3948-4009` — F3.3 F2 SERVE: the `EDGE_PUBLIC_READ === "serve"` short-circuit on a `brew`/`pip` `_public` HIT — `readPublicHit` at `worker/src/index.ts:3964`, the $-ceiling exemption rationale at `worker/src/index.ts:3965-3972`, the `Content-Type`/`Accept-Ranges` headers at `worker/src/index.ts:3978-3980`, and the `parseByteRange`-driven `200`/`206`/`416` branch at `worker/src/index.ts:3982-3997`; the deliberate omission of `stOrigin*` (so `Server-Timing` proves the bypass by omitting `origin`) at `worker/src/index.ts:3955-3956`.
6c. `worker/src/index.ts:2427` — the `/_internal/public/revoke` invalidation seam (B1b): the revoke forwards to the `_system` DO → container (authoritative D1 blocklist + `cache_map` delete + R2 erase) and, on an `ok` response with `METADATA_KV` bound (`worker/src/index.ts:2443`), reads the RESOLVED `content_hash` from the container's buffered RESPONSE body (`worker/src/index.ts:2453`, F-1: authoritative for both the raw `content_hash` and the `upstream_digest` revoke spaces) and best-effort-writes the content-hash-keyed edge blocklist KV via `ctx.waitUntil(writePublicBlocklistKv(...))` (`worker/src/index.ts:2469`), collapsing the `_public` edge-serve revocation window from the ~60 s map-cache TTL to KV propagation without ever failing the revoke on a KV fault.
6d. `worker/src/index.ts:3258` — `meter = !isFanout && requestQuotaEnabled`, the gate that decides whether the P3 `qdo` hop runs at all; `worker/src/index.ts:3311-3357` — the awaited `serveViaDO(...)` call timed in a `try/finally` (`stQDoMs`, captured even on the fail-open `catch`); `worker/src/index.ts:4202-4230` — the `qdo`/`qother` emission, gated on `SERVER_TIMING_WDB_DETAIL === "on"`.
