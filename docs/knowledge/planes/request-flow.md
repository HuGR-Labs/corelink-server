...
1. The request enters `baseHandler.fetch`: request-id, CORS, then `matchRoute` selects a `RouteKind` +
   tenant (`worker/src/index.ts:1605-1618`).
2. The Worker authenticates the Bearer PAT — HMAC fast-reject then a D1 `token_id` lookup + expiry —
   resolving the trusted tenant (`worker/src/index.ts:1007-1154`). EXCEPTION (Artifact 1): the unauth
   ...
   verifiable, so this route skips the auth hop entirely (matchRoute arm `worker/src/index.ts:884-885`,
   forward arm `worker/src/index.ts:2184-2206`). The edge expiry check honors the
...
3. The Worker derives the per-tenant DO with `idFromName(resolvedTenantId)`, making isolation structural
   (`worker/src/index.ts:2773-2775`). A non-local-region tenant may first be routed to the LOCAL
   (this-region) `_system` container branch above this forward; the fall-through then lands on the
   per-tenant DO derivation here (`worker/src/index.ts:1780-1782`).
4. It strips any client-supplied trust headers (delete-then-set discipline), sets its own verified
   tenant-id/scope/token-prefix, and dispatches via `stub.fetch` (`worker/src/index.ts:2785-2842`).
...
7. The container's composed router (built by `build_with_factory`) receives the request and routes it to
   the matching handler (`crates/corelink-container/src/routes.rs:371-379`).
...
- The container re-verifies possession at the shared handler chokepoint rather than trusting the hop
  blindly (`crates/corelink-container/src/routes.rs:440-549`).
...
2. `worker/src/index.ts:1007-1154` — edge PAT auth (HMAC fast-reject + D1 lookup + expiry).
3. `worker/src/index.ts:1605-1618` — the Worker `fetch` entry + `matchRoute`.
4. `worker/src/index.ts:2773-2775` — `idFromName(resolvedTenantId)` DO derivation (structural isolation).
5. `worker/src/index.ts:2785-2821` — strip-then-set trust headers on the forward.
6. `worker/src/index.ts:2785-2846` — the augmented forward + `stub.fetch` dispatch to the DO.
...
10. `crates/corelink-container/src/routes.rs:371-379` — the container's composed router receiving the request.
11. `crates/corelink-container/src/routes.rs:440-549` — the shared CAS/AC handlers (accounting + tombstone + PAT gate) executing the op.
