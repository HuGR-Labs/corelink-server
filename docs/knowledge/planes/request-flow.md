...
2. The Worker authenticates the Bearer PAT — HMAC fast-reject then a D1 `token_id` lookup + expiry —
   ...
   forward arm `worker/src/index.ts:2053-2077`). The edge expiry check honors the
...
3. The Worker derives the per-tenant DO with `idFromName(resolvedTenantId)`, making isolation structural
   (`worker/src/index.ts:2643-2646`).
4. It strips any client-supplied trust headers (delete-then-set discipline), sets its own verified
   tenant-id/scope/token-prefix, and dispatches via `stub.fetch` (`worker/src/index.ts:2647-2710`).
...
- The `internal` route arm (not the cache data plane traced here) has ONE multi-hop exception: a DSR
  erase originating at this worker fans out to every regional worker (`PROD_LHR`/`_SAM`/`_NRT`/`_SYD`
  service bindings) in addition to the local DO hop, and fails CLOSED 502 unless the local sweep AND
  every region confirm (`worker/src/index.ts:1676-1679`, `worker/src/index.ts:1733-1734`) — see
  [the worker edge plane](/planes/worker-edge.md).
...
4. `worker/src/index.ts:2643-2646` — `idFromName(resolvedTenantId)` DO derivation (structural isolation).
5. `worker/src/index.ts:2647-2705` — strip-then-set trust headers on the forward.
6. `worker/src/index.ts:2647-2710` — the augmented forward + `stub.fetch` dispatch to the DO.
