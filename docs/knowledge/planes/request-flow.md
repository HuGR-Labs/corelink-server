...
1. The request enters `baseHandler.fetch`: request-id, CORS, then `matchRoute` selects a `RouteKind` +
   tenant (`worker/src/index.ts:1703-1716`).
2. The Worker authenticates the Bearer PAT — HMAC fast-reject then a D1 `token_id` lookup + expiry —
   resolving the trusted tenant (`worker/src/index.ts:1099-1254`). EXCEPTION (Artifact 1): the unauth
   ...
   forward arm `worker/src/index.ts:2288-2316`). The edge expiry check honors the
...
3. The Worker derives the per-tenant DO with `idFromName(resolvedTenantId)`, making isolation structural
   (`worker/src/index.ts:2943-2945`). A non-local-region tenant may first be routed to the LOCAL
   (this-region) `_system` container branch above this forward; the fall-through then lands on the
   per-tenant DO derivation here (`worker/src/index.ts:1884-1886`).
4. It strips any client-supplied trust headers (delete-then-set discipline), sets its own verified
   tenant-id/scope/token-prefix, and dispatches via `stub.fetch` (`worker/src/index.ts:2948-3026`).
...
2. `worker/src/index.ts:1099-1254` — edge PAT auth (HMAC fast-reject + D1 lookup + expiry).
3. `worker/src/index.ts:1703-1716` — the Worker `fetch` entry + `matchRoute`.
4. `worker/src/index.ts:2943-2945` — `idFromName(resolvedTenantId)` DO derivation (structural isolation).
5. `worker/src/index.ts:2956-2979` — strip-then-set trust headers on the forward.
6. `worker/src/index.ts:2948-3026` — the augmented forward + `stub.fetch` dispatch to the DO.
