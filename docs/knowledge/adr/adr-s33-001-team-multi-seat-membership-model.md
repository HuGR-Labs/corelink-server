...
CoreLink sells a paid `team` tier, but the feature does not exist — the prod handler serves `team list` as a single synthesized owner row and `team invite` as a 501, so the multi-seat journeys are honestly gated. This ADR (Proposed) fixes the design so the feature can be built for real: an additive custom membership layer rather than migrating every solo tenant to Clerk Organizations, which would be a high-risk pre-launch identity rewrite. It exists to close the charging-for-an-unbuilt-feature gap without destabilising the working solo-tenant model. Related: [the PAT moat](/auth/pat-moat.md) and [the D1 PAT store](/auth/d1-pat-store.md) that member PATs reuse unchanged.
