...
two statements, each with the exact `additive-trigger-replacement: ADR-0143`
annotation required by `scripts/check_migrations_additive.py`:
...
The checker constrains the exception by migration path, complete statement,
trigger name, and annotation.  It rejects every other trigger drop and all
table, index, column, and other destructive DDL.  The replacement preserves
