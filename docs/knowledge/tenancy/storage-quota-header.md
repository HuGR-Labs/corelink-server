...
- `STORAGE_QUOTA_HEADER` (`x-corelink-storage-quota-bytes`) is set SOLELY by the Worker and stripped from
  ...
  (`crates/corelink-container/src/byte_accounting.rs:87-96`).
- `storage_quota_from_headers` parses it fail-CLOSED: absent/empty/non-ASCII/non-`i64`/negative → `None`
  ...
  (`crates/corelink-container/src/byte_accounting.rs:98-112`).
- After a successful write the handler calls an atomic DB-side check-and-accrue
  ...
  `crates/corelink-container/src/byte_accounting.rs:352-374`, whose serialized cap predicate
  (`WHERE ?5 = 0 OR tenant_storage_state.bytes_used + ?3 <= ?5`) is at
  `crates/corelink-container/src/byte_accounting.rs:358-365`.
- `AccrueOutcome::OverCap` means the caller must reject the write (bytes NOT counted) and `Indeterminate`
  ...
  (`crates/corelink-container/src/byte_accounting.rs:114-132`).
- The size reserved is the **committed** (stored) size, not the raw request length: for a BYOK-`active`
  tenant the R2 object is the convergent ciphertext blob (plaintext + the fixed 32-byte BYOK envelope
  overhead), so the decorator reserves THAT size and the delete path releases the same — `reserve ==
  release`, so `bytes_used` cannot drift once encryption engages. A non-BYOK / inactive tenant reserves
  the plaintext length, byte-identical to before; a config-read error fails CLOSED. The sizing decision is
  `byok_committed_len` (`crates/corelink-container/src/byte_accounting.rs:598-622`).
...
- The cap header is server-trusted only — set by the Worker and stripped from client input
  (`crates/corelink-container/src/byte_accounting.rs:87-90`).
- A fresh tenant with an indeterminate cap is NEVER seeded uncapped; absence fails CLOSED rather than
  defaulting to unlimited (`crates/corelink-container/src/byte_accounting.rs:122-130`).
- The cap check is serialized with the increment in one SQL statement, so two concurrent over-cap writes
  cannot both pass (`crates/corelink-container/src/byte_accounting.rs:358-365`).
...
1. `crates/corelink-container/src/byte_accounting.rs:352-374` — the executed atomic check-and-accrue SQL (DB-side add via INSERT…ON CONFLICT…RETURNING).
2. `crates/corelink-container/src/byte_accounting.rs:358-365` — the cap check serialized with the increment (the `WHERE` predicate in the one statement).
...
6. `crates/corelink-container/src/byte_accounting.rs:87-90` — `STORAGE_QUOTA_HEADER` is server-trusted, stripped from client input.
7. `crates/corelink-container/src/byte_accounting.rs:87-96` — the header name + value semantics.
8. `crates/corelink-container/src/byte_accounting.rs:98-112` — fail-CLOSED header parse (`None` indeterminate, `"0"` unlimited).
9. `crates/corelink-container/src/byte_accounting.rs:114-132` — `AccrueOutcome` (`OverCap` / `Indeterminate` fail-closed).
10. `crates/corelink-container/src/byte_accounting.rs:122-130` — a fresh row is never seeded uncapped from absence.
