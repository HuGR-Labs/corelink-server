...
- The batch variant charges `n * cost_each` in ONE atomic check using `saturating_mul` to stop integer
  overflow from an untrusted batch size (`crates/corelink-container/src/tenant_quota.rs:755-761`).
...
- The DURABLE accrue is a serialized DB-side increment (the `D1QuotaStore` override), so concurrent ops
  ...
  (`crates/corelink-container/src/tenant_quota.rs:816-876`). The in-memory `LeasedQuotaStore` in front
  ...
  (`crates/corelink-container/src/tenant_quota.rs:126-127`, `crates/corelink-container/src/tenant_quota.rs:355-502`).
...
7. `crates/corelink-container/src/tenant_quota.rs:755-761` — `check_batch` atomic charge with `saturating_mul`.
8. `crates/corelink-container/src/tenant_quota.rs:784-790` — clock-unavailable `503` fail-close.
9. `crates/corelink-container/src/tenant_quota.rs:801-808` — store-unavailable `503` fail-close.
10. `crates/corelink-container/src/tenant_quota.rs:816-876` — atomic roll + check-and-accrue (no lost-update / over-admission).
11. `crates/corelink-container/src/tenant_quota.rs:900-908` — over-ceiling `402 Payment Required` (guard mapping).
12. `crates/corelink-container/src/tenant_quota.rs:252-273` — the `QuotaStore` trait DEFAULT `check_and_accrue`: a non-atomic `get`→check→`accrue` two-step with a TOCTOU window (the per-op atomicity is store-specific to the override below).
13. `crates/corelink-container/src/tenant_quota.rs:1060-1088` — `D1QuotaStore::check_and_accrue`: the single atomic `UPDATE … WHERE accrued + ?delta <= budget RETURNING …` — the SOLE durable ceiling authority.
...
15. `crates/corelink-container/src/tenant_quota.rs:355-502` — `LeasedQuotaStore`: the in-memory 16-op budget lease (amortised D1 round-trip, bounded ~0.32% overshoot, over-charge-not-over-serve, fail-CLOSED on drained+unreachable).
