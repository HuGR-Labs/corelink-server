...
checkpoint_sha: "b3f7513cf3bc4b3a4d4fa9c659aea4fc77b30835"
provenance: "AUTHORED"
...
- The default ceiling is **effectively-unlimited** (`$1,000,000/mo` = `1_000_000_000_000` micro-dollars);
  ...
  backstop (primarily for the unbounded team/enterprise tiers) (`crates/corelink-container/src/tenant_quota.rs:79-84`).
- `QuotaGuard::check` reads the wall clock first and fail-CLOSES with `503` when it is unavailable
  ...
  (`crates/corelink-container/src/tenant_quota.rs:1090-1098`).
- The LAST per-op D1 READ is now served from the lease when warm: BEFORE the durable `get`, `check`
  ...
  over-serve bound is unchanged (`crates/corelink-container/src/tenant_quota.rs:1106-1128`,
  `crates/corelink-container/src/tenant_quota.rs:430-437`).
- A quota-store transport/decode error also returns `503` — a billable op that cannot be cost-checked is
  never served (`crates/corelink-container/src/tenant_quota.rs:1132-1140`).
- The DURABLE ceiling authority is an atomic DB-side `check_and_accrue` (`UPDATE … WHERE accrued + ?delta <= budget RETURNING …`), so the inner store can never durably exceed `accrued + delta <= budget`; over the ceiling returns `402` (`crates/corelink-container/src/tenant_quota.rs:1414-1442`). **CF-4 hardened the trait floor: the `QuotaStore` trait's DEFAULT `check_and_accrue` is no longer a non-atomic `get` → check → `accrue` two-step — it now FAILS CLOSED (returns `Err`, mapped to `503`), so a future non-D1 backend that forgets to override can never silently over-admit past the `$`-ceiling; the `InMemoryQuotaStore` carries an explicit atomic override under its `Mutex` and the `D1QuotaStore` override is unchanged (`crates/corelink-container/src/tenant_quota.rs:285-302`, `crates/corelink-container/src/tenant_quota.rs:1321-1344`).**
- **Production does NOT call the durable store on every op.** `quota_guard_from_env` fronts `D1QuotaStore` with a `LeasedQuotaStore` (`crates/corelink-container/src/tenant_quota.rs:137-149`): the hot path pre-DEBITS a 16-op chunk into an in-memory lease via ONE inner `check_and_accrue`, then serves subsequent ops FROM MEMORY without a per-op D1 round-trip — both the accrue-write and, WP-2a step 2, the rolling-decision read (via `try_serve_from_lease`) — refilling (with a halving partial-lease fallback near the ceiling) when the lease drains (`crates/corelink-container/src/tenant_quota.rs:436-1018`). Consequences, all by design: (1) **over-CHARGE, never over-SERVE** — the full chunk is debited durably UP FRONT, so a near-ceiling refill may CHARGE for ops it never serves (over-charge), and a crash loses the already-debited unused tail (the tenant is then slightly under-served for budget already spent) — but no op is ever served without first being paid for in D1. (2) **bounded overshoot ~0.32%** — `16 ops × $0.001 = $0.016` against the `$5/mo` tripwire is the worst-case crash-tail. (3) **up to ~16-op staleness** — a durable budget change (ceiling drop, or charges from another container) is not seen until the current lease drains. The durable D1 atomic `accrued + delta <= budget` remains the SOLE ceiling authority, so this stays a hard cost-blast-radius bound: a tenant can be slightly over-charged but can NEVER be over-served past the ceiling.
- A cycle rolls (accrued resets to 0, the anchor advances) once the clock is `CYCLE_LENGTH_MS` (~30 days)
  past the tenant's `cycle_anchor_ms` (`crates/corelink-container/src/tenant_quota.rs:176-182`).
- The batch variant charges `n * cost_each` in ONE atomic check using `saturating_mul` to stop integer
  overflow from an untrusted batch size (`crates/corelink-container/src/tenant_quota.rs:1047-1054`).
...
- The DURABLE accrue is a serialized DB-side increment (the `D1QuotaStore` override), so concurrent ops
  ...
  check-and-accrue `crates/corelink-container/src/tenant_quota.rs:1414-1442` + the atomic conditional
  cycle-roll `crates/corelink-container/src/tenant_quota.rs:1515-1529`). The in-memory `LeasedQuotaStore`
  ...
  (`crates/corelink-container/src/tenant_quota.rs:137-149`, `crates/corelink-container/src/tenant_quota.rs:436-1018`).
...
- The per-op cost is a deliberately coarse FLAT charge (default `$0.001/op`) — a preventive backstop,
  ...
  (`crates/corelink-container/src/tenant_quota.rs:96-110`).
- The guard is `None` (so billable routes run WITHOUT the gate) when the storage env is unset, so a
  ...
  (`crates/corelink-container/src/tenant_quota.rs:126-137`).
...
3. `crates/corelink-container/src/tenant_quota.rs:79-84` — the effectively-unlimited (`$1,000,000/mo`) default-ceiling constant (ADR-0068 reconciliation).
4. `crates/corelink-container/src/tenant_quota.rs:96-110` — the coarse flat per-op cost model.
5. `crates/corelink-container/src/tenant_quota.rs:126-137` — `None` guard when the storage env is unset (dev/CI).
6. `crates/corelink-container/src/tenant_quota.rs:176-182` — cycle-elapsed test against `CYCLE_LENGTH_MS`.
7. `crates/corelink-container/src/tenant_quota.rs:1047-1054` — `check_batch` atomic charge with `saturating_mul`.
8. `crates/corelink-container/src/tenant_quota.rs:1090-1098` — clock-unavailable `503` fail-close.
9. `crates/corelink-container/src/tenant_quota.rs:1132-1140` — store-unavailable `503` fail-close.
10. `crates/corelink-container/src/tenant_quota.rs:1414-1442` (atomic check-and-accrue) + `crates/corelink-container/src/tenant_quota.rs:1515-1529` (atomic conditional roll) — the `D1QuotaStore` durable roll + check-and-accrue (no lost-update / over-admission).
11. `crates/corelink-container/src/tenant_quota.rs:1149-1152` — over-ceiling `402 Payment Required` (guard mapping via `quota_exceeded_response`).
12. `crates/corelink-container/src/tenant_quota.rs:285-302` — the `QuotaStore` trait DEFAULT `check_and_accrue`: CF-4 made it FAIL CLOSED (`Err`→503, no non-atomic generic fallback); every backend MUST provide its own atomic check-and-increment. In-memory atomic override at `crates/corelink-container/src/tenant_quota.rs:1321-1344`.
13. `crates/corelink-container/src/tenant_quota.rs:1414-1442` — `D1QuotaStore::check_and_accrue`: the single atomic `UPDATE … WHERE accrued + ?delta <= budget RETURNING …` — the SOLE durable ceiling authority.
14. `crates/corelink-container/src/tenant_quota.rs:137-149` — `quota_guard_from_env` fronts `D1QuotaStore` with `LeasedQuotaStore` (the live production wiring).
15. `crates/corelink-container/src/tenant_quota.rs:436-1018` — `LeasedQuotaStore`: the in-memory 16-op budget lease (amortised D1 round-trip, bounded ~0.32% overshoot, over-charge-not-over-serve, fail-CLOSED on drained+unreachable). Its `QuotaStore` impl is annotated `#[async_trait::async_trait]` (explicit crate path post the axum-0.8 bump, which dropped the re-exported `axum::async_trait`).
16. `crates/corelink-container/src/tenant_quota.rs:430-437` (trait method) + `crates/corelink-container/src/tenant_quota.rs:985-1028` (`LeasedQuotaStore` override) + `crates/corelink-container/src/tenant_quota.rs:1106-1128` (`check` call site) — `try_serve_from_lease`: the WP-2a warm-lease READ short-circuit; the last per-op D1 read is served from the lease when warm, the durable atomic accrue remains the sole ceiling authority so the over-serve bound is unchanged.
