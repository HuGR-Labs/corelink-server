...
checkpoint_sha: "65ebf3655eee552986c21f7400431ce86ea562f7"
provenance: "AUTHORED"
...
- `QuotaGuard::check` reads the wall clock first and fail-CLOSES with `503` when it is unavailable
  ...
  (`crates/corelink-container/src/tenant_quota.rs:936-944`).
- The LAST per-op D1 READ is now served from the lease when warm: BEFORE the durable `get`, `check`
  ...
  over-serve bound is unchanged (`crates/corelink-container/src/tenant_quota.rs:952-974`,
...
- The batch variant charges `n * cost_each` in ONE atomic check using `saturating_mul` to stop integer
  overflow from an untrusted batch size (`crates/corelink-container/src/tenant_quota.rs:893-900`).
...
- The DURABLE accrue is a serialized DB-side increment (the `D1QuotaStore` override), so concurrent ops
  ...
  check-and-accrue `crates/corelink-container/src/tenant_quota.rs:1260-1288` + the atomic conditional
  cycle-roll `crates/corelink-container/src/tenant_quota.rs:1361-1375`). The in-memory `LeasedQuotaStore`
  ...
  (`crates/corelink-container/src/tenant_quota.rs:116-128`, `crates/corelink-container/src/tenant_quota.rs:415-864`).
...
7. `crates/corelink-container/src/tenant_quota.rs:893-900` — `check_batch` atomic charge with `saturating_mul`.
8. `crates/corelink-container/src/tenant_quota.rs:936-944` — clock-unavailable `503` fail-close.
9. `crates/corelink-container/src/tenant_quota.rs:978-986` — store-unavailable `503` fail-close.
10. `crates/corelink-container/src/tenant_quota.rs:1260-1288` (atomic check-and-accrue) + `crates/corelink-container/src/tenant_quota.rs:1361-1375` (atomic conditional roll) — the `D1QuotaStore` durable roll + check-and-accrue (no lost-update / over-admission).
11. `crates/corelink-container/src/tenant_quota.rs:995-998` — over-ceiling `402 Payment Required` (guard mapping via `quota_exceeded_response`).
12. `crates/corelink-container/src/tenant_quota.rs:264-281` — the `QuotaStore` trait DEFAULT `check_and_accrue`: CF-4 made it FAIL CLOSED (`Err`→503, no non-atomic generic fallback); every backend MUST provide its own atomic check-and-increment. In-memory atomic override at `crates/corelink-container/src/tenant_quota.rs:1167-1190`.
13. `crates/corelink-container/src/tenant_quota.rs:1260-1288` — `D1QuotaStore::check_and_accrue`: the single atomic `UPDATE … WHERE accrued + ?delta <= budget RETURNING …` — the SOLE durable ceiling authority.
...
15. `crates/corelink-container/src/tenant_quota.rs:415-864` — `LeasedQuotaStore`: the in-memory 16-op budget lease (amortised D1 round-trip, bounded ~0.32% overshoot, over-charge-not-over-serve, fail-CLOSED on drained+unreachable). Its `QuotaStore` impl is annotated `#[async_trait::async_trait]` (explicit crate path post the axum-0.8 bump, which dropped the re-exported `axum::async_trait`).
16. `crates/corelink-container/src/tenant_quota.rs:409-416` (trait method) + `crates/corelink-container/src/tenant_quota.rs:831-874` (`LeasedQuotaStore` override) + `crates/corelink-container/src/tenant_quota.rs:952-974` (`check` call site) — `try_serve_from_lease`: the WP-2a warm-lease READ short-circuit; the last per-op D1 read is served from the lease when warm, the durable atomic accrue remains the sole ceiling authority so the over-serve bound is unchanged.
