...
- The Worker maps each tenant to its own Durable Object instance via `idFromName(resolvedTenantId)` —
  ...
  serialization point for that tenant's state — `worker/src/index.ts:2549-2551`.
...
1. `worker/src/index.ts:2549-2551` — one DO instance per tenant via `idFromName(resolvedTenantId)`.
