...
- The Worker maps each tenant to its own Durable Object instance via `idFromName(resolvedTenantId)` —
  ...
  `resolvedTenantId`, never the shared `_pending_auth` — `worker/src/index.ts:3131`.
...
1. `worker/src/index.ts:3131` — one DO instance per tenant via `idFromName(resolvedTenantId)` (the implementing call).
