...
- The Worker maps each tenant to its own Durable Object instance via `idFromName(resolvedTenantId)` —
  ...
  serialization point for that tenant's state — `worker/src/index.ts:2533-2535`.
- Non-tenant system traffic uses reserved sentinel DO names (e.g. `_system`, `_oci`) that are deliberately
  distinct from any real tenant id — `worker/src/index.ts:1525`.
...
1. `worker/src/index.ts:2533-2535` — one DO instance per tenant via `idFromName(resolvedTenantId)`.
2. `worker/src/index.ts:1525` — reserved `_system` sentinel DO name for non-tenant traffic.
