...
- The Worker maps each tenant to its own Durable Object instance via `idFromName(resolvedTenantId)` —
  ...
  `resolvedTenantId`, never the shared `_pending_auth` — `worker/src/index.ts:2746-2748`.
...
1. `worker/src/index.ts:2746-2748` — one DO instance per tenant via `idFromName(resolvedTenantId)`.
2. `worker/src/index.ts:582` — reserved `_system` sentinel DO name for non-tenant traffic.
