...
- The Worker maps each tenant to its own Durable Object instance via `idFromName(resolvedTenantId)` —
  ...
  `resolvedTenantId`, never the shared `_pending_auth` — `worker/src/index.ts:2818-2820`.
- Non-tenant system traffic uses reserved sentinel DO names (e.g. `_system`, `_oci`) that are deliberately
  distinct from any real tenant id — `worker/src/index.ts:599`.
...
1. `worker/src/index.ts:2818-2820` — one DO instance per tenant via `idFromName(resolvedTenantId)`.
2. `worker/src/index.ts:599` — reserved `_system` sentinel DO name for non-tenant traffic.
