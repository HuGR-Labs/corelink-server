...
- The Worker maps each tenant to its own Durable Object instance via `idFromName(resolvedTenantId)` —
  ...
  `resolvedTenantId`, never the shared `_pending_auth` — `worker/src/index.ts:3236`.
- Non-tenant system traffic uses reserved sentinel DO names (e.g. `_system`, `_oci`) that are deliberately
  distinct from any real tenant id — `worker/src/index.ts:702`.
...
1. `worker/src/index.ts:3236` — one DO instance per tenant via `idFromName(resolvedTenantId)` (the implementing call).
2. `worker/src/index.ts:702` — reserved `_system` sentinel DO name for non-tenant traffic (the `_health/container` route return).
