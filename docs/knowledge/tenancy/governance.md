...
- The rate limiter is wired as ONE `.layer(...)` line at the end of `build_with_factory`
  (`crates/corelink-container/src/routes.rs:867-870`), so it covers exactly the composed data-plane router;
...
- Revoking a credential is a destructive admin op gated on cache-write capability: a read-only principal
  is rejected `403` (`crates/corelink-container/src/routes/customer.rs:768-795`).
...
- The `/v1/customer/usage` surface reports real reads/writes/hit-rate/ROI: `handle_usage`
  (`crates/corelink-container/src/routes/customer.rs:486`) reads a `usage_daily` rollup, and those rows are
  fed by the in-process DISPLAY aggregator `crate::usage_meter::UsageMeter` — a hot-path-safe
  `record(tenant, ReadHit|ReadMiss|Write)` that only bumps in-memory counters under a lock
  (`crates/corelink-container/src/usage_meter.rs:157-169`) plus a background flusher that additively
  UPSERTs the accumulated deltas into the `usage_daily` D1 table (migration 0089)
  (`crates/corelink-container/src/usage_meter.rs:240-260`). It is per-tenant DISPLAY telemetry only —
  never a rate-limit, quota, or billing input.
...
- The data-plane rate limiter never covers the DO readiness probe or the internal shared-secret surfaces:
  ...
  (`crates/corelink-container/src/routes.rs:867-870`; the executed enforcer is
  `crates/corelink-container/src/routes/ratelimit_layer.rs:437-463`).
...
- Credential revocation requires cache-write scope; a read-only token cannot revoke any credential in the
  tenant (intra-tenant lockout defence) (`crates/corelink-container/src/routes/customer.rs:789-795`).
...
- The limiter is wired with bounded NoOp sinks in production — `RateLimitLayerState::new` /
  `with_tier_resolver` construct them (`crates/corelink-container/src/routes.rs:848-870`); the crate's
...
2. `crates/corelink-container/src/routes.rs:867-870` — the single `.layer(...)` wiring; `/_health` + `/_internal/*` are merged AFTER it in `main.rs`, so exclusion is structural (not a path check inside the layer).
...
4. `crates/corelink-ratelimit/src/audit.rs:147`, `crates/corelink-ratelimit/src/metrics.rs:177` — the `InMemoryRateLimit*` capture sinks (unbounded `Vec`/`HashMap`); the prod NoOp sinks are constructed at `crates/corelink-container/src/routes.rs:848-870`.
...
10. `crates/corelink-container/src/routes/customer.rs:768-795` — `handle_keys_revoke`: revoke requires cache-write scope; read-only → `403`.
...
13. `crates/corelink-container/src/routes.rs:867-870` — the single `.layer(...)` wiring of the rate limiter in `build_with_factory`.
14. `crates/corelink-container/src/routes/customer.rs:486` — `handle_usage`, the `/v1/customer/usage` surface that reads the `usage_daily` rollup for the dashboard ROI numbers.
15. `crates/corelink-container/src/usage_meter.rs:157-169` — `UsageMeter::record`: the hot-path-safe in-memory counter increment (DISPLAY telemetry, never a gate).
16. `crates/corelink-container/src/usage_meter.rs:240-260` — the `usage_daily` D1 sink: the background flusher's additive `INSERT … ON CONFLICT DO UPDATE` UPSERT of accumulated deltas (migration 0089).
