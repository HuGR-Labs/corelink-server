...
- Revoking a credential is a destructive admin op gated on cache-write capability: a read-only principal
  is rejected `403` (`crates/corelink-container/src/routes/customer.rs:826-872`).
- Team-management mutations are now OWNER/ADMIN-gated on the server-trusted `x-corelink-role` (migration
  ...
  requires the caller be the OWNER (`crates/corelink-container/src/routes/customer.rs:934-954`), and
  `handle_team_remove` rejects a non-owner/admin caller before it flips a seat to `removed` + revokes that
  member's PATs (`crates/corelink-container/src/routes/customer.rs:997-1003`) — closing the member→owner
...
- Credential revocation requires cache-write scope; a read-only token cannot revoke any credential in the
  tenant (intra-tenant lockout defence) (`crates/corelink-container/src/routes/customer.rs:846-853`).
...
10. `crates/corelink-container/src/routes/customer.rs:826-872` — `handle_keys_revoke`: revoke requires cache-write scope; read-only → `403`.
