...
- The rate limiter is wired as ONE `.layer(...)` line at the end of `build_with_factory`
  (`crates/corelink-container/src/routes.rs:1104-1107`), so it covers exactly the composed data-plane router;
...
- The OCI plane reaches the layer with the tenant header deleted, so it runs a SEPARATE velocity gate keyed
  ...
  internet ONE bucket per scope (`crates/corelink-container/src/routes/ratelimit_layer.rs:636-650`). The
...
- Revoking a credential is a destructive admin op gated on cache-write capability: a read-only principal
  is rejected `403` (`crates/corelink-container/src/routes/customer.rs:828-874`).
- Team-management mutations are now OWNER/ADMIN-gated on the server-trusted `x-corelink-role` (migration
  ...
  requires the caller be the OWNER (`crates/corelink-container/src/routes/customer.rs:936-956`), and
  `handle_team_remove` rejects a non-owner/admin caller before it flips a seat to `removed` + revokes that
  member's PATs (`crates/corelink-container/src/routes/customer.rs:999-1005`) — closing the member→owner
...
- The data-plane rate limiter never covers the DO readiness probe or the internal shared-secret surfaces:
  ...
  (`crates/corelink-container/src/routes.rs:1104-1107`; the executed enforcer is
  `crates/corelink-container/src/routes/ratelimit_layer.rs:480-506`).
...
- Credential revocation requires cache-write scope; a read-only token cannot revoke any credential in the
  tenant (intra-tenant lockout defence) (`crates/corelink-container/src/routes/customer.rs:848-855`).
...
- The limiter is wired with bounded NoOp sinks in production — `RateLimitLayerState::new` /
  `with_tier_resolver` construct them (`crates/corelink-container/src/routes.rs:1067-1085`); the crate's
...
2. `crates/corelink-container/src/routes.rs:1104-1107` — the single `.layer(...)` wiring; `/_health` + `/_internal/*` are merged AFTER it in `main.rs`, so exclusion is structural (not a path check inside the layer).
...
4. `crates/corelink-ratelimit/src/audit.rs:147`, `crates/corelink-ratelimit/src/metrics.rs:177` — the `InMemoryRateLimit*` capture sinks (unbounded `Vec`/`HashMap`); the prod NoOp sinks are constructed at `crates/corelink-container/src/routes.rs:1067-1085`.
...
10. `crates/corelink-container/src/routes/customer.rs:828-874` — `handle_keys_revoke`: revoke requires cache-write scope; read-only → `403`.
...
13. `crates/corelink-container/src/routes.rs:1104-1107` — the single `.layer(...)` wiring of the rate limiter in `build_with_factory`.
...
24. `crates/corelink-container/src/routes/ratelimit_layer.rs:636-650` — `oci_bucket_key`: the OCI velocity-gate key. Credential-gated per-repo scopes key on the repo name verbatim; the two credential-FREE synthetic scopes (`_token`, `_v2root`) fold in the server-trusted `x-corelink-client-ip`, with absent/empty collapsing to one dedicated `_no_ip` partition that is disjoint from every real per-IP bucket.
