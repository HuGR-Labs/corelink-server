...
- Customer PAT revocation requires the server-trusted `owner`/`admin` role; admins cannot target owner/legacy
  ...
  `crates/corelink-container/src/customer_d1_billing_keys.rs:334-352`).
...
10. `crates/corelink-container/src/routes/customer/part-01.rs:202-269` — `handle_keys_revoke`: trusted role `owner`/`admin` gate and delegation to the D1 handler; `crates/corelink-container/src/customer_d1_billing_keys.rs:334-352` — the D1 handler rejects an admin targeting an owner/legacy PAT and returns before issuing an `UPDATE`.
