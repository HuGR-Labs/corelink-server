...
source_blobs:
  ...
  - "crates/corelink-container/src/adapter_pat.rs@2fea7ee74fcd7fd4d33cb4d681477e6e6de0f1f9"
...
- In the container the **first** verification step is the HMAC fast-reject: the plaintext is parsed and
  ...
  permit (`crates/corelink-container/src/adapter_pat.rs:927-928`).
- Only a token that passes HMAC and resolves to a live D1 row reaches the expensive layer: an Argon2id
  ...
  permit (`crates/corelink-container/src/adapter_pat.rs:1073-1080`).
...
- A token that fails the cheap HMAC fast-reject NEVER reaches the Argon2id layer
  (`crates/corelink-container/src/adapter_pat.rs:927-928`).
...
5. `crates/corelink-container/src/adapter_pat.rs:927-928` — the HMAC fast-reject, pre-D1, no permit consumed.
6. `crates/corelink-container/src/adapter_pat.rs:1073-1080` — the deep Argon2id possession proof on a blocking thread. After it, the container's scope gate additionally fail-CLOSES a find-only PAT before the read grant (`crates/corelink-container/src/adapter_pat.rs:1096-1098`, ADR-0071): a find-only PAT's CHECK-safe `read-only` base would otherwise `docker pull` via the header-less OCI `/token` exchange.
