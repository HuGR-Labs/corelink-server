...
wire never tells an attacker *why* a token failed (`crates/corelink-container/src/adapter_pat_verifier.rs:29-60`).
...
(`crates/corelink-container/src/adapter_pat_verifier.rs:1-24`). The same cheap-then-deep shape protects the
...
- In the container the **first** verification step is the HMAC fast-reject: the plaintext is parsed and
  ...
  permit (`crates/corelink-container/src/adapter_pat_verifier.rs:310-513`).
- Only a token that passes HMAC and resolves to a live D1 row reaches the expensive layer: an Argon2id
  ...
  permit (`crates/corelink-container/src/adapter_pat_verifier.rs:517-704`).
- On the Worker edge the per-request `pat` read behind `extractAuth` is served through a THREE-TIER read
  ...
  consulted ONLY AFTER the HMAC possession proof above (`worker/src/index_auth.ts:612`), so a wrong-secret
  ...
  `extractAuth` threads onto its `AuthResult` as `patSource` (`worker/src/index_auth.ts:611-612`) purely so
...
- **L2 — Workers KV (`patrow:<token_id>`, 60 s TTL).** On an L1 miss the read consults a globally
  ...
  (`worker/src/index_auth.ts:541-545`). L2 holds POSITIVE rows only and its write is best-effort — a KV
  ...
  forwards it into `verifyPatRowCached` only when present (`worker/src/index_auth.ts:545`). A KV MISS, a
...
- **L3 — D1 (replica session → primary), the source-of-truth.** On an L1+L2 miss the underlying `pat`
  ...
  `withSession` is absent (`worker/src/index_auth.ts:534-537`). `readPatRow` keeps that replica read SAFE
...
- Both layers are constant-time with no length or content oracle — the edge compare
  ...
  (`crates/corelink-container/src/adapter_pat_verifier.rs:29-60`).
...
- The edge PAT-verify cache keeps D1 the source-of-truth (INV-AUTH-NEON-IS-SOT): NEGATIVES are never
  ...
  denied; expiry is re-checked by the caller on every hit (`worker/src/index_auth.ts:565`); and a D1 fault
...
- **Availability-vs-auth at the Worker edge: a transient D1 PAT-lookup FAULT now maps to a retryable
  ...
  `extractAuth` returns the distinct reason `d1_lookup_error` (`worker/src/index_auth.ts:547-551`). The
  ...
  a cause of a 401 there; it is a 503. The error-branch comment at `worker/src/index_auth.ts:548-550`
  ...
  and the `kv` pass into `verifyPatRowCached` (`worker/src/index_auth.ts:541-545`) — each of which shifted
...
- **A valid, unexpired PAT is not sufficient — the tenant fast-suspend gate (go-live G4).** After the D1
  ...
  (`worker/src/index_auth.ts:584-591`). Like the `pat` read above, the check is a THREE-TIER read —
...
3. `crates/corelink-container/src/adapter_pat_verifier.rs:1-24` — why the container re-runs full verification (Option B).
4. `crates/corelink-container/src/adapter_pat_verifier.rs:29-60` — uniform `InvalidPat`: no on-the-wire oracle.
5. `crates/corelink-container/src/adapter_pat_verifier.rs:310-513` — the HMAC fast-reject, pre-D1, no permit consumed.
6. `crates/corelink-container/src/adapter_pat_verifier.rs:517-704` — the deep Argon2id possession proof on a blocking thread. After it, the container's scope gate additionally fail-CLOSES a find-only PAT before the read grant (`crates/corelink-container/src/adapter_pat_verifier.rs:719-780`, ADR-0071): a find-only PAT's CHECK-safe `read-only` base would otherwise `docker pull` via the header-less OCI `/token` exchange.
...
8. `worker/src/index_auth.ts:584-591` — the `extractAuth` fast-suspend arm: a valid PAT whose tenant is suspended/erased returns `tenant_suspended` (G4), now passing the `kv` (`METADATA_KV`) + `waitUntil` opts into `isTenantSuspended`.
...
12. `worker/src/lib/pat_verify_cache.ts:326-333` — the best-effort L2 KV write on a D1 hydrate, handed to `ctx.waitUntil` so it survives the response (a bare `void kv.put(...)` is cancelled when the Worker returns, leaving KV un-populated), falling back to `await` when no `waitUntil` is passed (tests); `extractAuth` threads `ctx.waitUntil.bind(ctx)` from the handler (`worker/src/index_auth_stage.ts:43`) into `verifyPatRowCached` (`worker/src/index_auth.ts:545`). Writes go via `kvPutPatRow` (`worker/src/lib/pat_verify_cache.ts:383-389`); a KV write failure is swallowed and never breaks auth (POSITIVE rows only).
...
14. `worker/src/index_auth.ts:541-545` — the `extractAuth` L2 wiring: feature-detect the `METADATA_KV` binding and pass it as `kv` into `verifyPatRowCached` only when bound (a build without the binding skips L2).
