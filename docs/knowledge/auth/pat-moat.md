...
- **Availability-vs-auth at the Worker edge: a transient D1 PAT-lookup FAULT now maps to a retryable
  ...
  (`worker/src/index.ts:1130-1140`). The PAT-gate caller (H1 fix) now maps BOTH
  `signing_key_not_configured` AND `d1_lookup_error` to `503 authentication service unavailable`
  (`worker/src/index.ts:2432-2437`) — a D1 hiccup is a TRANSIENT infra fault, not a bad credential, so
  ...
  in-line `catch` comment at `worker/src/index.ts:1137-1139` now correctly states that the caller maps
  ...
  per-tenant DO forward (`worker/src/index.ts:1781-1783`)). Both the D1-fault and the
...
- **A valid, unexpired PAT is not sufficient — the tenant fast-suspend gate (go-live G4).** After the D1
  ...
  (`worker/src/index.ts:1169-1170`). The check is a single-flight, ~30s-TTL cached D1 read
  ...
  bad-credential arms and the 503 transient-infra arms (`worker/src/index.ts:2447`).
...
8. `worker/src/index.ts:1169-1170` — the `extractAuth` fast-suspend arm: a valid PAT whose tenant is suspended/erased returns `tenant_suspended` (G4).
