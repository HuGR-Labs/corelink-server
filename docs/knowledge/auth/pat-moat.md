...
wire never tells an attacker *why* a token failed (`crates/corelink-container/src/adapter_pat.rs:56-60`).
...
- The Worker-edge internal gate compares a presented secret against the expected one in constant time,
  ...
  (`worker/src/lib/internal_auth.ts:271-285`).
- That edge gate is fail-CLOSED: an unbound or too-short secret makes the endpoint unavailable (503),
  ...
  (`worker/src/lib/internal_auth.ts:313-325`).
- Per-consumer key resolution is `resolveConsumerKey`, which prefers a consumer's dedicated key. A
  ...
  (`worker/src/lib/internal_auth.ts:200-217`). When the dedicated key is genuinely UNSET the fallback now
  SPLITS by consumer: a DEDICATED-REQUIRED consumer — currently `quota_read`, listed in the
  `DEDICATED_REQUIRED_CONSUMERS` set (`worker/src/lib/internal_auth.ts:155-163`) — fails CLOSED (`null`,
  ...
  (`worker/src/lib/internal_auth.ts:232-240`); every other (shared-allowed) consumer such as `admin`
  still falls back to the shared key when it is ≥floor (`worker/src/lib/internal_auth.ts:241-246`).
- The internal consumers now split the DSR surface into TWO authorities. The irreversible physical-erase
  ...
  catch-all (`worker/src/index.ts:468-470`) and resolved by the `dsr_anchor` branch of the
  consumer-key ternary (`worker/src/lib/internal_auth.ts:193-194`). This is an anti-forge two-authority
...
- Only a token that passes HMAC and resolves to a live D1 row reaches the expensive layer: an Argon2id
  ...
  permit (`crates/corelink-container/src/adapter_pat.rs:1930-1941`).
- On the Worker edge the per-request `pat` read behind `extractAuth` is served through a THREE-TIER read
  ...
  consulted ONLY AFTER the HMAC possession proof above (`worker/src/index.ts:1549`), so a wrong-secret
  ...
  `extractAuth` threads onto its `AuthResult` as `patSource` (`worker/src/index.ts:1548-1549`) purely so
  the handler can surface it in the `Server-Timing` response header's `auth` desc for a client latency
  probe (`worker/src/index.ts:4374`); it is observability-only, never a trust signal and never forwarded
  to the container.
...
- **L2 — Workers KV (`patrow:<token_id>`, 60 s TTL).** On an L1 miss the read consults a globally
  ...
  (`worker/src/index.ts:1478-1482`). L2 holds POSITIVE rows only and its write is best-effort — a KV
  ...
  handler binds and passes `ctx.waitUntil.bind(ctx)` (`worker/src/index.ts:3188`) and `extractAuth`
  forwards it into `verifyPatRowCached` only when present (`worker/src/index.ts:1482`). A KV MISS, a
...
- **L3 — D1 (replica session → primary), the source-of-truth.** On an L1+L2 miss the underlying `pat`
  ...
  `withSession` is absent (`worker/src/index.ts:1471-1474`). `readPatRow` keeps that replica read SAFE
...
- Both layers are constant-time with no length or content oracle — the edge compare
  (`worker/src/lib/internal_auth.ts:271-285`) and the uniform `InvalidPat` collapse in the container
...
- The edge PAT-verify cache keeps D1 the source-of-truth (INV-AUTH-NEON-IS-SOT): NEGATIVES are never
  ...
  denied; expiry is re-checked by the caller on every hit (`worker/src/index.ts:1502`); and a D1 fault
...
- **Availability-vs-auth at the Worker edge: a transient D1 PAT-lookup FAULT now maps to a retryable
  ...
  `extractAuth` returns the distinct reason `d1_lookup_error` (`worker/src/index.ts:1484-1488`). The
  PAT-gate caller (H1 fix) then maps BOTH `signing_key_not_configured` AND `d1_lookup_error` to
  `503 authentication service unavailable` (`worker/src/index.ts:3208-3216`) — a D1 hiccup is a
  ...
  a cause of a 401 there; it is a 503. The error-branch comment at `worker/src/index.ts:1485-1487`
  ...
  per-tenant DO forward (`worker/src/index.ts:2279-2280`), when the `extractAuth`
  ...
  and the `kv` pass into `verifyPatRowCached` (`worker/src/index.ts:1478-1482`) — each of which shifted
...
- **A valid, unexpired PAT is not sufficient — the tenant fast-suspend gate (go-live G4).** After the D1
  ...
  (`worker/src/index.ts:1521-1528`). Like the `pat` read above, the check is a THREE-TIER read —
  ...
  **OPEN** on a transient D1 fault (availability), but a KNOWN-suspended cached value still denies
  ...
  the 503 transient-infra arms (`worker/src/index.ts:3223-3227`).

# Citations

1. `worker/src/lib/internal_auth.ts:271-285` — the edge constant-time secret compare with no length oracle.
2. `worker/src/lib/internal_auth.ts:313-325` — the fail-CLOSED edge gate (503 unbound / 401 wrong / `null` pass).
2a. `worker/src/lib/internal_auth.ts:200-217` — `resolveConsumerKey`: prefers a consumer's dedicated key; a SET-but-sub-floor (<32-char) dedicated key fails LOUD + fail-CLOSED (null) instead of silently widening to the shared key (deep-audit C/sub-floor). On an UNSET dedicated key the fallback SPLITS: a DEDICATED-REQUIRED consumer (`quota_read`, in `DEDICATED_REQUIRED_CONSUMERS` at `worker/src/lib/internal_auth.ts:155-163`) fails CLOSED with no shared-master fallback (`worker/src/lib/internal_auth.ts:232-240`); a shared-allowed consumer (e.g. `admin`) still falls back to the shared key (`worker/src/lib/internal_auth.ts:241-246`).
2b. `worker/src/lib/internal_auth.ts:193-194` — the `dsr_anchor` branch of the consumer-key ternary (`CORELINK_DSR_ANCHOR_AUTH_KEY`).
2c. `worker/src/index.ts:468-470` — `internalConsumerForPath` special-cases `/_internal/dsr/anchor` → `dsr_anchor` before the `/_internal/dsr/*` erase catch-all.
...
6. `crates/corelink-container/src/adapter_pat.rs:1930-1941` — the deep Argon2id possession proof on a blocking thread. After it, the container's scope gate additionally fail-CLOSES a find-only PAT before the read grant (`crates/corelink-container/src/adapter_pat.rs:1980-1982`, ADR-0071): a find-only PAT's CHECK-safe `read-only` base would otherwise `docker pull` via the header-less OCI `/token` exchange.
...
8. `worker/src/index.ts:1521-1528` — the `extractAuth` fast-suspend arm: a valid PAT whose tenant is suspended/erased returns `tenant_suspended` (G4), now passing the `kv` (`METADATA_KV`) + `waitUntil` opts into `isTenantSuspended`.
...
12. `worker/src/lib/pat_verify_cache.ts:326-333` — the best-effort L2 KV write on a D1 hydrate, handed to `ctx.waitUntil` so it survives the response (a bare `void kv.put(...)` is cancelled when the Worker returns, leaving KV un-populated), falling back to `await` when no `waitUntil` is passed (tests); `extractAuth` threads `ctx.waitUntil.bind(ctx)` from the handler (`worker/src/index.ts:3188`) into `verifyPatRowCached` (`worker/src/index.ts:1482`). Writes go via `kvPutPatRow` (`worker/src/lib/pat_verify_cache.ts:383-389`); a KV write failure is swallowed and never breaks auth (POSITIVE rows only).
...
14. `worker/src/index.ts:1478-1482` — the `extractAuth` L2 wiring: feature-detect the `METADATA_KV` binding and pass it as `kv` into `verifyPatRowCached` only when bound (a build without the binding skips L2).
