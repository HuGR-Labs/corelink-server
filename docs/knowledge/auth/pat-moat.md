...
- The internal consumers now split the DSR surface into TWO authorities. The irreversible physical-erase
  ...
  catch-all (`worker/src/index.ts:425-427`) and resolved by the `dsr_anchor` branch of the
...
- On the Worker edge the per-request `pat` read behind `extractAuth` is served through a THREE-TIER read
  ...
  consulted ONLY AFTER the HMAC possession proof above (`worker/src/index.ts:1414`), so a wrong-secret
  ...
  `extractAuth` threads onto its `AuthResult` as `patSource` (`worker/src/index.ts:1483-1484`) purely so
  the handler can surface it in the `Server-Timing` response header's `auth` desc for a client latency
  probe (`worker/src/index.ts:3938`); it is observability-only, never a trust signal and never forwarded
  to the container.
...
- **L2 — Workers KV (`patrow:<token_id>`, 60 s TTL).** On an L1 miss the read consults a globally
  ...
  (`worker/src/index.ts:1413-1417`). L2 holds POSITIVE rows only and its write is best-effort — a KV
  ...
  handler binds and passes `ctx.waitUntil.bind(ctx)` (`worker/src/index.ts:3003`) and `extractAuth`
  forwards it into `verifyPatRowCached` only when present (`worker/src/index.ts:1417`). A KV MISS, a
...
- **L3 — D1 (replica session → primary), the source-of-truth.** On an L1+L2 miss the underlying `pat`
  ...
  `withSession` is absent (`worker/src/index.ts:1406-1409`). `readPatRow` keeps that replica read SAFE
...
- The edge PAT-verify cache keeps D1 the source-of-truth (INV-AUTH-NEON-IS-SOT): NEGATIVES are never
  ...
  denied; expiry is re-checked by the caller on every hit (`worker/src/index.ts:1437`); and a D1 fault
...
- **Availability-vs-auth at the Worker edge: a transient D1 PAT-lookup FAULT now maps to a retryable
  ...
  `extractAuth` returns the distinct reason `d1_lookup_error` (`worker/src/index.ts:1419-1423`). The
  PAT-gate caller (H1 fix) then maps BOTH `signing_key_not_configured` AND `d1_lookup_error` to
  `503 authentication service unavailable` (`worker/src/index.ts:3023-3031`) — a D1 hiccup is a
  ...
  a cause of a 401 there; it is a 503. The error-branch comment at `worker/src/index.ts:1420-1422`
  ...
  per-tenant DO forward (`worker/src/index.ts:2174-2176`), when the `extractAuth`
  ...
  and the `kv` pass into `verifyPatRowCached` (`worker/src/index.ts:1413-1417`) — each of which shifted
...
- **A valid, unexpired PAT is not sufficient — the tenant fast-suspend gate (go-live G4).** After the D1
  ...
  (`worker/src/index.ts:1456-1463`). Like the `pat` read above, the check is a THREE-TIER read —
  ...
  **OPEN** on a transient D1 fault (availability), but a KNOWN-suspended cached value still denies
  ...
  the 503 transient-infra arms (`worker/src/index.ts:3038-3043`).
...
2c. `worker/src/index.ts:425-427` — `internalConsumerForPath` special-cases `/_internal/dsr/anchor` → `dsr_anchor` before the `/_internal/dsr/*` erase catch-all.
...
8. `worker/src/index.ts:1456-1463` — the `extractAuth` fast-suspend arm: a valid PAT whose tenant is suspended/erased returns `tenant_suspended` (G4), now passing the `kv` (`METADATA_KV`) + `waitUntil` opts into `isTenantSuspended`.
...
12. `worker/src/lib/pat_verify_cache.ts:326-333` — the best-effort L2 KV write on a D1 hydrate, handed to `ctx.waitUntil` so it survives the response (a bare `void kv.put(...)` is cancelled when the Worker returns, leaving KV un-populated), falling back to `await` when no `waitUntil` is passed (tests); `extractAuth` threads `ctx.waitUntil.bind(ctx)` from the handler (`worker/src/index.ts:3003`) into `verifyPatRowCached` (`worker/src/index.ts:1417`). Writes go via `kvPutPatRow` (`worker/src/lib/pat_verify_cache.ts:383-389`); a KV write failure is swallowed and never breaks auth (POSITIVE rows only).
...
14. `worker/src/index.ts:1413-1417` — the `extractAuth` L2 wiring: feature-detect the `METADATA_KV` binding and pass it as `kv` into `verifyPatRowCached` only when bound (a build without the binding skips L2).
