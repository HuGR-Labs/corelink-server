...
source_blobs:
  - "worker/src/lib/internal_auth.ts@df1e555fa147a4c8c380d475e357cd5f150bf645"
...
- The Worker-edge internal gate compares a presented secret against the expected one in constant time,
  ...
  (`worker/src/lib/internal_auth.ts:216-231`).
- That edge gate is fail-CLOSED: an unbound or too-short secret makes the endpoint unavailable (403),
  ...
  (`worker/src/lib/internal_auth.ts:210-222`).
- Per-consumer key resolution is `resolveConsumerKey`, which prefers a consumer's dedicated key. A
  ...
  (`worker/src/lib/internal_auth.ts:133-165`).
- The internal consumers now split the DSR surface into TWO authorities. The irreversible physical-erase
  ...
  consumer-key ternary (`worker/src/lib/internal_auth.ts:128-129`). This is an anti-forge two-authority
...
- Both layers are constant-time with no length or content oracle — the edge compare
  (`worker/src/lib/internal_auth.ts:216-231`) and the uniform `InvalidPat` collapse in the container
...
- The edge gate fails CLOSED: an unbound/short secret is unavailable, never an open gate
  (`worker/src/lib/internal_auth.ts:210-216`).
...
1. `worker/src/lib/internal_auth.ts:216-231` — the edge constant-time secret compare with no length oracle.
2. `worker/src/lib/internal_auth.ts:210-222` — the fail-CLOSED edge gate (403 unbound / 401 wrong / `null` pass).
2a. `worker/src/lib/internal_auth.ts:133-165` — `resolveConsumerKey`: prefers a consumer's dedicated key; a SET-but-sub-floor (<32-char) dedicated key fails LOUD + fail-CLOSED (null) instead of silently widening to the shared key (deep-audit C/sub-floor); a genuinely UNSET dedicated key still falls back to shared.
2b. `worker/src/lib/internal_auth.ts:128-129` — the `dsr_anchor` branch of the consumer-key ternary (`CORELINK_DSR_ANCHOR_AUTH_KEY`).
...
12. `worker/src/lib/pat_verify_cache.ts:326-333` — the best-effort L2 KV write on a D1 hydrate, handed to `ctx.waitUntil` so it survives the response (a bare `void kv.put(...)` is cancelled when the Worker returns, leaving KV un-populated), falling back to `await` when no `waitUntil` is passed (tests); `extractAuth` threads `ctx.waitUntil.bind(ctx)` from the handler (`worker/src/index.ts:2655`) into `verifyPatRowCached` (`worker/src/index.ts:1270`). Writes go via `kvPutPatRow` (`worker/src/lib/pat_verify_cache.ts:383-389`); a KV write failure is swallowed and never breaks auth (POSITIVE rows only).
