...
source_blobs:
  ...
  - "crates/corelink-container/src/adapter_pat.rs@2c3b7341b50b1d1c3d6fd9b6daa1a4c30f2026fc"
...
- In the container the **first** verification step is the HMAC fast-reject: the plaintext is parsed and
  ...
  permit (`crates/corelink-container/src/adapter_pat.rs:932-933`).
- Only a token that passes HMAC and resolves to a live D1 row reaches the expensive layer: an Argon2id
  ...
  permit (`crates/corelink-container/src/adapter_pat.rs:1056-1064`).
...
- A token that fails the cheap HMAC fast-reject NEVER reaches the Argon2id layer
  (`crates/corelink-container/src/adapter_pat.rs:932-933`).
...
5. `crates/corelink-container/src/adapter_pat.rs:932-933` — the HMAC fast-reject, pre-D1, no permit consumed.
6. `crates/corelink-container/src/adapter_pat.rs:1056-1064` — the deep Argon2id possession proof on a blocking thread. After it, the container's scope gate additionally fail-CLOSES a find-only PAT before the read grant (`crates/corelink-container/src/adapter_pat.rs:1085-1087`, ADR-0071): a find-only PAT's CHECK-safe `read-only` base would otherwise `docker pull` via the header-less OCI `/token` exchange.
