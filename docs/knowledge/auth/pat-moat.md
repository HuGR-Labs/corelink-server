...
source_blobs:
  ...
  - "crates/corelink-container/src/adapter_pat.rs@4d81d9e9b5386ea5c5abb80c449fe813d6ceabc1"
...
- In the container the **first** verification step is the HMAC fast-reject: the plaintext is parsed and
  ...
  permit (`crates/corelink-container/src/adapter_pat.rs:983-984`).
- Only a token that passes HMAC and resolves to a live D1 row reaches the expensive layer: an Argon2id
  ...
  permit (`crates/corelink-container/src/adapter_pat.rs:1148-1155`).
...
5. `crates/corelink-container/src/adapter_pat.rs:983-984` — the HMAC fast-reject, pre-D1, no permit consumed.
6. `crates/corelink-container/src/adapter_pat.rs:1148-1155` — the deep Argon2id possession proof on a blocking thread. After it, the container's scope gate additionally fail-CLOSES a find-only PAT before the read grant (`crates/corelink-container/src/adapter_pat.rs:1171-1173`, ADR-0071): a find-only PAT's CHECK-safe `read-only` base would otherwise `docker pull` via the header-less OCI `/token` exchange.
