...
the shared chokepoint (`worker/src/lib/session_exchange.ts:409-417`,
...
(`worker/src/lib/session_exchange.ts:574-592`). The single authority is also the single
...
- `mintScopedPat` is the sole exported mint authority; it SHA-256-derives a stable
  ...
  never supply (`worker/src/lib/session_exchange.ts:456`, `worker/src/lib/session_exchange.ts:493-501`).
- It canonicalizes the requested scope label to a D1-legal `pat.scope`
  ...
  is never minted (`worker/src/lib/session_exchange.ts:153-163`, `worker/src/lib/session_exchange.ts:464-467`).
- A per-principal fixed-window throttle gates the mint: over the cap returns 429
  ...
  (`worker/src/lib/session_exchange.ts:474-476`). The durable counter is ONE atomic
...
- After a 200 from the container it persists the `pat` row (using the returned Argon2id
  ...
  not written is never returned (`worker/src/lib/session_exchange.ts:569-610`).
- **Consumer 1 — session→PAT exchange (hugit Seam C):** `handleSessionExchange` verifies
  ...
  chokepoint with a 1-hour TTL. The minted scope is now **role-gated (RBAC)**: a resolved
  `viewer` seat mints `read-only`, any other role mints the default `cas:rw`
  (`worker/src/lib/session_exchange.ts:405-406`, `worker/src/lib/session_exchange.ts:409-417`).
- **Consumer 1b — RFC 8693 token exchange (githugr):** `handleTokenExchange` first caps a
  `viewer` seat's scope DOWN to `read-only` regardless of the requested scope
  (`worker/src/lib/session_exchange.ts:729-731`), then adds the cross-tenant defense —
  `audience !== tenantId` is a hard 403, so a session for tenant A can never obtain a PAT
  for tenant B — before delegating to the same chokepoint
  (`worker/src/lib/session_exchange.ts:737-738`, `worker/src/lib/session_exchange.ts:742-750`).
...
- The cross-tenant audience check is fail-CLOSED: a session whose resolved tenant differs
  ...
  (`worker/src/lib/session_exchange.ts:737-738`).
- A `viewer` seat can NEVER obtain a write-capable PAT from either exchange surface:
  the session exchange mints `read-only` for a viewer role
  (`worker/src/lib/session_exchange.ts:405-406`) and the token exchange caps the
  requested scope DOWN to `read-only` (`worker/src/lib/session_exchange.ts:729-731`) —
  least privilege, applied server-side after the shared role resolution.
...
- Every mint is throttled and every issuance failure fails CLOSED — an unmappable scope
  ...
  (`worker/src/lib/session_exchange.ts:464-467`, `worker/src/lib/session_exchange.ts:474-476`).
...
- The container's `/_internal/pat/mint` is a PURE function — it returns the token + hash
  ...
  route directly (`worker/src/lib/session_exchange.ts:574-592`).
- `mintScopedPat` performs NO authentication of its own beyond the per-principal throttle
  ...
  (`worker/src/lib/session_exchange.ts:430-433`).
...
- A non-200 from the container mint is collapsed to a single fail-CLOSED 500 at the edge,
  ...
  (`worker/src/lib/session_exchange.ts:515-522`). See [the 2-level PAT moat](/auth/pat-moat.md)
...
3. `worker/src/lib/session_exchange.ts:409-417` — Consumer 1: `handleSessionExchange` delegates to `mintScopedPat` (1h TTL, role-gated scope).
3a. `worker/src/lib/session_exchange.ts:405-406` — Consumer 1 RBAC: a resolved `viewer` role mints `read-only`; any other role mints the default `cas:rw`.
4. `worker/src/lib/session_exchange.ts:430-433` — `mintScopedPat` does no auth of its own beyond the throttle; the caller MUST have authorized.
5. `worker/src/lib/session_exchange.ts:456` — SHA-256-derived stable per-principal UUID for the mint.
6. `worker/src/lib/session_exchange.ts:464-467` — unmappable scope fails CLOSED with a 500 before any container call.
7. `worker/src/lib/session_exchange.ts:474-476` — per-principal mint throttle → 429 fail-CLOSED.
...
8. `worker/src/lib/session_exchange.ts:493-501` — the FRESH server-to-server request to `/_internal/pat/mint` with the server-trusted internal-auth header.
9. `worker/src/lib/session_exchange.ts:515-522` — a non-200 container mint collapses to a single fail-CLOSED 500 (no internal oracle).
10. `worker/src/lib/session_exchange.ts:574-592` — the caller-side `pat`-row persist (the load-bearing fix): the container mint writes NO `pat` row, so the chokepoint INSERTs it here before handing the token back.
11. `worker/src/lib/session_exchange.ts:569-610` — hash-presence guard + `INSERT INTO pat` + fail-CLOSED on any FK/UNIQUE/CHECK/transport error (or a zero-rows-changed no-op).
12. `worker/src/lib/session_exchange.ts:737-738` — Consumer 1b: `audience !== tenantId` → 403 cross-tenant rejection.
12a. `worker/src/lib/session_exchange.ts:729-731` — Consumer 1b RBAC: a `viewer` role caps the requested scope DOWN to `read-only` before the mint.
13. `worker/src/lib/session_exchange.ts:742-750` — `handleTokenExchange` delegates to the same chokepoint (300s).
