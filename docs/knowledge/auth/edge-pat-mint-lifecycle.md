...
checkpoint_sha: "fff4da676e7b4a4cc06ffc053c8329f5c52df8ea"
provenance: "AUTHORED"
...
the shared chokepoint (`worker/src/lib/session_exchange.ts:540-548`,
`worker/src/lib/runner_mint.ts:503-511`, `worker/src/lib/auth_rotate.ts:285-291`). Each
...
alone no longer authorizes a mint (`worker/src/lib/session_exchange.ts:501-502`,
...
`worker/src/lib/session_exchange.ts:215-259`, `worker/src/lib/session_exchange.ts:630-636`).
...
no `pat` row ever existed (`worker/src/lib/session_exchange.ts:747-799`). The single authority is also the single
...
- `mintScopedPat` is the sole exported mint authority; it takes a branded `MintGrant`,
  ...
  (`worker/src/lib/session_exchange.ts:609`, `worker/src/lib/session_exchange.ts:660-671`).
- **L12(b) capability + scope ceiling.** The tenant is sourced FROM the grant (a loose
  ...
  (`worker/src/lib/session_exchange.ts:215-259`). Over the total lattice `rank()`
  ...
  (`worker/src/lib/session_exchange.ts:181-190`, `worker/src/lib/session_exchange.ts:630-636`).
- It canonicalizes the requested scope label to a D1-legal `pat.scope`
  ...
  is never minted (`worker/src/lib/session_exchange.ts:154-165`, `worker/src/lib/session_exchange.ts:617-621`).
- A per-principal fixed-window throttle gates the mint: over the cap returns 429
  ...
  (`worker/src/lib/session_exchange.ts:641-644`). The durable counter is ONE atomic
  ...
  (`worker/src/lib/session_exchange.ts:326-335`, `worker/src/lib/session_exchange.ts:309-317`).
- **Transient-D1-fault handling (F20):** when the durable throttle's D1 write THROWS
  ...
  outage (`worker/src/lib/session_exchange.ts:339-347`,
  `worker/src/lib/session_exchange.ts:360-388`). The map is LRU-bounded at
  ...
  (`worker/src/lib/session_exchange.ts:367-376`).
- After a 200 from the container it persists the `pat` row (using the returned Argon2id
  ...
  not written is never returned (`worker/src/lib/session_exchange.ts:747-799`).
- **RBAC scope cap (least privilege):** the resolved `role` from the shared pipeline now caps the
  ...
  (`worker/src/lib/session_exchange.ts:529`), and `handleTokenExchange` caps the requested/default
  scope DOWN to `read-only` for a viewer regardless of what was asked
  (`worker/src/lib/session_exchange.ts:921-923`); owner/admin/member keep read-write. This composes
...
- **Consumer 1 — session→PAT exchange (hugit Seam C):** `handleSessionExchange` verifies
  ...
  role-capped TTL scope (`worker/src/lib/session_exchange.ts:540-548`).
- **Consumer 1b — RFC 8693 token exchange (githugr):** `handleTokenExchange` adds the
  ...
  (`worker/src/lib/session_exchange.ts:930-931`, `worker/src/lib/session_exchange.ts:943-951`).
- **Consumer 2 — runner mint/revoke (cf-multitenant WP2):** `handleRunnerMint` has no
  ...
  (`worker/src/lib/runner_mint.ts:503-511`). It also honors an optional **lease-bound
  ...
  (`worker/src/lib/runner_mint.ts:605-616`).
...
- The cross-tenant audience check is fail-CLOSED: a session whose resolved tenant differs
  ...
  (`worker/src/lib/session_exchange.ts:930-931`).
- Scope is capped by RBAC role: a `viewer` seat's mint is forced to `read-only` regardless of the
  ...
  (`worker/src/lib/session_exchange.ts:529`, `worker/src/lib/session_exchange.ts:921-923`).
...
- Every mint is throttled and every issuance failure fails CLOSED — an unmappable OR
  ...
  issued-but-unusable or over-privileged token (`worker/src/lib/session_exchange.ts:617-621`,
  `worker/src/lib/session_exchange.ts:630-636`, `worker/src/lib/session_exchange.ts:641-644`).
- A D1 fault on the throttle path never opens the mint gate: the durable-counter write
  ...
  fail-LOUD, never fail-open (`worker/src/lib/session_exchange.ts:339-347`,
  `worker/src/lib/session_exchange.ts:378-388`).
- Revocation shares ONE surface: both the runner-revoke and the rotate-old-key write are
  ...
  the native plane honors (`worker/src/lib/runner_mint.ts:605-616`,
...
- The container's `/_internal/pat/mint` is a PURE function — it returns the token + hash
  ...
  route directly (`worker/src/lib/session_exchange.ts:713-716`).
- `mintScopedPat` performs NO session authentication of its own — it trusts that its
  ...
  what its grant proves (`worker/src/lib/session_exchange.ts:630-636`,
  `worker/src/lib/session_exchange.ts:641-644`).
...
- A non-200 from the container mint is collapsed to a single fail-CLOSED 500 at the edge,
  ...
  (`worker/src/lib/session_exchange.ts:682-690`). See [the 2-level PAT moat](/auth/pat-moat.md)
...
4. `worker/src/lib/session_exchange.ts:215-259` — the branded `MintGrant`: PRIVATE constructor + four per-caller factories binding `{tenantId, principalSource, maxScope}` (session/token/runner → `read-write`; rotation → the old scope).
5. `worker/src/lib/session_exchange.ts:630-636` — L12(b) scope-ceiling assert: `rank(requested) > rank(grant.maxScope)` → fail-CLOSED 500, no container mint, no D1 row, no token.
6. `worker/src/lib/session_exchange.ts:540-548` — Consumer 1: `handleSessionExchange` delegates to `mintScopedPat` via `MintGrant.fromVerifiedSession` (1h, role-capped scope).
6a. `worker/src/lib/session_exchange.ts:529` — RBAC: `handleSessionExchange` caps a `viewer` role's mint scope to `read-only`.
7. `worker/src/lib/session_exchange.ts:609` — SHA-256-derived stable per-principal UUID from the grant's `principalSource`.
8. `worker/src/lib/session_exchange.ts:617-621` — unmappable scope fails CLOSED with a 500 before any container call.
9. `worker/src/lib/session_exchange.ts:641-644` — per-principal mint throttle → 429 fail-CLOSED (the chokepoint's only self-gate beyond the ceiling).
9a. `worker/src/lib/session_exchange.ts:309-317` — `checkMintThrottle` is exported with parametrizable window + in-memory caps, so the runner path reuses it as the M22(b) per-tenant ceiling.
9b. `worker/src/lib/session_exchange.ts:326-335` — durable throttle is ONE atomic `INSERT … ON CONFLICT … DO UPDATE … RETURNING count` (concurrent mints cannot race the cap).
9c. `worker/src/lib/session_exchange.ts:339-347` — F20: a thrown D1 write logs fail-LOUD and falls through to the in-memory backstop (never fail-open).
9d. `worker/src/lib/session_exchange.ts:360-388` — the in-memory per-isolate burst backstop (default cap 5 < durable 10) fires on EVERY request to bound Argon2id CPU during a D1 outage.
9e. `worker/src/lib/session_exchange.ts:367-376` — LRU bound (50 000 entries, delete-then-set + oldest-key eviction) so a long-lived isolate can't grow the map unbounded.
10. `worker/src/lib/session_exchange.ts:660-671` — the FRESH server-to-server request to `/_internal/pat/mint` with the server-trusted internal-auth header.
11. `worker/src/lib/session_exchange.ts:682-690` — a non-200 container mint collapses to a single fail-CLOSED 500 (no internal oracle).
12. `worker/src/lib/session_exchange.ts:713-716` — the container mint writes NO `pat` row; the caller persists it (the load-bearing fix).
13. `worker/src/lib/session_exchange.ts:747-799` — `INSERT INTO pat` + fail-CLOSED on any FK/UNIQUE/CHECK/transport error.
14. `worker/src/lib/session_exchange.ts:817` — `mintScopedPat` merges the caller's `extraFields` bag into the 200 body (runner-mint's `max_concurrency`).
15. `worker/src/lib/session_exchange.ts:930-931` — Consumer 1b: `audience !== tenantId` → 403 cross-tenant rejection.
15a. `worker/src/lib/session_exchange.ts:921-923` — RBAC: `handleTokenExchange` caps a `viewer` role's scope DOWN to `read-only` regardless of the requested scope.
16. `worker/src/lib/session_exchange.ts:943-951` — `handleTokenExchange` delegates via `MintGrant.fromTokenExchange` (300s).
17. `worker/src/lib/session_exchange.ts:274-276` — `deriveTenantCeilingThrottleKey`: the domain-separated (`"runner-tenant:"`), hashed per-tenant throttle key (M22(b)).
...
24. `worker/src/lib/runner_mint.ts:503-511` — delegates via `MintGrant.fromRunnerDerivation` with the DERIVED tenant + `job_id` principal source, threading `max_concurrency` through `extraFields`.
...
25. `worker/src/lib/runner_mint.ts:605-616` — idempotent `UPDATE pat SET revoked_at_ms ... WHERE ... revoked_at_ms IS NULL` (tenant-scoped when `owner_tenant` is present, else by `pat_id` alone).
