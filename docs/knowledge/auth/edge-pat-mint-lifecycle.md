...
the shared chokepoint (`worker/src/lib/session_exchange.ts:401-409`,
`worker/src/lib/runner_mint.ts:188-196`, `worker/src/lib/auth_rotate.ts:276-284`). The
...
(`worker/src/lib/session_exchange.ts:537-543`). The single authority is also the single
...
- `mintScopedPat` is the sole exported mint authority; it SHA-256-derives a stable
  ...
  never supply (`worker/src/lib/session_exchange.ts:448`, `worker/src/lib/session_exchange.ts:485-496`).
...
- **Consumer 1 — session→PAT exchange (hugit Seam C):** `handleSessionExchange` verifies
  ...
  chokepoint with a 1-hour `cas:rw` TTL (`worker/src/lib/session_exchange.ts:392-409`).
- **Consumer 1b — RFC 8693 token exchange (githugr):** `handleTokenExchange` adds the
  ...
  (`worker/src/lib/session_exchange.ts:719-720`, `worker/src/lib/session_exchange.ts:724-732`).
- **Consumer 2 — runner mint/revoke:** `handleRunnerMint` has no session; it is
  ...
  runner `job_id` as the principal source (`worker/src/lib/runner_mint.ts:173-174`,
  `worker/src/lib/runner_mint.ts:183-184`, `worker/src/lib/runner_mint.ts:188-196`).
  Teardown revoke is a tenant-scoped, idempotent soft-revoke on the shared `pat` table
  (`worker/src/lib/runner_mint.ts:279-280`).
- **Consumer 3 — `clw auth rotate`:** `handleAuthRotate` reads the OLD `pat` row, refuses
  ...
  (`worker/src/lib/auth_rotate.ts:233-234`, `worker/src/lib/auth_rotate.ts:243-247`,
  `worker/src/lib/auth_rotate.ts:254-260`, `worker/src/lib/auth_rotate.ts:276-287`,
  `worker/src/lib/auth_rotate.ts:315-319`).
...
- The cross-tenant audience check is fail-CLOSED: a session whose resolved tenant differs
  ...
  (`worker/src/lib/session_exchange.ts:719-720`).
- Runners is a SEPARATE authorization axis from the cache tier — a tenant with no
  ...
  subscription (`worker/src/lib/runner_mint.ts:173-174`, `worker/src/lib/runner_mint.ts:183-184`).
...
- The container's `/_internal/pat/mint` is a PURE function — it returns the token + hash
  ...
  route directly (`worker/src/lib/session_exchange.ts:537-543`).
- `mintScopedPat` performs NO authentication of its own beyond the per-principal throttle
  ...
  (`worker/src/lib/session_exchange.ts:421-425`).
- Rotation cannot reproduce a `read-only` scope (the container mint has no `read-only`
  ...
  `read-write` — rotate preserves privilege EXACTLY (`worker/src/lib/auth_rotate.ts:262-266`).
...
3. `worker/src/lib/session_exchange.ts:401-409` — Consumer 1: `handleSessionExchange` delegates to `mintScopedPat` (1h `cas:rw`).
4. `worker/src/lib/session_exchange.ts:421-425` — `mintScopedPat` does no auth of its own beyond the throttle; the caller MUST have authorized.
...
11. `worker/src/lib/session_exchange.ts:567-602` — `INSERT INTO pat` + fail-CLOSED on any FK/UNIQUE/CHECK/transport error.
12. `worker/src/lib/session_exchange.ts:719-720` — Consumer 1b: `audience !== tenantId` → 403 cross-tenant rejection.
13. `worker/src/lib/session_exchange.ts:724-732` — `handleTokenExchange` delegates to the same chokepoint (300s).
...
16. `worker/src/lib/runner_mint.ts:173-174` — `SELECT tenant_id FROM runners_entitlement WHERE tenant_id = ?1` (the separate Runners axis, migration 0070).
17. `worker/src/lib/runner_mint.ts:183-184` — no entitlement row → 403 (fail-CLOSED).
18. `worker/src/lib/runner_mint.ts:188-196` — delegates to `mintScopedPat` with `job_id` as the principal source.
19. `worker/src/lib/runner_mint.ts:279-280` — tenant-scoped idempotent `UPDATE pat SET revoked_at_ms ... WHERE pat_id=?2 AND tenant_id=?3 AND revoked_at_ms IS NULL`.
20. `worker/src/lib/auth_rotate.ts:48` — Consumer 3 imports `mintScopedPat` (no second mint path).
21. `worker/src/lib/auth_rotate.ts:233-234` — `SELECT tenant_id, scope, expires_ms, revoked_at_ms FROM pat WHERE pat_id = ?1` (read the old row).
22. `worker/src/lib/auth_rotate.ts:243-247` — unknown OR already-revoked PAT → 404 (never silently mint).
23. `worker/src/lib/auth_rotate.ts:254-260` — caller-named tenant ≠ the row's tenant → 403 (REV-S2, no cross-tenant rotate).
24. `worker/src/lib/auth_rotate.ts:262-266` — unmappable (`read-only`) scope → 422 (preserve privilege exactly).
25. `worker/src/lib/auth_rotate.ts:276-287` — mint NEW via the chokepoint; on non-200 return it and do NOT revoke (no zero-PAT window).
26. `worker/src/lib/auth_rotate.ts:315-319` — revoke OLD only after the new mint succeeds (shared idempotent soft-revoke).
