...
- `scope` may be NULL on legacy rows; the verifier's D1 row reader maps that to `""`
  (`crates/corelink-container/src/adapter_pat.rs:164-168`), which then fails CLOSED at the scope gate
  ...
  (`crates/corelink-container/src/adapter_pat.rs:172`; field at `crates/corelink-container/src/adapter_pat.rs:108`).
...
12. `crates/corelink-container/src/adapter_pat.rs:164-168` — the verifier's D1 row reader mapping a NULL `scope` to `""` (the same reader parses the additive `find_only` marker at `crates/corelink-container/src/adapter_pat.rs:172` into the `PatRow.find_only` field at `crates/corelink-container/src/adapter_pat.rs:108`).
