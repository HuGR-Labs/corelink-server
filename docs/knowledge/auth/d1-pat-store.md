...
- Self-serve key create mints a PAT then writes the row with `INSERT INTO pat (pat_id, tenant_id,
  ...
  (`crates/corelink-container/src/customer_d1.rs:1146-1161`).
...
- Revoke is an idempotent, tenant-scoped soft delete: `UPDATE pat SET revoked_at_ms = ?1 WHERE pat_id =
  ...
  (`crates/corelink-container/src/customer_d1.rs:1224-1226`).
- Listing is read-only and always self-tenant-scoped, mapping the stored `scope` back to a dashboard
  scopes list (`crates/corelink-container/src/customer_d1.rs:1057-1058`).
...
3. `crates/corelink-container/src/customer_d1.rs:1057-1058` — tenant-scoped key listing SELECT.
4. `crates/corelink-container/src/customer_d1.rs:1146-1161` — `INSERT INTO pat` (hash + token_id persisted, plaintext not).
5. `crates/corelink-container/src/customer_d1.rs:1224-1226` — idempotent tenant-scoped revoke UPDATE.
