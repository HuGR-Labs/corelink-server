...
- A sibling endpoint `POST /internal/v1/auth/resolve-tenant` is mounted on the SAME router and state
  ...
  Per the ratified A1 ordering contract (Option 2), provisioning (the signup-worker webhook / the
  Option-B token exchange) is the SOLE `tenant_org_map` writer, so the 404 arm
  (`crates/corelink-container/src/routes/auth_introspect.rs:799-803`) means "not-yet-provisioned" —
  TRANSIENT during webhook-delivery lag; consumers treat 404 as retryable with bounded backoff. The
  endpoint behaviour itself is unchanged (contract doc: `docs/integrations/resolve-tenant-contract.md`).
...
3. `crates/corelink-container/src/routes/auth_introspect.rs:831-839` — `build_state_from_env`: reads the dedicated `FABRIC_INTROSPECT_AUTH_KEY`, enforces `len() < MIN_FABRIC_AUTH_KEY_LEN`, returns `None` (route NOT mounted) when absent/short — fail-CLOSED.
...
9. `crates/corelink-container/src/routes/auth_introspect.rs:760-810` — sibling `POST /internal/v1/auth/resolve-tenant` handler (same gate; `clerk_org_id`→`tenant_id` via `tenant_org_map`; 404 unmapped, 503 fail-CLOSED).
10. `crates/corelink-container/src/routes/auth_introspect.rs:799-803` — the `Ok(None)` → 404 `org_not_mapped` arm: lookup-only miss, transient during the provisioning window (A1 Option-2 retry contract; consumers re-poll with bounded backoff).
