...
- A sibling endpoint `POST /internal/v1/auth/resolve-tenant` is mounted on the SAME router and state
  ...
  Per the ratified A1 ordering contract (Option 2), provisioning (the `user.created` webhook /
  the githugr token exchange) is the SOLE `tenant_org_map` writer, so a 404 during the
  webhook-delivery window means "not-yet-provisioned" and is RETRYABLE by the consumer — the handler
  itself stays lookup-only and returns 404 on a miss
  (`crates/corelink-container/src/routes/auth_introspect.rs:792-803`).
...
9a. `crates/corelink-container/src/routes/auth_introspect.rs:792-803` — the lookup-only D1 read + `Ok(None)` → 404 `org_not_mapped` (the A1/Option-2 retryable "not-yet-provisioned" miss; the handler never writes `tenant_org_map`).
