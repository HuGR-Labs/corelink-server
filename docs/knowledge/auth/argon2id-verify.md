...
checkpoint_sha: "4d654766292826ef54b9bc318b79a3ffc42efc07"
provenance: "AUTHORED"
...
- After HMAC + D1, the full verify runs `verify_with_hash_multi` on a `spawn_blocking` thread: it
  ...
  the stored PHC hash (`crates/corelink-container/src/adapter_pat.rs:1810-1821`).
- Before that blocking call, the verifier consults `SecretMatchMemo` — a bounded (32 768-entry), TTL'd
  ...
  (`crates/corelink-container/src/adapter_pat.rs:1743-1758`; `crates/corelink-container/src/adapter_pat.rs:894-908`).
- A COLD miss is coalesced on a shared future (`FlightGroup`): the first caller for a key leads the one
  ...
  (`crates/corelink-container/src/adapter_pat.rs:1128-1187`;
  `crates/corelink-container/src/adapter_pat.rs:1766-1844`).
- The burn arm is coalesced too, on its own key and in its own map. The two keys need not be equal —
  ...
  arms (`crates/corelink-container/src/adapter_pat.rs:1017-1043`;
  `crates/corelink-container/src/adapter_pat.rs:1564-1660`).
- Concurrent Argon2id work is capped process-wide at 16 permits so a flood of valid-PAT requests cannot
  OOM-kill the shared container (`crates/corelink-container/src/adapter_pat.rs:500`).
- A per-tenant sub-cap (¼ of the global pool, floor 2) keeps one tenant flooding distinct PATs from
  ...
  (`crates/corelink-container/src/adapter_pat.rs:522-529`). On the row-FOUND arm the permit acquires
  ...
  (`crates/corelink-container/src/adapter_pat.rs:1781-1809`). The dummy-burn arm is the one
...
- An unknown/expired/revoked `token_id` runs a dummy Argon2id burn for timing parity so latency does not
  ...
  (`crates/corelink-container/src/adapter_pat.rs:1513-1681`). The D1 read that decides which arm is
  ...
  (`crates/corelink-container/src/adapter_pat.rs:1521-1524`).
- That dummy burn is routed through ONE shared synthetic bucket so a leaked-key flood across bogus
  ...
  *occupancy*. With the global permit taken first (as originally written), a request already destined
  ...
  (`crates/corelink-container/src/adapter_pat.rs:1607-1622`;
  `crates/corelink-container/src/adapter_pat.rs:743-770`).
- When the burn cannot get a permit the request is SHED — at EITHER tier — and the shed returns
  ...
  (`crates/corelink-container/src/adapter_pat.rs:1640-1644`;
  `crates/corelink-container/src/adapter_pat.rs:1617-1621`).
- The final scope gate fail-CLOSES a find-only PAT FIRST — `if row.find_only { return InvalidPat }` runs
  ...
  (`crates/corelink-container/src/adapter_pat.rs:1859-1865`).
...
- Argon2id concurrency is bounded; a failed acquire fails CLOSED as `Backend` (503) — on a timeout at
  ...
  (`crates/corelink-container/src/adapter_pat.rs:1772-1793`).
- A dummy-burn request that SHEDS holds NO global permit at any instant. The shared
  ...
  message (`crates/corelink-container/src/adapter_pat.rs:1607-1622`).
- `INV-AUTH-PAT-OVERLOAD-SHED-UNIFORM` — the shed is SYMMETRIC across D1 row existence. Under
  ...
  (`crates/corelink-container/src/adapter_pat.rs:1640-1644`;
  `crates/corelink-container/src/adapter_pat.rs:1617-1621`;
  `crates/corelink-container/src/adapter_pat.rs:1678`;
...
- Coalescing does not weaken that invariant — it narrows the surface. The acquires now live INSIDE the
  ...
  (`crates/corelink-container/src/adapter_pat.rs:1661-1680`).
- Coalescing adds no wait that the admitted path does not already have: a shared future has no queue,
  ...
  identically for every joiner (`crates/corelink-container/src/adapter_pat.rs:1128-1187`).
- A joiner can only receive a decision its OWN inputs already determined: the flight key IS the
  ...
  scope-rejected request does (`crates/corelink-container/src/adapter_pat.rs:1743-1758`).
...
- A permit is acquired only AFTER the cheap HMAC fast-reject (`crates/corelink-container/src/adapter_pat.rs:1509-1510`),
  so a forged token never reaches the permit acquire (`crates/corelink-container/src/adapter_pat.rs:1772-1793`).
- The memo carries NO authorization state. Tenant, scope, `find_only`, expiry and revocation all come
  ...
  window on this plane (`crates/corelink-container/src/adapter_pat.rs:1513-1681`;
  `crates/corelink-container/src/adapter_pat.rs:1859-1865`).
- A WRONG secret can never hit the memo: the plaintext is part of the key, so the two ways to earn a
  ...
  still pay full Argon2id, preserving timing parity (`crates/corelink-container/src/adapter_pat.rs:1000-1015`).
- The memo is populated PAST the step-4 scope gate, never at the end of the Argon2id step. A PAT whose
  ...
  (`crates/corelink-container/src/adapter_pat.rs:1867-1892`).
- `scope` and `find_only` are in the memo KEY, so a scope DOWNGRADE (as opposed to a revocation)
  ...
  latency (`crates/corelink-container/src/adapter_pat.rs:1000-1015`).
- The memoised fact is a pure function of its key and cannot become false: the stored PHC hash is IN the
  ...
  (`crates/corelink-container/src/adapter_pat.rs:1743-1758`).
...
- ⚠️ A dummy timing-burn that RUNS still consumes a global permit (a shed one does not — see the
  ...
  skipped is exactly why the status cannot be allowed to differ (`crates/corelink-container/src/adapter_pat.rs:1513-1681`).
- ⚠️ **`SECRET_MATCH_MEMO_TTL` (300 s) is NOT a revocation window — do not "harmonise" it down to the
  ...
  (`crates/corelink-container/src/adapter_pat.rs:773-793`).
- ⚠️ **Do not "simplify" the coalescer into a mutex, and do not move the memo write into the flight.**
  ...
  of these re-opens a finding (`crates/corelink-container/src/adapter_pat.rs:1703-1742`).
...
1. `crates/corelink-container/src/adapter_pat.rs:500` — the global Argon2id concurrency cap (OOM guard).
2. `crates/corelink-container/src/adapter_pat.rs:522-529` — the per-tenant Argon2id sub-cap (fairness).
3. `crates/corelink-container/src/adapter_pat.rs:1607-1622` — the dummy burn routed through the shared synthetic bucket (`UNKNOWN_TOKEN_BUCKET`).
4. `crates/corelink-container/src/adapter_pat.rs:1513-1681` — the None-row constant-time Argon2id timing-burn.
5. `crates/corelink-container/src/adapter_pat.rs:1810-1821` — the Argon2id possession verify on a blocking thread.
6. `crates/corelink-container/src/adapter_pat.rs:1772-1793` — permit-acquire timeout → fail-CLOSED `Backend` (row-FOUND arm).
6b. `crates/corelink-container/src/adapter_pat.rs:1640-1644` — the row-NOT-FOUND global-permit shed, returning the SAME `Backend("pat verifier overloaded")` (`INV-AUTH-PAT-OVERLOAD-SHED-UNIFORM`).
6c. `crates/corelink-container/src/adapter_pat.rs:1617-1621` — the row-NOT-FOUND per-tenant sub-cap shed, likewise `Backend`.
6d. `crates/corelink-container/src/adapter_pat.rs:1678` — the terminal `InvalidPat` AFTER the burn ran (deliberately NOT a shed).
7. `crates/corelink-container/src/adapter_pat.rs:1859-1865` — the scope gate: find-only fail-close FIRST (`if row.find_only`, ADR-0071), then fail-CLOSED read grant + write-bit surfacing.
...
12. `crates/corelink-container/src/adapter_pat.rs:1743-1758` — the memo consult, keyed on `(plaintext, token_id, stored pat_hash, scope, find_only)`, sitting between the D1 row read and Argon2id.
13. `crates/corelink-container/src/adapter_pat.rs:773-793` — why the memo's TTL is a memory bound and not a revocation window (contrast with the two 5 s decision caches).
14. `crates/corelink-container/src/adapter_pat.rs:831-867` — `SecretMatchMemo`: what is memoised, why it is sound, and why it adds no timing oracle.
15. `crates/corelink-container/src/adapter_pat.rs:1000-1015` — `secret_match_fingerprint`: domain-separated, length-prefixed SHA-256; only the digest is retained.
16. `crates/corelink-container/src/adapter_pat.rs:919-954` — bounded insert: expired-purge then oldest-survivor eviction, O(n) only when the map is full.
17. `crates/corelink-container/src/adapter_pat.rs:894-908` — `SecretMatchMemo::contains`: TTL check, evict-on-read, fail-safe `false` on a poisoned lock.
18. `crates/corelink-container/src/adapter_pat.rs:1065-1085` — `FlightGroup`: the no-cache shared-future coalescer, and why it is not a mutex.
19. `crates/corelink-container/src/adapter_pat.rs:1128-1187` — `FlightGroup::run`: join-or-lead, the SPAWN that keeps an abandoned run from stranding a permit, resolved flights refused and retired, bounded map, fail-safe uncoalesced run on a poisoned lock.
20. `crates/corelink-container/src/adapter_pat.rs:1017-1043` — `dummy_burn_fingerprint`: why the two arms' keys partition identically for a given plaintext, and why they live in separate maps.
21. `crates/corelink-container/src/adapter_pat.rs:1766-1844` — the hot-path flight: permits acquired INSIDE it in global→per-tenant order; the proof, and ONLY the proof, lives here.
22. `crates/corelink-container/src/adapter_pat.rs:1564-1660` — the mirrored dummy-burn flight (both arms or neither).
23. `crates/corelink-container/src/adapter_pat.rs:1661-1680` — the shed-uniformity mapping: EVERY shed, at either tier, is `Backend`; only a completed burn is `InvalidPat`.
24. `crates/corelink-container/src/adapter_pat.rs:1703-1742` — the adversarial findings against the earlier mutex, and why the shared future answers each.
