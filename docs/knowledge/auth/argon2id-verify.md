...
- After HMAC + D1, the full verify runs `verify_with_hash_multi` on a `spawn_blocking` thread: it
  ...
  the stored PHC hash (`crates/corelink-container/src/adapter_pat.rs:1097-1104`).
- Before that blocking call, the verifier consults `SecretMatchMemo` — a bounded (32 768-entry), TTL'd
  ...
  (`crates/corelink-container/src/adapter_pat.rs:1048-1059`; `crates/corelink-container/src/adapter_pat.rs:486-500`).
...
- An unknown/expired/revoked `token_id` runs a dummy Argon2id burn for timing parity so latency does not
  leak whether the token exists (`crates/corelink-container/src/adapter_pat.rs:961-1002`).
- That dummy burn is routed through ONE shared synthetic bucket so a leaked-key flood across bogus
  ...
  (`crates/corelink-container/src/adapter_pat.rs:989`).
- The final scope gate fail-CLOSES a find-only PAT FIRST — `if row.find_only { return InvalidPat }` runs
  ...
  (`crates/corelink-container/src/adapter_pat.rs:1120-1126`).
...
- Argon2id concurrency is bounded; an acquire timeout fails CLOSED as `Backend` (503), never piling on
  more 64-MiB allocations (`crates/corelink-container/src/adapter_pat.rs:1071-1084`).
...
- A permit is acquired only AFTER the cheap HMAC fast-reject (`crates/corelink-container/src/adapter_pat.rs:945-946`),
  so a forged token never reaches the permit acquire (`crates/corelink-container/src/adapter_pat.rs:1071-1084`).
- The memo carries NO authorization state. Tenant, scope, `find_only`, expiry and revocation all come
  ...
  window on this plane (`crates/corelink-container/src/adapter_pat.rs:949-1002`;
  `crates/corelink-container/src/adapter_pat.rs:1120-1126`).
- A WRONG secret can never hit the memo: the plaintext is part of the key, so the two ways to earn a
  ...
  still pay full Argon2id, preserving timing parity (`crates/corelink-container/src/adapter_pat.rs:592-607`).
- The memo is populated PAST the step-4 scope gate, never at the end of the Argon2id step. A PAT whose
  ...
  (`crates/corelink-container/src/adapter_pat.rs:1128-1146`).
- `scope` and `find_only` are in the memo KEY, so a scope DOWNGRADE (as opposed to a revocation)
  ...
  latency (`crates/corelink-container/src/adapter_pat.rs:592-607`).
- The memoised fact is a pure function of its key and cannot become false: the stored PHC hash is IN the
  ...
  (`crates/corelink-container/src/adapter_pat.rs:1048-1059`).
...
- The dummy timing-burn still consumes a global permit; under sustained overload the burn is skipped and
  ...
  its fate (`crates/corelink-container/src/adapter_pat.rs:961-1002`).
...
3. `crates/corelink-container/src/adapter_pat.rs:989` — the dummy burn routed through the shared synthetic bucket (`UNKNOWN_TOKEN_BUCKET`).
4. `crates/corelink-container/src/adapter_pat.rs:961-1002` — the None-row constant-time Argon2id timing-burn.
5. `crates/corelink-container/src/adapter_pat.rs:1097-1104` — the Argon2id possession verify on a blocking thread.
6. `crates/corelink-container/src/adapter_pat.rs:1071-1084` — permit-acquire timeout → fail-CLOSED `Backend`.
7. `crates/corelink-container/src/adapter_pat.rs:1120-1126` — the scope gate: find-only fail-close FIRST (`if row.find_only`, ADR-0071), then fail-CLOSED read grant + write-bit surfacing.
...
12. `crates/corelink-container/src/adapter_pat.rs:1048-1059` — the memo consult, keyed on `(plaintext, token_id, stored pat_hash)`, sitting between the D1 row read and Argon2id.
...
15. `crates/corelink-container/src/adapter_pat.rs:592-607` — `secret_match_fingerprint`: domain-separated, length-prefixed SHA-256; only the digest is retained.
