...
- After HMAC + D1, the full verify runs `verify_with_hash_multi` on a `spawn_blocking` thread: it
  ...
  the stored PHC hash (`crates/corelink-container/src/adapter_pat.rs:1602-1613`).
- Before that blocking call, the verifier consults `SecretMatchMemo` — a bounded (32 768-entry), TTL'd
  ...
  (`crates/corelink-container/src/adapter_pat.rs:1535-1550`; `crates/corelink-container/src/adapter_pat.rs:721-735`).
- A COLD miss is coalesced on a shared future (`FlightGroup`): the first caller for a key leads the one
  ...
  `crates/corelink-container/src/adapter_pat.rs:1558-1636`).
- The burn arm is coalesced too, on its own key and in its own map. The two keys need not be equal —
  ...
  `crates/corelink-container/src/adapter_pat.rs:1356-1452`).
...
- A per-tenant sub-cap (¼ of the global pool, floor 2) keeps one tenant flooding distinct PATs from
  ...
  (`crates/corelink-container/src/adapter_pat.rs:1573-1601`). The dummy-burn arm is the one
...
- An unknown/expired/revoked `token_id` runs a dummy Argon2id burn for timing parity so latency does not
  ...
  (`crates/corelink-container/src/adapter_pat.rs:1313-1473`).
- That dummy burn is routed through ONE shared synthetic bucket so a leaked-key flood across bogus
  ...
  *occupancy*. With the global permit taken first (as originally written), a request already destined
  ...
  (`crates/corelink-container/src/adapter_pat.rs:1399-1414`;
...
- When the burn cannot get a permit the request is SHED — at EITHER tier — and the shed returns
  ...
  (`crates/corelink-container/src/adapter_pat.rs:1432-1436`;
  `crates/corelink-container/src/adapter_pat.rs:1409-1413`).
- The final scope gate fail-CLOSES a find-only PAT FIRST — `if row.find_only { return InvalidPat }` runs
  ...
  (`crates/corelink-container/src/adapter_pat.rs:1651-1657`).
...
- Argon2id concurrency is bounded; a failed acquire fails CLOSED as `Backend` (503) — on a timeout at
  ...
  (`crates/corelink-container/src/adapter_pat.rs:1564-1585`).
- A dummy-burn request that SHEDS holds NO global permit at any instant. The shared
  ...
  message (`crates/corelink-container/src/adapter_pat.rs:1399-1414`).
- `INV-AUTH-PAT-OVERLOAD-SHED-UNIFORM` — the shed is SYMMETRIC across D1 row existence. Under
  ...
  (`crates/corelink-container/src/adapter_pat.rs:1432-1436`;
  `crates/corelink-container/src/adapter_pat.rs:1409-1413`;
  `crates/corelink-container/src/adapter_pat.rs:1470`;
...
- Coalescing does not weaken that invariant — it narrows the surface. The acquires now live INSIDE the
  ...
  (`crates/corelink-container/src/adapter_pat.rs:1453-1472`).
...
- A joiner can only receive a decision its OWN inputs already determined: the flight key IS the
  ...
  scope-rejected request does (`crates/corelink-container/src/adapter_pat.rs:1535-1550`).
...
- A permit is acquired only AFTER the cheap HMAC fast-reject (`crates/corelink-container/src/adapter_pat.rs:1309-1310`),
  so a forged token never reaches the permit acquire (`crates/corelink-container/src/adapter_pat.rs:1564-1585`).
- The memo carries NO authorization state. Tenant, scope, `find_only`, expiry and revocation all come
  ...
  window on this plane (`crates/corelink-container/src/adapter_pat.rs:1313-1473`;
  `crates/corelink-container/src/adapter_pat.rs:1651-1657`).
...
- The memo is populated PAST the step-4 scope gate, never at the end of the Argon2id step. A PAT whose
  ...
  (`crates/corelink-container/src/adapter_pat.rs:1659-1684`).
...
- The memoised fact is a pure function of its key and cannot become false: the stored PHC hash is IN the
  ...
  (`crates/corelink-container/src/adapter_pat.rs:1535-1550`).
...
- ⚠️ A dummy timing-burn that RUNS still consumes a global permit (a shed one does not — see the
  ...
  skipped is exactly why the status cannot be allowed to differ (`crates/corelink-container/src/adapter_pat.rs:1313-1473`).
...
- ⚠️ **Do not "simplify" the coalescer into a mutex, and do not move the memo write into the flight.**
  ...
  of these re-opens a finding (`crates/corelink-container/src/adapter_pat.rs:1495-1534`).
...
3. `crates/corelink-container/src/adapter_pat.rs:1399-1414` — the dummy burn routed through the shared synthetic bucket (`UNKNOWN_TOKEN_BUCKET`).
4. `crates/corelink-container/src/adapter_pat.rs:1313-1473` — the None-row constant-time Argon2id timing-burn.
5. `crates/corelink-container/src/adapter_pat.rs:1602-1613` — the Argon2id possession verify on a blocking thread.
6. `crates/corelink-container/src/adapter_pat.rs:1564-1585` — permit-acquire timeout → fail-CLOSED `Backend` (row-FOUND arm).
6b. `crates/corelink-container/src/adapter_pat.rs:1432-1436` — the row-NOT-FOUND global-permit shed, returning the SAME `Backend("pat verifier overloaded")` (`INV-AUTH-PAT-OVERLOAD-SHED-UNIFORM`).
6c. `crates/corelink-container/src/adapter_pat.rs:1409-1413` — the row-NOT-FOUND per-tenant sub-cap shed, likewise `Backend`.
6d. `crates/corelink-container/src/adapter_pat.rs:1470` — the terminal `InvalidPat` AFTER the burn ran (deliberately NOT a shed).
7. `crates/corelink-container/src/adapter_pat.rs:1651-1657` — the scope gate: find-only fail-close FIRST (`if row.find_only`, ADR-0071), then fail-CLOSED read grant + write-bit surfacing.
...
12. `crates/corelink-container/src/adapter_pat.rs:1535-1550` — the memo consult, keyed on `(plaintext, token_id, stored pat_hash, scope, find_only)`, sitting between the D1 row read and Argon2id.
...
21. `crates/corelink-container/src/adapter_pat.rs:1558-1636` — the hot-path flight: permits acquired INSIDE it in global→per-tenant order; the proof, and ONLY the proof, lives here.
22. `crates/corelink-container/src/adapter_pat.rs:1356-1452` — the mirrored dummy-burn flight (both arms or neither).
23. `crates/corelink-container/src/adapter_pat.rs:1453-1472` — the shed-uniformity mapping: EVERY shed, at either tier, is `Backend`; only a completed burn is `InvalidPat`.
24. `crates/corelink-container/src/adapter_pat.rs:1495-1534` — the adversarial findings against the earlier mutex, and why the shared future answers each.
