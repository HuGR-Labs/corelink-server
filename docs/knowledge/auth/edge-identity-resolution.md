...
checkpoint_sha: "b6c994a3c69e8dbddb03df85235b2736f673e6d3"
provenance: "AUTHORED"
...
denial, never an open gate. The pipeline result now also carries a resolved RBAC **`role`**
(`ClerkAuthResult` includes `role: string`, `worker/src/lib/clerk_auth.ts:61-63`): the owner arm yields
`owner`, the team-member arm yields the seat's 0074 role, and the fail-safe default is the
least-privilege `viewer` — downstream, `viewer` maps to a read-only scope. Two surfaces share this spine — the in-Worker dashboard pipeline
...
- Tenant resolution has **two ordered arms** in the dashboard pipeline. **(1) Owner arm:**
  ...
  (`worker/src/lib/clerk_auth.ts:255`). **(2) Team-member fallback** — ONLY when the owner arm returns no
  ...
  (`worker/src/lib/clerk_auth.ts:269-272`). So a verified Clerk user resolves to the tenant it OWNS, or
  failing that to the team-owning tenant of an active `team_member` row — not strictly to a single
  owner-keyed row.
- **RBAC role resolution (additive):** the pipeline initialises the role to the least-privilege
  `viewer` before either arm runs (`worker/src/lib/clerk_auth.ts:252`); the owner arm overwrites it with
  `owner` (`worker/src/lib/clerk_auth.ts:261`), and the team-member arm takes the seat's `role` column
  (0074 roles: owner|admin|member|viewer) with a fail-safe `|| "viewer"` for a missing/unexpected value
  (`worker/src/lib/clerk_auth.ts:282`). The resolved role is returned alongside the tenant
  (`worker/src/lib/clerk_auth.ts:293`) and gates the scope the caller receives downstream (viewer →
  read-only; owner/admin/member → read-write).
- The server-to-server endpoint runs only the owner-arm parameterized lookup
  ...
  is a fail-CLOSED 500, never fail-open (`worker/src/lib/clerk_auth.ts:284-291`).
...
- A subject that matches NEITHER the owner row NOR an active team_member row is a hard denial: the
  team-member-MISS branch returns 403 in the dashboard pipeline (`worker/src/lib/clerk_auth.ts:273-278`),
...
- **DORMANT githugr arm — per-`sub` provision-or-lookup (NOT a fixed tenant):** a separate-instance
  ...
  authoritatively (`worker/src/lib/clerk_auth.ts:372`). On success the verified `sub` becomes the
  `clerkUserId` (`worker/src/lib/clerk_auth.ts:385`), and the tenant is derived NOT from a fixed
  configured value but by **provision-or-lookup**: `provisionOrLookupGithugrTenant(env.CONFIG_DB, sub)`
  returns a deterministic per-`sub` isolated tenant — resolved with `role: "owner"` (a federated-login
  owner of its own isolated tenant, not a team_member seat; `worker/src/lib/clerk_auth.ts:405`) — and
  ANY D1 fault there is a fail-CLOSED **500** — never a fall-back to a shared tenant
  (`worker/src/lib/clerk_auth.ts:402`, `worker/src/lib/clerk_auth.ts:406-412`). With the secrets unset
  the whole routing `if` is false, so this arm never executes today.
- **githugr per-`sub` tenant derivation + LOOKUP-FIRST, transactional provisioning:** `deriveGithugrTenantId(sub)`
  ...
  (`worker/src/lib/githugr_provision.ts:91`, `worker/src/lib/githugr_provision.ts:92-101`).
  ...
  writes** (`worker/src/lib/githugr_provision.ts:134-140`). ONLY on the first-ever login for a `sub` (no
  ...
  order preserved by statement order: `tenant` first (`worker/src/lib/githugr_provision.ts:148-154`), then
  ...
  (`worker/src/lib/githugr_provision.ts:156-190`). AFTER the batch it READS BACK the authoritative
  `tenant_org_map` row so a concurrent/prior login's row wins (`worker/src/lib/githugr_provision.ts:198-201`); an
  absent read-back THROWS, which the caller treats as fail-CLOSED
  (`worker/src/lib/githugr_provision.ts:202-204`).
...
- Tenant resolution is always keyed on the verified `clerk_user_id` via parameterized D1 queries (no
  ...
  (`worker/src/lib/tenant_lookup.ts:114`, `worker/src/lib/clerk_auth.ts:255`) is tried first, then an
  additive `team_member` fallback resolves the user to the team-OWNING tenant when no owner row matches
  (`worker/src/lib/clerk_auth.ts:269-272`).
- The `team_member` fallback resolves ONLY an `status = 'active'` seat — an `invited` or `removed`
  ...
  (`worker/src/lib/clerk_auth.ts:269-272`).
- The resolved RBAC role FAILS SAFE to least privilege: it is initialised to `viewer` before any arm
  runs (`worker/src/lib/clerk_auth.ts:252`), and a team_member row with a missing/unexpected `role`
  value resolves to `viewer` (read-only) rather than granting write
  (`worker/src/lib/clerk_auth.ts:282`). Only an explicit owner row yields `owner`
  (`worker/src/lib/clerk_auth.ts:261`).
...
- A githugr session resolves to a **per-`sub` isolated tenant**, NOT one fixed shared tenant: the
  ...
  the same sub is idempotent (`worker/src/lib/githugr_provision.ts:91`,
  `worker/src/lib/githugr_provision.ts:92-101`). Provisioning is LOOKUP-FIRST (existing `sub` → single
  ...
  read-back (`worker/src/lib/githugr_provision.ts:124`,
...
- The githugr provision/lookup is fail-CLOSED: any D1 fault (or an absent read-back) throws / returns a
  ...
  fixes) (`worker/src/lib/clerk_auth.ts:406-412`, `worker/src/lib/githugr_provision.ts:202-204`).

# Gotchas

- **Designed vs wired:** the CoreLink Clerk session verify is wired + live; the **githugr** arm
  (`worker/src/lib/clerk_auth.ts:125`, `:372`) is DORMANT — gated on `GITHUGR_CLERK_ISSUER_URL` and
...
- **No more fixed githugr tenant:** the arm used to map EVERY githugr user to one configured
  ...
  (`worker/src/lib/clerk_auth.ts:406-412`).
...
9. `worker/src/lib/clerk_auth.ts:273-278` — the **team-member-MISS** branch: a subject with no owner row AND no active `team_member` row → 403 (dashboard pipeline). NOT a plain no-row branch — it is reached only after the owner arm misses and the `team_member` fallback also misses.
10. `worker/src/lib/clerk_auth.ts:255` — owner arm: `SELECT tenant_id FROM tenant WHERE clerk_user_id = ?1`.
11. `worker/src/lib/clerk_auth.ts:269-272` — additive team-member fallback: `SELECT tenant_id, role FROM team_member WHERE user_id = ?1 AND status = 'active'` resolves to the team-owning tenant (migration 0074).
11a. `worker/src/lib/clerk_auth.ts:252` — RBAC role initialised to the least-privilege `viewer` before either resolution arm runs.
11b. `worker/src/lib/clerk_auth.ts:261` — owner arm sets `role = "owner"`.
11c. `worker/src/lib/clerk_auth.ts:282` — team-member arm takes the seat's 0074 `role`, failing safe to `viewer` on a missing/unexpected value.
11d. `worker/src/lib/clerk_auth.ts:293` — the pipeline returns `{ ok, tenantId, clerkUserId, role }` — the resolved role travels with the identity.
11e. `worker/src/lib/clerk_auth.ts:61-63` — `ClerkAuthResult` success variant carries `role: string`.
...
13. `worker/src/lib/clerk_auth.ts:284-291` — D1 fault → 500 fail-CLOSED, never fail-open.
...
15. `worker/src/lib/clerk_auth.ts:372` — githugr arm authoritative issuer exact-pin (only reached when the dormant gate opens).
16. `worker/src/lib/clerk_auth.ts:402` — githugr arm resolves the tenant via `provisionOrLookupGithugrTenant(env.CONFIG_DB, sub)` — a per-`sub` isolated tenant, NOT a fixed configured id (returned with `role: "owner"` at `:405`).
17. `worker/src/lib/clerk_auth.ts:406-412` — githugr provision/lookup fail-CLOSED: a D1 fault is a 500, never a fall-back to a shared tenant.
...
20a. `worker/src/lib/githugr_provision.ts:134-140` — LOOKUP-FIRST `SELECT tenant_org_map`: on a repeat-login hit, return the tenant_id with ZERO writes.
21. `worker/src/lib/githugr_provision.ts:148-154` — `tenant` row written FIRST (FK target) via `INSERT OR IGNORE`, inside the transactional `batch([...])`.
22. `worker/src/lib/githugr_provision.ts:156-190` — the child + identity rows inside the same `batch([...])`: `tier_selections` (free/active), `runners_entitlement`, `tenant_quota`, and the `tenant_org_map` row keyed on `clerk_org_id = sub`, all `INSERT OR IGNORE` (transactional / all-or-nothing).
23. `worker/src/lib/githugr_provision.ts:198-201` — authoritative read-back of `tenant_org_map` AFTER the batch (a concurrent/prior login's row wins).
24. `worker/src/lib/githugr_provision.ts:202-204` — absent read-back THROWS → caller fails CLOSED (500).
