...
- The single shared pipeline that the `customer_v1` dual-auth dispatch and the onboarding/checkout arm
  both call, so the M1/M2 hardening lives in one place (`worker/src/lib/clerk_auth.ts:105`).
...
- The bearer token is extracted from the `Authorization` header; a missing token is a 401 before any
  verification work (`worker/src/lib/clerk_auth.ts:115`).
...
- **M1 — azp re-assert:** because the Clerk library SKIPS its `authorizedParties` check when the token
  ...
  not in `CLERK_AZP_ALLOWLIST` (`worker/src/lib/clerk_auth.ts:191-196`).
- **M2 — issuer pin:** when `CLERK_ISSUER_URL` is set, `iss` must equal it exactly or the token is 401
  (`worker/src/lib/clerk_auth.ts:211`); in production with the pin unset the path fails CLOSED rather
  than fall back to the weak shape-check (`worker/src/lib/clerk_auth.ts:217-231`).
- A verified token with no `sub` is rejected 401; otherwise `sub` becomes the `clerkUserId`
  (`worker/src/lib/clerk_auth.ts:248`).
- Tenant resolution has **two ordered arms** in the dashboard pipeline. **(1) Owner arm:**
  ...
  (`worker/src/lib/clerk_auth.ts:285`); a hit maps to the `owner` role
  (`worker/src/lib/clerk_auth.ts:291`). **(2) Team-member fallback** — ONLY when the owner arm returns no
  ...
  resolve, so a revoked seat is denied (`worker/src/lib/clerk_auth.ts:299-302`). So a verified Clerk user
...
- **RBAC role derivation:** the resolver now returns a `role` alongside the tenant on `ClerkAuthResult`.
  The default is the LEAST-privilege `viewer` (`worker/src/lib/clerk_auth.ts:282`); the owner arm sets
  `owner` (`worker/src/lib/clerk_auth.ts:291`); the team-member arm carries the seat's `role`, failing
  safe to `viewer` when the column is missing/unexpected (`worker/src/lib/clerk_auth.ts:312`). This role
...
- The server-to-server endpoint runs only the owner-arm parameterized lookup
  ...
  is a fail-CLOSED 500, never fail-open (`worker/src/lib/clerk_auth.ts:314-321`).
...
- A subject that matches NEITHER the owner row NOR an active team_member row is a hard denial: the
  team-member-MISS branch returns 403 in the dashboard pipeline (`worker/src/lib/clerk_auth.ts:303-308`),
...
- Tenant resolution is always keyed on the verified `clerk_user_id` via parameterized D1 queries (no
  ...
  (`worker/src/lib/tenant_lookup.ts:114`, `worker/src/lib/clerk_auth.ts:285`) is tried first, then an
  additive `team_member` fallback resolves the user to the team-OWNING tenant when no owner row matches
  (`worker/src/lib/clerk_auth.ts:299-302`).
- The resolved `role` fails safe to the LEAST privilege: it defaults to `viewer` and only a matched
  owner row (`worker/src/lib/clerk_auth.ts:291`) or a `team_member` seat's stored role
  (`worker/src/lib/clerk_auth.ts:312`) raises it — a missing/unexpected role stays `viewer` (read-only).
...
- The githugr multi-issuer arm cannot run unless BOTH real `GITHUGR_*` settings (issuer + public jwtKey)
  ...
  `GITHUGR_TENANT_ID` gate condition is REMOVED (`worker/src/lib/clerk_auth.ts:137`).
...
- **Designed vs wired:** the CoreLink Clerk session verify is wired + live; the **githugr** arm
  (`worker/src/lib/clerk_auth.ts:137`, `:365`) is DORMANT — gated on `GITHUGR_CLERK_ISSUER_URL` and
...
- A non-production deploy with `CLERK_ISSUER_URL` unset falls back to a conservative shape-check
  ...
  pin before deploy or all Clerk auth 401s (`worker/src/lib/clerk_auth.ts:213-227`).
...
1. `worker/src/lib/clerk_auth.ts:105` — `verifyClerkSessionAndResolveTenant`, the single shared pipeline.
2. `worker/src/lib/clerk_auth.ts:115` — missing bearer token → 401 before any verify.
3. `worker/src/lib/clerk_auth.ts:151-154` — `CLERK_SECRET_KEY` unbound → 403 fail-CLOSED.
4. `worker/src/lib/clerk_auth.ts:182-185` — `verifyToken` signature check with the azp allowlist.
5. `worker/src/lib/clerk_auth.ts:191-196` — M1 post-verify azp present-and-allowlisted re-assert → 401.
6. `worker/src/lib/clerk_auth.ts:211` — M2 exact issuer pin (`iss !== clerkIssuerUrl` → 401).
7. `worker/src/lib/clerk_auth.ts:217-231` — production fails CLOSED when `CLERK_ISSUER_URL` is unset.
8. `worker/src/lib/clerk_auth.ts:248` — verified token without `sub` → 401.
9. `worker/src/lib/clerk_auth.ts:303-308` — the **team-member-MISS** branch: a subject with no owner row AND no active `team_member` row → 403 (dashboard pipeline). NOT a plain no-row branch — it is reached only after the owner arm misses and the `team_member` fallback also misses.
10. `worker/src/lib/clerk_auth.ts:285` — owner arm: `SELECT tenant_id FROM tenant WHERE clerk_user_id = ?1` (a hit sets `role = "owner"` at `:261`).
11. `worker/src/lib/clerk_auth.ts:299-302` — additive team-member fallback: `SELECT tenant_id, role FROM team_member WHERE user_id = ?1 AND status = 'active'` resolves to the team-owning tenant and carries the seat's RBAC role (migration 0074); the role fails safe to `viewer` at `:282`.
11a. `worker/src/lib/clerk_auth.ts:282` — RBAC role default: `role` initialized to the least-privilege `viewer` before either resolution arm runs.
