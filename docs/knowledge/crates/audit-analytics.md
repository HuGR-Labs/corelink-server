...
- **The live chain producer is the hourly audit-drain** `crates/corelink-container/src/routes/audit_drain.rs:208-235` (mounted in `main.rs`): it seals pending `audit_outbox` rows (written PLAIN/UNCHAINED first, `emitted_at IS NULL`) into the per-tenant BLAKE3 chain in D1 via `link_chain_hash_from_canonical` `crates/corelink-audit-chain/src/chain.rs:183`, advancing `audit_chain_head` under a single-writer compare-and-set `crates/corelink-container/src/routes/audit_drain.rs:457-512`. The ≤1h pre-seal window is mutable with no chain evidence, and R2 Object-Lock is unwired — so this is tamper-EVIDENCE, not proof-against-insider.
...
- The live BLAKE3 link is UN-KEYED (`Hasher::new()`, no `keyed_hash`/HMAC — `crates/corelink-audit-chain/src/chain.rs:156`), so the chain is tamper-EVIDENCE (detect-at-verify vs an independent head), NOT tamper-proof against an insider who can recompute the whole chain; R2 Object-Lock + the pre-seal `audit_outbox` window leave storage mutable.
...
2d. `crates/corelink-container/src/routes/audit_drain.rs:208-235` — `seal_rows`, the LIVE BLAKE3 chain producer (`BLAKE3(prev || JCS(payload))`).
2e. `crates/corelink-container/src/routes/audit_drain.rs:457-512` — `drain_partition`: seal to D1 + single-writer CAS advance of `audit_chain_head` (no R2; ≤1h pre-seal window mutable).
