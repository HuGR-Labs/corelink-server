...
description: "The audit and observability layer — CloudEvents taxonomy with PII-as-hash typing, a mounted per-tenant BLAKE3 drain with signed heads, a daily verifier, and an implemented witnessed-epoch writer that requires signing and independent witness configuration. The drain fails closed on versioned checkpoints when that configuration or witness verification is absent or invalid. Rekor submission remains a separate post-hoc seam. `corelink-audit` SHA-256 link primitives are dead/legacy; the mounted legacy link hash is `corelink-audit-chain` BLAKE3."
source_files:
...
source_blobs:
  ...
  - "crates/corelink-audit/src/lib.rs@93aa6c1f953afda5911004918a9b4cff2b686611"
  - "crates/corelink-audit/src/link_hash.rs@cdfa9ca64d8c98db8ff9059d4b8902c42c995dc4"
  - "crates/corelink-audit/src/redact.rs@3bcf59535c2234726d60fdd1944a70907a267bb7"
  - "crates/corelink-audit-chain/src/lib.rs@6c041e31dc84bc28245e92a48c47584a78591e14"
  - "crates/corelink-audit-chain/src/chain.rs@8bd9a6509392c86d55b2a533e1af2aad0377447a"
  - "crates/corelink-audit-chain/src/verifier.rs@17652f622add0a604fd6d2488cddc92a65c6e80c"
...
Every security-relevant action in CoreLink must leave tamper-EVIDENT (detect-at-verify) trace. Events are serialized via RFC 8785 JCS before hashing, then linked without re-canonicalization. **The mounted legacy link hash is `corelink-audit-chain`'s un-keyed BLAKE3, not `corelink-audit`'s SHA-256.** `link_chain_hash_from_canonical` hashes the canonical payload (`crates/corelink-audit-chain/src/chain.rs:183`); the mounted hourly drain seals pending `audit_outbox` rows into the per-tenant D1 chain (`crates/corelink-container/src/routes/audit_drain/b126_m2_impl_01_part2.rs:244-254`; `crates/corelink-container/src/routes/audit_drain/b126_m2_impl_02_part2.rs:165-351`). The daily `ChainVerifier` walks the chain (`crates/corelink-audit-chain/src/verifier.rs:73`). `corelink-audit` owns the event taxonomy and PII-as-hash types; its SHA-256 `compute_content_hash` and `link_chain_hash` (`crates/corelink-audit/src/link_hash.rs:156`, `:171`) are dead/legacy and have no non-test callers. `corelink-transparency-log` is a separate post-hoc submission seam; production Rekor witnessing remains deferred.
...
- **The mounted hourly audit-drain** (`main.rs`) seals pending `audit_outbox` rows (written plain/unchained first, `emitted_at IS NULL`) into a per-tenant legacy BLAKE3 chain in D1 (`crates/corelink-container/src/routes/audit_drain/b126_m2_impl_01_part2.rs:244-254`; `crates/corelink-audit-chain/src/chain.rs:183`) and advances the legacy checkpoint with a compare-and-set (`crates/corelink-container/src/routes/audit_drain/b126_m2_impl_02_part2.rs:319-335`). The legacy link is un-keyed, so the body is tamper-evidence rather than proof against an insider with D1 write. CF-6 signs and verifies legacy checkpoint heads with Ed25519 (`crates/corelink-container/src/routes/audit_drain/b126_m2_impl_01.rs:366-447`; `crates/corelink-container/src/routes/audit_drain/b126_m2_impl_02.rs:9-66`). Versioned heads dispatch to the mounted `drain_partition_v2`; it proceeds only with a configured signing seed and complete independent witness configuration, matching signing key, and valid witness state (`crates/corelink-container/src/routes/audit_drain/b126_m2_impl_02_part2.rs:181-197`, `:364-377`). Missing or invalid configuration/state fails closed. The ≤1h pre-seal window remains mutable and R2 Object-Lock is unwired; an independent prior head is needed to detect a full D1 rewrite.
...
- The live per-link BLAKE3 link is UN-KEYED (`Hasher::new()`, no `keyed_hash`/HMAC — `crates/corelink-audit-chain/src/chain.rs:156`), so the chain BODY is tamper-EVIDENCE (detect-at-verify vs an independent head), NOT tamper-proof against an insider who can recompute the whole chain body — though the chain HEAD is now Ed25519-SIGNED (CF-6, `crates/corelink-container/src/routes/audit_drain/b126_m2_impl_01.rs:366-447`), so a forged head fails verification on resume (fail-closed SEV-1); R2 Object-Lock + the pre-seal `audit_outbox` window still leave storage mutable.
...
2d. `crates/corelink-container/src/routes/audit_drain/b126_m2_impl_01_part2.rs:244-254` — `seal_rows`, the mounted legacy BLAKE3 chain producer (`BLAKE3(prev || JCS(payload))`).
2e. `crates/corelink-container/src/routes/audit_drain/b126_m2_impl_02_part2.rs:181-197` — mounted `drain_partition_inner` dispatches versioned heads to `drain_partition_v2` before the legacy path. The v2 writer requires a configured signing seed and complete witness client (`:364-377`) and checks the key id and latest witness before continuing; absent or invalid witness inputs fail closed. The legacy path retains its checkpoint validation, seal loop, and flag-gated lease fence (`:199-351`).
2f. `crates/corelink-container/src/routes/audit_drain/b126_m2_impl_01.rs:366-447` — CF-6 legacy checkpoint signing and verification (`sign_head` / `verify_head`, Ed25519 over the canonical head tuple).
2g. `crates/corelink-container/src/routes/audit_drain/b126_m2_impl_02.rs:8-66` — `check_head_on_resume`: re-verify the resumed legacy head signature; a non-verifying / unseeded signed head = TAMPER → fail-CLOSED (SEV-1).
