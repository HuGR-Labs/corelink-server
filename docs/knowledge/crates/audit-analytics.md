...
2d. `crates/corelink-container/src/routes/audit_drain/b126_m2_impl_01.rs:517-544` — `seal_rows`, the LIVE BLAKE3 chain producer (`BLAKE3(prev || JCS(payload))`).
2e. `crates/corelink-container/src/routes/audit_drain/b126_m2_impl_02.rs:353-501` — `drain_partition_inner`: seal to D1 + single-writer CAS advance of `audit_chain_head` (no R2; ≤1h pre-seal window mutable) + CF-6 head sign-on-advance / verify-on-resume. B-038 wraps it in `drain_partition` `crates/corelink-container/src/routes/audit_drain/b126_m2_impl_02.rs:544-621` with a flag-gated (default-OFF) per-partition lease + seal-loop fence.
