...
Every security-relevant action in CoreLink must leave a tamper-evident trace, and this cluster is the machinery that *can* make the trace canonical, PII-free, and chained so a deletion or edit is detectable — **with one load-bearing carve-out: the chain logic here is real and unit-tested but has NO LIVE PRODUCER, and the deployed trail is therefore freely tamperable (red-team-confirmed).** Nothing in the deployed container calls `HashChainBuilder::append` outside tests; the live DSR audit sink writes plain *unchained* CloudEvents to the D1 `audit_outbox` table with **`digest=NULL`, `emitted_at=NULL`, and no `prev_hash`/`sequence_number`** (`crates/corelink-container/src/routes/dsr/audit.rs:7-8`, `crates/corelink-container/src/routes/dsr/audit.rs:82-84`). Nothing in the container ever `SET`s `emitted_at`, seals a digest, or applies R2 Object-Lock to these rows, and no compliance reader validates a chain over them. The S-09 chain-sealing drain that would flip `emitted_at` and link those rows into the immutable chain is the unbuilt **WI-S09-007**. So on the deployed plane `audit_outbox` is a **plain MUTABLE D1 table**: the SAME D1 credential the app already holds can `DELETE`/`UPDATE` any audit row **undetectably**, post-hoc — the "audit started.v1 before any mutation, fail-closed" guarantee secures only *capture ordering*, not the *durability* of the captured evidence. "A deletion or edit is detectable" describes the chain code, NOT the deployed trail today — on the deployed trail that property is **FALSE**. It is grouped around one design spine: events are serialized via RFC 8785 JCS before hashing so the digest is deterministic, then linked without ever re-canonicalizing. The two chain crates use *different* link-hash algorithms, though: `corelink-audit` links via SHA-256 (`sha256(prev_chain_hash || content_hash)`, `crates/corelink-audit/src/lib.rs:20`), while `corelink-audit-chain` links via BLAKE3 (`crates/corelink-audit-chain/src/lib.rs:31`, `crates/corelink-audit-chain/src/lib.rs:36`). `corelink-audit` owns the event taxonomy and the PII-as-hash type system; `corelink-audit-chain` builds and daily-verifies the per-tenant hash chain; `corelink-transparency-log` is the seam that witnesses a signed entry to the public sigstore/Rekor log.
...
12. `crates/corelink-container/src/routes/dsr/audit.rs:7-8`, `crates/corelink-container/src/routes/dsr/audit.rs:82-84` — the live DSR audit sink: appends *unchained* CloudEvents to `audit_outbox` with `digest=NULL`, `emitted_at=NULL` (no `prev_hash`/`sequence_number`); nothing ever seals/`SET`s those columns, so the deployed trail is a mutable D1 table; the chain-sealing drain that would link them is the unbuilt WI-S09-007 — i.e. the chain crates have no live producer.
