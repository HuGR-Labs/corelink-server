...
description: "The audit and observability layer — CloudEvents taxonomy with PII-as-hash typing, a mounted legacy per-tenant BLAKE3 drain with signed heads, a daily verifier, and an unwired witnessed-epoch writer. The current drain fails closed on versioned checkpoints until the external witness runtime is wired. Rekor submission remains a separate post-hoc seam. `corelink-audit` SHA-256 link primitives are dead/legacy; the mounted legacy link hash is `corelink-audit-chain` BLAKE3."
source_files:
...
- **The mounted legacy chain producer is the hourly audit-drain** (`main.rs`): it seals pending `audit_outbox` rows (written plain/unchained first, `emitted_at IS NULL`) into a per-tenant legacy BLAKE3 chain in D1 (`crates/corelink-container/src/routes/audit_drain/b126_m2_impl_01_part2.rs:244-254`; `crates/corelink-audit-chain/src/chain.rs:183`) and advances the head from `drain_partition_inner` (`crates/corelink-container/src/routes/audit_drain/b126_m2_impl_02_part2.rs:165-351`). The legacy link is un-keyed, so the body is tamper-evidence rather than proof against an insider with D1 write. CF-6 signs and verifies legacy checkpoint heads with Ed25519 (`crates/corelink-container/src/routes/audit_drain/b126_m2_impl_01.rs:366-447`; `b126_m2_impl_02.rs:9-66`). The versioned witnessed writer is present but the mounted legacy route rejects versioned checkpoints until the external witness runtime is wired (`b126_m2_impl_02.rs:58-95`). The ≤1h pre-seal window remains mutable and R2 Object-Lock is unwired; an independent prior head is needed to detect a full D1 rewrite.
...
2e. `crates/corelink-container/src/routes/audit_drain/b126_m2_impl_02_part2.rs:165-351` — mounted `drain_partition_inner`, which validates the legacy checkpoint, seals rows, and advances the D1 head; versioned checkpoints fail closed at this legacy boundary. The partition drain also applies the flag-gated lease and seal-loop fence.
...
2g. `crates/corelink-container/src/routes/audit_drain/b126_m2_impl_02.rs:8-66` — `check_head_on_resume`: re-verify the resumed head signature; a non-verifying / unseeded signed head = TAMPER → fail-CLOSED (SEV-1).
