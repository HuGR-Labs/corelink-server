...
14. `crates/corelink-container/src/routes/cas.rs:919` + `crates/corelink-container/src/routes/cas.rs:1085` — the LIVE native CAS write enforcer: `handle_write` / batch call `state.write.write(req)`, a `CasWriteHandler` trait object where content-verify + R2 PUT + D1 commit happen (the real verify→R2→D1 guarantee on the native path).
