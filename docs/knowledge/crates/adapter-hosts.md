...
checkpoint_sha: "3620515351982d28dfcd6292f8b7e8ec8918ac9d"
provenance: "AUTHORED"
...
- The bridge is REST-only by invariant `INV-BAZEL-NO-GRPC` — no tonic/prost/gRPC dependency (`crates/corelink-bazel-bridge/src/lib.rs:48-50`). The invariant id is spelled correctly as `INV-BAZEL-NO-GRPC` in the code (the prior `GROPC` one-char typo was fixed in #516).
