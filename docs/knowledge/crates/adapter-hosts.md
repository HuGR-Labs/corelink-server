...
CoreLink sells one cache but speaks many protocols: cargo, npm, pip, brew, OCI, Bazel REAPI, Turborepo — plus a `corelink-byok` microkernel for encrypting at rest under a customer's own KMS. **Carve-out: BYOK at-rest encryption is DESIGNED-not-wired** — the `corelink-byok` crate and the container's BYOK provider orchestrator are real, but no `KmsProvider`/`wrap_dek` is invoked on the container's CAS storage write path (zero such references under `crates/corelink-container/src/storage/`), so "it MUST encrypt at rest under a customer's own KMS" is engineered intent, not a live control today. This cluster is the set of translation crates that keep that protocol/provider sprawl out of the core. Each adapter maps a foreign wire format onto the same canonical workspace SPI traits (`CasReadHandler`, `CasWriteHandler`, `PatValidator`, `KvBackend`, `AuditEmitter`) so the CAS core never learns what a Homebrew bottle or an OCI manifest is; and `corelink-byok` keeps the four KMS providers behind a compile-time microkernel so exactly one provider SDK is linked per build.
