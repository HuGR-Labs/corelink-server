...
- `corelink-pat` defines the canonical PAT plaintext `corelink_<env>_<token_id>.<random_secret>.<hmac_sig>` and a four-step verify pipeline: parse → `verify_hmac_sig` (rejects unsigned spam in ≤100µs) → DB row lookup by `token_id` → `verify_argon2id` possession proof (`crates/corelink-pat/src/lib.rs:11-43`).
- The cold path (parse fail / sig mismatch / row absent) must call `dummy_verify_for_constant_time` so end-to-end latency cannot be used as an existence oracle (`crates/corelink-pat/src/lib.rs:38-43`, `crates/corelink-pat/src/lib.rs:105-108`).
...
- The HMAC fast-fail layer runs before any DB hit, bounding the cost a forged token can impose (`crates/corelink-pat/src/lib.rs:28-36`).
...
5. `crates/corelink-pat/src/lib.rs:28-36` — HMAC fast-fail rejects unsigned spam pre-DB in ≤100µs.
6. `crates/corelink-pat/src/lib.rs:38-43` — the constant-time dummy-verify pad against the existence oracle.
7. `crates/corelink-pat/src/lib.rs:45-56` — secret-handling newtype invariants (no Display/Serialize; zeroize; ≥32B key).
8. `crates/corelink-pat/src/lib.rs:105-108` — `dummy_verify_for_constant_time` / Argon2id exports.
