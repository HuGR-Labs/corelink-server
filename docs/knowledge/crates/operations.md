...
source_blobs:
  - "crates/corelink-ratelimit/src/limiter.rs@bfce13dc8812d826bf0cb1a18b4a0a810d3c2647"
...
- `INV-AVAIL-ISOLATION` / `INV-TENANT-ISOLATION`: rate-limit buckets are per-tenant with a tenant-leftmost PK and an executed tenant-mismatch guard, so cross-tenant throttling is impossible by design (`crates/corelink-ratelimit/src/limiter.rs:535-544`).
...
5. `crates/corelink-ratelimit/src/limiter.rs:535-544` — `INV-AVAIL-ISOLATION` / `INV-TENANT-ISOLATION`: the executed tenant-mismatch guard.
