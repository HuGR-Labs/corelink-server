...
- `crates/corelink-container/src/routes/public_revoke.rs:171` — the `_public` revocation route holds the SAME `Arc<dyn CasBlobEraser>` seam (second consumer beyond DSR erase).
- `crates/corelink-container/src/routes/public_revoke.rs:486` — `build_state_from_env` mounts the revoke route fail-CLOSED, reusing the erase seam + `CORELINK_ERASE_AUTH_KEY`.
