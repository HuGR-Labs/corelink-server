...
- `corelink-config-do` holds a versioned `ConfigPayload` (feature flags + rate-limit tunables + retention) with a rollback API (`crates/corelink-config-do/src/lib.rs:1-48`); the store's `update` does CAS atomicity (`current_version != expected_version` → `VersionConflict`) with `validate_payload` before the lock (`crates/corelink-config-do/src/store.rs:230-255`) and fail-CLOSED audit-before-mutation ordering — the executed `self.audit.emit(&entry)?` `:273` runs BEFORE the first `state.…insert` `:276` (`crates/corelink-config-do/src/store.rs:272-276`); the version is a monotonic `u64` counter — `new_version = state.current_version + 1` (`crates/corelink-config-do/src/store.rs:257`), and the CAS conflict is a counter compare (`current_version != expected_version`, `crates/corelink-config-do/src/store.rs:243`); `payload_hash` is a SEPARATE content-integrity field on `ConfigVersionEntry` — SHA-256 over the JCS-canonical payload (`crates/corelink-config-do/src/hash.rs:20-32`), NOT the version — and a write is gated by `validate_payload` first (`crates/corelink-config-do/src/validation.rs:22-57`).
...
- Config writes are atomic and fail-CLOSED: the store's `update` rejects on `current_version != expected_version` with `VersionConflict` (`crates/corelink-config-do/src/store.rs:243-255`), and audit `emit` runs BEFORE state mutation so a failed emit aborts the write unchanged — the executed `self.audit.emit(&entry)?` at `:273` `?`-propagates before the first `state.payloads.insert(...)` at `:276` (`crates/corelink-config-do/src/store.rs:272-276`).
...
6. `crates/corelink-config-do/src/hash.rs:20-32` — `compute_payload_hash`: SHA-256 over the JCS-canonical payload — the content-integrity `payload_hash` field, NOT the version. The version is the monotonic counter `state.current_version + 1` (`crates/corelink-config-do/src/store.rs:257`), with the CAS conflict at `:243`.
7. `crates/corelink-config-do/src/store.rs:243-255` — `update`'s CAS `expected_version`/`VersionConflict` reject (with `validate_payload` at `:231`); the audit-before-mutation enforcer is the executed `self.audit.emit(&entry)?` `:273` BEFORE the first `state.payloads.insert(...)` `:276` (`crates/corelink-config-do/src/store.rs:272-276`).
