...
Today everything lives in `HuGR-dev/corelink-server` (closed).
Post-GA we **split**:

| Repo | Visibility | Contents |
|---|---|---|
| `HumanGuardrail/corelink` | **PUBLIC** | OSS crates (above) + customer-facing docs + SDK examples. |
| `HuGR-dev/corelink-server` | **PRIVATE** | Closed crates + server deployment + compliance docs + internal runbooks. |
