...
| **C4b** Blob anchor valid (fail-closed) | Every `source_blobs` entry is `path@<40-hex>`, its path is declared in `source_files`, appears at most once, and the id names a **blob object present in this clone** AND is the content that path had at some commit **reachable from HEAD**. There is NO tolerance and NO fallback on this path: a commit id can legitimately be unreachable (rebase/squash rewrote it) but a blob id cannot — blobs are content hashes and survive history rewriting, so the blob that LANDS is in the `fetch-depth: 0` clone. Presence alone is not enough: an anchor taken from an INTERMEDIATE PR commit that a later commit on the same branch superseded never lands, and is reachable only from the PR head ref GitHub auto-deletes at merge — so a `push:main` run cloning seconds after the merge resolves it (false green) while every later clone cannot. Reachability from HEAD is a property of the gated history, not of when the gate cloned, so it returns the same verdict at merge time and forever after. | Malformed entry, an anchor for an undeclared path, a duplicate anchor, a **commit** id where a blob id belongs (right shape, wrong object type), an id that resolves to nothing, or an id that resolves but names content this history never carried. This is what closes the forged-unreachable-SHA laundering path C4 accepts: an absent blob REDs instead of re-anchoring to a base ref that already contains the landed drift. |
...
| **C5c** Anchor re-verification (B-123) | When a concept adds or changes a `source_blobs` anchor, compare the previous concept's cited content with the new anchored file. If that content moved, the corresponding citation MUST move by the same line offset. Unique content matches are required; rewritten or ambiguous blocks are undecidable and remain C5's/human review's responsibility. | A moved, non-blank cited block is uniquely present at new line(s), but the corresponding current citation still names the old range. This prevents advancing an anchor from making C5 compare the file with itself. |
