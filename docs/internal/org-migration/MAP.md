...
| --- | --- | --- |
| Repositório coberto | `corelink-server`, ID `1232040291` | Somente este repositório está no escopo do kit. |
| Origem | `HuGR-Labs`, owner ID `311862110` | Confirmar por ID e leitura autenticada; redirect não prova identidade. |
| Destino | Não definido no perfil de exemplo | Registrar owner/ID aprovados e fazer readback de identidade, elegibilidade, namespace e autoridade. |
...
Essa matriz cobre os controles GitHub, hosted/self-hosted e externos do escopo do servidor. O auditor examina exclusivamente objetos da árvore Git; não descobre nem certifica qualquer linha live acima, permissões atuais, hosted plane ou consumers externos. Busca vazia, `403`/`404`, truncamento, redirect ou falha de ferramenta permanecem `UNKNOWN`.

## Auditoria e incerteza

Uma execução de auditoria é triagem textual de um commit explícito; caminhos, extensões, blobs opacos, histórico não alcançado, settings live e consumidores externos requerem revisão. Cada achado fica `UNRESOLVED` até disposição por ocorrência, owner, evidência e revisor. Não inferir ausência de acesso negado.
