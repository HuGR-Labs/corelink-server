...
Esta matriz registra contraexemplos para o desenho e uso do kit. É um roteiro de verificação, não aprovação independente nem atestado live. O checker confirma estrutura, não a verdade do operador.
...
| A18 | Tratar um kit, checklist ou relatório local como autorização operacional. | Ferramentas preservam `migration_ready:false` e `authorization_verified:false`; autoridade e gates são aceites separadamente. |
