...
**`corelink-admin.humangr.com`**; API: **`corelink-api.humangr.com`**.
...
| 4 | **e2e / playwright prod defaults** | `.github/workflows/e2e-prod.yml:52`, `apps/admin-ui/playwright.prod.config.ts:30`, `apps/admin-ui/e2e/signup-welcome.spec.ts:29` | Fallbacks default to dead `app.corelink`. If repo var `E2E_BASE_URL=https://corelink-admin.humangr.com` is set, the gate passes; otherwise it fails. **Set the repo var** or align the fallbacks. | Med (gate hygiene) |
...
- Confirm Clerk **redirect URLs** use `https://corelink-admin.humangr.com`
