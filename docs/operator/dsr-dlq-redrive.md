...
The receipt is atomically claimed, then a durable `ambiguous` send intent is
written before the queue call. This fence prevents lease cleanup from reopening
the receipt while submission is in flight. A successful send advances the
intent to `submitted`; an exception, a stale claim lease, or a post-send
receipt failure remains `ambiguous` and must not be retried automatically. The
