...
**Root cause:** The `STRIPE_WEBHOOK_SECRET` set in the worker via `wrangler secret put` did not match the signing secret of Stripe webhook endpoint `we_1TcaMDLh0hhAZjwol9KDCJTp`. The `.env.local` value (`whsec_<REDACTED — a REAL webhook signing secret was committed here and redacted at HEAD on 2026-06-18 by the gitleaks baseline. **Credential status: REVOKED — closed 2026-08-03, nothing left to rotate.** A Stripe signing secret is scoped to exactly ONE endpoint, and the endpoint above was DELETED on 2026-05-29 (Fix step 1 below) and replaced by `we_1TcaeeLh0hhAZjwoWambOOhJ`; deleting the endpoint revokes its secret. Three independent confirmations: (1) this section; (2) the live Stripe API enumeration of every v1 webhook endpoint on the production account in `docs/handoff/2026-07-03-REPLY-from-clw-coordinator-webhook-reconcile-DONE-and-no-stray-endpoint-exists.md` (2026-07-03) — the deleted endpoint is absent; (3) the owner's read of the live Stripe dashboard on 2026-08-03 — absent again. `gitleaks detect` scans the COMMIT GRAPH, so this finding survives in commit `b59c4862` no matter what HEAD says; it is therefore pinned closed by a commit+path-scoped allowlist entry in `.gitleaks.toml`, not edited away>`) was stale/wrong.  
**Fix:**
