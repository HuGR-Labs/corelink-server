...
Cada PR recebe exatamente um cold review de um Luna independente. O reviewer recebe issue, cinco axiomas, base/head, diff completo e receipts hospedados, sem o raciocínio do autor. Ele retorna APPROVE, FIX-FIRST com uma lista consolidada, BLOCKED ou TERMINAL_REJECT. Existe no máximo uma rodada consolidada de correção. Depois dela, o mesmo reviewer verifica o SHA exato corrigido apenas contra a lista original e regressões introduzidas pela correção, e retorna APPROVE final ou encerramento terminal. Isso faz parte do mesmo ciclo limitado; não abra nova descoberta de defeitos, loops de review ou successor PRs.
