-- Durable, single-use nonce ledger for authenticated staging admission.
-- Raw nonces and credentials are never persisted by this migration.

CREATE TABLE IF NOT EXISTS staging_load_test_admission_nonces (
    nonce_digest TEXT NOT NULL CHECK (
        length(nonce_digest) = 64 AND nonce_digest NOT GLOB '*[^0-9a-f]*'
    ),
    run_id TEXT NOT NULL,
    scenario TEXT NOT NULL,
    issued_at_ms INTEGER NOT NULL CHECK (issued_at_ms >= 0),
    expires_at_ms INTEGER NOT NULL CHECK (expires_at_ms > issued_at_ms),
    consumed_at_ms INTEGER NOT NULL CHECK (
        consumed_at_ms >= issued_at_ms AND consumed_at_ms < expires_at_ms
    ),
    PRIMARY KEY (nonce_digest),
    UNIQUE (run_id, scenario),
    FOREIGN KEY (run_id, scenario)
        REFERENCES staging_load_test_runs(run_id, scenario)
);

CREATE TRIGGER IF NOT EXISTS trg_staging_load_test_admission_nonce_no_update
BEFORE UPDATE ON staging_load_test_admission_nonces
FOR EACH ROW
BEGIN
    SELECT RAISE(ABORT, 'staging admission nonce ledger is append-only');
END;

CREATE TRIGGER IF NOT EXISTS trg_staging_load_test_admission_nonce_no_delete
BEFORE DELETE ON staging_load_test_admission_nonces
FOR EACH ROW
BEGIN
    SELECT RAISE(ABORT, 'staging admission nonce ledger is append-only');
END;
