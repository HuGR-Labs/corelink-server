...
-- must read terms from this table and claim every staged event in the same D1
...
-- A claim is the durable, per-event boundary for a logical aggregate batch.
-- The writer uses the #1635 disposition semantics: a first insert is Inserted;
-- an exact durable fingerprint replay is Deduped; every other existing winner
-- is Conflict and aborts the complete D1 batch before any accounting mutation.
