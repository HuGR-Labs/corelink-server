export const meta = {
  name: 'corelink-redteam-nuclear-r34',
  description: 'NUCLEAR red-team — FOCUSED recovery variant: runs ONLY the rate-limited ROUND 3 (deep/TOCTOU/composition) + ROUND 4 (exotic/critic-seeded) + chain + critic against the now-HARDENED main, deduped against the prior cycle-1+cycle-2 findings (passed as args) so it reports only STILL-open or NET-NEW.',
...
// FOCUSED variant: the prior cycle-1+cycle-2 findings (passed as args) are
// injected into every hunt directive so the hunters DEDUP against them and
// report only STILL-open-despite-the-fix or genuinely NET-NEW. Skip ROUND 1+2
// (already run); start at ROUND 3 (the rate-limited deep/TOCTOU + exotic rounds).
const PRIOR = (typeof args === 'string' && args.trim())
  ? `\n\nPRIOR FINDINGS (the audit ALREADY found + mostly FIXED these — do NOT re-report as-is; report ONLY what is STILL exploitable despite the fix, or genuinely NET-NEW):\n${args}`
  : ''
const START_ROUND = 2 // 0-indexed: ROUND 3

for (let round = START_ROUND; round < ROUND_DIRECTIVES.length; round++) {
