...
**1. We split GA into two gates.** An engineering gate that is binary and unappealable — PRR globally approved, completed external pentest with retest, 30 days of sustained staging, three lighthouse customers with SLA met, SOC 2 gap analysis delivered, zero active waivers in CRITICAL controls. The external pentest remains planned; no vendor has been contracted or report issued today. And a launch orchestration gate — this post, the press release, the blog series — which is soft and could shift its date without affecting the engineering decision. The engineering gate gates the launch. Not the other way around.
...
To the engineering team at HuGR Labs and the engineering teams at the three lighthouse customers — thank you. To external counsel (Cooley / DLA Piper / Bird & Bird) for the DPA review, to the planned pentest engagement that will challenge the things we want it to find, to the on-call rotation for the synthetic page exercises that nobody enjoys — thank you.
