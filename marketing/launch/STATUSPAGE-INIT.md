...
> **Purpose:** the **concrete, click-by-click** provisioning playbook that takes us from "no account" to "`status.corelink.dev` live with 8 components, PagerDuty hooks armed, lighthouse subscribers imported". This implements `STATUS-PAGE-SPEC.md` §1–§7.
...
> **Hard rule:** **status page URLs in this playbook are placeholders** until DevOps confirms account creation at step 1; do not flip DNS or publish links to `status.corelink.dev` until §3 is complete.
...
1. DevOps signs into Atlassian under the **`corelink.dev`** organization (Atlassian admin URL: `https://admin.atlassian.com/o/<org-id>`).
...
4. Billing contact: `finance@corelink.dev`. Technical contact: `sre@corelink.dev`.
...
2. **Create new page:**
   ...
   - Support URL: `https://corelink.dev/support`
   - Notification "from" email: `status@corelink.dev`
...
- Add the following Atlassian accounts with **Page Admin** role:
  - `sre-lead@corelink.dev`
  - `devops@corelink.dev`
  - `vpsec@corelink.dev` (read-write for severity gating)
- Add with **Page Editor** role:
  - `vpmkt@corelink.dev`
  - `support-lead@corelink.dev`
- Add with **Page Viewer** role:
  - `ceo@corelink.dev`
  - `cto@corelink.dev`
- **Do not** add personal Gmail accounts; only `corelink.dev` SSO-backed accounts. Atlassian SSO is configured via the existing Atlassian Access subscription (`docs/internal/sso-vendor-registry.md`).
...
4. **Footer:** in Page → Style → Custom HTML/CSS footer, paste:
   ...
   ```html
   <p>
     <a href="https://corelink.dev/security.txt">security.txt</a> ·
     <a href="https://corelink.dev/SECURITY.md">Security policy</a> ·
     <a href="https://corelink.dev/trust">Trust center</a>
   </p>
   ```
...
## 3. Custom domain — `status.corelink.dev`

### 3.1 Statuspage side

1. Page settings → **Domain & SSL** → **Use a custom domain** → enter `status.corelink.dev`.
...
1. Open the Cloudflare zone for `corelink.dev`.
...
5. Confirm `https://status.corelink.dev` resolves and shows the (still empty) status page.

### 3.3 Fallback static page

Per `STATUS-PAGE-SPEC.md` §2, pre-stage a static fallback at `https://corelink.dev/status`:

1. Author `apps/docs/static/status/index.html` with the message:
   > "We are checking with our status provider. For real-time updates please follow `@corelinkdev` on X/Twitter or email `status@corelink.dev`."
2. Commit + deploy as part of the `wt-r8-1` work-tree.
3. **Validation:** `curl -sI https://corelink.dev/status` returns 200 before T-24h.
...
| **C7** | `Docs` | "docs.corelink.dev Docusaurus site + CDN; includes security.txt + trust center." | `Operations` | Public | availability |
| **C8** | `Admin Console` | "admin.corelink.dev operator console — Clerk-gated; org/tenant/user admin." | `Operations` | Public | availability |
...
3. Update the static fallback page at `corelink.dev/status` (single-line replacement via `apps/docs/static/status/index.html` push + `wrangler pages publish`).
...
10. Page → **Domain & SSL** → request custom domain `status.corelink.dev`; copy CNAME target.
...
13. Confirm `https://status.corelink.dev` resolves with valid TLS.
...
28. Run `STATUSPAGE-PRE-LAUNCH-TEST.md` end-to-end. Until that test passes, **do not** import lighthouse subscribers and **do not** announce `status.corelink.dev` externally.
...
- **Cost monitoring.** Atlassian invoice posts to `finance@corelink.dev` monthly. Track against the budget line `H-18` in `ROADMAP-TO-GA.md`; SMS-fanout cost is the unpredictable variable — review monthly.
