...
| Row | Time | Phase | Owner | Action | Success metric | Fallback if action fails |
|---|---|---|---|---|---|---|
| L1 | T-24h 06:00 PT | Engineering Gate revalidation | CEO + CTO | Reconfirm `PRR-S20-GA` work_status = `APPROVED`; no SEV-1 in last 48h staging. `CAP-GA-002` must have an external report and passed retest with no outstanding HIGH/CRITICAL findings. | Written GO recorded in war room log. | **DEFER** — invoke `CRISIS-COMMS-TEMPLATES.md` §F (defer comms). Marketing reverts press kit recall. No further row executes. |
