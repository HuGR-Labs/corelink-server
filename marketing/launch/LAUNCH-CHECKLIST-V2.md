...
| Row | Time | Phase | Owner | Action | Success metric | Fallback if action fails |
|---|---|---|---|---|---|---|
| L1 | T-24h 06:00 PT | Engineering Gate revalidation | CEO + CTO | Reconfirm `PRR-S20-GA` work_status = `APPROVED`; no SEV-1 in last 48h staging. | Written GO recorded in war room log. | **DEFER** — invoke `CRISIS-COMMS-TEMPLATES.md` §F (defer comms). Marketing reverts press kit recall. No further row executes. |
...
| L23 | T-0 06:00:30 PT | Trust center unlock | VPSec | Unlock TLA+ specs, SBOM, DPA package at `corelink-docs.humangr.com/trust`. (No pentest letter — none has been commissioned.) | All 3 assets behind their gates load on incognito. | If NDA-gated download breaks: temporarily route to email-on-request; do not expose pentest letter publicly. |
