...
| L22 | T-0 06:00:00 PT | Status page → "All systems operational" | SRE-OC | Flip status page banner from "Preparing" to "All Systems Operational"; publish launch announcement post. | Banner green; RSS feed updated. | Statuspage.io outage → manual `corelink.humangr.com/status` static page update; raise SEV-2 internally. |
| L23 | T-0 06:00:30 PT | Trust center unlock | VPSec | Unlock TLA+ specs, SBOM, pentest letter (NDA-gated download), DPA package at `corelink-docs.humangr.com/trust`. | All 4 assets behind their gates load on incognito. | If NDA-gated download breaks: temporarily route to email-on-request; do not expose pentest letter publicly. |
