...
| Row | Time | Phase | Owner | Action | Success metric | Fallback if action fails |
|---|---|---|---|---|---|---|
| L1 | T-24h 06:00 PT | Engineering Gate revalidation | CEO + CTO | Reconfirm `PRR-S20-GA` work_status = `APPROVED`; no SEV-1 in last 48h staging; completed external pentest report and passed retest on file. | Written GO recorded in war room log. | **DEFER** — invoke `CRISIS-COMMS-TEMPLATES.md` §F (defer comms). Marketing reverts press kit recall. No further row executes. |
...
| L22 | T-0 06:00:00 PT | Status page → "All systems operational" | SRE-OC | Flip status page banner from "Preparing" to "All Systems Operational"; publish launch announcement post. | Banner green; RSS feed updated. | Statuspage.io outage → manual `humangr.com/corelink/status` static page update; raise SEV-2 internally. |
| L23 | T-0 06:00:30 PT | Trust center unlock | VPSec | Unlock TLA+ specs, SBOM, current pentest status (the external engagement remains planned), DPA package at `corelink-docs.humangr.com/trust`. | All assets behind their gates load on incognito. | If the status page breaks: temporarily route to email-on-request; do not imply that a report exists. |
...
| L29 | T+15min 06:15 PT | Support inbox check | CS-OC | Open support@humangr.com; categorize first 10 tickets. | Inbox triaged; no SEV-1 customer reports. | SEV-1 customer ticket → invoke `CRISIS-COMMS-TEMPLATES.md` §A; status page update. |
