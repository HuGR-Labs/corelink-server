...
| D.6 | Are licenses (OSS + commercial) tracked? | Y | OSS license allowlist + CycloneDX SBOM + Dependency-Track per release. `LICENSE-APACHE-2.0` (code) + `LICENSE-MIT` (selected) + OSS matrix landed 2026-05-15 (commit `44cdf15`). | `LICENSE-APACHE-2.0`; commit `44cdf15`; ADR-0037 |
| D.7 | Do you maintain a software bill of materials (SBOM)? | Y | CycloneDX SBOM signed and published with every release; SLSA Level 3 build attestation; Cosign-signed releases; Rekor transparency entries. | `apps/docs/docs/trust/iso27001.mdx` §A.5.21; release artifacts `github.com/humangr-labs/corelink/releases` |
...
| G.9 | Is patch management automated for OS / runtime? | Y | Cloudflare-managed runtime; CoreLink crates pull-request-tested + Dependency-Track + cargo-deny daily. | ADR-0037 Dependency-Track |
...
| G.11 | Are pentest findings tracked to remediation? | P | Internal adversarial summaries tracked; external pentest scoped under R-6 staging-bake. | `specs/_audits/2026-05-14-cargo-fuzz-summary-s15.md` |
...
| I.5 | Is static application security testing (SAST) automated? | Y | CodeQL + Semgrep (custom rules) on every PR. Baseline `specs/_audits/2026-05-15-static-analysis-baseline.md`. Triage runbook `specs/_runbooks/RB-STATIC-ANALYSIS-TRIAGE.md`. | SOC 2 rollup §2.8 (CC8.1) |
| I.6 | Is dynamic / fuzz testing performed? | Y | cargo-fuzz daily; summary at `specs/_audits/2026-05-14-cargo-fuzz-summary-s15.md`. Property tests (100k synthetic PANs + 10k concurrent signup). | `crates/corelink-logpush/tests/pii_redaction_100k_synthetic.rs` |
| I.7 | Is software composition analysis (SCA) automated? | Y | Dependency-Track + cargo-deny + license allowlist (ADR-0037). DAILY cadence. | SOC 2 rollup §2.7 (CC7.1) |
...
| K.4 | Do you support GDPR? | Y | Yes — processor (joint controller for limited service-telemetry purposes). DPA at `legal/dpa/v1.0.0` (EN-EU+UK, PT-BR, ES-LATAM). SCC Modules 2/3 executed (`specs/_compliance/GDPR-SCC-EXECUTION-2026-05-15.md`). DPIA library `specs/_compliance/GDPR-DPIA-LIBRARY.md`. DSR rights (Arts. 15–22). | `legal/dpa/v1.0.0`; `specs/_compliance/GDPR-FULL-AUDIT-2026-05-15.md` |
...
| M.7 | Are data-residency commitments enforced? | Y | **`primary_region` pin is structural** — `INV-REGION-NO-CROSS-LEAK` CRITICAL invariant. Verifiable via `GET /v1/tenant/me/residency-proof` (signed attestation + Merkle proof). | `apps/docs/docs/trust/data-handling.mdx#residency`; `specs/_compliance/LGPD-RESIDENCY-ATTESTATION-2026-05-15.md` |
...
| # | Question | A | CoreLink answer | Evidence |
|---|---|---|---|---|
| N.1 | Where is customer data physically hosted? | Y | Per-tenant `primary_region` pin (closed enum): `sam` São Paulo, `nam` Iowa/Virginia, `eur` Frankfurt/Dublin, `oce` Sydney, `apc` Tokyo/Singapore, `mea` Dubai. Storage: Cloudflare R2 (blobs) + D1 (metadata) + DO (per-tenant state) + KV (hot-path counters). | `apps/docs/docs/trust/data-handling.mdx#residency` |
...
| N.4 | Is data encrypted at rest? | Y | AES-256-GCM on R2 / D1 / KV / DO. Optional BYOK envelope encryption per blob (per-tenant DEK wrapped by tenant's KMS root). 4 KMS providers: AWS KMS, GCP Cloud KMS, Azure Key Vault, HashiCorp Vault. | `apps/docs/docs/trust/data-handling.mdx#encryption`; `compliance/byok-fips-matrix.md` |
