...
| K.10 | Are cross-border data transfers contractually governed? | Y | SCC Modules 2/3 in DPA + **one** supplementary measure: the EU-region pin, a served fail-closed guard (`INV-REGION-NO-CROSS-LEAK`). **BYOK envelope encryption is NOT a supplementary measure here and this row previously counted it as one — BYOK is not shipped** (activation returns `501 byok_not_available`; the shipped default binary reports `ActiveProvider::Unavailable`; no real provider is compiled in — see N.6). The EDPB-aligned Schrems II TIA is documented but was written counting customer-held keys, and must be re-done without them. **EU (`weur`) tenants' data stays in the EU** (physically-EU R2 buckets via the `lhr` cluster), so for them there is no third-country transfer to assess. US (`enam`) is the default region; additional jurisdictions (`sam`, APAC) are on the roadmap, governed by SCCs in the interim. | `specs/_compliance/GDPR-SCC-EXECUTION-2026-05-15.md`; `legal/dpa/v1.0.0` §6 |
...
| N.2 | Who are your hosting / IaaS providers? | Y | Cloudflare primary data plane (Workers / R2 / D1 / DO / KV / Pages / Email). **No other IaaS provider hosts customer data.** AWS / GCP / Azure / HashiCorp Vault appear in the BYOK design as customer-side KMS custodians, but **BYOK is not shipped** (activation returns `501 byok_not_available`; the shipped default binary reports `ActiveProvider::Unavailable`; no real provider is compiled in — see N.6), so today **no CoreLink tenant reaches any of them and none of them holds any key on CoreLink's behalf.** They are listed for the designed flow only, not as anything in service. | `apps/docs/docs/trust/subprocessors.mdx` |
...
| N.4 | Is data encrypted at rest? | Y | AES-256-GCM on R2 / D1 / KV / DO, under provider-managed keys. Customer-managed BYOK envelope encryption per blob is **not shipped** — activation returns `501 byok_not_available` and the shipped default binary reports `ActiveProvider::Unavailable`; no real provider is compiled in. See N.6. | `apps/docs/docs/trust/data-handling.mdx#encryption`; `crates/corelink-container/src/routes/byok_admin.rs` |
