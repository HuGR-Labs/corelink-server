...
| K.5 | Do you support LGPD (Brazil)? | Y | Yes, as processor. DPO appointed; 72h ANPD notification; DSR support. Data is currently stored in a single US (ENAM) region under SCCs + supplementary measures for cross-border transfer; in-country Brazilian residency (`sam` São Paulo region pin) is on the roadmap (Enterprise-on-request, not GA). | `specs/_compliance/LGPD-FULL-AUDIT-2026-05-15.md` |
...
| K.10 | Are cross-border data transfers contractually governed? | Y | SCC Modules 2/3 in DPA + supplementary measures (BYOK envelope encryption). EDPB-aligned Schrems II TIA documented. Data is currently US-stored (ENAM); a per-jurisdiction region pin is on the roadmap, not GA. | `specs/_compliance/GDPR-SCC-EXECUTION-2026-05-15.md`; `legal/dpa/v1.0.0` §6 |
...
| M.7 | Are data-residency commitments enforced? | P | Today CoreLink operates from a single US (ENAM) region; all customer data is stored in the United States (no in-jurisdiction residency commitment is offered at launch). Per-region residency enforcement (`INV-REGION-NO-CROSS-LEAK` + the `GET /v1/tenant/me/residency-proof` attestation API) is on the roadmap, available to Enterprise on request — not GA. | `apps/docs/docs/trust/data-handling.mdx#residency` |
...
| # | Question | A | CoreLink answer | Evidence |
|---|---|---|---|---|
| N.1 | Where is customer data physically hosted? | Y | In the **United States** (Cloudflare ENAM / Eastern North America). CoreLink currently operates from a single US region; all CAS/AC data is stored in US Cloudflare R2 (blobs) + D1 (metadata) + DO (per-tenant state) + KV (hot-path counters). Other jurisdictions (`sam` São Paulo, `eur` Frankfurt/Dublin, `oce` Sydney, `apc` Tokyo/Singapore, `mea` Dubai) are roadmap items, Enterprise-on-request — not GA. | `apps/docs/docs/trust/data-handling.mdx#residency` |
