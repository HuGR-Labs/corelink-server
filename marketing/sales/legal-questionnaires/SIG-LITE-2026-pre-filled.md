...
| A.4 | Have you completed a third-party risk assessment / pentest in the last 12 months? | P | No external pentest has been commissioned. `CAP-GA-002` remains an unmet hard pre-GA gate; no external report, NDA availability, or date is promised. Internal adversarial-review + cargo-fuzz summaries exist at `specs/_audits/sealed/2026-05-14-cargo-fuzz-summary-s15.md`. | `specs/_audits/sealed/2026-05-14-cargo-fuzz-summary-s15.md`; `apps/docs/docs/trust/index.mdx` |
...
| G.2 | Is change management formally controlled? | Y | GitHub branch protection + `PAT-DUAL-APPROVAL-001` + signed-deploy pipeline + sprint-contract §5.1 sign-off + static-analysis gate (CodeQL + Semgrep custom rules). | SOC 2 rollup §2.8 (CC8.1) |
...
| K.10 | Are cross-border data transfers contractually governed? | Y | SCC Modules 2/3 in DPA + **one** supplementary measure: the EU-region pin, a served fail-closed guard (`INV-REGION-NO-CROSS-LEAK`). **BYOK envelope encryption is NOT a supplementary measure here and this row previously counted it as one — BYOK is not shipped** (activation returns `501 byok_not_available`; the only compiled provider is `InMemoryFake` — see N.6). The EDPB-aligned Schrems II TIA is documented but was written counting customer-held keys, and must be re-done without them. **EU (`weur`) tenants' data stays in the EU** (physically-EU R2 buckets via the `lhr` cluster), so for them there is no third-country transfer to assess. US (`enam`) is the default region; additional jurisdictions (`sam`, APAC) are on the roadmap, governed by SCCs in the interim. | `specs/_compliance/GDPR-SCC-EXECUTION-2026-05-15.md`; `legal/dpa/v1.0.0` §6 |
...
| N.2 | Who are your hosting / IaaS providers? | Y | Cloudflare primary data plane (Workers / R2 / D1 / DO / KV / Pages / Email). **No other IaaS provider hosts customer data.** AWS / GCP / Azure / HashiCorp Vault appear in the BYOK design as customer-side KMS custodians, but **BYOK is not shipped** (activation returns `501 byok_not_available`; the only compiled provider is `InMemoryFake` — see N.6), so today **no CoreLink tenant reaches any of them and none of them holds any key on CoreLink's behalf.** They are listed for the designed flow only, not as anything in service. | `apps/docs/docs/trust/subprocessors.mdx` |
...
| N.4 | Is data encrypted at rest? | Y | AES-256-GCM on R2 / D1 / KV / DO, under provider-managed keys. The "optional BYOK envelope encryption per blob" this row previously offered is **not shipped** — activation returns `501 byok_not_available` and the only compiled provider is `InMemoryFake`. See N.6. | `apps/docs/docs/trust/data-handling.mdx#encryption`; `crates/corelink-container/src/routes/byok_admin.rs` |
...
| N.6 | Are customer-managed encryption keys supported (BYOK / CMK)? | N | **No — BYOK is not shipped.** `POST /v1/admin/byok/activate` fails closed with `501 Not Implemented` / `byok_not_available` in the shipped binary (`crates/corelink-container/src/routes/byok_admin.rs:245-257`), and the only key provider compiled in is `InMemoryFake`. Four-provider envelope encryption (AWS KMS / GCP Cloud KMS / Azure Key Vault / HashiCorp Vault) is designed and wired into CAS+AC storage but gated-inert, with zero active tenants. The weekly kill-switch drill workflow (`.github/workflows/byok_kill_switch_drill_weekly.yml`, `cron: 0 3 * * 0`) is real and its 8 most recent runs (all `schedule`, 2026-07-05 → 2026-08-30) are green — but **the drill itself is a shell simulation with the PASS values hard-coded**. `scripts/byok_kill_switch_drill.sh` contacts no customer KMS (every KMS credential in the job is commented out), no CoreLink API, and **not the `InMemoryFake` provider either** — it invokes no CoreLink binary. `TENANT_STATUS="active"` (`:51`), `DETECTED=true` (`:82`), `TENANT_STATUS_POST="degraded_read_only"` (`:101`) and the rest are literals, so no assertion can fail; the `byok-kill-switch-rtt` "≤ 5 min" figure is `date +%s` across a `sleep 2` (`:62`, `:81`, `:112`) and measures nothing — `DETECT_LATENCY_S` (`:75` → `:86`) and `TOTAL_S` (`:62` → `:112`) span the **same** single `sleep 2`, and the two `sleep 1` calls (`:126`, `:128`) run after `:112`, outside both. The report step commits without pushing, so no run has left an artifact — `specs/_audits/` contains **zero** `*-byok-kill-switch-drill-*.md` files. Data at rest is AES-256-GCM under provider-managed keys. | `crates/corelink-container/src/routes/byok_admin.rs`; CAIQ CEK-09.1 / CEK-10.1 |
