...
| K.10 | Are cross-border data transfers contractually governed? | Y | SCC Modules 2/3 in DPA + **one** supplementary measure: the EU-region pin, a served fail-closed guard (`INV-REGION-NO-CROSS-LEAK`). **BYOK envelope encryption is NOT a supplementary measure here and this row previously counted it as one — BYOK has no verified customer rollout** (activation returns `501 byok_not_available` if provider construction or CMK access fails; the Dockerfile production build selects `byok-aws-real`, but customer CMK activation, revocation and kill-switch p99 lack runtime evidence — see N.6). The EDPB-aligned Schrems II TIA is documented but was written counting customer-held keys, and must be re-done without them. **EU (`weur`) tenants' data stays in the EU** (physically-EU R2 buckets via the `lhr` cluster), so for them there is no third-country transfer to assess. US (`enam`) is the default region; additional jurisdictions (`sam`, APAC) are on the roadmap, governed by SCCs in the interim. | `specs/_compliance/GDPR-SCC-EXECUTION-2026-05-15.md`; `legal/dpa/v1.0.0` §6 |
