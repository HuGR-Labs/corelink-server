...
| N.6 | Are customer-managed encryption keys supported (BYOK / CMK)? | N | **No — BYOK is not shipped.** `POST /v1/admin/byok/activate` fails closed with `501 Not Implemented` / `byok_not_available` in the shipped binary (`crates/corelink-container/src/routes/byok_admin.rs:245-257`), and the only key provider compiled in is `InMemoryFake`. Four-provider envelope encryption (AWS KMS / GCP Cloud KMS / Azure Key Vault / HashiCorp Vault) is designed and wired into CAS+AC storage but gated-inert, with zero active tenants. The weekly kill-switch drill (`.github/workflows/byok_kill_switch_drill_weekly.yml`) is real and green, but it exercises the in-memory fake — every KMS credential in the job is commented out — so the `byok-kill-switch-rtt` ≤ 5 min figure measures the fake, not a customer KMS. Data at rest is AES-256-GCM under provider-managed keys. | `crates/corelink-container/src/routes/byok_admin.rs`; CAIQ CEK-09.1 / CEK-10.1 |
