...
| BCR-04.1 | Backups encrypted? | Y | CSP | Same envelope as source: AES-256-GCM, provider-managed keys. 35-day rolling window. **BYOK is NOT shipped.** `POST /v1/admin/byok/activate` fails closed with `501 Not Implemented` / `byok_not_available` in the shipped binary (`crates/corelink-container/src/routes/byok_admin.rs:245-257`); the shipped default binary reports `ActiveProvider::Unavailable`; no real provider is compiled in. See CEK-09.1. Backups are encrypted; they are not customer-key-encrypted. | C1.1 |
...
| CEK-02.1 | Encryption at rest enforced? | Y | CSP | AES-256-GCM on R2 / D1 / KV / DO, provider-managed keys. No customer-managed encryption envelope exists: **BYOK is NOT shipped.** `POST /v1/admin/byok/activate` fails closed with `501 Not Implemented` / `byok_not_available` in the shipped binary (`crates/corelink-container/src/routes/byok_admin.rs:245-257`); the shipped default binary reports `ActiveProvider::Unavailable`; no real provider is compiled in. See CEK-09.1. | C1.1 |
...
| CEK-04.1 | Cryptographic algorithms compliant with industry standards (FIPS / NIST)? | P | CSP | AES-256-GCM, ChaCha20-Poly1305, BLAKE3 / SHA-256, Ed25519. The per-provider FIPS attestation matrix (`specs/_compliance/BYOK-FIPS-ATTESTATION-MATRIX.md`, **GAP-02**) describes BYOK KMS endpoints and is therefore **not in effect**. **Not shipped today: BYOK activation returns `501 byok_not_available` and the shipped default binary reports `ActiveProvider::Unavailable`; no real provider is compiled in — see CEK-09.1.** This row describes the designed customer-KMS lifecycle, which no tenant can enter. No FIPS-validated-module claim is made for the served path. | C1.1 |
| CEK-05.1 | Key generation procedures documented? | P | CSP | Documented, not served. KMS-side per provider (AWS / GCP / Azure / Vault); CoreLink never generates root keys. **Not shipped today: BYOK activation returns `501 byok_not_available` and the shipped default binary reports `ActiveProvider::Unavailable`; no real provider is compiled in — see CEK-09.1.** This row describes the designed customer-KMS lifecycle, which no tenant can enter. | C1.1 |
| CEK-06.1 | Key rotation supported? | P | CSP | **Customer-key rotation: no.** **Not shipped today: BYOK activation returns `501 byok_not_available` and the shipped default binary reports `ActiveProvider::Unavailable`; no real provider is compiled in — see CEK-09.1.** This row describes the designed customer-KMS lifecycle, which no tenant can enter. Provider-managed key rotation for data at rest is Cloudflare-side and transparent. | C1.1 |
| CEK-07.1 | Key escrow / recovery documented? | P | CSP | Documented as customer-side (the KMS provider handles escrow per the customer's IAM policy) — and CoreLink genuinely never holds CMK material, because it holds none at all. **Not shipped today: BYOK activation returns `501 byok_not_available` and the shipped default binary reports `ActiveProvider::Unavailable`; no real provider is compiled in — see CEK-09.1.** This row describes the designed customer-KMS lifecycle, which no tenant can enter. | C1.1 |
...
| CEK-09.1 | Customer-managed key option (BYOK) available? | N | CSP | **No — BYOK is not shipped.** Four-provider envelope encryption (AWS KMS / GCP Cloud KMS / Azure Key Vault / HashiCorp Vault) is design-only and gated-inert, with zero active tenants. `POST /v1/admin/byok/activate` fails closed with `501 Not Implemented` / `byok_not_available`; the default shipped binary reports `ActiveProvider::Unavailable`; no real provider is compiled in. Self-serve CMK onboarding is roadmap work, not a current capability. | C1.1 |
| CEK-10.1 | Kill-switch / key-revocation supported? | N | CSP | Not available — it is the revocation half of BYOK, and **BYOK is NOT shipped.** `POST /v1/admin/byok/activate` fails closed with `501 Not Implemented` / `byok_not_available` in the shipped binary (`crates/corelink-container/src/routes/byok_admin.rs:245-257`); the shipped default binary reports `ActiveProvider::Unavailable`; no real provider is compiled in. See CEK-09.1. **On the weekly drill, stated precisely, because the workflow is real but the drill is not:** `.github/workflows/byok_kill_switch_drill_weekly.yml` exists, is on `cron: 0 3 * * 0`, and its 8 most recent runs (all `schedule`, 2026-07-05 → 2026-08-30) are all `success`. What runs is `scripts/byok_kill_switch_drill.sh` (181 lines of shell), and **it is a shell simulation with the PASS values written into it as literals.** It contacts nothing: no customer KMS (every KMS credential in the job is commented out, `# AWS_ACCESS_KEY_ID: …`), no CoreLink API or D1 — and **not the `ActiveProvider::Unavailable` provider either**, because the script invokes no CoreLink binary at all. Every asserted outcome is a hard-coded assignment: `TENANT_STATUS="active"` (`:51`), `DETECTED=true` (`:82`), `DEK_CACHE_EMPTY=true` (`:100`), `TENANT_STATUS_POST="degraded_read_only"` (`:101`), `RECOVERY_STATUS="active"` (`:129`); the provider revoke/restore calls exist only as comments (`:64-67`, `:125`). The timing is `date +%s` taken before and after a `sleep 2` (`:62`, `:81`, `:112`), so **the `byok-kill-switch-rtt` "≤ 5 min" figure is a measurement of that sleep, not of any revocation**. Precisely: `DETECT_LATENCY_S` (`:75` → `:86`) and `TOTAL_S` (`:62` → `:112`) span the **same** single `sleep 2`, so the two reported numbers are not independent measurements; the two `sleep 1` calls (`:126`, `:128`) run after `:112` and fall outside both. And because no assertion can be false, the green result is structurally unfailable. No run has ever left evidence in the repo: the "Commit drill report" step of the workflow `git commit`s without `git push` (cited by step name, not line number, because the file shifts), and `specs/_audits/` contains **zero** `*-byok-kill-switch-drill-*.md` files. Treat the drill as unexecuted for assurance purposes. | C1.1 |
| CEK-11.1 | HSM-protected key material? | N | CSP | CoreLink holds no customer key material in an HSM, because it holds no customer key material at all: **BYOK is NOT shipped.** `POST /v1/admin/byok/activate` fails closed with `501 Not Implemented` / `byok_not_available` in the shipped binary (`crates/corelink-container/src/routes/byok_admin.rs:245-257`); the shipped default binary reports `ActiveProvider::Unavailable`; no real provider is compiled in. See CEK-09.1. The HSM posture described in `BYOK-FIPS-ATTESTATION-MATRIX.md` is the posture those providers *would* bring once BYOK ships. | C1.1 |
...
| CEK-16.1 | Key-management audit logged? | P | CSP | The audit chain and `INV-AUDIT-APPEND-ONLY` are real and cover served key operations. The `EVT-KMS-*` events are BYOK events and are **never emitted in production**. **Not shipped today: BYOK activation returns `501 byok_not_available` and the shipped default binary reports `ActiveProvider::Unavailable`; no real provider is compiled in — see CEK-09.1.** This row describes the designed customer-KMS lifecycle, which no tenant can enter. | PI1.2 |
| CEK-17.1 | Key destruction / decommissioning documented? | P | CSP | **Customer-controlled crypto-shredding is not available.** **Not shipped today: BYOK activation returns `501 byok_not_available` and the shipped default binary reports `ActiveProvider::Unavailable`; no real provider is compiled in — see CEK-09.1.** This row describes the designed customer-KMS lifecycle, which no tenant can enter. What IS served is verifiable erasure: `DELETE /v1/tenant/me` with `INV-DATA-ERASURE-COMPLETE` + `INV-ERASURE-ATTESTATION-SIGNED` (see DSP-08.1). | C1.1 |
| CEK-18.1 | Cryptographic boundaries documented? | P | CSP | `specs/03_architecture/security_model.md` documents the served boundaries. The BYOK key flow is documented at `apps/docs/docs/explanation/security/byok.mdx`, but it documents a **design**, not a served boundary: **BYOK is NOT shipped.** `POST /v1/admin/byok/activate` fails closed with `501 Not Implemented` / `byok_not_available` in the shipped binary (`crates/corelink-container/src/routes/byok_admin.rs:245-257`); the shipped default binary reports `ActiveProvider::Unavailable`; no real provider is compiled in. See CEK-09.1. | C1.1 |
| CEK-19.1 | Key-management roles separated? | P | CSP | Internal break-glass requires PAT-DUAL-APPROVAL-001 — that half is served. The "CoreLink staff cannot access CMK material" half is vacuously true: there is no CMK. **Not shipped today: BYOK activation returns `501 byok_not_available` and the shipped default binary reports `ActiveProvider::Unavailable`; no real provider is compiled in — see CEK-09.1.** This row describes the designed customer-KMS lifecycle, which no tenant can enter. | CC6.1 |
...
| DSP-09.1 | Cross-border transfers governed? | P | CSP | SCC Modules 2/3 executed (`GDPR-SCC-EXECUTION-2026-05-15.md`), and **region pinning is a real supplementary measure** — a served, fail-closed two-layer guard. **BYOK is NOT one, and this row previously claimed it as one:** **BYOK is NOT shipped.** `POST /v1/admin/byok/activate` fails closed with `501 Not Implemented` / `byok_not_available` in the shipped binary (`crates/corelink-container/src/routes/byok_admin.rs:245-257`); the shipped default binary reports `ActiveProvider::Unavailable`; no real provider is compiled in. See CEK-09.1. A transfer-impact assessment that counted customer-held keys as a supplementary measure must be re-done without them. | P-CONSENT |
...
| LOG-03.1 | Audit log immutability enforced? | P | CSP | **Tamper-EVIDENT, not immutable.** Append-only BLAKE3 hash-chained audit log with a keyed signed head (**`INV-AUDIT-APPEND-ONLY`**); daily fail-closed chain verifier; sealed-chain chunks archived to R2 (`/_internal/audit/archive`). Tampering is *detected at verify time*; it is not *prevented*. Cloudflare R2 **does not implement Object Lock** — the API returns `NotImplemented` — so WORM retention is not available on this substrate and is not on a delivery schedule. A lawful order to delete an audit event is technically executable. | PI1.2 |
