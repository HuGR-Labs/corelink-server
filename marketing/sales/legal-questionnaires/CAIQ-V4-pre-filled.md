...
| CEK-10.1 | Kill-switch / key-revocation supported? | N | CSP | Not available — it is the revocation half of BYOK, and **BYOK is NOT shipped.** `POST /v1/admin/byok/activate` fails closed with `501 Not Implemented` / `byok_not_available` in the shipped binary (`crates/corelink-container/src/routes/byok_admin.rs:245-257`); the only key provider compiled in is `InMemoryFake`. See CEK-09.1. **On the weekly drill, stated precisely, because the workflow is real:** `.github/workflows/byok_kill_switch_drill_weekly.yml` exists, is on `cron: 0 3 * * 0`, and its last 5 runs are green. It exercises the **`InMemoryFake` provider** — every KMS credential in the job is commented out (`# AWS_ACCESS_KEY_ID: …`), so no customer KMS is contacted and no customer key is revoked. The `byok-kill-switch-rtt` ≤ 5 min figure is a measurement of the fake. | C1.1 |
