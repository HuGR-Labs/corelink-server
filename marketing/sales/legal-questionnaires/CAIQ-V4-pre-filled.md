...
| CEK-02.1 | Encryption at rest enforced? | Y | CSP | AES-256-GCM on R2 / D1 / KV / DO, provider-managed keys. The "optional BYOK envelope" this row previously offered does not exist: **BYOK is NOT shipped.** `POST /v1/admin/byok/activate` fails closed with `501 Not Implemented` / `byok_not_available` in the shipped binary (`crates/corelink-container/src/routes/byok_admin.rs:245-257`); the only key provider compiled in is `InMemoryFake`. See CEK-09.1. | C1.1 |
...
| CEK-04.1 | Cryptographic algorithms compliant with industry standards (FIPS / NIST)? | P | CSP | AES-256-GCM, ChaCha20-Poly1305, BLAKE3 / SHA-256, Ed25519. The per-provider FIPS attestation matrix (`specs/_compliance/BYOK-FIPS-ATTESTATION-MATRIX.md`, **GAP-02**) describes BYOK KMS endpoints and is therefore **not in effect**. **Not shipped today: BYOK activation returns `501 byok_not_available` and the only compiled provider is `InMemoryFake` — see CEK-09.1.** This row describes the designed customer-KMS lifecycle, which no tenant can enter. No FIPS-validated-module claim is made for the served path. | C1.1 |
| CEK-05.1 | Key generation procedures documented? | P | CSP | Documented, not served. KMS-side per provider (AWS / GCP / Azure / Vault); CoreLink never generates root keys. **Not shipped today: BYOK activation returns `501 byok_not_available` and the only compiled provider is `InMemoryFake` — see CEK-09.1.** This row describes the designed customer-KMS lifecycle, which no tenant can enter. | C1.1 |
...
| CEK-16.1 | Key-management audit logged? | P | CSP | The audit chain and `INV-AUDIT-APPEND-ONLY` are real and cover served key operations. The `EVT-KMS-*` events are BYOK events and are **never emitted in production**. **Not shipped today: BYOK activation returns `501 byok_not_available` and the only compiled provider is `InMemoryFake` — see CEK-09.1.** This row describes the designed customer-KMS lifecycle, which no tenant can enter. | PI1.2 |
| CEK-17.1 | Key destruction / decommissioning documented? | P | CSP | **Customer-controlled crypto-shredding is not available.** **Not shipped today: BYOK activation returns `501 byok_not_available` and the only compiled provider is `InMemoryFake` — see CEK-09.1.** This row describes the designed customer-KMS lifecycle, which no tenant can enter. What IS served is verifiable erasure: `DELETE /v1/tenant/me` with `INV-DATA-ERASURE-COMPLETE` + `INV-ERASURE-ATTESTATION-SIGNED` (see DSP-08.1). | C1.1 |
| CEK-18.1 | Cryptographic boundaries documented? | P | CSP | `specs/03_architecture/security_model.md` documents the served boundaries. The BYOK key flow is documented at `apps/docs/docs/explanation/security/byok.mdx` (this row previously cited `apps/docs/docs/security/byok`, a path that does not exist) — but it documents a **design**, not a served boundary: **BYOK is NOT shipped.** `POST /v1/admin/byok/activate` fails closed with `501 Not Implemented` / `byok_not_available` in the shipped binary (`crates/corelink-container/src/routes/byok_admin.rs:245-257`); the only key provider compiled in is `InMemoryFake`. See CEK-09.1. | C1.1 |
| CEK-19.1 | Key-management roles separated? | P | CSP | Internal break-glass requires PAT-DUAL-APPROVAL-001 — that half is served. The "CoreLink staff cannot access CMK material" half is vacuously true: there is no CMK. **Not shipped today: BYOK activation returns `501 byok_not_available` and the only compiled provider is `InMemoryFake` — see CEK-09.1.** This row describes the designed customer-KMS lifecycle, which no tenant can enter. | CC6.1 |
