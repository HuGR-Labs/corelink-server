...
6. **Audit chain: per-tenant BLAKE3 hash chain, RFC 8785 (JCS) canonical leaves, Ed25519-signed head, 7-year retention.** Source: `BLOG-POSTS/03-audit-chain-merkle-proofs.md`; `audit_immutability.tla`; `INV-AUDIT-APPEND-ONLY` + `INV-OBS-AUDIT-CHAIN-INTEGRITY` CRITICAL invariants; `apps/docs/docs/trust/data-handling.mdx#retention`. The sealed chain is tamper-**evident** (detect-at-verify): a relying party holding an independently-pinned prior head can detect a retroactive edit of a sealed row, and the Ed25519-signed chain head is verified fail-closed on every drain resume (an insider lacks the signing seed). Storage-level Object-Lock immutability, a hosted in-browser inclusion-proof viewer, and customer R2 proof-bundle export are on the roadmap (WI-S09-007).
...
| 7.3 | Audit-chain leaf canonicalization: RFC 8785 JCS; per-tenant linear BLAKE3 hash chain with Ed25519-signed head (Merkle inclusion/consistency proofs are roadmap, WI-S09-007) | `BLOG-POSTS/03-audit-chain-merkle-proofs.md` |
