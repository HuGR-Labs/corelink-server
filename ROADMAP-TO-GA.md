...
> [`specs/_audits/sealed/2026-05-14-slo-instrumentation-gaps.md`](specs/_audits/sealed/2026-05-14-slo-instrumentation-gaps.md)
> §6.1.
...
| R4-6 | **4-locale stub translations replaced with real translations** — pt-BR + es-419 + de native speakers (`de` added by R-prep i18n-de for DACH/EU enterprise GA; 2-3 weeks effort by translators across the three locales) | Translators (hired contractors) | 100 + 100 + 156 stubs · see `apps/docs/i18n/TRANSLATION-WORKFLOW.md` |
| R4-7 | **Status page** at `status.corelink.humangr.com` — Statuspage.io or self-hosted; subscribed to SLO breaches | 1 Sonnet + Gustavo (Statuspage account) | Customer-facing |
...
| # | Work item | Owner | Notes |
|---|---|---|---|
| R7-1 | **Aggregate D+60 evidence pack** — single archive with: pentest retest letter, SOC 2 readiness scorecard, 3 lighthouse SLA attestations, 30d staging report, all sprint impl-sealed tags, 13 canonical sign-offs signed, **static-analysis baseline + 30d drift ledger** (`specs/_audits/sealed/2026-05-15-static-analysis-baseline.md` + `specs/_runbooks/RB-STATIC-ANALYSIS-TRIAGE.md` + green runs from `.github/workflows/codeql.yml` and `.github/workflows/semgrep.yml`) | 1 Sonnet | Output: `specs/_audits/D60-GA-EVIDENCE-PACK.tar.gz` |
...
| R7-3 | **CONDITIONALLY_APPROVED → APPROVED transition commit** — `specs/04_sprints/_sealed/S20/PRR-S20-GA.md` work_status update + spec contract bump + tag `ga-approved` | Orchestrator | Final commit |
...
- **Per-endpoint baseline:** [`specs/_audits/sealed/2026-05-15-api-stability-baseline.md`](specs/_audits/sealed/2026-05-15-api-stability-baseline.md) — 35 endpoints classified (23 GA / 2 Preview / 10 Internal).
...
| H-9 | **Pentest vendor RFP + contract** — engagement scoping pkg `specs/_audits/sealed/pentest/PENTEST-EVIDENCE-PACKAGE.md` v2.0.0 finalized 2026-05-14 (R5-1). Engagement scheduled **2026-06-15** (D+0); retest letter due **2026-07-29** (D+44). | R-5 day 0 | $70-150k | R5-1, R5-2 |
...
| H-10c | **FedRAMP Moderate informational crosswalk (NOT-IN-SCOPE; not GA-blocking)** — informational-only artefact landed 2026-05-15 via `wt/r-prep-fedramp-info`: master `specs/_compliance/FEDRAMP-MODERATE-CROSSWALK-2026-05-15.md` (18 NIST 800-53 Rev 5 families × Moderate baseline × SOC 2 CC overlap × CTRL ID × status; ~85% substance coverage today via SOC 2 + ISO 27001 overlap; 13 families ≥ 80% threshold) + not-in-scope rationale `specs/_compliance/FEDRAMP-NOT-IN-SCOPE-RATIONALE.md` (market / cost $500-900k / 12-18 month timeline / re-evaluation trigger conditions documented) + customer-facing one-pager `apps/docs/docs/trust/fedramp-info.mdx` (explicit "we do NOT pursue FedRAMP today; SOC 2 + ISO 27001 covers ~85% of Moderate baseline; contact `enterprise@humangr.com` for sponsorship discussion"). **Marked informational, not blocking GA.** Refresh cadence: annual or upon Rev 5 baseline change or upon hard-trigger sponsorship event. | informational (no scheduled spend) | $0 GA-window (sponsorship would unlock $500-900k first-year incremental) | none — informational only |
...
| H-16 | **Translators** for pt-BR + es-419 + de (replace ~356 MT-stubs; de added per R-prep i18n-de for DACH/EU enterprise GA) | R-4 day 0 | $6-12k flat | R4-6 |
...
`specs/_audits/sealed/2026-05-15-secrets-coverage-baseline.md`.
