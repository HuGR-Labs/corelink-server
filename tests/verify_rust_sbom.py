...
def build_sbom() -> dict[str, object]:
    ...
    return {
        ...
        "metadata": {
            ...
            "component": {
                "bom-ref": "pkg:cargo/corelink-server-workspace@0.1.2",
                "type": "application",
                "name": "corelink-server-workspace",
                "version": "0.1.2",
                "purl": "pkg:cargo/corelink-server-workspace@0.1.2",
...
def component_identities(
    ...
    expected_checksums: dict[tuple[str, str, str], str | None],
    ...
    for component in components:
        ...
        if not isinstance(properties, list):
            raise SystemExit(f"component properties are not a list: {component.get('name')}")
        source_values = [
            property_.get("value")
            for property_ in properties
            if isinstance(property_, dict) and property_.get("name") == SOURCE_PROPERTY
        ]
        source = source_values[0] if len(source_values) == 1 else None
        if not isinstance(source, str):
            raise SystemExit(f"component has no {SOURCE_PROPERTY}: {component.get('name')}")
        component_identity = (str(component.get("name")), str(component.get("version")), source)
        if component_identity not in expected_checksums:
            raise SystemExit(f"component is not present in Cargo.lock: {component_identity}")
        checksum_values = [
            property_.get("value")
            for property_ in properties
            if isinstance(property_, dict) and property_.get("name") == CHECKSUM_PROPERTY
        ]
        expected_checksum = expected_checksums[component_identity]
        if expected_checksum is None:
            if checksum_values:
                raise SystemExit(f"path component unexpectedly has a Cargo checksum: {component_identity}")
        elif checksum_values != [expected_checksum]:
            raise SystemExit(f"component checksum does not match Cargo.lock: {component_identity}")
...
def verify(sbom: dict[str, object]) -> None:
    packages = lock_packages()
    expected = {identity(package) for package in packages}
    expected_checksums = {
        identity(package): str(package["checksum"]) if package.get("checksum") is not None else None
        for package in packages
    }
    actual = component_identities(sbom, expected_checksums)
    if expected != actual:
