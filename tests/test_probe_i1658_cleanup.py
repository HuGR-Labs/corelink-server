...
class ProbeCleanupTests(unittest.TestCase):
    def test_successful_put_is_cleaned_after_timing_parse(self) -> None:
        ...
        self.assertEqual(mocked.call_args_list[1].args[4], mocked.call_args_list[0].args[4])
    ...
    def test_timeout_with_ambiguous_write_still_cleans_exact_key(self) -> None:
        put = response("PUT", None, transport_error="TimeoutError")
        delete = response("DELETE", 204)
        with patch.object(probe, "request", side_effect=[put, delete]) as mocked:
            with self.assertRaisesRegex(probe.ProbeError, "HTTP None; cleanup status 204") as caught:
                probe.observed_put(BASE, TENANT, TOKEN, "serial-timeout")

        self.assertEqual([entry.args[3] for entry in mocked.call_args_list], ["PUT", "DELETE"])
        self.assertEqual(mocked.call_args_list[1].args[4], mocked.call_args_list[0].args[4])
        self.assertEqual(caught.exception.observation["transport_error"], "TimeoutError")
