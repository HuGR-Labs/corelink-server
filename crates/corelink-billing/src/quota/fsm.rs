...
#[allow(
    ...
    reason = "absorbed `corelink-quota-fsm` keeps its canonical `fsm` module name; \
              the outer `quota::fsm` aggregator stays as the consumer-facing import \
              surface so the W35 P2 absorption preserves the public API path \
