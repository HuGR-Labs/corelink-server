...
//!      [`super::retry_after::provisional_retry_after_secs`].
...
mod tests {
    ...
    use super::super::reservation::InMemoryReservationTracker;
    ...
    fn read_path_returns_allow() {
        ...
        assert_eq!(
            metrics.counter_total(super::super::metrics::QuotaMetricKind::CheckTotal),
            1
        );
    }
    ...
    fn write_at_100pct_returns_deny_429() {
        ...
        assert_eq!(
            metrics.counter_total(super::super::metrics::QuotaMetricKind::DenialsTotal),
            1
        );
    }
    ...
    fn audit_failure_aborts_decision() {
        use super::super::audit::FailingQuotaAuditSink;
        let state = Arc::new(InMemoryTenantStorageStateStore::new());
        state
            .push_row(fresh_state_row(ten_a(), EvictionRegion::Sam, 50, 100))
            .unwrap();
