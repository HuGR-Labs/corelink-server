...
proptest! {
    ...
    fn prop_score_in_range_0_1(
        ...
        let s = corelink_billing::abuse::compute_score(&f, &w, Tier::Team);
...
proptest! {
    ...
    fn prop_score_monotonic_in_cpu(
        ...
        let s_a = corelink_billing::abuse::compute_score(&f_a, &w, Tier::Team);
        let s_b = corelink_billing::abuse::compute_score(&f_b, &w, Tier::Team);
        prop_assert!(
...
proptest! {
    ...
    fn prop_decision_threshold_boundary(
        ...
        let d = corelink_billing::abuse::decide(s, &cfg);
    ...
    fn prop_suspend_idempotent(
        ...
        for _ in 0..n_calls {
            ...
            let is_forbidden = matches!(
                err,
                corelink_billing::abuse::AbuseError::AutoSuspendForbidden { .. }
            );
...
proptest! {
    ...
    fn prop_overshoot_does_not_panic(
        ...
        let _ = corelink_billing::abuse::compute_score(&f, &w, Tier::Free);
        let _ = corelink_billing::abuse::compute_score(&f, &w, Tier::Enterprise);
    }
}
