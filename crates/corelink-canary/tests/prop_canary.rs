...
fn surface_pinning_canonical_canary_regions_count() {
    let regions = corelink_canary::canonical_canary_regions();
    assert_eq!(regions.len(), 3);
}

#[test]
fn surface_pinning_canonical_assertions_count() {
    let assertions = corelink_canary::canonical_canary_assertions();
    assert_eq!(assertions.len(), 4);
}

#[test]
fn surface_pinning_canonical_audit_event_strings_count() {
    let events = corelink_canary::canonical_canary_audit_event_strings();
...
fn surface_pinning_canary_schema_version() {
    assert_eq!(corelink_canary::canary_schema_version(), 1);
}

#[test]
fn surface_pinning_72h_loop_target_lote_10_9bis_p0_a() {
    assert_eq!(corelink_canary::SUSTAINED_72H_LOOP_TARGET, 12_960);
}

#[test]
fn surface_pinning_dispatch_lag_threshold() {
    assert_eq!(corelink_canary::DISPATCH_LAG_SEV3_MS, 90_000);
}

#[test]
fn surface_pinning_cron_interval() {
    assert_eq!(corelink_canary::CRON_INTERVAL_SECS, 60);
}
...
fn full_loop_smoke_pass_and_health_check_canonical() {
    ...
    for region in corelink_canary::canonical_canary_regions() {
        let r = p
            .execute_canary_loop(
                *region,
...
fn audit_failure_no_ledger_advance_fail_closed_pin() {
    ...
    use corelink_canary::FailingCanaryAuditSink;
