...
use corelink_ops::config::api::handlers::{
    handle_get_current, handle_put, handle_rollback, AdminContext, PutConfigRequest,
};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(
        drill_elapsed_ms < 5 * 60 * 1000,
        "SLO: rollback ≤ 5 min p99"
    );
