...
//!    [`crate::event::LayerDriftSummary`] → compute the canonical
...
//!    persisted via the
//!    [`crate::idempotency::ReplayIdempotencyLedger::record_outcome`]
...
//!    [`crate::error::ReplayIdempotencyError::DivergentPayload`] on
//!    tampering signal).
//!
...
//! [`crate::error::ReplayError::Audit`] (caller sees no state
...
use crate::audit::{ReplayAuditEventType, ReplayAuditRecord, ReplayAuditSink};
...
use crate::idempotency::{RecordOutcome, ReplayIdempotencyLedger};
...
/// [`crate::audit::FailingReplayAuditSink`] +
/// [`crate::idempotency::FailingReplayIdempotencyLedger`] +
/// [`crate::archive::FailingReplayArchive`]) compose directly at
/// construction.
#[derive(Clone)]
...
/// Compute the canonical [`crate::event::LayerDriftSummary`] from the
...
mod tests {
    use super::*;
    use crate::archive::{FailingReplayArchive, InMemoryReplayArchive};
    use crate::audit::{FailingReplayAuditSink, InMemoryReplayAuditSink};
    use crate::idempotency::{FailingReplayIdempotencyLedger, InMemoryReplayIdempotencyLedger};
    use uuid::Uuid;
    ...
    fn req_for(rid: Uuid, tenant: Uuid, period: &str, reason: ReplayReason) -> ReplayRequest {
        ReplayRequest::new(
            rid,
            Uuid::now_v7(),
            crate::event::BILLING_FORENSICS_ADMIN_ROLE,
            tenant,
            period,
            reason,
        )
