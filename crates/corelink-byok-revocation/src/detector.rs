...
use crate::event::{RevocationAuditEvent, EVENT_TYPE_CMK_RESTORED, EVENT_TYPE_CMK_REVOKED};
        ...
        for provider in &self.providers {
            ...
            for key_id in active_keys {
                ...
                match result {
                    ...
                    Ok(_) => {
                        // Future KmsAccessStatus variants: treat as transient.
                        warn!(
                            provider = provider.provider_kind().as_str(),
                            kms_key_id = key_id.as_str(),
                            "unknown KmsAccessStatus variant; treating as transient"
                        );
                    }
                }
            }
