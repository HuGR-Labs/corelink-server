...
const KNOWN_HAZARDS: &[(&str, &str)] = &[
    (
        ...
        "references billing_events_staging — table not declared by any migration in this directory; \
         tracked as a real D1 dependency on a table created by sister Postgres / outbox flow",
    ),
    (
        "0028_tenant_primary_region.sql",
        "ALTER TABLE tenant ADD COLUMN IF NOT EXISTS primary_region — rewrite path runs but the \
         column was already declared in 0023 so SQLite reports duplicate-column (idempotent skip).",
    ),
    (
        "0031_byok_tenant_status.sql",
        "references `tenants` (plural) — only `tenant` (singular) is declared; \
         this is a known table-naming inconsistency from S-14 work; flagged for follow-up.",
    ),
    (
        "0041_dpa_versioning.sql",
        "ALTER TABLE tenants — same plural-naming issue as 0031; flagged for follow-up.",
    ),
];
