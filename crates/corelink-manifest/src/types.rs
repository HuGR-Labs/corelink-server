...
use crate::bounds::{MANIFEST_PREIMAGE_LEN, MANIFEST_SIG_LEN};
...
/// Sealed manifest envelope. Returned by [`crate::ManifestBuilder::build`]
/// and consumed by [`crate::ManifestVerifier::verify_full`] /
/// [`crate::ManifestVerifier::verify_structure`]. The streaming verifier
/// consumes the envelope MINUS its `chunks` field — the streaming surface
/// reads chunks one at a time from a [`crate::ChunkRefSource`] (the D1
...
pub struct Manifest {
    ...
    /// — pinned via [`crate::error::ManifestError::ChunkCountMismatch`]
    /// when divergent.
    pub chunk_count: u32,
...
/// Canonical chunk-stream entry consumed by [`crate::ManifestBuilder::build`].
