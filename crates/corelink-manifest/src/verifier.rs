...
use crate::bounds::{MAX_CHUNK_SIZE_BYTES, MAX_TOTAL_SIZE_BYTES};
use crate::error::{ManifestError, VerifyError};
use crate::merkle::{hash_inner, hash_leaf};
use crate::sig::ManifestVerifierSig;
use crate::types::{ChunkRef, Manifest, CURRENT_MANIFEST_VERSION, DIGEST_LEN};
...
impl ManifestVerifier {
    ...
    fn verify_structure_internal(&self, manifest: &Manifest) -> Result<(), VerifyError> {
        ...
        let recomputed = crate::merkle::build_root(&manifest.chunks)
...
pub fn verify_streaming(
    ...
    if header.chunk_count > crate::bounds::MAX_CHUNKS_PER_BLOB {
...
mod tests {
    ...
    use crate::sig::{ManifestSigner, ManifestVerifierSig};
    use crate::types::{ChunkInput, ChunkerAlgorithm};
    ...
    fn streaming_verify_root_matches_corelink_ac_pattern_for_3_leaves() {
        ...
        let payloads: &[&[u8]] = &[b"AAA", b"BBB", b"CCC"];
        let m = build_test_manifest(payloads);
        let direct_root = crate::merkle::build_root(&m.chunks).unwrap();
        let header = StreamingManifestHeader::from_manifest(&m);
        let mut refs = InMemoryChunkRefSource::new(m.chunks.clone());
        let mut bytes = InMemoryChunkBytesSource::new();
        for (i, p) in payloads.iter().enumerate() {
