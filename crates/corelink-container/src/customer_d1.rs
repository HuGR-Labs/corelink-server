...
/// Accept only roles that can be inserted by a self-serve invite. Unlike the
/// old fallback, an unknown role is never silently persisted as `member`: the
/// caller must see the role that actually takes effect.
...
/// The database CHECK makes these the only persisted values. Decode them
/// explicitly before putting them on the response wire, so a malformed row is
/// never displayed as an invented role.
...
impl CustomerTeamHandler for D1CustomerHandler {
    ...
    fn invite(&self, req: TeamInviteRequest) -> Result<TeamInviteResponse, CustomerHandlerError> {
        ...
        // The role is validated against the frozen 0074 CHECK domain. Invalid
        // values are rejected instead of being silently rewritten to `member`.
        ...
        // The response reflects the canonical role that was actually persisted,
        // never the caller's requested spelling. The raw email is reflected only
        // to the caller that supplied it — it is NOT persisted (only `email_hash`
        // is).
...
mod tests {
    ...
    fn team_invite_inserts_invited_member_row() {
        ...
        // Response is the effective role persisted in the CHECK domain, not the
        // caller's capitalization.
        ...
        // The accepted value is persisted unchanged in canonical lowercase.
    ...
    #[test]
    fn normalize_invite_role_has_an_exact_assignable_matrix() {
        for (requested, persisted) in [
            ("ADMIN", "admin"),
            ("  member ", "member"),
            ("Viewer", "viewer"),
        ] {
            assert_eq!(normalize_invite_role(requested).unwrap(), persisted);
        }
        for rejected in ["Owner", "Developer", "", "anything-else"] {
            assert!(
                matches!(
                    normalize_invite_role(rejected),
                    Err(CustomerHandlerError::InvalidRequest(_))
                ),
                "{rejected:?} must not be silently remapped"
            );
        }
    }
