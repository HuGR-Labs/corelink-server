...
mod tests {
    ...
    /// Fixture variant that exposes the customer audit sink so a rejected
    /// limiter pipeline can prove it never reached the mint handler.
    ...
    async fn public_pat_issue_rate_limit_is_tenant_scoped_and_audited() {
        ...
        assert_eq!(
            records
                .iter()
                .filter(
                    |record| record.event_type == corelink_ratelimit::RateLimitEventType::Allowed
                )
                .count(),
            11
        );
        assert_eq!(
            records
                .iter()
                .filter(
                    |record| record.event_type == corelink_ratelimit::RateLimitEventType::Denied429
                )
                .count(),
            1
        );
    ...
    fn pat_issue_refill_is_exact_at_360_second_boundary() {
        ...
        let key =
            corelink_ratelimit::BucketKey::per_tenant_per_endpoint(tenant, PAT_ISSUE_ENDPOINT_ID);
