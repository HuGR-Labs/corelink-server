    ...
    fn rig_full(
        ...
        let resolver = UpstreamManifestResolver::with_parts(UpstreamManifestResolverParts {
            manifest_fetcher: Arc::clone(&manifest_fetcher) as Arc<dyn UpstreamManifestFetcher>,
            blob_fetcher: Arc::clone(&blob_fetcher) as Arc<dyn UpstreamBlobFetcher>,
            kv: Arc::clone(&kv) as Arc<dyn ManifestKvStore>,
            moat: moat(Arc::clone(&cas), Arc::clone(&map)),
            cap_resolver: None, // RecordingCas ignores the absent cap resolver.
            limiter,
            allowlist,
            dedup,
        });
