...
use super::tests_support::{
    put, router_with, router_with_put_probe, router_with_quota_observable, test_key, EmptyLookup,
    FailingLookup, FakeKv, FakeMap, OneTokenLookup, StubCas, SCOPE_RW,
};
...
#[tokio::test]
async fn put_with_resolver_backend_is_503_retryable_and_fail_closed() {
    // Unlike an unknown token (which is an InvalidPat result), this is a
    // syntactically authentic PAT whose D1 row lookup itself fails. The F27
    ...
    assert_eq!(
        ...
        "a resolver backend error must fail closed as retryable 503, never admit or claim invalid PAT"
    );
    assert_eq!(
        resp.headers()
        ...
        Some("1"),
        "a resolver backend error must advertise the bounded retry interval"
    );
    assert_eq!(
        lookup.calls(),
        1,
        "the F27 gate must invoke the resolver exactly once per write"
    );
}

#[tokio::test]
async fn put_with_write_capable_pat_reaches_downstream_probe_once() {
    // Both layers agree, so the gate must call `next.run`. The ordinary pip
    ...
    assert_eq!(
        ...
        "a write-capable PAT must reach the test-only downstream PUT probe"
    );
    assert_eq!(
