...
impl D1ErasureAuditSink {
    fn emit_attributed(
        ...
        // `region` MUST equal the tenant's `primary_region`. Do NOT rely on the
        // `'wnam'` column default: migration 0107 rejects a missing or mismatched
        // tenant, so a DSR audit write fails CLOSED instead of creating an
        // unevaluable row. DSR audit is emitted before the destructive arm, while
        // the tenant still exists; deterministic INSERT OR IGNORE keeps retries
        // of an already-retained row idempotent after erasure.
