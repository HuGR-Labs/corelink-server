...
// The tests live in the sibling `pip/` directory, one file per PROPERTY the
// pip surface has to keep — so a failure names the property it broke:
//
//   `tests_gate_scope` — admission: the server-trusted scope header decides,
//                        and an unmapped method is denied, not routed.
//   `tests_gate_auth`  — every credential the verifier cannot vouch for ends
//                        as a flat 401, and does so via the RESOLVER (which
