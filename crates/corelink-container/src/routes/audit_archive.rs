...
fn parse_existing_chunk(bytes: &[u8]) -> Result<Vec<SealedArchiveLine>, String> {
    ...
    // Accept the one conventional NDJSON terminator emitted by historical
    // archive writers. A second terminator still produces an empty line and
    // remains malformed evidence.
