...
async fn main() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    let (mut app, reapi_deps) =
        routes::build_with_factory_and_byok_with_reapi(shadow_factory, byok_data_plane);
    if let (Some(cas), Some(byte_stream), Some(action_cache), Some(capabilities)) = (
