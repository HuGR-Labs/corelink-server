...
impl Ingress {
    ...
    async fn authenticate<T>(&self, request: &Request<T>, write: bool) -> Result<String, Status> {
        let raw = request.metadata().get("authorization")
            .and_then(|value| value.to_str().ok())
            .ok_or_else(|| Status::unauthenticated("missing authorization"))?;
        let (tenant, can_write) = self.pat
            .verify_capability(raw.strip_prefix("Bearer ").unwrap_or(raw).trim())
            .await
...
fn validate_digest(digest: Option<Digest>) -> Result<(String, i64), Status> {
    ...
    let too_large = usize::try_from(digest.size_bytes)
...
fn validate_inline_digest(bytes: &[u8], digest: Option<Digest>) -> Result<(), Status> {
    ...
    {
        return Err(Status::invalid_argument("inline result data does not match its digest"));
    }
    Ok(())
}
...
fn map_ac_error(error: AcHandlerError) -> Status {
    match error {
        ...
        AcHandlerError::DivergentBody { .. } => Status::already_exists("action result already exists"),
...
impl ContentAddressableStorage for CasService {
    async fn batch_update_blobs(
        ...
        let total = body.requests.iter().try_fold(0_usize, |sum, item| {
                        ...
                        && size == i64::try_from(item.data.len()).unwrap_or(i64::MAX) =>
                {
                    self.0.charge(&tenant).await.and_then(|_| {
                        tokio::task::block_in_place(|| {
                            self.0.cas_write.write(write_request(&tenant, hash, item.data))
                        })
                        .map(|_| ())
                        .map_err(map_cas_error)
                    })
                }
                Ok(_) if item.compressor != 0 => {
                    Err(Status::invalid_argument("compressed batch uploads unsupported"))
                }
                Ok(_) => Err(Status::invalid_argument("digest size does not match blob bytes")),
                Err(error) => Err(error),
            };
    ...
    async fn batch_read_blobs(
        ...
        for digest in body.digests {
            ...
            match result {
                Ok(blob) if blob.bytes.len() == usize::try_from(digest.size_bytes).unwrap_or(usize::MAX) => {
                    responses.push(batch_read_blobs_response::Response {
                        digest: Some(digest), data: blob.bytes, compressor: 0,
                        status: Some(wire_status(Status::new(Code::Ok, ""))),
                    });
                }
                Ok(_) => responses.push(batch_read_blobs_response::Response {
                    digest: Some(digest), data: vec![], compressor: 0,
                ...
                Err(error) => responses.push(batch_read_blobs_response::Response {
                    digest: Some(digest), data: vec![], compressor: 0, status: Some(wire_status(error)),
                }),
            }
        }
    ...
    async fn find_missing_blobs(
        ...
        for digest in body.blob_digests {
            ...
            let exists = tokio::task::block_in_place(|| {
                self.0.cas_read.exists(read_request(&tenant, hash))
            })
            .map_err(map_cas_error)?;
            if !exists {
                missing.push(digest);
...
impl ActionCache for ActionCacheService {
    async fn get_action_result(
        ...
        ActionResult::decode(result.result_payload.as_slice())
            .map(Response::new)
    ...
    async fn update_action_result(
        ...
        Ingress::require_instance(&body.instance_name, &tenant)?;
        let (hash, _) = validate_digest(body.action_digest)?;
        let result = body.action_result.ok_or_else(|| Status::invalid_argument("action_result is required"))?;
        validate_action_result(&result)?;
        self.0.charge(&tenant).await?;
        let update = AcUpdateRequest::new(&tenant, hash, result.encode_to_vec(), "grpc-pat", &tenant, now_ms());
...
fn parse_resource(resource: &str, write: bool) -> Result<(String, String, i64), Status> {
    ...
    let (instance, hash, size) = if write {
        if parts.len() != 6 || parts[1] != "uploads" || parts[2].is_empty() || parts[3] != "blobs" {
            return Err(Status::invalid_argument("invalid ByteStream write resource"));
        }
    ...
    let size = size.parse::<i64>().map_err(|_| Status::invalid_argument("invalid ByteStream digest size"))?;
    let (hash, checked_size) = validate_digest(Some(Digest { hash: hash.to_owned(), size_bytes: size }))?;
...
impl ByteStream for ByteStreamService {
    ...
    async fn read(&self, request: Request<ReadRequest>) -> Result<Response<Self::ReadStream>, Status> {
        ...
        let blob = tokio::task::block_in_place(|| self.0.cas_read.read(read_request(&tenant, hash)))
            .map_err(map_cas_error)?;
        if i64::try_from(blob.bytes.len()).unwrap_or(i64::MAX) != declared_size {
            return Err(Status::internal("stored blob size mismatch"));
        }
        let start = usize::try_from(body.read_offset).map_err(|_| Status::out_of_range("read offset outside blob"))?;
        ...
        let end = if body.read_limit == 0 {
            ...
            start.saturating_add(usize::try_from(body.read_limit).unwrap_or(usize::MAX)).min(blob.bytes.len())
        };
        let responses: Vec<_> = blob.bytes[start..end].chunks(READ_CHUNK_BYTES)
            .map(|data| Ok(ReadResponse { data: data.to_vec() })).collect();
    ...
    async fn write(
        ...
        while let Some(chunk) = stream.message().await? {
            if finished { return Err(Status::invalid_argument("chunk after finish_write")); }
            if resource.is_none() {
                if chunk.resource_name.is_empty() { return Err(Status::invalid_argument("first write chunk requires resource_name")); }
                resource = Some(chunk.resource_name.clone());
            } else if !chunk.resource_name.is_empty() && resource.as_deref() != Some(chunk.resource_name.as_str()) {
                return Err(Status::invalid_argument("write resource changed mid-stream"));
            }
            if chunk.write_offset != expected_offset { return Err(Status::invalid_argument("write offset is not contiguous")); }
            if data.len().checked_add(chunk.data.len()).map_or(true, |size| size > MAX_BLOB_BYTES) {
                return Err(Status::resource_exhausted("blob exceeds CAS byte limit"));
            }
            expected_offset = expected_offset.saturating_add(i64::try_from(chunk.data.len()).unwrap_or(i64::MAX));
            data.extend_from_slice(&chunk.data);
            finished = chunk.finish_write;
        }
        if !finished { return Err(Status::invalid_argument("write closed before finish_write")); }
        ...
        if size != i64::try_from(data.len()).unwrap_or(i64::MAX) {
            return Err(Status::invalid_argument("declared size does not match streamed bytes"));
        }
        self.0.charge(&tenant).await?;
        tokio::task::block_in_place(|| self.0.cas_write.write(write_request(&tenant, hash, data)))
            .map_err(map_cas_error)?;
        Ok(Response::new(WriteResponse { committed_size: size }))
    }
    ...
    async fn query_write_status(
        ...
        Err(Status::unimplemented("resumable ByteStream writes are unsupported"))
    }
}
...
impl Capabilities for CapabilitiesService {
    async fn get_capabilities(
        ...
        Ok(Response::new(ServerCapabilities {
            ...
            low_api_version: Some(corelink_reapi::proto::semver::SemVer { major: 2, minor: 12, patch: 0, prerelease: String::new() }),
            high_api_version: Some(corelink_reapi::proto::semver::SemVer { major: 2, minor: 12, patch: 0, prerelease: String::new() }),
        }))
    }
}
