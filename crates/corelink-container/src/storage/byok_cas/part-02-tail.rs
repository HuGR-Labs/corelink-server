...
#[tokio::test]
async fn mode_b_allocations_have_independent_envelopes_with_raw_digest_aad() {
    let store = Arc::new(MemEnvelopeStore::default());
    let me = mode_b_enc(Arc::clone(&store), false);
    let ctx = mode_b_ctx();
    let pt = b"same logical object";

    let first = me
        .encrypt_for_allocation(pt, &ctx, "allocation-1")
        .await
        .unwrap();
    let repeated = me
        .encrypt_for_allocation(pt, &ctx, "allocation-1")
        ...
        .unwrap();
    let next = me
        .encrypt_for_allocation(pt, &ctx, "allocation-2")
        .await
        .unwrap();

    assert_eq!(first, repeated, "one allocation remains idempotent");
    assert_ne!(first, next, "a new allocation mints an independent DEK");
    assert_eq!(
        me.decrypt_for_allocation(&first, &ctx, "allocation-1")
            .await
            .unwrap(),
        pt
    );
    assert!(
        me.decrypt_for_allocation(&first, &ctx, "allocation-2")
            .await
            .is_err(),
        "ciphertext cannot be opened through another allocation"
    );

    let rows = lock(&store.inner);
    ...
    for row in rows.values() {
        assert_eq!(
            ...
            Some(ctx.plaintext_digest.as_str()),
            "allocation identity must not replace the raw digest in KMS context"
        );
    }
}

#[tokio::test]
async fn mode_b_allocation_reclaim_is_exact() {
    let store = Arc::new(MemEnvelopeStore::default());
    let me = mode_b_enc(Arc::clone(&store), false);
    let ctx = mode_b_ctx();
    let first = me
        .encrypt_for_allocation(b"payload", &ctx, "allocation-1")
        .await
        .unwrap();
    let second = me
        .encrypt_for_allocation(b"payload", &ctx, "allocation-2")
        .await
        .unwrap();

    me.reclaim_for_allocation(&ctx, "allocation-1")
        .await
        .unwrap();
    assert!(me
        .decrypt_for_allocation(&first, &ctx, "allocation-1")
        .await
        .is_err());
    assert_eq!(
        me.decrypt_for_allocation(&second, &ctx, "allocation-2")
            .await
            .unwrap(),
        b"payload"
    );
...
#[test]
fn allocation_envelope_key_is_surface_digest_and_allocation_qualified() {
    assert_eq!(
        allocation_envelope_blob_key("cas", "abc", "generation-7-write-1").unwrap(),
        "cas:abc:allocation:generation-7-write-1"
    );
    assert_ne!(
        allocation_envelope_blob_key("cas", "abc", "one").unwrap(),
        allocation_envelope_blob_key("cas", "abc", "two").unwrap()
    );
    assert!(allocation_envelope_blob_key("cas", "abc", "").is_err());
}
