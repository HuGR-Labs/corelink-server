...
// `corelink-ops` dep is platform-gated to native in `Cargo.toml`; the
// wasm32 hot path in `src/scheduler.rs` falls back to direct
// `corelink_statuspage_real::*` imports under cfg.
