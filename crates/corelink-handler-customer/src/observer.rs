...
//!
//! Each handler entry emits exactly one [`SliObservation`] using
//! [`Sli::AvailControlPlane`] (customer dashboard is control-plane)
//! before returning. The fake captures every observation so tests can
//! pin `INV-HANDLER-SLI-EMIT-ENTRY`.
...
/// CAS / AC / Admin / Customer handlers so a future
/// `corelink-slo::Aggregator` can read it uniformly.
