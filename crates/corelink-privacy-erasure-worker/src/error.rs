//! Canonical error taxonomy for the
//! `corelink-privacy-erasure-worker` crate.
//!
...
/// Audit sink failure surface (lifted into [`ErasureWorkerError::Audit`]).
///
/// The 5 canonical CloudEvents (`dev.hugr.corelink.dsr.erasure.{started,
/// backend_completed, verification_passed, verification_failed,
/// completed}.v1`) emit BEFORE state mutation per the canonical S-07
/// P1-1 fix + Lote 10.6bis pattern: audit failure aborts the pipeline;
/// the durable backends remain at their pre-call state.
#[derive(Debug, Error)]
#[non_exhaustive]
pub enum ErasureAuditSinkError {
    ...
    /// maps this to [`ErasureWorkerError::Audit`] and returns to the caller;
...
pub enum ErasureBackendError {
    ...
    /// [`crate::event::BackendErasureOutcome::NotApplicable`] in the report;
    /// no SEV alert.
    ...
    /// [`crate::event::BackendErasureOutcome::PartialFailure`] in the report
...
/// [`ErasureWorkerError::Idempotency`]).
///
...
mod tests {
    ...
    fn from_audit_sink_error_lifts_cleanly() {
        ...
        let e: ErasureWorkerError = inner.into();
        let lifted = matches!(e, ErasureWorkerError::Audit(_));
        assert!(lifted);
    }
    ...
    fn from_backend_transport_lifts_cleanly() {
        ...
        let e: ErasureWorkerError = inner.into();
        let lifted = matches!(e, ErasureWorkerError::Backend(_));
        assert!(lifted);
    }
    ...
    fn from_idempotency_backend_lifts_cleanly() {
        ...
        let e: ErasureWorkerError = inner.into();
        let lifted = matches!(e, ErasureWorkerError::Idempotency(_));
        assert!(lifted);
    }
    ...
    fn from_report_canonicalization_lifts_cleanly() {
        ...
        let e: ErasureWorkerError = inner.into();
        let lifted = matches!(e, ErasureWorkerError::Report(_));
        assert!(lifted);
    }
    ...
    fn config_displays_diagnostic() {
        let e = ErasureWorkerError::Config("tenant mismatch".to_string());
        let s = format!("{e}");
    ...
    fn internal_displays_diagnostic() {
        let e = ErasureWorkerError::Internal("mutex poisoned".to_string());
        let s = format!("{e}");
    ...
    fn audit_displays_lifted_inner() {
        let e: ErasureWorkerError = ErasureAuditSinkError::Store("inner-msg".to_string()).into();
        let s = format!("{e}");
