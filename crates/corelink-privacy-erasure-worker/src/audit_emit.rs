...
//! GDPR 4% global).
//!
//! WI-S11-002 §1 freezes the canonical 5-event CloudEvents taxonomy
//! per privacy_model.md §6.2 + Lote 10.9bis P0-G prefix discipline:
//!
//! - `dev.hugr.corelink.dsr.erasure.started.v1` — orchestrator emitted
//!   a canonical 12-arm plan + dispatched the per-backend fan-out;
//!   audit fires BEFORE the first backend mutation.
//! - `dev.hugr.corelink.dsr.erasure.backend_completed.v1` — a
//!   per-backend erasure step completed (Erased / Pseudonymized /
//!   PartialFailure / Failed / NotApplicable); audit fires BEFORE the
//!   canonical D1 `dsr_erasure_log` tombstone insert.
//! - `dev.hugr.corelink.dsr.erasure.verification_passed.v1` —
//!   verification sweep confirmed every backend reports a successful
//!   outcome; audit fires BEFORE the canonical
//!   `dsr.completed.v1` event + R2 evidence-dsr signed URL binding.
//! - `dev.hugr.corelink.dsr.erasure.verification_failed.v1` —
//!   verification sweep detected at least one non-successful backend
//!   outcome OR the SLA window (24h) elapsed with unverified backends
//!   OR a cross-tenant rejection fired; audit fires BEFORE the SEV-1
//!   alert + RB-DSR-ERASURE-INCOMPLETE runbook activation.
//! - `dev.hugr.corelink.dsr.erasure.completed.v1` — pipeline terminal:
...
pub struct ErasureAuditRecord {
    /// Canonical CloudEvents type (5-arm taxonomy).
    pub event_type: ErasureCloudEventType,
    ...
    /// `BackendCompleted` arm; `None` on the other arms).
    pub outcome: Option<BackendErasureOutcome>,
...
impl InMemoryErasureAuditSink {
    ...
    #[must_use]
    pub fn snapshot_of(&self, event_type: ErasureCloudEventType) -> Vec<ErasureAuditRecord> {
        self.snapshot()
            .into_iter()
    ...
    /// Filter snapshot down to records for a single DSR id.
    #[must_use]
    pub fn snapshot_for_dsr(&self, dsr_id: Uuid) -> Vec<ErasureAuditRecord> {
        self.snapshot()
            .into_iter()
            .filter(|r| r.dsr_id == dsr_id)
            .collect()
    }
}
...
mod tests {
    ...
    fn rec(t: ErasureCloudEventType) -> ErasureAuditRecord {
        ErasureAuditRecord {
            event_type: t,
            dsr_id: fixed_uuid(1),
            tenant_id: fixed_uuid(2),
            subject_id: fixed_uuid(3),
            backend: None,
            outcome: None,
            now_ms: 1,
    ...
    fn each_event_type_has_unique_canonical_string() {
        ...
        for t in v {
            assert!(t.as_str().starts_with("dev.hugr.corelink.dsr.erasure."));
            assert!(t.as_str().ends_with(".v1"));
    ...
    fn sev_classification_pinned() {
        let p = ErasureCloudEventType::VerificationFailed.is_sev1();
        assert!(p);
        let q = ErasureCloudEventType::VerificationPassed.is_sev1();
        assert!(!q);
    }
    ...
    fn failing_sink_returns_store_error() {
        ...
        let err = sink.emit(rec(ErasureCloudEventType::Started)).unwrap_err();
        let is_store = matches!(err, ErasureAuditSinkError::Store(_));
        assert!(is_store);
    }
    ...
    fn cloned_sink_shares_buffer() {
        ...
        let s2 = s1.clone();
        s1.emit(rec(ErasureCloudEventType::Started)).unwrap();
        assert_eq!(s2.len(), 1);
    }

    #[test]
    fn snapshot_for_dsr_filters() {
        let sink = InMemoryErasureAuditSink::new();
        let d1 = fixed_uuid(10);
        let d2 = fixed_uuid(20);
        let mut r1 = rec(ErasureCloudEventType::Started);
        r1.dsr_id = d1;
        let mut r2 = rec(ErasureCloudEventType::Started);
        r2.dsr_id = d2;
        sink.emit(r1).unwrap();
        sink.emit(r2).unwrap();
        assert_eq!(sink.snapshot_for_dsr(d1).len(), 1);
        assert_eq!(sink.snapshot_for_dsr(d2).len(), 1);
    }
}
