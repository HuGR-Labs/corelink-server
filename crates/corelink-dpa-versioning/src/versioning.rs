...
//! Pipeline (Major bump):
//!
//! 1. Classify `(old, new)` via [`crate::version::classify_bump`].
//! 2. Persist the new version record via [`crate::store::DpaStore::append_version`].
//! 3. Flag every existing tenant for re-acceptance + seed
//!    `grace_expires_at = now + 30d`.
//! 4. Dispatch a [`crate::broadcast::BroadcastKind::MajorBumpNotice`]
...
use crate::broadcast::{BroadcastEnvelope, BroadcastKind, BroadcastSink};
use crate::error::DpaVersioningError;
use crate::schema::{BumpReceipt, DpaVersionRecord, UnixSeconds, GRACE_PERIOD_SECONDS};
use crate::store::{DpaStore as _, InMemoryDpaStore};
use crate::version::{classify_bump, BumpClassification, BumpKind, SemverVersion};
