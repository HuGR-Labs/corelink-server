...
/// trait so test fakes (e.g. [`crate::audit::FailingQuotaAuditSink`]
/// plus [`crate::store::FailingQuotaFsmStore`]) compose directly at
/// construction.
...
mod tests {
    use super::*;
    use crate::audit::{FailingQuotaAuditSink, InMemoryQuotaAuditSink};
    use crate::store::{FailingQuotaFsmStore, InMemoryQuotaFsmStore};
    ...
    fn store_failure_on_upsert_lands_audit_then_propagates() {
        ...
        impl QuotaFsmStore for LookupOkUpsertFailStore {
            fn lookup(
                &self,
                tenant_id: Uuid,
            ) -> Result<Option<QuotaFsmStateRow>, crate::error::QuotaFsmStoreError> {
                Ok(Some(QuotaFsmStateRow::genesis(tenant_id, 0)))
            }
            fn upsert(
                &self,
                _row: &QuotaFsmStateRow,
            ) -> Result<(), crate::error::QuotaFsmStoreError> {
                Err(crate::error::QuotaFsmStoreError::Backend(
