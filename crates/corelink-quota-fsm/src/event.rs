...
impl UtilizationPct {
    ...
    /// # Errors
    ///
    /// Returns [`crate::error::QuotaFsmError::Config`] when the value is
    /// `NaN`, infinite, negative, or above 200.
    pub fn new(value: f64) -> Result<Self, crate::error::QuotaFsmError> {
        if !value.is_finite() {
            return Err(crate::error::QuotaFsmError::Config(format!(
...
impl QuotaFsmConfig {
    ...
    /// # Errors
    ///
    /// Returns [`crate::error::QuotaFsmError::Config`] when an invariant
    ...
    pub fn new(
        ...
        over_quota_100pct_threshold: f64,
        suspension_invoice_failure_threshold: u32,
    ) -> Result<Self, crate::error::QuotaFsmError> {
        for (name, v) in [
            (
...
mod tests {
    ...
    fn config_rejects_non_finite() {
        ...
        assert!(matches!(r, Err(crate::error::QuotaFsmError::Config(_))));
    }
