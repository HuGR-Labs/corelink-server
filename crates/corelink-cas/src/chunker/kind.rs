...
//! ([`crate::chunker::fixed::FixedChunker`] /
//! [`crate::chunker::fastcdc::FastCdcChunker`]). The shared dispatch enum
...
use crate::chunker::{Chunk, ChunkerAlgorithm, ChunkerConfig};
...
/// ([`crate::chunker::ChunkerConfig`], [`crate::chunker::ChunkerAlgorithm`]) and
/// [`crate::chunker::ChunkerError`], all `#[non_exhaustive]`.
#[derive(Debug)]
pub enum ChunkerStep<'a> {
    ...
    /// canonical bounds (`crate::chunker::bounds`). The chunker MUST be
    /// `reset` before reuse.
    Error(ChunkerError),
}
...
/// against dangling references — `crate::chunker::Chunk<'a>::to_owned`
...
pub enum ChunkerKind {
    /// [`crate::chunker::fixed::FixedChunker`].
    Fixed(crate::chunker::fixed::FixedChunker),
    /// [`crate::chunker::fastcdc::FastCdcChunker`].
    FastCdc(crate::chunker::fastcdc::FastCdcChunker),
}
