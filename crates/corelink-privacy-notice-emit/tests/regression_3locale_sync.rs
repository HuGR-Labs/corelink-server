...
use corelink_privacy_notice_emit::{
...
fn make_emitter(
    ...
    let emitter = InMemoryNoticeEmitter::new(
        Arc::new(Mutex::new(store)) as Arc<Mutex<dyn corelink_privacy_notice_emit::store::NoticeStateStore>>,
        sink.clone() as Arc<dyn corelink_privacy_notice_emit::audit::NoticeAuditSink>,
    );
    (emitter, sink)
}
...
fn ac006_notice_text_hash_crlf_lf_parity() {
    use corelink_privacy_notice_emit::notice_text_hash;
...
fn decision_carries_all_3_locale_hashes() {
    ...
    for locale in corelink_privacy_notice_emit::canonical_notice_locales() {
        assert!(
