...
use crate::audit::{emit as audit_emit, OciAuditEvent};
use crate::digest::{OciDigest, OciDigestAlgo};
use crate::error::OciAdapterError;
use crate::ports::ManifestKvStore;
...
fn check_repo_push(repo: &str, scope: &crate::auth::OciScope) -> Result<(), OciAdapterError> {
    crate::server::validate_repo_name(repo)?;
...
pub async fn put(
    ...
    let key_by_reference = crate::pull::manifest::manifest_key(repo, reference);
    let key_by_digest = crate::pull::manifest::manifest_key(repo, &manifest_digest.to_wire());
    ...
    kv.put(
        tenant,
        &crate::pull::manifest::manifest_ct_key(repo, reference),
        ct_bytes.clone(),
    )
    .await
    .map_err(OciAdapterError::Kv)?;
    kv.put(
        tenant,
        &crate::pull::manifest::manifest_ct_key(repo, &manifest_digest.to_wire()),
        ct_bytes,
    )
    .await
    .map_err(OciAdapterError::Kv)?;
