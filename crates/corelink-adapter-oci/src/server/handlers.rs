...
use crate::audit::OciAuditEvent;
use crate::error::OciAdapterError;
use crate::server::core::{err_response, status_for, AppState};
...
pub async fn api_version(
    ...
    if let Some(token_header) = auth.and_then(|h| h.strip_prefix("Bearer ")) {
        ...
        match crate::auth::verify(&state.config.token_signing_key, token_header, now) {
            Ok(_) => {
...
pub async fn token(
    ...
    let pat = match crate::auth::parse_basic_authorization(basic) {
        Ok(p) => p,
    ...
    let scope_str = uri
        ...
        .unwrap_or_default();
    let scope = match crate::auth::OciScope::parse(&scope_str) {
        Ok(s) => s,
    ...
    let token = match crate::auth::mint(
...
pub async fn dispatch_v2(
    ...
    let now_secs = (state.clock_unix_ms)() / 1000;
    let verified = match crate::auth::verify(
        &state.config.token_signing_key,
        bearer,
        now_secs,
    ) {
...
async fn route_dispatch(
    ...
    scope: &crate::auth::OciScope,
    now_ms: u64,
) -> Result<axum::response::Response, OciAdapterError> {
    match parsed {
        V2Path::Blob { repo, digest } => match method {
            Method::GET => {
                crate::pull::blob::get(state.config.cas.as_ref(), tenant, scope, &repo, &digest)
                    .await
            }
            Method::HEAD => {
                crate::pull::blob::head(state.config.cas.as_ref(), tenant, scope, &repo, &digest)
                    .await
            }
        ...
        V2Path::BlobUploadsOpen { repo } => match method {
            Method::POST => {
                crate::push::upload::open(state.config.cas.as_ref(), tenant, scope, &repo).await
            }
            _ => Err(OciAdapterError::NotFound),
        },
        ...
        V2Path::TagsList { repo } => match method {
            Method::GET => {
                crate::tags::list(state.config.metadata_kv.as_ref(), tenant, scope, &repo).await
            }
            _ => Err(OciAdapterError::NotFound),
        },
    }
