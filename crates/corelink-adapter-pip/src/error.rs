...
//! response shaping in [`crate::server`] is a single `match` (no
...
pub enum PipAdapterError {
    ...
    ///
    /// Surfaces only at boot from [`crate::server::run_pip_adapter`].
    #[error("bind: {0}")]
    Bind(std::io::Error),
    ...
    /// [`crate::config::PipAdapterConfig::wheel_size_limit_bytes`]
