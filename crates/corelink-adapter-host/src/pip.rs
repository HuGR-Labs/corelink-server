//! Bridges for `corelink-adapter-pip` port traits.
//!
//! The pip adapter declares three port traits:
//! - [`corelink_adapter_pip::ports::CasStore`] — async CAS `get`/`put` by
//!   `(&TenantId, &Digest)`, errors as [`corelink_adapter_pip::error::PipAdapterError`].
//! - [`corelink_adapter_pip::ports::KvStore`] — async metadata KV `get`/`put` with
//!   a `(value, inserted_at_unix_ms)` tuple.
//! - [`corelink_adapter_pip::ports::TenantResolver`] — async PAT → [`TenantId`].
//!
//! # Bridges
//!
...
use corelink_adapter_pip::error::PipAdapterError;
use corelink_adapter_pip::ports::{CasStore, KvStore, TenantResolver};
...
/// Bridges `corelink-adapter-pip::ports::CasStore` onto
...
/// Bridges `corelink-adapter-pip::ports::KvStore` onto `KvBackend`.
///
...
/// Bridges `corelink-adapter-pip::ports::TenantResolver` onto
/// [`PatValidator`].
#[non_exhaustive]
