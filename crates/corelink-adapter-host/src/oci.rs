//! Bridges for `corelink-adapter-oci` port traits.
//!
//! The OCI adapter declares three port traits:
//! - [`corelink_adapter_oci::ports::BlobStore`] — chunked upload protocol
//!   (`open_upload`, `append_chunk`, `finalize_upload`, `cancel_upload`,
//!   `get_blob`, `blob_exists`); errors as `String` via
//!   [`corelink_adapter_oci::ports::PortResult`].
//! - [`corelink_adapter_oci::ports::ManifestKvStore`] — KV for manifests +
//!   tag lists; errors as `String`.
//! - [`corelink_adapter_oci::ports::TenantResolver`] — async
...
use corelink_adapter_oci::ports::{BlobStore, ManifestKvStore, PortResult, TenantResolver};
...
/// Bridges `corelink-adapter-oci::ports::BlobStore` onto
...
impl BlobStore for OciBlobBridge {
    async fn open_upload(&self, tenant: &TenantId) -> PortResult<String> {
        // uuid format: "{tenant}:{ts}" — unique per tenant + wall-clock ms.
    ...
    async fn append_chunk(
        ...
        // The uuid already encodes the tenant; look up directly.
        let mut g = self
            .uploads
            .lock()
            .map_err(|e| format!("upload buf lock poisoned: {e}"))?;
    ...
    async fn finalize_upload(
        ...
        // Persist to CasWriteHandler. `blob_key` is used as the claimed_hash
        // — callers must supply a `blob_key` that equals `fake_hash(bytes)` in
        // tests, and a proper BLAKE3/sha256 hex in production. The tenant
        // isolation is enforced by the handler's `caller_tenant == tenant`
        // check (both are set to `_tenant` here).
    ...
    async fn cancel_upload(&self, _tenant: &TenantId, upload_uuid: &str) -> PortResult<()> {
        // uuid already encodes tenant; look up directly.
        self.uploads
            .lock()
            .map_err(|e| format!("upload buf lock poisoned: {e}"))?
            .remove(upload_uuid);
    ...
    async fn get_blob(&self, tenant: &TenantId, blob_key: &str) -> PortResult<Option<Bytes>> {
        ...
        let principal = self.principal.clone();
        // Use blob_key directly as the CAS hash — same key used in finalize_upload.
...
/// Bridges `corelink-adapter-oci::ports::ManifestKvStore` onto `KvBackend`.
///
...
/// Bridges `corelink-adapter-oci::ports::TenantResolver` onto
/// [`PatValidator`].
///
...
impl TenantResolver for OciTenantBridge {
    async fn resolve_pat(&self, pat: &SecretWrap) -> PortResult<TenantId> {
        // Expose the secret only long enough to copy into the blocking closure.
...
mod tests {
    ...
    async fn blob_open_append_finalize_roundtrip() {
        ...
        // blob_key must be the fake_hash of the final assembled bytes so that
        // InMemoryCasHandler's hash verification passes.
    ...
    async fn blob_cancel_upload_removes_session() {
        ...
        // After cancel, append_chunk returns "session not found"
    ...
    async fn manifest_kv_list_prefix_returns_empty_in_bridge() {
        ...
        // bridge does not scan KV — always returns []
