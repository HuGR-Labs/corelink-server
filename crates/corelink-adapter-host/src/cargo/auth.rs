...
///
/// Maps [`TenantResolveError::InvalidPat`] onto [`CargoAdapterError::Auth`]
/// (401 — the credential was judged and rejected) and
/// [`TenantResolveError::Backend`] onto
/// [`CargoAdapterError::VerifierOverloaded`] (503 + `Retry-After`).
///
...
pub async fn resolve_tenant(
    ...
    match resolver.resolve(pat.expose_secret()).await {
        ...
        Err(TenantResolveError::Backend(msg)) => Err(CargoAdapterError::VerifierOverloaded(msg)),
    }
}
...
mod tests {
    ...
    impl crate::cargo::ports::TenantResolver for AlwaysErr {
    ...
    async fn invalid_pat_maps_to_auth() {
        ...
        let err = resolve_tenant(&resolver, &SecretString::from("corelink_pat_x".to_owned()))
            .await
            .unwrap_err();
        assert!(matches!(err, CargoAdapterError::Auth(_)), "got {err:?}");
    }
    ...
    async fn backend_maps_to_verifier_overloaded_not_auth() {
        ...
        let err = resolve_tenant(&resolver, &SecretString::from("corelink_pat_x".to_owned()))
            ...
            .unwrap_err();
        assert!(
            matches!(err, CargoAdapterError::VerifierOverloaded(_)),
