...
//!
//! The brew adapter consumes two side-effecting subsystems through
...
//! re-export — call sites stay stable. See dispatch packet §5 for the
//! conceptual signatures this module realizes.
...
/// fall-through) and MUST treat `(tenant_id, cas_key)` as the unique
/// identity of a stored blob. `cas_key` is BLAKE3 of the canonicalized
/// upstream URL (see `crate::brew::bottle`).
///
...
/// `auth.rs` call site.
///
/// PAT scope is hard-coded to `brew:read` at the adapter layer; the
/// resolver itself returns only the tenant id so the scope check stays
/// local to this crate.
#[async_trait]
