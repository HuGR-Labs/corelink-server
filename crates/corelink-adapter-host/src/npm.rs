//! Bridges for `corelink-adapter-npm` port traits.
//!
//! The npm adapter declares three port traits:
//! - [`corelink_adapter_npm::ports::CasStore`] — async CAS `get`/`put` by
//!   `(&TenantId, &Digest)`, errors as [`corelink_adapter_npm::error::NpmAdapterError`].
//! - [`corelink_adapter_npm::ports::KvStore`] — async metadata KV `get`/`put` with
//!   a `(value, inserted_at_unix_ms)` tuple.
//! - [`corelink_adapter_npm::ports::TenantResolver`] — async PAT → [`TenantId`].
//!
//! # Bridges
//!
...
use corelink_adapter_npm::error::NpmAdapterError;
use corelink_adapter_npm::ports::{CasStore, KvStore, TenantResolver};
...
/// Bridges `corelink-adapter-npm::ports::CasStore` onto
...
/// Bridges `corelink-adapter-npm::ports::KvStore` onto `KvBackend`.
///
...
/// Bridges `corelink-adapter-npm::ports::TenantResolver` onto
/// [`PatValidator`].
#[non_exhaustive]
...
mod tests {
    ...
    async fn cas_put_wrong_hash_errors() {
        ...
        // Digest derived from "something-else" won't match bytes for "actual"
    ...
    async fn kv_tenant_scoping_isolates_keys() {
        ...
        // tenant-b doesn't see tenant-a's key
