//! Bridges for `corelink-adapter-cargo` port traits.
//!
//! The cargo adapter declares two port traits in its `ports` module:
//! - [`corelink_adapter_cargo::ports::CasStore`] — async `get`/`put` by
//!   `(tenant_id: &str, digest_hex: &str)`, errors as
//!   [`corelink_adapter_cargo::ports::CasError`].
//! - [`corelink_adapter_cargo::ports::TenantResolver`] — async `resolve`
//!   a PAT plaintext to a tenant-id `String`, errors as
//!   [`corelink_adapter_cargo::ports::TenantResolveError`].
//!
//! # Bridges
//!
...
use corelink_adapter_cargo::ports::{
...
/// Bridges `corelink-adapter-cargo::ports::CasStore` onto the workspace
...
/// Bridges `corelink-adapter-cargo::ports::TenantResolver` onto the
...
mod tests {
    ...
    async fn cas_get_different_tenant_returns_none() {
        ...
        let hash = fake_hash(&bytes);
        // Seed under "t1" directly (bypasses audit noise)
        h.seed("t1", &hash, bytes).expect("seed");
        // Reading under a different tenant with our bridge returns None
        // because caller_tenant == tenant_id (same in bridge) but different
        // from seeded tenant — so handler returns NotFound → bridge → None.
    ...
    async fn cas_put_hash_mismatch_surfaces_as_cas_error() {
        let h = make_inmem_handler();
        let bridge = make_cas_bridge(h);
        // claimed hash doesn't match what fake_hash would compute for b"data"
    ...
    async fn cas_put_idempotent_second_write_succeeds() {
        ...
        let hash = fake_hash(&bytes);
        bridge.put("t1", &hash, bytes.clone()).await.unwrap();
        // Second put with same bytes + hash — idempotent by CAS semantics.
