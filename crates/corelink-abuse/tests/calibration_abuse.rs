...
fn calibration_fp_rate_under_5pct_tp_rate_over_80pct() {
    ...
    for (i, (features, tier)) in benign.iter().enumerate() {
        let score = corelink_abuse::compute_score(features, &weights, *tier);
        let decision =
            corelink_abuse::decide(score, &corelink_abuse::AbuseConfig::canonical());
...
fn calibration_orchestrator_pipeline_emits_audit_and_metrics() {
    ...
    assert_eq!(
        audit.snapshot_of(corelink_abuse::AbuseEventType::ScoreComputed)
            .len(),
        100
    );
    ...
    let benign_audits = audit
        .snapshot_of(corelink_abuse::AbuseEventType::DecisionBenign)
        .len();
    let suspicious_audits = audit
        .snapshot_of(corelink_abuse::AbuseEventType::DecisionSuspicious)
        .len();
    let malicious_audits = audit
        .snapshot_of(corelink_abuse::AbuseEventType::DecisionMalicious)
        .len();
    assert_eq!(benign_audits + suspicious_audits + malicious_audits, 100);

    // 0 SuspendApplied audits (LGPD Art. 20 humane response).
    assert_eq!(
        audit.snapshot_of(corelink_abuse::AbuseEventType::SuspendApplied)
            .len(),
        0
    );
