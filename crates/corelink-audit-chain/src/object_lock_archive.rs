...
mod tests {
    ...
    const LEGACY_PROVIDER_TIME_CLAIM: &str =
        concat!("Provider-recorded immutable-write ", "time.");
    ...
    fn timestamp_semantics_are_local(contract: &str, adapter: &str, docs: &str) -> bool {
        contract.contains("pub observed_at_unix_ms: u64")
            ...
            && docs.contains("`observed_at_unix_ms` field is the adapter's local clock reading")
    ...
    fn receipt_timestamp_semantics_reject_provider_recorded_mutations() {
        ...
        let adapter_field_mutation =
            adapter.replace("observed_at_unix_ms", LEGACY_TIMESTAMP_FIELD);
        assert!(!timestamp_semantics_are_local(
            contract,
