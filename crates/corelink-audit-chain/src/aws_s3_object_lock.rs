...
impl AwsS3ObjectLockAdapter {
    ...
    fn is_object_lock_delete_denial(code: Option<&str>, message: Option<&str>) -> bool {
        code == Some("AccessDenied")
            && message.is_some_and(|message| {
                message
                    .to_ascii_lowercase()
                    .contains("protected by object lock")
            })
    }
    ...
    fn delete_exact_version(
        ...
        let denied_by_lock = Self::block_on("delete denial probe", async move {
            match probe
                .delete_object()
                .bucket(&bucket)
                .key(&key)
                ...
                .await
            {
                Ok(_) => Ok(false),
                Err(error) => {
                    let object_lock_denial =
                        error.as_service_error().is_some_and(|service_error| {
                            Self::is_object_lock_delete_denial(
                                service_error.code(),
                                service_error.meta().message(),
                            )
                        });
                    if object_lock_denial {
                        Ok(true)
                    } else {
                        Err(
                            "delete probe failed without a structured Object Lock denial"
                                .to_string(),
                        )
                    }
                }
        ...
        if denied_by_lock {
            Ok(DeleteAttempt::Denied {
                ...
                reason: "s3_object_lock_access_denied".to_string(),
            })
        } else {
...
impl ObjectLockArchiveAdapter for AwsS3ObjectLockAdapter {
    ...
    fn read_retention(
        ...
        Ok(Self::block_on("retention readback", async move {
            ...
            let legal_hold = legal_hold_request
                .send()
                .await
