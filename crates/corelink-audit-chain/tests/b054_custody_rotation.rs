...
use corelink_audit_chain::{
    key_matches_commitment, link_for_epoch, link_key_commitment,
    split_verifying_prefix_for_epoch, ChainEpoch, EpochChainState, LinkKeyring,
    SealedArchiveLine, KEYED_ALGORITHM_ID, SEALED_LINE_SCHEMA_V2,
};
...
fn protected_hex_secret(name: &str) -> Zeroizing<String> {
    let value = env::var(name).unwrap_or_else(|_| panic!("required protected value {name} is absent"));
...
fn approval_receipt() -> Value {
    ...
    assert_eq!(
        receipt["schema"], "corelink.b054.custody-approval-receipt.v1"
    );
...
fn rotation_retains_history_and_failed_successor_rolls_back_to_e1() {
    ...
    let e2_link_key = full_keyring.get(E2_LINK_KEY_ID).expect("E2 material is available");
    ...
    let e1_epoch = ChainEpoch::keyed_successor(
        E1_EPOCH,
        E1_LINK_KEY_ID,
        e0.next_sequence(),
        *e0.head(),
    ...
    let mut e1 = e0.transition(e1_epoch.clone()).expect("E0 to E1 is forward-only");
