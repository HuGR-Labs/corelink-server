...
# least-privilege cache scope `cas:rw`.
#
...
# tokens only need `cas:rw`. This back-fill enforces least privilege on the
# legacy rows. New PATs are already minted `cas:rw` by the signup-worker.
#
# ORDER: this is a NO-OP (and would FAIL the CHECK) until migration
#   migrations/d1/0061_pat_scope_colon_grammar.sql has been applied to prod,
#   which widens the `pat.scope` CHECK to ('cas:rw','cas:r','cas:w','admin').
#   Apply migrations first (scripts/apply-d1-migrations-prod.sh), THEN this.
#
# SAFETY:
#   * READ-ONLY by default — prints how many rows WOULD change and the live
#     scope distribution. No mutation without --apply.
#   * --apply runs exactly: UPDATE pat SET scope='cas:rw' WHERE scope='admin'
...
echo "  → ${ADMIN_N} row(s) with scope='admin' would be demoted to 'cas:rw'."

if [[ "$APPLY" -eq 0 ]]; then
  echo
  echo "DRY-RUN. No changes made. Re-run with --apply to execute the UPDATE."
  echo "(Reminder: migration 0061 must be applied to prod FIRST, else the CHECK rejects 'cas:rw'.)"
  exit 0
fi
...
echo
echo "── APPLY: UPDATE pat SET scope='cas:rw' WHERE scope='admin' ───────────"
RES="$(d1 "UPDATE pat SET scope='cas:rw' WHERE scope='admin';")"
if ! ok "$RES"; then
  echo "FATAL: UPDATE failed (did migration 0061 land? CHECK still rejects cas:rw if not):"
  echo "$RES" | jq '.errors'
  exit 1
fi
