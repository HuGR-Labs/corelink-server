...
# - CORELINK_DEPLOYED_SHA / CORELINK_DEPLOYED_REGION: response provenance
#   metadata deliberately exposed as headers; neither is credential material.
# - STAGING_CLERK_ISSUER_URL / STAGING_R2_S3_ENDPOINT: public endpoint aliases
#   consumed only by the staging bootstrap; keys, tokens, account IDs, and the
#   bounded target-identity receipt remain matrix rows.
