...
class ReusableKitContractTests(unittest.TestCase):
    ...
    REQUIRED_DOCS = (
        ...
        'WORKPACKAGES.md', 'ADVERSARIAL-REVIEW.md', 'topology.example.json',
        'gate-ledger.template.json',
    )

    def test_exact_reusable_deliverables_exist(self):
        for name in self.REQUIRED_DOCS:
            with self.subTest(path=name):
                self.assertTrue((self.DOC / name).is_file())
        self.assertEqual(sorted(path.name for path in self.DOC.iterdir()),
                         sorted(self.REQUIRED_DOCS))
    ...
    def test_ci_triggers_and_required_suites_cover_the_reusable_kit(self):
    ...
    def test_live_control_surface_matrix_is_explicit_and_unknown_by_default(self):
        for name in ('MAP.md', 'RUNBOOK.md'):
            ...
            self.assertTrue(any(term in doc.casefold()
                                for term in ('git tree', 'árvore git', 'git-tree-only')))
