...
def commands_preserve_intent(relative: str, job: str, original: tuple[str, ...], candidate: tuple[str, ...]) -> bool:
    if original == candidate:
        return True
    if (relative, job) != (".github/workflows/dependabot-policy.yml", "policy-gate"):
        return False
    old = 'cargo deny --manifest-path "$POLICY_TREE/Cargo.toml" --config "$DENY_CONFIG" check licenses bans'
    corrected = 'cargo deny --manifest-path "$POLICY_TREE/Cargo.toml" check --config "$DENY_CONFIG" licenses bans'
    normalized = tuple(block.replace(old, corrected) for block in original)
    return old in "\n".join(original) and normalized == candidate
...
def verify_baseline(candidate: Path, baseline: Path) -> None:
    ...
    policy_text = (candidate / ".github/workflows/dependabot-policy.yml").read_text(encoding="utf-8")
    if 'cargo deny --manifest-path "$POLICY_TREE/Cargo.toml" check --config "$DENY_CONFIG" licenses bans' not in policy_text:
        fail("Dependabot policy must use cargo-deny 0.19.8's valid check --config argument order")
