...
def validate_localized_api_indexes(endpoints: list[Endpoint]) -> list[str]:
    """Require each localized landing page to be an exact, bounded mirror.

    The localized trees are maintained separately from the English generator,
    so ``--check`` must inspect them explicitly.  Comparing both links and
    endpoint files catches a deleted page that remains linked as well as a
    stale extra page.  Bounds keep a malformed generated tree from becoming
    an unbounded CI scan.
    """
    expected = {endpoint.file_name for endpoint in endpoints}
    failures: list[str] = []
    link_pattern = re.compile(r"\]\(\./endpoints/([A-Za-z0-9_.-]+\.mdx)\)")
    for locale, api_root in LOCALIZED_API_ROOTS:
        ...
        try:
            ...
            continue
        links = link_pattern.findall(index)
...
def main(argv: list[str] | None = None) -> int:
    ...
    localized_failures = validate_localized_api_indexes(endpoints) if args.check else []
