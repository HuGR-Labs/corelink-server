...
from check_migrations_additive import (
...
def main() -> None:
    ...
    assert not scan_sql(
        forward_control_migration, B071_TRIGGER_REPLACEMENT_PATH
    ), "B-071 forward migration is outside the name-scoped additive policy"
    for name, mutant in (
        (
            "other trigger",
            forward_control_migration.replace(
                "trg_gc_purge_accounting_required", "trg_gc_purge_legal_hold_guard", 1
            ),
        ),
        (
            "table drop",
            forward_control_migration.replace("DROP TRIGGER", "DROP TABLE", 1),
        ),
        (
            "generic waiver",
            forward_control_migration.replace(
                "additive-trigger-replacement", "additive-allowed", 1
            ),
        ),
        (
            "duplicate unpaired approved create",
            forward_control_migration
            + "\nCREATE TRIGGER trg_gc_purge_accounting_required "
            "AFTER DELETE ON blob_meta\nBEGIN SELECT 1; END;\n",
        ),
    ):
        assert scan_sql(mutant, B071_TRIGGER_REPLACEMENT_PATH), \
            f"destructive migration mutation unexpectedly passed: {name}"
