...
def _run_wrangler(args: list[str]) -> bytes:
    ...
    try:
        ...
        while True:
            ...
            if remaining <= 0:
                raise ProbeError("read-only provider command timed out")
            if not selector.select(remaining):
                raise ProbeError("read-only provider command timed out")
            chunk = os.read(
                process.stdout.fileno(),
                min(OUTPUT_CHUNK_BYTES, MAX_OUTPUT_BYTES + 1 - len(captured)),
            )
            if not chunk:
                try:
                    return_code = process.wait(timeout=max(0, deadline - time.monotonic()))
                except subprocess.TimeoutExpired as exc:
                    raise ProbeError("read-only provider command timed out") from exc
                if return_code != 0:
...
def _state_counts(payload: object) -> dict[str, int]:
    ...
    if len(payload) > MAX_INSTANCES_PER_APP:
        raise ProbeError("instance list exceeds the declared application ceiling")
