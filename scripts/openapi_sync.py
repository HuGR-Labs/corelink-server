...
def emit_worker_ts(spec: dict) -> str:
    ...
    """
    # Keep the served response byte-identical to ``openapi/corelink-v1.json``.
    # The parsed object remains exported for tests and callers, but the HTTP
    # handler uses RAW so formatting cannot drift between checked-in artifacts.
    payload = emit_json(spec)
    return (
        ...
        "\n"
        "export const corelinkV1Json = RAW;\n"
        "\n"
...
def emit_static_yaml() -> str:
    """Publish an exact copy of the canonical YAML for SDK generators.

    The documentation site serves this file directly. Keeping it in the
    generated set prevents a second, silently stale OpenAPI contract from
    surviving a canonical-spec change.
    """
