...
def verify_drift_workflow(source: str) -> None:
    ...
    if runners != ["ubuntu-24.04", "ubuntu-24.04", "ubuntu-24.04", "ubuntu-24.04"]:
    ...
    for marker in (
        ...
        "needs: verify-dispatch-revision",
    ):
        require(source, marker)
...
class Issue1720CloudflareDriftAuditTests(unittest.TestCase):
    ...
    def test_disable_failure_mutations_are_rejected(self) -> None:
        ...
        mutations = (
            ...
            source.replace('if state="$(gh api', 'state="$(gh api', 1),
            source.replace(
                '[[ "$main_sha" == "$GITHUB_SHA" ]]',
                '[[ "$main_sha" != "$GITHUB_SHA" ]]',
                1,
            ...
            source.replace(
                '"repos/${GITHUB_REPOSITORY}/git/ref/heads/main"',
                '"repos/${GITHUB_REPOSITORY}/git/ref/heads/missing"',
                1,
            ),
            source.replace('[[ ! "$main_sha" =~ ^[0-9a-f]{40}$ ]]', '[[ "$main_sha" =~ ^[0-9a-f]{40}$ ]]', 1),
            source.replace('if ! main_sha="$(gh api', 'if main_sha="$(gh api', 1),
            source.replace("--ref main \\\n", '--ref "$main_sha" \\\n', 1),
            source.replace(
                "if ! gh workflow run terraform-drift.yml",
                "if gh workflow run terraform-drift.yml",
                1,
    ...
    def test_dispatch_revision_mutations_are_rejected(self) -> None:
        ...
        mutations = (
            ...
            source.replace("needs: verify-dispatch-revision", "needs: other-job", 1),
        )
        for mutation in mutations:
            with self.subTest(mutation=mutation):
                with self.assertRaises(AssertionError):
