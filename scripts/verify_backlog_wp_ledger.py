...
CATALOGS = {
    ...
    REPO_ROOT / "docs/campaigns/remediation/work-packages/B131-B167.md": (131, 167),
}
...
WORKFLOW_MANIFEST_COUNT = 124
WORKFLOW_MANIFEST_SHA256 = "f1bf1a0d160fec1ef2b5ac5c8d18f491eced0fd6080c8ed45bd77fd41e2f3e5e"
...
def parse_ledger_state(text: str, source: str) -> dict[str, str]:
    ...
    if state["base-ref"] != "origin/main":
        raise LedgerError(f"{source}: ledger base-ref must be origin/main")
...
def main() -> int:
    try:
        ...
        if not re.fullmatch(r"[0-9a-f]{40}", expected_base_sha):
            raise LedgerError("origin/main SHA unavailable; cannot verify ledger base")
        catalog_counts: dict[str, int] = {}
        assignments: list[tuple[str, str, str]] = []
