...
def verify(root: Path = ROOT) -> int:
    ...
    checks = (
        ...
        (source["staging_deploy_workflow"], "runs-on: ubuntu-24.04", "GitHub-hosted staging deploy runner"),
        (source["staging_deploy_workflow"], "environment: synthetic-drill-staging", "protected staging deploy environment"),
        (source["staging_deploy_workflow"], "options: [staging]", "staging-only manual dispatch"),
        (source["staging_deploy_workflow"], 'if [[ "$DEPLOY_ENV" != "staging" ]]', "staging-only deploy guard"),
        (source["receiver_config"], 'SYNTHETIC_DRILL_ENABLED = "false"', "staging receiver remains inert by default"),
        (source["receiver_config"], "routes = []", "production receiver has no route"),
        (source["receiver_config"], "crons = []", "production receiver has no cron"),
