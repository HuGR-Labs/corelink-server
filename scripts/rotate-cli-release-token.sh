...
# fine-grained PAT scoped only to humangr-labs/corelink-cli contents:write.
#
# WHY: the current token is the operator's gh-CLI session token (broad
# `repo` scope across all humangr-labs repos). For defense-in-depth, the
...
release uploads on humangr-labs/corelink-cli (instead of the current
...
echo "Verifying PAT can write to humangr-labs/corelink-cli..."
if ! curl -fsS \
    -H "Authorization: token $NEW_PAT" \
    -H "Accept: application/vnd.github+json" \
    https://api.github.com/repos/humangr-labs/corelink-cli > /dev/null; then
    echo "ERROR: PAT can't read corelink-cli — wrong repo selection?" >&2
    exit 1
fi

# Set as secret on corelink-server (where the workflow lives)
echo "Setting CORELINK_CLI_RELEASE_TOKEN on humangr-labs/corelink-server..."
echo -n "$NEW_PAT" | gh secret set CORELINK_CLI_RELEASE_TOKEN \
    --repo humangr-labs/corelink-server

echo
echo "✓ Done. Verify with:"
echo "  gh secret list --repo humangr-labs/corelink-server | grep CORELINK_CLI"
echo
