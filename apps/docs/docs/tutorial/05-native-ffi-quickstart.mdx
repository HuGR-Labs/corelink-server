...
`corelink-go` is a pure Go module — no cgo, no native library to build
first. Fetch it through CoreLink's own module proxy:
...
The `GOPROXY` setting is mandatory: `go.corelink.humangr.com` has no DNS
record, so a plain `go get` fails with "no such host". The module is served
from CoreLink's own proxy — the first entry above; the second entry,
`https://proxy.golang.org`, is what fetches the module's ordinary public
dependencies.
...
Next: [Verify cache hit ratio](./06-verify-cache-hit).
