...
`corelink-go` is not published — there is no installable module and no Go
source in this repository. The code below illustrates the planned API; it
is not something you can `go get` today. To register interest, contact
...
import (
    ...
    corelink "corelink-api.humangr.com/corelink-go"
)
...
//
// Not published today; there is no package to fetch. This file
// illustrates the planned API. Once released, it would require:
//
