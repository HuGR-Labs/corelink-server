...
(22 vendors total) — only those who *actually process customer
personal data on CoreLink's behalf today* appear in the **Active
sub-processors** table (9 of them). A handful of registered
...
> **Last refreshed:** 2026-09-22. This page is **auto-generated** from
...
- **Status page subscription** — sub-processor changes are also published
  ...
  [hugrl.betteruptime.com](https://hugrl.betteruptime.com). Subscribe via RSS,
...
| 7 | **Functional Software, Inc. (Sentry)** | Application error monitoring | telemetry (exception/breadcrumb diagnostic events; `sendDefaultPii: false`, `beforeSend`/`beforeSendTransaction`/`beforeBreadcrumb` route every event through `@/lib/sentry-scrub.ts`, which redacts secret/PII-shaped substrings and denies known-sensitive keys wholesale before the event leaves the process) | US / EU (selectable) | [Sentry DPA](https://sentry.io/legal/dpa/) |
| 8 | **Plausible Insights OÜ (Plausible Analytics)** | Cookieless web analytics | telemetry (page-view aggregates only; cookieless, no fingerprinting per the surrounding config comment — no cookie-consent gate is required for this domain as a result) | US / EU (selectable) | [Plausible DPA](https://plausible.io/dpa) |
| 9 | **Better Stack, Inc. (BetterStack / Statuspage)** | Uptime/status monitoring | telemetry (synthetic uptime/latency probe results against CoreLink's own public endpoints — HTTP status codes, JSON body assertions; no customer PII or content is transmitted to or from BetterStack) | US / EU (selectable) | [Trust portal](https://betterstack.com/privacy) |
...
- **Sigstore (Linux Foundation)** — the former Worker OCI signing workflow was
  removed on **2026-09-08** after it never executed; no Fulcio certificate or
  Rekor entry was issued for that lane. Separate release-SLSA and CAS signing
  paths remain independently gated. Current release-SLSA, CAS, and TSA paths
  send only CoreLink-owned artifact/signing metadata to Sigstore; no
  customer-data path is wired. The separate transparency-log seam is not a
  live transport; any future pseudonymous-tenant use requires a new
  Legal/DPO review. Sigstore is not a customer-data sub-processor
  in the current register; the separate GDPR transfer table remains subject to
  Legal/DPO review.
...
The full vendor risk register (22 vendors with inherent risk scoring,
