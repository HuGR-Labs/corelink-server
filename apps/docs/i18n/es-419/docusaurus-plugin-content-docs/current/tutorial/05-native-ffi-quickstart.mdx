...
If you are building a custom pipeline (ML training cache, CI artifact store, data lake), use the language SDKs rather than Bazel. All SDKs share the same REAPI v2 semantics and enable `client_verify` by default ([CTRL-CAS-002](../reference/reapi/index.mdx)).
