...
`corelink-go` es un módulo Go puro — sin cgo, sin biblioteca nativa que
compilar primero. Obténgalo mediante el proxy de módulos propio de
CoreLink:
...
El parámetro `GOPROXY` es obligatorio: `go.corelink.humangr.com` no
tiene registro DNS, así que un `go get` simple falla con "no such
host". El módulo se sirve desde el proxy propio de CoreLink — la
primera entrada anterior; la segunda entrada,
`https://proxy.golang.org`, es la que obtiene las dependencias públicas
ordinarias del módulo.
...
Ejemplo completo (`examples/go/quickstart.go`), con una llamada a
`Stat` y manejo explícito de errores:
