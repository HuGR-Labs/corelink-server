...
`corelink-go` é um módulo Go puro — sem cgo, sem biblioteca nativa para
compilar antes. Baixe-o pelo proxy de módulos do próprio CoreLink:
...
A variável `GOPROXY` é obrigatória: `go.corelink.humangr.com` não tem
registro DNS, então um `go get` simples falha com "no such host". O
módulo é servido pelo proxy próprio do CoreLink — a primeira entrada
acima; a segunda entrada, `https://proxy.golang.org`, é o que busca as
dependências públicas comuns do módulo.
