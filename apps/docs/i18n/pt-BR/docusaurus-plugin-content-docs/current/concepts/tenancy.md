...
| Revogável | A revogação no painel via `POST /v1/customer/keys/{pat_id}/revoke` exige o papel confiável pelo servidor `owner`/`admin`; Admins não podem revogar PATs do Owner. |
