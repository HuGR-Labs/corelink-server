...
>
> Canonical EN source: `docs/tutorial/05-native-ffi-quickstart.mdx`
...
`corelink-go` ist ein reines Go-Modul — kein cgo, keine vorher zu bauende
native Bibliothek. Beziehen Sie es über CoreLinks eigenen Modul-Proxy:
...
Die `GOPROXY`-Einstellung ist verpflichtend: `go.corelink.humangr.com`
hat keinen DNS-Eintrag, daher schlägt ein einfaches `go get` mit
„no such host" fehl. Das Modul wird über CoreLinks eigenen Proxy
ausgeliefert — der erste Eintrag oben; der zweite Eintrag,
`https://proxy.golang.org`, holt die gewöhnlichen öffentlichen
