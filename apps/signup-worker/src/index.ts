      ...
      runAuditArchiveSweep(env, nowMs)
        .then((r) => {
          if (r.skipped) {
            ...
            // ok=true line — see RB-AUDIT-ARCHIVE-ABSENT §5.
            if (r.rowsQuarantined > 0) {
              console.warn(
                `[audit-archive-cron] QUARANTINED rows=${r.rowsQuarantined} partitions=${r.partitionsQuarantined} — unarchivable chain segment, see RB-AUDIT-ARCHIVE-ABSENT`,
              );
            }
          }
        })
