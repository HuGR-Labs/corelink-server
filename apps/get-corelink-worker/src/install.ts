 ...
 *   5. Unsupported options are rejected rather than silently ignored; tenant
 *      residency is configured server-side, not by this client installer.
 *
 * Bash conventions:
...
# taking any caller-side \`&& corelink ...\` with it. The installer writes only
# configuration fields consumed by the CLI; tenant residency is server-side.
