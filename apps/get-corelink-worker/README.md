...
4. Writes `~/.corelink/config.toml` with `[auth].pat` and
   ...
   top-level `token` that the CLI's TOML deserializer silently ignored, so
   `[auth].pat` stayed unset and every install's own `corelink whoami` call
   below failed with "No PAT found" — see CHANGELOG.)
